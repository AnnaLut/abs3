	...
	public class PhoneService : Bars.BarsWebService
	{
		...
		private string MobipayInvoke(string urlParams)
		{
			...
			{				
				...
				if(null!=erp)
				{
					_dbLogger.Error("Mobipay: Invoke: "+RestrictURL(strFullURL));
				}
				else
				{
					_dbLogger.Info("Mobipay: Invoke: "+RestrictURL(strFullURL)+"\n"
						+"Result:\n"+strResponse);
				}
			}
			return strResponse;
		...
		{	
			_dbLogger.Info("Mobipay: Начало пополнения тел. "
			...
			try
			{
				...
				_dbLogger.Info("Mobipay: Получен № транзакции: "+pay_id);
			}
			finally
			{
				connect.Close();
		...
		{
			_dbLogger.Info("Mobipay: Инициирована проверка транзакции № "+pay_id);
            OracleConnection connect = Bars.Classes.OraConnector.Handler.UserConnection;
			string strUsername = "";
			string strPassword = "";
			time_stamp  = "";
			...
			try
			{
				...
				_dbLogger.Info("Mobipay: Проверка транзакции № "+pay_id
		...
		public void RollbackTrans(long pay_id, 
			out string time_stamp, out int status_code, out string status_msg)
		{
			_dbLogger.Info("Mobipay: Инициирован откат транзакции № "+pay_id);
            OracleConnection connect = Bars.Classes.OraConnector.Handler.UserConnection;
			string strUsername = "";
			string strPassword = "";
			time_stamp  = "";
			...
			try
			{
				...
				_dbLogger.Info("Mobipay: Результат отката транзакции № "+pay_id
		...
		public void CommitTrans(long pay_id, 
			...
			out long receipt)
		{
			_dbLogger.Info("Mobipay: Инициировано подтверждение транзакции № "+pay_id);
            OracleConnection connect = Bars.Classes.OraConnector.Handler.UserConnection;
			string strUsername = "";
			string strPassword = "";
			time_stamp  = "";
			...
			try
			{
				...
				_dbLogger.Info("Mobipay: Результат подтверждения транзакции № "+pay_id
					+": "+status_code+"("+status_msg+")");
			}
			finally
			{
		...
		public void RevokeTrans(int act, long pay_id, 
			out string time_stamp, out int status_code, out string status_msg)
		{
			_dbLogger.Info("Mobipay: Инициировано аннулирование транзакции № "+pay_id);
            OracleConnection connect = Bars.Classes.OraConnector.Handler.UserConnection;
			string strUsername = "";
			string strPassword = "";
			time_stamp  = "";
			...
			try
			{
				...
				_dbLogger.Info("Mobipay: Результат аннулирования транзакции № "+pay_id
					+": "+status_code+"("+status_msg+")");
			}
			finally
			{
		...
		{
			_dbLogger.Info("Mobipay: Инициирован запрос состояния транзакции № "+pay_id);
            OracleConnection connect = Bars.Classes.OraConnector.Handler.UserConnection;
			string strUsername = "";
			string strPassword = "";
			time_stamp  = "";
			...
			try
			{
				...
				_dbLogger.Info("Mobipay: Результат запроса состояния транзакции № "+pay_id
					+": "+pay_status+"("+pay_msg+")"
					+": "+status_code+"("+status_msg+")");
			}
			finally
		...
		public void GetState(string strPhone, 
			...
			out string time_stamp, out int status_code, out string status_msg)
		{
			_dbLogger.Info("Mobipay: Инициирован запрос инф. об абоненте "+strPhone);
            OracleConnection connect = Bars.Classes.OraConnector.Handler.UserConnection;
			string strUsername = "";
			string strPassword = "";
			time_stamp  = "";
			...
			_dbLogger.Info("Mobipay: Абонент "+strPhone
		...
		{
			_dbLogger.Info("Mobipay: Инициирована оплата транзакции "+pay_id
			...
			try
			{
				...
				_dbLogger.Info("Mobipay: Транзакция "+pay_id
        ...
        {
            _dbLogger.Info("Mobipay: Проверка доступа пользователя к операции и счетам");
            OracleConnection connect = Bars.Classes.OraConnector.Handler.UserConnection;            
            try
            {                
                OracleCommand command = new OracleCommand();
                ...
                command.ExecuteNonQuery();                
                _dbLogger.Info("Mobipay: Проверка прошла успешно");
            }
            finally
            {
                connect.Close();
		...
		{
			_dbLogger.Info("Mobipay: Инициировано формирование платежа в СЭП c "
				+date1+" по "+date2);
			CultureInfo cinfo = CultureInfo.CreateSpecificCulture("en-GB");
			cinfo.DateTimeFormat.FullDateTimePattern = "dd.MM.yyyy HH:mm:ss";
			cinfo.DateTimeFormat.DateSeparator = ".";
			...
			_dbLogger.Info("Mobipay: Платеж в СЭП успешно сформирован");
			return refs;
		}
		/// <summary>
