...
var v_PageSizeText = "Cтрок на стр.:";	//ширина ячейки поля ввода кол-тва строк грида
...
function InitGrid(notFill)
{
  ...
  if(v_ShowFilterOnStart) {
  	...
  	if(filter_array.length == 0 && v_data[20].length == 0){
  	 	ShowFilterWindow();
  		fnSaveViewState();
  	}	
  }	
...
function fnLoadViewState()
{
  ...
  if(v_EnableViewState && parent.frames[0].document.getElementById("global_obj")){
  	if(parent.frames[0].document.getElementById("global_obj").value == null) 
  		parent.frames[0].document.getElementById("global_obj").value = new Array();//если пустой, то инициализируем  
  	view_state = parent.frames[0].document.getElementById("global_obj").value;//достаем из фрейма obj
...
{
  if(parent.frames.length == 0) return null;
  if(!parent.frames[0].document.getElementById("global_obj")) return;	
  var href = (location.hash == "")?(location.href):(location.href.substring(0,location.href.length-1));
  view_state = parent.frames[0].document.getElementById("global_obj").value;
  ...
  parent.frames[0].document.getElementById("global_obj").value = view_state;//Запихиваем обратно в глобальний объект
}

function getViewStateParam(param)
{
	if(parent.frames.length == 0) return null;
	if(parent.frames[0].document.getElementById("global_obj") == null) return null;
	if(parent.frames[0].document.getElementById("global_obj").value == null) return null;
	var href = (location.hash == "")?(location.href):(location.href.substring(0,location.href.length-1));
...
function fnClearViewState()
{
 if(parent.frames.length != 0)
  if(v_EnableViewState && parent.frames[0].document.getElementById("global_obj"))
   	parent.frames[0].document.getElementById("global_obj").value = null; 
}
...
function CreateControls()
{
  ...
  if(document.getElementById('tblFooter') == null){
	  ...
	  {
	     //strFooterHTML +="<TD align="+v_alignPager+" width="+v_PagerCellWidth+"><img id='aRight' src='/Common/WebGrid/aRight.gif' title='Следующая страница' onclick='fnNext()' class=pager_img><span id='PageNum' class=pager_num> 1 </span><img id='aLeft' onclick='fnPrev()' title='Предыдущая страница' src='/Common/WebGrid/aLeft.gif' class=pager_img></TD>";
	     strFooterHTML +="<TD align="+v_alignPager+" width="+v_PagerCellWidth+"><img id='aLeft' onclick='fnPrev()' title='Предыдущая страница' src='/Common/WebGrid/aLeft.gif' class=pager_img><span id='PageNum' class=pager_num> 1 </span><img id='aRight' src='/Common/WebGrid/aRight.gif' title='Следующая страница' onclick='fnNext()' class=pager_img></TD>";
	  }
	  //Добавляем возможность изменения к-тво строк в гриде
	  if(v_EnablePageSize == true || v_EnablePageSize == "true")
	  {
	     strFooterHTML +="<TD width="+v_PageSizeCellWidth+"><DIV nowrap>"+v_PageSizeText+"&nbsp;<input type='text' id='edPageSize' class='footer_PageSize' title='Кол-тво строк в гриде' onkeypress='return KeyPressedOnEdit(event)' onchange='OnEditChanged()' value="+pageSize+"></DIV></TD>";
	  }
	  strFooterHTML += "<TD></TD>";
	  strFooterHTML += "</TR>";
	  strFooterHTML += "</TABLE>";
  ...
  if(v_EnableSort == true || v_EnableSort == "true")
  {
   if(document.getElementById('sort_ord') == null){
    ...
    document.getElementById('sort_ord').innerHTML ="<img title='По возрастанию' onclick='fnSortAsc()' src='/Common/WebGrid/sort_asc.gif'><BR><img title='По убыванию' onclick='fnSortDesc()' src='/Common/WebGrid/sort_desc.gif'>";
   } 
  } 
}
//Refresh grid
...
//Выделение строки
var multSelRows = new Array();
function SelectRow(val,id,mode)
{
 if(mode != "" || mode != null) acces_mode = mode;
 if(event.ctrlKey)
 {
 	if(selectedRow != null && document.getElementById('r_'+id).style.backgroundColor == '#d3d3d3')
 		document.getElementById('r_'+id).style.backgroundColor = v_row_style;
 	else
 		SelectSingleRow(val,id);
 	multSelRows[multSelRows.length] = new Array(id,val);
 	return;
 }
 
 DeSelectRow(id);
 SelectSingleRow(val,id);
 multSelRows = new Array();
 multSelRows[multSelRows.length] = new Array(id,val);
...
{
 if(multSelRows.length != 0)
 {
 	for (key in multSelRows)
 		if(document.getElementById('r_'+multSelRows[key][0]))
 	 		document.getElementById('r_'+multSelRows[key][0]).style.backgroundColor = v_row_style;
 }
 if(selectedRow != null) selectedRow.style.backgroundColor = v_row_style;
 if(document.getElementById('r_'+id).style.backgroundColor != '#d3d3d3')
 	v_row_style =  document.getElementById('r_'+id).style.backgroundColor;	
}

function SelectSingleRow(val,id)
{
...
function Sort(val){
  sort_exp = val;
  sort_ord.style.left = event.x + document.body.scrollLeft;
  sort_ord.style.top = event.y;
 ...
 { 
  ...
  for( i = 0; i < v_editFields.length ; i++ ){
     if(v_editFields[i].indexOf("dd") != -1){
       ...
       document.getElementById(v_editFields[i]).value = document.getElementById(v_editFields[i]+"_"+id).innerHTML;
  }   
 ...
 function makeEditControls()
 {
  ...
  for( i = 0; i < params.split(";").length ; i++)
  {
   ...
   if(value.substring(0,1) == "["){
	 ...
	 value = dd_name;
	 controls += "<TR><TD style='width:245px'><span class='edit_label'>"+text+"</span></TD><TD style='width:245px'><select style='width:100%' id="+dd_name+" onclick='d_dlg(this)'><option value=''></option></select></TD></TR>";
   }
   //textbox
   else {
    controls += "<TR><TD style='width:245px'><span class='edit_label'>"+text+"</span></TD><TD style='width:245px'><input style='width:100%' type=text "+ro+" id="+value+"></TD></TR>";
   } 
    
   v_editFields[i] = value; 
  }
  controls += "</TABLE><div align=center><INPUT class='filter_bt' style='WIDTH: 100px' onclick='CloseEdit()' type='button' value='Сохранить'><INPUT class='bt' style='WIDTH: 100px' onclick='CancelEdit()' type='button' value='Отмена'></div>"; 
  return controls;
 }
 //
 function CancelEdit()
...
function DelSelRows()
{
 ...
 {
 	for (key in multSelRows)
 		v_Obj_Soap.documentElement.removeChild(FindNode(multSelRows[key][1]));
 	 RefreshGrid();
 }
}
...
{
   var add = "&"+Math.random(); 
   var result = window.showModalDialog("/Bars.WebServices/Filter.aspx?table="+v_FilterTable+add,window,"dialogWidth:615px;dialogHeight:450px;center:yes;edge:sunken;help:no;status:no;scroll:no");
   var str = "";
   if(result != null){
		...
		if(array != null && array.length > 0)
		{
			...
			for(i=0;i<array.length;i++)
			{
				...
				var type = (obj.attr.split(';').length > 1)?(obj.attr.split(';')[1]):("");
				if(obj.op == '4')
				{
					v_data[0] +=  " ) ";
					continue;
				...
				if(obj.attr.split(';').length == 2)
				{
				  	if(obj.sign == '7' || obj.sign == '8') line_filter += 'UPPER($ALIAS$.'+field+') ';
				  	else 	    	  			  		   line_filter += '$ALIAS$.'+field+' ';
				...
				{
				  	var tabname = obj.attr.split(';')[2];  
				  	var rel1 = obj.attr.split(';')[3];
				  	var rel2 = obj.attr.split(';')[4];
				  	if(obj.sign == '7' || obj.sign == '8') line_filter += 'UPPER('+tabname+"."+field+') ';
				  	else 	    	  			  		   line_filter += tabname+'.'+field+' ';
				  	line_filter += GetOperation(obj.sign);
				  	line_filter += ' @'+tabname;
				  	if(obj.sign != '9' && obj.sign != '10')
				  	{
				...
				if(line_filter.indexOf('$ALIAS$.OST') > 0 )
				{
...
function drawFilterElement(meta)
{
 ...
 var draw_filter = "<div class=filter_title>Фильтр</div>"+
 "<TABLE cellSpacing=1 cellPadding=1 width='100%' border='0'>"+
 "<TR><TD align='center' class=filter_fields>Атрибут</TD>"+
 "<TD align='center' class=filter_fields>Оператор</TD>"+
 ...
 "</select></TD></TR></TABLE>"+
 "<div><span class=filter_fields>Значение</span><input class=filter_val type='text' id='tbValue' /></div>"+
 "<INPUT class='filter_bt' id='F_Go' onclick='fnApplyFilter()' type='button' value='Применить'>&nbsp;<input class='filter_bt' id='f_c' onclick='fnFilterCancel()' type='button'"+
 "value='Отменить'><BR><span class=filter_cur>Текущий фильтр:<span><BR>"+
 "<select size='3' id='lbFilter' class=filter_list></select>"+
 "<BR><INPUT class='filter_bt' id='F_Clear_Line' onclick='fn1Line()' type='button' value='Удалить строку'>&nbsp;"+
 "<INPUT class='filter_bt' id='F_Clear' onclick='fnAllLines()' type='button' value='Удалить все'>";
 return draw_filter;
}
...
function ShowPopupMenu()
{
  ...
  popupmenu.style.left = event.x + document.body.scrollLeft;
...
function MakePopupMenu()
{
	...
	if(v_FilterInMenu == true || v_FilterInMenu == "true")
		result += item.replace("#func","ShowFilter();HidePopupMenu()").replace("#item_name","Установить фильтр");	
	result += item.replace("#func","fnNext()").replace("#item_name","Следующая страница");
	result += item.replace("#func","fnPrev()").replace("#item_name","Предыдущая страница");	
	result += "<hr>";
