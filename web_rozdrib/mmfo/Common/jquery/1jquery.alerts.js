...
(function ($) {
    ...
    $.popups = {
        ...
        confirm: function (message, title, callback) {
            ...
            $.popups._dialog(title, message, null, 'confirm', function (result) {
                if (callback) callback(result);
            });
        },
        ...
        // Private methods
        // Отрисовка диалога
        _dialog: function (title, msg, value, type, callback) {
            ...
            if (type == "confirm")
                eval("$('#dialogContent').dialog('option', 'buttons', { '" + jsres$core.close + "': function () { $(this).dialog('close'); a return 0;} })");
            else
                eval("$('#dialogContent').dialog('option', 'buttons', { '" + jsres$core.close + "': function () { $(this).dialog('close'); } })");
            ...
            if (type == "error2") {
                $("#dialogContent p").load("handler.axd?action=error&r=" + Math.random(), function (response, status, xhr) {
                    if (status == "error") {
        ...
        _callBack: function (returnValue) {
            if (callBackFunction) eval(callBackFunction + '(returnValue)');
        },
    ...
    core$ConfirmBox = function (message, title, callback) {
        $.popups.confirm(message, title, callback);
    };
