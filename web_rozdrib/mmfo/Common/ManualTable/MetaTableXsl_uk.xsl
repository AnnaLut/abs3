...
<div align="center" style="color:green;White;font-family:Verdana;font-size:8pt;"><xsl:text>Довідник3:</xsl:text><xsl:value-of select="name" /></div>
</xsl:for-each>
...
<TR>
    ...
    <xsl:attribute name="onclick"><xsl:text>ReturnResult('</xsl:text><xsl:value-of select="COL1"/><xsl:text>',document.getElementById('</xsl:text><xsl:value-of select="concat('td_2',$id)" /><xsl:text>').innerHTML</xsl:text>
    <xsl:if test="$col_count>=3">
    <xsl:text>,document.getElementById('</xsl:text><xsl:value-of select="concat('td_3',$id)" /><xsl:text>').innerHTML</xsl:text>
    </xsl:if>
    <xsl:if test="$col_count>=4">
    <xsl:text>,document.getElementById('</xsl:text><xsl:value-of select="concat('td_4',$id)" /><xsl:text>').innerHTML</xsl:text>
    </xsl:if>
    <xsl:if test="$col_count>=5">
    <xsl:text>,document.getElementById('</xsl:text><xsl:value-of select="concat('td_5',$id)" /><xsl:text>').innerHTML</xsl:text>
    </xsl:if>
    <xsl:text>)</xsl:text></xsl:attribute>
	...
	<xsl:attribute name="id"><xsl:value-of select="concat('td_2',$id)" /></xsl:attribute>
	<xsl:value-of select="COL2" /></TD>
	</xsl:if>
	<xsl:if test="$col_count>=3">
	<TD noWrap="true"><xsl:attribute name="id"><xsl:value-of select="concat('td_3',$id)" /></xsl:attribute><xsl:value-of select="COL3" /></TD>
	</xsl:if>
	<xsl:if test="$col_count>=4">
	<TD noWrap="true"><xsl:attribute name="id"><xsl:value-of select="concat('td_4',$id)" /></xsl:attribute><xsl:value-of select="COL4" /></TD>
	</xsl:if>
	<xsl:if test="$col_count>=5">
	<TD noWrap="true"><xsl:attribute name="id"><xsl:value-of select="concat('td_5',$id)" /></xsl:attribute><xsl:value-of select="COL5" /></TD>
	</xsl:if>
	<xsl:if test="$col_count>=6">
	<TD noWrap="true"><xsl:attribute name="id"><xsl:value-of select="concat('td_6',$id)" /></xsl:attribute><xsl:value-of select="COL6" /></TD>
	</xsl:if>
