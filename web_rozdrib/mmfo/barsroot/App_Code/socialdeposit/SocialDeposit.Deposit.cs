...
public class SocialDeposit
{
    ...
    public void WriteToDatabase()
    {
        ...
        try
        {
            _dbLogger.Debug("Користувач почав запис депозитного договору для клієнта №"
            ...
            // Создаем договор
            _dbLogger.Debug("Почалась реєстрація депозитного договору для клиєнта №"
                + this.Client.ID.ToString(), "SocialDeposit");
            WriteContract(connect);
            _dbLogger.Debug("Депозитний договір бул успішно зареєстрований під номером "
    ...
    public void AddContractText(string[] _templates)
    {
        ...
        try
        {
            _dbLogger.Debug("Користувач розпочав формування пачки документів для договору №"
                + this.ID.ToString(), "SocialDeposit");
            ...
            _dbLogger.Debug("Користувач завершив формування пачки документів для договору №" +
                this.ID.ToString() + ".", "SocialDeposit");
        }
        finally
        {
    ...
    public void UpdateContarct(HttpContext ctx)
    {
        ...
        try
        {
            _dbLogger.Debug("Обновлення депозитного договору №" + this.ID.ToString() + " розпочалось.",
                "SocialDeposit");
    ...
    public string CreateContractTextFile(HttpContext ctx)
    {
        ...
        try
        {
            _dbLogger.Debug("Пользователь дал запрос на чтение из базы текста депозитного договора №" + this.ID.ToString(),
                "SocialDeposit");
        ...
        _dbLogger.Debug("Пользователь успешно вычитал из базы текст депозитного договора №" + this.ID.ToString(),
            "SocialDeposit");
    ...
    public void ClearContractTextFile(HttpContext ctx)
    {
        ...
        catch
        {
            ...
            _dbLogger.Warning("Тимчасові файли невдалося очистити!",
                "SocialDeposit");
        }

        if (success)
            _dbLogger.Debug("Временные файлы были успешно очищены", "SocialDeposit");
    }
    /// <summary>
    /// 
    /// </summary>
    ...
    public decimal WriteAddAgreement(String agr_id, String template, String trust_id)
    {
        ...
        try
        {
            ...
            _dbLogger.Debug("Формирование доп. соглашения (тип = " +
    ...
    public string CreateAddAgreementTextFile(HttpContext ctx)
    {
        ...
        using (OracleConnection connect = conn.GetUserConnection())
        using (OracleCommand cmdSetRole = connect.CreateCommand(),
                  cmdSelectContractText = connect.CreateCommand())
        {
            cmdSetRole.CommandText = conn.GetSetRoleCommand("DPT_ROLE");
            cmdSetRole.ExecuteNonQuery();
            ...
            _dbLogger.Debug("Пользователь дал запрос на чтение из базы текста доп. соглашения №" + add_num.ToString() +
        ...
        _dbLogger.Debug("Пользователь успешно вычитал из базы текст доп. соглашения №" + add_num.ToString() +
            " для депозитного договора №" + this.ID.ToString(),
            "SocialDeposit");
