...
0020: 0000000000C30000 002C000000020000 0003000000000000 00B1010000F8FFFF
...
.head 4 +  Actions
.head 5 +  If SalModalDialog( dlgTagBrowse, hWndForm, SalStrUpperX(strSelect), strTagVal )
...
.head 7 -  Call CurrentLangTable.AddAtom('dlgAsk2Pay.bgSum2','Предупреждение')
.head 7 -  Call CurrentLangTable.AddAtom('dlgAsk2Pay.Acnt','Счет')
.head 7 -  Call CurrentLangTable.AddAtom('dlgAsk2Pay.NoDeb','Заблокирован на ДЕБЕТ')
.head 7 -  Call CurrentLangTable.AddAtom('dlgAsk2Pay.NoKre','Заблокирован на КРЕДИТ')
.head 7 -  Call CurrentLangTable.AddAtom('dlgAsk2Pay.NoMoni','Недостаточно средств для выполнения операции')
.head 7 -  Call CurrentLangTable.AddAtom('dlgAsk2Pay.TooMuch','Превышение максимального остатка')
.head 7 -  ! dlgClient
...
.head 4 -  Next Class Child Key: 167
...
.head 10 -  Set dfOkpoA=GetBankOkpoS()
...
.head 9 -  Call SalMessageBox( CurrentLangTable.GetAtomTitle('DocWMesErrDateValue'),
                    CurrentLangTable.GetAtomTitle('DlgMSG_Error'), MB_IconExclamation )
.head 9 -  Return VALIDATE_Cancel
.head 8 -  ! Інструкція про безготівкові розрахунки в Україні в національній валюті 
.head 8 -  ! 3.5 Банк платника приймає платіжне доручення до виконання протягом 30 календарних днів з дати його виписки.
.head 8 -  ! День оформлення платіжного доручення не враховується.
.head 8 +  If dfDocD < dDat-30 or dfDocD > dDat+30
.head 9 -  Call SalMessageBox( CurrentLangTable.GetAtomTitle('DocWMesErrDateValue'),
                    CurrentLangTable.GetAtomTitle('DlgMSG_Error'), MB_IconExclamation )
.head 9 -  Return VALIDATE_Cancel
.head 8 +  If bFixDocD
...
.head 7 +  On UM_Update
.head 8 +  If nDigA
.head 9 -  Set sTmp=IifS(bTriad,'# ##0','#0')||sDecimal||SalStrRepeatX( '0', nDigA )
.head 8 +  Else
.head 9 -  Set sTmp=IifS(bTriad,'# ##0','#0')
.head 8 +  If nFlv
.head 9 -  Call SalFmtSetPicture( dfSumA, sTmp )
.head 9 -  Set dfSumNA=CurrentLangTable.GetAtomTitle('DocWDfNamSumma')||' '||dfKvA
.head 8 +  Else
.head 9 +  If aFlg[58] ! nFcb
...
.head 9 +  If dfSumA
.head 10 -  Call SalSendMsg( dfSumA, SAM_Validate, 0, 0 )  !-- MIK
.head 8 +  If aFlg[14]=1 and not bTotVal ! nFeq
...
"SELECT s.nms,NVL(TRIM(k.nmkk),TRIM(k.nmk)),k.nmkv,k.okpo, k.country  
  ...
  FROM "||IifS(aFlg[26],"accounts","saldo"||sTmp) ||" s,customer k,cust_acc a   
  WHERE nls = :dfNlsA AND kv = :nKvA AND a.rnk=k.rnk AND s.acc=a.acc AND s.dazs IS NULL" ))
.head 11 +  If SqlFetchNext( hSql(), nFetchRes )
.head 12 -  Set sNamAStored1=SalStrLeftX(SalStrTrimX(sNamAStored1),38)
.head 12 -  Set sNamAStored2=SalStrLeftX(SalStrTrimX(sNamAStored2),38)
.head 12 +  If sKlF   !! Уточнение ОКПО по признаку б/с
.head 13 -  Call OKPOamnd(dfNlsA,dfOkpoA)
.head 12 +  If aFlg[56] ! Платник - Банк
.head 13 -  Set sNamAStored1=SalStrLeftX(SalStrTrimX(GetBankName()),38)
.head 13 -  Set sNamAStored2=SalStrLeftX(SalStrTrimX(GetBankName()),38)
.head 13 -  Set dfOkpoA=GetBankOkpoS()
.head 12 +  If NOT dfOkpoA
.head 13 -  ! ОКПО нерезидентов
.head 13 +  If GetBankCountryCode()!=nTmpCountry
.head 14 -  Set dfOkpoA=SalNumberToStrX( nTmpCountry, 0 )
.head 13 -  ! Недорегистрированные клиенты
.head 13 +  Else
.head 14 -  Call SalMessageBox( CurrentLangTable.GetAtomTitle('DocWMesErrOkpoNlsA')||' '||dfKvA||"-"||dfNlsA ,
                    CurrentLangTable.GetAtomTitle('DlgMSG_Error'),  MB_IconExclamation )
.head 14 -  Return VALIDATE_Cancel
.head 12 +  If not SalStrTrimX(dfNamA)
.head 13 +  If nFli = 1  ! Если межбанк СЭП то клиента
.head 14 -  Set dfNamA=sNamAStored2
.head 13 +  Else ! Иначе - Наименован Счета
.head 14 -  Set dfNamA=sNamAStored1
.head 13 +  If nFli = 2 
.head 14 -  Set i = VisArrayFindString (aTag, '50')
.head 14 +  If i > 0
.head 15 +  If str50
.head 16 -  Set aVal[i]=str50
.head 15 +  Else
.head 16 -  Set aVal[i]=sNamAStored2
.head 12 -  Call SalSendMsg( dfOst, SAM_Validate, 0, 0 )
.head 11 +  Else
.head 12 -  Call SalMessageBox( CurrentLangTable.GetAtomTitle('DocWMesErrNlsNotFound')||' '||dfKvA||"-"||dfNlsA ,
                    CurrentLangTable.GetAtomTitle('DlgMSG_Error'),  MB_IconExclamation )
.head 12 -  Set hWnd4Focus=hWndItem
.head 12 -  Return VALIDATE_Cancel
.head 9 +  Else
.head 10 -  Call SqlPrepareAndExecute( hSql(),
T("SELECT name,okpo INTO :dfNamA,:dfOkpoA from alien  where mfo = :dfBankA AND nls = :dfNlsA and kv = :nKvA" ) )
.head 10 +  If not SqlFetchNext( hSql(), nFetchRes )
.head 11 -  Set dfNamA=''
.head 11 -  Set dfOkpoA=''
.head 8 +  If bFixNlsA 
...
.head 6 -  Class DLL Name:
.head 6 -  Data
.head 7 -  Maximum Data Length: 128
.head 7 -  Data Type: String
.head 7 -  Editable? Yes
.head 6 -  Display Settings
.head 7 -  Window Location and Size
...
.head 9 -  Call HideIt( hWndItem )
.head 7 +  On SAM_SetFocus
.head 8 +  If SalStrLength(dfNamA)>38
.head 9 -  Call SalColorSet( hWndItem, COLOR_IndexWindow, COLOR_LightPeriwinkle )
.head 8 +  Else
.head 9 -  Call SalColorSet( hWndItem, COLOR_IndexWindow, COLOR_Sky )
.head 8 +  If not SalStrTrimX(sNlsA_TT)
.head 9 -  Set bNamA=TRUE
.head 9 -  Set bAccA=2
...
.head 9 -  Return VALIDATE_Ok
.head 8 +  If SalIsValidInteger( hWndItem )
.head 9 +  If SalStrLength(dfOkpoA) > 8 and SalStrLength(dfOkpoA) < 11 or dfOkpoA='99999'
.head 10 -  Return VALIDATE_Ok
.head 9 +  If SalStrLength(dfOkpoA) < 8 
.head 10 -  Set dfOkpoA=SalStrRightX( '00000000'||dfOkpoA, 8 )
.head 9 +  If not SalIsNull ( hWndItem ) and dfOkpoA=V_Okpo(dfOkpoA) and dfOkpoA !='00000000'
.head 10 -  Return VALIDATE_Ok
...
.head 8 +  If aFlg[28] = 0 and ( nFli = 0 or nFli = 2 )  ! Если Внутренние или SWIFT
.head 9 -  Set dfBankB = GetBankMfo()
.head 8 +  If SalStrLength(dfBankB)>0
.head 9 -  Call SalSendMsg( hWndItem, SAM_Validate, 0, 0 )
.head 8 +  If (aFlg[28] = 0 and nFli = 0) or nFli = 2 or SalStrLength(sBankB)>0
.head 9 -  Call HideIt( hWndItem )
.head 7 +  On SAM_Validate
...
.head 7 +  On UM_Update
.head 8 +  If nDigB
.head 9 -  Set sTmp=IifS(bTriad,'# ##0','#0')||sDecimal||SalStrRepeatX( '0', nDigB )
.head 8 +  Else
.head 9 -  Set sTmp=IifS(bTriad,'# ##0','#0')
.head 8 +  If nFlv
.head 9 -  Call SalFmtSetPicture( dfSumB, sTmp )
...
.head 10 -  Set dfRatNom = nRatO
.head 9 +  If dfSumB
.head 10 -  Call SalSendMsg( dfSumB, SAM_Validate, 0, 0 )  !-- MIK
.head 7 +  On SAM_SetFocus
.head 8 -  Call SalColorSet( hWndItem, COLOR_IndexWindow, COLOR_Sky )
.head 7 +  On SAM_KillFocus
.head 8 -  Call SalColorSet( hWndItem, COLOR_IndexWindow, COLOR_SysWindow )
...
.head 9 +  If dfBankB = dfBankA and aFlg[27]!=9   ! Для своїх
.head 10 +  If nDk=0 or nDk=2
.head 11 -  Set sTmp='D'
.head 10 +  Else If nDk=1 or nDk=3
.head 11 -  Set sTmp='K'
...
"SELECT s.nms,NVL(TRIM(k.nmkk),TRIM(k.nmk)),k.okpo, k.country 
  ...
  FROM "||IifS(aFlg[27],"accounts","saldo"||sTmp) ||" s,customer k,cust_acc a 
...
.head 9 -  Call HideIt( hWndItem )
.head 7 +  On SAM_SetFocus
.head 8 +  If SalStrLength(dfNamB)>38
.head 9 -  Call SalColorSet( hWndItem, COLOR_IndexWindow, COLOR_LightPeriwinkle )
.head 8 +  Else
.head 9 -  Call SalColorSet( hWndItem, COLOR_IndexWindow, COLOR_Sky )
.head 8 +  If not SalStrTrimX(sNlsB_TT)
.head 9 -  Set bNamB=TRUE
.head 9 -  Set bAccB=2
.head 9 -  Call SalEnableWindow( pbHelp )
.head 7 +  On SAM_KillFocus
.head 8 -  Call SalColorSet( hWndItem, COLOR_IndexWindow, COLOR_SysWindow )
...
.head 9 -  Return VALIDATE_Ok
.head 8 +  If SalIsValidInteger( hWndItem )
.head 9 +  If SalStrLength(dfOkpoB) > 8 and SalStrLength(dfOkpoB) < 11 or dfOkpoB='99999'
.head 10 -  Return VALIDATE_Ok
.head 9 +  If SalStrLength(dfOkpoB) < 8
.head 10 -  Set dfOkpoB=SalStrRightX( '00000000'||dfOkpoB, 8 )
.head 9 +  If not SalIsNull ( hWndItem ) and dfOkpoB=V_Okpo(dfOkpoB) and dfOkpoB !='00000000'
.head 10 -  Return VALIDATE_Ok
...
.head 8 +  If SalIsNull( hWndItem ) or (dfSumA=0 and not aFlg[14]) ! nFeq
...
.head 8 +  If SalIsNull( hWndItem ) or (dfSumB=0 and not aFlg[14]) ! nFeq
.head 9 -  Call SalMessageBox( CurrentLangTable.GetAtomTitle('DocWMesErrSumBlank'),
                    CurrentLangTable.GetAtomTitle('DlgMSG_Error'), MB_IconExclamation )
.head 9 -  Return VALIDATE_Cancel
.head 8 +  If nFlv and aFlg[59] and dfSumA and not bTotVal    ! nFtr ! and not SalQueryFieldEdit ( dfSumA )
.head 9 +  If aFlg[59]=1 and aFlg[11]=0 ! nFtr ! nFhr
.head 10 -  Set dfRatNom=dfSumB/dfSumA
.head 10 -  Call SetTagKurs(dfRatNom,'KURS')
.head 9 +  Else If aFlg[59]=3 and aFlg[11]=0 ! nFtr ! nFhr
.head 10 -  Set dfSumB=dfSumA*dfRatNom
.head 8 +  If aFlg[11]=1 and dfRatNom and not bTotVal ! nFhr
...
.head 8 +  Else
.head 9 -  Call SalHideWindow( hWndItem )
.head 8 +  If aFlg[11] ! nFhr    ! Не дать корректоровать курс
.head 9 -  Call HideIt( hWndItem )
.head 7 +  On SAM_Validate
.head 8 -  Call DecPoint(hWndItem)
.head 8 +  If aFlg[58] ! nFcb
...
.head 8 +  Else If nFlv and aFlg[59]=1 ! nFtr
...
.head 9 -  Call SalMessageBox( CurrentLangTable.GetAtomTitle('DocWMesErrSumFormat'),
                    CurrentLangTable.GetAtomTitle('DlgMSG_Error'), MB_IconExclamation )
.head 9 -  Return VALIDATE_Cancel
.head 8 +  If not (dfSumC!=NUMBER_Null and dfSumC<0 and (nDk=0 or nDk=1) and nFli = 0)
.head 9 -  Set dfSumC=SalNumberAbs(dfSumC)
.head 8 +  If SalIsNull( hWndItem ) or (dfSumC=0 and not aFlg[14]) ! nFeq
.head 9 -  Call SalMessageBox( CurrentLangTable.GetAtomTitle('DocWMesErrSumBlank'),
                    CurrentLangTable.GetAtomTitle('DlgMSG_Error'), MB_IconExclamation )
.head 9 -  Return VALIDATE_Cancel
...
.head 6 +  Message Actions
.head 7 +  On SAM_Create
.head 8 +  If aFlg[14] and not nFlv ! nFeq
.head 9 -  Call SalShowWindow( hWndItem )
...
.head 9 -  Call SalListAdd( hWndItem, sMan ||CurrentLangTable.GetAtomTitle('DocWNazn') )
...
.head 12 -  Call SalListAdd( hWndItem, IifS(aOpt[i]='M',sMan,' ')||SalStrTrimX(sTmp2))
...
.head 9 +  If not sTranN(dfNazn, dfNazn, sTT, dfNlsA, dfBankA, nKvA, dfSumA*SalNumberPower( 10, nDigA),
                                   dfNlsB, dfBankB, nKvB, dfSumB*SalNumberPower( 10, nDigB),dfSk,dfEquiv*100,dfRatNom)
...
.head 11 -  Set i=i-1
.head 8 +  If bNazn
.head 9 +  If SalStrLength(dfNazn)>157
.head 10 -  Call SalColorSet( hWndItem, COLOR_IndexWindow, COLOR_LightPeriwinkle )
.head 9 +  Else
.head 10 -  Call SalColorSet( hWndItem, COLOR_IndexWindow, COLOR_Sky )
.head 7 +  On SAM_Create
.head 8 -  Call UnHideIt( hWndItem )
.head 7 +  On SAM_SetFocus
.head 8 -  Call SalColorSet( hWndItem, COLOR_IndexWindow, COLOR_Sky )
.head 8 -  Call SalSetMaxDataLength ( hWndItem, IifN( SalListQuerySelection( lb1 )=0, 160, 217 ))
.head 8 +  If bNazn
.head 9 +  If SalStrLength(dfNazn)>157
.head 10 -  Call SalColorSet( hWndItem, COLOR_IndexWindow, COLOR_LightPeriwinkle )
.head 9 +  Else
.head 10 -  Call SalColorSet( hWndItem, COLOR_IndexWindow, COLOR_Sky )
.head 8 +  If SalListQuerySelection( lb1 )=0 or aFlg[35] and aBro[SalListQuerySelection( lb1 )]
...
.head 8 -  Return FALSE
.head 7 +  On UM_ParentSize
.head 8 -  Set nWMax=wParam-0.85
.head 8 +  If aFlg[35] ! Допреквізити
.head 9 -  Call SalShowWindow (lb1)
.head 9 -  Call SalGetWindowLoc(lb1, nWMin, nHMin)
.head 9 -  Call SalSetWindowLoc( dfNazn, nWMin+nWMax*2/5, nHMin)
.head 9 -  Call SalSetWindowSize( lb1,    nWMax*2/5, nHMax )
.head 9 -  Call SalSetWindowSize( dfNazn, nWMax*3/5, nHMax )
.head 8 +  Else
.head 9 -  Call SalHideWindow (lb1)
.head 9 -  Call SalGetWindowLoc( lb1, nWMin, nHMin)
.head 9 -  Call SalSetWindowLoc( dfNazn, nWMin, nHMin)
.head 9 -  Call SalSetWindowSize( dfNazn, nWMax, nHMax)
.head 5 +  Data Field: dfSub
.head 6 -  Class Child Ref Key: 0
.head 6 -  Class ChildKey: 167
.head 6 -  Class:
.head 6 -  Property Template:
.head 6 -  Class DLL Name:
.head 6 -  Data
...
.head 6 +  Message Actions
.head 7 +  On SAM_Create
.head 8 +  If aFlg[24] ! Субсчет
.head 9 -  Call SalShowWindow( dfSubN )
.head 9 -  Call SalShowWindow( hWndItem )
.head 9 -  Call UnHideIt( hWndItem )
.head 8 +  Else
.head 9 -  Call SalHideWindow( dfSubN )
.head 9 -  Call SalHideWindow( hWndItem )
.head 9 -  Call HideIt( hWndItem )
.head 8 -  Set dfSub=STRING_Null  
.head 7 +  On SAM_Validate
.head 8 -  Set dfSub=SalStrTrimX(dfSub)
.head 8 -  Return VALIDATE_OkClearFlag
.head 7 +  On SAM_SetFocus
.head 8 -  Call SalColorSet( hWndItem, COLOR_IndexWindow, COLOR_Sky )
.head 7 +  On SAM_KillFocus
...
.head 8 -  Call SalSetWindowLoc ( hWndItem, wParam-0.85-1.65, nY )
...
.head 6 +  Message Actions
.head 7 +  On SAM_Create
.head 8 -  Set dfSkN=SalStrLowerX(SalStrTrimX(CurrentLangTable.GetAtomTitle('DocWDfSkN')))
.head 7 +  ! On UM_NoFontChange	! Отказ от смены фонта
.head 8 -  Return TRUE
.head 5 +  ! Data Field: dfSub2
.winattr
.head 6 -  Class Child Ref Key: 0
.head 6 -  Class ChildKey: 159
.head 6 -  Class:
.head 6 -  Property Template:
.head 6 -  Class DLL Name:
.head 6 -  Data
...
.head 7 +  On SAM_Create
.head 8 -  Set dfSk=nSk
.head 8 +  If aFlg[9] = 0    ! nFsk некассовые
.head 9 -  Call SalDisableWindow( hWndItem )
.head 9 -  Call SalHideWindow(dfSk)
.head 9 -  Call SalHideWindow(dfSkN)
.head 8 +  Else If ( aFlg[9] = 1 or aFlg[9] = 2) and nSk > 0  ! кассовые и задан умалчиваемый символ
.head 9 -  Call HideIt( hWndItem )
.head 8 +  Else
...
.head 9 -  Return VALIDATE_Ok
.head 8 +  If (aFlg[9]=1 or aFlg[9]=5) and (nDk=0 or nDk=2) and dfSk < 40 
or (aFlg[9]=1 or aFlg[9]=5) and (nDk=1 or nDk=3) and dfSk >= 40
or (aFlg[9]=2 or aFlg[9]=6) and (nDk=0 or nDk=2) and dfSk >= 40
or (aFlg[9]=2 or aFlg[9]=6) and (nDk=1 or nDk=3) and dfSk < 40
...
.head 7 +  On UM_ParentSize
.head 8 -  Call SalGetWindowLoc ( hWndItem , nX, nY )
.head 8 -  Call SalSetWindowLoc ( hWndItem, wParam/2-2.5, nY )
...
.head 7 +  On UM_ParentSize
.head 8 -  Call SalGetWindowLoc ( hWndItem , nX, nY )
.head 8 -  Call SalSetWindowLoc ( hWndItem, wParam-0.85-3.2, nY )
...
.head 8 +  Else
.head 9 +  If aFlg[5]=1 
.head 10 -  Call SqlPrepareAndExecute( hSql(), "SELECT TRUNC(SYSDATE) INTO :dfValD1 FROM dual" )
.head 10 +  If not SqlFetchNext( hSql(), nFetchRes )
.head 11 -  Set dfValD1=dDat
.head 9 +  Else If aFlg[5]=2 
.head 10 -  Set dfValD1=SalDateCurrent(  )
.head 9 +  Else
.head 10 -  Set dfValD1=dDat
.head 8 +  If aFlg[3] ! nFvD1
.head 9 -  Call SalShowWindow( hWndItem )
...
.head 7 +  On UM_ParentSize
.head 8 -  Call SalGetWindowLoc ( hWndItem , nX, nY )
.head 8 -  Call SalSetWindowLoc ( hWndItem, wParam/2-1.14, nY )
...
.head 6 +  Message Actions
.head 7 +  On UM_ParentSize
.head 8 -  Call SalGetWindowSize ( hWndItem , nX, nY )
.head 8 -  Call SalSetWindowSize ( hWndItem , wParam-0.85, nY )
.head 5 +  Picture: pic2f
.head 6 -  Class Child Ref Key: 0
.head 6 -  Class ChildKey: 157
.head 6 -  Class:
.head 6 -  Property Template:
.head 6 -  Class DLL Name:
.head 6 -  Window Location and Size
...
.head 5 -  ! Флаги
.head 5 -  String: sFlags      ! Строка флагов
.head 5 -  Number: aFlg[*]     ! Массив флагов
...
.head 5 -  !
.head 5 -  Boolean: bTriad    ! показ суми в тріадах
...
.head 5 -  String: sMan         ! Символ для выделения обязат полей
...
.head 5 -  String: strMT
.head 5 -  Number: nX     !ширина
.head 5 -  Number: nY     !высота
.head 5 -  Number: nZ     
.head 5 -  Number: nWinW     !ширина окна
.head 5 -  Number: nWinH     !высота окна
.head 4 +  Functions
...
.head 9 -  Set i=i-1
.head 7 -  ! 28.01.2015 - убрали насильное заполнение реквизита 50
...
.head 5 +  Function: Flgs2aFlg     ! Розбирає флаги операцій в масив aFlg
.head 6 -  Description:
.head 6 -  Returns
.head 6 +  Parameters
.head 7 -  String: sFlags
.head 6 -  Static Variables
.head 6 +  Local variables
.head 7 -  Number: i
.head 6 +  Actions
.head 7 +  While i<64
.head 8 -  Set aFlg[i]=SalStrToNumber(SalStrMidX(sFlags,i,1))
.head 8 -  Set i=i+1
...
.head 6 -  Set sMan      = GetGlobalOption('MANSYM')  ! Символ для выделения обязат полей 
.head 6 -  Set bTriad    = IifN(GetGlobalOption('TRISUM')='Y',1,0)  ! Показ суми в тріадах 
...
.head 8 +  If nFlv
.head 9 +  If dfSumA<0 
.head 10 -  Set strSumPr=SumPr( -dfSumB*SalNumberPower( 10, nDigB), 'F', dfKvB,sDigNb ) || ' за ' ||SalNumberToChar( 13 )||SalNumberToChar( 10 )||
                          SumPr( -dfSumA*SalNumberPower( 10, nDigA), 'F', dfKvA,sDigNa ) 
.head 9 +  Else
.head 10 -  Set strSumPr=SumPr( dfSumA*SalNumberPower( 10, nDigA), 'F', dfKvA,sDigNa ) || ' за ' ||SalNumberToChar( 13 )||SalNumberToChar( 10 )||
                          SumPr( dfSumB*SalNumberPower( 10, nDigB), 'F', dfKvB,sDigNb ) 
.head 8 +  Else
.head 9 -  Set strSumPr=SumPr( SalNumberAbs( dfSumA )*SalNumberPower( 10, nDigA), 'F', dfKvA,sDigNa )
.head 8 +  If not SalModalDialog( dlgAsk2Pay,hWndForm,strSumPr,bFaktPlan,nDk,dfNlsA,nKvA,dfSumA*SalNumberPower( 10, nDigA),dfNlsB,nKvB,dfSumB*SalNumberPower( 10, nDigB),IifN(dfBankA=dfBankB,1,0))
.head 9 -  Break
.head 8 +  If dfSumC!=NUMBER_Null and dfSumC<0 and (nDk=0 or nDk=1)
.head 9 -  Call SetDocQ( 0, sTT, 1-nDk, nVob, dfNd, dfDocD, dDat, dfValD1, dfValD2,
     dfNlsA, SalStrLeftX(dfNamA,SalStrScan(dfNamA||'==','==')), dfBankA, dfNbA, nKvA, -dfSumA*SalNumberPower( 10, nDigA), dfOkpoA,
     dfNlsB, SalStrLeftX(dfNamB,SalStrScan(dfNamB||'==','==')), dfBankB, dfNbB, nKvB, -dfSumB*SalNumberPower( 10, nDigB), dfOkpoB, dfNazn, sDrec,GetIdOper(),'',dfSk, cbUgrnt ,-dfEquiv*100)
.head 9 -  ! Call Debug('1A'||Str(A.S))
.head 9 -  ! Call Debug('1B'||Str(B.S))
.head 8 +  Else
.head 9 -  Call SetDocQ( 0, sTT, nDk, nVob, dfNd, dfDocD, dDat, dfValD1, dfValD2,
     dfNlsA, SalStrLeftX(dfNamA,SalStrScan(dfNamA||'==','==')), dfBankA, dfNbA, nKvA, dfSumA*SalNumberPower( 10, nDigA), dfOkpoA,
     dfNlsB, SalStrLeftX(dfNamB,SalStrScan(dfNamB||'==','==')), dfBankB, dfNbB, nKvB, dfSumB*SalNumberPower( 10, nDigB), dfOkpoB, dfNazn, sDrec,GetIdOper(),'',dfSk, cbUgrnt ,dfEquiv*100)
.head 8 +  If GetGlobalOption('SUB')='1' 
.head 9 -  Set m_sSub=dfSub
...
.head 7 +  Else
.head 8 +  If SalModalDialog( dlgClient,hWndForm,nFli,sTT,(bAccA>0)!=SalNumberMod(nDk,2),bAccA+bAccB,sNls,sNms,sNmk,nKv,sLcv,sOkpo,str50 )
.head 9 +  If bAccA      ! Поищем счет А
.head 10 -  Set sNamAStored1=SalStrLeftX(SalStrTrimX(IifS(aFlg[56]=1,GetBankName(),sNms)),38)
.head 10 -  Set sNamAStored2=SalStrLeftX(SalStrTrimX(IifS(aFlg[56]=1,GetBankName(),sNmk)),38)
.head 10 -  Set dfNlsA=sNls
.head 10 -  Set dfNamA=sNamAStored1
.head 10 -  Set nKvA=nKv
.head 10 -  Set dfKvA=sLcv
.head 10 -  Set dfOkpoA=IifS(aFlg[56]=1,GetBankOkpoS(),sOkpo)
.head 10 +  If nFli = 2
...
.head 7 +  If aFlg[35] and i ! nFdp  
.head 8 +  If aBro[i]
.head 9 -  Set strBrowserMask=SalStrUpperX(SalStrTrimX(dfNazn))
.head 9 -  Set nType = SalCompileAndEvaluate( aBro[i],
    nError, nPos, nTmp, sTmp, dTmp, hNextDF, TRUE, SalContextCurrent())
.head 9 -  Set strBrowserMask=''
.head 9 +  If nError = 0
...
.head 8 -  Return FALSE
.head 7 +  If sTmp
.head 8 +  ! If aFlg[35] and i and SalStrLeftX(aTag[i],1)='5' ! nFdp
.head 9 -  Set dfNazn=IifS(SalStrTrimX(dfNazn)='','',SalStrTrimX(dfNazn)||PutCrLf())||sTmp
.head 8 -  ! Else
.head 8 -  Set dfNazn=sTmp
...
'SELECT sk,name FROM sk WHERE sk'||
IifS((aFlg[9]=1 or aFlg[9]=5) and (nDk=0 or nDk=2) or
     (aFlg[9]=2 or aFlg[9]=6) and (nDk=1 or nDk=3),'<','>=')||"40 AND (d_close IS NULL OR d_close>TO_DATE('"||SalFmtFormatDateTime( dDat, 'yyyyMMdd' )||"','YYYYMMDD')) ORDER BY sk",sTmp )
.head 8 -  Return FALSE
.head 7 +  Else
...
.head 5 +  On UM_HotFio
.head 6 +  If aFlg[35] ! nFdp
.head 7 +  ! If dfNlsA
.head 8 -  Call SqlPrepareAndExecute(hSql(),"SELECT count(*) INTO :nCount FROM podotw WHERE tag='NLS' AND val=:dfNlsA")
.head 8 +  If not SqlFetchNext(hSql(),nFetchRes)
.head 9 -  Return FALSE
.head 8 +  If nCount=1
.head 9 -  Call SqlPrepareAndExecute(hSql(),"SELECT id INTO :nIdPodot FROM podotw WHERE tag='NLS' AND val=:dfNlsA")
.head 9 +  If not SqlFetchNext(hSql(),nFetchRes)
.head 10 -  Return FALSE
.head 8 -  Else 
.head 7 +  If not SalModalDialog( dlgInkast, hWndForm, '',nIdPodot)
.head 8 -  Return FALSE
.head 7 -  Call SqlPrepareAndExecute(hSql(),'SELECT tag,val INTO :sTmp1,:sTmp2 FROM podotw WHERE id=:nIdPodot')
.head 7 +  While SqlFetchNext(hSql(),nFetchRes)
.head 8 -  Set nCount = VisArrayFindString( aTag, sTmp1 )
.head 8 +  If nCount > 0
.head 9 -  Set aVal[nCount]=sTmp2
.head 9 +  If SalListQuerySelection( lb1 )=nCount
.head 10 -  Set dfNazn=aVal[nCount]
.head 5 +  On UM_Clear
...
.head 6 -  ! Символ кассы прыгает туда-сюда
.head 6 +  If aFlg[9]=1 or aFlg[9]=5
.head 7 -  Call XPlaceSk(dfSkN,dfSk,dfBankB)
.head 6 +  Else If aFlg[9]=2 or aFlg[9]=6
.head 7 -  Call XPlaceSk(dfSkN,dfSk,dfBankA)
.head 6 -  ! --- WindowS ReSize ---
.head 6 -  Set nWinW=10
.head 6 -  Set nWinH=5.6
.head 6 +  ! If aFlg[24] or aFlg[25] ! Субсчета
.head 7 -  Set nWinW=12
.head 6 +  If aFlg[61]             ! Чеки
.head 7 -  Set nWinH=7.1
.head 6 -  Call SalSetWindowSize ( hWndForm , nWinW, nWinH )
.head 6 -  Call SalSendMsgToChildren( hWndForm, UM_ParentSize, nWinW, nWinH )
...
.head 7 +  If not SalModalDialog( dlgDocStart,hWndForm,sTT,sTTN,nFli,nFlv,nDk,nKvA_TT,nKvB_TT,sNlsA_TT,sNlsB_TT,sBankB_TT,nSk_TT,sS,sSS,sFlags,sTTFilter )
.head 8 -  Call SalDestroyWindow( hWndForm )
.head 8 -  Return TRUE
.head 7 -  Call Flgs2aFlg(sFlags)
...
.head 5 +  On UM_Update
.head 6 +  If nFli = 1 or aFlg[28]  ! Если межбанк или псевдомежбанк (фл-28)
.head 7 -  Call SqlPrepareAndExecute(hSql(),
...
.head 6 +  If aFlg[3]           ! Дата валютировния 1 
...
.head 6 +  If dDat and aFlg[60] ! nFbk
.head 7 -  Call Pl_dat(dDat)
.head 5 +  On WM_KeyUp
...
"SELECT DISTINCT t.tt,t.name,t.fli,t.flv,NVL(t.dk,1),t.kv,t.kvk,t.nlsa,t.nlsb,t.mfob,NVL(t.sk,0),t.flags,TRIM(t.s),TRIM(t.s2)
  ...
  WHERE s.tt=t.tt AND s.id IN ("|| GetSubstListId( nId, TRUE ) ||") AND 
        t.fli<3 AND t.flags[1,1]='1' AND t.flags[63,63]='0' " || strSecure || sTTFilter ||
" ORDER BY tt") , TBL_FillAll )
...
.head 6 -  Set sFlags=colFlags
.head 6 -  Set nDk=colDk
.head 6 +  If SalStrMidX(colFlags,9,1)='0' ! nFsk
...
.head 6 -  Call SalSetWindowSize ( hWndItem , wParam-0.5, 0.8*lParam )
...
.head 5 +  On UM_ParentSize
.head 6 -  Call SalSetWindowLoc ( hWndItem, wParam-0.5-1.35, 0.75*lParam+0.6 )
...
.head 5 +  On UM_ParentSize
.head 6 -  Call SalSetWindowLoc ( hWndItem, 0.3, 0.75*lParam+0.6 )
.head 3 +  Picture: pic0f
.head 4 -  Class Child Ref Key: 0
.head 4 -  Class ChildKey: 0
.head 4 -  Class:
.head 4 -  Property Template:
...
.head 4 +  Message Actions
.head 5 +  On UM_ParentSize
.head 6 -  Call SalSetWindowLoc ( hWndItem, 0.1, 0.75*lParam+0.55 )
.head 6 -  Call SalGetWindowSize ( hWndItem , nX, nY )
.head 6 -  Call SalSetWindowSize ( hWndItem , wParam-0.5, nY )
.head 2 -  Functions
...
.head 3 -  Receive String: sFlags
.head 3 -  String: sTTFilter           ! Внешний фильтр для операций
...
.head 4 -  Call SalSetWindowSize ( hWndForm , 10, 5.6 )
...
.head 5 +  On UM_ParentSize
.head 6 -  Call SalGetWindowLoc ( hWndItem , nZ, nY )
.head 6 -  Call SalGetWindowSize ( hWndItem , nX, nY )
.head 6 -  Call SalSetWindowSize ( hWndItem , wParam-2.2-nZ, nY )
.head 5 +  On WM_KeyDown
.head 6 -  Call SalSendMsg( tbl, WM_KeyDown, wParam, lParam )
...
.head 6 -  Call SalWaitCursor(TRUE)
.head 6 -  Call SqlPrepareAndExecute(hSql(),"SELECT nbs,"||
     IifS(GetGlobalOption('XPS_TTS')='Y','NVL(x,1)','1')||' INTO :sTmp,:x'||
   " FROM ps_tts WHERE tt=:sTT and dk=:nDk")
.head 6 +  If SqlFetchNext(hSql(), nFetchRes) ! Есть ограничения на б/с
.head 7 +  If GetGlobalOption('OB22')='1'
.head 8 -  Set sPs = ' AND '||IifS(x=1,'','NOT')||' EXISTS (
    SELECT 1
      FROM ps_tts p
     WHERE p.tt=:sTT and p.dk=:nDk
       AND ( TRIM(p.nbs) = s.nbs[1,1] or
             TRIM(p.nbs) = s.nbs[1,2] or
             TRIM(p.nbs) = s.nbs[1,3] or 
                  p.nbs  = s.nbs )
       AND ( p.ob22 IS NULL or p.ob22 = s.ob22 ))'
.head 7 +  Else
.head 8 -  Set sPs = ' AND '||IifS(x=1,'','NOT')||' EXISTS (
    SELECT 1
      FROM ps_tts p
...
.head 7 -  Set sSql = ''
.head 6 -  Set sSql = 
   "SELECT s.nls,s.nms,NVL(TRIM(k.nmkk),TRIM(k.nmk)),s.kv,t.lcv,k.okpo,k.nmkv,s.blkd,s.blkk
    INTO :colNls,:colNms,:colNmk,:colKv,:colLcv,:colOkpo,:colNmkV,:nBlkD,:nBlkK   
    FROM saldo" || sSql || " s, tabval t, customer k  
    WHERE s.tip<>'VP' and s.kv=t.kv and 
          s.rnk=k.rnk and s.dazs is null"
.head 6 +  If nKv
...
.head 6 -  Set strDSql=T(sSql || ' ORDER BY s.nls')
.head 6 +  If bPages !Переводим выборку в формат со страницами
.head 7 -  Call SalTblQueryVisibleRange ( hWndForm, nRangeMin, nRangeMax )
.head 7 -  Set nRow1=(nRangeMax-nRangeMin+1)*(nPage-1)+1
.head 7 -  Set nRow2=(nRangeMax-nRangeMin+1)* nPage
.head 7 -  Set strDSql=ToPageStyle(strDSql)
...
.head 5 +  On UM_ParentSize
.head 6 -  Call SalSetWindowSize ( hWndItem , wParam-0.5, 0.7*lParam )
.head 5 +  On SAM_FetchRowDone
.head 6 +  If nDk=0 and nBlkD>0 or nDk=1 and nBlkK>0
.head 7 -  Call XSalTblSetRowBackColor( hWndForm, lParam, COLOR_LightGray)
.head 5 +  On WM_KeyDown
.head 6 +  If bPages and wParam=VK_PageUp
.head 7 -  Call SalSendMsg( pbPageUp, SAM_Click, 0, 0 )
.head 6 +  If bPages and wParam=VK_PageDown
.head 7 -  Call SalSendMsg( pbPageDn, SAM_Click, 0, 0 )
.head 6 +  If wParam=VK_F12
.head 7 -  Set bPages=not bPages
.head 7 -  Call SalSendMsg( pbPageUp, SAM_Create, 0, 0 )
.head 7 -  Call SalSendMsg( dfPage, SAM_Create, 0, 0 )
.head 7 -  Call SalSendMsg( pbPageDn, SAM_Create, 0, 0 )
...
.head 5 +  On UM_ParentSize
.head 6 -  Call SalSetWindowLoc ( hWndItem, 0.3, 0.7*lParam+0.6 )
.head 5 +  On WM_KeyDown
.head 6 -  Call SalSendMsg( tbl, WM_KeyDown, wParam, lParam )
...
.head 5 +  On UM_ParentSize
.head 6 -  Call SalGetWindowLoc ( hWndItem , nZ, nY )
.head 6 -  Call SalSetWindowLoc ( hWndItem, wParam/2-0.7, 0.7*lParam+0.6 )
.head 5 +  On SAM_Create
...
.head 5 +  On UM_ParentSize
.head 6 -  Call SalSetWindowLoc ( hWndItem, wParam-0.5-1.2, 0.7*lParam+0.6 )
.head 5 +  On WM_KeyDown
.head 6 -  Call SalSendMsg( tbl, WM_KeyDown, wParam, lParam )
...
.head 6 +  Else
.head 7 -  Set nStat=3
.head 6 -  Call ShowWindow(hWndForm,nStat)  ! 9--SW_RESTORE,3--SW_MAXIMIZE)
.head 3 +  Pushbutton: pbPageUp
.head 4 -  Class Child Ref Key: 0
.head 4 -  Class ChildKey: 0
...
.head 4 +  Message Actions
.head 5 +  On SAM_Create
.head 6 -  Set strTip = 'PgUp'
.head 6 +  If bPages
.head 7 -  Call SalShowWindow(hWndItem)
.head 6 +  Else
.head 7 -  Call SalHideWindow(hWndItem)
.head 5 +  On SAM_Click
.head 6 -  Set dfPage=IifN(dfPage=1,1,dfPage-1)
.head 6 -  Set nPage=dfPage
.head 6 -  Call SalSendMsg( tbl, UM_Populate, 0, 0 )
.head 5 +  On UM_ParentSize
.head 6 -  Call SalGetWindowLoc ( hWndItem , nZ, nY )
.head 6 -  Call SalSetWindowLoc ( hWndItem, wParam-2.0, nY )
.head 5 +  On WM_KeyDown
.head 6 -  Call SalSendMsg( tbl, WM_KeyDown, wParam, lParam )
.head 3 +  Data Field: dfPage
.head 4 -  Class Child Ref Key: 0
.head 4 -  Class ChildKey: 0
.head 4 -  Class:
.head 4 -  Property Template:
...
.head 5 +  On UM_ParentSize
.head 6 -  Call SalGetWindowLoc ( hWndItem , nZ, nY )
.head 6 -  Call SalSetWindowLoc ( hWndItem, wParam-1.4, nY )
.head 5 +  On WM_KeyDown
.head 6 -  Call SalSendMsg( tbl, WM_KeyDown, wParam, lParam )
.head 3 +  Pushbutton: pbPageDn
.head 4 -  Class Child Ref Key: 0
.head 4 -  Class ChildKey: 0
.head 4 -  Class: cTBarButton
.head 4 -  Property Template:
...
.head 4 +  Message Actions
.head 5 +  On SAM_Create
.head 6 -  Set strTip = 'PgDn'
.head 6 +  If bPages
.head 7 -  Call SalShowWindow(hWndItem)
.head 6 +  Else
.head 7 -  Call SalHideWindow(hWndItem)
.head 5 +  On SAM_Click
.head 6 -  Set dfPage=dfPage+1
.head 6 -  Set nPage=dfPage
.head 6 -  Call SalSendMsg( tbl, UM_Populate, 0, 0 )
.head 5 +  On UM_ParentSize
.head 6 -  Call SalGetWindowLoc ( hWndItem , nZ, nY )
.head 6 -  Call SalSetWindowLoc ( hWndItem, wParam-0.9, nY )
.head 5 +  On WM_KeyDown
.head 6 -  Call SalSendMsg( tbl, WM_KeyDown, wParam, lParam )
.head 2 +  Functions
...
.head 2 +  Message Actions
.head 3 +  On SAM_Create
.head 4 -  Set bPages=IifN(GetGlobalOption('VVPAGE')='Y',1,0)
.head 4 -  Call PrepareWindowEx(hWndForm)
.head 4 -  Call SalSendMsgToChildren(hWndForm, UM_QueryLabelText, 0, 0)
...
.head 6 -  Call SalGetWindowLoc ( hWndItem , nZ, nY )
.head 6 -  Call SalGetWindowSize ( hWndItem , nX, nY )
.head 6 -  Call SalSetWindowSize ( hWndItem , wParam-0.4-nZ, nY )
.head 3 +  Child Table: tbl
.head 4 -  Class Child Ref Key: 0
.head 4 -  Class ChildKey: 0
.head 4 -  Class:
...
.head 6 -  Call SalTblPopulate( tbl, hSql(),
 T("SELECT mfo, nb FROM banks WHERE mfo<>:strOurMfo and blk<>4 "||strCond||" INTO :tbl.colMFO, :tbl.colName  ORDER BY mfo " ),   TBL_FillAll )
...
.head 6 -  Call SalWaitCursor( TRUE )
.head 6 +  If SalIsNull( dfSearchStr )
.head 7 -  Call SalTblPopulate( tbl, hSql(), 
"SELECT np, ROWID FROM np
   INTO :tbl.colText, :tbl.colRowid  
  WHERE id=:nId or id=0 or id IS NULL ORDER BY 1", TBL_FillNormal )
.head 6 +  Else
.head 7 -  Call SalTblPopulate( tbl, hSql(), 
"SELECT np, ROWID FROM np
   INTO :tbl.colText, :tbl.colRowid 
  WHERE np LIKE \'"|| StrMatches2Like( dfSearchStr ) ||"\' 
    AND (id=:nId or id=0 or id IS NULL) ORDER BY 1", TBL_FillNormal )
.head 6 -  Call SalWaitCursor( FALSE )
.head 5 +  On UM_ParentSize
.head 6 -  Call SalGetWindowLoc ( hWndItem , nZ, nY )
.head 6 -  Call SalGetWindowSize ( hWndItem , nX, nY )
...
"SELECT np, ROWID FROM np
   INTO :tbl.colText, :tbl.colRowid 
  WHERE id=:nId or id=0 or id IS NULL ORDER BY 1", TBL_FillAll )
.head 6 -  Call VisTblAutoSizeColumn( tbl, hWndNULL )
.head 5 +  On SAM_KillFocus
.head 6 -  Call SalMapEnterToTab(TRUE)
.head 5 +  On SAM_SetFocus
...
.head 4 +  Message Actions
.head 5 +  On SAM_Click
.head 6 -  Call SqlPrepare( hSql(), 'DELETE FROM np WHERE ROWID = :tbl.colRowid' )
.head 6 -  Call SalTblDeleteSelected( tbl, hSql() )
.head 6 -  Call SqlCommit( hSql() )
.head 5 +  On UM_ParentSize
.head 6 -  Call SalSetWindowLoc ( hWndItem, wParam/2-0.7, 0.7*lParam+0.6 )
...
.head 3 -  ! Сума документа
.head 3 +  Data Field: bgSum1
.head 4 -  Class Child Ref Key: 0
.head 4 -  Class ChildKey: 0
.head 4 -  Class:
.head 4 -  Property Template:
...
.head 4 +  Message Actions
.head 5 +  On SAM_Create
.head 6 -  Set bgSum1=' '||CurrentLangTable.GetAtomTitle('dlgAsk2Pay.bgSum')
...
.head 4 +  Message Actions
.head 5 +  On SAM_Create
.head 6 +  If UseCustomFonts() 
.head 7 -  Call SalFontSet(hWndItem, GetFontName(GetFontTables()), GetFontSize(GetFontTables()), GetFontEnhancement(GetFontTables()))
.head 3 +  Picture: pic1
.head 4 -  Class Child Ref Key: 0
.head 4 -  Class ChildKey: 0
.head 4 -  Class:
...
.head 4 +  Message Actions
.head 5 +  On SAM_Create
.head 6 -  Set bgSum2=' '||CurrentLangTable.GetAtomTitle('dlgAsk2Pay.bgSum2')
...
"SELECT   ostb,  ostc,  ostx,  lim,  pap,  blkd,  blkk
   INTO :nOstB,:nOstC,:nOstX,:nLim,:nPap,:nBlkD,:nBlkK
   FROM accounts WHERE nls = :sNls AND kv = :nKv AND dazs IS NULL")
.head 5 +  If SqlFetchNext( hSql(), nFetchRes )
.head 6 -  If sOpt='1'
.head 6 -  Set nOst = IifN(bFaktPlan,nOstC,nOstB)
.head 6 +  If nDlta<0 and nBlkD>0 or nDlta>0 and nBlkK>0
.head 7 -  ! Set sTmp='Заблоковано на '||IifS(nDlta<0,'ДЕБЕТ','КРЕДИТ')
.head 7 +  If nDlta<0
.head 8 -  Set sTmp=CurrentLangTable.GetAtomTitle('dlgAsk2Pay.NoDeb')
.head 7 +  Else
.head 8 -  Set sTmp=CurrentLangTable.GetAtomTitle('dlgAsk2Pay.NoKre')
.head 6 +  Else If sOpt='1' and ( nPap = 1 and nDlta > 0 and nOst + nLim + nDlta > 0
                    or nPap = 2 and nDlta < 0 and nOst + nLim + nDlta < 0 ) or
        sOpt='2' and ( nPap = 1 and nDlta > 0 and nOstB + nLim + nDlta > 0
                    or nPap = 2 and nDlta < 0 and nOstB + nLim + nDlta < 0 
                    or nPap = 1 and nDlta > 0 and nOstC + nLim + nDlta > 0
                    or nPap = 2 and nDlta < 0 and nOstC + nLim + nDlta < 0 )
.head 7 -  ! Set sTmp='Недостатньо коштів для здійснення операції'
.head 7 -  Set sTmp=CurrentLangTable.GetAtomTitle('dlgAsk2Pay.NoMoni')
.head 6 +  Else If sOpt='1' and ( nOstX != NUMBER_Null and nPap = 1 and nDlta < 0 and nOst + nDlta - nOstX < 0
                    or nOstX != NUMBER_Null and nPap = 2 and nDlta > 0 and nOst + nDlta - nOstX > 0 ) or
        sOpt='2' and ( nOstX != NUMBER_Null and nPap = 1 and nDlta < 0 and nOstB + nDlta - nOstX < 0
                    or nOstX != NUMBER_Null and nPap = 2 and nDlta > 0 and nOstB + nDlta - nOstX > 0 
                    or nOstX != NUMBER_Null and nPap = 1 and nDlta < 0 and nOstC + nDlta - nOstX < 0
                    or nOstX != NUMBER_Null and nPap = 2 and nDlta > 0 and nOstC + nDlta - nOstX > 0 )
.head 7 -  ! Set sTmp='Перевищення максимального залишку'
.head 7 -  Set sTmp=CurrentLangTable.GetAtomTitle('dlgAsk2Pay.TooMuch')
.head 5 +  If sTmp
.head 6 -  Set sTmp = CurrentLangTable.GetAtomTitle('dlgAsk2Pay.Acnt')||' '||
                 sNls||'('||Str(nKv)||'):'||PutCrLf()||sTmp||'.'
.head 5 -  Return sTmp
.head 2 +  Window Parameters
.head 3 -  String: strSumPr
.head 3 -  Boolean: bFaktPlan
.head 3 -  Number: nDk
.head 3 -  String: sNlsA
.head 3 -  Number: nKvA
.head 3 -  Number: nSA
...
.head 3 -  Number: nX
.head 3 -  Number: nY
.head 3 -  String: sOpt   ! Режим вывода сообщения об ошибке
.head 2 +  Message Actions
.head 3 +  On SAM_Create
.head 4 -  Call SalSendMsgToChildren( hWndForm, UM_QueryLabelText, 0, 0 )
.head 4 -  Call SalSetWindowText(hWndForm, CurrentLangTable.GetAtomTitle('dlgAsk2Pay.cTWndTitle'))
.head 4 -  Set ml1 = strSumPr
.head 4 -  Set sOpt=GetGlobalOption('WARNPAY')
.head 4 +  If sOpt and (nDk=0 or nDk=1)
.head 5 -  Set ml2 = IsWarning(bFaktPlan,sNlsA,nKvA,IifN(nDk=0,nSA,-nSA))
.head 5 +  If nFli
.head 6 -  Set ml2 = IifS(SalStrLength(ml2)>0,ml2||PutCrLf()||PutCrLf(),'')||
          IsWarning(bFaktPlan,sNlsB,nKvB,IifN(nDk=0,-nSB,nSB))
.head 4 -  Call SalMapEnterToTab( FALSE )
.head 4 -  Call SalSendMsg( hWndForm, UM_ObjActivate, 0, 0 )
.head 3 +  On SAM_Destroy
.head 4 -  Call SalMapEnterToTab( TRUE )
.head 3 +  On UM_ObjActivate
.head 4 +  If SalStrLength(ml2)=0
.head 5 -  Call SalGetWindowSize ( hWndForm , nWinW, nWinH )
.head 5 -  Call SalGetWindowSize ( pic2 , nX, nY )
.head 5 -  Call SalSetWindowSize ( hWndForm , nWinW, nWinH-nY-0.16 )
.head 5 -  Call SalHideWindow(bgSum2)
.head 4 -  Call SalInvalidateWindow ( hWndForm )
.head 4 -  Call PrepareWindowEx( hWndForm )
.head 1 +  Dialog Box: dlgTagBrowse   ! Бровзер допреквизитов
.head 2 -  Class:
.head 2 -  Property Template:
.head 2 -  Class DLL Name:
.head 2 -  Title: Справочник контрагентов
...
.head 6 -  Visible? Yes
.head 6 -  Editable? No
.head 6 -  Maximum Data Length: 1024
.head 6 -  Data Type: String
.head 6 -  Justify: Left
.head 6 -  Width:  30.0"
.head 6 -  Width Editable? Yes
...
.head 6 -  Call SalSendMsgToChildren( hWndForm, UM_QueryLabelText, 0, 0 )
.head 6 -  Call SalTblSetTableFlags( hWndItem, TBL_Flag_SingleSelection, TRUE )
.head 6 -  Call SalTblPopulate( hWndItem, hSql(), strCond, TBL_FillAll)
.head 6 -  Call VisTblAutoSizeColumn( hWndItem, hWndNULL )
.head 5 +  On SAM_DoubleClick
...
.head 4 -  Call SalCenterWindow(hWndForm)
.head 4 +  If strBrowserMask
.head 5 -  Set strCond = SalStrMidX(strSelect,7,SalStrScan(strSelect,' FROM ')-7)
.head 5 -  !
.head 5 -  Set sTmp = strCond
.head 5 -  Set i=0
.head 5 -  Set j=0
.head 5 +  Loop
.head 6 -  Set x = SalStrLop(sTmp)
.head 6 +  If x=0
.head 7 -  Break
.head 6 +  Else If x=40 ! '('
.head 7 -  Set j=j+1
...
.head 8 -  Break
.head 6 -  Set i=i+1
.head 5 -  Set strCond = SalStrReplaceX ( strCond, i, 1, '||' )
.head 5 -  ! Call Debug('2 '||strCond)
.head 5 -  Set strCond = IifS(SalStrScan(strSelect,' WHERE ')<0,' WHERE ',' AND ')||'UPPER('||strCond||')'||
            " LIKE '%"||StrMatches2Like(strBrowserMask)||"%'"
.head 5 -  ! Call Debug('3 '||strCond)
.head 5 -  Set i=SalStrScan(strSelect,' ORDER ')
.head 5 -  Set strCond = SalStrReplaceX ( strSelect, IifN(i>=0,i,SalStrLength(strSelect)), 0, strCond )
.head 5 -  ! Call Debug('4 '||strCond)
.head 4 +  Else
.head 5 -  Set strCond=strSelect
.head 3 +  On SAM_CreateComplete
...
.head 3 +  On UM_NoFontChange	! Отказ от смены фонта
.head 4 -  Return TRUE
