...
begin
  ...
  select url into l_url from zay_recipients;
...
if gZAYMODE = 2 then
  ...
  select z.url into l_url from zay_recipients z;
...
begin
 -- трасса входящих параметров
  ...
  if gZAYMODE = 1 then
     ...
     begin
        select 1 into l_zay_source from zayavka where id = p_id;
     exception when no_data_found then
        begin
           select 2 into l_zay_source from zayavka_ru where id = p_id;
        exception when no_data_found then
  ...
  if l_zay_source = 1 then
    ...
    -- первоначальной заявке меняем sos
    update zayavka_ru set sos = -1 where id = p_id;
    -- позвать web-сервис для отправки запроса в РУ на разбиение заявки
    select url, to_char(mfo) into l_url, l_mfo from zay_recipients where to_char(mfo) = (select mfo from zayavka_ru where id = p_id);
    ...
    select req_id into l_req_id from zayavka_ru where id = p_id;
...
begin
    ...
    if p_bank_name is not null then l_upd := l_upd||', bank_name = '''||p_bank_name||''''; end if;
  ...
  if (p_kurs_f is not null and p_vdate is not null) then
     if l_priorverify = 1  then
        if gZAYMODE = 0 then
           ...
           -- Определяем, чья заявка: ЦА или РУ
           begin
              select 1 into i from zayavka where id = p_id;
              update zayavka
                 set kurs_f = p_kurs_f,
                     sos    = p_sos,
                     vdate  = p_vdate,
           ...
           exception when no_data_found then
              begin
                 select 1 into i from zayavka_ru where id = p_id;
                 update zayavka_ru
                    set kurs_f = p_kurs_f,
                        sos    = p_sos,
                        vdate  = p_vdate,
...
begin
  ...
  if gZAYMODE = 0 then
     ...
     -- Определяем, чья заявка: ЦА или РУ
     begin
        select id into l_reqid from zayavka where id = p_id;
        update zayavka
           set sos  = 1,
               datz = p_datz
         where id  = p_id
     ...
     exception when no_data_found then
        begin
           ...
           select req_id, mfo, viza into l_reqid, l_mfo, l_viza from zayavka_ru where id = p_id;
           update zayavka_ru
              set sos  = 1,
                  datz = p_datz
            where id  = p_id
...
begin
  ...
  if gZAYMODE = 2 then
    ...
    dbms_xmlparser.freeparser(l_parser);
    DBMS_XMLDOM.freeDocument(l_doc);
  select url,mfo into l_url,l_mfo from zay_recipients;
  if l_par = 1 then
    l_type := 1;
...
is
begin
  for c in (select r.url from zay_recipients r) loop
