...
, p_img_tp         in     customer_images.type_img%type
, p_image          in     customer_images.image%type
);
...
procedure check_attr_foropenacc (p_rnk in number, p_msg out varchar2);
  ...
  --
  -- @p_field_code -  код поля карточки клиента
  --
  ---------------------------------------------------
...
$if KL_PARAMS.SBER $then
  ...
  begin
    ...
    select c.CUSTTYPE, c.SED
      ...
      from CUSTOMER c
     where c.RNK = p_rnk
  /*   and (bc = 0 or bc is null)
       and date_off is null */
    ;

    bars_audit.trace( 'kl.get_ebk_cust_tp: rnk=%s, custtype=%s, sed=%s.', g_modcode, to_char(p_rnk), to_char(l_custtype), l_sed );

    return case
             when ( l_custtype = 2 )
             then 'L'
             when ( l_custtype = 3 and l_sed = '91  ' )
             then 'P'
             when ( l_custtype = 3 and l_sed = '00  ' )
             then 'I'
             else null
           end;
  ...
  begin
    ...
    l_cust_tp := get_ebk_cust_tp( p_rnk );
  ...
  then
     ...
     ELSE
        ...
        IF sql%rowcount = 0 THEN
           ...
           VALUES (Rnk_, Tag_, trim(Val_), Otd_);
...
begin
  ...
  or l_flag_visa = 1 and not is_customer_visa(Rnk_) then
     l_id := case when Id_ is null then bars_sqnc.get_nextval('S_CORPS_ACC') else Id_ end;
     begin
        -- Добавление
...
begin
  ...
  begin
    Insert into BARS.CUSTOMER_IMAGES
      ( RNK, TYPE_IMG, DATE_IMG, IMAGE )
    Values
     ( p_rnk, p_img_tp, sysdate, p_image );
  exception
    when DUP_VAL_ON_INDEX then
      update BARS.CUSTOMER_IMAGES
         set DATE_IMG = sysdate
           , IMAGE    = p_image
       where RNK      = p_rnk
         and TYPE_IMG = p_img_tp;
  end;
...
begin
  ...
  then
    ...
    id_ := bars_sqnc.get_nextval('S_CUSTOMER_BIN_DATA');
