...
---------------------------------------
function Z8 ( p_nd cc_deal.nd%type) return int  IS -- проверка на своевременность выноса на просрочку тела
  ll cc_lim%rowtype;  PLAN_sp number;  FAKT_sp number;
begin

  begin select * into ll from cc_lim where nd= p_nd and fdat = (select max(fdat) from cc_lim where nd = p_nd and fdat <gl.bdate) ;
        select LEAST ( ostc+ll.lim2, 0)   into  PLAN_sp from accounts where acc = ll.acc;
  EXCEPTION WHEN NO_DATA_FOUND THEN return 0; -- raise_application_error(-(20203), ' Не знайдено попереднього ГПК для КД реф=' || p_nd );
  end;

  select nvl( sum (a.ostc),0) into FAKT_SP from accounts a, nd_acc n where  a.acc= n.acc and n.nd = p_nd and tip = 'SP ';
  If PLAN_sp <> FAKT_sp then  RETURN 0;    --raise_application_error(-(20203), ' Невідповідність PLAN_SP та FAKT_SP для КД реф=' || p_nd );  
  end if;
  RETURN 1;
end Z8;
-----------------

function Day_PL ( p_nd cc_deal.nd%type) return int is   -- определение K2_ number    ;  -- Платежный день
 l_dat date ;
 l_K2  int  ; 
begin
  begin select i.s into l_k2 from int_accn i, accounts a, nd_acc n 
        where n.nd=p_ND and n.acc=a.acc and a.tip='LIM' and a.acc=i.acc and i.id=0 and i.s >0 and i.s <= 31 ;
  EXCEPTION WHEN NO_DATA_FOUND THEN 
        Select    max(fdat) into l_dat from cc_lim where nd = p_nd and fdat < gl.BD ;
        If l_dat  is null then   Select NVL( min(fdat) , gl.BD)  into l_dat from cc_lim where nd = p_nd and fdat > gl.BD ;  end if;
...
begin
  ...
  If cck_dpk.K0_ = 1 then ---------------------------------- 1-Ануитет
     ...
     If l_adat < Least( (p_datn-1), l_STP_DAT) then  
        CCK.INT_METR_A( l_Accc, l_Acc, 0, l_adat, Least( (p_datn-1), l_STP_DAT ),  l_Int, Null, 0 ) ; 
...
--cck_dpk.K3_ :=  1      ;  -- с сохранением суммы одного платежа
  ...
  begin
     ...
     from accounts a8, nd_acc n8   where n8.nd = p_nd and n8.acc= a8.acc and a8.tip ='LIM'  and a8.ostc = a8.ostb and a8.ostb < 0  ;

     If datk_ < gl.bdate then raise_application_error(-(20203), ' КД реф=' || p_nd ||' 100% просрочено ' );  end if ;

  EXCEPTION WHEN NO_DATA_FOUND THEN raise_application_error(-(20203), ' Не знайдено рах.8999 для КД реф=' || p_nd ||', або план НЕ= факт' );
  end;

  If Lim2_ = 0 then    raise_application_error(-(20203), ' Не визначено сумму для ГПК КД реф=' || p_nd , TRUE);   end if;

  begin    select * into II from int_accn where acc= acc8_ and id = 0;
  EXCEPTION WHEN NO_DATA_FOUND THEN raise_application_error(-(20203), ' Не знайдено проц.картка(id=0) для рах.8999_' || p_nd );
  end;
...
begin
  ...
  begin   -- Z2k_-  Норм.комиссия положит число
    select -nvl(sum(a.ostb),0) into Z2K_ from accounts a, nd_acc n where  n.ACC = a.acc and n.nd = p_nd and a.tip='SK0' ;
     ...
     select  i.* into II 
    from int_accn i, nd_acc n, accounts ss where n.nd=p_ND and N.acc=ss.acc and ss.tip= 'SS ' and i.id=0 and i.acc=ss.ACC and ss.ostb<0;
  ...
  If ii.STP_DAT > II.acr_dat then 
     If p_vid = 4 then -- для ануитета  
        begin select  (lb.sumo-lb.sumg)  *  ( ii.STP_DAT - II.acr_dat ) /  (lb.fdat-lp.fdat)         
              into Zn_
              from cc_lim lb, (select max(fdat) fdat from cc_lim where nd=p_ND and fdat <= ii.STP_DAT ) lp
              where lb.nd   = p_ND and lb.fdat > lp.fdat  
                and lb.fdat = (select min(fdat) from cc_lim where nd=p_ND and fdat > ii.STP_DAT )  ;
...
begin
  ...
  begin select -a.ostx + a.ostb into l_Del from accounts a, nd_acc n  where n.nd = p_ND and n.acc = a.acc and a.tip ='LIM' ;
  EXCEPTION WHEN NO_DATA_FOUND THEN  raise_application_error(-(20203), ' КД '|| p_nd|| ' Не знайдено рах.LIM*');
  end;

  If l_Del < 0 then  l_Del := - l_Del;
     begin select sumg  into l_SumG  from cc_lim  where nd = p_ND and fdat = (select min(fdat) from cc_lim where nd=p_ND and fdat >= gl.BDate); 
     EXCEPTION WHEN NO_DATA_FOUND THEN  l_SumG := 0;
     end ;
...
begin
  begin
    select d.cc_id, d.sdate, c.okpo, SG.nls, substr(SG.nms,1,38), sg.kv,  SS.nls, substr(SS.nms,1,38), ss.ostc, ss.ostb,
           substr('Дострокове погаш КД № '|| d.cc_id || ' вiд ' || to_char(d.sdate,'dd.mm.yyyy') ||
                  '. ( Зі збереженням ' || Decode (CCK_DPK.k3_ , 1, 'суми 1-го платежу','кiнцевого термiну') || ' )' ,1, 160)
    into   dd.cc_id, dd.sdate, RR1.id_a,RR1.nlsa, RR1.nam_a, RR1.kv,  RR1.nlsb, RR1.nam_b, ostc_,ostb_, RR1.nazn
    from accounts SG, nd_acc NSG,  accounts SS, nd_acc NSS, customer c, cc_deal  d
    where d.nd  = p_ND and d.rnk  = c.rnk
      and NSG.nd= d.ND and NSG.acc= SG.acc and SG.acc = p_acc2620  and SG.dazs is null
      and NSS.nd= d.ND and NSS.acc= SS.acc and SS.tip = 'SS '      and SS.dazs is null    ;

    if ostc_ <> ostb_          then bars_error.raise_nerror( p_errmod=> 'CCK', p_errname=> 'PLAN#FAKT'   , p_param1=> to_char(p_nd), p_param2=> RR1.nlsb    ) ;  end if;
  EXCEPTION WHEN NO_DATA_FOUND THEN  raise_application_error(-(20203), 'НЕ знайдено рах. p_acc2620='|| p_acc2620 || ' або SS по КД ='||p_ND  );
  end;

  --- для доначис и пог %
  begin  select SN.nls,   SD.nls, substr(SN.nms,1,38),substr(SD.nms,1,38), SN.KV,  SD.KV , -SN.ostb   
         into RR2.nlsa, RR2.nlsb,       RR2.nam_a,          RR2.nam_b,    RR2.kv, RR2.kv2, RR2.s
         from accounts SN,  accounts SD    where SN.acc = II.acra and SD.acc = II.acrb  and SN.dazs is null  and SD.dazs is null ;
  EXCEPTION WHEN NO_DATA_FOUND THEN  raise_application_error(-(20203), 'НЕ знайдено рах. з проц.картки по К ' );
  end;
  ...
  If II.acr_dat < NVL(II.stp_dat, (gl.bdate -1) )  and ZN_ > 0 then
     ...
     -- доначислить %  ZN
     gl.ref (RR2.REF);
     gl.in_doc3(ref_  => RR2.REF ,  tt_   => '%%1'  , vob_  => 6 ,      nd_ => substr(to_char(RR2.REF),-10), pdat_  => SYSDATE, vdat_  => gl.BDATE, dk_ => 1 ,
               kv_    => RR2.kv  ,  s_    => ZN_    , kv2_  => RR2.kv , s2_ => ZN_,      sk_    => null    , data_ => gl.BDATE, datp_ => gl.bdate ,
               nam_a_ =>RR2.nam_a,  nlsa_=> RR2.nlsa, mfoa_ => gl.aMfo, nam_b_=>RR2.nam_b, nlsb_=> RR2.nlsb, mfob_ => gl.aMfo ,
               nazn_  => Substr( 'Донарахування вiдсоткiв з '||
                         to_char (  II.acr_dat+1 , 'dd.mm.yyyy' ) || ' по ' ||
                         to_char ( ii.STP_dat, 'dd.mm.yyyy' ) || ' в зв`язку з достроковим погаш КД № '|| dd.cc_id || ' вiд ' ||
                         to_char (  dd.sdate     , 'dd.mm.yyyy' ) , 1, 160),
               d_rec_ => null    ,   id_a_ => RR1.id_a,    id_b_ => gl.aOkpo,
               id_o_  => null    ,   sign_ => null ,
               sos_   => 1       ,   prty_ => null , uid_  => null) ;

     gl.payv( 0, RR2.REF, gl.bDATE, '%%1',1,   RR2.kv ,  RR2.nlsa,  ZN_ ,  RR2.kv2,  RR2.nlsb,  ZN_  );
 
     -- проставить дату, по кот % уже начислены
     update int_accn   set acr_dat = NVL(II.stp_dat, (gl.bdate-1) ) 
               where id = 0 and acc in  (select a.acc from nd_acc n, accounts a where n.nd = p_nd and n.acc=a.acc and tip in ('SS ','SP ') );
  ...
  If  RR1.nlsa  like '2625%' then l_tt := cck_dpk.TT_W4 ;    rr1.s := CCK_DPK.K1_ + cck_dpk.Z2N_ + cck_dpk.Z2K_ + cck_dpk.Z1_  ;  -- 29.05.2015
  else                            l_tt := 'ASD'         ;    rr1.s := CCK_DPK.K1_ + cck_dpk.Z2N_ + cck_dpk.Z2K_                ;
  end if; 
/*
...
*/
  ...
  If rr1.S <= 0 then Return; end if;

  gl.ref (RR1.REF);
  gl.in_doc3(ref_  => RR1.REF,  tt_   => l_tt     , vob_ => 6       , nd_   => substr(dd.cc_id,1,10)      , pdat_=> SYSDATE , vdat_ => gl.BDATE , 
             dk_   => 1      , kv_    => RR1.kv   , s_   => rr1.s   , kv2_  => RR1.kv   , s2_  => rr1.s   , sk_  => null    , data_ => gl.BDATE , datp_=> gl.bdate ,
            nam_a_ => RR1.nam_a, nlsa_=> RR1.nlsa , mfoa_=> gl.aMfo , nam_b_=> RR1.nam_b, nlsb_=> RR1.nlsb, mfob_=> gl.aMfo , nazn_ => RR1.nazn ,
            d_rec_ => null   , id_a_  => RR1.id_a , id_b_=> RR1.id_a, id_o_ => null,      sign_=> null    , sos_ => 1 ,       prty_ => null, uid_=>null) ;
  insert into operw ( REF, TAG, VALUE ) values (RR1.REF, 'ND   ', to_char(p_ND                   ) ); -- Дата дострокового погашення DD.MM.YYYY
  insert into operw ( REF, TAG, VALUE ) values (RR1.REF, 'MDATE', to_char(gl.bdate, 'dd.mm.yyyy' ) ); -- Реф KД

  If l_tt = cck_dpk.TT_W4 then 
     nls_2924 := bpk_get_transit('20', RR1.nlsb, RR1.nlsa, RR1.kv ) ;  
     gl.payv (0, RR1.REF, gl.BDATE, l_tt, 1, RR1.kv, RR1.nlsa, rr1.s      , RR1.kv, nls_2924, rr1.s       );  ---------- итоговая проводка

     iF cck_dpk.Z1_ > 0 THEN  -- 29.05.2015 ПРОСРОЧКИ в том же реф =  -NVL(sum(a.ostb),0) into cck_dpk.Z1_  -- положительное число

        for x in (select a.* from accounts a, nd_acc n 
                  where n.nd = p_ND and n.acc = a.acc and ostb < 0  and a.kv = RR1.kv  
                    and ( a.tip in ( 'SP ','SL ', 'SPN', 'SK9','SLN','SLK')    OR   a.tip in ('SN8') and  RR1.kv = gl.baseval     )
                  ) 
        loop 
           if    x.TIP='SPN' then rr1.d_rec := 'Погашення просрочених вiдсоткiв' ;
           elsIF x.TIP='SK9' then rr1.d_rec := 'Погашення просроченої комiсiї'   ;
           elsIF x.TIP='SP ' then rr1.d_rec := 'Погашення просроченого осн.боргу';
           elsIF x.TIP='SL ' then rr1.d_rec := 'Погашення сумнiвного осн.боргу'  ;
           elsIF x.TIP='SLN' then rr1.d_rec := 'Погашення сумнiвного проц.боргу' ;
           elsIF x.TIP='SLK' then rr1.d_rec := 'Погашення сумнiвного комiс.боргу';
           elsIF x.TIP='SN8' then rr1.d_rec := 'Погашення пенi'                  ;  
              gl.payv (0, RR1.REF, GL.BDATE, 'ASG', 1, x.kv, cck_dpk.NLS_8006, -x.ostb, x.kv, x.nls, -x.ostb );     -- сворачиваем пеню 
              x.nls :=  cck_dpk.NLS_6397;
           end if;

           gl.payv (0, RR1.REF, gl.BDATE, l_tt, 1, RR1.kv, nls_2924, -x.ostb , RR1.kv, x.nls, -x.ostb );  
           update opldok set txt = rr1.d_rec where ref = RR1.REF and stmt = gl.aStmt;
  ...
  If CCK_DPK.K1_  > 0 then  ---- частная проводка по 2203

     l_Z3 := 0 ; --- -- плановая сумма пог по телу на "сегодня"? это не есть досрочное погаш

     If cck_dpk.Z3_ > 0 then 
        begin select cck_dpk.Z3_ into l_Z3 from dual where gl.BD in (select fdat from cc_lim where nd = p_ND);
        EXCEPTION WHEN NO_DATA_FOUND THEN null;
        end;
     end if;

     If l_Z3 > 0 then
        gl.payv (0, RR1.REF, gl.BDATE, l_tt, 1, RR1.kv, nls_2924, l_Z3  , RR1.kv, RR1.nlsb, l_Z3   );  
        update opldok set txt = 'Z3. Сума для планового погашення' where ref = RR1.REF and stmt = gl.aStmt;
     end if;

     If CCK_DPK.K1_ >  l_Z3 then
        gl.payv (0, RR1.REF, gl.BDATE, l_tt, 1, RR1.kv, nls_2924, (CCK_DPK.K1_ - l_Z3) , RR1.kv, RR1.nlsb, (CCK_DPK.K1_ - l_Z3)  );  
        update opldok set txt = 'K1. Сума для дострокового погашення' where ref = RR1.REF and stmt = gl.aStmt;
     end if;

  end if; 

  If CCK_DPK.Z2N_ > 0 then  ---- частная проводка по 2208
     gl.payv (0, RR1.REF, gl.BDATE, l_tt, 1, RR1.kv, nls_2924, CCK_DPK.Z2N_, RR1.kv, RR2.nlsa, CCK_DPK.Z2N_ );  
     update opldok set txt = 'Z2N_. погасить все %%' where ref = RR1.REF and stmt = gl.aStmt;
  end if; 

  If CCK_DPK.Z2K_ > 0 then   ---- частная проводка по 3578
     begin select Sk.nls, Sk.KV, -Sk.ostb into RR2.nlsa, RR2.kv, RR2.s          from accounts Sk, nd_acc n 
           where Sk.acc = n.acc and sk.tip ='SK0' and sk.ostb <0 and n.nd = p_ND and sk.kv = rr1.kv and rownum=1;
     EXCEPTION WHEN NO_DATA_FOUND THEN null; 
         raise_application_error(-(20203), 'НЕ знайдено рах. з tip = SK0'  );
     end;
     gl.payv (0, RR1.REF, gl.BDATE, l_tt, 1, RR1.kv, RR1.nlsa, CCK_DPK.Z2K_, RR1.kv, RR2.nlsa, CCK_DPK.Z2K_ );       
     update opldok set txt = 'Z2K_ погасить все комис' where ref = RR1.REF and stmt = gl.aStmt;
  End if;
...
begin
  ...
  --cck_app.set_nd_txt ( P_ND, 'CCRNG',  '10' ); --COBUSUPABS-6420
...
begin
  ...
  If p_ref is not null then   p_back_dok (p_ref, 5, null, l_p1, l_p2);  p_REF := null ;  end if;  -- откат проводок 
end MODI_RET_EX ;
-------------------
procedure RET_GPK ( p_Nd   IN cc_deal.nd%type,
                    p_REF  IN oper.ref%type ,
...
begin
  ...
  CCK_DPK.NLS_6397 :=  nbs_ob22_bra('6397', '01',  sys_context('bars_context','user_branch') ) ;
