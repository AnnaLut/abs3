...
/* Версия 1.0 10-01-2018
   Визнаяення непогашенного залишку по рахунку
   -------------------------------------
*/
...
begin
   l_dat31 := Dat_last_work (p_dat - 1);  -- последний рабочий день месяца
                  ...
                  if sql%rowcount=0 then
                     ...
                     exception when others then
                       if SQLCODE = -00001 then
                          raise_application_error(-20000, 'ND_KOL_otc dubl РНК'|| p_rnk || ' ND = '|| p_nd || ' Дата= ' || p.fdat);
                       else raise;
                       end if;
                     end;
                  end if;
