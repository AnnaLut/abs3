...
COMMENT ON TABLE BARSUPL.UPL_STATS IS 'Статистика выгрузки';
COMMENT ON COLUMN BARSUPL.UPL_STATS.PARENT_ID IS 'код родителя(код выгружаемой группы)из таблицы upl_stats.id только для записи - файл';
COMMENT ON COLUMN BARSUPL.UPL_STATS.FILE_NAME IS 'Имя выгружаемого файла ( для группы -  имя архива)';
...
COMMENT ON COLUMN BARSUPL.UPL_STATS.UPL_BANKDATE IS 'Банковская дата выгрузки';
COMMENT ON COLUMN BARSUPL.UPL_STATS.GROUP_ID IS 'Код группы файлов, если запускалди из группы';
COMMENT ON COLUMN BARSUPL.UPL_STATS.FILE_ID IS 'Код файла выгрузки';
COMMENT ON COLUMN BARSUPL.UPL_STATS.FILE_CODE IS '';
COMMENT ON COLUMN BARSUPL.UPL_STATS.SQL_ID IS '';
COMMENT ON COLUMN BARSUPL.UPL_STATS.START_TIME IS 'Дата+время начала выгрузки';
COMMENT ON COLUMN BARSUPL.UPL_STATS.STOP_TIME IS 'Дата+время окончания выгрузки';
COMMENT ON COLUMN BARSUPL.UPL_STATS.STATUS_ID IS 'Статус выгрузки';
COMMENT ON COLUMN BARSUPL.UPL_STATS.PARAMS IS 'Список параметров в виде param1=..., param2=....';
COMMENT ON COLUMN BARSUPL.UPL_STATS.UPL_ERRORS IS 'Ошибки, если выгрузка закончилдась неуспешно';
COMMENT ON COLUMN BARSUPL.UPL_STATS.ROWS_UPLOADED IS 'Кол-во строк при выгрузке';
...
begin   
 execute immediate '
  ALTER TABLE BARSUPL.UPL_STATS ADD CONSTRAINT FK_UPLSTATSPARENTID FOREIGN KEY (PARENT_ID)
	  REFERENCES BARSUPL.UPL_STATS (ID) ON DELETE CASCADE ENABLE';
exception when others then
