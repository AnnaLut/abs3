...
CREATE OR REPLACE PACKAGE BODY BARSAQ.DATA_IMPORT is
  ...
  -- global consts
  G_BODY_VERSION constant varchar2(64)  := 'version 1.95 16/01/2016';
  ...
  begin
	...
	if l_individual.id_serial is null then
  ...
  begin
    ...
    rpc_sync.sync_customer(p_kf, p_rnk);
    rpc_sync.sync_individual(p_kf, p_rnk);
    --
    rpc_sync.set_constraints_immediate();
    --
  exception
  ...
  begin
    ...
    -- синхронизируем схемы BANK и CORE в IBANK
    rpc_sync.sync_customer(p_kf, p_rnk);
    rpc_sync.sync_company(p_kf, p_rnk);
	--
    rpc_sync.set_constraints_immediate();
    --
  exception
    ...
    loop
        ...
        for cur_r in (
          select c.bank_id, c.acc_num,
		       ...
		       (select nvl(max(dat), d.pdat) from bars.oper_visa where ref=d.ref and status = 2 and groupid not in (80, 81, 30))  dat,
  ...
  begin
    ...
    begin
        ...
        --
	    insert
          into doc_curex_custcommissions(rnk, buy_commission, sell_commission, conv_commission, bank_id)
		select rnk, cc.kom, cc.kom2, cc.kom3, (select val from bars.params where par = 'GLB-MFO')
  ...
  begin
    ...
    begin
        ...
        insert
		  ...
		  select ce.id, ce.rnk, ce.dk, ce.kv_grp, ce.kv, ce.limit, ce.rate, ce.fix_sum, ce.date_on, ce.date_off, (select val from bars.params where par = 'GLB-MFO')
