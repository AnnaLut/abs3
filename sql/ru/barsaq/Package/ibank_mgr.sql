 ...
 PROMPT ===================================================================================== 
  ...
  --------------------------------------------------------------------------------
  -- recreate_streams - полное пересоздание процессов
  --
...
CREATE OR REPLACE PACKAGE BODY BARSAQ.IBANK_MGR is
  ...
  begin
    if p_flag = true then
       update SYNC_TABLES set sync_sql='begin barsaq.data_import.job_sync_doc_export_mod(:p_startdate); end;' where table_name='DOC_EXPORT';
    else
       update SYNC_TABLES set sync_sql='begin barsaq.data_import.job_sync_doc_export(:p_startdate); end;' where table_name='DOC_EXPORT';
    end if;
  end;
  ...
  -- create_dblink - создать dblink на базу IBANK
  --
  procedure create_dblink(p_dblink_name in varchar2, p_user_name in varchar2, p_user_pass in varchar2, p_ibank_host in varchar2, p_ibank_port in varchar2, p_ibank_service_name in varchar2) is
  begin
    begin
    ...
    case p_action
      ...
      when g_action_create then
        begin
          ...
          dbms_capture_adm.create_capture(queue_name => p_queue_name, capture_name => p_capture_name, start_scn => l_first_scn, first_scn => l_first_scn, checkpoint_retention_time => 10);
        exception
          when others then
