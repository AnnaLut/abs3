prompt создаем бэкап tmp_dpt_bonus_complex
begin 
...
/
prompt обновляем dpt_bonus_complex чтобы исключить обнуление веб-банкингом меньших бонусов
             ...
             (select dpt_id, bonus_value_fact from (
              select t1.dpt_id, bonus_value_fact, count(*)cc, min(t1.bonus_id) b, max(bonus_value_fact) over (order by t1.dpt_id) max_bonus_value_fact
                ...
                where t1.dpt_id = :p_dptid
                  ...
                  and T3.REQTYPE_ID = 1 -- Р·Р°РїРёС‚ РЅР° Р±РѕРЅСѓСЃРЅСѓ СЃС‚Р°РІРєСѓ
                  and T2.BONUS_CONFIRM = ''N'' -- РЅРµ РїРѕС‚СЂРµР±СѓС” РґРѕРґР°С‚РєРѕРІРѕРіРѕ РїС–РґС‚РІРµСЂРґР¶РµРЅРЅСЏ
