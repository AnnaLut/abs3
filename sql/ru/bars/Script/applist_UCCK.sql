declare
    l_app_code varchar2(10 char) := 'UCCK';
    l_app_name varchar2(300 char) := 'ÀÐÌ Êðåäèòè ÞÎ (WEB)';
    l_funcname varchar2(4000) := '/barsroot/CreditUi/NewCredit/?custtype=2';
    l_name varchar2(250 Byte) := '1) Ââåäåííÿ íîâîãî ÊÄ ÞÎ ';
    l_main_oper_code number;
    l_funcname1 varchar2(4000) := '/barsroot/ndi/referencebook/GetRefBookData/?tableName=V_CCK_NU'||'&'||'accessCode=1'||'&'||'sPar=[NSIFUNCTION][showDialogWindow=>false]';
    l_name1 varchar2(250 Byte) := '2) Ïîðòôåëü ÍÎÂÈÕ êðåäèò³â ÞÎ';
    l_main_oper_code1 number;
    l_funcname2 varchar2(4000) := '/barsroot/ndi/referencebook/GetRefBookData/?tableName=V_CCK_RU'||'&'||'accessCode=1'||'&'||'sPar=[NSIFUNCTION]';
    l_name2 varchar2(250 Byte) := '3) Ïîðòôåëü ÐÎÁÎ×ÈÕ êðåäèò³â ÞÎ';
