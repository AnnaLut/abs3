 ...
 ELSE
          ...
          THEN
             ...
             BEGIN
                INSERT INTO int_ratn (ACC,ID,BDAT,IR,BR,OP,IDU)
                     VALUES (:old.acc,l_id,gl.bd,l_ir,NULL,NULL,user_id); -- инд проц ставка = 0 с момента начала ареста
             EXCEPTION
       ...
       THEN
          ...
          THEN
           -- 1.a Вычитаем параметры действующей процентной карточки счета
              BEGIN
                SELECT ir0.id, ir0.ir, ir0.br, ir0.op
                  INTO l_id, l_ir, l_br, l_op
                  FROM int_ratn ir0
                 WHERE acc = :old.acc
                       AND bdat = (SELECT MAX (bdat)
                                     FROM (SELECT it.acc,
                                                  it.id,
                                                  it.bdat,
                                                  LEAD (it.bdat) OVER (ORDER BY it.acc) bdat2,
                                                  it.ir
                                             FROM int_ratn it, int_accn ia
                                            WHERE it.acc = ia.acc AND it.acc = :old.acc)
          ...
          -- 2. Восстанавливаем в процентной карточке значение ставки = предыдущее значение перед 0
              BEGIN
                 INSERT INTO int_ratn (ACC,ID,BDAT,IR,BR,OP,IDU)
                      VALUES (:old.acc,l_id,gl.bd+1,l_ir,l_br,l_op,user_id); -- инд проц ставка = 0 с момента начала ареста
              EXCEPTION
                 WHEN DUP_VAL_ON_INDEX
                 THEN delete from int_ratn where acc = :old.acc and bdat = gl.bd;
              END;
          END IF;
       END IF;
  END IF;
