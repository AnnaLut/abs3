...
DECLARE CURSOR c0 IS
        SELECT pdat,fdat FROM saldoa
        WHERE  acc=gl.val.a_acc AND fdat >= gl.bDATE
        FOR UPDATE OF ostf,pdat,dos,kos,trcn;

        CURSOR c1 IS
        SELECT ostf FROM saldoa
        WHERE acc = gl.val.a_acc AND fdat =
             (SELECT min(fdat) FROM saldoa
...
BEGIN
IF gl.fRCVR = 1 THEN   -- Recovery process - no arc
   RETURN;
END IF;
...
IF gl.val.b_ost > gl.val.a_ost THEN
...
OPEN c0;
LOOP FETCH c0 INTO pdat_, fdat_;
     EXIT WHEN c0%NOTFOUND;

     IF fdat_ = gl.bDATE THEN
        UPDATE saldoa
           SET dos  = dos  + dos_,  kos  = kos  + kos_,
               trcn = trcn + 1 WHERE CURRENT OF c0;
        trcn_ := 1;
     ELSE
        UPDATE saldoa
...
END LOOP;
CLOSE c0;
IF trcn_ IS NULL THEN
   BEGIN            -- Get previous history record
     SELECT ostf-dos+kos,fdat INTO ostf_,pdat_
       FROM saldoa
      WHERE acc = gl.val.a_acc AND fdat =
           (SELECT max(fdat) FROM saldoa
            WHERE fdat < gl.bDATE AND acc = gl.val.a_acc);
   EXCEPTION
   ...
   LOOP FETCH c1 INTO ostp_;
      IF c1%NOTFOUND THEN           -- No history found ->
         IF ostf_ IS NULL THEN
            ostf_ := gl.val.b_ost;  -- use current
         END IF;
      ELSE
         UPDATE saldoa SET pdat = gl.bDATE WHERE CURRENT OF c1;
         IF ostf_ IS NULL THEN
   ...
   INSERT INTO saldoa( acc, fdat, pdat, ostf, dos, kos, trcn
		, kf
   )
        VALUES(gl.val.a_acc,gl.bDATE,pdat_,ostf_,dos_,kos_,1
		, gl.aMFO
   );
END IF;
