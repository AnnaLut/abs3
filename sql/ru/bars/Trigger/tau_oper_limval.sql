...
for each row
 WHEN (
old.sos!=5 and new.sos=5 and new.tt in ('AA4','AA6','AAL','AAE','TMP','TM8', 'TIP', 'TI8', 'TM2')
      ) declare
  l_sq      number;
  l_vidd    number;
  l_serd    varchar2(32);
...
--end;
  ...
  begin
            select count(1) into l_exs from opldok o where  EXISTS (select * from accounts a
...
--    bars_audit.info('limval: kuro='||l_kuro);
   if :new.tt in ('AA4','AA6','AAL','AAE','TMP','TM8','TM2') then
      ...
      where  w.ref=:new.ref  and
             w.tag='PASPN'   and
             w1.ref=:new.ref and
             w1.tag= case when :new.tt in('TMP','TM8','TM2') then 'PASP ' else 'PASPV' end  and
             w2.ref=:new.ref and
             w2.tag='FIO  '  and
             w3.ref=:new.ref and
             w3.tag= case when :new.tt in('TMP','TM8','TM2') then 'MKURS' else 'KURS ' end  and
             w4.ref=:new.ref and
             w4.tag='DRDAY'  and
             p.name(+)=w1.value;
