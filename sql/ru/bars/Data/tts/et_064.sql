...
begin
  ...
  begin
    insert into tts(tt, name, dk, nlsm, kv, nlsk, kvk, nlss, nlsa, nlsb, mfob, flc, fli, flv, flr, s, s2, sk, proc, s3800, rang, flags, nazn)
    values ('K62', 'K62  Комiсiя за доставку готiвки "УКРПОШТI"', 1, '#(BRANCH_USR.GET_BRANCH_PARAM2(''3578_UKRPOSTA'',0))', 980, '#(nbs_ob22 (''6119'',''17''))', 980, null, null, null, null, 0, 0, 0, 0, 'case  WHEN #(KVA) = 980 THEN  F_TARIF(62, #(KVA),#(NLSA), #(S))  ELSE 0  END', null, null, null, null, null, '0000100000000000000000000000000000000100000000000000000000000000', null);
  exception
    when dup_val_on_index then 
      update tts
         set tt='K62', name='K62  Комiсiя за доставку готiвки "УКРПОШТI"', dk=1, nlsm='#(BRANCH_USR.GET_BRANCH_PARAM2(''3578_UKRPOSTA'',0))', kv=980, nlsk='#(nbs_ob22 (''6119'',''17''))', kvk=980, nlss=null, nlsa=null, nlsb=null, mfob=null, flc=0, fli=0, flv=0, flr=0, s='case  WHEN #(KVA) = 980 THEN  F_TARIF(62, #(KVA),#(NLSA), #(S))  ELSE 0  END', s2=null, sk=null, proc=null, s3800=null, rang=null, flags='0000100000000000000000000000000000000100000000000000000000000000', nazn=null
       where tt='K62';
  end;
  --------------------------------
  ----------- Реквизиты ----------
...
begin
  ...
  exception
    ...
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (tts_vob: 81, ''064'', 1) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  begin
  ...
  exception
    ...
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (tts_vob: 83, ''064'', 2) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  --------------------------------
