...
begin
  ...
  begin
    insert into op_rules(TAG, TT, OPT, USED4INPUT, ORD, VAL, NOMODIFY)
    values ('EXCTX', 'DPG', 'O', 1, 12, null, 1);
  exception
    when dup_val_on_index then null;
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (op_rules: ''EXCTX'', ''DPG'', ''O'', 1, 12, null, 1) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  begin
  ...
  begin
    insert into op_rules(TAG, TT, OPT, USED4INPUT, ORD, VAL, NOMODIFY)
    values ('REZID', 'DPG', 'M', 1, 8, null, 1);
  exception
    when dup_val_on_index then null;
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (op_rules: ''REZID'', ''DPG'', ''M'', 1, 8, null, 1) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  begin
  ...
  begin
    ...
    values (1, 'DPG', 3, null, '(f_teller_staff (userid,s,kv)=1 and branch_edit.get_branch_parameter_ex(branch, ''NOT2VISA'', ''0'') = ''0'')', null);
  exception
    when dup_val_on_index then null;
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (chklist_tts: 1, ''DPG'', 3, null, ''(f_teller_staff (userid,s,kv)=1 and branch_edit.get_branch_parameter_ex(branch, ''''NOT2VISA'''', ''''0'''') = ''''0'''')'', null) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  begin
  ...
  begin
    insert into chklist_tts(idchk, tt, priority, f_big_amount, sqlval, f_in_charge)
    values (7, 'DPG', 2, null, 'case when substr(NLSA, 1,4) in (''2620'', ''2625'') and NLSB like ''100%'' and KV<>980 then ( select sum(decode(c.contr_type, 2, 1, 0)) from accounts a join cim_contracts c on c.rnk=a.rnk where a.nls=NLSA ) else 0 end >0', null);
  exception
    when dup_val_on_index then null;
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (chklist_tts: 7, ''DPG'', 2, null, ''case when substr(NLSA, 1,4) in (''''2620'''', ''''2625'''') and NLSB like ''''100%'''' and KV<>980 then ( select sum(decode(c.contr_type, 2, 1, 0)) from accounts a join cim_contracts c on c.rnk=a.rnk where a.nls=NLSA ) else 0 end >0'', null) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  --------------------------------
