...
prompt Наименование операции: d: розрахунки за виплачені вклади згідно реєстру ФГВФО
declare
  cnt_  number;
begin
  --------------------------------
  ...
  begin
    ...
    values ('KC7', 'd: розрахунки за виплачені вклади згідно реєстру ФГВФО', 1, '#(nbs_ob22 (''3739'',''08''))', 980, '#(nbs_ob22 (''2809'',''24''))', 980, null, null, null, null, 0, 0, 0, 0, null, null, 5, null, null, null, '0000100000000000000000000000000000000100000000000000000000000000', 'розрахунки за виплачені вклади згідно реєстру ФГВФО');
  exception
    when dup_val_on_index then 
      update tts
         set tt='KC7', name='d: розрахунки за виплачені вклади згідно реєстру ФГВФО', dk=1, nlsm='#(nbs_ob22 (''3739'',''08''))', kv=980, nlsk='#(nbs_ob22 (''2809'',''24''))', kvk=980, nlss=null, nlsa=null, nlsb=null, mfob=null, flc=0, fli=0, flv=0, flr=0, s=null, s2=null, sk=5, proc=null, s3800=null, rang=null, flags='0000100000000000000000000000000000000100000000000000000000000000', nazn='розрахунки за виплачені вклади згідно реєстру ФГВФО'
       where tt='KC7';
  end;
  --------------------------------
  ----------- Реквизиты ----------
...
begin
  ...
  exception
    ...
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (chklist_tts: 1, ''457'', 2, null, ''(f_teller_staff (userid,s,kv)=1 and f_universal_box2(USERID)=1)'', null) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  begin
