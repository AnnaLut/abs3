...
prompt Наименование операции: Переказ коштів з арешт.рах. 2625 зг.плат. вимоги(внутр.) 
declare
  cnt_  number;
begin
  --------------------------------
  ...
  begin
    ...
    values ('W42', 'Переказ коштів з арешт.рах. 2625 зг.плат. вимоги(внутр.) ', 1, '#(bpk_get_transit('''',#(NLSB),#(NLSA),#(KVA)))', null, null, null, null, null, null, null, 0, 0, 0, 0, null, null, null, null, null, null, '1001000000001000000000000010000000000000000000000000000000000000', null);
  exception
    when dup_val_on_index then 
      update tts
         set tt='W42', name='Переказ коштів з арешт.рах. 2625 зг.плат. вимоги(внутр.) ', dk=1, nlsm='#(bpk_get_transit('''',#(NLSB),#(NLSA),#(KVA)))', kv=null, nlsk=null, kvk=null, nlss=null, nlsa=null, nlsb=null, mfob=null, flc=0, fli=0, flv=0, flr=0, s=null, s2=null, sk=null, proc=null, s3800=null, rang=null, flags='1001000000001000000000000010000000000000000000000000000000000000', nazn=null
       where tt='W42';
  end;
  --------------------------------
  ----------- Реквизиты ----------
  ...
  begin
    ...
    values (2, 'W42', 2, null, null, 3);
  exception
    when dup_val_on_index then null;
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (chklist_tts: 2, ''W42'', 2, null, null, 3) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  begin
