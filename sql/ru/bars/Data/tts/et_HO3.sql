...
prompt Наименование операции: Приймання  гот переказу коштiв ФО та перерах.по СЕП (з комiсiєю)
declare
  cnt_  number;
begin
  --------------------------------
  ...
  begin
    insert into tts(tt, name, dk, nlsm, kv, nlsk, kvk, nlss, nlsa, nlsb, mfob, flc, fli, flv, flr, s, s2, sk, proc, s3800, rang, flags, nazn)
    values ('HO3', 'Приймання  гот переказу коштiв ФО та перерах.по СЕП (з комiсiєю)', 1, '#(nbs_ob22 (''2909'',''35''))', 980, '#(get_proc_nls(''T00'',#(KVA)))', 980, null, '#(nbs_ob22 (''2909'',''35''))', null, null, 1, 1, 0, 0, null, null, null, null, null, null, '1201100000000000000001000000000000110000000000000000000000000000', null);
  exception
    when dup_val_on_index then 
      update tts
         set tt='HO3', name='Приймання  гот переказу коштiв ФО та перерах.по СЕП (з комiсiєю)', dk=1, nlsm='#(nbs_ob22 (''2909'',''35''))', kv=980, nlsk='#(get_proc_nls(''T00'',#(KVA)))', kvk=980, nlss=null, nlsa='#(nbs_ob22 (''2909'',''35''))', nlsb=null, mfob=null, flc=1, fli=1, flv=0, flr=0, s=null, s2=null, sk=null, proc=null, s3800=null, rang=null, flags='1201100000000000000001000000000000110000000000000000000000000000', nazn=null
       where tt='HO3';
  end;
  --------------------------------
  ----------- Реквизиты ----------
  ...
  begin
    ...
    values (1, 'HO3', 2, null, '(f_teller_staff (userid,s,kv)=1 and f_universal_box2(USERID)=1)', null);
  exception
    when dup_val_on_index then null;
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (chklist_tts: 1, ''HO3'', 2, null, ''(f_teller_staff (userid,s,kv)=1 and f_universal_box2(USERID)=1)'', null) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  begin
