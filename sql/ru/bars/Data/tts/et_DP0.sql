...
begin
  ...
  exception
    ...
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (op_rules: ''REZID'', ''DP0'', ''M'', 1, 98, null, 1) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  begin
  ...
  exception
    ...
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (chklist_tts: 1, ''DP0'', 3, null, ''(f_teller_staff (userid,s,kv)=1 and (branch_edit.get_branch_parameter_ex(branch, ''''NOT2VISA'''', ''''0'''') = ''''0'''') AND (EBP.HAS_EBP_DEPOSIT_ACCOUNTS(ref) = 0))'', null) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  begin
