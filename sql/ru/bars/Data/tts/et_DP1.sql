...
begin
  ...
  exception
    ...
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (tts_vob: 113, ''DP1'', null) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  --------------------------------
  ...
  begin
    ...
    values (1, 'DP1', 2, null, '(f_teller_staff (userid,s,kv)=1 and branch_edit.get_branch_parameter_ex(branch, ''NOT2VISA'', ''0'') = ''0'')', null);
  exception
    when dup_val_on_index then null;
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (chklist_tts: 1, ''DP1'', 2, null, ''(f_teller_staff (userid,s,kv)=1 and branch_edit.get_branch_parameter_ex(branch, ''''NOT2VISA'''', ''''0'''') = ''''0'''')'', null) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  begin
