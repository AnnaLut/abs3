...
prompt Наименование операции: Стягнення штрафу за дострокове розірвання договору
declare
  cnt_  number;
begin
  --------------------------------
  ...
  begin
    ...
    values (3, 'DUS', 1, null, null, null);
  exception
    when dup_val_on_index then null;
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (chklist_tts: 3, ''DUS'', 1, null, null, null) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  --------------------------------
