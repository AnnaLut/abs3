...
prompt Наименование операции: Повернення застави по iнкасованих iменних чеках
declare
  cnt_  number;
begin
  --------------------------------
  ...
  begin
    ...
    values (1, '127', 2, null, 'f_universal_box2(USERID)=1', null);
  exception
    when dup_val_on_index then null;
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (chklist_tts: 1, ''127'', 2, null, ''f_universal_box2(USERID)=1'', null) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  begin
