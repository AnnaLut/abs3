...
prompt Наименование операции: Списання з БПК згідно листа (розпорядження) / 20-Зняття грошей з рах.
declare
  cnt_  number;
begin
  --------------------------------
  ...
  begin
    insert into tts(tt, name, dk, nlsm, kv, nlsk, kvk, nlss, nlsa, nlsb, mfob, flc, fli, flv, flr, s, s2, sk, proc, s3800, rang, flags, nazn)
    values ('PK!', 'Списання з БПК згідно листа (розпорядження) / 20-Зняття грошей з рах.', 1, null, null, '#(bpk_get_transit(''20'',#(NLSB),#(NLSA),#(KVA)))', null, null, null, null, null, 1, 0, 0, 0, null, null, null, null, null, null, '1000100000000000000000000010000000010000000000000000000000000000', 'Списання з картрахунку згідно листа (розпорядження)');
  exception
    when dup_val_on_index then 
      update tts
         set tt='PK!', name='Списання з БПК згідно листа (розпорядження) / 20-Зняття грошей з рах.', dk=1, nlsm=null, kv=null, nlsk='#(bpk_get_transit(''20'',#(NLSB),#(NLSA),#(KVA)))', kvk=null, nlss=null, nlsa=null, nlsb=null, mfob=null, flc=1, fli=0, flv=0, flr=0, s=null, s2=null, sk=null, proc=null, s3800=null, rang=null, flags='1000100000000000000000000010000000010000000000000000000000000000', nazn='Списання з картрахунку згідно листа (розпорядження)'
       where tt='PK!';
  end;
  --------------------------------
  ----------- Реквизиты ----------
