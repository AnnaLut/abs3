...
begin
  ...
  begin
    ...
    values ('TM4', '5) # TM4 - доч до TMK', 0, null, null, '#(tobopack.GetTOBOParam(''CASH''))', 980, null, null, null, null, 0, 0, 1, 0, 'ROUND(#(S)*(SELECT DECODE(NVL(VES_UN,0),0, VES/31.1034807, VES_UN )FROM V_BANK_METALS_BRANCH B,OPERW W WHERE W.REF=#(REF) AND W.TAG=''N_BMK'' AND W.VALUE=N_BMK(B.KOD)),0)', '(#(S)/100)*(select cena_k from v_bank_metals_branch b, operw w where w.ref=#(REF) and w.tag=''N_BMK'' and w.value=N_BMK(b.kod))', 30, null, '#(nbs_ob22 (''3800'',''09''))', null, '0000100001000000000000000000000000010000000000000000000000010000', null);
  exception
    when dup_val_on_index then 
      update tts
         set tt='TM4', name='5) # TM4 - доч до TMK', dk=0, nlsm=null, kv=null, nlsk='#(tobopack.GetTOBOParam(''CASH''))', kvk=980, nlss=null, nlsa=null, nlsb=null, mfob=null, flc=0, fli=0, flv=1, flr=0, s='ROUND(#(S)*(SELECT DECODE(NVL(VES_UN,0),0, VES/31.1034807, VES_UN )FROM V_BANK_METALS_BRANCH B,OPERW W WHERE W.REF=#(REF) AND W.TAG=''N_BMK'' AND W.VALUE=N_BMK(B.KOD)),0)', s2='(#(S)/100)*(select cena_k from v_bank_metals_branch b, operw w where w.ref=#(REF) and w.tag=''N_BMK'' and w.value=N_BMK(b.kod))', sk=30, proc=null, s3800='#(nbs_ob22 (''3800'',''09''))', rang=null, flags='0000100001000000000000000000000000010000000000000000000000010000', nazn=null
       where tt='TM4';
  end;
  --------------------------------
  ----------- Реквизиты ----------
