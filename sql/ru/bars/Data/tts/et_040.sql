...
begin
  ...
  begin
    insert into tts(tt, name, dk, nlsm, kv, nlsk, kvk, nlss, nlsa, nlsb, mfob, flc, fli, flv, flr, s, s2, sk, proc, s3800, rang, flags, nazn)
    values ('040', '040  Конвертація гот. вал. CAD->USD (з комісією)', 1, '#(tobopack.GetToboParam(''CASH''))', 124, '#(tobopack.GetToboParam(''CASH''))', 840, null, '#(tobopack.GetToboParam(''CASH''))', '#(tobopack.GetToboParam(''CASH''))', null, 1, 0, 1, 0, null, null, null, null, '#(nbs_ob22 (''3800'',''10''))', null, '0000100000000000000000000000000000010000000000000000000000000000', 'Конвертація гот. вал. CAD->USD (з комісією)');
  exception
    when dup_val_on_index then 
      update tts
         set tt='040', name='040  Конвертація гот. вал. CAD->USD (з комісією)', dk=1, nlsm='#(tobopack.GetToboParam(''CASH''))', kv=124, nlsk='#(tobopack.GetToboParam(''CASH''))', kvk=840, nlss=null, nlsa='#(tobopack.GetToboParam(''CASH''))', nlsb='#(tobopack.GetToboParam(''CASH''))', mfob=null, flc=1, fli=0, flv=1, flr=0, s=null, s2=null, sk=null, proc=null, s3800='#(nbs_ob22 (''3800'',''10''))', rang=null, flags='0000100000000000000000000000000000010000000000000000000000000000', nazn='Конвертація гот. вал. CAD->USD (з комісією)'
       where tt='040';
  end;
  --------------------------------
  ----------- Реквизиты ----------
