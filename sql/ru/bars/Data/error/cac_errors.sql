...
begin
  ...
  bars_error.add_lang(l_rus,'Русский');
  bars_error.add_lang(l_ukr,'Украинский');
  bars_error.add_lang(l_geo,'Грузинский');
  ...
  bars_error.add_message(l_mod, 2, l_exc, l_rus, 'Модульный счет! Невозможно перерегистрировать на другого контрагента', '', 1);
  bars_error.add_message(l_mod, 2, l_exc, l_ukr, 'Модульний рахунок! Неможливо перереєструвати на iншого контрагента', '', 1);
  bars_error.add_message(l_mod, 2, l_exc, l_geo, 'Account in module! Impossible re-register on other contragent', '', 1);

  -- tiu_acc
  bars_error.add_message(l_mod, 3, l_exc, l_rus, 'Запрещено групповое обновление счетов!', '', 1);
  bars_error.add_message(l_mod, 3, l_exc, l_ukr, 'Заборонено групове поновлення рахункiв!', '', 1);
  bars_error.add_message(l_mod, 3, l_exc, l_geo, 'Group accounts update is denied!', '', 1);

  -- tiu_cus
  bars_error.add_message(l_mod, 4, l_exc, l_rus, 'Запрещено групповое обновление клиентов!', '', 1);
  bars_error.add_message(l_mod, 4, l_exc, l_ukr, 'Заборонено групове поновлення контрагентiв!', '', 1);
  bars_error.add_message(l_mod, 4, l_exc, l_geo, 'Group clients update is denied!', '', 1);

  -- op_reg
  bars_error.add_message(l_mod, 5, l_exc, l_rus, 'Невозможно изменить реквизиты закрытого счета №%s', '', 1);
  bars_error.add_message(l_mod, 5, l_exc, l_ukr, 'Неможливо змiнити реквiзити закритого рахунку №%s', '', 1);
  bars_error.add_message(l_mod, 5, l_exc, l_geo, 'Impossible change attributes closed account N%s', '', 1);
  ...
  bars_error.add_message(l_mod, 7, l_exc, l_rus, 'Параметры счета изменяются другим пользователем. ACC=%s', '', 1);
  bars_error.add_message(l_mod, 7, l_exc, l_ukr, 'Параметри рахунку змiнюються iншим користувачем. ACC=%s', '', 1);
  bars_error.add_message(l_mod, 7, l_exc, l_geo, 'Accounts parameters change other user. ACC=%s', '', 1);

  bars_error.add_message(l_mod, 8, l_exc, l_rus, 'Недопустимый код валюты - %s', '', 1);
  bars_error.add_message(l_mod, 8, l_exc, l_ukr, 'Неприпустимий код валюти - %s', '', 1);
  bars_error.add_message(l_mod, 8, l_exc, l_geo, 'Improper currency code - %s', '', 1);

  bars_error.add_message(l_mod, 9, l_exc, l_rus, 'Неуспешное открытие/изменение реквизитов счета (ACCOUNTS) №%s', '', 1);
  bars_error.add_message(l_mod, 9, l_exc, l_ukr, 'Неуспiшне вiдкриття/поновлення реквiзитiв рахунку (ACCOUNTS) №%s', '', 1);
  bars_error.add_message(l_mod, 9, l_exc, l_geo, 'Unsuccessful opening/change account attributes (ACCOUNTS) N%s', '', 1);

  bars_error.add_message(l_mod,10, l_exc, l_rus, 'Неуспешная привязка счета (ND_ACC) к сделке №%s', '', 1);
  bars_error.add_message(l_mod,10, l_exc, l_ukr, 'Неуспiшна прив''язка рахунку (ND_ACC) до угоди №%s', '', 1);
  bars_error.add_message(l_mod,10, l_exc, l_geo, 'Unsuccessful account link (ND_ACC) to deal N%s', '', 1);

  bars_error.add_message(l_mod,11, l_exc, l_rus, 'Неуспешное открытие счета обеспечения (PAWN_ACC) №%s', '', 1);
  bars_error.add_message(l_mod,11, l_exc, l_ukr, 'Неуспiшне вiдкриття рахунку забезпечення (PAWN_ACC) №%s', '', 1);
  bars_error.add_message(l_mod,11, l_exc, l_geo, 'Unsuccessful account of providing opening (PAWN_ACC) N%s', '', 1);

  bars_error.add_message(l_mod,12, l_exc, l_rus, 'Уже открыт счет овердрафта (ACC_OVER) для счета ACC=%s', '', 1);
  bars_error.add_message(l_mod,12, l_exc, l_ukr, 'Вже вiдкрито рахунок овердрафту (ACC_OVER) для рахунка ACC=%s', '', 1);
  bars_error.add_message(l_mod,12, l_exc, l_geo, 'Overdraft account already open (ACC_OVER) for account ACC=%s', '', 1);

  bars_error.add_message(l_mod,13, l_exc, l_rus, 'Неуспешное открыие счета овердрафта (ACC_OVER) для счета ACC=%s', '', 1);
  bars_error.add_message(l_mod,13, l_exc, l_ukr, 'Неуспiшне вiдкриття рахунку овердрафта (ACC_OVER) для рахунка ACC=%s', '', 1);
  bars_error.add_message(l_mod,13, l_exc, l_geo, 'Unsuccessfuk overdraft account opening (ACC_OVER) for account ACC=%s', '', 1);

  bars_error.add_message(l_mod,14, l_exc, l_rus, 'Неуспешная привязка счета (BANK_ACC) к МФО %s', '', 1);
  bars_error.add_message(l_mod,14, l_exc, l_ukr, 'Неуспiшна прив''язка рахунку (BANK_ACC) до МФО %s', '', 1);
  bars_error.add_message(l_mod,14, l_exc, l_geo, 'Unsuccessful account link (BANK_ACC) to MFO %s', '', 1);

  bars_error.add_message(l_mod,15, l_exc, l_rus, 'Счет №%s привязан к нескольким МФО', '', 1);
  bars_error.add_message(l_mod,15, l_exc, l_ukr, 'Рахунок №%s прив''язано до дек_лька МФО', '', 1);
  bars_error.add_message(l_mod,15, l_exc, l_geo, 'Account %s link to few MFO', '', 1);

  bars_error.add_message(l_mod,16, l_exc, l_rus, 'Неизвестный режим открытия счета - %s', '', 1);
  bars_error.add_message(l_mod,16, l_exc, l_ukr, 'Невiдомий режим вiдкриття рахунку - %s', '', 1);
  bars_error.add_message(l_mod,16, l_exc, l_geo, 'Unknown account opening mode - %s', '', 1);

  -- sync_open
  bars_error.add_message(l_mod,17, l_exc, l_rus, 'Не найдено подразделение с кодом %s!', '', 1);
  bars_error.add_message(l_mod,17, l_exc, l_ukr, 'Не знайдено пiдроздiлу з кодом %s!', '', 1);

  bars_error.add_message(l_mod,18, l_exc, l_rus, 'Невозможно однозначно идентифицировать подразд. %s!', '', 1);
  bars_error.add_message(l_mod,18, l_exc, l_ukr, 'Неможливо однозначно iдентифiкувати пiдрозд. %s!', '', 1);

  bars_error.add_message(l_mod,19, l_exc, l_rus, 'SYNC_OPEN: счёт закрыт %s!', '', 1);
  bars_error.add_message(l_mod,19, l_exc, l_ukr, 'SYNC_OPEN: рахунок закрито %s!', '', 1);

  bars_error.add_message(l_mod,20, l_exc, l_rus, 'SYNC_OPEN: счёт %s зарегистрирован за др.подразд.!', '', 1);
  bars_error.add_message(l_mod,20, l_exc, l_ukr, 'SYNC_OPEN: рахунок %s зареєстровано за iнш.пiдрозд.!', '', 1);

  bars_error.add_message(l_mod,21, l_exc, l_rus, 'Некорректно заполнен спр-к параметров для подразд. %s!', '', 1);
  bars_error.add_message(l_mod,21, l_exc, l_ukr, 'Некоректно заповнено довiдник параметрiв для пiдрозд. %s!', '', 1);

  bars_error.add_message(l_mod,22, l_exc, l_rus, 'Не найден контрагент для подразд. %s!', '', 1);
  bars_error.add_message(l_mod,22, l_exc, l_ukr, 'Не знайдено контрагента для пiдрозд. %s!', '', 1);

  bars_error.add_message(l_mod,23, l_exc, l_rus, 'Невозможно однозначно идентифицировать контрагента для подразд. %s!', '', 1);
  bars_error.add_message(l_mod,23, l_exc, l_ukr, 'Неможливо однозначно iдентифiкувати контрагента для пiдрозд. %s!', '', 1);

  bars_error.add_message(l_mod,24, l_exc, l_rus, 'Ошибка в контр.разряде счета %s МФО: %s!', '', 1);
  bars_error.add_message(l_mod,24, l_exc, l_ukr, 'Помилка в контр.розрядi рахунку %s МФО: %s!', '', 1);

  bars_error.add_message(l_mod,25, l_exc, l_rus, 'Указанная валюта (%s) не найдена или закрыта!', '', 1);
  bars_error.add_message(l_mod,25, l_exc, l_ukr, 'Вказану валюту (%s) не знайдено чи закрито!', '', 1);
  ...
  -- op_reg
  bars_error.add_message(l_mod, 41, l_exc, l_rus, 'Невозможно открыть счет / изменить реквизиты счета - клиент РНК %s закрыт', '', 1);
  bars_error.add_message(l_mod, 41, l_exc, l_ukr, 'Неможливо відкрити рахунок / змiнити реквiзити рахунку - клієнта РНК %s закрито', '', 1);
  bars_error.add_message(l_mod, 41, l_exc, l_geo, 'Impossible open account / change accounts attributes - customer RNK %s closed', '', 1);

  bars_error.add_message(l_mod, 42, l_exc, l_rus, 'Клиент РНК %s не найден', '', 1);
  bars_error.add_message(l_mod, 42, l_exc, l_ukr, 'Клієнта РНК %s не знвйдено', '', 1);
  bars_error.add_message(l_mod, 42, l_exc, l_geo, 'Customer RNK %s not found', '', 1);
  ...
  bars_error.add_message(l_mod, 53, l_exc, l_rus, 'Запрещено устанавливать нулевую процентную ставку для БС %s', '', 1, 'ERROR_INTRATN0_NBS');
  bars_error.add_message(l_mod, 53, l_exc, l_ukr, 'Заборонено встановлювати нульову процентну ставку для БР %s', '', 1, 'ERROR_INTRATN0_NBS');
