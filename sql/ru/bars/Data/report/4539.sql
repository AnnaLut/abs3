prompt ===================================== 
prompt == 424/Депозити ФО за % ст. "до погашення", в тис (бранчi)
prompt ===================================== 
...
begin     
   l_zpr.name := '424/Депозити ФО за % ст. "до погашення", в тис (бранчi)';
   l_zpr.pkey := '\BRS\***\ANI\55b';
   ...
   exception when no_data_found then                       
    ...
    l_zpr.name         := '424/Депозити ФО за % ст. "до погашення", в тис (бранчi)';
    l_zpr.namef        := '';
    l_zpr.bindvars     := ':sFdat1='''',:BRANCH=''Бранч''';
    l_zpr.create_stmt  := '';
    l_zpr.rpt_template := 'ANI_6b.qrp';
    l_zpr.form_proc    := '';
    l_zpr.default_vars := ':BRANCH=''/''';
    l_zpr.bind_sql     := ':BRANCH=''OUR_BRANCH|BRANCH|NAME|WHERE length(branch)>8 ORDER BY BRANCH ''';
    l_zpr.xml_encoding := 'CL8MSWIN1251';
    l_zpr.txt          := 'SELECT r.BRANCH, r.name NAMEB, 
  a.kv ID, t.LCV,  t.NAME, a.IR,   
...
WHERE t.kv=a.KV and r.branch = :BRANCH 
ORDER BY  a.KV, a.IR  
  ';
    l_zpr.xsl_data     := '';
    l_zpr.xsd_data     := '';
