prompt ===================================== 
prompt == Cальдова вiдомiсть Пiдроздiлу по OB22   (данi ф.#02)
prompt ===================================== 
...
begin     
   l_zpr.name := 'Cальдова вiдомiсть Пiдроздiлу по OB22   (данi ф.#02)';
   l_zpr.pkey := '\BRS\SBM\REP\31\7\2';
   ...
   exception when no_data_found then                       
    ...
    l_zpr.name         := 'Cальдова вiдомiсть Пiдроздiлу по OB22   (данi ф.#02)';
    l_zpr.namef        := '';
    l_zpr.bindvars     := ':sFdat1='''',:sFdat2='''',:Param0=''Б/Рахунок_З_(%вс_)'',:Param1=''Б/Рахунок_ПО_(%вс_)'',:Param2=''Вал(%вс_)'',:Param3=''Код_в_дд_лення(виб_р)'',:Param4=''OB22_(%вс_)_або_Один__з(01,02)'',:Param5=''1(Детально)_або_0(Бал.рахунки)'',:Param6=''1(З_п_дсумком_ ОВ22)_або_0(Без п_дсумка_ОБ22)'',:Param7=''1_З найменуванням,0_Н_''';
    l_zpr.create_stmt  := '';
    l_zpr.rpt_template := 'SBM_317.qrp';
    l_zpr.form_proc    := 'P_SAL_SNP (102, to_date(:sFdat1,''dd/mm/yyyy''), to_date(:sFdat2,''dd/mm/yyyy''),:Param3)';
    l_zpr.default_vars := ':Param0=''%'',:Param1=''%'',:Param2=''%'',:Param3=''%'',:Param4=''%'',:Param5=''1'',:Param6=''1'',:Param7=''0''';
    l_zpr.bind_sql     := ':Param3=''V_BRANCH_OWN|BRANCH|NAME''';
    l_zpr.xml_encoding := 'CL8MSWIN1251';
    l_zpr.txt          := 'select s.dat1,    s.dat2,        s.kv,                  s.nbs,                 s.nls,                   s.nms,
          s.ostd ostd1,   s.ostk ostk1,               s.ostid ostid1,         s.ostik ostik1,          
          s.ostqd,          s.ostqk,                        s.ostiqd,       s.ostiqk,
          s.ob22, 
          decode(s.nbs,  ''2620'',  0,  ''2628'',  0,  ''2630'',  0,  ''2638'',  0,  s.det_b) det_b,   s.det_ob,   
          s.pnbs1,         s.pnbs2,     s.pkv,        s.pbranch,    s.pob22,      s.pnbsi,     s.pob22i ,
          s.nbsob22,     s.nlssort  , to_number(:Param7) pnms, s.pnmsi   
from
(SELECT   :sFdat1 DAT1,      :sFdat2  DAT2,   
       nbs||name   nbsob22,       substr(nls,1,4)||substr(nls,6,9)||name    nlssort,
       ...
       decode(:Param5,''1'',''Детально по анал_тичних'',''0'' ,''Стисло по балансових'')  PNBSI,
       decode(:Param6,''1'',''Виводити п_дсумок по ОБ22'',''0'' ,''Не виводити п_дсумок по ОБ22'')   POB22I,
       decode(:Param7,''1'',''З найменуванням'',''0'' ,''Без найменування'')   pnmsi
FROM  cck_an_tmp 
WHERE        kv=  decode(:Param2,''%'',kv, substr(:Param2,1,3))                          AND      
     to_number(nbs)>=
