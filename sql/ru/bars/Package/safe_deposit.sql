...
create or replace package safe_deposit is
  ...
  g_header_version  constant varchar2(64) := 'version 3.2 14/08/2017';
...
create or replace package body safe_deposit is
  g_body_version  constant varchar2(64) := 'version 3.8 12/09/2017';
  ...
  begin
    ...
    --Видалення з реестру візитів
  
    delete from skrynka_visit
     where nd in (select n.nd
                    from skrynka s, skrynka_nd n
                   where s.n_sk = n.n_sk
                     and n.sos = 15
    ...
    exception
      when no_data_found then
        ...
        if p_deal_id is not null then
          ...
          select s_cc_deal.nextval into l_nd from dual;
        end if;
        ...
        exception
          when others then
            bars_audit.info('safe:mainacc_' || sqlerrm);
        end;
      
        bars_audit.info('skrynka: p_rnk = ' || p_rnk);
             ...
             when 3 then
              (select nvl(w.value, p.cellphone)
                ...
                where p.rnk = c.rnk
                  ...
                  and lower(w.tag) = 'mpno')
             when 2 then
              (select nvl(coalesce(r.telr, r.telb), ' ')
                 from corps r
                where r.rnk = c.rnk)
             else
              null
