...
--***************************************************************--
G_HEADER_VERSION  CONSTANT VARCHAR2(64)  := 'Version 2.8 02.06.2009';
...
g_body_version  constant varchar2(64)  := 'version 2.8 02.06.2009';
...
--
smask  RAW(64);         -- маска смотреть
dmask  RAW(64);         -- маска дебетовать
kmask  RAW(64);         -- маска кредитовать
smasku RAW(64);         -- маска смотреть
dmasku RAW(64);         -- маска дебетовать
kmasku RAW(64);         -- маска кредитовать
--
smask_global  RAW(64);         -- маска смотреть
dmask_global  RAW(64);         -- маска дебетовать
kmask_global  RAW(64);         -- маска кредитовать
smasku_global RAW(64);         -- маска смотреть
dmasku_global RAW(64);         -- маска дебетовать
kmasku_global RAW(64);         -- маска кредитовать
--
smask_parent  RAW(64);         -- маска смотреть
dmask_parent  RAW(64);         -- маска дебетовать
kmask_parent  RAW(64);         -- маска кредитовать
smasku_parent RAW(64);         -- маска смотреть
dmasku_parent RAW(64);         -- маска дебетовать
kmasku_parent RAW(64);         -- маска кредитовать
...
begin
   ...
   return utl_raw.compare(
              utl_raw.bit_and(
                  utl_raw.overlay(p_sec, '0000',1,64),
                  utl_raw.bit_or(
                      p_smask_global,
                      utl_raw.bit_or(
...
begin
   ...
   return utl_raw.compare(
              utl_raw.bit_and(
                  utl_raw.overlay(sec_, '0000',1,64),
                  utl_raw.bit_or(
                      smask_global,
                      utl_raw.bit_or(
...
begin
   ...
   elsif ida_>=16 and ida_<512 then
   ...
   return utl_raw.compare(utl_raw.bit_and(
                          utl_raw.overlay(sec_, '0000',1,64),
                          utl_raw.overlay(mask_,'0000',1,64)),null);
end;
...
begin
   ...
   return
      sign(utl_raw.compare(utl_raw.bit_and(
        utl_raw.overlay(sec_, '0000',1,64),
        utl_raw.bit_or(utl_raw.bit_or(smasku,smasku_global),smask_parent)),null))*4+
      sign(utl_raw.compare(utl_raw.bit_and(
        utl_raw.overlay(sec_, '0000',1,64),
        utl_raw.bit_or(utl_raw.bit_or(dmasku,dmasku_global),dmask_parent)),null))*2+
      sign(utl_raw.compare(utl_raw.bit_and(
        utl_raw.overlay(sec_, '0000',1,64),
        utl_raw.bit_or(utl_raw.bit_or(kmasku,kmasku_global),kmask_parent)),null));
end;
--
...
begin
 ...
 for i in 0..2 loop
   ...
   if full_calc=1 then
       ...
       for c in (select b.ida, a.scope from groups_acc a, groups_staff_acc b where
                 a.id=b.ida and b.ida < 512 and b.idg in (
       ...
       sec_global_  := utl_raw.overlay(sec_global_, '0000',1,64);
       sec_parent_  := utl_raw.overlay(sec_parent_, '0000',1,64);
...
IS
BEGIN
    sys.dbms_session.clear_context(sec.ctx_namespace, sys_context('userenv', 'client_identifier'));
...
grant EXECUTE                                                                on SEC             to BARS_DM;
