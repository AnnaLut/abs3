...
CREATE OR REPLACE PACKAGE BODY BARS.BARS_XMLKLB is
  ...
  ;
   ...
   G_BODY_VERSION    constant varchar2(64) := 'version 10.4 08.05.2014';
...
--         when p_pack_name='PMNT' then return '*:B:D';
   ...
   begin
      ...
      p_doc.nd    := trim(substr(l_tmp, 1, 10));
      ...
      bars_audit.trace(l_trace||'-> '||l_tmp);
      p_doc.kv    := convert_to_number(substr(l_tmp,1,3), 153);
