...
IS
   ...
   ----
   --створює фінансовий документ на оплату (3570-6110)
   --
      ...
      BEGIN
         ...
         INSERT INTO oper (REF,
                 ...
                 VALUES (
                           ...
                           gl.aOKPO,
                           gl.aUID,
                           'Нарахування плати за відправку SMS-повідомлень рах. '
                           || l_nls_parent ||';к-сть СМС:'||p_sms_cnt);
   ...
   BEGIN
    ...
    --запоминаем номер основного счета (для назначения пл.)
      begin
      SELECT    NLS
      ...
      BEGIN
         ...
         THEN
          /*  g_err_num := -20005;
            g_erm :=
                  g_erm
               || ' - Рахунок заборгованості має незавізовані документи!'||to_char(l_ostc)||' '||to_char(l_ostb)||' '||to_char(l_nmsb);
            raise_application_error (g_err_num, g_erm);*/ null;
         END IF;
      END;

    --определим назн. плат. (l_nazn)

    select
        case

            when substr(l_nlsb,1,4)='3570'
            then 'Списання плати за надання виписок в електронному вигляді шляхом SMS-повідомлень по рах.'|| l_nls_parent

            when (substr(l_nlsa,1,4)='3570' and substr(l_nlsb,1,4)='3579')
            then 'Перенесення на рахунок простроченої заборгованості плати за надання виписок шляхом СМС повідомлень'

            else 'Погашення Заборгованості за SMS інформування рах. '|| l_nls_parent
        end
   ...
   IS
   p_clearance_p NUMBER;
   BEGIN
      logger.info ('find_clearance. Для счета ACC=' || p_acc_parent);

      BEGIN
         SELECT t1.acc,/*NVL (fost (t1.acc, SYSDATE), 0)*/ t1.ostc, t1.ostb
           INTO p_acc_clearance, p_clearance, p_clearance_p
           FROM accounts t1, SMS_ACC_CLEARANCE t2
          WHERE t1.acc = T2.ACC_CLEARANCE AND t2.acc = p_acc_parent;

         if p_clearance<>p_clearance_p then
          logger.info ('find_clearance_exp. Для счета ACC=' || p_acc_parent||' есть незавизированные документы по 3570');
          p_clearance :=0;
         end if;
   ...
   IS
   p_clearance_exp_p NUMBER;
   BEGIN
      logger.info ('find_clearance_exp. Для счета ACC=' || p_acc_parent);

      BEGIN
         SELECT t1.acc,/*NVL (fost (t1.acc, SYSDATE), 0)*/ t1.ostc, t1.ostb
           INTO p_acc_clearance_exp, p_clearance_exp, p_clearance_exp_p
          FROM accounts t1, SMS_ACC_CLEARANCE t2, sms_acc_clearance_exp t3
          WHERE t1.acc = T3.ACC_CLEARANCE_EXP AND T3.ACC_CLEARANCE = T2.ACC_CLEARANCE and t2.acc = p_acc_parent;

          if p_clearance_exp<>p_clearance_exp_p then
          logger.info ('find_clearance_exp. Для счета ACC=' || p_acc_parent||' есть незавизированные документы по 3579');
          p_clearance_exp :=0;
          end if;
