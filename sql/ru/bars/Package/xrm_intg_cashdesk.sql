...
is
   ...
   begin
     ...
     -- dbms_xslprocessor.valueof (l_analytic, 'root/body/row/dk/text()', l_rec.dk);
      ...
      then
         ...
         l_oper.nam_a:= l_nms_a; 
         l_oper.nam_b:= l_nms_b;   
   ...
   is
      ...
      l_oper            oper%rowtype;
      l_tts             tts%rowtype;
      l_signin          oper_visa.sign%type;
      l_signsep         oper.sign%type;
...
/*      type t_num_array is table of number
   ...
   begin
      ...
      loop
         ...
         dbms_xslprocessor.valueof (l_analytic, 'ref/text()', l_docs_parse (l_docs_parse.last).ref);
         dbms_xslprocessor.valueof (l_analytic, 'docsignin/text()', l_docs_parse (l_docs_parse.last).int_sign_hex);
         dbms_xslprocessor.valueof (l_analytic, 'docsignsep/text()', l_docs_parse (l_docs_parse.last).ext_sign_hex);
      end loop;

      dbms_xmldom.freedocument (l_doc);

      -- есть ли документы
      if l_docs_parse.count = 0
      then
      ...
      dbms_lob.append (l_clob_data,
                       '<docs4visa grpid="' || l_idchk || '" key="'||substr(l_tabn,1,6)||'">');

      for k in l_docs_parse.first .. l_docs_parse.last
      loop
         dbms_lob.append (l_clob_data,
                          '<doc ref="' || xrm_maintenance.add_ru_tail (l_docs_parse (k).ref) || '" />');
      end loop;

      dbms_lob.append (l_clob_data, '</docs4visa>');

      chk.make_data4visa_xml (l_clob_data, l_clob_data);
      ...
      loop
         ...
         l_xmlbufsnode :=
            dbms_xmldom.item (dbms_xmldom.getelementsbytagname (l_xmldocelement, 'bufs'), 0);
         l_xmlbufselement := dbms_xmldom.makeelement (l_xmlbufsnode);
         l_docs (l_docs.last).int_buffer_hex := dbms_xmldom.getattribute (l_xmlbufselement, 'inner_buf');
         l_docs (l_docs.last).ext_buffer_hex := dbms_xmldom.getattribute (l_xmlbufselement, 'outer_buf');

         --добавляем подписи с request(дико..) нужно переделать
          for g in l_docs_parse.first .. l_docs_parse.last
          loop
            if xrm_maintenance.add_ru_tail (l_docs_parse (g).ref)=l_docs (l_docs.last).ref
             then
                  l_docs (l_docs.last).int_sign_hex:=l_docs_parse (g).int_sign_hex;
                  l_docs (l_docs.last).ext_sign_hex:=l_docs_parse (g).ext_sign_hex;
                  exit;
            end if;
          end loop;
      ...
      loop
         ...
         then
            dbms_lob.append (
               l_clob_data2,
               '<doc ref="' || l_docs (k).ref || '" key="' || substr(l_tabn,1,6) || '">');
            dbms_lob.append (
               l_clob_data2,
                  '<bufs inner_buf="'
      ...
      then
         chk.put_visas_xml (l_clob_data2, l_clob_data2);
         l_parser := dbms_xmlparser.newparser;
