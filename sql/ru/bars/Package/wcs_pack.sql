 ...
 PROMPT ===================================================================================== 
  ...
  procedure quest_pars_ref_set(p_question_id    varchar2, -- Идентификатор вопроса
                               ...
                               p_tab_name       varchar2 default null, -- Идентификатор таблицы справочника
 ...
 procedure sbprod_clone(p_subproduct_id     wcs_subproducts.id%type, -- Идентификатор субпродукта
                         ...
                         p_credit_data_copy  boolean default false);
  ...
  procedure mac_refer_param_set(mac_id_         varchar2, -- Идентификатор МАКа
                                tab_name_         varchar2, -- Идентификатор таблицы справочника
...
-- Удаление истории изменений состояния заявки
  ...
  procedure scor_quest_decimal_set(p_scoring_id  wcs_scoring_qs_decimal.scoring_id%type, -- Идентификатор карты скоринга
                                   ...
                                   p_score_max   wcs_scoring_qs_decimal.score_max%type -- Максимальное значение балов
                                   );
...
CREATE OR REPLACE PACKAGE BODY BARS.WCS_PACK is
  ...
  -- ===============================================================================================
  g_body_version constant varchar2(64) := 'version 2.0 28/07/2016';
  ...
  */
  ...
  procedure sbprod_clone(p_subproduct_id     wcs_subproducts.id%type, -- Идентификатор субпродукта
                         p_src_subproduct_id wcs_subproducts.id%type, -- Идентификатор субпродукта-источника для клонирования
                         p_credit_data_copy  boolean default false) is -- копіювати чи ні дані кредиту
  begin
    -- МАКи
    sbprod_macs_clone(p_subproduct_id, p_src_subproduct_id);
    -- Данные кредита
    if p_credit_data_copy= true then
  ...
  begin
    ...
    --05/02/2016 MPivanova закоментовано, оскільки даний етап проходження заявки неактуальний
    ...
    if (l_b_row.id is not null) then
      ...
      begin
        ...
        exception when no_data_found  then
          raise_application_error(-20008,'Анкета клієнта для заявки '||l_cur_b_row.id || ' по субпродукту '||l_cur_b_row.subproduct_id||' ');
  ...
  begin
    ...
    select s_cc_deal.nextval into l_bid_id from dual;
    ...
    for cur in (select *
                 ...
                 where sg.subproduct_id = p_subproduct_id
                   and sg.is_required = 1
                   and wcs_utl.has_bid_state(l_bid_id,'NEW_FT_DATAINPUT') = 0) loop
...
*/
  ...
  begin
    ...
    if (1 = wcs_utl.has_bid_state(p_bid_id, 'NEW_DATAREINPUT') or (1 = wcs_utl.had_bid_state(p_bid_id,'NEW_PREVIOUS_SOLUTION') and 1 = wcs_utl.has_bid_state(p_bid_id, 'NEW_DATAINPUT'))) then
  ...
  begin
    ...
    -- значение позиции вопроса
    select nvl(max(ord) + 1, 0) into l_ord from wcs_scoring_questions where scoring_id = p_scoring_id;
    ...
    if (sql%rowcount = 0) then
      insert into wcs_scoring_questions
        (scoring_id, question_id, result_qid, multiplier, else_score,ord)
      values
        (p_scoring_id,
         p_question_id,
         l_result_qid,
  ...
  */
  ...
  begin
    ...
    -- Удаляем
    if (l_count = 1 and l_sg_row.is_required = 1 and wcs_utl.has_bid_state(p_bid_id,'NEW_FT_DATAINPUT') = 0) then
      null;
      -- Нельзя удалить последний обязательный тип
    else
