 ...
 PROMPT ===================================================================================== 
  ...
  CREATE OR REPLACE PACKAGE BARS.CP IS
   ...
   G_HEADER_VERSION    constant varchar2(64) := 'v.1.13.4  04.15.2015';
                                              --'v.1.13.2 11.09.2015'
   ...
   l_user int;
   l_mon int :=0;   -- наявність пакета  MONITOR_USER
...
END CP;
/
CREATE OR REPLACE PACKAGE BODY BARS.CP IS
----------------------------------------------------------------------
--
--  НБУ. Работа с ценными бумагами
--   отдел сопровождения САБО. (040) 43-86,  253-14-38
--   mail-to: Довбня Л.О. <larysa@bank.gov.ua>
----------------------------------------------------------------------
    G_BODY_VERSION      constant varchar2(64) := 'v.2.84 25/12/2015';
...
IS
BEGIN
  bars_audit.trace('%s:move_potok start.',G_MODULE);
   BEGIN
   ...
   END;
 bars_audit.trace('%s:move_potok - finish.',G_MODULE);
END move_potok;
----------------------------------------------------------------------
...
BEGIN
   ...
   ELSIF    p_mode = 10
    THEN
      l_ref :=NVL (p_ref,TO_NUMBER (SUBSTR (pul.Get_Mas_Ini_Val ('CP_REF'), 1, 10)));
...
(p_REF1 number, -- реф первичной сделки покурки
 ...
 p_VDAT date  ,
 p_N number   ,  p_R number   , -- резерв. не исп
...
-- Sta Анулирование старых ден.потоков  и  Построение новых при частичной продаже/перемещении пакета ЦБ.
  ...
  dd     cp_dat%rowtype;
  ACCR_  number ;            ACCR2_ number;         ACCD_  number;
...
BEGIN
  ...
  begin    ------------------------- есть ли номинал
     select id,accR,accR2, fost(acc,p_vdat), NVL(accD,accP), op
     into ID_, ACCR_, ACCR2_, N_, ACCD_, l_op
 ...
 EXCEPTION WHEN NO_DATA_FOUND THEN l_fl:=1;
  ...
  If gl.aMfo in ('300001','321024')  or p_ref1 = p_ref2 then
     ...
     insert into TMP_IRR(n,s) values (1,N_+D_+r_+R2_ );
     ...
     --1) день-1 выдачи средств
     insert into cp_many(REF,FDAT,SS1,SDP,SN2) values (p_REF1,p_VDAT, N_/100,D_/100, (r_+R2_)/100);
     --2) день-i погаш купона и част.номинала
     ...
     delete from cp_many where ref=p_REF1 and fdat >= gl.bdate;

     --1) день-1 продажи ЦБ
     insert into cp_many(REF,FDAT,SS1,SDP,SN2) select p_REF1,gl.bdate, x.N/100, (-x.D+x.P)/100, x.R/100       from cp_arch x where x.ref = p_REF2 ;
     --2) день-i погаш купона и част.номинала
     insert into cp_many(REF,FDAT,SS1,SDP,SN2) select P_REF1,DOK, nKol_*nvl(nom,0), 0, nKol_*Nvl (KUP,nInt1_) from cp_dat where id=ID_ and DOK>p_VDAT and DOK< kk.DATP ;
     --3) день-к погаш посл.купона и ост.номинала
     insert into cp_many(REF,FDAT,SS1,SDP,SN2) select P_REF1,DOK, N1_, 0 , nKol_*Nvl (KUP,nInt1_)             from cp_dat where id=ID_ and dOK=kk.DATP ;
...
l_ret varchar2(2);
begin
 for k in (select m.REF1, m.REF2, m.VDAT,m.SS,m.SN, o.sos, d.op
 ...
 loop
     ...
     If k.sos >= 5 then
        ...
        cp.RMany(k.ref1, k.REF2, k.VDAT, k.ss, k.sn, l_ret);
        if l_ret!='00' then
...
BEGIN
   ...
   FOR k
      IN (SELECT n.ID,
                 ...
                 (select ir from cp_dat where dok = n.dok and id = e.id) ir
            FROM cp_deal e,
                 accounts a,
                 cp_kod o,
                 (  SELECT id,
                           MAX (TO_DATE (iif_d (dok,
                                                DATN_,
                                                dok,
                                                dok,
                                                NULL)))
                              DOK,
                           MIN (TO_DATE (iif_d (dok,
                                                DATN_,
                                                NULL,
                                                NULL,
                                                dok)))
...
begin
  ...
  VALUES (ref_,FXB_,VOB_,ref_,0, sysdate, VDAT_, greatest(VDAT_,gl.bDATE), gl.bDATE,NMS_,   NLSGA_,gl.AMFO,kk.KV,SN_,
  ...
  bars_audit.info('CP_MOVE:nREF_='||to_char(nREF_));
  bars_audit.info('CP_MOVE: REF_='||to_char( REF_));
  SSQ_:=0;
  ...
  If kk.DOX >= 2 then
     if kk.ir >0 then
        ...
        begin /* открыть сч зачислять начисленный % */
          select a.acc, substr(a.nls,1,5)||'0'||S8_, substr(cp_id_r||a.nms,1,38), a6.acc, decode(kk.KV,gl.baseval,p.TT,p.TTV),
          NVL(p.io,0)
          --nvl( kk.io, NVL(p.io,0) )   --  6/08-14
          into accc_, NLS_, NMS_, accR6_ , TTV_, IO_
          from accounts a, ACCOUNTS A6, proc_dr p
          where a.nls=ac2.nlsR and a.kv=kk.KV AND a6.kv=gl.baseval
                and a6.nls=ac2.NLS_PR and p.nbs=ac2.vidd and rownum = 1 ;
        EXCEPTION WHEN NO_DATA_FOUND THEN     sERR:='14.Нет сч.купона ИСХ в ФУ або ?PROC_DR'; return;
        END;
           ...
           insert into int_accn (acc  ,id,acra ,acrb  ,metr,tt             ,BASEY   ,FREQ, ACR_DAT       , io)
            ...
            INSERT INTO int_accn (acc,
                 ...
                 VALUES (acc_,
                         ...
                         accR_,
                         accR6_,
                         iif_n (pr1_kup_,2,kk.metr,8,kk.metr),
                         NVL (TTV_, 'FX%'),
                         kk.BASEY,
                         1,
                         gl.BDATE - 1 + IO_,
     ...
     if SR2_ >0 then
        begin
          select acc,  substr(cp_id_r||nms,1,38),  substr(nls,1,5) ||'2'||S8_    into accc_, NMS_, NLS_
          from accounts   where nls = ac2.nlsR2 and kv= kk.KV  ;
     ...
     if SR9_ >0 then
        begin
          ...
          from accounts   where nls = ac2.nlsexpR and kv= kk.KV  ;
        EXCEPTION WHEN NO_DATA_FOUND THEN sERR:='16.Нет сч.купона-9 ИСХ в ФУ'; return;
        END;
        cp.CP_REG_EX(99,0,0,GRP_,r1_, kk.RNK ,nls_, kk.KV ,NMS_,'ODB',gl.aUid, accR9_);
        UPDATE accounts SET mdate= kk.DNK-1,accc=ACCC_,seci=4,daos=VDAT_, pos=1
        WHERE acc=accR9_;
  ...
  If ac2.NLSS is not null and no_p_ <>1 then /* сч.переоценки */
     ...
     begin
       ...
       into accc_, NLSS_, NMS_ from accounts where nls=ac2.NLSS and kv= kk.KV ;
...
*/
  ...
  If ac2.NLSS5 is not null and ( s5_ <> 0 or s6_ <> 0 or accS_ is not null ) then
     begin
       select acc, substr(cp_id_r||nms,1,38), substr(nls,1,5)||'0'||s8_  into accc_, NMS_, NLS_
       from accounts  where nls = ac2.NLSS5 and kv = gl.baseval ;
  ...
  If ac2.NLS_FXP is not null and ( s6_ <> 0 or accS_ is not null )
     ...
     begin
       select acc, substr(cp_id_r||nms,1,38), substr(nls,1,5)||'0'||s8_   into accc_, NMS_, NLS_
       from accounts where nls = ac2.NLS_FXP and kv = gl.baseval ;
...
PROCEDURE CP_PROD (TIPD_       IN     cp_kod.tip%type,      -- 1 акт (чужие), 2 пас (свои)
                   ...
                   REF_MAIN    IN OUT INT)                  -- референс основной сделки
IS
   title       constant    varchar2(8) := 'CP_PROD:';
   aa          accounts%ROWTYPE;
...
------------------------------------------------------------------------------
  ...
  FOR k IN (  SELECT a.acc,
                ...
                FROM accounts a,
                     ...
                     accounts r,
                     accounts r2,
                     (SELECT REF, ACC, ACCD, ACCP, ACCR, ACCR2, ACCS,
                             (SELECT SUM(S)
                                FROM opldok
                               WHERE acc = ee.acc
                                 AND REF >= REF_
               ...
               WHERE     a.acc = e.acc
                     ...
                     AND e.accr = r.acc(+)
                     AND e.accr2 = r2.acc(+)
                     AND (d.ostb <> 0 OR p.ostb <> 0 OR r.ostb <> 0 OR r2.ostb <> 0)
            ORDER BY e.REF)
  LOOP
     -- Вычитать l_koeff
...
BEGIN
  ...
  begin
    ...
    sERR:='30.Нет ЦБ '||nID_      ;  select * into kk from cp_kod where id = nId_;
    ...
    SELECT SUBSTR (a.nms, 1, 38), c.okpo
     ...
     WHERE a.acc = u.acc
       ...
       AND a.kv = kk.KV
       AND nls = S_TRANS_DK_NLS;
    exception when no_data_found then sERR:='31.Нет сч.транзита '||S_TRANS_DK_NLS; return;
    end;
    ...
    if  kk.DOX >= 2  and TIPD_=1 and nvl(r_pf2.NO_A,0) <>1 then     --  r_pf2.pf in (4,6,7)
       sERR:='32.Нет сч.6 кл.по аморт.';
       select aD.acc, aP.acc, aD.nls, aP.nls    INTO ACRB_, ACRBP_,NLS703, NLS703P     FROM accounts aD, accounts aP, cp_accc c
       where aD.kv=gl.baseval and aD.nls=c.s605 and aP.kv=gl.baseval and aP.nls=c.s605P and c.vidd=nNBS_ and c.RYN=nRYN_ and c.emi= kk.EMI;
    end if;
  EXCEPTION WHEN NO_DATA_FOUND THEN   RETURN;
  END;
  ...
  then
    R3_ := (R_/l_normal) * (l_normal-l_awry_first);      -- кривой купон первого купонного периода
    R_  := (RR_*100) - R3_;                              -- накопленный купон за вычетом кривого купона
    bars_audit.info(title||'/ R_ = '||to_char(R_)||', R3_ = '||to_char(R3_));
  end if;
  ...
  begin
    ...
    where a.nls=p.nlsA and a.kv= kk.KV  and p.vidd=nNBS_ and p.emi= kk.EMI and p.RYN=nRYN_ and f.pf = p.pf;
  EXCEPTION WHEN NO_DATA_FOUND THEN    sERR:='33.Нет осн.сч. в ФУ'; RETURN;
  END;
      ...
      -- LOG('CP_KUP ref='||ref_||' rnk='||kk.rnk,'INFO');
       ...
       If NO_P_ <> 1 then
          ...
          --НБУ.NLSS5 Род.счет 5121.
              ...
              EXCEPTION WHEN NO_DATA_FOUND THEN accS5_:=null;     sERR:='34.Нет сч.NLSS5 Род.счет 5121';   return;
              END;
          ...
          --6300 (6310) Рах.NLS_FXP~НЕреалiз.рез~переоц
              ...
              EXCEPTION WHEN NO_DATA_FOUND THEN accS6_:=null;     sERR:='34.Нет сч.NLS_FXP~НЕреалiз.рез~переоц в ФУ';   return;
              END;
          end if;    -- NBUBNK
        bars_audit.trace('!!!');
          --1305, 1315
          ...
          EXCEPTION WHEN NO_DATA_FOUND THEN accS_:=null;   sERR:='34.Нет сч.перец. в ФУ';   return;
          END;
       ...
       if R_ <> 0  then
          ...
          EXCEPTION WHEN NO_DATA_FOUND THEN   sERR:='37.Нет сч.КУП.% в ФУ';  RETURN;
          END;
          cp.CP_REG_EX(99,0,0,GRP_,r1_, kk.RNK , nls_, kk.kv ,NMS_,'ODB', gl.aUid ,accR2_);
          ...
          BEGIN
            select a.acc,
             ...
             where a.nls = p.nlsR2
               ...
               and p.RYN = nRYN_ ;
          EXCEPTION WHEN NO_DATA_FOUND
                    THEN sERR:='37/3.Нет сч.КУП.% в ФУ';
                         RETURN;
          END;
       ...
       if kk.ir > 0 or kk.CENA_KUP > 0  then
          ...
          begin
            ...
            from accounts a, cp_accc C, accounts a6, proc_dr P
            where a.nls=C.nlsR and a.kv= kk.KV and C.vidd=nNBS_ and C.emi= kk.EMI
              and C.RYN=nRYN_  and P.nbs=nNBS_ AND a6.kv=gl.baseval    and a6.nls=C.NLS_PR ;
       ...
       then
        ...
        -- купить ЦБ чужие OP=1
           INSERT into cp_arch(REF,ID,DAT_UG,DAT_OPL,DAT_ROZ,acc,SUMB,N,D,P,R,STR_REF,OP) values (Ref_,nID_,DAT_UG,DAT_OPL,DAT_ROZ,acc_,S_,N_,D_,P_,R_,sREF,1 );
                  ...
                  if kk.DOX >= 2 then
                     If DK_= 0 then --Вирт.П
                       --insert into int_accn  (acc,acra,acrb,id,metr,ACR_DAT,s,STP_DAT,IO, basey,freq) values (ACC_,ACCP_,ACRBP_,3,6,gl.bdate-1,V1_, kk.DATP-1,IO_,0,1);
                        begin
                        update int_accn set acra = ACCP_
...
PROCEDURE CP_vneb (DK_       IN     oper.dk%type,
                   ...
                   sErr         OUT VARCHAR2)
IS
 title constant varchar2(8) := 'CP_vneb:';
...
begin
 ...
 bars_audit.info(l_title||'Номер текущего купонного периода = '|| to_char(p_npp));
 ...
 then
     ...
     bulk collect
       ...
       from cp_dat
     where id = p_id;
   bars_audit.info(l_title ||'1) p_id='||to_char(p_id)||',l_periodcount='||to_char(1)|| ',l_cpdat_set('||to_char(1)||').DOK = '||to_char(l_cpdat_set(1).DOK,'dd/mm/yyyy'));
   bars_audit.info(l_title ||'2) p_id='||to_char(p_id)||',l_periodcount='||to_char(l_periodcount)|| ',l_cpdat_set('||to_char(l_periodcount)||').DOK = '||to_char(l_cpdat_set(l_periodcount).DOK,'dd/mm/yyyy'));
   bars_audit.info(l_title ||'3) p_id='||to_char(p_id)||',NPP='||to_char(l_cpdat_set(1).NPP));
   l_days_normal_period       := l_cpdat_set(3).DOK - l_cpdat_set(2).DOK;
   p_normal := l_days_normal_period;
   bars_audit.info(l_title ||'4) p_id='||to_char(p_id)||',l_days_normal_period='||to_char(to_char(l_days_normal_period)));
   bars_audit.info(l_title ||'4a'||to_char(l_cpdat_set(1).NPP-1)||','||to_char(l_cpdat_set(1).dok));
   ...
   bars_audit.info(l_title ||'5) p_id='||to_char(p_id)||',l_days_awry_period_first='||to_char(to_char(l_days_awry_period_first)));

   l_days_awry_period_last   := l_cpdat_set(l_periodcount).DOK - l_cpdat_set(l_periodcount-1).DOK;
   bars_audit.info(l_title ||'6) p_id='||to_char(p_id)||',l_days_awry_period_last='||to_char(to_char(l_days_awry_period_last)));
...
grant EXECUTE                                                                on CP              to CP_ROLE;
