...
begin
  ...
  if l_wdate < p_d_plan then
        raise_application_error(-(20000+555),' Транш № ' || p_npp  || ', строк не вiдповiдає кінцевому строку договору '|| to_char(l_wdate,'dd/mm/yyyy') );
  end if;
end chk_term;
...
--------------------------------------------------------------------
 ...
 (p_npp    cc_trans.npp%TYPE   ,
  ...
  P_d_plan cc_trans.d_plan%type)
IS
 srok_ number (10,3); nbs_  accounts.nbs%type;
BEGIN
  srok_ := months_between (p_d_plan, p_fdat) ;
  If srok_ >12 then
     select nbs into nbs_ from accounts where acc = p_acc;
     If nbs_ like '___2' then
        raise_application_error(-(20000+444),' Транш № ' || p_npp  ||     ', строк не вiдповiдає бал.рах.'|| nbs_ );
     end if;
  end if;
...
--------------------------------------------------------------------
 ...
 begin
   --разные проверки
   begin   select * into l_trans from cc_trans where npp =p_NPP;
   EXCEPTION WHEN NO_DATA_FOUND THEN    raise_application_error(n_Err,s_Err||'не знайдено в таблицi траншiв CC_TRANS' );
   end;

   If l_Trans.D_FAKT is not null then
      raise_application_error(n_Err,s_Err||'уже погашено '||to_char(l_Trans.D_FAKT,'dd.mm.yyyy'));
   end if;
...
--------------------------------------------------------------------
 ...
 begin
   ...
   update cc_trans set SV=  p_SV , SZ = p_Sz , comm =  p_comm  where npp=p_npp;
...
BEGIN
 begin
   ...
   select CASE  WHEN p_tip ='SP '                                    THEN p_fdat-1
                WHEN p_nbs like '2__2' or gl.amfo not in ('380764')  THEN least(p_mdate, p_fdat+365 )
                ELSE                                                      p_mdate
                END
   into l_dat1  from cc_deal d, nd_acc n
   where d.vidd in (2,3) and nvl(to_number(cck_app.get_nd_txt_ex(d.nd, 'PR_TR', bankdate)),0) = 1  and n.acc = p_acc and n.nd = d.nd ;
 EXCEPTION WHEN NO_DATA_FOUND THEN return ;
 end;

 If p_tip = 'SS ' then
   begin  select to_date(value,'dd.mm.yyyy') into l_dat2 from operw where ref = p_ref and tag = 'MDATE' ;      -- кажд?й транш может иметь свой срок
   EXCEPTION WHEN NO_DATA_FOUND THEN null;
   end;
 end if;

 If P_tip = 'SP '  then
   begin  select to_number (value)           into l_Id0  from operw where ref = p_ref and tag = 'REF92' ;   -- Iд.Поч.Траншу в процедуре выноса на просрочку
   EXCEPTION WHEN NO_DATA_FOUND THEN null;
   end;
 end if;
 ...
 If srok_ >12 then
    If p_nbs like '___2' then
       raise_application_error(-(20000+444),    'Cтрок траншу реф=' ||p_ref || ':
       '|| to_char(p_fdat,'dd.mm.yyyy') ||  ' - ' || to_char(l_dat2,'dd.mm.yyyy') ||'
 ...
 If p_tip = 'SS '  then   --свободный ост лимита
    for k in (select * from cc_lim   where acc= p_ACCC and fdat > p_FDAT and  fdat < l_dat2   order by fdat)
    loop
      -- несколько выдач в один день
      select  Nvl(sum(sv),0) into l_sv1  from cc_trans where acc = p_acc and fdat = p_fdat;
      l_sv   := p_ost - l_sv1 - k.lim2 +l_lim1 ;
      If l_sv > 0 and l_s > 0 then
         l_sv:= least ( l_sv, l_s );
         INSERT INTO CC_TRANS (ref,   acc,   fdat,   sv, sz, D_PLAN ) values ( p_ref, p_acc, p_fdat, l_sv, 0 , k.fdat);
         l_s := l_s - l_sv;
      end if ;
    end loop ;
 end if;
 ...
 If l_s > 0 then
    INSERT INTO CC_TRANS (id0, ref,   acc,   fdat,   sv, sz, D_PLAN ) values ( l_Id0, p_ref, p_acc, p_fdat, l_s , 0 , l_dat2);
 end if;
...
begin
  If nvl(p_SZ_OLD,0) <> nvl(p_SZ_NEW,0) then
     ...
     update cc_trans set sz=p_SZ_NEW where nvl(sz,0)=nvl(p_SZ_OLD,0) and npp=P_NPP;
     if SQL%rowcount = 0 then
        raise_application_error(n_Err,s_Err||'одночасна робота користувачiв з траншем ! ' );
     end if;
  end if;
...
begin
   ...
   EXCEPTION WHEN NO_DATA_FOUND THEN
     raise_application_error(n_Err,s_Err||'не знайдено в таблицi траншiв CC_TRANS' );
   end;
   ...
   If l_sv1 > 0 and l_sv1 >= l_sv then
      raise_application_error(n_Err,s_Err  ||
      'сума видачi вiдокремленого траншу ' || to_char(l_sv1) || '
 >= суми видачi основного траншу '         || to_char(l_sv )  );
   end if;

   l_sz := nvl(l_trans.Sz,0);
   If l_sz1 > 0 and l_sz1 >= l_SZ then
      raise_application_error(n_Err,s_Err     ||
      'сума погашення вiдокремленого траншу ' || to_char(l_sz1) || '
 >= суми погаш. основного траншу '            || to_char(l_Sz )  );
   end if;

   If l_sv1 >  0 and p_D_plan1 is     null or
      l_sv1 <= 0 and p_D_plan1 is NOT null OR
      l_sv1 <  l_sz1                       then
      raise_application_error(n_Err,s_Err     ||
      'Помилка вiдокремленого траншу :
 сума видачi=' || to_char(l_sv1) || '
 сума погаш.=' || to_char(l_sz1) || '
 дата пог   =' || to_char(p_D_plan1, 'dd.mm.yyyy' ) );
   ...
   If p_d_PLAN <> l_trans.D_PLAN or l_sv1 > 0 or l_sz1 > 0 then
      ...
      update cc_trans set d_PLAN = p_D_PLAN,
                          sv     = sv - l_sv1 *100,
                          sz     = sz - l_sz1 *100
             where npp = p_NPP;
      ...
      If l_sv1 > 0  then
         ...
         INSERT INTO CC_TRANS (ref,         acc,         fdat,   sv     ,   sz     ,    D_PLAN  )
               values (l_trans.ref, l_trans.acc, l_trans.fdat, l_sv1*100, l_sz1*100, p_D_PLAN1 );
      end if;
...
begin
     update CC_TRANS set sz = nvl(sz,0) + p_SZ, d_fakt = p_D_FAKT, refp = p_REFP
            where npp = p_NPP;
     insert into  cc_trans_ref (npp, ref, s_ref) values (p_NPP, p_REFP, p_SZ);
end otm1_UPD;
--------------------------------------------------------------------
--Otm1_sel : Вычитка и проверка реквизитов о погашении  1-го  транша
--------------------------------------------------------------------
...
begin
   ...
   If nvl(p_SZ,0) <= 0 then
      raise_application_error(n_Err,s_Err || 'Пом. в Сумi погашення =' ||to_char(p_SZ/100));
   end if ;

   If p_refp is null then
      raise_application_error(n_Err,s_Err || 'Не указано реф.погашення ' );
   end if ;
   ---------------------------------------------
   begin
     ...
     If l_s1 < p_SZ then
        raise_application_error(n_Err,s_Err||'НЕпогашений залишок траншу='||to_char(l_s1/100)||'
               МЕНШЕ вашої суми= ' || to_char(p_SZ/100)   );
     end if;

     If l_s1 = p_SZ then p_D_FAKT := gl.BDATE ;
     else                p_D_FAKT := null     ;
     end if;
...
for p in (select acc, nvl( max(dapp), min(fdat)-1 ) DAPP
          ...
          where d_fakt is null
            and ( p_mode = 0 or acc in (select acc from nd_acc where nd=p_mode ) )
          group by acc )
loop
   for k in (select *
             from opldok  o
             where acc = p.acc and dk=1 and sos=5 and fdat > p.dapp
               and fdat >= G_startdat
	       --and fdat < gl.BDATE
   ...
   loop
     ...
     for m in (select * from CC_TRANS where acc=p.acc
                  and (d_fakt is null or sv>nvl(sz,0)) order by d_plan, fdat, ref )
     loop
        l_s1 := m.sv - nvl(m.sz,0)  ; -- столько надо
        l_s2 := least( l_s, l_s1)   ; -- столько можем дать

        If l_s1 = l_s2 then l_dfakt := k.fdat;
        else                l_dfakt := null;
        end if;
   ...
   update CC_TRANS set dapp = dat_next_u(gl.BDATE,-1) where acc=p.acc;
...
begin
  ...
  for k in (select t.npp, t.sv, t.sz, c.s_ref, t.sz - c.s_ref sz_old
     ...
     loop
        ...
        begin
          select nvl(max(ref),null) into l_refp from cc_trans_ref where npp = k.npp and ref <> p_REFP;
        exception when no_data_found then l_refp := null;
        end;
        ...
        delete from cc_trans_ref where npp = k.npp and ref = p_REFP;
     end loop;
...
If p_mode = 0 then
   ...
   delete from CC_TRANS_REF where npp in (select npp from CC_TRANS where acc in (select acc from nd_acc where nd = p_MODE));
   delete from CC_TRANS where acc in (select acc from nd_acc where nd = p_MODE) ;
end if;


-- выбрать все нужные ссудные счета - суть кредитные линии
for k in (select a.acc, a.daos, n.nd, a.ostc, a.mdate, a.tip, a.accc, a.nbs
          from accounts a, nd_acc n,  cc_deal d
          where d.vidd in (2,3) and d.sos<15
            and nvl(to_number(cck_app.get_nd_txt_ex(d.nd, 'PR_TR', bankdate)),0) = 1
            and a.acc = n.acc   and tip ='SS '  and n.nd  = d.nd
...
loop
   ...
   If l_fdat is not null then
      select ostf into l_ostf from saldoa where acc=k.acc and fdat=l_fdat;
   ...
   for r in (select * from opldok o
             where dk  = 0  and acc = K.ACC   and fdat >= k.daos and o.sos=5
               and not exists (select 1 from CC_TRANS where ref=o.REF and acc= o.ACC)
   ...
   for p in (select * from accounts a  where tip='SP '  and dazs is null
               and exists (select 1 from nd_acc where nd=k.ND and acc = A.acc) )
   loop
      INSERT INTO CC_TRANS (ref, acc, fdat, sv, D_PLAN )
      select ref, acc, fdat, s, fdat-1
      from opldok o where dk = 0 and acc = P.ACC and fdat >= p.daos and o.sos=5 and o.fdat >= G_startdat
       and not exists (select 1 from CC_TRANS where ref=o.REF and acc= o.ACC);
   end loop;               -- p
...
BEGIN
-- вынос на просрочку
for m in (select Npp, t.acc, (t.sv - nvl(t.sz,0) ) S    from CC_TRANS t
          where fdat<d_plan and d_fakt is null and cck_app.correctDate2(980,d_plan,1)<gl.bdate   and (sv - nvl(sz,0) ) > 0
----and acc = 294827
          order by acc )
loop
   --начало транзакции
   If l_acc <> m.acc then
      begin
        select a.accc , a.kv, a.isp  , a.grp, nvl(s.s080,'2') s080, a.mdate,   d.cc_id, d.nd, d.sdate, a.nls, substr(a.nms,1,38)
        into   l_acc8 , l_kv, l_isp  , l_grp, l_s080, l_mdate,               l_CC_ID, l_nd, l_sdate, l_nls, l_nms
        from accounts a, nd_acc n , cc_deal d, specparam s
        where a.ostc=a.ostb and a.acc= m.acc and a.acc=n.acc and n.nd=d.nd and a.acc=s.acc(+);
        l_acc := m.acc;
      EXCEPTION WHEN NO_DATA_FOUND THEN goto KIN7_;
      end;
   end if;
   cck.CC_ASP111( CC_KVSD8, gl.BDATE, 46, l_ND, l_acc8, l_KV, l_ISP, l_GRP, l_S080, l_mdate, l_nms, l_nls, l_CC_ID, l_sdate, m.s );

   if nvl(cck.G_REPORTS,0)=0
   then
     insert into operw (ref, tag, value) values ( gl.aRef, 'REF92', to_char(m.Npp) );
   end if;

   If i_Commit >= n_Commit and nvl(cck.G_REPORTS,0)=0
   then
     COMMIT;
  ...
  If i_Commit >= n_Commit and nvl(cck.G_REPORTS,0)=0 then     COMMIT;  l_KOL := l_KOL + i_Commit;       i_Commit :=0;   end if;
  l_KOL := l_KOL + i_Commit;
  if nvl(cck.G_REPORTS,0)=0 then  COMMIT; end if;
  i_Commit :=0;
