...
g_body_version  constant varchar2(64)  := 'version 2.50 09/09/2010';
...
begin
  ...
  bars_audit.info('BARS_MAIL: Сообщение помещено в очередь, ID='||rawtohex(p_msgid)
...
begin
  ...
  then
    logger.trace('BARS_MAIL: Начало обработки очереди e-mail сообщений');
  end if;
  ...
  while true loop
    begin
      ...
      if logger.trace_enabled
      then
        logger.trace('BARS_MAIL: Из очереди изъято сообщение, ID='||rawtohex(l_msgid)
      ...
      begin
        ...
        logger.info('BARS_MAIL: Отослано сообщение, ID='||rawtohex(l_msgid)
                        ||chr(10)||dump_mail_info(l_msg));
      exception when others then
        logger.error('BARS_MAIL: Ошибка при посылке сообщения ID='||rawtohex(l_msgid)||' по адресу '
