...
begin  
  ...
  begin select substr(trim(txt),1,1) into I_CR9 from nd_txt where tag='I_CR9' and nd= p_ND ;  exception when no_data_found then null;  end ;
...
begin 
  ...
  ----------------------------- accounts -8999 -------
  A8_new := A8_old;
  Op_Reg(99,0,0,0, p4_, dd_old.RNK, MSFZ9.Get_NLS(l_KV,'8999'), l_kv, A8_new.NMS, 'LIM', A8_new.isp, A8_new.acc);
  ...
  loop begin dop.ND :=DD_new.ND;  INSERT INTO nd_txt values dop; 
       exception when others then   if SQLCODE = -00001 then null;   else raise; end if;   
       end;
  end loop;
...
grant execute on bars.msfz9 to start1;
grant execute on bars.MSFZ9 to BARS_ACCESS_DEFROLE;