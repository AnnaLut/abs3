...
PROCEDURE PL_INS_S ( p_S number, p_RI varchar2 ) ;
...
--------------------------------------------------------------------------
PROCEDURE ISG1 (p_kvD IN int, p_nlsD IN varchar2, p_sD IN number, p_kvk IN int, p_nlsK IN varchar2, p_nazn IN varchar2 , p_REF OUT number);

--Актуализация текущим лимитом дня - вынесла в отдельную процеду
PROCEDURE LIM_BDATE (p_nd number, p_dat date);
...
----------------------------
PROCEDURE UPDATE_OBS (DAT_ date, ND_ int) ;
...
---------------------------------------------
FUNCTION FBS(ND_ int, ACC_ int, DAT_ date) return NUMBER;
-- фактична балансова вартiсть КД в вал КД

FUNCTION PBS(ND_ int, ACC_ int, DAT_ date) return NUMBER;
-- планова балансова вартiсть КД в вал КД
-----------------
PROCEDURE CC_WDATE (CUSTTYPE_ int, DAT_ date, Mode_ int);
...
-----------------
PROCEDURE CC_ASG_SN8 (ND_ int, NLS_6397 varchar2, NLS_8006 varchar2);
--Авто-погашение пени.
----------------
PROCEDURE CC_UPDATE
(ND_ int,
...
PROCEDURE AN_GPK(ND_ int, S1_ number, Den_ int );
--построение ГПК с суммой гашения 1 платежа (S1_) в указанный день(Den_)

FUNCTION R011_S181 ( acc_ int, ND_ int ) RETURN char ;
--Спецпарам R011 + S181

FUNCTION SUM_INT( KV8_ int , ND_ int, Dat_ date) RETURN number ;
...
---------------------------------------------
FUNCTION SUM_ASG( ND_ int, KV_ int ) RETURN number ;
...
----------------------------------------
FUNCTION CorrectDate( KV_ int, OldDate_ date, EndDate_ date) RETURN DATE ;
--Вычисление ближайшей рабочей даты
--------------------------
FUNCTION NLS0 ( ND_  int, TIP_ char ) RETURN varchar2;
...
---------------
FUNCTION  PMT(prcperiod_ number, kolrazb_ int, summakred_ number) RETURN NUMBER;
--        PMT(RATE_*(DAT4_-DAT3_)/365,KOL2_,-LIM2_))
--PRAGMA RESTRICT_REFERENCES ( PMT, WNDS );
--  вычисление константы для кредита
---------------
FUNCTION  PMT1(nR  number, nN  number, nPV number, nFV number) RETURN NUMBER;
--Calculate rent payment (Пономаренко Андрей)

---------------
FUNCTION  OST_V ( ACC_ INT, FDAT_ DATE) RETURN NUMBER;
...
-----------------
PROCEDURE cc_KV (ND_ int, KV1_ int, KV2_ int );
...
-------------
PROCEDURE cc_ANALIZ1 (BKV_ int, DAT1_ date, DAT2_ date );
-- процедура-1(более сложная по алгоритму ПЕТРОКОМЕРЦ) анализа КП
-----------------
PROCEDURE cc_open
(ND_ in OUT int , nRNK   int , CC_ID_  varchar2,
 Dat1 DATE,
 Dat4 DATE,
 Dat2 DATE,
 Dat3 DATE,
 nKV    int , nS     number, nVID   int,
 nISTO  int , nCEL   int   ,
 MS_NX  varchar2,
 nFIN   int , nOBS   int   , sAIM   varchar2,
 ID_    int , NLS  varchar2, nBANK  number  , nFREQ  int   ,
 dfPROC number, nBasey int, dfDen int, DATNP date, nFREQP int, nKom number) ;
-- процедура открытия КД
---------------------------
PROCEDURE cc_KOR
(ACC_    int    , -- ACC сч 8999
 ND_     int    , -- реф КД
...
-- Процедура открытия КД ОБНОВЛЕННАЯ
  ...
  (p_ND in OUT     int,    -- Реф кредитного договора (системный)
  ...
  (p_ND         int, -- Реф кредитного договора (системный)
...
---------------------
PROCEDURE cc_CLOSE (ND_ int, sERR_ OUT varchar2  ) ;
-- процедура закрытия КД
----------------------
PROCEDURE cc_autor (ND_ int, sAIM_  varchar2, UROV_ varchar2 ) ;
...
PROCEDURE MULTI_INT    (ND_ number,                      K1_ int, P1_ number, K2_ int, P2_ number, K3_ int, P3_ number, K4_ int, P4_ number ) ;
PROCEDURE BR_INT       (ND_ number, BR_ number );
---------------------------------------------------
PROCEDURE CC_DAY_LIM (FDAT_ date, NN_ int);
---
PROCEDURE RATE_LIM (ACC8_ int);
-- Переоценка остатка родительского счета при дочерних в другой валюте
----------------------
PROCEDURE CC_9129 (FDAT_ date, ND_ int, TIP_ int );
-- поддержка бс 9129
----------------------
procedure CC_9031(FDAT_ date, ND_ int, TIP_ int );
-- проводка по 9031 на сумму начального лимита по договору
-----------------------
procedure CC_CLOSE9031(FDAT_ date, ND_ int, TIP_ int );
-- снятие с 9031

---------------------------
PROCEDURE CC_grf_LIM (MODE_ int, ND_ int, ACC_ int, FDAP_ date);
...
----------------------
PROCEDURE cc_Sum_POG (DAT1_ date, DAT2_ date, nTip_Kl int);
...
-----------------------
PROCEDURE CC_SV12( ND1_ int, ND2_ int);
--обединить 2 КД
-----------------------
PROCEDURE CC_M_ACCP( mod_ int, ND_ int, ACCZ_ int);
--присoединить, oтсоединить залoговый счет
-------------------
PROCEDURE CC_9819(nd_ int,pr_ int);
-- ND_ - номер договора
...
--Учет количества договоров
 ...
 PROCEDURE CC_CRD (nd_ int,pr_ int,REFd_ int,NLS_ varchar2,GOLD_ varchar2,NDDOP_ number,
                  T9819_ varchar2 , O9819_ varchar2,FIO_ varchar2,VLASN_ varchar2, NAZN_ varchar2,CRDVD_ date,
                  CRDSN_ varchar2 ,CRDSK_ varchar2  );
--   ND_     номер договора
...
-----------------------
PROCEDURE CC_PRINT(mod_ int, dat1_ date, dat2_ date);
-- Печати разные
-- mod_  - режим
-- dat1_ - дата начала
-- dat2_ - дата конца
...
-----------------------
PROCEDURE CC_ASG (nREGIM_ int, mode_ int default 1) ;
...
PROCEDURE CC_ASP( p_ND int,DAY_ int);
   --авто перенос на просрочку основного долга

PROCEDURE CC_ASP111  (
  CC_KVSD8 varchar2, -- гл.пар CC_KVSD8=1 счет пени в вал КД. Иначе в нац.вал - незав от вал КД.
  DAT7_    date    ,
  p_VOB46  int     ,
  p_ND     number  , -- реф.КД
...
-----------------------
PROCEDURE CC_ASPN( CUSTTYPE_ int, ND_ int, DAY_ int) ;
--авто перенос на просрочку долга по процентам


PROCEDURE CC_ASPN_DOG( p_ND int, p_CC_ID in varchar2, p_okpo in varchar2, p_nmk in varchar2, DAY_ int, p_max number) ;
...
procedure CC_TMP_GPK
 (ND_      int,    -- реф КД
  ...
  SUMR_    number, -- Новый лимит по КД
...
CREATE OR REPLACE PACKAGE BODY BARS.CCK IS
  -------------------------------------------------------------------
  g_body_version CONSTANT VARCHAR2(64) := 'ver.3.24.0  12/01/2018 ';
  ------------------------------------------------------------------
  ...
  */
  ...
  /*
  31.01.2017 LSO Додано сортування при авторозборі. Курсор при достроковому погашенні
  23.01.2017 LSO Додано призначення платежу для ручного розбіру
  21.10.2016 Sta http://jira.unity-bars.com.ua:11000/browse/COBUSUPABS-4219
   ...
   21-08-2015 Sta Я тут маленькую ош нашла (в построении анутета с началом в будущем, просто такого ранее не встречалось) .
       Исправила  PDAT_ := CCK.f_dat (L_DD, TRUNC(greatest(GL.BDATE,p_datn) ,'MM'));
  ...
  */
  ...
  BEGIN
    ...
    IF TRIM(length(p_nam_b)) < 3
       OR TRIM(length(p_nam_b)) > 38 THEN
      raise_application_error(-20203
  ...
  BEGIN
    ...
    */
    ...
    LOOP
      ...
      LOOP
        EXIT WHEN j IS NULL;
        IF mas_obs_(i).rnk = mas_obs_(j).rnk
            AND mas_obs_(j).obs_new > obs_ THEN
          nd_bad_ := j;
      ...
      IF mas_obs_(i).obs_new = obs_
          AND mas_obs_(i).obs_old <> mas_obs_(i).obs_new THEN
        ...
        IF nvl(mas_obs_(i).obs_old, 0) <> obs_ THEN
          ...
          VALUES
            ...
            ,'Змiна ОБС.боргу з ' ||
             ...
             ' на ' || TRIM(substr(txt_, obs_ * 10 + 1, 10)) ||
               ...
               WHERE n.nd = nd_
                 ...
                 AND (s.acc, s.fdat) = (SELECT acc, MAX(fdat)
                                         ...
                                         WHERE acc = s.acc
                                           AND fdat <= dat_
                                         GROUP BY acc)
               ORDER BY decode(a.tip, 'LIM', 1, 2))
    LOOP
      IF k.tip = 'LIM'
         OR kv8_ IS NULL THEN
        kv8_ := k.kv;
      END IF;
  ...
  BEGIN
    ...
    FOR k IN (SELECT num FROM conductor ORDER BY num)
    LOOP
      fdat_ := dat_ + k.num;
      IF fdat_ > dat4_ THEN
        EXIT;
  ...
  BEGIN
    ...
    LOOP
      ...
      FOR k1 IN (SELECT a.tip, a.ostb, a.nls, a.nms, a.kv
                  ...
                  ORDER BY decode(a.tip, 'SN8', 1, 'SG ', 2, 3))
      LOOP

        IF k1.tip = 'SN8'
           AND k1.kv = gl.baseval THEN
          IF k1.ostb >= 0 THEN
            GOTO not_;
          END IF;
          nls_8008 := k1.nls;
  ...
  BEGIN
    ...
    IF nlsb_ IS NOT NULL
       AND mfob_ = f_ourmfo() THEN
      BEGIN
        SELECT acc
          INTO tempacc_
          FROM accounts
    ...
    IF dat4_ <> wdate_old THEN
      ...
      LOOP
        UPDATE accounts
           SET mdate = (CASE
                         WHEN k.tip = 'SS '
                              AND nvidd_ IN (2, 3, 12, 13)
                              AND mdate IS NOT NULL
                              AND mdate <> wdate_old
                              AND mdate < dat4_ THEN
                          mdate
                         ELSE
                          dat4_
                       END)
  ...
  BEGIN
    ...
    LOOP
      ...
      FOR k1 IN (SELECT kv, acc FROM accounts WHERE accc = acc8_)
      LOOP
        int_ := 0;
      ...
      IF zoirr_ = 1 THEN
        BEGIN
          IF gl.baseval = 980
             AND to_char(gl.bdate, 'yyyyMM') > to_char(dat_, 'yyyyMM') THEN
            BEGIN
              SELECT MAX(fdat)
                INTO vdat_
                FROM fdat
      ...
      IF sn_ = 0
         AND NOT (basey_ = 2 AND dat_ - acr_dat_ = 1 AND
  ...
  (
   ...
   ,kv1_ INT
   ,kv2_ INT
  ) IS --переустановка другой баз.валюты
  BEGIN
    raise_application_error(- (20203)
  ...
  BEGIN
    ...
    op_reg_ex(99
             ...
             ,ret_
             ,nrnk
             ,vkrzn(substr(gl.amfo, 1, 5), '89990' || ntmp_)
             , -----'89990'||ND_,
              nkv
             ,nms_
             ,'LIM'
  ...
  (
   ...
   , -- Цель кредитования
    p_ms_nx      VARCHAR2
   , -- Галузь (S260)       -- НЕ ИСП! ЧТО БЫ НИ ПЕРЕДАВАЛИ - БУДЕМ ВЫЧИТЫВАТЬ ИЗ CC_POTRA ВСЕ РАВНО!!!!!!
    p_fin        INT
   , -- Фин стан
    p_obs        INT
   , -- Обслуживание долга
  ...
  BEGIN
    ...
    IF nvl(p_nd, 0) = 0 THEN
      SELECT s_cc_deal.nextval INTO p_nd FROM dual;
    ELSE
    ...
    IF p_fin NOT IN (1, 2, 3, 4, 5) THEN
      l_err := l_err ||
               ...
               p_fin || chr(13);
    END IF;
    IF (p_ir IS NULL AND p_br IS NULL)
       OR p_basey IS NULL THEN
      l_err := l_err ||
    ...
    IF p_deni IS NOT NULL
       AND p_datfi IS NOT NULL THEN
  ...
  */
  ...
  BEGIN
    ...
    IF p_txt IS NULL
       AND p_psys IS NOT NULL
       AND p_id IS NULL THEN
  ...
  BEGIN
    ...
    IF r_aa.tip IN ('SS ', 'SP ', 'SL ')
       AND r_aa.accc IS NOT NULL THEN
      UPDATE accounts
         SET accc = NULL
       WHERE acc = p_acc
      ...
      FOR k IN (SELECT a.nls, a.kv
                 ...
                 WHERE a.acc = n.acc
                   ...
                   AND (a.ostc <> 0 OR a.ostb <> 0 OR a.ostf <> 0 OR
                       (a.dos + a.kos) > 0 AND a.dapp >= gl.bdate))
      LOOP
...
END IF;
    ...
    /*
       ...
       При цьому параметр VNCRP "Первинний ВКР", повинен заповнюватися автоматично і бути недоступним для редагування , при першому збереженні "Поточного ВКР" .
        ...
        begin select r.code into sTmp_ from  CCK_RATING r where r.code = sTmp_;
        EXCEPTION WHEN NO_DATA_FOUND THEN
              raise_application_error(-20203,'\8999-CCK.cc_autor:
    НЕдопустимий/НЕзаповнений дод.реквізит VNCRR = Поточний ВКР '|| sTmp_ ) ;
        end;
        ...
        EXCEPTION WHEN NO_DATA_FOUND THEN
              raise_application_error(-20203,'\8999-CCK.cc_autor:
    НЕдопустимий/НЕзаповнений дод.реквізит VNCRP = Первинний ВКР '|| sTmp_ ) ;
        end;*/
    /*
        COBUSUPABS-4376
        Забезпечення неможливості авторизації заявок на кредитні угоди (окрім кредитів по БПК),
...
ЗАБОРОНЕНО Авторизацію договора - клієнт являється ІНСАЙДЕРОМ/Ознака = ' ||
  ...
  (
   ...
   ,p4_ NUMBER
    ...
    rat_ VARCHAR2(10);
  BEGIN
    FOR k IN (SELECT kv FROM tabval)
    LOOP
      DELETE FROM nd_txt
       WHERE nd = nd_
         AND tag = 'P' || k.kv;
    ...
    LOOP
      ...
      IF k.sos >= 10 THEN
        ...
        IF s_ = ss_ THEN
          ...
          END IF;
          IF k.sos = 10
             AND (s_ != 0 OR k.wdate - fdat_ < 0) THEN
      ...
      FOR p IN (SELECT sumg, fdat
                 ...
                 WHERE nd = k.nd
                   AND fdat >= gl.bdate
                   AND nvl(otm, 0) <> 1
                 ORDER BY decode(par1_, '0', -1, +1) * (k.wdate - fdat))
      LOOP
        IF s1_ <= 0
           OR s1_ < p.sumg THEN
          s1_ := 0;
    ...
    LOOP
      BEGIN
        ...
        SELECT a9.acc
              ...
              ,s.r013
              ,-fost(a9.acc, fdat_)
              , -- текущий остаток 9129
               substr(TRIM(cck_app.get_nd_txt(k.nd, 'I_CR9')), 1, 1)
              ,nvl(cck_app.to_number2(cck_app.get_nd_txt(k.nd, 'D9129'))
                  ,0)
          INTO acc9_
              ,nls9_
              ,nms9_
      ...
      IF i_commit >= n_commit
         AND nvl(g_reports, 0) = 0 THEN
        COMMIT;
  ...
  BEGIN
    ...
    IF freq_ IN (1, 3, 5, 7, 180, 360) THEN
      ...
      BEGIN
        SELECT nvl(trunc(i.s), to_char(fdat_, 'dd'))
          INTO day_pog
          FROM nd_acc n, accounts a, int_accn i
         WHERE n.nd = nd_
           AND n.acc = a.acc
      ...
      LOOP
        ...
        IF datj_ < dat4_ THEN
          INSERT INTO cc_lim (nd, fdat, acc) VALUES (nd_, datj_, acc_);
          kol2_ := kol2_ + 1;
          dati_ := datj_;
        END IF;
      END LOOP;
    ...
    BEGIN
      SELECT lim2
       ...
       WHERE fdat = gl.bdate
         AND nd = nd_;
      UPDATE accounts SET ostx = -lim2_ WHERE acc = acc_;
    ...
    IF l_ssr = 0 THEN
      ...
      IF p_gpk = 4 THEN
        ...
        LOOP
          ...
          pdat_ := add_months(pdat_, 1);
        END LOOP;
        sum1_ := -round(cck.pmt1((p_ir / 100) / 12, kol_, p_lim2, 0), 0); -- ануитет

      ELSE
        kol_  := months_between(trunc(p_datk, 'MM'), trunc(p_datn, 'MM'));
        sum1_ := round(p_lim2 / (kol_), 0);
      END IF;
  ...
  BEGIN
    ...
    --logger.info('DP p_lim2='||p_lim2||'*p_gpk='||p_gpk||'*p_dd=' ||p_dd ||'*p_datn='||p_datn||'*p_datk='||p_datk||
    --              '*p_ir='  ||p_ir  ||'*p_pl1='||p_pl1||'*p_ssr='||p_ssr||'*p_ss='  ||p_ss  ||'*p_acrd='||p_acrd||'*p_basey='||p_basey);
  ...
  BEGIN
    /*
    logger.info ('CCK.CC_GPKM( MODE_=>'||MODE_||',  ND_=>'||nd_||',  ACC_=>'||acc_||
    ', BDAT_1=>'|| to_char( BDAT_1, 'dd.mm.yyyy') ||
    ', DATN_=>' || to_char( DATN_ , 'dd.mm.yyyy') ||
    ', DAT4_=>' || to_char( DAT4_ , 'dd.mm.yyyy') ||
    ', SUM1_=>' || SUM1_ ||', FREQ_=>' || FREQ_ ||', RATE_=>' || RATE_ ||', DIG_=>' || DIG_ );
    */
    BEGIN
      SELECT *
        INTO dd
    ...
    IF dd.vidd = 11
      ...
      UPDATE accounts SET vid = l_gpk WHERE acc = acc_;

      --Cделать курсор по датам изменения ставки - должно раб и для кла и для наст ануитета

      lim2_ := sum1_ * 100; -- новый лимит

      FOR k IN (SELECT greatest(bdat, bdat_1) bdat, ir
                  FROM int_ratn
                 WHERE acc = acc_
                   AND id = 0
                 ORDER BY 1)
      LOOP
        ...
        INSERT INTO cc_lim
          ...
          SELECT nd_, fdat, lim2, acc_, sumg, sumo, sumk
            FROM tmp_gpk
           WHERE fdat > k.bdat;
      END LOOP;
      RETURN;
    ...
    --ПЕРВАЯ  дата погашения
    datnk_ := cck_app.correctdate2(gl.baseval, datn_, l_daynp);
    ...
    -- и ПОСЛЕДНЯЯ
    dat4k_ := cck_app.correctdate2(gl.baseval, dat4_, l_daynp);
    UPDATE cc_lim
       SET sumg = 0, sumo = 0
     WHERE nd = nd_
       AND fdat = dat4k_;
    ...
    IF kol2_ <= 2 THEN
      ...
      LOOP
        BEGIN
          ...
          VALUES
            (nd_, datj_, acc_, 0, decode(day_pog_sn, NULL, NULL, 1));
        EXCEPTION
          WHEN dup_val_on_index THEN
            UPDATE cc_lim
      ...
      BEGIN
        ...
        SELECT metr, nvl(acrn.fprocn(acc, 2, datn_), 0)
          INTO metr96_, irk_
          FROM int_accn
         WHERE acc = acc_
           AND id = 2
      ...
      IF mode_ = 1 THEN
        ...
        UPDATE cc_lim
         ...
         WHERE acc = acc_
           AND nd = nd_
           AND fdat = dat4k_;
        UPDATE cc_lim SET sumo = sumg WHERE acc = acc_;

      ELSIF mode_ = 2 THEN
        ------------------------------------------- одной суммой в конце срока
        IF metr96_ = 96 THEN
          UPDATE cc_lim
             SET sumg = decode(fdat, dat4k_, lim2_, 0), sumk = irk_ * sum1_
           WHERE nd = nd_
             AND fdat >= datn_;
        ELSE
          UPDATE cc_lim
      ...
      ELSE
        ...
        SELECT COUNT(*)
         ...
         WHERE nd = nd_
           AND fdat >= datnk_;
        DECLARE
          pv_    NUMBER := sum1_ * 100; --  Текущая стоимость = сумма кредита
          fv_    NUMBER := 0; --  Будущая стоимость=0
          r_     NUMBER; ----  RATE_/100 ; --  Годовая % ст (коеф)
          n_     NUMBER := (dat4k_ - bdat_1) / 365; --  срок операции (лет)
        ...
        BEGIN
          SELECT round(decode(i.basey, 3, 365 / 360, 1), 2)
                ,decode(a.ostc, 0, rate_, acrn.fprocn(acc_, 0, bdat_1)) / 100
            INTO g_, r_
            FROM int_accn i, accounts a
           WHERE a.acc = acc_
             AND i.id = 0
          ...
          LOOP
            ...
            so_  := 0;
            sk0_ := 0;
            IF k.fdat > bdat_1 THEN
              -- не первая банковская дата, начислить %
              acrn.p_int(acc_, 0, fdat1_, k.fdat - 1, int_, -pv_, 0);
              int_ := round(-int_, 0);
              IF irk_ > 0 THEN
                acrn.p_int(acc_, 2, fdat1_, k.fdat - 1, sk0_, -pv_, 0);
                sk0_ := round(-sk0_, 0);
              END IF;
      ...
      IF day_pog_sn IS NOT NULL
         OR (nfreqp != nfreq) THEN
        kol2_      := 0;
        datn_sn    := nvl(datn_sn, datn_);
        fdat_      := cck_app.check_max_day(datn_sn
                                           ,day_pog_sn
                                           ,-2
                                           ,gl.baseval);
        day_pog_sn := nvl(day_pog_sn, day_pog);
        WHILE (datn_sn > fdat_ OR kol2_ = 0)
              AND nfreqp > 3
        LOOP
          fdat_ := cck_app.check_max_day(add_months(fdat_, kol2_)
                                        ,day_pog_sn
    ...
    --переливка в лимиты
    cck.cc_gpk_lim(nd_, acc_, bdat_1, datn_, sum1_);
  ...
  BEGIN
    ...
    FOR k IN (SELECT fdat, ost FROM cck_int WHERE osti IS NULL ORDER BY fdat)
    LOOP
  ...
  BEGIN
    ...
    SELECT acc
     ...
     WHERE nd = nd2_
       AND rownum = 1;
    FOR k IN (SELECT acc FROM accounts WHERE accc = acc1_)
    LOOP
  ...
  BEGIN
    ...
    IF sos_kd < 10 THEN
      logger.trace('CCK.CC_CRD: Договiр "' || nls_new.gold ||
                   '" не авторизований!');
      raise_application_error(- (20000 + 6)
                             ,'\8999 CCK.CC_CRD: Договiр "' ||
                              nls_new.gold || '" не авторизований!'
                             ,TRUE);
    END IF;
    IF sos_kd > 14 THEN
      logger.trace('CCK.CC_CRD: Договiр "' || nls_new.gold ||
    ...
    IF mod_ = 1 THEN
      ...
      FOR k IN (SELECT acc, kv
                 ...
                 WHERE kv <> 980
                   ...
                   AND (dazs IS NULL OR dazs >= dat1_ AND dazs <= dat2_))
      LOOP
        BEGIN
          --эквивалент на DAT1_
          SELECT ostf - dos + kos
  ...
  BEGIN
    ...
    BEGIN
      ...
      SELECT funcname
        INTO p_str
        FROM (SELECT id, funcname
                FROM v_cck_rep_list_fl
              UNION
              SELECT id, funcname
                FROM v_cck_rep_list_yl)
       WHERE id = p_id
         AND rownum = 1;
  ...
  (
    ...
    */
    ...
    l_ndi   NUMBER;
    l_cc_id VARCHAR2(50);
    l_sdate DATE; --    досрочное погашение
  BEGIN
    BEGIN
    ...
    LOOP
      ...
      ELSE
        ...
        */
        ...
        IF k.sos = 10
           AND k.wdate >= gl.bdate
           AND k.blk IN (10, 11, 14, 15) THEN
          IF pl_den_ = 0 THEN
            GOTO met_kon;
        ...
        LOOP
          ...
          IF p.tip = 'SN ' THEN
            ...
            ------------------------------------  SK4   --------------------------------------------
          ELSIF p.tip = 'LIM'
                AND (k.blk = 0 OR k.blk IN (1, 2, 10) AND pl_den_ = 1) THEN
          ...
          IF s_ > 0 THEN
            ...
            ELSE
              ...
              END IF;
            END IF;
            --NAZN_:= NAZN_ ||  ' згiдно угоди '||p.CC_ID || ' вiд '||to_char(p.SDATE,'dd/mm/yyyy');
            --COBUSUPABS-4952
            IF substr(p.prod, 1, 6) IN ('206219', '206309')
               AND p.ndi IS NOT NULL
               AND p.nd <> p.ndi THEN
              BEGIN
                SELECT cc.ndi
                  INTO l_ndi
              ...
              EXCEPTION
                WHEN no_data_found THEN
                  ...
                  l_sdate := p.sdate;
              END;
              nazn_ := nazn_ || ' згiдно угоди ' || l_cc_id || ' вiд ' ||
    ...
    FOR k IN (SELECT a.acc8
               ...
               WHERE nd_ IN (0, d.nd)
                 ...
                 AND (l_vidd IS NULL OR l_vidd = 1 AND d.vidd IN (1, 2, 3) OR
                     l_vidd = 11 AND d.vidd IN (11, 12, 13)))
    LOOP
      /*
  ...
  END;  -- блок пересчета проц
   ...
   update accounts set vid = nVID_ where acc = ACC8_ ; SUMO_CF := null;
  ...
  BEGIN
    ...
    EXCEPTION
      WHEN no_data_found THEN
        RETURN;
    END;
    SELECT nvl(MIN(fdat), dat3_), nvl(MAX(fdat), dat4_)
      INTO dat_min, dat_max
      FROM cc_lim
     WHERE nd = nd_;
    ...
    FOR k IN (SELECT fdat
              ...
              SELECT add_months(dat1_, num) fdat
               ...
               ORDER BY 1)
    LOOP
      acrn.p_int(acc8_, 0, fdat_, k.fdat - 1, ntmp_, -lim_, 0);
      ntmp_ := round(-ntmp_, 0);
      IF irk_ > 0
         AND metr96_ <> 96 THEN
        acrn.p_int(acc8_, 2, fdat_, k.fdat - 1, ktmp_, -lim_, 0);
        ktmp_ := round(-ktmp_, 0);
      END IF;
      ...
      IF k.gpk = 1 THEN
        ...
        IF k.fdat = dat_max THEN
          ...
          IF mode2_ <> '0'
             AND dat_min > dat1_ THEN
            acrn.p_int(acc8_, 0, dat1_, dat_min - 1, ntmp_, NULL, 0);
            ntmp_ := round(-ntmp_, 0);
            IF irk_ > 0 THEN
              acrn.p_int(acc8_, 2, dat1_, dat_min - 1, ktmp_, NULL, 0);
              ktmp_ := round(-ktmp_, 0);
            END IF;
            UPDATE cc_lim
      ...
      fdat_ := k.fdat;
    END LOOP;
    UPDATE accounts SET vid = nvid_ WHERE acc = acc8_;
    sumo_cf := NULL;
