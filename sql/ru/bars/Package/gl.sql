  ...
  begin
    ...
    sys.dbms_session.set_context( 'bars_gl', 'bankdate', to_char(p_bnk_dt,'mm/dd/yyyy')
     ...
     is
         ...
         l_clientid   varchar2(64) := bars_login.get_session_clientid;
     begin
        l_mfog := sys_context('bars_context','glb_mfo');
        --
        if l_mfog is null
        then
            begin
               select substr(val, 1, 6)
                 into l_mfog
                 from params$global
                where par = 'GLB-MFO';
            exception
                   when no_data_found then
                       raise_application_error(-20000, 'Parameter GLB-MFO is not defined');
            end;
        end if;
        --
        -- устанавливаем пока только банковскую дату головного банка
        --
        -- BANKDATE
        l_bankdate := sys_context('bars_gl', 'bankdate');
        --
        if l_bankdate is null
        then
              begin
                select substr(val, 1, 10)
                  into l_bankdate
                  from params$base
                 where par = 'BANKDATE'
                   and kf = l_mfog;
              exception
                    when no_data_found then
                        raise_application_error(-20000, 'Parameter BANKDATE is not defined');
              end;
                  end if;
             gl.bDATE := to_date(l_bankdate, 'MM/DD/YYYY');
             sys.dbms_session.set_context('bars_gl', 'bankdate', l_bankdate, client_id=> l_clientid);
   ...
   BEGIN
      SELECT val INTO gl.baseval
        FROM params WHERE par='BASEVAL';
   EXCEPTION WHEN NO_DATA_FOUND THEN gl.baseval := 980;
   END;


   BEGIN
      SELECT TRIM(val) INTO pay_schema
        FROM params WHERE par = 'PYSCHEMA';
   EXCEPTION WHEN NO_DATA_FOUND THEN pay_schema := 3;
   END;
    ...
    if (sys_context('bars_context', 'user_branch')='/' or
        sys_context('bars_context', 'user_branch') is null) then -- для внешних пользователей (не из таблицы STAFF)

        -- для корневого пользователя устанавливаем реквизиты головного банка
        init_root();
        --
    ...
    gl.bDATE:=to_date(sys_context('bars_global', 'user_bankdate'),'MM/DD/YYYY');



    begin
       select substr(val, 1, 10) into bankdate_
         from params
        where par = 'BANKDATE';


    exception when NO_DATA_FOUND then
        erm := 'Parameter BANKDATE is not defined';
        raise err;
    end;

    gl.gbDATE := to_date(bankdate_,'MM-DD-YYYY');


    -- установка глобальной  банковской даты
    sys.dbms_session.set_context('bars_gl', 'bankdate', bankdate_, client_id=> l_clientid);



    -- при представлении корневых пользователей отделением без указания
    -- банковской даты устанавливаем текущую банковскую дату отделения
    if (gl.bDATE is null and sys_context('bars_context', 'user_branch') != '/') then
       gl.bDATE := TO_DATE(bankdate_,'MM-DD-YYYY');

    end if;

    BEGIN
       SELECT val INTO  aOKPO
         FROM params WHERE par='OKPO';
    EXCEPTION WHEN NO_DATA_FOUND THEN NULL;
    END;

    BEGIN
       SELECT val INTO  aRNK
         FROM params WHERE par='OUR_RNK';
    EXCEPTION WHEN NO_DATA_FOUND THEN gl.aRNK := 1;
    END;
    ...
    -- Устанавливаем признак инициализации
    sys.dbms_session.set_context('bars_gl', 'last_init', to_char(sysdate, 'dd.mm.yyyy hh24:mi:ss'), client_id=>l_clientid);
...
BEGIN
   ...
   ELSE
      BEGIN
         SELECT id INTO id_ FROM staff WHERE id=uid_;
      EXCEPTION
         WHEN NO_DATA_FOUND THEN
...
SELECT ref,fdat,tt,dk,acc,s,sq,stmt,
       txt,NVL(sos,0) sos,rowid FROM opldok
 WHERE ref = ref_ AND NVL(sos,0) <= lev_ ORDER BY stmt,sos
   FOR UPDATE OF ref NOWAIT;

CURSOR c1 IS
SELECT fdat FROM opldok
 WHERE ref = ref_ GROUP BY fdat ORDER by fdat;
...
bars_audit.info('BCK: STARTED ref='||ref_||',bDATE='||gl.bDATE||',lev='||lev_);
   ...
   IF lev_>=5 THEN -- ful_bak
      BEGIN
         SELECT 1 INTO sos_ FROM dual
          WHERE EXISTS(SELECT 1 FROM opldok WHERE ref = ref_ AND sos=5 AND fdat>gl.bDATE);

          raise_application_error(-(20000+ern),
         '\9323 - Unable to ful_bak transaction dated after #'||TO_DATE(gl.bDATE,'dd-mm-yyyy'), TRUE);
