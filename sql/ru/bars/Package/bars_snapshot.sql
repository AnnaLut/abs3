...
is
  ...
  -- constants
  --
  g_body_version          constant varchar2(64)  := 'version 1.4 27.10.2016';
  ...
  is
  begin
    return 'Package BARS_SNAPSHOT header '||g_header_version;
  end header_version;
  ...
  is
  begin
    return 'Package BARS_SNAPSHOT body '||g_body_version;
  end body_version;
  ...
  */
    title  constant  varchar2(60) := 'bars_snapshot.daily_currency_revaluation';
    l_bank_dt        date;
    l_first_day      date;
  ...
  %usage   переоцінка валютних позицій коригуючими проводками.
  */
    title  constant  varchar2(60) := 'bars_snapshot.currency_revaluation';
    l_bank_dt        date;
    l_first_day      date;
    l_ref            bars.oper.ref%type;
    l_dk             bars.oper.dk%type;
  ...
  begin
    ...
    for cur in ( select t.BRANCH, 'Finis/Реал.Курс.Рiзниця для рах.'||t.NLS||'/'||t.KV as PAYMENT_DESC
                   ...
                   from ( select v.ACC3801, v.ACC_RRR
                            ...
                            join BARS.AGG_MONBALS_EXCHANGE b0
                              on ( b0.acc = v.ACC3800 and b0.FDAT = l_first_day )
                            join BARS.AGG_MONBALS_EXCHANGE b1
                              on ( b1.acc = v.acc3801 and b1.FDAT = l_first_day )
  ...
  */
    ...
    title   constant  varchar2(60) := 'bars_snapshot.create_monthly_snapshot';
  ...
  */
    title   constant  varchar2(60) := 'bars_snapshot.create_annual_snapshot';
  ...
  BEGIN
    ...
    then  -- for all KF
      ...
      select LISTAGG(to_char(k.FDAT,'dd-mm-yyyy'),', ') WITHIN GROUP (order by k.FDAT)
       ...
       where Not Exists ( select 1 from BARS.AGG_MONBALS where FDAT = k.FDAT and KF = l_kf );
      ...
      execute immediate 'TRUNCATE TABLE BARS.AGG_YEARBALS_EXCHANGE';
      
    end if;
    
    -- Фіксуємо SCN на якому формуємо знімок балансу по табл. SALDOY
    BARS_UTL_SNAPSHOT.set_table_scn( 'SALDOY', l_jan_01_dt, dbms_flashback.get_system_change_number() );
    ...
    */
    ...
    then  -- for all KF
      ...
      execute immediate 'TRUNCATE TABLE BARS.AGG_YEARBALS_EXCHANGE_KF';
  ...
  */
    title   constant  varchar2(60) := 'bars_snapshot.removing_obsolete_dm';
  begin
