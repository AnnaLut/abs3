...
begin
  ...
  begin
    ...
    execute immediate 'begin bars_web_ui.select_function(:b1, :b2); end;' using str_page||str_query, g_host_data;
  ...
  select count(*)
    into l_cnt
    from operlist_acspub
   where frontend = 1
     and funcname like str_page || '%'
     and bars_regexp.match('^' || replace(funcname, '?', '\?') || '$',
                           str_page || str_query) = 1;
  ...
  -- Смотрим на функции пользователя
  select count(1)
    into l_cnt
    from (select codeapp
            from applist_staff
           where id = l_usrid
             and decode(l_state, 0, 1, approve) = 1
             and date_is_valid(adate1, adate2, rdate1, rdate2) = 1) aps,
         (select codeoper
            from operlist
           where funcname like str_page || '%'
             and bars_regexp.match('^' || replace(funcname, '?', '\?') || '$',
                                   str_page || str_query) = 1) o
   where exists (select 1
            from operapp_acs oaa
           where oaa.codeapp = aps.codeapp
             and oaa.codeoper = o.codeoper)
     and rownum = 1;
  ...
  -- Смотрим на делегированные функции
  select count(1)
    into l_cnt
    from (select codeapp
            from applist_staff
