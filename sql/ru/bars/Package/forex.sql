CREATE OR REPLACE PACKAGE BODY BARS.forex is
  ...
  g_body_version    constant varchar2(64)  := 'version 1.3.3 10.03.2017';

/*
 10.03.2017 Sta закоментарено
...
for xx in (select x.* 
           ...
           where x.swap_tag is null 
             and FOREX.get_forextype3K(x.deal_tag)  = l_kod 
...
--logger.info('FFF *'|| p_mode||'*'||p_SWAP_TAG ||'*'||p_DEAL_TAG ||'*');
...
is
  ...
  l_p4  number;
begin
  select f_newnls2(null, l_tip, p_nbs, p_rnk, null, p_kv),
         substr(f_newnms (null, l_tip, p_nbs, p_rnk, null),1,70)
    into l_nls, l_nms
    from dual;
  -- открытие счета
  op_reg_ex(99, 0, 0, null, l_p4, p_rnk, l_nls, p_kv, l_nms, l_tip, l_isp, l_acc);
  -- спецпараметры
  set_sparam(l_acc);
  return l_acc;
end open_accf;
...
begin
  ...
  select decode(p_kvtype, 'A', decode(l_dealtype,g_fxmode_swap,s9av,s9a), decode(l_dealtype,g_fxmode_swap,s9pv,s9p)) into l_column
    from forex_ob22
   where kod = decode(l_dealtype,g_fxmode_forex,p_fxtype,'SWAP_'||p_fxtype);

  l_nbs  := substr(l_column, 1, 4);
  l_ob22 := substr(l_column, 6, 2);
  if l_nbs is null then
     raise_application_error(-20000, 'Невідомий тип FOREX-угоди');
  end if;
  ...
  insert into fx_deal_acc (id, rnk, fx_type, kv_type, kv, acc9)
  values (l_id, p_rnk, p_fxtype, p_kvtype, p_kv, l_acc);
...
begin
  begin
     select f.acc9 into l_acc
       from fx_deal_acc f, accounts a
      where f.rnk = p_rnk
        and f.fx_type = p_fxtype
        and f.kv_type = p_kvtype
        and f.kv = p_kv
         -- счет не занят
        and not exists (select 1 from fx_deal where decode(p_kvtype,'A',acc9a,acc9b) = f.acc9 and (dat_a >= bankdate or dat_b >= bankdate) )
         -- без остатка
        and fost(f.acc9, bankdate) = 0
        and f.acc9 = a.acc
        and a.ostb = 0
        and (a.dapp is null or a.dapp < bankdate-10)
        and rownum = 1;
  exception when no_data_found then
     FOREX.open_accP (p_swaptag, p_rnk, p_fxtype, p_kvtype, p_kv, l_acc);
  end;
  -- обнуляем % ставку
  ...
  begin
     select i.bdat, i.ir, br into l_dat, l_ir, l_br
      ...
      where i.acc = l_acc
        and i.id = l_id
        and i.bdat = ( select max(bdat) from int_ratn where acc = i.acc and id = i.id );
     if l_dat = gl.bdate and ( nvl(l_ir,0) <> 0 or nvl(l_br,0) <> 0 ) then
        update int_ratn set ir = 0, br = null, op = null where acc = l_acc and id = l_id and bdat = l_dat;
     elsif l_dat <> gl.bdate then
        insert into int_ratn (acc, id, bdat, ir, br, op)
        values (l_acc, l_id, gl.bdate, 0, null, null);
     end if;
  exception when no_data_found then null;
  end;
  return l_acc;
...
begin
  ...
  if p_fxdeal.acc9a is null or p_fxdeal.acc9b is null then
     forex.get_fxacc(p_fxdeal.swap_tag, p_fxdeal.rnk, p_fxtype, p_fxdeal.kva, p_fxdeal.kvb, l_acca, l_accb);
  ...
  if l_oper.nlsa is null or l_oper.nlsb is null then
     raise_application_error(-20000, 'Неизвестны счета для оплаты! Не заполнен справочник счетов Spot/Forward.');
  end if;

  l_oper.tt := p_tt;
  select nlsk into l_tt_nls from tts where tt = l_oper.tt;
  if (l_tt_nls like '#%') then
      execute immediate 'select ' || substr(l_tt_nls, 2) || ' from dual'
      into l_tt_nls;
  end if;
  ...
  if p_fxdeal.deal_tag is null then
     ...
     set_doc (gl.bdate, gl.bdate, l_oper.tt, l_oper.vob, 1,
        ...
        p_cust.nmk, l_oper.nlsb, g_bankmfo, g_bankokpo,
...
begin
  ...
  -- sFX_SPOT = 'FX5' как FORWARD
  if g_FX_SPOT is not null and g_FX_SPOT = 'FX5' then

     pay_forward_vn(p_fxdeal, g_fxtype_spot, p_cust, 'FX5', p_nazn);

  -- sFX_SPOT = 'FXO' - без 9 кл.
  elsif g_FX_SPOT is not null and g_FX_SPOT = 'FXO' then
...
begin
  ...
  -- 1.1, 1.1a - TOD, SPLIT
  if p_fxtype = g_fxtype_tod or p_fxtype = g_fxtype_split then
  ...
  -- 1.3 - FORVARD
     ...
     pay_forward(p_fxdeal, p_cust, p_nazn);
  end if;
...
begin
  ...
  exception when no_data_found then  raise_application_error(-20000, 'Не указан торговый счет в справочнике!');
  end;
  ...
  select substr(nvl(c.nmkk,c.nmk),1,38), c.okpo, t.rezid, b.kod_b, c.country
   ...
   where c.rnk = p_fxdeal.rnk
     and c.rnk = b.rnk
     and c.codcagent = t.codcagent;
...
begin
  if p_ref is not null then
     begin
        insert into fx_deal_ref (deal_tag, ref)
        values (p_deal_tag, p_ref);
...
begin
  ...
  bars_audit.info( 'forex.create_deal: Entry with'
     || chr(10) || '( p_dealtype    = ' || p_dealtype
     || chr(10) || ', p_mode        = ' || p_mode
     || chr(10) || ', p_swap_tag    = ' || p_swap_tag
     || chr(10) || ', p_ntik        = ' || p_ntik         
     || chr(10) || ', p_dat         = ' || to_char(p_dat,'dd.mm.yyyy') 
     || chr(10) || ', p_kva         = ' || p_kva          
     || chr(10) || ', p_data        = ' || to_char(p_data,'dd.mm.yyyy') 
     || chr(10) || ', p_suma        = ' || p_suma         
     || chr(10) || ', p_sumc        = ' || p_sumc         
     || chr(10) || ', p_kvb         = ' || p_kvb          
     || chr(10) || ', p_datb        = ' || to_char(p_datb,'dd.mm.yyyy')         
     || chr(10) || ', p_sumb        = ' || p_sumb         
     || chr(10) || ', p_sumb1       = ' || p_sumb1        
     || chr(10) || ', p_sumb2       = ' || p_sumb2        
     || chr(10) || ', p_rnk         = ' || p_rnk          
     || chr(10) || ', p_nb          = ' || p_nb           
     || chr(10) || ', p_kodb        = ' || p_kodb         
     || chr(10) || ', p_swi_ref     = ' || p_swi_ref      
     || chr(10) || ', p_swi_bic     = ' || p_swi_bic      
     || chr(10) || ', p_swi_acc     = ' || p_swi_acc      
     || chr(10) || ', p_nlsa        = ' || p_nlsa         
     || chr(10) || ', p_swo_bic     = ' || p_swo_bic      
     || chr(10) || ', p_swo_acc     = ' || p_swo_acc      
     || chr(10) || ', p_nlsb        = ' || p_nlsb         
     || chr(10) || ', p_b_payflag   = ' || p_b_payflag    
     || chr(10) || ', p_agrmnt_num  = ' || p_agrmnt_num   
     || chr(10) || ', p_agrmnt_date = ' || to_char(p_agrmnt_date,'dd.mm.yyyy') 
     || chr(10) || ', p_interm_b    = ' || p_interm_b     
     || chr(10) || ', p_alt_partyb  = ' || p_alt_partyb   
     || chr(10) || ', p_bicb        = ' || p_bicb         
     || chr(10) || ', p_curr_base   = ' || p_curr_base    
     || chr(10) || ', p_telexnum    = ' || p_telexnum     
     || chr(10) || ', p_kod_na      = ' || p_kod_na       
     || chr(10) || ', p_kod_nb      = ' || p_kod_nb       
     || chr(10) || ', p_field_58d   = ' || p_field_58d    
     || chr(10) || ', p_vn_flag     = ' || p_vn_flag      
     || chr(10) || ', p_nazn        = ' || p_nazn || ' ).' );
  
  l_dealtype := p_dealtype;
  ...
  then
    ...
    begin
        ...
        if l_fxdeal.dat_a > l_data
        or l_fxdeal.dat_b > l_datb
        or l_fxdeal.dat_a = l_data and l_fxdeal.dat_b = l_datb then
...
---- 04/10/2016 Sta Счета крд и деб задолж по переоценке до справ.стоимости - в разрезе контрагентов
  ...
  If NVL(L_PVq,0) = 0 and  p_IRAE > 0 and p_IRBE > 0  then
     ...
     FOREX.ADD_IR (xx.dat_A, l_DAT, xx.kva , p_IRAE) ;
     FOREX.ADD_IR (xx.dat_B, l_DAT, xx.kvB , p_IRBE) ;
