...
is
  head_ver  constant varchar2(64)  := 'version 1.15 28.07.2014';
  ...
  --
  function clob2str( p_clob clob, p_str_size number ) return varchar2_list  pipelined;

  --
  -- повертає insert на заповнення вказаної таблиці
  --
  function get_insert4table( p_tab_name    all_tab_cols.table_name%type,  -- назва таблиці
                             p_condition   varchar2,                      -- умова відбору записів з табоиці
                             p_offset      number  default 0              -- к-ть пробілів для відступу
                           ) return clob;
...
-- constants
--
body_ver  constant varchar2(64)  := 'version 1.21 27.11.2015';
...
function get_insert4table( p_tab_name    all_tab_cols.table_name%type,
...
is
  ...
  l_pkey_cols_tab   VARCHAR2(200); -- dbms_utility.uncl_array;
  --
  l_Cursor          integer;
  l_status          integer;
  ...
  begin
    -- якщо поле перв.ключ
    if (p_col_name = l_pkey_cols_tab) then
      l_sTmp3 := l_sTmp3 || p_col_name || ' = ' || chr(39) || dblquote(p_col_value) || chr(39);
    else
  ...
  begin
    -- якщо поле перв.ключ
    if (p_col_name = l_pkey_cols_tab) then
      l_sTmp3 := l_sTmp3 || p_col_name || ' = ' || to_char(p_col_value);
    else
  ...
  begin
    -- якщо поле перв.ключ
    if (p_col_name = l_pkey_cols_tab) then
      l_sTmp3 := l_sTmp3 || p_col_name || ' = to_date(' || chr(39) || to_char(p_col_value, 'dd/mm/yyyy') || chr(39) || ',''dd/mm/yyyy'')';
    else
...
begin
  ...
  bars_audit.trace( '%s Start with: tab_name = %s, condition = %s', title, nvl(p_tab_name,'null'), nvl(p_condition,'null') );
  ...
  if ( p_offset > 0 )
  then
    -- l_offset := SubStr(Rpad(' ', p_offset), 1, 10);
    l_offset := Rpad(' ', p_offset);
  else
    l_offset := '';
  end if;

  -- пошук унікального ключа таблиці і полів які у нього входять
  begin
    select cc.column_name
      -- bulk collect
      into l_pkey_cols_tab
      from all_constraints c,
           all_cons_columns cc
     where c.owner = 'BARS'
       and c.table_name = UPPER( p_tab_name )
  ...
  exception
    when NO_DATA_FOUND then
    -- якщо ключа немає то NULL
      null;
    when TOO_MANY_ROWS then
      null;
  end;

  -- дізнаємося структуру талиці (назву та типи полів)
  select COLUMN_NAME, DATA_TYPE
    bulk collect
    into l_cols_list_tab, l_cols_type_tab
    from ALL_TAB_COLS
   where table_name = UPPER( p_tab_name )
   order by column_id;

  -- отримуєм к-ть полів (l_cols_count) та перелік полів таблиці розділених комою (l_cols_list_str)
  dbms_utility.table_to_comma(l_cols_list_tab, l_cols_count, l_cols_list_str);

  l_Cursor := dbms_sql.open_cursor;

  if p_condition is null then
    l_sTmp := ( 'select ' || l_cols_list_str || ' from ' || p_tab_name );
  else
    l_sTmp := upper(p_condition);
    l_sTmp := replace(p_condition, 'WHERE');
    l_sTmp := ( 'select ' || l_cols_list_str || ' from ' || p_tab_name || ' where ' || l_sTmp );
  end if;
  ...
  LOOP
    ...
    l_text := l_text || l_offset ||'  Insert into ' || p_tab_name           ||nlchr;
    ...
    loop
      case
        ...
        when l_cols_type_tab(col) in ('DATE') then
          ...
          if l_ValueDate is null then
            l_sTmp  := l_sTmp  || 'null';
            l_sTmp2 := l_sTmp2 || l_cols_list_tab(col) || ' = null';
          else
            -- Обробка годин, хвилин і секунд (якщо є то повн. формат - інакше тільки дата)
            if (l_ValueDate > trunc(l_ValueDate)) then
...
begin
  ...
  exception
    when no_data_found then
      raise_application_error(-20000, 'Рахунки по вкладу не реанімовані!');
  end;
...
BEGIN
   ...
   BEGIN
      ...
      THEN
         UPDATE bars.dpt_vidd_scheme
            SET flags = p_flags
          WHERE vidd = p_vidd AND id = p_template;
      ELSE
         UPDATE bars.dpt_vidd_scheme
            SET flags = p_flags
          WHERE vidd = p_vidd AND id_fr = p_template;
      END IF;
   EXCEPTION
