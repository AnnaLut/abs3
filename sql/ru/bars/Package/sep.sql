...
G_HEADER_VERSION  CONSTANT VARCHAR2(64)  := 'version 6.27 09/09/2013';
...
G_BODY_VERSION  CONSTANT VARCHAR2(100)  := '$Ver: 6.58.13720 2016-05-27 09:43:25Z michael.martseniuk SEP_PACK.SQL $';
         ...
         IF  ref_ IS NOT NULL THEN
             UPDATE oper
                SET userid=DECODE(NVL(userid,0),0,gl.aUID,userid),
                 ...
                 ref_a=ref_a_,id_o=id_o_,sign=sign_,
                 nd    = nd_,
...
-- ----------------------------------------------------------------
   ...
   SELECT s_arc_rrp.NEXTVAL INTO rec_ FROM dual;
...
-- ------------------------------------------------------------------
   ...
   VALUES (rec_,ref_,
          ...
          nam_a_,nam_b_,nazn#,naznk#,nazns_,id_a#,id_b#,id_o_,ref_a_,bis_,
...
--    dbms_output.put_line('__'||mfob_||' '||nlsb_||' '||tip_b_||' '||ostcb_);
      ...
      begin
         ...
         VALUES (ref_,'KLR',6,ref_,dk_,SYSDATE,gl.bDATE,gl.bDATE,
