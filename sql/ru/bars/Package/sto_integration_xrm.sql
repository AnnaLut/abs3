 ...
 G_HEADER_VERSION  CONSTANT VARCHAR2(64)  :=  'version 1.3  17.06.2016';
 ...
 function body_version   return varchar2;
    ...
    TYPE r_declined_storec IS record (
         ru                varchar2(6 char),
         rnk               number(38),
         telephone_number  varchar2(15 char),
         payment_id        number(38),
    ...
    type r_declined_stolist is table of r_declined_storec;
    function get_sto_lst
    return r_stolist pipelined;

    function get_declined_sto_lst
    return r_declined_stolist pipelined;
...
CREATE OR REPLACE PACKAGE BODY BARS.STO_INTEGRATION_XRM as
  ...
  G_BODY_VERSION  CONSTANT VARCHAR2(64)  :=  'version 1.3  17.06.2016';
  ...
  END body_version;
    ...
    begin
        ...
        loop
            if (i.send_sms = 'Y' and i.state in (sto_payment_utl.STO_PM_STATE_READY_TO_WITHDRAW) and i.payment_amount > 0 and VERIFY_CELLPHONE(i.phone_number) = 1 and i.value_date >=DAT_NEXT_U(gl.bd, 3)) then
                l_storec.ru                 := i.kf;
                l_storec.rnk                := i.rnk;
    ...
    return r_declined_stolist pipelined
    is
        l_declined_storec r_declined_storec;
        l_redundant_payment_ids number_list := number_list();
        l_old_fashioned_id varchar2(38 char);
        l_kf varchar2(6 char);
    begin
        ...
        loop
            if (i.send_sms = 'Y' and i.state in (sto_payment_utl.STO_PM_STATE_READY_TO_WITHDRAW) and i.payment_amount > 0 and VERIFY_CELLPHONE(i.phone_number) = 1 and i.value_date >= DAT_NEXT_U(gl.bd, 3)) then
                l_declined_storec.ru                 := i.kf;
                l_declined_storec.rnk                := i.rnk;
                l_declined_storec.telephone_number   := i.phone_number;
                l_declined_storec.payment_amount     := i.payment_amount;
                l_declined_storec.fee_amount         := i.fee_amount;
                l_declined_storec.payment_date       := to_char(i.value_date);

                bars_sqnc.split_key(i.payment_id, l_old_fashioned_id, l_kf);
                l_declined_storec.payment_id := l_old_fashioned_id;

                bars_sqnc.split_key(i.order_id, l_old_fashioned_id, l_kf);
                l_declined_storec.contract_number := l_old_fashioned_id;

                pipe row (l_declined_storec);
            else
                if (i.state not in (sto_payment_utl.STO_PM_STATE_NEW, sto_payment_utl.STO_PM_STATE_SBON_AMOUNT_GOT)) then
                    l_redundant_payment_ids.extend(1);
                    l_redundant_payment_ids(l_redundant_payment_ids.last) := i.payment_id;
