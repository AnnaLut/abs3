CREATE OR REPLACE PACKAGE BODY bars.OVRN IS  
 G_BODY_VERSION  CONSTANT VARCHAR2(64)  :='ver.1.0 16.03.2017-2';   
 g_errN number := -20203 ;
 g_errS varchar2(5) := 'OVRN:' ;

/*
16.03.2017 Відсутнє договірне списання
Sta .........
...
begin
 ...
 If l_Dat_Tek < l_dat21  and l_Dat_Nxt >= l_dat21 then
    select min(fdat) into l_Dat_Lim from OVR_LIM where acc = p_acc  and fdat > l_Dat_Tek and ok = 1 ;
    -- gl.Bdate <= ... 18, 19, 20
    -- Dat_Next >= 21, 22, 23  ...  
    -- Dat_Lim  >= 21, 22, 23  ...  
    ...
    If l_Dat_Lim <= l_Dat_Nxt  then
       begin select lim  into l_lim  from OVR_LIM  where acc = p_acc and fdat = l_Dat_Lim  and ok = 1 ; 
    ...
    select min(fdat) into l_Dat_Lim from OVR_LIM_DOG where acc = p_acc  and fdat > l_Dat_Tek ;
    If l_Dat_Lim <= l_Dat_Nxt  then
       begin select lim  into l_lim from OVR_LIM_DOG where acc = p_acc  and fdat = l_Dat_Lim ; 
             update accounts set lim = l_lim where acc = p_acc and lim <> l_lim; 
       EXCEPTION WHEN NO_DATA_FOUND THEN null;
       end ;
   end if;
...
*/
  ...
  begin  ---- Просроченное тело -- абсолютный блок
     ...
     where n.nd = dd.ND and a.acc= n.acc and a.nbs ='2067' and OSTC < 0 and rownum = 1 ;        goto ReT_;
  EXCEPTION WHEN NO_DATA_FOUND THEN  null;
  end;
  ...
  delete from OVR_TERM_TRZ t 
   where TRZ < 4 and 
       ( DATSP < gl_bdate and exists (select 1 from nd_acc where nd = dd.nd and acc = t.acc )  OR ovrn.fost_sal(t.acc, gl_bdate) >= 0  )  ;     

  begin -- Просроченные проценты несвоевременная оплата %/комиссии,
     select 11 into n_sos from accounts a, nd_acc n  
     where n.nd = dd.ND and a.acc= n.acc and a.nbs ='2069' and OSTC < 0  and rownum = 1 ;          goto ReT_;
  EXCEPTION WHEN NO_DATA_FOUND THEN  null;
  end;
  ...
  If dd.sos <> n_sos then 
     ...
     loop  
        If n_Sos = 10 and  dd.wdate >= gl_bdate  then  -------------------------------------- УРА ! УЛУЧШЕНИЕ ! Нормальный (sos=10) 
           ...
           --- убираем планы выноCа на просрочку
           If aaa.acc <> a89.acc then delete from OVR_TERM_TRZ where acc = aaa.acc ; end if ; 

           --- восстанавливаем договорные ставки 
           OVRN.SetIR ( dd.nd, aaa, 0, gl_bdate, acrn.fprocn ( aaa.acc, 0, dd.sdate) )   ; --\ для 8998 и для 26* 
           OVRN.SetIR ( dd.nd, aaa, 1, gl_bdate, acrn.fprocn ( aaa.acc, 1, dd.sdate) )   ; --/ 
        ...
        Else   ------------------------------------------------------------------------------- УВЫ ... Ухудшение...(sos=11,12,13) 
           ...
           If aaa.tip = OVRN.tip   then    --- проблема вых дня !!!!
              OVRN.SetIR ( dd.nd, aaa, 0, gl_bdate, 0 ) ;  ---8998*
              OVRN.SetIR ( dd.nd, aaa, 1, gl_bdate, 0 ) ;
...
begin
  ...
  l_term := to_number(OVRN.GetW(p_acc, 'TERM_OVR' )) ;
  l_mdat := l_mdat + l_term - 1 ;    
  l_mdat := Dat_Next_U ( l_mdat , 0 ) ; 
  RETURN l_mdat;
end Get_MDAT; 
...
begin
  begin select * into tt from  OVR_TERM_TRZ where acc1 = p_acc and trz = 4 and (DATVZ <= gl.bdate or datsp >= gl.bdate) and rownum = 1 ;
     ...
     else raise_application_error (g_errn,'На рах.'||p_nls||' '||to_date(tt.DATVZ,'dd.mm.yyyy')||' вже було накладене ручне блокування');
     end if ;
  ...
  EXCEPTION WHEN NO_DATA_FOUND THEN  null;
     ...
     else raise_application_error (g_errn,'На рах.'||p_nls||' ще НЕ було ще накладене ручне блокування');
     end if ;
...
-------    ElsIf l_kodK = 11 then l_ob2 := '13' ; l_ob7 := '21' ;  --  Тепло
           ...
           EXCEPTION WHEN NO_DATA_FOUND THEN  --  2608
              l_nls := vkrzn (substr(gl.aMfo,1,5), substr(aa.nls,1,3) || '80'|| substr(aa.nls,6,9) ) ;
           ...
           OP_BS_OB1 (PP_BRANCH => substr(aa.branch,1,15) , P_BBBOO =>'7020'||l_ob7 ) ;   ------------  2).  Дозаполнение ACRB 

           begin select acc into ii.AcrB from accounts where nls = nbs_ob22_null('7020',l_ob7, substr(aa.branch,1,15) ) and kv = gl.baseval;
           EXCEPTION WHEN NO_DATA_FOUND THEN  null;
           end ;
...
function  GetAL  ( p_acc number , p_dat date ) return  number  is
 ...
 l_ret int := 0; l_dat21 date;
begin
 l_dat21 := trunc ( p_dat, 'MM') + 20 ;
 begin select 1 into l_ret from ovr_lim where acc = p_acc and fdat = l_dat21 and OK = 1 ;
...
begin
  ...
  begin select d.limit*100, to_number(OVRN.GetW(a.acc, 'PCR_CHKO')), a.acc, a.lim 
        into   LD0_       ,    PD_                   , acc8_,               LDT_ 
  ...
  dat_   := trunc( nvl ( p_date, gl.bdate), 'MM') ; --- пусть сег = 05.05.2016. 
  ...
  For a26 in (select acc, to_number(OVRN.GetW(acc,'PCR_CHKO')) PK, lim LKT from accounts where  accc = acc8_ and nbs in ('2600','2650')  )
  loop 
...
---    select DIV0( sum(s), count(*) )  into CKO_ from OVR_CHKO where acc = a26.acc and datm >= dat1_ and datm <= dat3_ and s > 0 ;
       ...
       ElsIf nTmp_ < 0.1                               then  LKN_ := a26.LKT;
       end if;
  ...
  If nTmp_  < 0.1 then  LDN_ := LDT_; end if;
...
--   update cc_deal set sos = 0 where nd = l_ND;
  ...
  select count(*), min(acc), min (lim) into u_Kol, x_acc26, x_lim26 from accounts where accc = a89.acc and nbs in ('2600','2650') ;
...
*/
  ...
  FOR a26 in (select * from accounts where nbs in ('2600','2650') and acc in ( select acc from nd_acc where nd = dd.nd))
  loop 
...
------         select 1 s from OVR_CHKO where acc = a26.acc and s >0 and datm = add_months(dTmp_,-3) union all 
    ...
    if n_Kom < 6 then
       raise_application_error(g_errn, g_errS||a26.nls||'.Не заповнено всі дані про комісійні тарифи'); 
    end if;

    If n_Sum <> 0 and u_Kol = 1 then
       raise_application_error(g_errn, g_errS||a26.nls||'.Тариф 145='||n_Sum/100 || ' > 0 можливий тільки для Холдингу (більше 1-го учасника) !' );  
    end if;
...
--62 - Изменить  ИТОГ   ЧКО за месяц в архиве вручную
  ...
  elsIf p_mode = 1  then  ------ 1 - наполнить месяц в архиве
     ...
     BEGIN SELEct c.OKPO, a.nls, d.nd   INTO l_OKPO, l_nls, l_nd   from customer c, accounts a, nd_acc n, cc_deal d 
           where a.acc = l_acc and a.rnk = c.rnk and a.acc= n.acc and n.nd = d.nd and d.vidd = ovrn.vidd and d.sos < 15   ;
     EXCEPTION WHEN NO_DATA_FOUND  THEN raise_application_error(g_errn, ' Не знайдено в угоді асс= '||l_acc )  ;  
     end;

/*   -- эта версия работает хуже., хотя только OPER 
     ...
     where o.sos = 5 
       ...
       and o.mfob  = gl.aMfo   
       and o.id_a not in 
          ( select distinct cc.okpo from customer cc, accounts aa, nd_acc nn where cc.rnk = aa.rnk and aa.acc = nn.acc and nn.nd = l_nd ) ;
*/
     ...
     SELECT O.REF,  l_acc, dat1_
     FROM OPER O , 
        (select oo.ref from opldok oo, saldoa ss   
         where ss.acc= l_acc  and ss.acc= oo.acc and ss.fdat >= dat1_ and ss.fdat < dat2_ and ss.kos> 0 and ss.fdat = oo.fdat
           and oo.dk= 1 and oo.sos= 5     ) p 
     where o.id_b = l_OKPO    
       and o.nlsb = l_nls   
       and o.mfob = gl.aMfo   
       and o.ref  = p.ref  
  ...
  elsIf p_mode = 9 then  -- 9 - сжать в одну сумм
     ...
     delete from OVR_CHKO where datm >= dat1_ and datm < dat2_ and acc = l_acc;
...
begin -- проверка на допустимость счета
  ...
  If p_mode >= 0 then 
     ...
     If  aa.nbs not in ('2600','2650') then raise_application_error(g_errn, l_err || ' НЕ 2600/2650 ')             ;  end if;
  ...
  If p_mode = 0 then
     ...
     begin select d.* into dd from cc_deal d, nd_acc n where d.sos < 14 and n.nd = d.nd and n.acc = aa.acc and rownum = 1; 
           raise_application_error(g_errn, l_err||' належить іншій угоді, реф='||dd.nd||',вид='||dd.vidd) ;
     EXCEPTION WHEN NO_DATA_FOUND THEN null ;
     end;
  end if;
...
begin
  ...
  If p_ND = 0 then  
     ...
     While 1<2     loop
        ...
        begin select 1 into nTmp_ from accounts where nls like '8998_'||nTmp_;
        EXCEPTION WHEN NO_DATA_FOUND THEN EXIT ;
        end;
     end loop;
     a8.nls := vkrzn ( substr(gl.aMfo,1,5) ,'89980'||nTmp_ );
  ...
  If p_ND = 0 then
     insert into int_accn (ACC,ID,METR,BASEM,BASEY,FREQ, stp_DAT, acr_dat ) values (a8.acc,0,0,0,0,1, dd.wdate, dd.sdate-1 );
...
begin
  ...
  ----------------------------------------------------
  OP_BS_OB1 ( PP_BRANCH => substr(aa.branch,1,15) , P_BBBOO =>'6020'||'06' );
  OP_BS_OB1 ( PP_BRANCH => substr(aa.branch,1,15) , P_BBBOO =>'6111'||'05' );
  -------------------------------------------------------------------------
  ...
  -----------------------------------------------------------------
  --2607   -- Акт % ставка
  sn.nls := vkrzn (substr(gl.aMfo,1,5), substr(aa.nls,1,3) || '70'|| substr(aa.nls,6,9) ) ;
  ...
  begin insert into int_accn (ACC,ID,METR,BASEM,BASEY,FREQ, acr_dat, acra, acrb) 
        select aa.acc,0,0,0,0,1, dd.sdate-1 , sn.acc, b.acc 
        from accounts b where nls = nbs_ob22_null('6020','06', substr(aa.branch,1,15) );
  exception when dup_val_on_index then  null; 
  end ;
  ...
  if nvl(p_don,0) <> 1 then 
     ...
     EXCEPTION WHEN NO_DATA_FOUND THEN 
       a9.nls := vkrzn (substr(gl.aMfo,1,5), F_NEWNLS(aa.acc,'CR9','9129') ) ;
       ...
       Accreg.setAccountSParam(a9.Acc, 'OB22', '04');
     end ;
...
begin
  ...
  delete from accountsW    where  acc = a26.acc  and tag in ('TERM_OVR', 'TERM_DAY', 'TERM_TRZ', 'PCR_CHKO', 'NEW_KL', 'DONOR' ); --доп.рекв счета
  delete from accountsW    where  acc = a26.accc and tag in ('TERM_OVR', 'TERM_DAY', 'TERM_TRZ', 'PCR_CHKO', 'NEW_KL', 'DONOR' ); --доп.рекв счета

  delete from OVR_LIM      where  acc = a26.acc ;                             --лимиты
...
-- p_mode = 1 - finish Список закриття дня	Функції, які виконуються при закритті дня
  ...
  k9129_ number;  
  m_date date  ;
  l_dat05 date ;  l_dat15 date;  l_dat21 date ;
...
begin
  ...
  l_dat15 := trunc      ( l_date , 'MM') +14  ;  l_dat15 := Dat_Next_U ( l_dat15, 0 ) ; 
  l_dat21 := trunc      ( l_date , 'MM') +20  ;  l_dat21 := Dat_Next_U ( l_dat21, 0 ) ; 
      ...
      loop  
        ...
        If a26.ostc <= 0 and  p_mode = 0 then  
           ...
           If a26.ostc < 0 then   --просроч тело 
              l_mdat := OVRN.Get_mdat (a26.acc);
              update accounts set mdate = l_mdat where acc = a26.acc and (mdate <> l_mdat or mdate is null) ;

              select max(datSP) into dTmp_ from OVR_TERM_TRZ where acc = a26.acc and datSP <= l_date ;    ----- 2) СТАРТ: вынос на просрочку
         ...
         loop  
            ...
            If dd.sos = 10 and  dd.wdate > l_date  then  
               ...
               begin select max(fdat) into Dat_Lim from OVR_LIM where acc = aaa.acc and nd = dd.ND and fdat <= gl.bdate  and ok = 1;
                     select lim into aaa.lim from OVR_LIM where acc = aaa.acc  and fdat = Dat_Lim  and ok = 1;
               ...
               If aaa.acc <> a89.acc then delete from OVR_TERM_TRZ where acc = aaa.acc ; end if ; -- убираем планы выноCа на просрочку

               OVRN.SetIR ( dd.nd, aaa, 0, l_date+1, acrn.fprocn ( aaa.acc, 0, dd.sdate) )   ; --\ для 8998 и для 26* 
               OVRN.SetIR ( dd.nd, aaa, 1, l_date+1, acrn.fprocn ( aaa.acc, 1, dd.sdate) )   ; --/ восстанавливаем договорные ставки 
               ...
               If aaa.tip = 'OVN' then    --- проблема вых дня !!!!
                  ...
                  OVRN.SetIR ( dd.nd, aaa, 1, l_date, 0 ) ;
               else
               ...
               end if;  --             If dd.sos = 10 and  dd.wdate > l_date  then  
            end if ; ---  dd.sos = 10 
...
begin 
  ...
  ----------------------
  for a8 in (SELECT a.acc , NVL(i.acr_dat+1, d.sdate)  DAT1, i.metr 
  ...
  loop 
     ...
     select count(*) into l_kol from accounts where accc = a8.acc and nbs in ('2600','2650');
...
begin oo.tt := '%%1';  oo.vob := 6; oo.vdat := gl.bdate; oo.kv2 := gl.baseval ; oo.mfoa := gl.aMfo; oo.mfob := gl.aMfo; oo.id_b := gl.aOkpo;
...
loop
  begin  
     ...
     If x.pr > 0 then oo.s := x.pr ; oo.s2 := gl.p_icurval(x.kv,oo.s,oo.vdat);   oo.ref := null ;  oo.dk := x.id ;   
        ...
        If x.VN = 0 then acrn.acr_dati (acc_=> x.acc, id_=> (1-x.id ), ref_ => oo.ref, dat_ => x.acr_dat, remi_ => 0) ;
        else             delete  from ACC_OVER_COMIS v where v.acc = x.acc and v.fdat >= x.dat1 and v.fdat <= x.dat2  ;
                         insert into operw (ref,tag, value) values (oo.ref, 'KTAR ', '144' );
        end if ; 
     end if;
...
procedure OP_3600 (dd IN cc_deal%rowtype, a26 IN accounts%rowtype , a36 IN OUT accounts%rowtype) IS   -- откр дисконта  3600
  ...
  u_ND number;
begin
  begin select a.* into a36 from accounts a, nd_acc n 
         where a.rnk = a26.rnk and a.nbs = '3600' and a.acc =n.acc and n.nd = dd.nd and a.ob22 = '09' ; 
  EXCEPTION WHEN NO_DATA_FOUND THEN 
     -- открыть 3600
     a36.nls :=  f_newnls (a26.acc, 'OV2067', '')   ;     
     a36.nls :=  '3600'|| '0'|| substr(a36.nls,6,9) ;     
     a36.nls := VKrzn(substr(gl.aMfo,1,5), a36.nls) ;
...
begin
  ...
  EXCEPTION WHEN NO_DATA_FOUND THEN
     -- открыть 8008
     sn8.nls := VKrzn ( substr( gl.aMfo,1,5),  '80080'|| substr(a26.nls,6,9)  ) ;
  ...
  -- счет 2069 SPN
  begin select a.* into a69 from accounts a, nd_acc n where a.rnk = a26.rnk and a.nbs = '2069' and a.acc =n.acc and n.nd = dd.nd; 
  EXCEPTION WHEN NO_DATA_FOUND THEN 
     -- открыть 2069
     a69.nls := VKrzn ( substr( gl.aMfo,1,5),  f_newnls (a26.acc, 'OV2069', '') ) ;
     ...
     Accreg.setAccountSParam(a69.Acc, 'OB22', '04');
  end;
...
------  Insert into INT_RATN  (ACC, ID, BDAT, IR, BR, OP) Values (a69.acc, 2, gl.bdate, 2, 10001,  3);
  ...
  --- счет SP
  begin select a.* into a67 from accounts a, nd_acc n where a.rnk = a26.rnk and a.nbs = '2067' and a.acc =n.acc and n.nd = dd.nd; 
  EXCEPTION WHEN NO_DATA_FOUND THEN 
     -- открыть 2067
     a67.nls := VKrzn ( substr( gl.aMfo,1,5),  f_newnls (a26.acc, 'OV2067', '') ) ;
     ...
     Accreg.setAccountSParam(a67.Acc, 'OB22', '01');
  end;
...
-- p_mode = 3                  then    --- 06 число Проверить погашены ли % и ести нет - вынос на просточку 
  ...
  l_term int   ;  l_donor int ;  l_ir8 number ; l_kol int ; sTmp_ varchar2(500);
  l_OST number ;
  --------------
  Dat0_ date   ;
  KOLD_ int    ;
...
--logger.info ('XXX*'||oo.dk ||'*'|| oo.nlsa ||'*'|| a26.nls ||'*'||  oo.tt ||'*'||  oo.nlsb ||'*'|| sp.nls||'*'||sp.tip ) ;
...
begin
  ...
  l_donor := NVL( to_number (OVRN.GetW(a26.acc , 'DONOR'    ) ), 0) ;
  l_term  := NVL( to_number (OVRN.GetW(a26.accc, 'TERM_DAY' ) ), 5) ;
...
If p_mode = 0 then    --- Первонач комиссия
   ...
   select count(*) into l_kol from accounts where accc = a26.accc and nbs in ('2600','2650');
   ...
   loop
      ...
      if oo.s > 0 then     
         ...
         loop
            ...
            else                       spl1_ := ROUND(  oo.S * (x.dat31-Dat0_) / KOLD_ -0.5 ,0) ;
            end if; 
            Dat0_ := x.dat31; 
            If spl1_ > 0 then      
               itog_ := itog_ - spl1_ ;
               gl.payv( 0, oo.REF, x.dat31, '%%1', 1,  
                        gl.baseval,  a36.nls,  spl1_,  
                        gl.baseval,  nbs_ob22_null('6020', '06',  substr(a26.branch,1,15) ) ,   spl1_ 
                       );
               update opldok set txt = 'Лін.амотриз.дисконту' where ref = oo.ref and stmt = gl.aStmt;
   ...
   begin 
      ...
      EXCEPTION WHEN NO_DATA_FOUND THEN  -- открыть 3739
         a37.nls :=  f_newnls (a26.acc, 'OV2067', '')   ;     
         a37.nls := '3739'|| '0'|| substr(a37.nls,6,9)  ;     
         a37.nls := VKrzn(substr(gl.aMfo,1,5), a37.nls) ;
...
If p_mode = 1  then    --- Погашение
   ...
   loop  oo.s := 0;
      ...
      If sp.tip  = 'SN ' then   ------ g_TAGD = TERM_DAY -- Термiн(день мiс) для сплати %%
         ...
         begin select add_months(trunc(acr_dat,'MM'),1) + l_term into ii.acr_dat from int_accn where id=0 and acc = x26.acc ;         
               If p_dat < ii.acr_dat AND DD.WDATE > P_DAT then oo.s := 0 ; -- от 01 - 05 ничего не делаем. Дата уплаты еще НЕ наступила
               else    
...
--If p_mode = 3 and l_donor <> 1 and to_number(to_char(p_dat,'DD')) > l_term  then  
If p_mode = 3 and to_number(to_char(p_dat,'DD')) > l_term  then  
...
If p_mode = 99 and l_donor <> 1 then
   ...
   begin 
     ...
     EXCEPTION WHEN NO_DATA_FOUND THEN 
      ...
      where  o.nlsb = a26.nls  and o.id_b = oo.id_a and o.id_a not in 
              ( select cc.okpo from customer cc, accounts aa, nd_acc nn where cc.rnk = aa.rnk and aa.acc = nn.acc and nn.nd = dd.nd )
...
begin
 ...
 loop
    ...
    delete from accountsW  where  acc = dd.acc8 and tag in ( 'TERM_OVR', 'TERM_DAY', 'TERM_TRZ', 'PCR_CHKO', 'NEW_KL', 'DONOR' ); --доп.рекв счета
    delete from OVR_LIM    where  acc = dd.acc8 ;                             --лимиты
...
BEGIN
  ...
  begin select a.* into AA  from accounts  a, nd_acc n 
        where a.acc=n.acc and n.nd = dd.nd and a.nbs in ('2067','2069','2607') and a.ostc <0 and rownum =1; 
        l_txt := ' НЕ закрито ! Не погашено заборг.'|| aa.nls ||' '|| aa.ostc/100 ;
        goto RET;
  EXCEPTION  WHEN NO_DATA_FOUND THEN null ;
  end;
...
------- P_ADD_ZAl     ( dd.ND, k.accs, k.rnk, null, k.ACC, k.kv, null, - ABS(k.ostc)/100, null, null, null, null, null, null, null);  
  ...
  for k in (select * from accounts 
            where acc in (select acc from nd_acc where nd = dd.nd) 
              and nbs not in ('2600','2650','2608','2658')      and dazs is null 
             ) 
  loop if k.ostc <> 0 then   raise_application_error(g_errn,'Рах.'||k.nls ||' має залишок='|| k.ostc/100 );  end if;
       update accounts set dazs = l_bDat_Next where acc = k.ACC;
  end loop;
...
function TAGT return varchar2 is begin  return 'TERM_TRZ' ; end TAGT ;  --- TERM_TRZ  Термін відсрочки винесення на просрочку кіл.днів
...
begin                                                                    
 ...
 G_TAGT := 'TERM_TRZ'; PUL.Set_Mas_Ini('G_TAGT', 'TERM_TRZ', 'Сол.ОВР.тормоз.путь'       ); -- Термiн для вiдсрочки винесення на просрочку кiл.днiв
