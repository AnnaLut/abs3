...
IS
   ...
   g_body_version      CONSTANT VARCHAR2 (64) := 'version 2.08 02/06/2016';
   ...
   PROCEDURE prepare_acc_msg (
      ...
      p_vipiska          IN acc_msg.VIPISKA%TYPE default 'N')
   ...
   BEGIN
      ...
      select  get_acc_type (p_acc) , get_acc_nbs (p_acc)
      into l_tip, l_nbs from dual;
    ...
    EXCEPTION
         ...
         THEN
                                                logger.info (
                                                      'Не определен шаблон для балансовый счет для АСС='
                                                   || p_acc
                                                   || ' для операции = '
                                                   || p_tt
                                                   || ' для типа = '
      ...
      THEN
         RETURN;        -- якщо не потрібно відправляти смс шаблон то виходимо
      END IF;                                                               --


     if (l_SMS_ACC_TEMPLATES.ID = 8)  -- якщо смс шаблон по теплому кредиту то перевіряємо чи з рахунку відшкодування прийшли кошти
       THEN
         ...
         if  (l_warm_nls = p_nlsa )
            THEN
            logger.info ('warm_credit_sms for acc:' || to_char(p_acc) );--відповідно до заявки: якщо було відшкодування по теплому кредиту - смс по даному рахунку відключаємо
--            /*update accounts t1
         ...
         THEN                             -- только для самых свежих изменений
            BEGIN
               SELECT *
                 ...
                 FROM acc_sms_phones
                WHERE acc = c.acc;
                logger.info ('l_phones.phone ='|| l_phones.phone ||
   ...
   BEGIN
      ...
      LOOP
         FOR c IN (SELECT ROWID,
                    ...
                    and b.acc = cur_phones.acc /*and tt<>'SMS'*/ )
         LOOP
            -- готовим данные для посылки SMS по конкретному изменению остатка
            IF c.id = c.max_id
            THEN                          -- только для самых свежих изменений
