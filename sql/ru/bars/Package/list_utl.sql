...
CREATE OR REPLACE PACKAGE BODY BARS.LIST_UTL as
    ...
    begin
        ...
        values (list_type_seq.nextval, p_list_code, p_list_name, 'Y')
        returning id
        into l_list_id;
    ...
    begin
        if (p_lock) then
            ...
            into   l_list_type_row
            from   list_type lt
            where  lt.list_code = p_list_code
            for update wait 60;
        else
            select *
            into   l_list_type_row
    ...
    begin
        ...
        update list_type lt
        set    lt.is_active = 'N'
        where  lt.list_code = l_list_type_row.list_code;
    end;
    ...
    begin
        ...
        if (p_parent_item_code is not null) then
            l_parent_item_row := read_list_item(l_list_row.id, p_parent_item_code);
        end if;
...
grant EXECUTE                                                                on LIST_UTL        to BARS_DM;
