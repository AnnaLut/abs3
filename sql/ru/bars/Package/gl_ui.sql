...
/*    function get_current_bank_date_utl
    ...
    is
    begin
        return to_date(branch_attribute_utl.get_value('/', 'BANKDATE', p_def_value => null), 'mm/dd/yyyy');
    end;
    ...
    begin
        ...
        left join tms_branch_stage s on s.branch = b.branch and
                                        s.run_id is null
        where  regexp_like(b.branch, '^/\d{6}/$')
        order by b.name;
    ...
    begin
        ...
        where  regexp_like(b.branch, '^/\d{6}/$') and
               b.branch like sys_context('bars_context', 'user_branch_mask')
        order by b.name;
    ...
    begin
        ...
        -- Перервати виконання
        if (p_task_run_state_id in (tms_utl.TASK_RUN_STATE_IN_PROGRESS) and
