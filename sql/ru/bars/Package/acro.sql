 ...
 PROMPT ===================================================================================== 
 PROMPT *** Run *** ========== Scripts /Sql/BARS/package/acro.sql =========*** Run *** ======
 PROMPT ===================================================================================== 
 
  CREATE OR REPLACE PACKAGE BARS.ACRO IS

G_HEADER_VERSION  CONSTANT VARCHAR2(64)  := 'версия 5.11 26/03/2012';
...
/*%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% PROCEDURE 	: p0_acr/p2_acr
%
% DESCRIPTION	: Start of day/night Interest acrual
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%*/

PROCEDURE p0_acr( dt_ DATE, acc_ INTEGER DEFAULT NULL );
PROCEDURE p2_acr( mode_ SMALLINT, dat_ DATE, filt_ VARCHAR2);
...
END acro;
/
CREATE OR REPLACE PACKAGE BODY BARS.ACRO 
IS

g_body_version  constant varchar2(64) := 'версия 5.29 19/04/2012';
acc_FORM int := null;
...
begin
  return 'Заголовок пакета acro '||G_HEADER_VERSION||'.'||chr(13)||chr(10)
	   ||'AWK definition: '||chr(10)
	   ||G_AWK_HEADER_DEFS;
end header_version;
function body_version return varchar2 is
begin
  return 'Тело пакета acro '||G_BODY_VERSION||'.'||chr(13)||chr(10)
	   ||'AWK definition: '||chr(10)
	   ||G_AWK_BODY_DEFS;
end body_version;
function ver return varchar2 is
begin
  return 'Package header acro '||G_HEADER_VERSION||'.'||chr(10)
	   ||'AWK definition: '||chr(10)
	   ||G_AWK_HEADER_DEFS||chr(10)||chr(10)||
         'Package  body acro '||G_BODY_VERSION||'.'||chr(10)
	   ||'AWK definition: '||chr(10)
	   ||G_AWK_BODY_DEFS;
end ver;
...
--  В моді2 тільки змінюємо залишки
         IF mode_=2 AND nbs_ LIKE '8%' AND accc_ IS NULL THEN
            ...
            acro.p_int(acc_,id_,dat1_,dat2_,acrd_,NULL,0);
...
--              ORDER BY dat,otm;
	...
	c_label	    constant varchar2(128)  := 'acro.p_int';
...
BEGIN
  ...
  acro.SumO  := 0;  -- Сумма остатков
  acro.SumOP := 0;  -- Сумма остатков на процент
  acro.SumOD := 0;  -- Количество дней сумма остатков
  ...
  WHILE dat1_<=TRUNC(dt2_) LOOP
    ...
    LOOP
       ...
       if mode_ = 2 then  -- 06.08.2012  Для игрового режима без ХО-ХО
        ...
        IF acc.basey=0 or
          (acc.basey=4 and acro.cur_Basey is null) THEN  -- факт/факт ГОДОВАЯ % ставка
        ...
        IF acc.metr = 3 THEN
           bars_audit.trace ('acro.p_int: metr = 3, acc = %s, %s - %s', to_char(acc_),
           ...
           BEGIN
             ...
             bars_audit.trace ('acro.p_int: дата посл.начисления = %s', to_char(l_datacr,'dd.mm.yyyy'));
             l_datend := least (dat2_, l_datacr);
             bars_audit.trace ('acro.p_int: дата по* = %s', to_char(l_datend,'dd.mm.yyyy'));
             FOR sal IN c_amo LOOP
                 ...
                 IF (id_ = 0 AND sal.ostf < 0 AND l_datacr >= dat1_) THEN
                     ...
                     bars_audit.trace ('acro.p_int: ost = %s, delta = %s, acrd = %s, cnt = %s',
        ...
        --
          ...
          bars_audit.trace ('acro.p_int(4): acc/id => %s/%s, saltype => %s, term => %s-%s',
          ...
          loop
              ...
              bars_audit.trace ('acro.p_int(4): saldo = %s, mdate=>%s',
              ...
              bars_audit.trace ('acro.p_int(4): "till" date = %s', to_char(dat2_,'dd.mm.yyyy'));
             ...
             bars_audit.trace ('acro.p_int(4): intamount(sal %s, %s/%s days) = %s',
...
---------------acro.cur_Nomin
               If Nvl(acro.cur_Nomin,0) > 0 then
                  KOL_  := Abs(ostf_) / acro.cur_Nomin;
                  acrd_ :=
                   ( ir_ * Sign(ostf_)*acro.cur_Nomin + ostp_ )
                         * dlta_/b_yea/100;
               else
                  KOL_  := 1;
               ...
               IF acrd_ <> 0 or acc.metr IN (1,2) THEN
                  ...
                  acro.SumO  := acro.SumO  + osts_;
                  acro.SumOP := acro.SumOP + osts_ * (ir_ + br_);
                  acro.SumOD := acro.SumOD + dlta_;
                  acr_ := acr_ + acrd_;
               END IF;
            END IF;
...
IS
BEGIN
   RETURN acro.FPROCN(acc_,NULL,datp_);
END FPROC;
...
/
 ...
 PROMPT ===================================================================================== 
 PROMPT *** End *** ========== Scripts /Sql/BARS/package/acro.sql =========*** End *** ======
 PROMPT ===================================================================================== 
