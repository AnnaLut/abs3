       ...
       if p_dk = 1 then
         if p_kv = 980 then --грн.
           ...
           if -- Вихідні
              ...
              ( ( p_nlsa like '26%' or l_nlsa in ('2530', '2541', '2542', '2544', '2545', '2560' ) ) and  l_nlsb in ( '1919', '3739' ) )
              ...
              or ( l_nlsa = '2600' and l_nlsb = '2605' )
              or
              ( l_nlsa in ( '2600', '2650', '2560', '2620', '2625' ) and p_nlsb like '100%' )
              or ( l_nlsa='2625' and l_nlsb='2620' )
             then l_res := 1;

             elsif -- Вхідні
                   -- з рахунку 2603 на 2900, 2600 або 2650 (для ЦА та РУ)
                   ( l_nlsa = '2603' and l_nlsb in ( '2900', '2600', '2650', '2603' ) )
                   or -- з рахунку 3739 на 2603, 2600 або 2650 (тільки для ЦА)
                   ( l_nlsa = '3739' and l_nlsb in ( '2603', '2600', '2650' ) )
                   or -- з рахунку 2909 на 2603, 2600, 2620 або 2650 (тільки для РУ)
             ...
             then l_res :=  0;
             elsif l_nlsa in ( '2520', '2541', '2542') then
               select nvl((select ob22 from accounts where kv=p_kv and nls=p_nlsb), 'xx') into l_txt from dual;
               if l_nlsb='2520' and l_txt in ('02', 'xx') or l_nlsb in ( '2541', '2542' ) and l_txt in ('01', 'xx')
                 then l_res :=  0; else l_res := null;
               end if;
             else l_res := null;
           end if;
         end if;
         ...
         if p_tt is not null and p_ref is not null and p_tt = 'GO1'then
           ...
           if l_nlsa in ( '2600', '2650', '2603', '2605', '2541', '2542', '2544', '2545', '2530', '2520') then l_res:=0;
           else l_res := null;
           end if;
         else
           l_res := null;
