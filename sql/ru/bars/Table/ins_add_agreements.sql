...
BEGIN 
        execute immediate  
          'begin  
               bpa.alter_policy_info(''INS_ADD_AGREEMENTS'', ''FILIAL'' , null, null, null, null);
...
begin   
 execute immediate '
  ALTER TABLE BARS.INS_ADD_AGREEMENTS ADD CONSTRAINT FK_INSADDAGRS_DID_DEALS FOREIGN KEY (DEAL_ID)
...
begin   
 execute immediate '
  ALTER TABLE BARS.INS_ADD_AGREEMENTS ADD CONSTRAINT PK_INSADDAGRS PRIMARY KEY (ID)
...
PROMPT *** Create  constraint SYS_C003101308 ***
begin   
 execute immediate '
...
begin   
 execute immediate '
  ALTER TABLE BARS.INS_ADD_AGREEMENTS ADD CONSTRAINT UK_INSADDAGRS UNIQUE (DEAL_ID, SER, NUM, SDATE)
  USING INDEX PCTFREE 10 INITRANS 2 MAXTRANS 255 COMPUTE STATISTICS 
  TABLESPACE BRSBIGI  ENABLE';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
...
begin   
 execute immediate '
  CREATE UNIQUE INDEX BARS.PK_INSADDAGRS ON BARS.INS_ADD_AGREEMENTS (ID) 
...
begin   
 execute immediate '
  CREATE UNIQUE INDEX BARS.UK_INSADDAGRS ON BARS.INS_ADD_AGREEMENTS (DEAL_ID, SER, NUM, SDATE) 
  PCTFREE 10 INITRANS 2 MAXTRANS 255 COMPUTE STATISTICS 
  TABLESPACE BRSBIGI ';
exception when others then
  if  sqlcode=-955  then null; else raise; end if;
