...
PROMPT *** Create  constraint FK_PERSONVALIDDOCUMENT_RNK ***
begin   
 execute immediate '
  ALTER TABLE BARS.PERSON_VALID_DOCUMENT ADD CONSTRAINT FK_PERSONVALIDDOCUMENT_RNK FOREIGN KEY (RNK)
...
begin   
 execute immediate '
  ALTER TABLE BARS.PERSON_VALID_DOCUMENT ADD CONSTRAINT CC_PERSONVALIDDOC_DOCSTATE CHECK ( DOC_STATE in (0,1) ) ENABLE';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
begin   
 execute immediate '
  ALTER TABLE BARS.PERSON_VALID_DOCUMENT MODIFY (DOC_STATE CONSTRAINT CC_PERSONVALIDDOC_DOCSTATE_NN NOT NULL ENABLE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
begin   
 execute immediate '
  ALTER TABLE BARS.PERSON_VALID_DOCUMENT MODIFY (RNK CONSTRAINT CC_PERSONVALIDDOC_RNK_NN NOT NULL ENABLE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
grant INSERT,SELECT,UPDATE                                                   on PERSON_VALID_DOCUMENT to BARS_ACCESS_DEFROLE;
