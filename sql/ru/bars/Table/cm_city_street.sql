...
PROMPT *** Create  constraint FK_CMCITYSTREET_CMSTREETTYPE ***
begin   
 execute immediate '
  ALTER TABLE BARS.CM_CITY_STREET ADD CONSTRAINT FK_CMCITYSTREET_CMSTREETTYPE FOREIGN KEY (STREET_TYPE_ID)
	  REFERENCES BARS.CM_STREET_TYPE (ID) ENABLE';
exception when others then
...
PROMPT *** Create  constraint FK_CMCITYSTREET_CMCITYAREA ***
begin   
 execute immediate '
  ALTER TABLE BARS.CM_CITY_STREET ADD CONSTRAINT FK_CMCITYSTREET_CMCITYAREA FOREIGN KEY (CITY_AREA_ID)
	  REFERENCES BARS.CM_CITY_AREA (ID) ENABLE';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
