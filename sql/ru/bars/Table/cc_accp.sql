...
BEGIN 
        execute immediate  
          'begin  
               bpa.alter_policy_info(''CC_ACCP'', ''FILIAL'' , null, null, null, null);
               bpa.alter_policy_info(''CC_ACCP'', ''WHOLE'' , null, null, null, null);
               null;
           end; 
          '; 
END; 
...
begin   
 execute immediate '
  ALTER TABLE BARS.CC_ACCP MODIFY (KF CONSTRAINT CC_CCACCP_KF_NN NOT NULL ENABLE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
begin   
 execute immediate '
  ALTER TABLE BARS.CC_ACCP MODIFY (ACCS CONSTRAINT NK_CC_ACCP_ACCS NOT NULL ENABLE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
begin   
 execute immediate '
  ALTER TABLE BARS.CC_ACCP MODIFY (ACC CONSTRAINT NK_CC_ACCP_ACC NOT NULL ENABLE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
grant SELECT                                                                 on CC_ACCP         to BARSDWH_ACCESS_USER;
...
grant SELECT                                                                 on CC_ACCP         to BARS_SUP;
