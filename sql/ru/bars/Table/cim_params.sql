...
BEGIN 
        execute immediate  
          'begin  
               bpa.alter_policy_info(''CIM_PARAMS'', ''FILIAL'' , null, null, null, null);
...
begin   
 execute immediate '
  ALTER TABLE BARS.CIM_PARAMS ADD CONSTRAINT PK_CIMPARAMS PRIMARY KEY (PAR_NAME)
...
begin   
 execute immediate '
  ALTER TABLE BARS.CIM_PARAMS MODIFY (PAR_NAME CONSTRAINT CC_CIMPARAMS_PARNAME_NN NOT NULL ENABLE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
begin   
 execute immediate '
  CREATE UNIQUE INDEX BARS.PK_CIMPARAMS ON BARS.CIM_PARAMS (PAR_NAME) 
...
grant SELECT                                                                 on CIM_PARAMS      to BARS_ACCESS_DEFROLE;
