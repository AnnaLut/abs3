...
BEGIN 
        execute immediate  
          'begin  
               bpa.alter_policy_info(''OW_FILES'', ''FILIAL'' , null, null, null, null);
...
begin   
 execute immediate '
  ALTER TABLE BARS.OW_FILES ADD CONSTRAINT FK_OWFILES_OWFILETYPE FOREIGN KEY (FILE_TYPE)
	  REFERENCES BARS.OW_FILE_TYPE (FILE_TYPE) ENABLE';
exception when others then
...
begin   
 execute immediate '
  ALTER TABLE BARS.OW_FILES ADD CONSTRAINT CC_OWFILES_FILESTATUS CHECK (file_status in (0,1,2,3)) ENABLE';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
