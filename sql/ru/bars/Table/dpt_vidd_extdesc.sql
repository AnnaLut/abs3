...
BEGIN 
        execute immediate  
          'begin  
               bpa.alter_policy_info(''DPT_VIDD_EXTDESC'', ''FILIAL'' , null, ''E'', ''E'', ''E'');
...
PROMPT *** Create  constraint FK_DPTVIDDEXTDESC_BRATES ***
begin   
 execute immediate '
  ALTER TABLE BARS.DPT_VIDD_EXTDESC ADD CONSTRAINT FK_DPTVIDDEXTDESC_BRATES FOREIGN KEY (BASE_RATE)
...
begin   
 execute immediate '
  ALTER TABLE BARS.DPT_VIDD_EXTDESC ADD CONSTRAINT CC_DPTVIDDEXTDESC_INDVRATE CHECK (nvl(indv_rate, 1) >= 0 and nvl(indv_rate, 1) < 100) ENABLE NOVALIDATE';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
