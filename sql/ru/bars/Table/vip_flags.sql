...
PROMPT *** Create  constraint FK_VIP_FLAGS_VIP ***
begin   
 execute immediate '
  ALTER TABLE BARS.VIP_FLAGS ADD CONSTRAINT FK_VIP_FLAGS_VIP FOREIGN KEY (KVIP)
	  REFERENCES BARS.VIP_CALC_TP (ID) ENABLE';
exception when others then
...
grant ALTER,DEBUG,DELETE,FLASHBACK,INDEX,INSERT,ON COMMIT REFRESH,QUERY REWRITE,REFERENCES,SELECT,UPDATE on VIP_FLAGS       to BARS_DM;
grant SELECT                                                                 on VIP_FLAGS       to BARS_SUP;
