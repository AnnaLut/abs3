...
BEGIN 
        execute immediate  
          'begin  
               bpa.alter_policy_info(''BRATES'', ''FILIAL'' , null, null, null, null);
...
begin   
 execute immediate '
  ALTER TABLE BARS.BRATES ADD CONSTRAINT FK_BRATES_BRTYPES FOREIGN KEY (BR_TYPE)
	  REFERENCES BARS.BR_TYPES (BR_TYPE) ENABLE';
exception when others then
...
grant SELECT                                                                 on BRATES          to BARS;
...
grant SELECT                                                                 on BRATES          to BARS_SUP;
