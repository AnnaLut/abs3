...
PROMPT *** Create  constraint FK_DPUPAYMENTS_OPER ***
begin   
 execute immediate '
  ALTER TABLE BARS.DPU_PAYMENTS ADD CONSTRAINT FK_DPUPAYMENTS_OPER FOREIGN KEY (REF)
...
PROMPT *** Create  constraint FK_DPUPAYMENTS_DPUDEAL ***
begin   
 execute immediate '
  ALTER TABLE BARS.DPU_PAYMENTS ADD CONSTRAINT FK_DPUPAYMENTS_DPUDEAL FOREIGN KEY (DPU_ID)
...
begin   
 execute immediate '
  ALTER TABLE BARS.DPU_PAYMENTS MODIFY (REF CONSTRAINT CC_DPUPAYMENTS_REF_NN NOT NULL ENABLE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
begin   
 execute immediate '
  ALTER TABLE BARS.DPU_PAYMENTS MODIFY (DPU_ID CONSTRAINT CC_DPUPAYMENTS_DPUID_NN NOT NULL ENABLE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
grant SELECT                                                                 on DPU_PAYMENTS    to BARS_ACCESS_DEFROLE;
grant SELECT                                                                 on DPU_PAYMENTS    to BARS_SUP;
