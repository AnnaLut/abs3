...
BEGIN 
        execute immediate  
          'begin  
               bpa.alter_policy_info(''SKRYNKA_TARIFF2'', ''FILIAL'' , ''Q'', ''Q'', ''Q'', ''Q'');
...
begin   
 execute immediate '
  ALTER TABLE BARS.SKRYNKA_TARIFF2 ADD CONSTRAINT PK_SKRYNKATARIFF2 PRIMARY KEY (TARIFF, TARIFF_DATE, DAYSFROM, DAYSTO)
...
PROMPT *** Create  constraint CK_FLAG1_SKRYNKA_TARIFF2 ***
begin   
 execute immediate '
  ALTER TABLE BARS.SKRYNKA_TARIFF2 ADD CONSTRAINT CK_FLAG1_SKRYNKA_TARIFF2 CHECK (flag1 is not null) ENABLE';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
begin   
 execute immediate '
  CREATE UNIQUE INDEX BARS.PK_SKRYNKATARIFF2 ON BARS.SKRYNKA_TARIFF2 (TARIFF, TARIFF_DATE, DAYSFROM, DAYSTO) 
