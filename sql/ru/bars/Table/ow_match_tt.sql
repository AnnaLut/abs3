...
PROMPT *** Create  constraint FK_OWMATCHTT_DK ***
begin   
 execute immediate '
  ALTER TABLE BARS.OW_MATCH_TT ADD CONSTRAINT FK_OWMATCHTT_DK FOREIGN KEY (DK)
...
PROMPT *** Create  constraint FK_OWMATCHTT_TTS ***
begin   
 execute immediate '
  ALTER TABLE BARS.OW_MATCH_TT ADD CONSTRAINT FK_OWMATCHTT_TTS FOREIGN KEY (TT)
...
begin   
 execute immediate '
  ALTER TABLE BARS.OW_MATCH_TT MODIFY (DK CONSTRAINT CC_OWMATCHTT_DK_NN NOT NULL ENABLE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
begin   
 execute immediate '
  ALTER TABLE BARS.OW_MATCH_TT MODIFY (CODE CONSTRAINT CC_OWMATCHTT_CODE_NN NOT NULL ENABLE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
grant SELECT                                                                 on OW_MATCH_TT     to BARS_ACCESS_DEFROLE;
