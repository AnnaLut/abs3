...
begin   
 execute immediate '
  CREATE UNIQUE INDEX BARS.UK_TMP_OB22NU ON BARS.TMP_OB22NU (ACC) ';
exception when others then
  if  sqlcode=-955  then null; else raise; end if;
 end;
/
...
begin
  ...
  execute immediate 'ALTER TABLE BARS.TMP_OB22NU ADD (AP char(1))';
exception
  when others then
...
begin
  execute immediate 'DROP INDEX BARS.UK_TMP_OB22NU';
exception
  when others then
...
begin
  execute immediate 'CREATE UNIQUE INDEX BARS.UK_TMP_OB22NU ON BARS.TMP_OB22NU (ACC, AP)';
exception
  when others then
    if (sqlcode = -955) then null; else raise; end if;  
end;
