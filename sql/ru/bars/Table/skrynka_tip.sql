...
BEGIN 
        execute immediate  
          'begin  
               bpa.alter_policy_info(''SKRYNKA_TIP'', ''FILIAL'' , ''Q'', ''Q'', ''Q'', ''Q'');
...
begin   
 execute immediate '
  ALTER TABLE BARS.SKRYNKA_TIP ADD CONSTRAINT PK_SKRYNKATIP PRIMARY KEY (O_SK)
  USING INDEX PCTFREE 10 INITRANS 2 MAXTRANS 255 COMPUTE STATISTICS 
...
begin   
 execute immediate '
  CREATE UNIQUE INDEX BARS.PK_SKRYNKATIP ON BARS.SKRYNKA_TIP (O_SK) 
  PCTFREE 10 INITRANS 2 MAXTRANS 255 COMPUTE STATISTICS 
  TABLESPACE BRSMDLI ';
exception when others then
  if  sqlcode=-955  then null; else raise; end if;
