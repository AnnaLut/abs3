...
BEGIN 
        execute immediate  
          'begin  
               bpa.alter_policy_info(''CUST_ZAY'', ''FILIAL'' , null, null, null, null);
               bpa.alter_policy_info(''CUST_ZAY'', ''WHOLE'' , null, null, null, null);
               null;
           end; 
          '; 
END; 
...
begin   
 execute immediate '
  ALTER TABLE BARS.CUST_ZAY ADD CONSTRAINT FK_CUST_ZAY_RNK FOREIGN KEY (RNK)
	  REFERENCES BARS.CUSTOMER (RNK) ON DELETE CASCADE ENABLE';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
