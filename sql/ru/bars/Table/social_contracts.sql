...
begin   
 execute immediate '
  ALTER TABLE BARS.SOCIAL_CONTRACTS ADD CONSTRAINT FK_SOCIALCONTR_SOCIALAGENCY FOREIGN KEY (AGENCY_ID)
	  REFERENCES BARS.SOCIAL_AGENCY (AGENCY_ID) ENABLE';
exception when others then
...
PROMPT *** Create  constraint FK_SOCIALCONTR_ACCOUNTS ***
begin   
 execute immediate '
  ALTER TABLE BARS.SOCIAL_CONTRACTS ADD CONSTRAINT FK_SOCIALCONTR_ACCOUNTS FOREIGN KEY (ACC)
	  REFERENCES BARS.ACCOUNTS (ACC) DEFERRABLE ENABLE';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
