...
BEGIN 
        execute immediate  
          'begin  
               bpa.alter_policy_info(''ZAY_DEBT'', ''FILIAL'' , null, null, null, null);
               bpa.alter_policy_info(''ZAY_DEBT'', ''WHOLE'' , null, null, null, null);
               null;
           end; 
          '; 
END; 
...
PROMPT *** Create  constraint FK_ZAYDEBT_OPER2 ***
begin   
 execute immediate '
  ALTER TABLE BARS.ZAY_DEBT ADD CONSTRAINT FK_ZAYDEBT_OPER2 FOREIGN KEY (REFD)
...
PROMPT *** Create  constraint FK_ZAYDEBT_OPER ***
begin   
 execute immediate '
  ALTER TABLE BARS.ZAY_DEBT ADD CONSTRAINT FK_ZAYDEBT_OPER FOREIGN KEY (REF)
	  REFERENCES BARS.OPER (REF) ENABLE';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
...
begin   
 execute immediate '
  ALTER TABLE BARS.ZAY_DEBT ADD CONSTRAINT CC_ZAYDEBT_TIP CHECK (tip in (1, 2)) ENABLE';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
begin   
 execute immediate '
  ALTER TABLE BARS.ZAY_DEBT ADD CONSTRAINT CC_ZAYDEBT_SOS CHECK (sos in (0, 1, 2)) ENABLE';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
