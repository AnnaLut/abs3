...
--begin
--    execute immediate 
--'create index I_TEMP on CUSTOMER_ADDRESS (TRANSLATE(TRIM( REGEXP_REPLACE ( REGEXP_REPLACE (UPPER("BARS"."STRTOK"(ADDRESS,'','',1)),''^(\,|\.|ПЛ |ПЛ\.|ПР |ПР\.|ПРОВ |ПРОВ\.|ПЕР |ПЕР\.|\ПР-К |\ПР-К.|ПРОВУЛОК |ПРОВУЛОК\.|ПЕРЕУЛОК |ПЕРЕУЛОК\.|ВУЛ\.|ВУЛ |УЛ |УЛ\.|ВУЛИЦЯ |ПРОСПЕКТ |ПРОСП |ПРОСПЕКТ\.|ПРОСП\.|ПР-Т |ПР-Т\.|ПРТ |ПРТ\.)'',''''),''(\,|\.|\*|\"|\\|\/| Б | Б\.| БУД | БУДИНОК |\d)'','''',3)),''ETYUIOPAHKXCBM'',''ЕТУИІОРАНКХСВМ''))
--  tablespace BRSBIGI
-- pctfree 10
--  initrans 2
--  maxtrans 167
