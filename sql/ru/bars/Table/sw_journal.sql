...
begin   
 execute immediate '
  ALTER TABLE BARS.SW_JOURNAL ADD CONSTRAINT UK_SWJOURNAL UNIQUE (MT, IO_IND, TRN, SENDER, RECEIVER, CURRENCY, VDATE, PAGE)
...
begin   
 execute immediate '
  CREATE UNIQUE INDEX BARS.UK_SWJOURNAL ON BARS.SW_JOURNAL (MT, IO_IND, TRN, SENDER, RECEIVER, CURRENCY, VDATE, PAGE) 
  PCTFREE 10 INITRANS 2 MAXTRANS 255 COMPUTE STATISTICS 
  TABLESPACE BRSMDLI ';
exception when others then
  if  sqlcode=-955  then null; else raise; end if;
...
grant SELECT                                                                 on SW_JOURNAL      to BARS_ACCESS_DEFROLE;
