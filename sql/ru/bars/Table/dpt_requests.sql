...
BEGIN 
        execute immediate  
          'begin  
               bpa.alter_policy_info(''DPT_REQUESTS'', ''FILIAL'' , null, null, null, null);
               bpa.alter_policy_info(''DPT_REQUESTS'', ''WHOLE'' , null, null, null, null);
               null;
           end; 
          '; 
END; 
...
begin   
 execute immediate '
  ALTER TABLE BARS.DPT_REQUESTS ADD CONSTRAINT FK_DPTREQS_DPTREQTYPES FOREIGN KEY (REQTYPE_ID)
	  REFERENCES BARS.DPT_REQ_TYPES (REQTYPE_ID) ENABLE';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
