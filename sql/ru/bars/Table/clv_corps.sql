...
PROMPT *** Create  constraint FK_CLVCORPS_CLVREQ ***
begin   
 execute immediate '
  ALTER TABLE BARS.CLV_CORPS ADD CONSTRAINT FK_CLVCORPS_CLVREQ FOREIGN KEY (RNK)
	  REFERENCES BARS.CLV_REQUEST (RNK) ENABLE';
exception when others then
...
begin   
 execute immediate '
  ALTER TABLE BARS.CLV_CORPS MODIFY (RNK CONSTRAINT CC_CLVCORPS_RNK_NN NOT NULL ENABLE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
