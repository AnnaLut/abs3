...
PROMPT *** Create  constraint FK_DPUTYPESFREQ_FREQ ***
begin   
 execute immediate '
  ALTER TABLE BARS.DPU_TYPES_FREQ ADD CONSTRAINT FK_DPUTYPESFREQ_FREQ FOREIGN KEY (FREQ_ID)
	  REFERENCES BARS.FREQ (FREQ) ON DELETE CASCADE ENABLE';
exception when others then
...
PROMPT *** Create  constraint FK_DPUTYPESFREQ_DPUTYPES ***
begin   
 execute immediate '
  ALTER TABLE BARS.DPU_TYPES_FREQ ADD CONSTRAINT FK_DPUTYPESFREQ_DPUTYPES FOREIGN KEY (TYPE_ID)
	  REFERENCES BARS.DPU_TYPES (TYPE_ID) ON DELETE CASCADE ENABLE';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
grant SELECT                                                                 on DPU_TYPES_FREQ  to BARS_ACCESS_DEFROLE;
