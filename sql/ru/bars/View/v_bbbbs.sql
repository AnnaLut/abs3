...
PROMPT *** Create  view V_BBBBS ***
  ...
  CREATE OR REPLACE FORCE VIEW BARS.V_BBBBS ("SDAT", "BRANCH", "NBS", "S985A", "S756A", "S124A", "S840A", "S643A", "S978A", "S826A", "S959A", "S961A", "S933A", "S985P", "S756P", "S124P", "S840P", "S643P", "S978P", "S826P", "S959P", "S961P", "S933P", "S985S", "S756S", "S124S", "S840S", "S643S", "S978S", "S826S", "S959S", "S961S", "S933S") AS 
  SELECT SUBSTR ( (pul.get_mas_ini_val ('sFdat1')), 1, 10) sdat,
     ...
     FROM (  SELECT branch,
                    ...
                    -SUM (DECODE (kv, 933, sa, 0)) / 100 S933A,
                     --------------------------
                    ...
                    SUM (DECODE (kv, 933, sp, 0)) / 100 S933p
               FROM (SELECT branch,
                            kv,
                            NBS,
                       ...
                       FROM (SELECT a.branch,
                               ...
                               FROM v_gl a
                              WHERE     a.nbs LIKE
                                           NVL (pul.get_mas_ini_val ('sFdat2'),
                                                '3800')
                                    AND a.kv <> 980))
           GROUP BY branch, NBS);
