...
PROMPT *** Create  view V_CIM_BOUND_VMD ***
  ...
  select a.bound_id, a.contr_id, a.vmd_id, a.direct, 0, (select max(name) from bars.cim_act_types where type_id=0), a.num,
   ...
   from (select b.bound_id, b.contr_id, b.vmd_id, b.direct, v.cnum_cst||'/'||v.cnum_year||'/'||to_char(v.cnum_num,'fm000000') as num, v.cim_date as doc_date,
           ...
           from bars.cim_link where delete_date is null and vmd_id=b.bound_id)/100,2) as s,
                (select round(nvl(sum(case when v.allow_dat+c.deadline<nvl2(l.payment_id,
                        (select max(vdat) from bars.oper where ref=(select max(ref)from bars.cim_payments_bound where bound_id=l.payment_id)),
                           (select max(val_date) from bars.cim_fantom_payments
                              where fantom_id=(select max(fantom_id) from bars.cim_fantoms_bound where bound_id=l.fantom_id))) then s else 0 end),0)
                             /100,2)
                 from bars.cim_link l where l.delete_date is null and l.vmd_id=b.bound_id
                ) as s_after, b.create_date, b.modify_date as modify_date, b.borg_reason
         from bars.cim_vmd_bound b, bars.customs_decl v, bars.cim_contracts_trade c
   ...
   from (select b.contr_id, b.bound_id, b.act_id, b.direct, a.act_type, a.file_name, a.file_date,
                (select max(name) from bars.cim_act_types where type_id=a.act_type) as type_name,
                ...
                round((select nvl(sum(s),0) from bars.cim_link where delete_date is null and act_id=b.bound_id)/100,2) as s,
                (select round(nvl(sum(case when a.allow_date+c.deadline<nvl2(l.payment_id,
                           (select max(vdat) from bars.oper where ref=(select max(ref)from bars.cim_payments_bound where bound_id=l.payment_id)),
                           (select max(val_date)
                              from bars.cim_fantom_payments
                              where fantom_id=(select max(fantom_id) from bars.cim_fantoms_bound where bound_id=l.fantom_id)))
                                     then s else 0 end),0)/100,2)
                 from bars.cim_link l where l.delete_date is null and l.act_id=b.bound_id
                ) as s_after, b.create_date, b.modify_date as modify_date, b.borg_reason
         from bars.cim_act_bound b, bars.cim_acts a, bars.cim_contracts_trade c
