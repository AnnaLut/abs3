CREATE OR REPLACE FORCE VIEW BARS.V_SWAP
(
   B,
   DAT,
   NTIK,
   ...
   SELECT B,
     ...
     FROM (SELECT d.B,
            ...
            WHERE     EXISTS
                  ...
                  AND c.rnk = x.rnk
                  AND NVL (x.sos, 10) < 15
                  --  AND x.dat >= SYSDATE - 30
                  AND (       FOREX.get_forextype3 (x.deal_tag) LIKE
                                 'SWAP%EN'
                          AND x.deal_tag <> x.swap_tag
                       OR x.swap_tag IS NULL)) S;


GRANT SELECT ON BARS.V_SWAP TO BARS_ACCESS_DEFROLE;

GRANT SELECT ON BARS.V_SWAP TO FOREX;

GRANT SELECT ON BARS.V_SWAP TO START1;
