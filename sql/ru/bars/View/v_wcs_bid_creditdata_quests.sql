...
PROMPT *** Create  view V_WCS_BID_CREDITDATA_QUESTS ***
  ...
  CREATE OR REPLACE FORCE VIEW BARS.V_WCS_BID_CREDITDATA_QUESTS ("BID_ID", "CRDDATA_ID", "CRDDATA_NAME", "QUESTION_ID", "QUESTION_NAME", "TYPE_ID", "IS_CALCABLE", "IS_READONLY", "IS_CHECKABLE", "CHECK_PROC", "HAS_REL", "GROUP_ID", "IS_REQUIRED") AS 
  SELECT b.id AS bid_id,
      ...
      WHERE     b.subproduct_id = scd.subproduct_id
            ...
            AND (scd.dnshow_if IS NULL
                 OR nvl(wcs_utl.calc_sql_bool(b.id, scd.dnshow_if,p_question_id=>scd.question_id),0) != 1)
   ORDER BY b.id,scd.ord;
