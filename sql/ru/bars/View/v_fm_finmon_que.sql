...
PROMPT *** Create  view V_FM_FINMON_QUE ***
  ...
  CREATE OR REPLACE FORCE VIEW BARS.V_FM_FINMON_QUE ("ID", "REF", "REFTYPE", "ND", "FDATE", "FTIME", "KV", "S", "SQ", "DATD", "ID_A", "MFOA", "NAM_A", "NLSA", "ID_B", "MFOB", "NAM_B", "NLSB", "NAZN", "FIO", "VOBNAME", "OZN", "STATUS", "MONITOR_MODE", "OPR_VID1", "OPR_VID2", "COMM_VID2", "OPR_VID3", "COMM_VID3", "RNK_A", "RNK_B", "OPR_OBL_KOD", "OPR_TERROR", "OPR_NBU", "DAT_I", "PDAT") AS 
  SELECT f.id,
          f.rec,
          'н' AS refType,
          O.ND,
     ...
     FROM                                                    /*BARS.STAFF S,*/
         BARS.VOB V,
          BARS.ARC_RRP o,
          bars.finmon_que f
          LEFT JOIN BARS.STAFF S ON S.ID = f.AGENT_ID
    WHERE                                               /*S.ID=O.USERID AND */
         V    .VOB = O.VOB
          AND f.rec IS NOT NULL
          AND f.REF IS NULL
          AND f.rec = o.rec
   UNION ALL
   SELECT p.id,
    ...
    WHERE     p.REF IS NOT NULL
          AND (   (S.ID = p.AGENT_ID AND p.AGENT_ID IS NOT NULL)
               OR (S.ID = O.USERID AND p.AGENT_ID IS NULL))
          AND V.VOB = O.VOB
          AND O.REF = p.REF;
