 ...
 END attribute_value
  FROM (SELECT tt.*
          FROM (SELECT
                ...
                ,nv.value number_value
                ,dv.value date_value
                ,sv.value string_value
                  FROM deal d
                  JOIN object_type o
                  ...
                  LEFT JOIN attribute_number_value nv
                    ON nv.object_id = d.id
                   AND nv.attribute_id = a.id
                  LEFT JOIN attribute_date_value dv
                    ON dv.object_id = d.id
                   AND dv.attribute_id = a.id
                  LEFT JOIN attribute_string_value sv
                    ON sv.object_id = d.id
                   AND sv.attribute_id = a.id) tt
         WHERE 1 = 1
