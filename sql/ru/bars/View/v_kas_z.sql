  ...
  SELECT sos,
          ...
          REPLACE (
             INITCAP (DECODE (vid,
                              ...
                              4, (SELECT UNIQUE replace(replace (replace (REPLACE (name, '"', ''), 'лотерейні білети','Лот.біл.'), 'інвестиц.монети','Інв.мон.'), 'номінал','ном.')
                                    FROM valuables
                                   WHERE ob22 = kz.kodv),
                              3, (SELECT UNIQUE REPLACE (namemoney, '"', '')
                                    FROM spr_mon
                                   WHERE kod_money = kz.kodv),
                              2, (SELECT UNIQUE replace(replace(REPLACE (name, '"', ''), '/',''),'\','')
                                    FROM bank_metals
                                   WHERE kod = kz.kodv),
                              kz.kodv)),
             q'[']',
