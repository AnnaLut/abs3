...
begin
  ...
  if p_mode = 1 then
     for z in ( select b.acc, c.nmk, c.rnk, to_date(w.value, 'dd/mm/yyyy') dat
                  from w4_acc o, accounts b, customer c, accountsw w
     ...
     loop
        ...
        insert into tmp_bpk_arsenal57 (nmk, acc, open_date, lim_begin, lim)
        values (z.nmk, z.rnk, z.dat, l_ost, abs(round(l_int)));
     end loop;
  end if;
end p_bpk_rep;
/
