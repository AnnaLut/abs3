...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%/%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 6     P          R013                                         
 7     X          S181 начальный срок погашения              
 8     L          S240 срок к погашению                      
 9     R          K030 резидентность  (1/2)                    
10     Щ          R012 вид счета                            
11     VVV        R030 код валюты                              
...
-------------------------------------------------------------------
   ...
   BEGIN
      ...
      IF mfou_ IN (300465)
      THEN
         IF SUBSTR (nbs_, 1, 3) IN ('140', '301') AND s180_ = '0'
         THEN
            s180_ := '1';
         END IF;

         IF SUBSTR (nbs_, 1, 3) IN ('140', '301') AND s242_ = '0'
         THEN
            s242_ := '1';
         END IF;
      END IF;
      ...
      if (nbs_ in ('1600', '2600', '2605', '2620', '2625', '2650', '2655') and sn_ < 0  or
          nbs_ = '1500') and
         (x_ <> '1' or s242_ <> '1')
      then
        if x_ <> '1' then
      ...
      then
        if x_ <> '1' then
         ...
         if nbs_||r013_ in ('16001', '2600A', '26053', '26203', '26252', '26501', '26553') and
            daos_ >= to_date('06062015','ddmmyyyy')
         then
             if s242_ not in ('1','2','3','4','5') then
                s242_ := '5';
                        ...
                        FROM (SELECT /*+ parallel(8) */
                               ...
                               WHERE aa.acc = s.acc and 
                                     nvl(aa.nbs, substr(aa.nls,1,4)) not in (''1490'',''1491'',''1492'',''1493'',
                                        ''1590'',''1592'',''1890'',''2400'',''2401'',''2890'',''3190'',''3290'',''3590'',''3599'') ) a,
                             kl_r030 l,
                             specparam p,
                             customer c
                       WHERE a.ost <> 0
...
--          mdate_ := f_mdate_hist(acc_, dat_);
             ...
             EXCEPTION
                ...
                THEN
                   fa7p_ := (case when substr(nls_,1,1) in ('1','2', '3') and
                        (substr(nls_,4,1) in ('8', '9') or substr(nls_,1,4) in ('2607', '2627'))
                        then 1 else 0 end);
             END;
    ...
    ------- !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                   ...
                   THEN
                      ...
                      THEN
                         ...
                         THEN
                            ...
                            THEN
                               kodp_ :=
                                  dk_ || nbs_ || o_r013_1 || x_ || s242_ || r031_;
                            ELSE
                               pp_doda;
                         ...
                         THEN
                            ...
                            THEN
                               kodp_ :=
                                  dk_ || nbs_ || o_r013_2 || x_ || s242_ || r031_;
                            ELSE
                               pp_doda;
                ...
                IF datn_ < zm_date_
                THEN
                   kodp_ := dk_ || nbs_ || r013_ || x_ || s242_ || r031_;
                ELSE
                   pp_doda;
             ...
             THEN
                ...
                THEN
                   ...
                   LOOP
                      ...
                      IF s242_ IN ('9', 'C', 'D', 'E', 'F', 'G', 'H') AND x_ = '1'
                      THEN
                         x_ := '2';
                      END IF;
                   ...
                   BEGIN
                      ...
                      THEN
                         SELECT gl.p_icurval (v.kv, v.min_summ * 100, dat_)
                          ...
                          WHERE c.acc = acc_
                            AND c.vidd = v.vidd
                            AND (   LOWER (v.type_name) LIKE '%рост%'
                                 OR LOWER (v.type_name) LIKE '%сбер%'
                                 OR LOWER (v.type_name) LIKE '%ощад%'
                                 OR LOWER (v.type_name) LIKE '%раст%'
                                )
                            AND ROWNUM = 1;
                      END IF;

                      IF 300175 IN (mfo_, mfou_)
                      THEN
...
--   определение s240 для счетов 2924 по ob22 (Киевгород)
                   ...
                   THEN
                      ...
                      THEN
                         x_ := '1';
                         s242_ := '1';
                      ELSIF nbs_ IN ('1911', '3739')
                      THEN
                         x_ := '1';
                         s242_ := '3';
                      ELSIF nbs_ IN ('2909', '3578')
                      THEN
                         BEGIN
                            SELECT COUNT (*)
                              INTO exists_
  ...
  values (nd_, acc_, rnls_, kv_, kodp_, abs(se_), abs(se_), 0, 0);
   ...
   for i in 1..l_rec_t8.count loop
      ...
      if o_se_1 <> 0 and o_se_2 <> 0 then
          for k in (select rowid, kodp, znap, comm, substr(kodp,6,1) r013
                     from rnbu_trace
                     where acc = acc_
                     order by recid)
          loop
              ...
              if o_se_1z <> 0 and k.r013 <> o_r013_1 then
                  if abs(o_se_1z) <= to_number(k.znap) then
                     ...
                     select null RECID, USERID, NLS, KV, ODATE,
                        substr(kodp,1,5)||o_r013_2||substr(kodp,7), to_char(se1_), NBUC,
                        ISP, RNK, ACC, REF, COMM, ND, MDATE, TOBO
                     from rnbu_trace
                     where rowid = k.rowid;

                     update rnbu_trace
                      set kodp = substr(kodp,1,5)||o_r013_1||substr(kodp,7),
                          znap = znap_
                      where rowid = k.rowid;
                  else
                     znap_ := k.znap;
              ...
              if o_se_2z <> 0 and k.r013 <> o_r013_2 and se1_ = 0 then
                  if abs(o_se_2z) <= to_number(k.znap) then
                     ...
                     update rnbu_trace
                      set kodp = substr(kodp,1,5)||o_r013_2||substr(kodp,7),
                          znap = znap_
                      where rowid = k.rowid;
                  else
                     znap_ := k.znap;
          ...
          update rnbu_trace
          set kodp = substr(kodp,1,5)||r013_||substr(kodp,7)
          where acc = acc_;
      end if;
   end loop;
   ...
   for k in (select acc, nls, kv, rnk, s080, szq, szq_30, isp, mdate, tobo, nbs, kodp, s240, sump, suma,
             ...
             from (
                 select t.acc, t.nls, decode(t.kv, 974, 933, t.kv) kv, t.rnk, t.s080,
                        ...
                        s.r013, NVL (DECODE (c.country, 804, '1', '2'), '1') rz,
                 ...
                 from v_tmp_rez_risk_c5_new t,
                  ...
                  where t.dat = datr_ and
                      ...
                      nvl(a.nbs, substr(a.nls, 1,4)) in (select r020
                                from tmp_kod_r020
                                WHERE r020 not in ('2924', '9001', '9020', '9100','9023','9122','9129')
   ...
   loop
      ...
      if k.szq <>0 then
          if TP_SND then
             ...
             znap_ := to_char(sum_);
             comm_ := SUBSTR (k.tobo || ' резерв під проcтрочені відсотки відносимо до R012='||r012_, 1,100);
          else
             r012_:='A';
             ...
             if k.cnt = 1 then
                znap_ := to_char(srez_);
                comm_ := SUBSTR (k.tobo || ' резерв під осн.борг відносимо до R012='||r012_, 1,100);
             else
                sum_ := round(srez_ * k.koef);
                ...
                comm_ := SUBSTR (k.tobo || ' резерв під осн.борг відносимо до R012='||r012_||' (розбивка по S240='||k.s240||')' , 1,200);
             end if;
          end if;
...
--        оствшийся нераспределенный резерв с R013=r013_,R012=B
             ...
             if srezp_ <> 0  then
               ...
               znap_ := srezp_;
               comm_ := SUBSTR (k.tobo || ' перевищення резерву над осн. боргом до R012='||r012_ , 1,100);
...
--  для нових договорів: є тільки сума договору, немає резерву,дисконту,премії
   ...
   for k in (select /*+ parallel(8) */
                    acc, nbs, nls, kv, rnk, s080, szq, isp, mdate, tobo, s240, s180, r031, r030, r013,
                    rez, discont, prem, nd, id, ob22, custtype, accr, tip 
             from (
                 SELECT
                T.ACC,
                 T.NLS,
            ...
            FROM V_TMP_REZ_RISK_C5_NEW T,
                 ...
                 KL_R030 L
           WHERE T.DAT = datr_
                 AND NOT exists (SELECT ACC FROM RNBU_TRACE r where r.acc = T.ACC and substr(kodp,1,5) not in ('21600', '22600', '22605', '22620', '22625', '22650', '22655'))
                 and T.ID not LIKE 'NLO%'
                 AND T.ACC = A.ACC
                 AND T.ACC = S.ACC(+)
                 AND A.KV = TO_NUMBER (L.R030)
                 AND nvl(A.NBS, substr(a.nls,1,4)) NOT IN ('2924', '9001', '9020', '9023', '9100', '9122', '9129')
   ...
   loop
      ...
      if k.nbs in ('2605', '2607', '2625') then
         ...
         if kodp1_ is not null then
            s181_ := substr(kodp1_, 7, 1);
            s240_ := substr(kodp1_, 8, 1);
         end if;
      end if;
      ...
      if k.szq <> 0 then
          if TP_SND then
             ...
             if sakt_ = 0 then
                s240_ := '1';
                comm_ := SUBSTR (k.tobo || ' резерв під погашені проценти (залишок = 0) відносимо до R012='||r012_, 1,100);
             else
                comm_ := SUBSTR (k.tobo || ' резерв під прострочені проценти відносимо до R012='||r012_, 1,100);
             end if;
          ...
          if srezp_ <> 0 and not TP_SND then
             ...
             if sakt_ = 0 then
                comm_ := SUBSTR (k.tobo || ' резерв за осн.боргом, у якого залишок = 0, відносимо до R012='||r012_, 1,100);
             else
                comm_ := SUBSTR (k.tobo || ' перевищення резерву над осн.боргом до R012='||r012_, 1,100);
             end if;
...
----------------------------------------------------
   ...
   begin
       ...
       update rnbu_trace
          set kodp = substr(kodp,1,7)||'Z'||substr(kodp,9),
...
--  ограничение s240 для "отзывных" депозитов после 06.06.2015
   ...
   for k in ( select /*+ parallel(r,8) */
                     r.rowid, r.kodp, r.znap, r.comm, substr(r.kodp,8,1) s240
                from rnbu_trace r,
                    (   select da.accid
   ...
   loop
       ...
       if k.s240 not in ('0','1','2','3','4','5','Z')  then

            update rnbu_trace
               set kodp = substr(k.kodp,1,7)||'5'||substr(k.kodp,9),
                   comm = substr(k.comm || ' заміна S240 ' || substr(k.kodp,8,1) ||
                 ...
                 from (select nbuc, rez, t020, nbs, kv, 
                     ...
                     (select  /*+ parallel(8) */
                         ...
                         where substr(kodp, 2, 4) in ('1410','1412','1415','1416','1417','1418',
                                       ...
                                       '2400','2401','2890','3190','3290','3590','3599','3690',
                                       '9010','9015','9030','9031','9036','9500')
                         group by nbuc, substr(kodp, 1, 1), substr(kodp,9,1), substr(kodp, 2, 4), kv) b
            ...
            begin
               if k.nbs not in ('2400','2401') then
                   ...
                   exception
                      when no_data_found then
                           begin
                              ...
                              from rnbu_trace
                              where nbuc = k.nbuc and
                                    kodp like k.t020||k.nbs||'___'||k.rez||'A'||lpad(k.kv, 3, '0')||'%' and
                  ...
                  begin
                     ...
                     from rnbu_trace
                     where nbuc = k.nbuc and
                           kodp like k.t020||k.nbs||'4__'||k.rez||'B'||lpad(k.kv, 3, '0')||'%' and
                           substr(kodp, 6, 1)||substr(kodp, 10, 1) = substr(k.R013_s580,2,2) and
                           (sign(k.rizn) = -1 and to_number(znap) >= abs(k.rizn) or
                           sign(k.rizn) = 1 and to_number(znap) > 0) and
                           rownum = 1;
                  exception
                     when no_data_found then
                        begin
                           ...
                           from rnbu_trace
                           where nbuc = k.nbuc and
                                 kodp like k.t020||k.nbs||'5__'||k.rez||'B'||lpad(k.kv, 3, '0')||'%' and
                                 substr(kodp, 6, 1)||substr(kodp, 10, 1) = substr(k.R013_s580,2,2) and
                                 (sign(k.rizn) = -1 and to_number(znap) >= abs(k.rizn) or
                                 sign(k.rizn) = 1 and to_number(znap) > 0) and
                                 rownum = 1;
...
--   delete from TMP_OTCN_FA7_REZ1 ;
--   insert into TMP_OTCN_FA7_REZ1 
--   select *
--   from OTCN_FA7_REZ1;   
--   
--   delete from TMP_OTCN_FA7_REZ2 ;   
--   insert into TMP_OTCN_FA7_REZ2 
--   select *
--   from OTCN_FA7_REZ2;
------------------------------------------------------------------
--exception
