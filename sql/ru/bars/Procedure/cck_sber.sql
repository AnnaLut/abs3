...
PROMPT *** Create  procedure CCK_SBER ***
  ...
  CREATE OR REPLACE PROCEDURE BARS.CCK_SBER (tip_ in Varchar2 ,vid_ in varchar2, p_branch in varchar2) is
...
-- Перенос на прострочку где день погашения % долга определяется по ГПК (на 01.04.2015 - нигде не используется)
  if  vid_ = '1' then
     ...
     select d.ND, d.rnk, d.CC_ID, c.OKPO, c.nmk, substr(t.txt,2,1) as DP, d.vidd , d.sdate, i.basem,i.basey,a.kv,
              (select max(l.fdat) from cc_lim l where l.nd=d.nd and fdat>l_dat2 and l.fdat<=l_dat1 and l.sumo>0) pl_dat
               from cc_deal d, saldo a, customer c, int_accn i, nd_acc n, nd_txt t, cc_add da
              where (tip_<=2 and vidd in (1,2,3)   or
                     tip_=3 and vidd in (11,12,13) )
                AND d.sos >0 AND d.sos<14
                AND d.nd=n.nd and n.acc=a.acc and a.tip='LIM' AND d.rnk=c.rnk and da.nd=d.nd
                AND i.acc=a.acc and i.id=0 and d.nd=t.nd and t.tag='FLAGS'
                -- установлено ежемесячное погашение
                and (nvl(to_number(cck_app.get_nd_txt (d.ND,'FREQP'),'99999999999D99', 'NLS_NUMERIC_CHARACTERS = ''. ''' ),da.freq)!=400 or d.wdate<gl.bdate)
                -- узнаем платежный ли день
                and exists (select 1 from cc_lim l where l.nd=d.nd and fdat>l_dat2 and l.fdat<=l_dat1 and l.sumo>0)
               -- дата первого погашения прошла
                and  nvl(nvl(to_date(cck_app.get_nd_txt (d.ND,'DATSN'),'dd/mm/yyyy'),i.apl_dat),d.sdate)<=cck_app.correctDate2(980,gl.bd-1,0)
             )
     loop
          ...
          If k.vidd = 11 and k.basem = 1 and k.basey =2  then
             begin
                ...
                select nvl(MAX(i.acr_dat),k.pl_dat-1), max(i.acra) into l_Acr_dat, l_acc_sn from int_accn i, accounts a, nd_acc n where n.nd=k.nd and n.acc=a.acc and i.acc=a.acc and i.id = 0 and a.tip='SS ' and a.dazs is null and a.kv=k.kv;
                -- Есть ли остаток на сч нач.% ?
                select abs(a.ostc) into l_ostc_sn from accounts a where a.acc = l_acc_sn and ostc < 0 and ostc=ostb and a.tip='SN ' and a.kv=k.kv;
                ...
                If k.pl_DAT<= l_Acr_dat  then
                   l_max := greatest( l_ostc_sn - cck.FINT ( k.ND,  k.pl_DAT, l_Acr_dat), 0);
                   else
          ...
          elsif k.DP=0 then -- за прошлый день (когда % платяться по предыдущий день)
                  cck.CC_ASPN_DOG(k.nd,k.cc_id,k.okpo,k.nmk,-3, null);
          else -- за прошлый месяц
...
-- Перенос на прострочку где день погашения %  определяется по ДНЮ погашения (аннуитет )
  if  vid_ = '4' then
      ...
      for k in (select d.ND, d.rnk, d.CC_ID, c.OKPO, c.nmk, substr(t.txt,2,1) as DP, d.vidd , d.sdate, i.basem,i.basey,a.kv,
     (select max(l.fdat) from cc_lim l where l.nd=d.nd and fdat>l_dat2 and l.fdat<=l_dat1 and l.sumo>0) pl_dat
               from cc_deal d, saldo a, customer c, int_accn i, nd_acc n, nd_txt t, cc_add da
              where (tip_<=2 and vidd in (1,2,3)   or
                     tip_=3 and vidd in (11,12,13) )
                AND d.sos >0 AND d.sos<14
                AND d.nd=n.nd and n.acc=a.acc and a.tip='LIM' AND d.rnk=c.rnk and da.nd=d.nd
                and nvl(to_number(cck_app.get_nd_txt (d.ND,'FREQP'),'99999999999D99', 'NLS_NUMERIC_CHARACTERS = ''. ''' ),da.freq)!=400
                AND i.acc=a.acc and i.id=0 and d.nd=t.nd and t.tag='FLAGS'
                AND
           ...
           (cck_app.correctDate2(980,( case when cck_app.valid_date(cck_app.Pay_Day_SN_to_ND (d.nd)||to_char(gl.bd,'/mm/yyyy') )<=gl.bd
                               then
                               cck_app.valid_date(cck_app.Pay_Day_SN_to_ND (d.nd)||to_char(gl.bd,'/mm/yyyy')  )
                               else
                               cck_app.valid_date(cck_app.Pay_Day_SN_to_ND (d.nd)||to_char(add_months(gl.bd,-1),'/mm/yyyy')  )
                               end
                              ),
                   ...
                   -- Теперь только вперед
                   cck_app.correctDate2(980,d.wdate,1)=cck_app.correctDate2(980,gl.bd-1,1)
                     )
                     --  дата первого погашения прошла
                and  nvl(nvl(to_date(cck_app.get_nd_txt (d.ND,'DATSN'),'dd/mm/yyyy'),i.apl_dat),d.sdate)<=cck_app.correctDate2(980,gl.bd-1,0)
             )
...
if vid_=5 then
  for k in ( select d.ND,d.CC_ID, c.OKPO, c.nmk, substr(t.txt,2,1) as DP
               from cc_deal d, accounts a, customer c, int_accn i, nd_acc n, nd_txt t, cc_add da
              where (tip_<=2 and vidd in (1,2,3)   or
                     tip_=3 and vidd in (11,12,13) )
                AND d.sos >0 AND d.sos<14
                AND d.nd=n.nd and n.acc=a.acc and a.tip='LIM' AND d.rnk=c.rnk and da.nd=d.nd
                ...
                AND to_char(gl.bd,'yyyymmdd')>
                         to_char(cck.CorrectDate(980, gl.bd-1, gl.bd-2),'yyyymm')||
                         nvl( (select lpad(txt,2,'0')
                                 from nd_txt
                                where nd=d.nd and tag='DAYSN')
