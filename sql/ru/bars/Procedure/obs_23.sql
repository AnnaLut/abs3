     ...
     if p_mode=3 THEN -- and ( getglobaloption('MFOP') = '300465' or gl.aMfo = '300465' ) then
        ...
        begin
           ...
           loop
              ...
              if k.nd=17011 then
                 logger.INFO('OBS23 (8) - nd='||k.nd||'Обс.='||obs_bpk_);
                 logger.INFO('OBS23 (8) - nd='||k.nd||'кат.='||l_kat23);
                 logger.INFO('OBS23 (8) - nd='||k.nd||'K.  ='||l_k23);
                 logger.INFO('OBS23 (8) - nd='||k.nd||'Фин.='||l_fin23);
              end if;
        ...
        begin
           for k in (select d.nd, d.vidd, 1 OBS,d.SDATE sdate,d.rnk, d.WDATE wdate,s.ostf-s.dos+s.kos ost
                     ...
                     where d.sos >=10 and d.sos<14 and  d.vidd >1000  and ad.accs=a.acc and d.nd=ad.nd
                           and ad.adds=0 and ad.accs=s.acc and s.ostf-s.dos+s.kos<>0 and d.wdate = a.mdate
                           and d.obs23 is null and (s.acc,s.fdat)=(select acc,max(fdat) from saldoa
                                                                   where acc=s.acc and fdat<=DAT_31
                                                                   group by acc)
                    )
           LOOP
              begin
        ...
        LOOP
           ...
           begin
              ...
              EXCEPTION WHEN NO_DATA_FOUND THEN
                 OPEN_ := 0;
              END;
              --logger.INFO('OBS23 - nd='||k.nd||'OPEN='||open_);
              if OPEN_=1 THEN
                 ...
                 --logger.INFO('OBS23 - nd='||k.nd||'pr_='||pr_);
                 if DATP_ is not null THEn
                    ...
                    if pr_=4 THEN
                       --logger.INFO('OBS23 -уходим nd='||k.nd||'obs_='||obs_);
                       goto M1;
                    end if;
                 end if;
                 ...
                 if (k.wdate+180 <= DAT_) then
                    ...
                    KOL_n := dat_-k.wdate;
                    KOL_  := kol_n;
                    logger.INFO('CCK_KAT - 8 k.s250='||k.s250);
                    GOTO M1;
                 end if;
                 --logger.INFO('OBS23 - Старый nd='||k.nd||'Обс.='||obs_);
                 --узнать тек остаток
                 begin
