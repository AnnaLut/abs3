...
LOOP
 i:= i+1;
  ...
  loop
    --dbms_application_info.set_client_info ('Формування acc '||k.acc);
     sTmp:= 0;
  select sum(case when not regexp_like(ad.nbs,'^2(0|1)') then ok.s else 0 end) kos
    into sTmp
    from (select * from saldoa where acc = k.acc and fdat between trunc(p_dat,'mm') and p_dat) s,
         opldok od, accounts ad,
         opldok ok--, accounts ak
    where s.fdat = ok.fdat and ok.dk = 1 and ok.acc = s.acc
