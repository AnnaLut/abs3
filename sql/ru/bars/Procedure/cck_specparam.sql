...
PROMPT *** Create  procedure CCK_SPECPARAM ***
  ...
  CREATE OR REPLACE PROCEDURE BARS.CCK_SPECPARAM 
(ACC_   int,       -- вн. номер подвязываемого счета под договор
...
l_SDATL:=SDATE_;
   ...
   If P080_ is null then
      begin
         SELECT s080
           ...
           WHERE f.fin=c.crisk and f.obs=d.obs and c.rnk=d.rnk and d.nd=ND_;
      EXCEPTION WHEN NO_DATA_FOUND THEN l_S080:=1;
      end;
   else
     l_S080:= P080_;
   end if;
  ...
  if    substr(NLS_,1,4)='2909' and VIDD_ in (1,2,3)    and TIP_ = 'SG ' then
        l_R011:=1;
  elsif substr(NLS_,1,4)='2909' and VIDD_ in (11,12,13) and TIP_ = 'SG ' then
        l_R011:=2;
  elsif tip_ in ('SP ', 'SN ', 'SPN', 'SL ', 'SLN','SK0','SK9') then
        l_R011:=substr(cck.R011_S181 (ACC_,ND_),1,1);
   end if;

   If    TIP_ in ('SN ','SK0')   then l_R013:='3';
   ElsIf TIP_ in ('SPN','SK9')   then l_R013:='1';
   ElsIf TIP_='SLN'              then l_R013:='3';
   ElsIf TIP_='CR9' then
      If gl.aMFO='353575' then l_R013:='1';
      else                     l_R013:='9';
      end if ;
   end if;
...
begin
  select substr(txt,1,2) into l_s260 from nd_txt where nd=ND_ and tag='S260';
exception when no_data_found then
   select min(s260) into l_s260 from cc_potra p,  cc_deal d  where d.nd = ND_ and d.prod = p.id;
end;
...
-----------------------------------------------------------------------
 ...
 if cck.g_cck_migr=0 then
   ...
   if SQL%rowcount = 0 then
      ...
      If    TIP_ ='SS ' then
             -- r011 отсутствует
         Insert into specparam (acc,s080,s200,s260,r013) values (Acc_,l_S080,l_S200,l_S260,l_r013);

      elsif Tip_ in ('SP ','SL ') then
         Insert into specparam (acc,R011,  s080, s190,  s200,s260 )
                     select ACC_,l_R011, s.S080,   '3', s.s200,l_S260
                       from specparam s
                      where s.acc=(select accs from cc_add where adds=0 and nd=ND_);

      elsif TIP_ in ('SN ','SPN','SLN' ) then
         Insert into specparam (acc, r011 ,s260  , r013)
         values               (ACC_,l_R011,l_S260,l_R013);

      elsif TIP_ in ('SK0','SK9' ) then
         Insert into specparam(acc, r011, s260  , r013)
         values               (ACC_,'Z',l_S260,l_R013);

      elsif tip_ = 'CR9' then
         insert into specparam (acc,r013,s080,s260) values (acc_,l_r013,l_S080,l_S260);

      elsif tip_ = 'SG '  then
         insert into specparam (acc,r013,s260) values (acc_,l_r013,l_S260);

      elsif tip_ in ( 'SDI','SPI') then
         insert into specparam (acc,s080,s260) values (acc_,l_S080,l_S260);
      end if;
