...
begin
   ...
   from  (select nls_rez,kv,acc_rez,nvl(sum(rez_0),0) s1,nvl(sum(rezq_0),0) sq1 from nbu23_rez, V_SFDAT v
          where  fdat=dat01_ and acc_rez <>0 and rez_0 <>0 group by nls_rez,kv,acc_rez
          union  all
          select nls_rezn,kv,acc_rezn,nvl(sum(rezn),0),nvl(sum(reznq),0) from nbu23_rez, V_SFDAT v
          where  fdat=dat01_ and acc_rezn<> 0 and rezn <>0 group by nls_rezn,kv,acc_rezn
          union  all
          select nls_rez_30,kv,acc_rez_30,nvl(sum(rez_30),0),nvl(sum(rezq_30),0) from nbu23_rez, V_SFDAT v
          where  fdat=dat01_ and acc_rez_30<> 0 and rez_30 <>0 group by nls_rez_30,kv,acc_rez_30
          union  all
          select nls_rez,kv,nvl(acc_rez,0),nvl(sum(rez_0),0),nvl(sum(rezq_0),0) from nbu23_rez, V_SFDAT v
          where  fdat=dat01_ and acc_rez is null and rez_0 <>0 group by nls_rez,kv,nvl(acc_rez,0)
          union  all
          select nls_rezn,kv,nvl(acc_rezn,0),nvl(sum(Rezn),0),nvl(sum(Reznq),0) from nbu23_rez, V_SFDAT v
          where  fdat=dat01_ and acc_rezn is null and rezn <>0 group by nls_rezn,kv,nvl(acc_rezn,0)
          union  all
          select nls_rez_30,kv,nvl(acc_rez_30,0),nvl(sum(rez_30),0),nvl(sum(rezq_30),0) from nbu23_rez, V_SFDAT v
          where  fdat=dat01_ and acc_rez_30 is null and rez_30 <>0 group by nls_rez_30,kv,nvl(acc_rez_30,0)) x,
         (select l.nlsa,l.kv,l.ob22,l.nbs,l.acc,decode(dk,1,l.ost+s,-1,l.ost,l.ost-s)/100 ost
          from  (select d.nlsa,d.kv,a.acc,a.ob22,a.nbs,ost_korr(a.acc,dat31_,null,a.nbs) ost
                 from rez_doc_maket d,accounts a, V_SFDAT v
                 where d.nlsa=a.nls and d.kv=a.kv ) l,rez_doc_maket d
          where d.nlsa=l.nlsa and d.kv=l.kv )  y
