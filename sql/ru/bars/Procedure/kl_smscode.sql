...
begin
 ...
 then
     ...
     if l_verified = 0
     then l_errmsg := 'Телефонний номер'|| p_phonenumber ||' не пройшов верифікацію';
      ...
      exception when others
                then l_errmsg := 'Повідомлення не вдалося підготувати'|| p_phonenumber;
                     BARS_AUDIT.INFO(title || l_errmsg);
      end;
           ...
           bars_sms.submit_msg(p_msgid => p_msgid);
            ...
            UPDATE kl_cellphone_code
             ...
             WHERE phonenumber = p_phonenumber
               ...
               and sms_id = p_msgid;
       exception when others
                then l_errmsg := 'Повідомлення не вдалося відправити'|| p_phonenumber;
                     BARS_AUDIT.INFO(title || l_errmsg);
      end;
