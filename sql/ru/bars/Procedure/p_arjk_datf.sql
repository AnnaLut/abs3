...
Is
 ...
 Q_int  number;          --экв сумма расчетных процентов
...
begin
 ...
 oo.nazn :=  'Перерахунок нарахованих %% в валютi, які передані АРЖК до Пулу № '|| p_id||
...
and a.kv <> 980
 ...
 loop
    ...
    ---------------------------------------
--Львівським ОУ було виявлено що розроділ нарахованих відсотків по 2 ПУЛу по валютним кредитам був в номінальній валюті.
--Прошу виправити розподіл по еквіваленту.
--Згідно договорів з ПАТ "АРЖК" курс долара США становить завжди 7,993 грн., Євро - 10,334949



    If    k.kv = 840  then  q_int  := round ( l_int *  7.993   , 0 ) ;
    elsIf k.kv = 978  then  q_int  := round ( l_int * 10.334949, 0 ) ;
    else                    q_int  := gl.p_icurval ( k.kv, l_int, l_dat2 );
    end if;
	...
	gl.ref (oo.REF);
       gl.in_doc3( oo.ref, '096', 96, substr(to_char(oo.REF),1,10), SYSDATE ,
                   to_date('31-12-2013','dd-mm-yyyy'),
                   1,  a6.kv, Q_int, b6.kv,Q_int, null,
       ...
       gl.payv( 0, oo.ref , gl.bDATE , '096', 0    , a6.kv, a6.nls, L_int, b6.kv, b6.nls, L_int );
       gl.payv( 0, oo.ref , gl.bDATE , '096', 1    , a6.kv, a6.nls, Q_int, b6.kv, b6.nls, Q_int );
