...
for i in (select --/*+ INDEX(p)*/
            ...
            from oper p,
                 (select --/*+ INDEX(opldok)*/
                   ...
                   where s.fdat = to_date(p_dat, 'dd-mm-yyyy')  and sos in (4,5)  and o.fdat = s.fdat and o.acc = s.acc and o.dk = 0 ) t
           where p.ref = t.ref
             and p.userid=to_number(trunc(replace(p_isp,'%',' ')))
			 )
loop
    for d in (select d1.TT,   d1.REF,  d2.KV,  d2.NLS NLSA,  d1.S,  d1.SQ,
     ...
     insert into tmp_rep_provod
      ...
      insert into tmp_rep_provod
       (TT,     REF,   KV,   NLSA,    S,   SQ,  NLSB,   NAZN,
        PDAT, NAMTT,  ISP,BRANCH)
      values
       (d.TT,   d.REF,   d.KV, d.NLSA,  d.S,
        decode(d.kv,980,d.s,gl.p_icurval(d.kv,d.s,to_date(p_dat,'dd-mm-yyyy'))),
        l_NLSB, l_nazn,
        to_date(p_dat,'dd-mm-yyyy'), l_namtt, i.USERID,i.branch) ;
     end if;
    end loop;
