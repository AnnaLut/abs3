...
begin
  ...
  begin
     select kat into p_kat23 from FIN_OBS_KAT where fin = p_fin23  and obs = p_obs23 and cus = decode(p_cus,5,1,p_cus) ;
 ...
 --    EXCEPTION WHEN NO_DATA_FOUND THEN nbs_:=null;
        ...
        if nbs_ in ('1500', '1502', '1508', '1509') THEN
           ...
           if p_grp=1 THEN
              ...
              for ob in (select B.FIN, B.OBS, R.ORD VNCRR, B.KHIST,B.NEINF,
                         ...
                         from TMP_OB_KORR B, CCK_RATING R , stan_kat23 S
              ...
              loop
                 p_kat23:=ob.kat23;
                 begin
                    select r.ord  into l_VNCRR from accountsw t,CCK_RATING r where t.acc = p_nd and t.tag = 'VNCRR' and r.code=t.value;
                    --   select substr(trim(value),1,4) into l_VNCRR from accountsw where acc = p_nd and tag = 'VNCRR';
                 EXCEPTION WHEN NO_DATA_FOUND THEN   return;
                 end;
                 if  l_VNCRR<=ob.VNCRR then
                     begin
                        select decode(p_kat23,1,0,5,0,to_number (value))  into  l_KHIST from accountsw where acc = p_nd and tag = 'KHIST';
                        logger.info ('k23_def 6 l_KHIST='||l_KHIST );
                     EXCEPTION WHEN NO_DATA_FOUND    THEN  l_KHIST := null;
                     end;
                     begin
                        select decode(p_kat23,1,0,5,0,to_number (value))  into  l_NEINF from accountsw where acc = p_nd and tag = 'NEINF';
              ...
              for ob in (select B.FIN, B.OBS, R.ORD VNCRR, B.KHIST, B.NEINF, B.KAT23, B.K23, S.k_max K3
                         from TMP_OB_KORR B, CCK_RATING R , stan_kat23 S
                         where B.obs=p_obs23 and B.fin=decode(p_obs23,1,0,5,0,p_fin23) and R.code=decode(p_obs23,1,'Д',5,'Д',B.VNCRR)
                               and B.kat23=s.kat)
              loop
                 p_kat23:=ob.kat23;
                 begin
                 ...
                 if  l_VNCRR<=ob.VNCRR then
                     begin
                        select decode(p_obs23,1,0,5,0,to_number (value))  into  l_KHIST from accountsw where acc = p_nd and tag = 'KHIST';
                        logger.info ('k23_def 6 l_KHIST='||l_KHIST );
                     EXCEPTION WHEN NO_DATA_FOUND    THEN  l_KHIST := null;
                     end;
                     begin
                        select decode(p_obs23,1,0,5,0,to_number (value))  into  l_NEINF from accountsw where acc = p_nd and tag = 'NEINF';
                        logger.info ('k23_def 8 l_NEINF='||l_NEINF );
                     EXCEPTION WHEN NO_DATA_FOUND    THEN  l_NEINF := null;
                     end;
     ...
     loop
        ...
        begin
           select r.ord  into l_VNCRR from nd_txt t,CCK_RATING r where nd = p_nd and tag = 'VNCRR' and r.code=t.txt;
