...
PROMPT ===================================================================================== 
PROMPT *** Run *** ========== Scripts /Sql/BARS/Procedure/P_LIC26.sql =========*** Run *** =
PROMPT ===================================================================================== 


PROMPT *** Create  procedure P_LIC26 ***

  CREATE OR REPLACE PROCEDURE BARS.P_LIC26 
             (p_s      date,           -- дата с
              p_po     date,           -- дата по
...
-- Module      : REP
-- Author      : ANNY
-- Description : Процедура для выписки с еквивалентами (файл К для Ощадбанка) по корпоративным
--               клиентам
--
-- Version     : v6.8 19-10-2015
--======================================================================================
...
begin
   ...
   execute immediate('truncate table tmp_liccorpc');

   bars_audit.trace('p_lic26: формирование К файла за: ' ||to_char(p_s,'dd/mm/yyyy') ||' маска корпорации: '||p_corpc);
   -----------------------------
   -- по счетам
   -----------------------------
   for c0 in (select s.acc sal_acc, a.acc, nvl(s.fdat, p_s) fdat, nvl(s.ost,0) ost,
		      ...
		      inst_kod kodu, c.pos
                 from sal s, customer m, v_corp_accounts a, accounts c
                where s.acc(+)    = a.acc
                  and a.acc= c.acc
                  and s.fdat (+)  = p_s
                  and a.use_invp  = 'Y'
   ...
   loop
      ...
      bars_audit.trace('p_lic26: за число : ' ||to_char(c0.fdat,'dd/mm/yyyy') ||' счет '||c0.nls||'-'||c0.kv);
      bars_audit.trace('p_lic26: за число : ' ||to_char(c0.fdat,'dd/mm/yyyy') ||' счет '||c0.nls||'-'||c0.kv);
      ...
      loop
         ...
         loop
           ...
           insert into tmp_liccorpc(
                  rowtype,  ourmfo,  nls,  kv,
		  ...
		  doctype,
            ...
            values(
                  G_ROWTYPE_DOC, gl.amfo, c0.nls, c0.kv,
      ...
      insert into tmp_liccorpc(
          rowtype,  ourmfo,  nls,  kv,  okpo,  postdat,
          obdb,  obdbq,  obkr,  obkrq,  ost,  ostq,
          kod_corp,  kod_ustan,  kod_analyt, dapp, namk, nms)
      values (  G_ROWTYPE_ACC, gl.amfo, c0.nls, c0.kv, c0.okpo, c0.fdat,
   ...
   insert into tmp_liccorpc(rowtype, nls, docdat, kod_corp)
   select G_ROWTYPE_TOTAL, count(*), docdat, kod_corp
     from tmp_liccorpc
...
PROMPT *** Create  grants  P_LIC26 ***
grant EXECUTE                                                                on P_LIC26         to BARS_ACCESS_DEFROLE;
grant EXECUTE                                                                on P_LIC26         to CORP_CLIENT;
grant EXECUTE                                                                on P_LIC26         to RPBN001;
grant EXECUTE                                                                on P_LIC26         to WR_ALL_RIGHTS;



PROMPT ===================================================================================== 
PROMPT *** End *** ========== Scripts /Sql/BARS/Procedure/P_LIC26.sql =========*** End *** =
PROMPT ===================================================================================== 
