...
% VERSION     :	15.01.2008 (предыдущая версия от 10.08.2006)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
CURSOR SALDO IS
   ...
   WHERE a.acc=s.acc     AND
         ...
         s.nbs=k.r020          AND
         k.kf='CC'             AND
         ((s.nbs not in ('2620','2625') and a.ostf-a.dos+a.kos<>0) OR
         (s.nbs in ('2620','2625') and a.ostf-a.dos+a.kos>0)) AND
         ((substr(s.nbs,1,1)='3' and c.codcagent in (5,6) and trim(c.sed)<>'91') OR
          (substr(s.nbs,1,1) not in ('3')));
