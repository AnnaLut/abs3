...
PROMPT *** Create  procedure ANI4_BU_EX ***
 ...
 (TERM_ int, -- = 1 за звытний мic з КОР.ОБ,
...
*/
  ...
  dTmp_   date ;
  DATQ_   date ;
  BRANCH_ accounts.branch%type;  Y_M char(7);
  ID2_    bu1.id%type         ;  ID1_    bu1.id%type         ;
  dd_  date  ;
  kk_  date  ; -- пред.кв
  mm_ char(2);
  nmm_ int   ;
  kol_ int   ;
  Di_  number;   Di1_  number;
  Ki_  number;
  Lik_ varchar2(50);
  nTmp_ int  ;
  K SYS_REFCURSOR;
  ...
  k_ID2    number;
  k_D6 number; k_D7 number; k_S6 number; k_S7 number; k_K6 number; k_K7 number;

  Len_ int;
--------------
procedure GAP (JDAT_ date) is
  IR_   NUMBER;
  ZNAP_ number;
  PAP_  int;
...
--  where index_name= 'XIE_ACC_RNBU_TRACE';
--  If l_cnt =0 then
--     execute immediate 'create index XIE_ACC_RNBU_TRACE on RNBU_TRACE(ACC) ';
--  end if;

   begin
     select caldt_ID into caldt_id_ from accm_calendar where caldt_DATE = JDAT_;
...
--------------------- 2) Цикл по всем счетам
  for k in (SELECT ACC, nls,kv,nbs, rnk, isp, mdate, tobo FROM accounts
  ...
  LOOP
     ...
     SELECT
       IR_,RECID,USERID,NLS,KV,ODATE,KODP,ZNAP,NBUC,ISP,RNK,ACC,REF,COMM,ND,MDATE,JDAT_
     FROM RNBU_TRACE WHERE acc=k.ACC;

     if SQL%rowcount = 0 then
        -- добавка неохваченных в А7 лиц.счетов
        begin
          SELECT ABS(ostq), decode( SIGN(ostq),1,2,1)   into ZNAP_, PAP_
          from ACCM_SNAP_BALANCES
          where acc=k.ACC and caldt_id= CALDT_ID_ ;
        EXCEPTION WHEN NO_DATA_FOUND THEN ZNAP_:= 0;
        end;
  ...
  Insert into RNBU_TRACE1 (fdat,kodp,KV, NBUC, znap)
     select JDAT_,'25     H', 980, a.tobo, to_char(sum(m.ostq))
     from accounts a, ACCM_SNAP_BALANCES m
     where a.nbs >='5000' and a.nbs<'8000'
       and a.acc=m.acc and caldt_id= CALDT_ID_
...
procedure Itg
(p_BR varchar2, p_ID int,
 p_d6 number,p_d7 number,
 p_s6 number,p_s7 number,
 p_k6 number,p_k7 number) is
begin
 update ANI_bu set d6 = d6 + nvl(p_D6,0), d7 = d7 + nvl(p_D7,0),
                   s6 = s6 + nvl(p_S6,0), s7 = s7 + nvl(p_S7,0),
                   k6 = k6 + nvl(p_k6,0), k7 = k7 + nvl(p_k7,0)
        where branch =p_BR and id=p_ID;
 if SQL%rowcount = 0 then
    INSERT INTO ANI_bu (d6  ,d7  ,s6  ,s7  ,k6  ,k7  ,branch,id)
...
Elsif mode_ in (-1, -2 ) then
   ...
   Update BU_PLAN set SP=0 where PDAT= DAT_ and length(branch) in (8,15) and mod(ID,100)=0;
   ...
   loop
     ...
     If ID1_ in (100,200,400,500) then ID1_:= iif_n (ID1_,200, 300, 300, 600);
        Update BU_PLAN set SP=SP+k.SP where PDAT=DAT_ and branch=k.br and id=ID1_;
        if SQL%rowcount = 0 then
           insert into bu_plan ( PDAT, ID, SP, BRANCH  ) values ( DAT_, ID1_, k.SP, k.br);
        end if;
   ...
   update bu_plan set sp = -sp where sp<0 and pdat=DAT_;
...
elsIf MODE_ = 0  then --GAP
   GAP(DAT_);

ElsIf MODE_ = 77 then
   -- Накопление в архив Бюджета
   declare
     y_ char(4) := substr(YYYY_MM,1,4); m_ char(2) := substr(YYYY_MM,6,2);
     sSql_  varchar2(1000);
   begin
...
----- Конец --- Накопление в архив

  -- страховочная синхронизация
  If TERM_ = 1 then
     ----- = 1 за звытний мic з КОР.ОБ

     If YYYY_MM is null then Y_M := to_char(DAT_,'YYYY-MM');
     else                    Y_M := YYYY_MM;
     end if;
     dd_ := to_date(Y_M ||'-01','yyyy-mm-dd');-- отч мес

     select max(fdat) into ANI_DAT from fdat where to_char(fdat,'YYYY-MM') =Y_M;

     bars_accm_sync.sync_AGG('MONBAL', DD_);
     ...
     If TERM_ = 2 and MODE_ not in ( -1,-2) then
        begin
          select 1 into nTmp_ from fdat where fdat=DD_;
        EXCEPTION WHEN NO_DATA_FOUND THEN
          raise_application_error(-(20203),
          '      Дату ' || to_char(dd_,'dd/mm/yyyy') || ' не знайдено !',TRUE);
          return;
        end;
     end if;

     bars_accm_sync.sync_snap('BALANCE', DD_);
--   bars_accm_sync.sync_AGG('MONBAL', DD_);
  ...
  begin
    select caldt_ID into Di_  from accm_calendar where caldt_DATE=DD_ ;
    select caldt_ID into Di1_ from accm_calendar
     where caldt_DATE = trunc( add_months(DD_, -1 ), 'MM');
   EXCEPTION WHEN NO_DATA_FOUND THEN
    return;
  end;
  ...
  logger.info('ANI4_bU_ex -3: Di_='|| Di_ || ' Di1_='|| Di1_|| ' DATQ_='||DATQ_ );
      ...
      (SELECT ID,BR,sum(decode(KV,980,S,0)) S0,sum(decode(KV,980,0,S)) S1
       FROM (select a.ID,a.BR,a.KV,Sum(decode(a.P,2,DECODE(b.Z, 1, b.S,0),1,DECODE(b.Z,-1,-b.S,0),b.S)) S
             from ( select a.acc,r.ID, substr(a.BRANCH,1,15) br,NVL(r.pap,3) P, a.kv
                    from accounts a, rezerv r where a.nbs=r.nbs    ) A,
                  ( select ACC,sign(ostQ+(CRkosQ-CRdosQ)) Z,ostQ+(CRkosQ-CRdosQ) S
                    From ACCM_AGG_MONBALS
                    where caldt_ID=Di_ and ostQ+(CRkosQ-CRdosQ) <>0) B
        ...
        from (SELECT b.ID id2,b.NBS,b.PAP,a.BRANCH,
              ...
              FROM BU2 b,
                   (select acc,
                      ...
                      from accounts where (nbs like '6%' or nbs like '7%')
     ...
     If to_char(DD_,'mm') ='01' then
        ...
        from (SELECT b.ID id2,b.NBS,b.PAP,a.BRANCH,
                ...
                FROM BU2 b,
                     ...
                     (select ACC,OST From accm_snap_balances
                       where caldt_ID = Di_
                      ) M
              WHERE A.acc= M.acc (+)
                and a.nbs like trim(b.nbs)||'%'
        ...
        from (SELECT b.ID id2,b.NBS,b.PAP,a.BRANCH,
              ...
              FROM BU2 b,
                  (select acc, nbs||OB22 as nbs, substr(branch,1,len_) BRANCH
                   from accounts
                   where (nbs like '6%' or nbs like '7%')
                     and BRANCH LIKE Lik_                   ) A,
                  (select ACC,OST From accm_snap_balances where caldt_ID=Di_) M,
                  (select ACC,ost+(CRkos-CRdos) VOST From ACCM_AGG_MONBALS where caldt_ID=Di1_) M1
              WHERE A.acc= M.acc (+)
                and A.acc= M1.acc (+)
                and a.nbs like trim(b.nbs)||'%'
              GROUP BY b.ID, b.NBS, b.PAP, a.BRANCH
      ...
      from kl_s180 k,
          (select substr(branch,1,len_) BR, a.ap,a.nbs,a.ob22, s.s180,a.KV, sum(a.S) S
           from (select a.acc, decode(a.kv,980,0,1) KV, a.ob22,
                 ...
                 from accounts a,
                     (select ACC, ostq S  From accm_snap_balances
                      where caldt_ID=Di_ and OSTq<>0
                      ) b
                 where a.acc=b.acc and a.nbs not like '8%'
                ) a,
                specparam s
      ...
      from kl_s180 k,
          (select substr(branch,1,len_) BR, a.ap,a.nbs,a.ob22, s.s180,a.KV, sum(a.S) S
           from (select a.acc, decode(a.kv,980,0,1) KV,a.ob22,
                 ...
                 from accounts a,
                     (
select m.acc, round(sum(m.ostQ)/ nTmp_ ,0) S
from  ACCM_CALENDAR k, ACCM_SNAP_BALANCES m
where m.caldt_id = k.bankdt_id
  and k.caldt_date >= dTmp_
  and k.caldt_date <= DAT_
group by m.acc
--select ACC, ostq S  From accm_snap_balances  where caldt_ID=Di_ and OSTq<>0
                      ) b
                 where a.acc=b.acc and a.nbs not like '8%'
                ) a,
                specparam s
   ...
   If MODE_ <> 13 then    Return; end if;
   --------------------------------------

   -- 08-01-2010 Доп.блок для врем.табл баланс по бранч-3
   declare     sSql_ varchar2 (4000) ; TEST_ char(8) :='ANI_BSB';
   begin
     begin     sSql_ := 'DROP table '|| TEST_ ;
       EXECute immediate sSql_ ;
     ...
     sSql_ :='CREATE table '||TEST_||' as select KOD,ID,NAME,ORD from bs1';
     EXECute immediate sSql_;

     sSql_ :='alter table ' || TEST_||' add (PA3OM number(24,2))';
     EXECute immediate sSql_;

     for b in (select substr(branch,9,6) ||'_'|| substr(branch,16,6) BR
               from branch where length(branch)=22 order by 1)
     loop
       sSql_ :='alter table ' || TEST_||' add (s_'||b.BR||' number(24,2))';
       EXECute immediate sSql_;
     end loop;

     for k in (select id, substr(branch,9,6) ||'_'|| substr(branch,16,6) BR, s
               from bsb where s<>0 and length(branch)=22 )
     LOOP
       sSql_ :=
       'update '||TEST_||' set PA3OM= nvl(PA3OM,0) + '|| k.S||'/ 100, s_'||k.br||'='||k.S||'/100   where id='||k.ID;
       EXECute immediate  sSql_;
     end loop;

     -- удаление 0-строк
     sSql_ :=  'delete from  '||TEST_||' where PA3OM is null';
     EXECute immediate  sSql_;
