...
/*   Розрахунок кредитного ризику по дебіторці
     -----------------------------------------
    Версия 7.0   11-07-2017  16-05-2017   27-04-2017   21-03-2017  15-02-2017  24-01-2017   03-01-2017
...
*/
 ...
 l_s080  specparam.s080%type; l_ddd  kl_f3_29.ddd%type; l_istval  specparam.istval%type;
 ...
 l_pd_0  INTEGER;  l_nd  integer;  l_MAX    integer;
   ...
   begin
      if    p_deb = 0   THEN
         OPEN c0 FOR
            select 17 tipa, decode(c.custtype,3,3,2) custtype, c.custtype cus, substr( decode(c.custtype,3, c.nmk, nvl(c.nmkk,c.nmk) ) , 1,35) NMK, 
                   ...
                   DECODE (NVL (c.codcagent, 1), '2', 2, '4', 2, '6', 2, 1) RZ, a.mdate wdate, a.acc nd, null sdate
            ...
            select 17 tipa,decode(c.custtype,3,3,2) custtype, c.custtype cus, substr( decode(c.custtype,3, c.nmk, nvl(c.nmkk,c.nmk) ) , 1,35) NMK, 
                   ...
                   a.rnk, a.branch, DECODE (NVL (c.codcagent, 1), '2', 2, '4', 2, '6', 2, 1) RZ, a.mdate wdate, a.acc nd, null sdate       
         ...
         OPEN c0 FOR
            select 21 tipa, decode(c.custtype,3,3,2) custtype, c.custtype cus, substr( decode(c.custtype,3, c.nmk, nvl(c.nmkk,c.nmk) ) , 1,35) NMK, 
                   ...
                   DECODE (NVL (c.codcagent, 1), '2', 2, '4', 2, '6', 2, 1) RZ, a.mdate wdate, x.id nd, x.fdat sdate 
            from   xoz_ref x, accounts a, customer c, rez_deb d 
            where  a.nbs = d.nbs and d.deb in (2) and d.deb is not null and x.fdat < p_dat01 and (x.datz >= p_dat01 or x.datz is null) and s0<>0 and s<>0 and x.acc=a.acc  
      ...
      loop
         ...
         if k.bv >0 THEN
            ...
            begin
               select s180, nvl(istval,0) into l_s180, l_istval from specparam where acc = k.acc;
            EXCEPTION  WHEN NO_DATA_FOUND  THEN l_s180 := null; l_istval := 0;
            end;
            if k.kv = 980 THEN l_istval := 0;  end if; 
            l_ddd := f_ddd_6B(k.nbs);
            l_tip_fin := f_get_nd_tip_fin (k.nd, p_dat01,k.rnk, k.tipa);
            if l_tip_fin is null THEN
               if rez_f_deb(k.nbs) in (1,2) THEN l_tip_fin := 0; end if;
            ...
            l_kol    := f_get_nd_val (k.nd, p_dat01, k.tipa, k.rnk);
            l_fin    := f_rnk_maxfin (p_dat01, k.rnk  , l_tip_fin, l_nd, l_max);
            logger.info('XOZ 1 : acc = ' || k.acc || 'rez_f_deb(k.nbs) = ' || rez_f_deb(k.nbs) || ' k.nbs = ' || k.nbs  ) ;                          
            if    l_tip_fin = 0 and l_fin = 1 THEN l_pd := f_rez_kol_fin_pd(rez_f_deb(k.nbs), 2, l_kol); --f_rez_kol_fin_pd(2, l_kol);
            ...
            INSERT INTO REZ_CR (fdat    , RNK   , NMK   , ND             , SDATE  , KV     , NLS   , ACC  , EAD    , EADQ  , FIN  , PD      , BV    , 
                                ...
                                istval  , s080  , ddd_6B, tip_fin        , ob22   , wdate  , tip   , KOL  , FIN23  , TEXT  )
                        VALUES (p_dat01 , k.RNK , k.NMK , nvl(l_nd,k.acc), l_sdate, k.kv   , k.nls , k.acc, l_ead  , l_eadq, l_fin, l_pd    , l_ead , 
                                l_CR    , l_CRQ , k.tipa, l_ead          , l_eadq , l_s180 , l_pd_0, k.rz , l_RC   , l_RCQ , k.nbs, k.cus   , l_eadq,
                                l_istval,l_s080 , l_ddd , l_tip_fin      , k.ob22 , k.wdate, k.tip , l_kol, l_fin23, l_TEXT || ' Фин.= ' || l_fin);  
         end if;
      END loop;
