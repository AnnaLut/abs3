-- консолидация для файлу #E2
-- если значение показателя 20NNN меньше равно 1000.00 $ 
-- то выполняем консолидацию
CREATE OR REPLACE PROCEDURE BARS.p_filee2 (Dat_ DATE )  IS
kurs1_ number;
   ...
   loop
      ...
      then
         ...
         INSERT INTO v_banks_report91         
               (nbuc, kodf, datf, kodp, znap) 
         VALUES
               (k.nbuc, 'E2', Dat_, '10' || k.NNN, k.KV); 

         INSERT INTO v_banks_report91         
               (nbuc, kodf, datf, kodp, znap) 
         VALUES
               (k.nbuc, 'E2', Dat_, '31' || k.NNN, decode(k.CUSTTYPE, '1', '006', '0')); 

         INSERT INTO v_banks_report91         
               (nbuc, kodf, datf, kodp, znap) 
         VALUES
               (k.nbuc, 'E2', Dat_, '20' || k.NNN, k.SUMP); 

         INSERT INTO v_banks_report91         
               (nbuc, kodf, datf, kodp, znap) 
         VALUES
               (k.nbuc, 'E2', Dat_, '40' || k.NNN, '00'); 

         INSERT INTO v_banks_report91         
               (nbuc, kodf, datf, kodp, znap) 
         VALUES
               (k.nbuc, 'E2', Dat_, '62' || k.NNN, '0'); 

         INSERT INTO v_banks_report91         
               (nbuc, kodf, datf, kodp, znap) 
         VALUES
               (k.nbuc, 'E2', Dat_, '64' || k.NNN, '000'); 

         INSERT INTO v_banks_report91         
               (nbuc, kodf, datf, kodp, znap) 
         VALUES
               (k.nbuc, 'E2', Dat_, '65' || k.NNN, '0'); 


         INSERT INTO v_banks_report91         
               (nbuc, kodf, datf, kodp, znap) 
         VALUES
               (k.nbuc, 'E2', Dat_, '66' || k.NNN, '0');

         INSERT INTO v_banks_report91         
               (nbuc, kodf, datf, kodp, znap) 
         VALUES
               (k.nbuc, 'E2', Dat_, '61' || k.NNN, 'Консолідований'); 
...
begin
    execute immediate 'DROP PUBLIC SYNONYM p_filee2';
exception
    when others then null;
end;
/    

create public synonym p_filee2 for bars.p_filee2;
grant execute on p_filee2 to rpbn002;
/
