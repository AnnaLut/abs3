...
PROMPT ===================================================================================== 
PROMPT *** Run *** ========== Scripts /Sql/BARS/Procedure/XV_380028.sql =========*** Run ***
PROMPT ===================================================================================== 


PROMPT *** Create  procedure XV_380028 ***

  CREATE OR REPLACE PROCEDURE BARS.XV_380028 
...
/*
31.03.2015 3800/28, 6204 ('27','28','29'

 Открыть новые 3800 и 3801 для  каждого бранча-2 и
 для валюты kv in (840,978,643)
...
for o in (select ob22, substr(txt,1,38) NMS from sb_ob22 where r020='3800' and ob22 in ('28') and d_close is null )
loop
  If    o.ob22 = '28' then txt_:= 'за оп.погаш.заборг.';
  end if;
  ...
  loop
    ...
    for k in (select distinct kv from accounts where kv in (840,978,643) and nls not like '3800%'
    ...
    loop
      ...
      nms3800_ := 'ВП '|| k.KV || trim(txt_) ||'. Бранч '||BBBB_;
      ...
      nms3801_ :='ЕВП '|| k.KV || trim(txt_)||'. Бранч '||BBBB_;
      ...
      ----------НKР-1)  найти/открить нужный 6204/27 в_д переоц_нки рахунк_в резерв_в в _ноземн_й валют_;
      ob62_ := '28' ;
      ...
      If acc6204_ is null then nls6_ := vkrzn( mfo5_,'62040' || BBBB_ || ob62_ );  nms6_ := ob62_||':НКР-1. Бранч '|| BBBB_;
      ...
      If acc_rrd_ is null then  nls6_ := vkrzn( mfo5_,'62040' || BBBB_ || ob62_ );  nms6_ := ob62_||':НКР-2. Бранч '|| BBBB_;
      ...
      If acc_rrs_ is null then nls6_ := vkrzn( mfo5_,'62040'   || BBBB_  || ob62_ ); nms6_ := ob62_||':РКР-3. Бранч '|| BBBB_;
...
PROMPT *** Create  grants  XV_380028 ***
grant EXECUTE                                                                on XV_380028       to START1;



PROMPT ===================================================================================== 
PROMPT *** End *** ========== Scripts /Sql/BARS/Procedure/XV_380028.sql =========*** End ***
PROMPT ===================================================================================== 
