...
BEGIN
   ...
   END IF;
   ---------------------------------------------------------------------------
   PUL_DAT (TO_CHAR (P_DAT1, 'DD.MM.YYYY'), TO_CHAR (P_DAT2, 'DD.MM.YYYY'));
   ...
   THEN
      ...
      THEN
         sSql_ :=
          ...
          from oper p, (select * from opldok where fdat >= :p_dat1 and fdat <= :p_dat2 and tt =''PS1'' and dk=1 and sos=5   ) o,
              (select x.ref, x.kva, x.sumb/x.suma kurs, (x.suma-to_number(y.value)) S
               from  fx_deal x, operw y
               where x.dat >= :p_dat1 and x.dat <= :p_dat2 and x.kva<>980 and x.kvb=980 and x.ref=y.ref and y.tag= ''SUMKL'' ) f
   ...
   THEN                                           -- начисление и перечисление
    bars_audit.trace('%s Старт с параметром p_mode = %s',title,p_mode);
      ...
      END;
      bars_audit.trace('%s MFOP_= %s',title,MFOP_);
      IF '300465' IN (MFOP_, gl.aMfo)
      THEN
         ob36_nal := '12';
         ob36_bez := '35';
         ob29_nal := '09';
      ...
      EXCEPTION
         WHEN NO_DATA_FOUND
         THEN raise_application_error ( -20000, 'Не знайдено рах 3622/' || ob36_nal || ' на МФО');
      END;
      ...
      EXCEPTION
         WHEN NO_DATA_FOUND
         THEN raise_application_error ( -20000, 'Не знайдено рах 3622/' || ob36_bez || ' на МФО');
      END;
      ...
      THEN
         sSql_ :=
            'insert into CCK_AN_TMP ( branch, kv   , n1,pr, n2 , n3                 , n4 ,          NLS, NLSALT )
             ...
             from accounts a,
                  (select * from opldok where fdat >= :p_dat1 and fdat <= :p_dat2 and tt =''PS1'' and dk=1 and sos=5   ) o,
                  (select x.ref, x.kva, x.sumb/x.suma kurs, (x.suma-to_number(y.value)) S
                   from  fx_deal x, operw y
                   where x.dat >= :p_dat1 and x.dat <= :p_dat2 and x.kva<>980 and x.kvb=980 and x.ref=y.ref and y.tag= ''SUMKL'' ) f
      ...
      LOOP
         ...
         THEN
            ...
            OO.NAZN := 'Авто-Збір на обов’язк. держ. пенс. страх. від купів. ІВ за ' || TO_CHAR (p_dat1, 'dd.mm.yyyy');
         ELSE
            oo.mfob  := '820019';
      ...
      LOOP
         ...
         THEN
            ...
            OO.NAZN := 'Авто-Перерахування на обов’язк. держ. пенс. страх. від продажу ІВ за ' || TO_CHAR (p_dat1, 'dd.mm.yyyy');
         ELSE
            oo.mfob  := '820019';
            oo.id_b  := '37995466'; -- Головне управління Державної казначейської служби України в м. Києві
            oo.nlsb  := '31217222700011';
         ...
         -- Плюс Все Кредиты
         oo.s := (-oo.s) + fkos (a12.acc, p_dat1, p_dat1);
         -- но не более, чем остаток
         oo.s := LEAST (oo.s, a12.ostc);

         IF gl.amfo = '380764'
         THEN                             -- в НАДРАХ счета 2902 на 2-е уровне
            SELECT NVL (SUM (LEAST (fkos (acc, p_dat1, p_dat1), ostc)), 0)
              INTO oo.s
              FROM accounts
             WHERE nbs = a12.nbs
               AND ob22 = a12.ob22
         ...
         THEN
            ...
            THEN
               FOR k
                  IN (SELECT nls, LEAST (fkos (acc, p_dat1, p_dat1), ostc) s
                        FROM accounts
                       WHERE nbs = a12.nbs
                         AND ob22 = a12.ob22
                         AND branch <> a12.branch)
