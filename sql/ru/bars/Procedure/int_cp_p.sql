...
--    LOG('NLS='||l_nls||' факт OSTC='||l_ost||' ostaf='||l_ostaf);
       ...
       if l_ostaf = 0 and l_ost = 0 and ABS(Fost(p_Acc,l_Dat2)) = 0 then
          LOG('NLS='||l_nls||' зал-к факт='||l_ost||' форвард='||l_ostaf);
          goto ZAP;
       end if;
       if l_ost = 0 then l_ost:= Fost(p_Acc,l_Dat2); end if;
    EXCEPTION WHEN NO_DATA_FOUND THEN
    LOG('! такого НЕ може бути acc='||p_acc||' '||p_id,'ERROR',0);
    RETURN ;
    end;
 end if;

 If p_Metr not in (8,23,515)  then   -- 14/06-11 or p_id <>0 then  /* случайный залет */
    LOG('! випадковий вхід acc='||p_acc||' metr='||p_metr,'INFO');
 ...
 begin
    ...
    LOG('! метод нарахування acc='||p_acc||' metr='||l_metr,'TRACE',0);
    ...
    if nvl(l_cena_kup0,0)=0 then
      log('ISIN='||l_ISIN||' '||ID_||' НЕ вказано купон ','ERROR',0);
      goto ZAP;
 ...
 --log('ISIN='||l_ISIN||' 2c D3='||D3_);
   ...
   l_dok:=D1_;   -- -1;
    ...
    If D1_ <> D3_ then
       LOG('ISIN='||l_ISIN||' '||ID_||' ref='||REF_||' !НЕ нараховано.'||
            ' Змiна куп.перiоду в перiодi нарахування','INFO',1);
       goto ZAP;
    else
    NULL;
 ...
 EXCEPTION WHEN NO_DATA_FOUND THEN
           LOG('ISIN='||l_ISIN||' !НЕ нараховано. pnt='||pnt,'INFO',0);
           goto ZAP;
           when others then
             LOG('ISIN='||l_ISIN||' !НЕ нараховано. 2d','ERROR',0);
             goto ZAP;
             -- RETURN ;
 end;
...
LOG('ISIN='||l_ISIN||' ref='||REF_||' pnt='||pnt);
 ...
 LOG('ISIN='||l_ISIN||' pnt='||pnt||' ref='||REF_||' ЦП штук='||kol1_,'INFO',5);
 pnt:=4;
 If KOL1_ <> KOL2_ then
    LOG('ISIN='||l_ISIN||' ref='||REF_||' !НЕ нараховано.'||
              ' Змiна кiлькостi ЦП в пакетi в перiодi нарахування','ERROR',1);
    -- return;
    goto ZAP;
 end if;
...
EXCEPTION WHEN NO_DATA_FOUND THEN
   LOG('ISIN='||l_ISIN||' '||ID_||' ref='||REF_||
       ' .Незавізовані док-ти по рах-ку купону ','ERROR');
   goto ZAP;
end;

     LOG('ISIN='||l_ISIN||' '||ID_||' реф.угоди '||REF_||
         ' .fost R','INFO',0);

l_ostr_dok:=FOST(l_accr,l_dok);
     LOG('ISIN='||l_ISIN||' '||ID_||' реф.угоди '||REF_||' dok='||l_dok||
...
if l_accr2 is not NULL then
   ...
   EXCEPTION WHEN NO_DATA_FOUND THEN
     LOG('ISIN='||l_ISIN||' '||ID_||' реф.угоди '||REF_||
         '.Незавізовані док-ти по рах-ку купону R2','ERROR');
     l_ostr2:=0;
     -- goto ZAP;
   end;
end if;
...
if kup_ =0 and l_kv=980 then
   LOG('ISIN='||l_ISIN||' ref='||REF_||
       '! Купон=0 '||' dat1='||l_dat1||' dat2='||l_dat2,'ERROR',1);
   goto ZAP;
end if;

l_kupon:=KUP_ * KOL1_;   -- коп/cnt
  LOG('ISIN='||l_ISIN||' ref='||REF_||
      ' Купон(...)= '||l_kupon/100||' dat1='||l_dat1||' dat2='||l_dat2,'INFO',5);
...
--   l_kupon := l_kupon * (l_nom/100);
   ...
   LOG('ISIN='||l_ISIN||' ref='||REF_||' повний Купон(036)= '||l_kupon/(100)
...
--   l_kupon := l_kupon * (l_nom/100);
   ...
   LOG('ISIN='||l_ISIN||' ref='||REF_||' Купон(036)= '||l_kupon/(100)
              ||' DOK='||D1_||' DNK='||D2_,'INFO',0);
...
if p_int = 0 then
   LOG('ISIN='||l_ISIN||' ref='||REF_||' int='||p_int/100||' днів='||l_dn,'INFO',5);
   goto ZAP;
end if;

 l_dn:= l_dat2 - l_dat1 + 1;
 LOG('ISIN='||l_ISIN||' ref='||REF_||' int='||p_int/100||' днів='||l_dn||' tip='||l_tip,'INFO',5);
if l_tip=2 then
   p_int:=p_int*(-1);
...
if l_sint > l_kupon and fl_pg=1 then
   ...
   LOG('ISIN='||l_ISIN||' ref='||REF_||' sint='||l_sint/100||' > '||l_kupon/100||
       ' Загальна сума раніше нарахованих %-в перевищує суму купона в періоді','ERROR',1);
goto ZAP;
--return;
end if;
