...
PROMPT *** Create  procedure PAY_23 ***
  ...
  CREATE OR REPLACE PROCEDURE BARS.PAY_23 (P_dat01_ DATE, mode_ NUMBER DEFAULT 0, p_user number default null,
                                        nal_ number)  IS

/* Версия 1.0 20-01-2017   26-07-2016  18-05-2016 (24-02-2016, 04-01-2016, 18-09-2015)
...
*/
   ...
   if mode_ = 0 THEN
      ...
      -- Списание с капитала уценки по ЦБ
      begin
         oo.tt := '096';
         l_dat := gl.bdate;
         z23.to_log_rez (user_id , -30 , dat01_ ,'Начало Врегулюв.переоц.в зв`язку з форм.рез.');
         for k in (select a.* from accounts a,nbu23_rez n
                   where substr(a.nls,1,4) in ('3107', '1435', '3115' ) and a.ostc > 0 and a.ostc=a.ostb and
                         a.nbs is null and n.fdat=dat01_ and a.acc=n.acc and n.nd in
                        (select nd from nbu23_rez b where b.fdat=dat01_ and (b.nbs like ('31%') or nbs like ('14%')) and b.rez<>0)
                   )
         LOOP
            begin
            ...
            oo.nazn  := 'Врегулювання переоцінки в зв`язку з формуванням резерву на '||dat01_||' р.';
            ...
            paytt(l_pay, oo.REF, dat31_, oo.TT, 1, k.kv, k.nls, k.ostc, k.kv, oo.nlsb, k.ostc) ;
         END LOOP;
         z23.to_log_rez (user_id , -30 , dat01_ ,'Конец Врегулюв.переоц.в зв`язку з форм.рез.');
      end;
