...
PROMPT ===================================================================================== 
PROMPT *** Run *** ========== Scripts /Sql/BARS/Procedure/P_LICCATDYNV11.sql =========*** Ru
PROMPT ===================================================================================== 


PROMPT *** Create  procedure P_LICCATDYNV11 ***

  CREATE OR REPLACE PROCEDURE BARS.P_LICCATDYNV11 (id_ SMALLINT, p_s DATE, p_po DATE,
                maska_ varchar2,  ext_ VARCHAR2 ) IS
...
--------   ***  15/04-05  ***
  ...
  nlsk_    varchar2(15);  acck_	   number;        fli_m	   number;
...
sql_:= ' SELECT s.acc,s.fdat,(s.ost-s.kos+s.dos),s.ost,b.dos,b.kos, '||
       '        s.dos,s.kos,s.nls,s.kv,b.pdat,s.isp,substr(s.nms,1,38) '||
       ' FROM sal s, saldob b '||
       ' WHERE s.acc=b.acc(+) AND s.kv<>980 AND s.fdat=b.fdat(+) AND '||
       '      (s.dazs IS NULL OR s.dazs > :p_s_) AND  s.nls LIKE '''|| maska_ ||''''||
       '     AND ( s.fdat > :p_s_   AND s.fdat < :p_po_  ) '||ext_||
       ' ORDER BY s.acc,s.fdat';

  OPEN SALDOA1 FOR sql_ USING p_s_, p_s_, p_po_ ;

  LOOP

   FETCH SALDOA1 INTO acc_,fdat_,ostf_,osti_, deb_pere_,crd_pere_,dos_,kos_,nls_,kv_,dapp_,isp_,nms_;
 ...
 --   dosq_:=deb_pere_;
 --   kosq_:=crd_pere_;
    dosq_:=gl.P_ICURVAL(kv_,dos_,fdat_);
    kosq_:=gl.P_ICURVAL(kv_,kos_,fdat_);
...
PROMPT *** Create  grants  P_LICCATDYNV11 ***
grant EXECUTE                                                                on P_LICCATDYNV11  to BARS_ACCESS_DEFROLE;
grant EXECUTE                                                                on P_LICCATDYNV11  to RPBN001;
grant EXECUTE                                                                on P_LICCATDYNV11  to WR_ALL_RIGHTS;



PROMPT ===================================================================================== 
PROMPT *** End *** ========== Scripts /Sql/BARS/Procedure/P_LICCATDYNV11.sql =========*** En
PROMPT ===================================================================================== 
