...
create or replace package nbu_gateway.nbu_601_parse_xml  as
 ...
 procedure p_parse_person_fo (p_id   in  NUMBER)
  ...
  begin
   ...
   begin
      ...
      -- raise_application_error(-20000,dbms_xslprocessor.valueof(l_header, 'CURRENT_USER/text()'));
        l_cur_user:=dbms_xslprocessor.valueof(l_header, 'CURRENT_USER/text()');
      -- raise_application_error(-20000,dbms_xslprocessor.valueof(l_header, 'REPORTING_TIME/text()'));

        dbms_xslprocessor.valueOf(l_header, 'REPORTING_TIME/text()',l_str);
        l_data_rep:=to_date(l_str,'dd.mm.yyyy hh24:mi:ss');
        --dbms_output.put_line(to_date(l_str,'dd.mm.yyyy hh24:mi:ss'));
   ...
   -- bars_audit.info(title || 'Start '||'p_id = '||p_id);
       ...
       -- bars_audit.trace(title || 'getdocument done');
        ...
        LOOP
            ...
            -- dbms_xslprocessor.valueof(l_row, 'STATUS/text()', l_str);
             --l_ADDRESS_FO (l_ADDRESS_FO.last).STATUS := trim(l_str);
...
procedure p_parse_finperformance_uo (p_id in  NUMBER)
  is
   ...
   -- l_type_id  barstrans.TRANSP_RECEIVE_DATA.type_id%type;
      ...
      --  l_clob := replace(replace(replace(l_clob,chr(38)||'lt;','<'),chr(38)||'gt;','>'),'Short','Detail');
       ...
       -- bars_audit.trace(title || 'getdocument done');
        ...
        LOOP
             ...
             --dbms_xslprocessor.valueof(l_row, 'STATUS/text()', l_str);
             --l_FINPERFORMANCE_UO (l_FINPERFORMANCE_UO.last).STATUS   := trim(l_str);
      ...
      --  l_clob := replace(replace(replace(l_clob,chr(38)||'lt;','<'),chr(38)||'gt;','>'),'Short','Detail');
       ...
       -- bars_audit.trace(title || 'getdocument done');
        ...
        LOOP
             ...
             --dbms_xslprocessor.valueof(l_row, 'STATUS/text()', l_str);
             --l_FINPERFORMANCEGR_UO (l_FINPERFORMANCEGR_UO.last).STATUS   := trim(l_str);
              ...
              BEGIN
                ...
                loop
                    ...
                    VALUES ( l_check_id,
                             ...
                             --l_FINPERFORMANCEGR_UO(j).status,
      ...
      --  l_clob := replace(replace(replace(l_clob,chr(38)||'lt;','<'),chr(38)||'gt;','>'),'Short','Detail');
       ...
       -- bars_audit.trace(title || 'getdocument done');
        ...
        LOOP
            ...
            -- raise_application_error(-20000, dbms_xslprocessor.valueof(l_row, 'COUNTRYCODOJ/text()'));
      ...
      --  l_clob := replace(replace(replace(l_clob,chr(38)||'lt;','<'),chr(38)||'gt;','>'),'Short','Detail');
       ...
       -- bars_audit.trace(title || 'getdocument done');
        ...
        LOOP
            ...
            -- raise_application_error(-20000, dbms_xslprocessor.valueof(l_row, 'COUNTRYCOD/text()'));
            ...
            -- dbms_xslprocessor.valueof(l_row, 'STATUS/text()', l_str);
             --l_OWNERPP_UO (l_OWNERPP_UO.last).STATUS   := trim(l_str);
      ...
      --  l_clob := replace(replace(replace(l_clob,chr(38)||'lt;','<'),chr(38)||'gt;','>'),'Short','Detail');
       ...
       -- bars_audit.trace(title || 'getdocument done');
        ...
        LOOP
             ...
             --dbms_xslprocessor.valueof(l_row, 'STATUS/text()', l_str);
             --l_PARTNERS_UO (l_PARTNERS_UO.last).STATUS   := trim(l_str);
      ...
      --  l_clob := replace(replace(replace(l_clob,chr(38)||'lt;','<'),chr(38)||'gt;','>'),'Short','Detail');
       ...
       -- bars_audit.trace(title || 'getdocument done');
        ...
        LOOP
           ...
           --  raise_application_error (-20000,to_date(dbms_xslprocessor.valueof(l_row, 'DOGDAY/text()'), l_date)) ;
             ...
             --dbms_xslprocessor.valueof(l_row, 'STATUS/text()', l_str);
             --l_NBU_CREDIT (l_NBU_CREDIT.last).STATUS   := trim(l_str);
      ...
      --  l_clob := replace(replace(replace(l_clob,chr(38)||'lt;','<'),chr(38)||'gt;','>'),'Short','Detail');
       ...
       -- bars_audit.trace(title || 'getdocument done');
        ...
        LOOP
             ...
             --dbms_xslprocessor.valueof(l_row, 'STATUS/text()', l_str);
             --l_NBU_CREDIT_PLEDGE (l_NBU_CREDIT_PLEDGE.last).STATUS   := trim(l_str);
