...
create or replace type body t_core_company is
    ...
    begin
        ...
        l_attributes(4) := json_utl.make_json_string('nameUr', nameur, p_mandatory => true);
        l_attributes(5) := json_utl.make_json_date('registryDay', registryday, p_mandatory => true);
        l_attributes(6) := json_utl.make_json_string('numberRegistry', numberregistry, p_mandatory => true);
        l_attributes(7) := json_utl.make_json_value('ecActivity', '{ ' || json_utl.make_json_string('k110', nvl(k110, 'ZZZZZ')) || ', ' ||
                                                                          json_utl.make_json_date('ec_year', nvl(ec_year, date '0001-01-01')) ||
                                                                   ' }');
        if (isrez = 'false') then
            l_attributes(8) := json_utl.make_json_string('countryCodNerez', countrycodnerez);
        end if;

        l_attributes(9) := json_utl.make_json_value('finPerformance', '{ ' ||
        ...
        l_attributes(10) := json_utl.make_json_value('isMember', nvl(ismember, 'false'));
        l_attributes(11) := json_utl.make_json_value('isController', nvl(iscontroller, 'false'));
        ...
        if (companies_group is not null and companies_group is empty) then
            ...
            if (l_group_members.count > 1) then
                l_attributes(12) := json_utl.make_json_value('groupUr', '[' || bars.tools.words_to_string(l_group_members, p_splitting_symbol => ', ', p_ignore_nulls => 'Y') || ']');
            else
                l_attributes(12) := json_utl.make_json_value('groupUr', bars.tools.words_to_string(l_group_members, p_splitting_symbol => ', ', p_ignore_nulls => 'Y'));
            end if;

            l_attributes(13) := json_utl.make_json_value('finPerformanceGr', '{ ' ||
        ...
        l_attributes(14) := json_utl.make_json_value('isPartner', ispartner, p_mandatory => true);
        ...
        l_attributes(16) := json_utl.make_json_value('isAudit', nvl(isaudit, 'false'));

        l_attributes(17) := json_utl.make_json_string('k060', case when k060 = '99' then null else k060 end, p_mandatory => true);
