...
/*  procedure set_death_debet_ref(p_recid in pfu_death_record.id%type,
  ...
  procedure r_branch_procesing(p_file_data in clob,
                                      p_file_id   in number);
--- реализация по COBUMMFO-9612 . Перенос в архив и из архива  pfu_file_arc. Логирование в pfu_file_arc_log
PROCEDURE move_to_arc (pi_fileid  bars.NUMBER_LIST , po_errortext out  varchar2);
PROCEDURE move_from_arc (pi_fileid bars.NUMBER_LIST , po_errortext out varchar2);
PROCEDURE move_to_arc_matchsend_job;
    ...
    begin
      ...
      begin
        select * into l_pensacc_row from pfu_pensacc pa
         where (pa.nls = p_rec.num_acc or pa.nlsalt = p_rec.num_acc) -- COBUMMFO-7501
               and pa.kf = p_rec.mfo /*and pa.kv = 980*/;
      exception
        when no_data_found then
          l_err_code    := 9;
...
--      if utl_match.edit_distance_similarity(l_pensioner_row.nmk,p_rec.full_name) < 80 then
      ...
      update pfu_pensacc pa
       ...
       where pa.kf = p_rec.mfo
             and (pa.nls = p_rec.num_acc or pa.nlsalt = p_rec.num_acc) -- COBUMMFO-7501
    ...
    begin
      for rec_frec in (select *
         ...
         begin
            ...
            from pfu_pensacc pp
             where (pp.nls = ltrim(rec_frec.num_acc, 0) or pp.nlsalt = ltrim(rec_frec.num_acc, 0)) -- COBUMMFO-7501
            ...
            select pp.block_type into l_ebp_state
             ...
             where pp.okpo = rec_frec.numident
               ...
               and pp.rnk = (
                               select pa.rnk 
                               from pfu_pensacc pa 
                               where (pa.nls = rec_frec.num_acc or pa.nlsalt = rec_frec.num_acc) -- COBUMMFO-7501
                                     and pa.kf = rec_frec.mfo
                            )
    ...
    begin
      ...
      loop
         ...
         select p.ispayed
           into l_ispayed
           from pfu_pensacc p
          where (p.nls = l_num_acc or p.nlsalt = l_num_acc) -- COBUMMFO-7501
                and p.kf = l_mfo;
...
----- реализация по COBUMMFO-9612
    ---проверочная что все файлы в перечне имеют подходящий статус. Возвращает ошибку с первым некорректным файлом
    FUNCTION check_file_state4archive (pi_fileid bars.number_list)
        RETURN VARCHAR2
    AS
        l_error_text   VARCHAR2 (2000);
    BEGIN
        SELECT 'Реєстр ID=' || p.id || ' cтатус "' || p.state || '" має недопустимий статус.' || CHR (10) || 'Перенос реєстрів до архіву неможливий.'
        INTO   l_error_text
        FROM   pfu.pfu_file p, TABLE (pi_fileid) t
        WHERE      p.id = t.COLUMN_VALUE
               AND p.state NOT IN ('MATCH_SEND', 'CHECKED', 'ERROR')
               AND ROWNUM = 1;
    ...
    PROCEDURE move_to_arc (pi_fileid          bars.number_list,
                           po_errortext   OUT VARCHAR2)
    AS
        l_status   pfu.pfu_file.state%TYPE;
    BEGIN
        IF pi_fileid.COUNT = 0
        THEN
            RETURN;
        END IF;

        --- проверяем корректность статусов только при вызове не из джоба
        IF SYS_CONTEXT ('USERENV', 'FG_JOB_ID') = 0
        THEN
            ---- если из джоба то проверять нет необходимости
            po_errortext := check_file_state4archive (pi_fileid => pi_fileid);

            IF po_errortext IS NOT NULL
            THEN
                RETURN;
            END IF;
        END IF;
        ...
        VALUES (id,
                state,
                'TO_ARC',
                NVL (SYS_CONTEXT ('bars_global', 'USER_NAME'),
                     SYS_CONTEXT ('userenv', 'SESSION_USER')))
            SELECT f.*,
                   TRUNC (SYSDATE) arcdate,
                   (SELECT fio
                    FROM   bars.staff$base b
                    WHERE      id =
                                   NVL (
                           ...
                           AND ROWNUM = 1)
                       arcdateuser
            FROM   pfu.pfu_file f, TABLE (pi_fileid) t
            WHERE  f.id = t.COLUMN_VALUE;

        /*        bars.bars_audit.trace (
                    'PFU_FILES_UTL.move_to_arc => Вставлено строк  %s',
                    TO_CHAR (SQL%ROWCOUNT));*/

        FORALL i IN 1 .. pi_fileid.COUNT
            DELETE FROM pfu.pfu_file f
            WHERE       f.id = pi_fileid (i);
    /*        bars.bars_audit.trace ('PFU_FILES_UTL.move_to_arc => Удалено строк  %s',
                                   TO_CHAR (SQL%ROWCOUNT));*/
    ...
    EXCEPTION
        ...
        THEN
            ROLLBACK;
            --- вызывается из веба. Договоренность что возвращаем текст ошиби
            po_errortext := SQLERRM;
            bars.bars_audit.error (
                'PFU_FILES_UTL.move_to_arc => Ошибка  ' || SQLERRM);
    END move_to_arc;


    PROCEDURE move_from_arc (pi_fileid          bars.number_list,
                             po_errortext   OUT VARCHAR2)
    AS
    BEGIN
        INSERT ALL
        INTO   pfu.pfu_file (id,
                             envelope_request_id,
                             check_sum,
                             check_lines_count,
                             payment_date,
        ...
        VALUES (
                   ...
                   NVL (SYS_CONTEXT ('bars_global', 'USER_NAME'),
                        SYS_CONTEXT ('userenv', 'SESSION_USER')))
            SELECT id, envelope_request_id, check_sum, check_lines_count,
                   payment_date, file_number, file_name, file_data,
                   state, crt_date, data_sign, userid,
                   pay_date, match_date, file_type
            FROM   pfu.pfu_file_arc f, TABLE (pi_fileid) t
            WHERE  f.id = t.COLUMN_VALUE --- не предполагается возвращать из архива файлы в статусе 'MATCH_SEND'
                                        AND f.state <> 'MATCH_SEND';

        FORALL i IN 1 .. pi_fileid.COUNT
            DELETE FROM pfu.pfu_file_arc f
            WHERE       f.id = pi_fileid (i) AND f.state <> 'MATCH_SEND';
    EXCEPTION
        WHEN OTHERS
        THEN
            ROLLBACK;
            ...
            po_errortext := SQLERRM;
            bars.bars_audit.error (
                'PFU_FILES_UTL.move_from_arc => Ошибка ' || SQLERRM);
    END move_from_arc;

    --- перенос файлов  в статусе MATCH_SEND в архив. COMMIT через каждые 100 файлов. Для запуска в job (COBUMMFO-9612)
    PROCEDURE move_to_arc_matchsend_job
    AS
        l_date        DATE;
        lt_fileid     bars.number_list;
        l_errortext   VARCHAR2 (2000);
    BEGIN
        ----  датою фактичної оплати цього реєстру  до  01 числа того місяця, який передує поточному місяцю щодо запуску процедури архівування.
        l_date := TRUNC (ADD_MONTHS (SYSDATE, -1), 'month');

        SELECT id
        BULK   COLLECT INTO lt_fileid
        FROM   pfu.pfu_file f
        WHERE  f.state = 'MATCH_SEND' AND f.pay_date < l_date;

        move_to_arc (lt_fileid, l_errortext);

        bars.bars_audit.trace (
               'PFU_FILES_UTL.move_to_arc_matchsend_job => '
            || 'Перенос файлов %s ПФУ в статусе MATCH_SEND по дату %s',
            TO_CHAR (lt_fileid.COUNT),
            TO_CHAR (l_date, 'dd-mm-yyyy'));

        COMMIT;
   END move_to_arc_matchsend_job;
