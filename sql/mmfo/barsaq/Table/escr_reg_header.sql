...
PROMPT ===================================================================================== 
PROMPT *** Run *** ========== Scripts /Sql/BARSAQ/Table/ESCR_REG_HEADER.sql =========*** Run
PROMPT ===================================================================================== 
...
begin 
  execute immediate '
  CREATE TABLE BARSAQ.ESCR_REG_HEADER 
   (	ID NUMBER(38,0), 
...
COMMENT ON TABLE BARSAQ.ESCR_REG_HEADER IS 'Інформація по КД,які включені в реєстр';
COMMENT ON COLUMN BARSAQ.ESCR_REG_HEADER.ID IS '';
COMMENT ON COLUMN BARSAQ.ESCR_REG_HEADER.CUSTOMER_ID IS 'РНК клієнта';
COMMENT ON COLUMN BARSAQ.ESCR_REG_HEADER.CUSTOMER_NAME IS 'ПІБ клієнта';
COMMENT ON COLUMN BARSAQ.ESCR_REG_HEADER.CUSTOMER_OKPO IS 'ІНН клієнта';
COMMENT ON COLUMN BARSAQ.ESCR_REG_HEADER.CUSTOMER_REGION IS 'Область, в якій зареєстрований клієнт ';
COMMENT ON COLUMN BARSAQ.ESCR_REG_HEADER.CUSTOMER_FULL_ADDRESS IS 'Повна адреса реєстрації клієнта';
COMMENT ON COLUMN BARSAQ.ESCR_REG_HEADER.SUBS_NUMB IS 'Номер субсидії';
COMMENT ON COLUMN BARSAQ.ESCR_REG_HEADER.SUBS_DATE IS 'Дата субсидії';
COMMENT ON COLUMN BARSAQ.ESCR_REG_HEADER.SUBS_DOC_TYPE IS 'Тип документу по субсидії';
COMMENT ON COLUMN BARSAQ.ESCR_REG_HEADER.DEAL_ID IS 'Референс КД';
COMMENT ON COLUMN BARSAQ.ESCR_REG_HEADER.DEAL_NUMBER IS 'Номер КД';
COMMENT ON COLUMN BARSAQ.ESCR_REG_HEADER.DEAL_DATE_FROM IS 'Дата початку дії КД';
COMMENT ON COLUMN BARSAQ.ESCR_REG_HEADER.DEAL_DATE_TO IS 'Дата закінчення дії КД';
COMMENT ON COLUMN BARSAQ.ESCR_REG_HEADER.DEAL_TERM IS 'Період дії КД в місяцях';
COMMENT ON COLUMN BARSAQ.ESCR_REG_HEADER.DEAL_PRODUCT IS 'Код продутку КД';
COMMENT ON COLUMN BARSAQ.ESCR_REG_HEADER.DEAL_STATE IS 'Статус КД';
COMMENT ON COLUMN BARSAQ.ESCR_REG_HEADER.DEAL_TYPE_NAME IS '';
COMMENT ON COLUMN BARSAQ.ESCR_REG_HEADER.DEAL_SUM IS '';
COMMENT ON COLUMN BARSAQ.ESCR_REG_HEADER.GOOD_COST IS '';
COMMENT ON COLUMN BARSAQ.ESCR_REG_HEADER.NLS IS '';
COMMENT ON COLUMN BARSAQ.ESCR_REG_HEADER.DOC_DATE IS 'Дата отримання документів про цільове використання';
COMMENT ON COLUMN BARSAQ.ESCR_REG_HEADER.COMP_SUM IS 'Сума компенсації';
COMMENT ON COLUMN BARSAQ.ESCR_REG_HEADER.BRANCH_CODE IS '';
COMMENT ON COLUMN BARSAQ.ESCR_REG_HEADER.BRANCH_NAME IS '';
COMMENT ON COLUMN BARSAQ.ESCR_REG_HEADER.MFO IS '';
COMMENT ON COLUMN BARSAQ.ESCR_REG_HEADER.USER_ID IS '';
COMMENT ON COLUMN BARSAQ.ESCR_REG_HEADER.USER_NAME IS '';
COMMENT ON COLUMN BARSAQ.ESCR_REG_HEADER.NEW_GOOD_COST IS 'Частина суми придбаного товару,яка підлягає компенсації';
COMMENT ON COLUMN BARSAQ.ESCR_REG_HEADER.NEW_DEAL_SUM IS '';
COMMENT ON COLUMN BARSAQ.ESCR_REG_HEADER.NEW_COMP_SUM IS '';
COMMENT ON COLUMN BARSAQ.ESCR_REG_HEADER.CREDIT_STATUS_ID IS '';
...
begin   
 execute immediate '
  ALTER TABLE BARSAQ.ESCR_REG_HEADER ADD CONSTRAINT CC_ESCR_REG_HEADER_CUSTOMER21 CHECK (CUSTOMER_NAME IS NOT NULL) ENABLE';
exception when others then
...
begin   
 execute immediate '
  ALTER TABLE BARSAQ.ESCR_REG_HEADER ADD CONSTRAINT CC_ESCR_REG_HEADER_CUSTOMER22 CHECK (CUSTOMER_OKPO IS NOT NULL) ENABLE';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
begin   
 execute immediate '
  ALTER TABLE BARSAQ.ESCR_REG_HEADER ADD CONSTRAINT PK_ESCR_REG_HEADER PRIMARY KEY (ID)
...
begin   
 execute immediate '
  CREATE UNIQUE INDEX BARSAQ.PK_ESCR_REG_HEADER ON BARSAQ.ESCR_REG_HEADER (ID) 
...
begin   
 execute immediate '
  CREATE UNIQUE INDEX BARSAQ.UK_ESCR_REG_HEADER ON BARSAQ.ESCR_REG_HEADER (CUSTOMER_ID, DEAL_ID) 
  PCTFREE 10 INITRANS 2 MAXTRANS 255 COMPUTE STATISTICS 
  TABLESPACE AQTS ';
exception when others then
  if  sqlcode=-955  then null; else raise; end if;
...
PROMPT ===================================================================================== 
PROMPT *** End *** ========== Scripts /Sql/BARSAQ/Table/ESCR_REG_HEADER.sql =========*** End
PROMPT ===================================================================================== 
