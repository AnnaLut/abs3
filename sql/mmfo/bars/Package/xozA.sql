...
procedure XOZ7   ( p_mode int,  -- 2 - разметка для проводок. 77 - проверка баланса и оплата
   ...
   oo oper%rowtype;
   xx xoz_ref%rowtype;
   name_br_a branch.name%type;
   name_br_b branch.name%type;
begin
   bc.go('/');
...
---------------------------------------
   ...
   If p_mode = 2 then -- разбить на частные суммы
      ...
      If p_ri is null then  insert into XOZ7_ca (REC,ACC7,S7,KODZ,OB40,nazn) select XX.REFD, acc , p_s7*100, p_kodz, p_ob40, p_nazn  from accounts where kv = gl.baseval and nls =p_nls7 ;
...
If p_mode = 77 then  -- оплатить
   ...
   -- проверка суммы
   select NVL( sum(s7), 0)  into oo.S from XOZ7_ca x where rec = XX.REFD and exists (select 1 from accounts where acc = x.acc7)  ;
...
-------------------------------------------------------
   ...
   select br.name into name_br_a from branch br where br.branch = 
   (select ac.branch from accounts ac where ac.nls = aa.nls and ac.kf ='300465' and ac.kv = 980 and ac.dazs is null);
   select br.name into name_br_b from branch br where br.branch = 
   (select ac.branch from accounts ac where ac.nls = o1.nlsb and ac.kf ='300465' and ac.kv = 980 and ac.dazs is null);      
   EXCEPTION WHEN NO_DATA_FOUND THEN     bc.go('/');  raise_application_error(-20000, 'XOZ7 НЕ знайдено транз.рах.300465/'|| aa.nls );
   end;

   oo.nam_a := name_br_a;--Tarasenko--Substr(aa.nms,1,38) ;  --\  транзитник
   oo.nlsa  := aa.nls   ;  -- \
   oo.mfoa  := aa.KF    ;  -- /
   oo.id_a  := gl.aOkpo ;  --/
