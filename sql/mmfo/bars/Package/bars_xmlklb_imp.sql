...
--проверка на соответсвие счета клиенту, считанного из файла, проверка по ОКПО клиента
   ...
   begin
      ...
      if changed(p_nama, l_doc.nam_a)  then
        ...
        l_columns := add_fld(l_columns,'nam_a', substr(p_nama,1,38), 'C');
      ...
      if changed(p_namb, l_doc.nam_b)  then
        /* if length(p_namb) > 38 then
         ...
         l_columns := add_fld(l_columns,'nam_b', substr(p_namb,1,38), 'C');
  ...
  --    p_date -  по какую дату ( НЕ включая )
  --
  procedure CLEAR_IMPORT_JOURNALS
  ( p_date   date
  ) is
  ...
  begin
   ...
   --    Так же инициализируется дата валютирования если это корректирующие проводки
   --
   ...
   begin
      ...
      then
         p_doc.datp := p_doc.vdat;
         p_doc.datd := trunc(sysdate);
         p_doc.vdat := dat_next_u(trunc(gl.bd, 'YYYY'),-1);  --add_months(last_day(gl.bd),-1);
      else
         p_doc.datp := p_doc.vdat;
         p_doc.vdat := gl.bd;
      end if;
  ...
  is
    ...
    l_outdomdoc  dbms_xmldom.DOMDocument;
    l_trace      varchar2(256) := G_TRACE||'make_import: ';
    l_file_nm    xml_impdocs.fn%type;
    l_file_dt    xml_impdocs.dat%type;
  ...
  --    По реквизитам документа, получить код операции для БАРС-а
  --
  --    I00 I00 Імпорт внутр_шн_х платеж_в
  --    I01 off I01 _мпорт м_жбанк_вських платеж_в (МВПС, СЕП)
  --    I02 I02 Iмпорт прих_д каси (ГРН)
  --    I03 I03 Імпорт видаток каси (ГРН)
  --    I04 I04 Імпорт прихiд каси (ВАЛ)
  --    I05 I05 Імпорт видаток каси (ВАЛ)
  --    I06 I06 Імпорт інформац_йний дебет (dk=2)
  --    I07 I07 - Iмпорт реальний ДЕБЕТ мiжбанк
  --    I09 I09 - Тест
  --    I0I I0I - Тест _нформац_йний (dk=3)
  --
