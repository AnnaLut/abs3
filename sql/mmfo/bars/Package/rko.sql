...
*/
  ...
  -- global consts
  G_HEADER_VERSION constant varchar2(64)  := 'version 8  14/08/2018';
...
/*
  ...
  COBUMMFO-5332:  3570/03 -> 3570/37 - переносится ВХОДЯЩИЙ на 01 число остаток счета 3570/03

*/

  G_BODY_VERSION constant varchar2(64)  := 'version 8  14/08/2018';
...
----  ( 1 )   Рассчет сумм платы за РО по счетам.  Рассчитанные суммы пишутся в rko_lst.S0     -----
...
IF  n_tarpak >= 38 THEN      ----     П А К Е Т Ы  >=  38        Отдельно считаем IB*,CL*   и  "ручные"
                                                                 ---------------------------------------
      ...
      IF MODE_<0 THEN           ---<--  Для 412 отчета
         ...
         INSERT INTO CCK_AN_TMP ( reg, NLS, acc, N1, PR, N2 )
          ...
          Group by rnk,nls,acc,s,pr
                              UNION ALL
          ----------------------------------------------------------
          -- 2 --   По ДЕБЕТУ / кроме IB1,IB2,CL1,CL2      -- 412 --          
          ----------------------------------------------------------
          SELECT rnk, nls , acc,  s,  pr,  COUNT(*)
          FROM (Select rnk, nls, acc, ref,s, pr , rownum R
                From (SELECT a.rnk, a.NLS, a.acc, o.ref,
...
--------------------------------------------------------------------------------------------------------------
ELSE       ---   П А К Е Т Ы   <  38       Считаем все вместе  ( НЕ надо отдельно считать IB*,CL*  и "ручные")
--------------------------------------------------------------------------------------------------------------
...
---  ( 2 )   Формирование автопроводок: 

--- mode_ = '0'  -  Списаниe платы прямо с 2600       (кнопка "0"):   Дт 2600 - Кт 6510     - в норме НЕ используется !
--- mode_ = '1'  -  Начисление платы на 3570/03       (кнопка "1"):   Дт 3570 - Кт 6510     - выполняется при Закрытии последнего раб.дня месяца 
--- mode_ = '2'  -  Гашение (списание) начисл. платы  (кнопка "2"):   Дт 2600 - Кт 3570/37, 3570/03 - выполняется ежедневно при Открытии Дня
--- mode_ = '3'  -  Перенос начисл.платы на просрочку (кнопка "3"):   Дт 3570/03 -> 3570/37   (выполняется 6-го числа каждого месяца !)
...
------   КНОПКАХ "0" и "1"  и  если начисл.сумма s0_>0 :        --------------
   ...
   IF ( INSTR(mode_,'0')>0  OR  INSTR(mode_,'1')>0 ) and s0_>0   then
      ...
      IF nlsb_tobo is NULL THEN
         ...
         nlsb_tobo := NBS_OB22_NULL( '6510','06',tobo_a );   --- Поиск 6510/06 на бранче счета 2600 

         If nlsb_tobo is NULL  then    ----    Счет 6510/06 не найден. Открываем его: 
                                       
            nlsb_tobo := RKO.Get_NLS_random ('6510') ;  -- получение № счета 6510 методом случ.чисел

            Select ISP, RNK, ACC into isp_6510, rnk_6510, acc_6510_1  --- для нахождения ISP,RNK берем любой счет 6% этого бранча
            from   Accounts
...
----------------------------------------------------------------------------
-- 2.  Гашение начисленной платы  (кнопка "2"):   2600 ->  3570/37, 3570/03
----------------------------------------------------------------------------
...
----------------------------------------------------------------------------
-- 3. Перенос начисл.платы на просрочку  (кнопка "3"):   3570/03 -> 3570/37
--    (выполняется 6-го числа каждого месяца !)
----------------------------------------------------------------------------
