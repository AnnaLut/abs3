CREATE OR REPLACE PACKAGE XOZ16 IS g_header_version   CONSTANT VARCHAR2 (64) := 'version 1  02.04.2019';
-----------------------------------------------
...
CREATE OR REPLACE PACKAGE BODY XOZ16 IS   g_body_version   CONSTANT VARCHAR2 (64) := 'version 1  02.04.2019';
-------------------------------------
...
begin
  ...
  If  p_ND > 0 then      update cc_deal set cc_id = p_cc_id , prod = p_Prod where nd = p_ND ; RETURN ; end if;
  -------------------------
  ...
  begin select * into aa from accounts X where rnk = aa.rnk and nbs = '3500' and ob22='05' and dazs is null and kv = 980 and NOT exists (select 1 from nd_acc where acc = x.ACC) and rownum = 1;
...
----------------------------------------------------------------------------------------------------------------
     If p_Mode in (11,12)   then   
        ...
        If oo.nlsa is null then  
           -- 4600.XX
           OP_BS_OB1( PP_BRANCH => Substr( dd.Branch, 1,8)|| '000000/', P_BBBOO => dd.prod ) ;  
           oo.nlsa := nbs_ob22 ( Substr(dd.prod,1,4),  Substr(dd.prod,5,2) ); 
...
--               Accreg.setAccountSParam(aa.ACC, 'R011', '2'  ) ;
        ...
        --1.1) Визнання зобов`язань~4600/**=>3615/04~BW
        If p_Mode = 11 then  oo.nlsB :=  XOZ16.NLS( dd.ND, dd.rnk, '3615', null ) ; XOZ16.OPL1(oo) ;                end if ;

        --1.2) Визнання активу~4600/**=>3500/04  ----,3519/26~CB
        If p_Mode = 12 then  oo.nlsB :=  XOZ16.NLS( dd.ND, dd.rnk, '3500', null ) ; 
           If oo.nlsb is not null then   XOZ16.OPL1(oo) ;  
           Else       NULL; -------------       oo.nlsB :=  XOZ16.NLS( dd.ND, dd.rnk, '3519', null ) ; XOZ16.OPL1(oo) ;  cl_xoz (OO) ; 
           end if ;
        End If ;
---------------------------------------------------------------------------------------------------------------
     --2.1) Нарахування витрат~7028/01=>3618/01~CG
     ElsIf p_Mode = 21 Then 
        oo.nazn := Substr( 'Нарахування процентних витрат з оренди за місяць згідно дог. №' ||dd.cc_id||'. '||p_Txt ,1,160) ;
        oo.nlsa := nbs_ob22_NULL ('7028', '01' );    -- найти 7028*01
        If oo.nlsa is null then  OP_BS_OB1( PP_BRANCH => dd.Branch, P_BBBOO =>'702801' ) ;    oo.nlsa := nbs_ob22_NULL ('7028', '01' );  end if;
        oo.nlsB := XOZ16.NLS( dd.ND, dd.rnk, '3618', null ) ; 
        XOZ16.OPL1(oo) ; 
     ----------------------------------------------------------------------------------
     --2.2)Переоцінка активу~3615/04<=>4600/**~CY
...
---------------------------------------------------------------------------------------------------------------
     ...
     ElsIf  p_Mode  in ( 31, 32, 33 )  then   
        ...
        --3.1)Закриття нар/витрат~3618/01=>3519/26~CO
        If p_Mode = 31 then 
        ...
        --3.2)Закриття ДЗ~3615/04=>3519/26~CP
        ElsIf p_Mode = 32 then 
        ...
        -- 3.3)Закриття ДЗ(ПДВ)~7399/67=>3519/26~CM
        ElsIf p_Mode = 33 then
           ...
           oo.nlsa := nbs_ob22_NULL ('7399', '67' );    -- найти 7399/67
           If oo.nlsa is null then  OP_BS_OB1( PP_BRANCH => dd.Branch, P_BBBOO =>'739967' ) ;    oo.nlsa := nbs_ob22_NULL ('7399','67' );  end if;
           XOZ16.OPL1(oo) ; 
        end if ;
        cl_xoz (OO) ;
---------------------------------------------------------------------------------------------------------------
