...
begin
  return 'Package header PACK_NU '||G_HEADER_VERSION;
end header_version;
...
PROCEDURE P_OB22NU (p_dat1 date,  p_dat2 date ) is
l_prizn char(1) ;
 l_dat1 date;
 dat_tek DATE;
...
l_sql varchar2(4000);
l_iduser staff$base.id%type := user_id;
c0    SYS_REFCURSOR;
begin
...
procedure del_stmt (p_stmt opldok.stmt%type)
is
 dat_tek DATE;
 l_kk    integer;       /* количество необработанных */
 l_grp   number;
 ...
 savepoint nal#ob22#autopay;
  ...
  begin
    ...
    select  /*+ INDEX(o)  INDEX(s)*/
      count(*) into l_kk
      from  opldok o,nu_ob22_funu s
     where  o.ref=s.ref
       and  o.stmt=s.stmt
	   and  o.stmt = p_stmt
       and bitand(nvl(o.otm,0),1) = 1   -- не оплачена в НУ
       and bitand(nvl(o.otm,0),2) = 2   -- не снята с визы в НУ
       and o.tt<>'PO3' ;
