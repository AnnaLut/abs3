...
procedure ACC1   ( p_acc number, p_dat date  ) is  -- обработка одной строки (птички )
   aa accounts%rowtype ; oo oper%rowtype ;
   rr NRU_OK%rowtype;
   z_dat date;
   Fl_Kl  int := 0;
 ...
 begin
   ...
   loop
     ...
     If k.nbs not in ('2608', '2518', '2528', '2538', '2548', '2558', '2568', '2658', '3570', '3578')
       ...
       (k.ostc > 0 and k.blkd > 0 )       OR
        ...
        Fl_Kl  := 1 ;
        goto Rec_next  ;  -- клиента закрывать не будем
     ...
     If k.ostc > 0 and k.nls < '4' then   -- кред.остат = сворачиваем на котел 2903
         ...
         oo.nazn := Substr( oo.nazn||'. '||
                            to_char (nvl(k.dapp,k.daos),'dd.mm.yyyy')||
   ...
   Update customer x
         set DATE_OFF = gl.bdate
      where x.rnk = aa.rnk and not exists (select 1 from accounts where rnk = aa.rnk and dazs is null)  ;
