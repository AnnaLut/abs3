...
CREATE OR REPLACE PACKAGE BODY BARS.BARS_SMS_INS is
    ...
    procedure prepare_sms_pre_plan_pay is
        ...
        select c.kf, to_char(psh.plan_date,'dd.mm') plan_date, cw.value cellphone
        from ins_deals dl
    ...
    procedure prepare_sms_pre_end_agr is
        ...
        -------------------------------------------------------------------------------------
        cursor cur_send_sms is
        select c.kf, to_char(dl.edate,'dd.mm') edate, cw.value cellphone
        from ins_deals dl
