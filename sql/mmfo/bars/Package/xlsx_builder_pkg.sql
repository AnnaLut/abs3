...
-- Параметри сторінки
/*
       paperSize pls_integer    9- A4 8- A3
      ,fitToWidth pls_integer
      ,fitToHeight pls_integer
      ,orientation varchar2(254)   portrait/ landscape
*/
...
/
CREATE OR REPLACE PACKAGE BODY BARS.XLSX_BUILDER_PKG 
as
  ...
  type tp_comments is table of tp_comment index by pls_integer;
    type tp_pagelayout is record
    (  paperSize          pls_integer
      ,fitToWidth         pls_integer
      ,fitToHeight        pls_integer
      ,orientation        varchar2(254)
	  ,horizontalCentered pls_integer
      ,verticalCentered	  pls_integer
	  ,Margins_sleft      number
  ...
  type tp_pagelayouts is table of tp_pagelayout index by pls_integer;

  -- фіксовані строки для друку
  type tp_definedName is record
    (  col_tl          pls_integer    --top left
	  ,col_br          pls_integer    -- bottom right
	  ,row_tl          pls_integer
      ,row_br          pls_integer
    );
  type tp_definedNames is table of tp_definedName index by pls_integer;
  ...
  type tp_sheet is record
    ...
    , page_layout tp_pagelayouts
    , definedName tp_definedNames
        );
...
--
  ...
  begin
    ...
    for i in 0 .. trunc( ( dbms_lob.getlength( p_blob ) - 1 ) / t_len )+1
    loop
...
--
  ...
  begin
    ...
    loop
      ...
      workbook.sheets( s ).page_layout.delete();
	  workbook.sheets( s ).definedName.delete();
    end loop;
...
--
  ...
  begin
    ...
    then
      ...
      t_ind := get_font( 'Times New Roman' );
    end if;
...
--
 ...
 t_width := trunc( ( t_nr_chr * 7 + 5 ) / 7 * 256 ) / 256; -- assume default 11 point Calibri
    ...
    then
      ...
      workbook.sheets( p_sheet ).widths( p_col ) := round(greatest( t_width, 8.43 ) );
    end if;
...
/*
	  ...
	  ,horizontalCentered pls_integer
      ,verticalCentered   pls_integer
      ,Margins_sleft      pls_integer
      ,Margins_right      pls_integer
      ,Margins_top        pls_integer
      ,Margins_bottom     pls_integer
      ,Margins_header     pls_integer
      ,Margins_footer     pls_integer
*/
 procedure page_layout
    ( p_paperSize pls_integer        := 9
     ,p_fitToWidth pls_integer       := 0
  ...
  begin
    t_ind := workbook.sheets( t_sheet ).comments.count() + 1;
    workbook.sheets( t_sheet ).page_layout( t_ind ).paperSize   := p_paperSize;
    workbook.sheets( t_sheet ).page_layout( t_ind ).fitToWidth  := p_fitToWidth;
    workbook.sheets( t_sheet ).page_layout( t_ind ).fitToHeight := p_fitToHeight;
    workbook.sheets( t_sheet ).page_layout( t_ind ).orientation := dbms_xmlgen.convert( p_orientation );
	workbook.sheets( t_sheet ).page_layout( t_ind ).horizontalCentered := p_horizontalCentered;
	workbook.sheets( t_sheet ).page_layout( t_ind ).verticalCentered   := p_verticalCentered;
	workbook.sheets( t_sheet ).page_layout( t_ind ).Margins_sleft  := p_Margins_sleft;
	workbook.sheets( t_sheet ).page_layout( t_ind ).Margins_right  := p_Margins_right;
	workbook.sheets( t_sheet ).page_layout( t_ind ).Margins_top    := p_Margins_top;
	workbook.sheets( t_sheet ).page_layout( t_ind ).Margins_bottom := p_Margins_bottom;
	workbook.sheets( t_sheet ).page_layout( t_ind ).Margins_header := p_Margins_header;
	workbook.sheets( t_sheet ).page_layout( t_ind ).Margins_footer := p_Margins_footer;
  end;
--
 ...
 procedure page_definedName
  ...
  begin
    t_ind := workbook.sheets( t_sheet ).comments.count() + 1;
    workbook.sheets( t_sheet ).definedName( t_ind ).row_tl := p_row_tl;
    workbook.sheets( t_sheet ).definedName( t_ind ).row_br := p_row_br;
    workbook.sheets( t_sheet ).definedName( t_ind ).col_tl := p_col_tl;
    workbook.sheets( t_sheet ).definedName( t_ind ).col_br := p_col_br;
  end;
...
--
  ...
  is
  begin
    workbook.sheets( nvl( p_sheet, workbook.sheets.count() ) ).heights ( p_row ) := p_width;
  end;*/
--
  procedure set_column_width
    ( p_col pls_integer
  ...
  is
  begin
      workbook.sheets(  nvl( p_sheet, workbook.sheets.count() ) ).Heights ( p_row ):= p_heigth;
  end;
  --
  procedure set_column
    ( p_col pls_integer
...
<sheet name="' || workbook.sheets( s ).name || '" sheetId="' || s || '" r:id="rId' || ( 9 + s ) || '"/>';
    ...
    for s in 1 .. workbook.sheets.count()
    loop
		 CASE  WHEN  workbook.sheets( s ).definedName.count() = 0
				  then  null;
				  ELSE  t_defin := t_defin ||'<definedName name="_xlnm.Print_Titles" localSheetId="'||(s-1)||'">';
				   l_xx := '';
                        for c in 1 .. workbook.sheets( s ).definedName.count()
                          loop
					            if     workbook.sheets( s ).definedName(c).row_tl is not null
                                   and workbook.sheets( s ).definedName(c).row_br is not null
                                then  t_defin := t_defin|| workbook.sheets( s ).name||'!$'||(workbook.sheets( s ).definedName(c).row_tl)||':$'||(workbook.sheets( s ).definedName(c).row_br);
								      l_xx := ',';
                                end if;

					            if     workbook.sheets( s ).definedName(c).col_tl is not null
                                   and workbook.sheets( s ).definedName(c).col_br is not null
                                then  t_defin := t_defin||l_xx||workbook.sheets( s ).name||'!$'||alfan_col(workbook.sheets( s ).definedName(c).col_tl)||':$'||alfan_col(workbook.sheets( s ).definedName(c).col_br);
                                end if;
					      end loop;
				  --Потестим!$A:$J,Потестим!$1:$1</definedName>';
				  --then  t_defin := t_defin|| workbook.sheets( s ).name||'!$'||alfan_col(workbook.sheets( s ).definedName(c).col_tl)||':$'||alfan_col(workbook.sheets( s ).definedName(c).col_br);

				        t_defin := t_defin ||'</definedName>'||chr(10);
		 END case;
    END LOOP;


t_xxx := t_xxx ||chr(10)||'<definedNames>'||chr(10)||t_defin||'</definedNames>';

t_xxx := t_xxx ||'<calcPr calcId="144525"/>
</workbook>';
...
<sheetFormatPr defaultRowHeight="15" x14ac:dyDescent="0.25"/>';
      ...
      loop
        -- Висота строки
		t_tmp_Heights := null;
		if workbook.sheets( s ).Heights.count() > 0
         then
		if workbook.sheets( s ).Heights.exists( t_row_ind )
			then
			--DBMS_OUTPUT.PUT_LINE('BBBBBBBB '||t_row_ind||' =' ||workbook.sheets( s ).Heights( t_row_ind ));
			t_tmp_Heights := ' ht="'||workbook.sheets( s ).Heights( t_row_ind )||'" customHeight="1"';
			end if;
  		end if;

        t_tmp :=  t_tmp || '<row r="' || t_row_ind || '" spans="' || t_col_min || ':' || t_col_max || '" '||t_tmp_Heights||'>';
      ...
      if workbook.sheets( s ).page_layout.count() = 0 then
          t_xxx := t_xxx || '<pageMargins left="0.2" right="0.2" top="0.2" bottom="0.2" header="0.2" footer="0.2"/>';
	  end if;


      for c in 1 .. workbook.sheets( s ).page_layout.count()
      loop


		t_xxx := t_xxx || '<printOptions horizontalCentered="'  || to_char(workbook.sheets( s ).page_layout(c).horizontalCentered) ||'"
										 verticalCentered="'    || to_char(workbook.sheets( s ).page_layout(c).verticalCentered)   ||'" />';

    	t_xxx := t_xxx || '<pageMargins left="'  || to_char(workbook.sheets( s ).page_layout(c).Margins_sleft,  g_number_format, g_number_nlsparam)  ||'"
									    right="' || to_char(workbook.sheets( s ).page_layout(c).Margins_right,  g_number_format, g_number_nlsparam)  ||'"
									    top="'   || to_char(workbook.sheets( s ).page_layout(c).Margins_top,    g_number_format, g_number_nlsparam)    ||'"
									    bottom="'|| to_char(workbook.sheets( s ).page_layout(c).Margins_bottom, g_number_format, g_number_nlsparam) ||'"
									    header="'|| to_char(workbook.sheets( s ).page_layout(c).Margins_header, g_number_format, g_number_nlsparam) ||'"
									    footer="'|| to_char(workbook.sheets( s ).page_layout(c).Margins_footer, g_number_format, g_number_nlsparam) ||'"  />';


        t_xxx := t_xxx || '<pageSetup paperSize="'  || to_char(workbook.sheets( s ).page_layout(c).paperSize)   ||'"
                                      fitToWidth="' || to_char(workbook.sheets( s ).page_layout(c).fitToWidth)  ||'"
                                      fitToHeight="'|| to_char(workbook.sheets( s ).page_layout(c).fitToHeight) ||'"
                                      orientation="'|| to_char(workbook.sheets( s ).page_layout(c).orientation) ||'"
                                      r:id="rId'    || to_char(s+9)                                             || '" />';
