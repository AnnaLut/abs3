    ...
    begin
      ...
      values
        (l_ref,
         ...
         l_sk,
         substr(l_ref,1,9));
      bars_audit.info('NER0 l_nls_a = '||l_nls_a||' l_nls_b= '||l_nls_b);
      paytt(null,
            l_ref,
            l_bankdate,
            l_tt,
      ...
      gl.ref(l_ref_inf);
       ...
       if length(l_nd_inf) > 10 then
           l_nd_inf := substr(l_nd_inf, -10);
        else
           l_nd_inf := l_ref_inf;
        end if;
...
*/
                ...
                select '<?xml version="1.0" encoding="windows-1251" ?>' ||
                 xmlelement("data_batch", xmlattributes(f_ourmfo as "mfo", c.batch_id as "batch_id", b.bsd as "bsd", b.ob22 as "ob22", b.kv as "kv", sum(nvl(a.ost,0)) as "sum", to_char(b.create_date,'yyyymmddhh24miss') as "batch_date"),
               ...
               group by f_ourmfo, c.batch_id,b.bsd, b.ob22,b.kv, to_char(b.create_date,'yyyymmddhh24miss');
