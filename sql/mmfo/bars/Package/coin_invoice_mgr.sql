 ...
 PROMPT ===================================================================================== 
  ...
  CREATE OR REPLACE PACKAGE BARS.COIN_INVOICE_MGR is
  ...
  g_header_version  constant varchar2(64) := 'version 1.0.0 27/06/2016';
  ...
  --
  procedure clear_invoice(p_nd in tmp_coin_invoice.nd%type);
  ...
  procedure create_invoice(p_type_id             in tmp_coin_invoice.type_id%type,
                           ...
                           p_total_with_vat      in tmp_coin_invoice.total_with_vat%type);

  --------------------------------------------------------------------------------
  -- pay_invoice - оплатить накладную
  --
  procedure pay_invoice(p_nd in varchar2);
  ...
  --
  procedure remove_invoice_detail(p_rn in number, p_code in varchar2);
...
--If nS_  > 0 then                            gl.payv(0,nRef_,gl.bdate,'MF1',1,n980_,NLSD3_ ,nS_  ,n980_,NLSK1_,nS_  ); end if ;  -- до сплати всього --------
  ...
  begin
    ...
    if (p_type_id = 1) then
      ...
      l_unit_price_vat := p_price + l_nominal_price;-- repaired by Andrii Mykhalevych for http://jira.unity-bars.com.ua:11000/browse/COBUSUPMMFO-751
	-- and http://jira.unity-bars.com.ua:11000/browse/COBUSUPMMFO-770
                          --was:-- l_nominal_price * 1.2;
    else
      l_unit_price     := 0;
      l_nominal_sum    := 0;
      l_unit_price_vat := 0;
    ...
    if( l_nominal_sum < 0 or l_unit_price_vat < 0 ) then
      raise_application_error(-20001, 'Ціна за одну штуку меньша за номінальну вартість');
    end if;
...
PROMPT *** Create  grants  COIN_INVOICE_MGR ***
grant EXECUTE                                                                on COIN_INVOICE_MGR to BARS_ACCESS_DEFROLE;
