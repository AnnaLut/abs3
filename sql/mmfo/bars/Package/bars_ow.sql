     ...
     select nms, okpo, tobo, opt, nls -- COBUMMFO-7501
       ...
       from (  -- COBUMMFO-7501
               ...
               where (a.nls = p_nls or (a.nlsalt = p_nls and a.tip like 'W4%' and regexp_like(a.nls, '^26[0,2,5]0'))) 
                     and a.kv = p_kv and a.dazs is null
               order by a.daos 
            )
      where rownum = 1;
      f := true;
     ...
     if p_mode = 0 then
        ...
        select a.acc, a.nls,  a.kv, t.dk, o.tt, decode(o.kv, o.kv2, o.s, decode(o.dk, 0, o.s, o.s2)), o.sos, o.currvisagrp, o.nextvisagrp, nvl2(trim(wt.tt),1,0), decode(o.mfoa, o.mfob, 1, 0), o.s2
         ...
         where q.ref_tr = p_ref
           ...
           and a.kf     = sys_context('bars_context','mfo')
           and (q.nls_tr = a.nls or
                 (q.nls_tr = a.nlsalt and a.tip like 'W4%' and regexp_like(a.nls, '^26[0,2,5]0'))) -- COBUMMFO-7501
        ...
        begin
           select q.id, q.ref_tr into l_id, l_ref
            ...
            where q.state  = 0
              ...
              and (q.nls_tr = a.nls or
                  (q.nls_tr = a.nlsalt and a.tip like 'W4%' and regexp_like(a.nls, '^26[0,2,5]0'))) -- COBUMMFO-7501
              and ( decode(t.dk, 0, l_atrn(i).debit_anlaccount, l_atrn(i).credit_anlaccount) = a.nls or
                  ( decode(t.dk, 0, l_atrn(i).debit_anlaccount, l_atrn(i).credit_anlaccount) = a.nlsalt
                    and a.tip like 'W4%' and regexp_like(a.nls, '^26[0,2,5]0'))) -- Comment COBUMMFO-7501
      ...
      if l_pk_nls is not null then
         begin
            ...
            from (
                    select a.acc
                    from accounts a
                    where (a.nls = l_pk_nls or (a.nlsalt = l_pk_nls and a.tip like 'W4%' and regexp_like(a.nls, '^26[0,2,5]0'))) 
                          and a.kv = p_kv  and a.dazs is null
                    order by a.daos
                 )
             where rownum = 1;
   ...
   if l_pk_nls is not null then
      begin
         ...
         from (
                 ...
                 from accounts a
                 where (a.nls = l_pk_nls or (a.nlsalt = l_pk_nls and a.tip like 'W4%' and regexp_like(a.nls, '^26[0,2,5]0'))) 
                       and a.kv = l_pk_kv and a.dazs is null
                 order by a.daos
              )
         where rownum = 1;
      exception when no_data_found then
  ...
  select xmlelement("ParamsEwa",
                    ...
                    xmlelement("param",
                               ...
                               xmlelement("customFields",
                                          ...
                                          --9623 ----------------------------------------------|                                                                
                                          xmlelement("CustomFields",
                                                     xmlforest('resident' "code",
                                                               p_insurance.rezident "value"
                                                               )
                                                     ),
                                          xmlelement("CustomFields",
                                                     xmlforest('citizenship' "code",
                                                               p_insurance.citizenship "value"
                                                               )
                                                     )
                                          ------------------------------------------------------|                                                                
...
begin
  ...
  -- Якщо працюємо в поточній банківській даті, то вибираємо
  -- значення ліміту з ACCOUNTS інакше з ACCOUNTS_UPDATE
