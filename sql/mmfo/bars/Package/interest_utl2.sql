 ...
 PROMPT ===================================================================================== 
 PROMPT *** Run *** ========== Scripts /Sql/BARS/package/interest_utl2.sql =========*** Run *
 PROMPT ===================================================================================== 
 
  CREATE OR REPLACE PACKAGE BARS.INTEREST_UTL2 is
...
/
CREATE OR REPLACE PACKAGE BODY BARS.INTEREST_UTL2 as
    ...
    begin
        ...
        if (p_int_reckoning_row.interest_tail <> 0 or p_int_reckoning_row.interest_amount <> 0) then
            ...
            if (l_interest_amount > 0 and l_income_amount > 0) then
                ...
                if (l_int_accn_row.metr = 4) then -- амортизация (змінює порядок рахунків: амортизація активного рахунку виконується за рахунок витрат)
...
*/
    ...
    begin
        for i in (select *
            ...
            pay_int_reckoning_row(i, p_silent_mode => true, p_do_not_store_interest_tails => p_do_not_store_interest_tails, p_dk => p_dk);
        end loop;
    end;
...
*/
    ...
    begin
        if (p_date_for is null or p_period is null or p_period_length is null) then
            return null;
        end if;
        ...
        when 'YEAR' then
             ...
             raise_application_error(-20000, 'Неочікуваний тип періоду {' || p_period || '}');
        end case;

        if (p_holidays_handling > 0) then
            l_previous_date := dat_next_u(l_previous_date, 0);
        elsif (p_holidays_handling < 0) then
            if (l_previous_date <> dat_next_u(l_previous_date, 0)) then
                l_previous_date := dat_next_u(l_previous_date, -1);
                if (l_previous_date < l_initial_date) then
                    l_previous_date := dat_next_u(l_previous_date, 1);
                end if;
            end if;
        end if;
    ...
    begin
        ...
        -- отримаємо попередню дату
        l_starting_date := find_previous_date(p_date_for, p_period, p_period_length, p_initial_date, p_strict_shift => 0, p_holidays_handling => 0);
        if (p_period_length <= 0) then
            raise_application_error(-20000, 'Довжина періоду повинна бути більше нуля');
        end if;
        ...
        case (upper(p_period))
        when 'DAY' then
             l_previous_date := p_date_for + p_period_length;
        when 'WEEK' then
             l_days_difference := (l_initial_date - p_date_for) mod 7;

             -- якщо потрапляємо не на той самий день тижня (l_week_days_difference <> 0), а будь-який інший, то шукаємо
             -- потрібний день на наступному тижні, а не в поточному
             if (l_days_difference <> 0) then
                 l_period_length := greatest(l_period_length, 1);
             end if;

             l_previous_date := p_date_for + (7 * l_period_length) + l_days_difference;
        when 'MONTH' then
             l_initial_day := extract(day from l_initial_date);
             l_day_for := extract(day from p_date_for);
             -- l_days_difference :=
             l_date_for := trunc(p_date_for, 'MONTH') + l_initial_day - 1;
        when 'QUARTER' then
             null;
        when 'YEAR' then
             null;
    ...
    is
        l_date_through date;
    begin
        if (p_date_from is null or p_date_through is null or p_period_kind is null) then
            return;
        end if;

        l_date_through := find_previous_date(p_date_from, p_period_kind, p_period_length, p_base_date);
        while (l_date_through is not null and l_date_through < p_date_through) loop
            l_date_through := find_next_date(l_date_through, p_period_kind, p_period_length, p_base_date);
        end loop;
        -- знайти таку базову дату, що буде меньше або рівною p_date_from і відповідатиме періоду p_periodicity + p_period_length відносно p_base_date
/*        case (upper(p_periodicity))
        when 'DAY'
             l_date_through :=
        when 'WEEK' then
        ...
        when 'YEAR' then
             ...
             raise_application_error(-20000, 'Неочікуваний тип періоду {' || p_periodicity || '}');
        end case;
        l_date*/
        return;
    end;
    ...
    begin
        ...
        if (l_reckoning_row.accrual_purpose is null) then
            ...
            return generate_accrual_purpose(l_int_accn_row.metr,
    ...
    begin
        ...
        if (l_reckoning_row.payment_purpose is null) then
            ...
            return generate_payment_purpose(l_int_accn_row.metr,
                                            account_utl.get_account_number(l_reckoning_row.account_id),
                                            l_reckoning_row.date_from,
                                            l_reckoning_row.date_through,
                                            l_reckoning_row.interest_rate);
    ...
    begin
        ...
        if (l_reckoning_row.state_id in (interest_utl.RECKONING_STATE_PAYED)) then
            ...
            return case when l_reckoning_row.payment_purpose is null then
                             generate_payment_purpose(read_int_accn(l_reckoning_row.account_id, l_reckoning_row.interest_kind_id).metr,
                                                      account_utl.get_account_number(l_reckoning_row.account_id),
                                                      l_reckoning_row.date_from,
                                                      l_reckoning_row.date_through,
                                                      l_reckoning_row.interest_rate)
    ...
    begin
        ...
        l := p_accounts.first;
        while (l is not null) loop
            dbms_output.put_line('account id: ' || p_accounts(l));
            l_date_from := null;
            for i in (select t.*,
                ...
                dbms_output.put_line('reckoning id: ' || i.id || ' ' || i.date_from);
                ...
                if (l_date_from is null) then
                    ...
                    if (l_interest_kind_id <> i.interest_kind_id or
                        -- trunc(i.date_from, 'MONTH') <> trunc(l_date_from, 'MONTH') or
...
/
 ...
 PROMPT ===================================================================================== 
 PROMPT *** End *** ========== Scripts /Sql/BARS/package/interest_utl2.sql =========*** End *
 PROMPT ===================================================================================== 
