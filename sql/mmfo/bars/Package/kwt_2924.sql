...
CREATE OR REPLACE PACKAGE BODY BARS.KWT_2924 IS
   G_BODY_VERSION  CONSTANT VARCHAR2(64)  :=   'ver.4.4 20.09.2018';
/*
 20.09.2018 Змінено призначення платежу COBUMMFO-8881
...
begin
   ...
   insert into ATM_REF3 
      (s,ref1,acc) 
   values 
      (p_S3*100,p_Ref1, p_acc);
         ...
         */
         ...
         loop
            begin 
               ...
               into l_D2
               from opldok 
               where ref in (select ref2 from atm_ref2 where ref1 = x.ref1 ) 
                     and acc = k.acc 
                     and dk =  l_dk2;
               ...
               If oo.s <> 0 then
                  ...
                  If oo.dk = 1 then 
				 	 oo.nlsb := nbs_ob22_null( '6399' , 'L9' , k.branch); 
                     oo.nazn  := 
                      'Віднесено кошти на доходи по АТМ/ІПТ '||regexp_substr(k.nms,'\w\d{3,}')||' по яким не було запитів протягом 180 днів, реф '||l_ref2||' від '|| to_char(l_dat2,'dd/mm/yyyy') ;
                  else
                     oo.nlsb := nbs_ob22_null( '7399' , '53' , k.branch); 
                     oo.nazn  := 
                      'Визнання витрат за сумами нестач готівки із касет АТМ/ІПТ '||regexp_substr(k.nms,'\w\d{3,}')||' після закінчення строку опротестування (180 днів), реф '||l_ref2||' від '|| to_char(l_dat2,'dd/mm/yyyy') ;
                  end if ;
                  /*
                  ...
                  */
                  ...
                  gl.in_doc3(
                               ref_=> oo.REF, tt_=> 'D66', vob_ => 6   , nd_ =>oo.nd, pdat_=> SYSDATE, vdat_ => gl.bdate, dk_  => oo.dk,
            ...
            EXCEPTION 
               WHEN NO_DATA_FOUND THEN oo.S := null ;
             end;

            If oo.S = 0 then 
               KWT_2924.DEL_ATM1(k.acc, x.REF1);  
            end if;
         end loop; -- x
      end if;
   end loop ; -- k
...
begin 
   ...
   l_ACC := NVL(p_acc, to_number( pul.get('ATM_ACC')));
   
   logger.info('DEL_ATM1_l_ACC:   '||l_ACC||'   DEL_ATM1_p_REF1:   '||p_REF1);
...
begin 
   l_ACC := NVL(p_acc, to_number( pul.get('ATM_ACC')));
   insert into atm_ref1
       (acc, ref1) 
   select a.acc,o.ref 
   from accounts a
   join opldok o on a.acc=o.acc
   where a.acc = l_acc 
         and a.acc = o.acc 
         and o.ref = p_ref1 
         and o.dk = a.pap-1 
         and a.tip in ('AT7','AT8');
end INS_ATM1;
-------------------------------------
...
begin 
   ...
   l_DK1  := to_number (pul.get('ATM_DK'));
   l_DK2 := 1- l_DK1;

   If gl.doc.tt ='015' 
	  and gl.doc.nlsb NOT like '2924%'
      and gl.doc.nlsb NOT like '2909%'  
      OR  gl.doc.tt ='013' 
      and gl.doc.nlsb NOT like '2924%' then 
         x_Err := 'ATM_ERR1'; 
         goto ERR_; -- '1)Рах.'||gl.doc.nlsb|| ' не відповідає коду операції' ; 
   end if;
   ...
   If gl.doc.nlsb like '2924%' or gl.doc.nlsb like '2909%' then
      ...
      If gl.doc.tt in ('015')  
	 	 and bb.ob22 = '81'  
         and gl.doc.nlsb like '2909%'  
         OR  
         gl.doc.tt in ('015','013')  
         and bb.ob22 = '10' and gl.doc.nazn like 'Спис.кошти по опрот.плат.%' 
         OR
         gl.doc.tt in ('015') 
         and bb.ob22 = '08'
         and gl.doc.nazn like 'Взаємозак%'                 
         OR 
         gl.doc.tt in ('015') 
         and bb.ob22 = '07'
         and gl.doc.nazn like 'Перенесено%'                
         OR  
	 	 gl.doc.tt in ('310')
         and gl.doc.nazn like 'Кошти%' then
            null;
      else
		 x_Err := 'ATM_ERR2';
         goto ERR_ ; -- '2)Рах.'||gl.doc.nlsb|| ' не відповідає призначенню'; 
      end if;
   end if ;

   If gl.doc.nlsb like '2924%'  
	  and gl.doc.tt in ('015') 
      and gl.doc.nazn like 'Взаємозак%' 
      and  l_Ref1 > 0 then -- "Кучку-> в -> ямку" только для один-в-один. не для нескольких кучек одновременноthen
         begin 
			select to_number(w.value) 
            into x_Ref 
            from operw w 
            where w.tag ='REFX' and w.ref = gl.aRef;
         exception 
			when others then
                 x_Err := 'ATM_ERR3'; goto ERR_ ; -- '3) Відсутній Дод.рекв.REFX = Реф."ямки "'; goto ERR_ ; 
         end;
         ...
         begin 
            ...
            where r.Ref1 = x_Ref 
                  and r.acc = bb.acc 
                  and o.ref = x_Ref 
                  and o.S - x_S >= gl.doc.S;

            insert into atm_ref2 (ref1, ref2) values (x_ref, l_ref2) ;
         exception 
			when others then
                 x_Err := 'ATM_ERR4'; goto ERR_ ; -- '4) Не відповідає умовам ""ямки"'; goto ERR_ ; 
         end;
      end if;
      ...
      */
      ...
      If x_Err is not null then   
		 bars_error.raise_nerror( modcode, x_Err); 
      end if;

      If l_Ref1 > 0 then
         insert into atm_ref2 (ref1, ref2, s ) values (l_ref1, l_ref2, null );
      else                   
		 insert into atm_ref2 (ref1, ref2, s ) select  ref1, l_ref2, s from atm_ref3 where acc = l_ACC;  
      end if ;
...
begin
   If p_dat is null then
      ...
      begin 
		 select * into dd from kwt_d_2924 where DAT_SYS = dd.DAT_SYS;
      EXCEPTION 
		 WHEN NO_DATA_FOUND THEN 
			  l_txt := g_TAG || ' Не знайдено дату'|| to_char( dd.DAT_SYS ,'dd.mm.yyyy') || ' в календарi квитовки 2924 ';
              If p_mode = 0 then 
			     logger.info(l_txt); else raise_application_error(n_err, l_txt); 
              end if ;
      end;
   else
      dd.DAT_KWT := p_dat ;
...
begin 
   for ka in (
                ...
                where dazs is null 
                      and dapp is not null 
                      and nbs = '2924' and ob22 = p_ob22
             )
   loop 
	  KWT_2924.KWT_aa(ka, p_dat); 
   end loop;
end KWT_ob22;
...
begin
   ...
   EXCEPTION 
	  WHEN NO_DATA_FOUND THEN  
		   l_dat_beg := l_Dat_VZ;
           insert into kwt_a_2924 
              (acc, branch, daos, nls, kv, ob22, KF) 
           values 
              (aa.acc, aa.branch, aa.daos, aa.nls, aa.kv, aa.ob22, aa.KF );
   end;
   ...
   begin 
      ...
      from saldoa 
      where acc = aa.acc 
            and fdat = l_dapp; -- подобный запрос выполняли чуть выше, зачем еще раз дергать базу
                              -- ( select max(fdat) from saldoa where acc = aa.acc and fdat <= l_dat_end );
   EXCEPTION 
	  WHEN NO_DATA_FOUND THEN l_ost := 0;
   end;

   If l_dapp < l_dat_Beg  then  ------------------------------------------------ Протокол старый. без изменений
      update kwt_a_2924 
      set dat_KWT = l_dat_end
      where acc = aa.acc;

   ElsIf l_ost =  0 then
         delete from kwt_t_2924 where acc = aa.acc ; ---- Протокол НУЛЕВОЙ = пустой
         update kwt_a_2924 
         set dapp = l_DAPP
             , datvz = l_dat_VZ
             , dat_KWT = l_dat_end
   ...
   Else  
	  -- Таблица очищается при commit
      -- EXECUTE   IMMEDIATE  ' truncate  TABLE TMP_KWT_2924 ';
      If l_Dat_VZ >= l_dat_beg then   
		 delete from kwt_t_2924 where acc = aa.acc;
         l_dat_beg :=  l_dat_VZ ; ---- Протокол вновь созданный
      else  
		 Insert into TMP_KWT_2924
             (s, fdat, ref, tt2) 
         select s, fdat, ref, tt2 
         from kwt_t_2924 
         where acc = aa.acc; ---- Протокод приращенный
     ...
     for oo in (
		          select o.tt
                         ...
                         , (2*dk-1) * o.s  S
                  from opldok o
                  join saldoa s on s.fdat = o.fdat and s.acc = o.acc
                  where s.fdat >= l_dat_Beg and s.fdat <= l_dat_end and s.acc = aa.ACC 
               )
      loop
         /*
         ...
         */
         begin 
            ...
            where s = -oo.s 
                  and REF= oo.REF 
                  and rownum = 1; --- будем искать противоположный знак в том же реф
         ...
         EXCEPTION WHEN NO_DATA_FOUND THEN
            begin 
               ...
               from TMP_KWT_2924 
               where s = -oo.s 
                     and rownum = 1; --- будем искать противоположный знак с любым реф
               delete from TMP_KWT_2924 where rowid = l_RI; --- сквитовано
            EXCEPTION 
			   WHEN NO_DATA_FOUND THEN
                    insert into TMP_KWT_2924 
                        (s, fdat, ref, tt2) 
                     values
                        (oo.s, oo.fdat, oo.ref, oo.tt ) ; --- будем писать со своим знаком
            END;
         END;
      end loop; --- oo

      -- Выгрузим несквитованное
      insert into kwt_t_2924
          (acc, fdat, s, ref, tt2) 
      select aa.acc, fdat, s, ref, tt2  
      from TMP_KWT_2924;
      ...
      If KWT_2924.DAT31(l_dat_end) = 1 then
         -- это месячная отчетная дата. сделаем копию
         insert into kwt_t31_2924 
            (acc, fdat, s, ref, tt2) 
         select aa.acc, fdat, s, ref, tt2  
         from TMP_KWT_2924;
         
         insert into kwt_a31_2924
             (ACC, BRANCH , NLS, KV, OB22, DAOS, DAPP, DATVZ, DAT_KWT, IXD, IXK)
         select ACC, BRANCH , NLS, KV, OB22, DAOS, DAPP, DATVZ, DAT_KWT, IXD, IXK
         from kwt_a_2924 
         where acc = aa.acc 
               and dat_KWT = l_dat_end;
      end if;
   end if ;
...
begin
  ...
  If l_s <> 0 then 
	 raise_application_error(-20000, 'KWT_2924: НЕ збалансовані відмічені для квитовки суми !');
  end if ;
  
  delete from kwt_t_2924 where acc = l_ACC and rkw = 1;
end OK1;
---------------
function DAT31(p_dat date) 
return int is
begin  
   return case to_char(p_dat,'YYYYMM') < to_char( DAT_NEXT_U(p_DAT,1), 'YYYYMM' )
               when true then 1
			   else 0
          end;
end DAT31;
...
begin
   If p_mode = 1 
	  and p_tt = '013' then 
      l_nazn := 'Спис.кошти по опрот.плат. АТМ А_______ тран. за __/__/__ (дата оскар. __/__/__ ) ';  ------реф. ___________ надл. __/__/__   411997******7295';                       
   ElsIf p_mode = 2 
 	     and p_tt = '013' then 
         l_nazn := 'Взаємозак. нестачі по АТМ/ІПТ А_______ зг. служб.записки №__ від __/__/__' ; --  реф. ___________ від __/__/__ за рах.надлишків реф. ___________ від __/__/__ зг. служб.записки №__ від __/__/__';
   ElsIf p_mode = 3 
	     and p_tt = '013' then 
         l_nazn := 'Перенесено на відповідний аналітичний рахунок кошти  '; -- _________ від __/__/__  на реф.___________ від __/__/__';                                           
   ElsIf p_mode = 1 then 
         If p_TT = 'PKR' then 
	 	    l_nazn := 'Зараховано на карт/рах. зг. звернення' ; 
         ElsIf p_TT = '310' then 
		    l_nazn := 'Кошти для зарах на рах.2625 ПІБ, ІНН;' ; 
         ElsIf p_tt = '015' then 
		    l_nazn := 'Кошти для виплати <ПІБ> зг. звернення' ; 
         end if;
   ...
   for oo in (
  	            select to_char(o.vdat,'dd/mm/yyyy') || ',реф.'|| o.REF||', ' txt 
                from oper o 
                where o.ref = p_REF1      
                union 
                select to_char(o.vdat,'dd/mm/yyyy') || ',реф.'|| o.REF||', ' txt 
  ...
  loop   
	 l_nazn := Substr(l_nazn || oo.txt, 1, 160); 
  end loop;
...
PROCEDURE jobs_kwt_2924
is

   l_job  varchar2(400):='KWT_JOB_';
...
begin
   ...
   loop
      ...
      l_job_name := null;
      begin
         l_job_name := l_job||c.kf;
         l_jobact   := 'begin bc.go('||c.kf||'); kwt_2924.kwt_all(0, null ); bc.home; end;';
         
         dbms_scheduler.create_job(job_name => l_job_name, job_type => 'PLSQL_BLOCK', job_action => l_jobact, auto_drop => false);
      exception 
		 when others then
      ...
      if l_job_name is not null 
		 and not running(l_job_name) then
         if l_jobact is not null then
            dbms_scheduler.set_attribute(name => l_job_name, attribute => 'job_action', value => l_jobact);
         end if;
...
is 
begin  
   return 'Package header KWT_2924 '||G_HEADER_VERSION; 
end header_version;
...
is 
begin  
   return 'Package body KWT_2924 '||G_BODY_VERSION; 
end body_version;
--------------
