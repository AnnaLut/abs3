...
$end
  ...
  begin
    ...
    when '#3K' then
      ...
      open p_recordset
       for select /* XML_RPT_3K */ 'A3K001' as EKP
                     , ekp_2                as Q003_1
                     , F091                 as F091
                     , R030                 as R030
                     , T071                 as T071
      ...
      when '27X' then
         open p_recordset
         for
            select 'A27001' as EKP                     --Код показателя
    ...
    when '39X' then
      ...
      open p_recordset
       for select substr(t.FIELD_CODE, 2, 6) as EKP
                , t.nbuc as KU
          ...
          group by t.nbuc
                 ...
                 , substr(t.field_code, 8, 3);

    when '73X' then

      open p_recordset
       for select substr(field_code, 1, 6) as EKP  -- Код показателя
                , t.nbuc as KU                     -- Код области
