 ...
 PROMPT ===================================================================================== 
 PROMPT *** Run *** ========== Scripts /Sql/BARS/package/mgr_commerc.sql =========*** Run ***
 PROMPT ===================================================================================== 
 
  CREATE OR REPLACE PACKAGE BARS.MGR_COMMERC is
  ---------------------------------------------------------------------------
...
/
CREATE OR REPLACE PACKAGE BODY BARS.MGR_COMMERC is
  -----------------------------------------------------------------------
  -- Copyryight : UNITY-BARS
  -- Author     : MOS
  -- Created    : 05.04.2013
  ...
  begin
    ...
    for t in (select *
               ...
               where table_name=l_table
                 and (column_name not in ('KF','BRANCH') or  table_name  like 'CIN%') 
               order by column_id
             )
    loop
    ...
    begin
       ...
       select nvl(max(idupd),1)
        ...
        select a.acc, a.r011, a.r013, a.s080, a.s180, a.s181, a.s190, a.s200, a.s230, a.s240, a.d020, a.kekd, a.ktk,
               ...
               a.s260, null as k150, null as r114, null as s280, null as s290, null as s370,
      ...
      while i < l_maxref
          loop
            ...
            select nvl(round((refcommit_opr - rmin)*100/((rmax-rmin)),2),0) into l_prc from imp_fill where pname = p_name;
             commit;
            DBMS_APPLICATION_INFO.SET_CLIENT_INFO('Заімпортовано записів(oper): '||l_prc||'% до реф '||l_minref||' Pname '||p_name);
          end loop;
      ...
      while i < l_maxref
          loop
            ...
            select nvl(round((refcommit_oprw - rmin)*100/((rmax-rmin)),2),0) into l_prc from imp_fill where pname = p_name;
             commit;
            DBMS_APPLICATION_INFO.SET_CLIENT_INFO('Заімпортовано записів(operw): '||l_prc||'% до реф '||l_minref||' Pname '||p_name);
          end loop;
       ...
       for c in (select index_name
                        from ( select i.index_name,  c.constraint_type
                                ...
                                where i.table_name = 'OPLDOK'
                                  ...
                                  and i.index_name = c.index_name (+)
                                  and c.constraint_type(+) = 'P'
                                   and i.index_name not like 'PK_OPLDOK'
                                   and i.index_name not like 'UK_ID_OPLDOK'    
                              )
                        where constraint_type is null
                     )
            loop
      ...
      while i < l_maxref
          loop
            ...
            select nvl(round((refcommit_opl - rmin)*100/((rmax-rmin)),2),0) into l_prc from imp_fill where pname = p_name;
             commit;
            DBMS_APPLICATION_INFO.SET_CLIENT_INFO('Заімпортовано записів(opldok): '||l_prc||'% до реф '||l_minref||' Pname '||p_name);
          end loop;
       ...
       for c in (select index_name
                        from ( select i.index_name,  c.constraint_type
                                ...
                                where i.table_name = 'ARC_RRP'
                                  and i.table_name = c.table_name (+)
                                  and i.index_name = c.index_name (+)
                                  and c.constraint_type(+) = 'P' and i.index_name<>'UK_ARCRRP'
                              )
                        where constraint_type is null
                     )
            loop
      ...
      while i < l_maxref
          loop
            ...
            select nvl(round((refcommit_arc - rmin)*100/((rmax-rmin)),2),0) into l_prc from imp_fill where pname = p_name;
             commit;
            DBMS_APPLICATION_INFO.SET_CLIENT_INFO('Заімпортовано записів(arc_rrp): '||l_prc||'% до рек '||l_minref||' Pname '||substr(p_name,-4));
          end loop;
...
/
 ...
 PROMPT ===================================================================================== 
 PROMPT *** End *** ========== Scripts /Sql/BARS/package/mgr_commerc.sql =========*** End ***
 PROMPT ===================================================================================== 
