 ...
 PROMPT ===================================================================================== 
 PROMPT *** Run *** ========== Scripts /Sql/BARS/package/migraas.sql =========*** Run *** ===
 PROMPT ===================================================================================== 
 
  CREATE OR REPLACE PACKAGE BARS.MIGRAAS 
is
  version_header  constant  varchar2(64) := 'version 1.18 11.08 14:26';
...
as
  version_body  constant  varchar2(64) := 'version 2.68 12.05.2015 17:06';
...
-- v.2.61 11.08.2014 14:27 - Изменения от Олега Музыки

-- v.2.62 27.08.2014 13:51 - Миграция неподвижных вкладов АСВО: уточнение в
--                           ветке импорта процентных ставок

-- v.2.63 27.08.2014 14:38 - Миграция неподвижных вкладов АСВО: исправлена
--                           процедура отката сворачивания котловых счетов

-- v.2.64 21.10.2014 12:47 - Миграция вкладов: изменение вызова
--                           dpt_web.p_open_vklad_rnk (значение параметра
--                           p_usagemode >= 'MI')

-- v.2.65 23.10.2014 17:46 - Миграция неподвижных вкладов АСВО:
--                           отсеиваются нецифровые символы в полях PHONE_H,
--                           PHONE_J, PASP_N, IDCODE;
--                           отсеиваются непечатные символы в полях ATTR, MARK

-- v.2.66 21.11.2014 12:56 - Миграция вкладов и неподвижных вкладов АСВО:
--                           модификация алгоритма процесса импорта для
--                           возможности работы НЕ от пользователя BARS;
--                           Миграция вкладов: изменение вызова
--                           dpt_web.p_open_vklad_rnk (значение параметра
--                           p_usagemode >= 'IO' или >= 'MI' в зависимости от
--                           версии пакета DPT_WEB)

-- v.2.67 06.01.2015 10:17 - Миграция вкладов СБОН: импортируются вклады и с
--                           нулевыми остатками

-- v.2.68 12.05.2015 17:06 - Миграция вкладов: при откате вкладов и удалении
--                           клиентов и счетов в перечень очищаемых объектов
--                           включена таблица int_accn_update
...
--  vidd_flag           t_listf;
    ...
    dpt_web_version_    varchar2(99);
    dpt_usage_mode_     char(2);
...
*/
    begin
      select dpt_web.body_version
      into   dpt_web_version_
      from   dual;
    exception when others then
      dpt_web_version_ := 'version 00.00';
    end;

    if substr(dpt_web_version_,instr(dpt_web_version_,'version')+8,5)>'45.68' then
      dpt_usage_mode_ := 'MI';
    else
      dpt_usage_mode_ := 'IO';
    end if;
...
--          bars_audit.info('create_dpstsAS - before dpt_web.p_open_vklad_rnk');
            dpt_web.p_open_vklad_rnk(p_usagemode        => dpt_usage_mode_,
...
--               cnt_>0 and date_>datb_ then
              if fl_dubl_>0                                                         and
                 ...
                 date_>datb_ then
                update accounts
                set    nlsalt=trim(substr(nsc_,1,15)),
                       mdate=date_
                where  acc=accdepo_;
              else
                update accounts
...
--    execute immediate 'delete from dpt_techaccounts   where dpt_id     in (select dpt from ASVO_TEMP_dpt_migr)';
      ...
      execute immediate 'delete from int_accn_update     where acc in (select acc from ASVO_TEMP_acc_migr)';
...
/
 ...
 PROMPT ===================================================================================== 
 PROMPT *** End *** ========== Scripts /Sql/BARS/package/migraas.sql =========*** End *** ===
 PROMPT ===================================================================================== 
