 ...
 PROMPT ===================================================================================== 
 PROMPT *** Run *** ========== Scripts /Sql/BARS/package/acrn_otc.sql =========*** Run *** ==
 PROMPT ===================================================================================== 
 
  CREATE OR REPLACE PACKAGE BARS.ACRN_OTC 
IS

G_HEADER_VERSION  CONSTANT VARCHAR2(64)  := 'версия 5.12 22/12/2016';
...
END;
/
CREATE OR REPLACE PACKAGE BODY BARS.ACRN_OTC 
IS
  g_body_version constant varchar2(64) := 'версия 5.6 13/03/2017';
  acc_FORM int := null;
...
--  В моді2 тільки змінюємо залишки
         IF mode_=2 AND nbs_ LIKE '8%' AND accc_ IS NULL THEN
            ...
            acrn_otc.p_int(acc_,id_,dat1_,dat2_,acrd_,NULL,0);
...
--              ORDER BY dat,otm;
    ...
    c_label        constant varchar2(128)  := 'acrn_otc.p_int';
...
BEGIN
  ...
  acrn_otc.SumO  := 0;  -- Сумма остатков
  acrn_otc.SumOP := 0;  -- Сумма остатков на процент
  acrn_otc.SumOD := 0;  -- Количество дней сумма остатков
  ...
  WHILE dat1_<=TRUNC(dt2_) LOOP
    ...
    LOOP
       ...
       if mode_ = 2 then  -- 06.08.2012  Для игрового режима без ХО-ХО
        ...
        IF acc.basey=0 or
          (acc.basey=4 and acrn_otc.cur_Basey is null) THEN  -- факт/факт ГОДОВАЯ % ставка
        ...
        IF acc.metr = 3 THEN
           bars_audit.trace ('acrn_otc.p_int: metr = 3, acc = %s, %s - %s', to_char(acc_),
           ...
           BEGIN
             ...
             bars_audit.trace ('acrn_otc.p_int: дата посл.начисления = %s', to_char(l_datacr,'dd.mm.yyyy'));
             l_datend := least (dat2_, l_datacr);
             bars_audit.trace ('acrn_otc.p_int: дата по* = %s', to_char(l_datend,'dd.mm.yyyy'));
             FOR sal IN c_amo LOOP
                 ...
                 IF (id_ = 0 AND sal.ostf < 0 AND l_datacr >= dat1_) THEN
                     ...
                     bars_audit.trace ('acrn_otc.p_int: ost = %s, delta = %s, acrd = %s, cnt = %s',
        ...
        --
          ...
          bars_audit.trace ('acrn_otc.p_int(4): acc/id => %s/%s, saltype => %s, term => %s-%s',
          ...
          loop
              ...
              bars_audit.trace ('acrn_otc.p_int(4): saldo = %s, mdate=>%s',
              ...
              bars_audit.trace ('acrn_otc.p_int(4): "till" date = %s', to_char(dat2_,'dd.mm.yyyy'));
             ...
             bars_audit.trace ('acrn_otc.p_int(4): intamount(sal %s, %s/%s days) = %s',
...
-- Collect balance history
        ...
        IF ost_ IS NULL THEN
           ...
           ELSIF acc.io IN (2,3) THEN
              ...
              SELECT fdat, acrn_otc.ho_ost(acc_,fdat,-dos+kos,rownum),dos,kos
                FROM (
...
---------------acrn_otc.cur_Nomin
               If Nvl(acrn_otc.cur_Nomin,0) > 0 then
                  KOL_  := Abs(ostf_) / acrn_otc.cur_Nomin;
                  acrd_ :=
                   ( ir_ * Sign(ostf_)*acrn_otc.cur_Nomin + ostp_ )
                         * dlta_/b_yea/100;
               else
                  KOL_  := 1;
               ...
               IF acrd_ <> 0 or acc.metr IN (1,2) THEN
                  ...
                  acrn_otc.SumO  := acrn_otc.SumO  + osts_;
                  acrn_otc.SumOP := acrn_otc.SumOP + osts_ * (ir_ + br_);
                  acrn_otc.SumOD := acrn_otc.SumOD + dlta_;
                  acr_ := acr_ + acrd_;
               END IF;
            END IF;
...
IS
BEGIN
   RETURN acrn_otc.FPROCN(acc_,NULL,datp_);
END FPROC;
...
BEGIN
   ...
   BEGIN
      IF id_ IS NULL THEN
         SELECT a.kv,fost(a.acc, dat_),p.pap,a.pap
           INTO kv_, ostc_, ppap_, apap_
           FROM accounts a, ps p
          WHERE a.nbs=p.nbs AND a.acc=acc_;
...
/
 ...
 PROMPT ===================================================================================== 
 PROMPT *** End *** ========== Scripts /Sql/BARS/package/acrn_otc.sql =========*** End *** ==
 PROMPT ===================================================================================== 
