...
PROMPT *** Create  view V_WEB_USERMAP ***
  ...
  CREATE OR REPLACE FORCE VIEW BARS.V_WEB_USERMAP ("WEBUSER", "LOWER_WEBUSER", "DBUSER", "ERRMODE", "WEBPASS", "ADMINPASS", "COMM", "CHGDATE", "BLOCKED", "ATTEMPTS", "USER_ID", "SHARED_USER", "LOG_LEVEL", "CHANGE_DATE", "BANK_DATE") AS 
  with conf_val as
(
...
SELECT m.webuser,
  ...
  FROM (
        select webuser, lower(webuser) lower_webuser,
               dbuser, errmode,webpass, adminpass, comm, chgdate, blocked, attempts
        from web_usermap
        union
        select lower(sau.active_directory_name),lower(sau.active_directory_name) lower_webuser,
               s.logname, 1, 'b1b3773a05c0ed0176787a4f1574ff0075f7521e', '', 'AD user id=' || s.id, sysdate, 0,0
        from STAFF_AD_USER sau, staff$base s
        where sau.user_id=s.id
        ) m,
       staff$base s,
       staff_templates t,
