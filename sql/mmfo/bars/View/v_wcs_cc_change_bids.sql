...
PROMPT *** Create  view V_WCS_CC_CHANGE_BIDS ***
  ...
  from v_wcs_bids b, wcs_bid_states bs, wcs_srv_hierarchy sh, staff$base sb
 where b.bid_id = bs.bid_id
   and bs.state_id in
       ('NEW_SECRETARYCC', 'NEW_RU_SECRETARYCC', 'NEW_CA_SECRETARYCC')
   and bs.checkouted = 1
   and sh.id = case
         when bs.state_id = 'NEW_SECRETARYCC' then
          'TOBO'
         when bs.state_id = 'NEW_RU_SECRETARYCC' then
          'RU'
         when bs.state_id = 'NEW_CA_SECRETARYCC' then
          'CA'
       end
   and bs.checkout_user_id = sb.id
   and sb.branch like sys_context('bars_context', 'user_branch_mask');

PROMPT *** Create  grants  V_WCS_CC_CHANGE_BIDS ***
grant SELECT                                                                 on V_WCS_CC_CHANGE_BIDS to BARSREADER_ROLE;
grant SELECT                                                                 on V_WCS_CC_CHANGE_BIDS to BARS_ACCESS_DEFROLE;
grant SELECT                                                                 on V_WCS_CC_CHANGE_BIDS to UPLD;



PROMPT ===================================================================================== 
PROMPT *** End *** ========== Scripts /Sql/BARS/View/V_WCS_CC_CHANGE_BIDS.sql =========*** E
PROMPT ===================================================================================== 
