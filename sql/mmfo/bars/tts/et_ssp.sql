...
begin
  ...
  begin
    ...
    values ('SSP', 'SSP - Компенсаційні виплати', 1, null, null, '#(bpk_get_transit(''1X'',#(NLSA),#(NLSB),#(KVA)))', null, null, null, null, null, 0, 1, 0, 0, null, null, null, null, null, null, '0200000000000000000000000001000000010000000000000000000000000000', null);
  exception
  ...
  begin
    ...
    values (23, 'SSP', 1, null, null, 3);
  exception
    when dup_val_on_index then null;
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (chklist_tts: 23, ''SSP'', 1, null, null, 3) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  begin
