...
begin
  ...
  begin
    insert into tts(tt, name, dk, nlsm, kv, nlsk, kvk, nlss, nlsa, nlsb, mfob, flc, fli, flv, flr, s, s2, sk, proc, s3800, rang, flags, nazn)
    values ('DU7', 'Виплата відсотків в ін.валюті (міжбанк)', 1, null, null, '#(case sys_context(''bars_context'',''user_mfo'') when ''300465'' then #(NLSB) else get_proc_nls(''T00'',#(KVA)) end)', null, null, null, '191992', '300465', 1, 2, 0, 0, null, null, null, null, null, 9, '0200100000000000000000000000000000010000000000000000000000000000', 'Відсотки по депозиту згідно #{DPU.F_NAZN(''U'',#(ND))}');
  exception
    when dup_val_on_index then 
      update tts
  ...
  exception
    ...
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (op_rules: ''NOS_A'', ''DU7'', ''M'', 0, null, ''0'', null) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  begin
  ...
  exception
    ...
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (chklist_tts: 96, ''DU7'', 3, null, ''sys_context(''''bars_context'''',''''user_mfo'''') <> ''''300465'''''', null) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  --------------------------------
