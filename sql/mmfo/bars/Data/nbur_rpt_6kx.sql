...
begin
  ...
  , p_scm_code         => 'C'
  , p_file_tp          => 1
  , p_file_nm          => '(xml) 6KX Дані для розрахунку коефіцієнта покриття ліквідністю (LCR)'
  , p_file_fmt         => 'XML'
  , p_scm_num          => '03'
  , p_unit_code        => '01'
  , p_period_tp        => 'D'
  ...
  , p_cnsl_tp          => '4'
  , p_val_tp_ind       => 'N'
  , p_view_nm          => 'V_NBUR_6KX'
  , p_f_turns          => 0
  );
  
  l_file_id := nvl(l_file_id, 35475);
  ...
  ( p_proc_id => l_proc_id
   ...
   , p_proc_active => 'Y'
   , p_scheme => 'BARS'
   , p_proc_name => 'NBUR_P_F6KX_NC'
   , p_description => '(xml) 6KX Дані для розрахунку коефіцієнта покриття ліквідністю (LCR)'
   , p_version => '1.0'
...
begin
    delete from NBUR_REF_PREPARE_XML where file_code = '#6K';
    Insert into NBUR_REF_PREPARE_XML
       (FILE_CODE, DESC_XML, DATE_START, ATTR_NIL)
     Values
       ('#6K', 'select substr(t.FIELD_CODE, 1, 6) as EKP
    ...
    where  report_date = :p_rpt_dt
          and kf = :p_kf
          and report_code = ''#6K''
    order by FIELD_CODE', TO_DATE('01/01/2018 00:00:00', 'MM/DD/YYYY HH24:MI:SS'), NULL);
    COMMIT;
