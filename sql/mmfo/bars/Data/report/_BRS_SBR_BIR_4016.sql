...
begin     
   ...
   exception when no_data_found then                       
    ...
    l_zpr.create_stmt  := '';
    l_zpr.rpt_template := 'zay_4016.frx';
    l_zpr.form_proc    := '';
    ...
    l_zpr.txt          := 'SELECT z.nd num_doc,'||nlchr||
                           ...
                           '         case when (select kv_base from zay_conv_kv where (kv1 = z.kv2 and kv2 = z.kv_conv) or (kv2 = z.kv2 and kv1 = z.kv_conv)) = z.kv_conv then'||nlchr||
                           '           round (z.s2 / 100 / z.kurs_f, 2)'||nlchr||
                           '         else'||nlchr||
                           '           round (z.s2 / 100 * z.kurs_f, 2)'||nlchr||
                           '         end  base_amount_in,'||nlchr||
