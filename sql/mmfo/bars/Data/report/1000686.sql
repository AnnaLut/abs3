...
'||nlchr||
    ...
    l_zpr.txt          := 'SELECT dat_now, DAT_FIRST,
       ...
       ad.kv, ad.daos as dat_begin, ad.mdate as dat_end,
       ...
       ai.nbs as nbs_int, ai.ob22 as ob22_int,
       to_char(fostQ(ai.acc,f.dat_last)/100,''FM9999999990D00'') as ost_int,
       0 as dsk_prm,
 ...
 where ((ad.nbs in (''2525'',''2546'',''2610'',''2650'',''2651'')) or
        (ad.nbs = ''2600'' and ad.ob22 not in(''01'',''02'',''10'')))
   and ad.mdate is not null
   and ad.acc  = i.acc
   and i.id   = 1
   and i.acra = ai.acc
   and ad.rnk  = c.rnk
   and ad.acc = s.acc(+)
   and ad.daos <= f.dat_last
   and (ad.dazs is null OR ad.dazs > f.dat_last)';
    l_zpr.xsl_data     := '';
    l_zpr.xsd_data     := '';
