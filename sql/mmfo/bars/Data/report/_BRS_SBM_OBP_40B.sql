...
PROMPT ===================================================================================== 
PROMPT *** Run *** ========== Scripts /Sql/Bars/Data/_BRS_SBM_OBP_40B.sql =========*** Run *
PROMPT ===================================================================================== 
prompt ===================================== 
prompt == Выписка ДСС все way4
prompt ===================================== 
...
begin     
   l_zpr.name := 'Выписка ДСС все way4';
   l_zpr.pkey := '\BRS\SBM\OBP\40B';
   ...
   exception when no_data_found then                       
    ...
    l_zpr.name         := 'Выписка ДСС все way4';
    l_zpr.namef        := '';
    l_zpr.bindvars     := ':sFdat1='''',:sFdat2='''',:p_rnk=''RNK комп.'',:ob22=''OB22'',:nbs=''NBS'',:kv=''KV''';
    l_zpr.create_stmt  := '';
    l_zpr.rpt_template := 'dss.frx';
    l_zpr.form_proc    := '';
    l_zpr.default_vars := ':p_rnk=''31609'',:nbs=''%'',:ob22=''%'',:kv=''%''';
    l_zpr.bind_sql     := '';
  ...
  left join accountsw aw
   ...
   and aw.tag=''W4_CPN''  --имя держателя карты
  left join operw ow
    on o.ref = ow.ref
   and ow.tag = ''OW_LD''  --дата транзакции OW, дата платеже во внешней системе
  LEFT JOIN operw ow2
    ON ow.ref=ow2.ref AND ow2.tag=''OW_DS''
  join accounts acc 
    on acc.acc=w4a.acc_pk
 where w4a.rnk=:p_rnk
   and w4a.daos_pk<=:sFdat2
   and (w4a.dazs_pk is null or w4a.dazs_pk>=:sFdat2)
   and (:ob22 = ''%'' or (regexp_instr(:ob22, ''(^|\D)''||acc.ob22||''(\D|$)'') > 0))
   and (:nbs = ''%''or (regexp_instr(:nbs, ''(^|\D)''||acc.NBS||''(\D|$)'') > 0))
   and (:kv = ''%''or (regexp_instr(:kv, ''(^|\D)''||acc.kv||''(\D|$)'') > 0))
  order by w4a.acc_pk,fdat ';
    ...
    l_rep.name        :='Empty';
    l_rep.description :='Выписка ДСС все way4';
...
exec umu.add_report2arm(4022,'$RM_DRU1');
commit;


PROMPT ===================================================================================== 
PROMPT *** End *** ========== Scripts /Sql/Bars/Data/_BRS_SBM_OBP_40B.sql =========*** End *
PROMPT ===================================================================================== 
