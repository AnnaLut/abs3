prompt ===================================== 
prompt == Сума резерву за позабал. по фіз. особам
prompt ===================================== 
...
begin     
   l_zpr.name := 'Сума резерву за позабал. по фіз. особам';
   l_zpr.pkey := '\BRS\SBM\REZ\19';
   ...
   exception when no_data_found then                       
    ...
    l_zpr.txt          := 'select branch "Відділення",
       ...
       sum(sm_2+sm_3+sm_4+sm_5)/100  "РАЗОМ_НЕСТАНДартні",
       sum(sm_1+sm_2+sm_3+sm_4+sm_5)/100  "ВСЬОГО"
from(           
    select b.branch,
           b.name br_name,
           sum(decode(t.s080,''1'',t.sm,0)) sm_1,
           sum(decode(t.s080,''2'',t.sm,0)) sm_2,
           sum(decode(t.s080,''3'',t.sm,0)) sm_3,
           sum(decode(t.s080,''4'',t.sm,0)) sm_4,
           sum(decode(t.s080,''5'',t.sm,0)) sm_5
    from
    ( select r.tobo branch,  r.s080, r.kv,
        ...
        from tmp_rez_risk r
        where dat = to_date(:sFdat,''dd.mm.yyyy'') and  id = user_id
