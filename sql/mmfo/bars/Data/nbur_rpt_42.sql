...
begin
    delete from BARS.NBUR_REF_PREPARE_XML WHERE FILE_CODE = '42X'; 
    Insert into NBUR_REF_PREPARE_XML
       (FILE_CODE, DESC_XML, DATE_START)
     Values
       ('42X', 'select EKP, F099, Q003_4, abs(sum(T070)) as T070
from nbur_log_f42X 
where report_date = :p_rpt_dt
           and kf = :p_kf    
group by EKP, F099, Q003_4
having  sum(T070)<>0
order by 1, 2,3', TO_DATE('01/01/2018 00:00:00', 'MM/DD/YYYY HH24:MI:SS'));
    COMMIT;
end;
/