...
begin
  ...
  l_tabsemantic := 'Фін.клас по ОКПО';
  l_tablinesdef := '30';
  l_tabselect_statement := 'SELECT x.*, (Select nmk from customer where rnk = x.rnk ) NMK FROM FIN_RNK_OKPO x ';
  l_tabname     := 'FIN_RNK_OKPO';
  ...
  bars_metabase.add_column(l_tabid, 1, 'FIN', 'N', 'Фін.клас', '', 22, 3, 0, 1, 0, 1, '', '', 0, 0, '', 0, 0, 0, '', '', 0);
  bars_metabase.add_column(l_tabid, 2, 'RNK', 'N', 'РНК', '', 22, 1, 0, 1, 0, 1, '', '', 0, 0, '', 0, 0, 0, '', '', 0);
  bars_metabase.add_column(l_tabid, 3, 'NMK', 'N', 'Назва клієнта', '', 22, 2, 0, 1, 0, 1, '', '', 0, 0, '', 1, 0, 0, '', '', 0);
  bars_metabase.add_column(l_tabid, 4, 'OKPO', 'C', 'ОКПО', '10', 70, 4, 0, 1, 0, 1, '', '', 0, 0, '', 0, 0, 0, '', '', 0);

  -- добавляем описание процедур
  bars_metabase.add_actiontbl(l_tabid, 'DELETE', 'P_FIN_RNK_OKPO_CORR(3,:FIN,:RNK,:OKPO)');

  -- добавляем описание процедур
  bars_metabase.add_actiontbl(l_tabid, 'INSERT', 'P_FIN_RNK_OKPO_CORR(1,:FIN,:RNK,:OKPO)');

  -- добавляем описание процедур
  bars_metabase.add_actiontbl(l_tabid, 'UPDATE', 'P_FIN_RNK_OKPO_CORR(2,:FIN,:RNK,:OKPO)');
