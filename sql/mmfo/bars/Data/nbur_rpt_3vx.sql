...
BEGIN 
  ...
  , p_file_nm          => '3VX - Дані про боржників банку'
  , p_file_fmt         => 'XML'
  ...
  ( p_proc_id => l_proc_id
   ...
   , p_proc_active => 'Y'
   , p_scheme => 'BARS'
   , p_proc_name => 'NBUR_P_F3VX_NC'
   , p_description => '3V XML-формат'
   , p_version => 'v.18.001'
...
begin
    delete from BARS.NBUR_REF_PREPARE_XML WHERE FILE_CODE = '3VX'; 
    Insert into NBUR_REF_PREPARE_XML
       ( DATE_START, FILE_CODE, DESC_XML, ATTR_NIL )
     Values
       (TO_DATE('01/01/2017 00:00:00', 'MM/DD/YYYY HH24:MI:SS'), '3VX',
'select  EKP, F059, K111, K031, F063, 
	 F064, S190, F073, F003, Q001, 
	 K020, Q026, rtrim(TO_CHAR(SUM(T100),'''||'fm0.999'||'''),'''||'.'||''') as T100
    from nbur_log_f3VX t
   where report_date = :p_rpt_dt and
                  kf = :p_kf
group by EKP, F059, K111, K031, F063, 
	 F064, S190, F073, F003, Q001, 
	 K020, Q026
--  having sum(T100) <> 0
order by K020, F059, K111',null );
