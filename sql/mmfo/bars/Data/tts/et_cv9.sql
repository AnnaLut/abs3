...
prompt Наименование операции: CV9-Конвертацiя безготiвк. БМ за операцiями купiвлi-продажу
declare
  cnt_  number;
begin
  --------------------------------
  ...
  begin
    ...
    values ('CV9', 'CV9-Конвертацiя безготiвк. БМ за операцiями купiвлi-продажу', 1, null, null, null, null, null, null, null, null, 0, 0, 1, 0, null, null, null, null, '#(nbs_ob22 (''3800'',''09''))', null, '1000100000000000000000000000000000010000000000000000000000010000', 'Конвертація безготiвк. БМ за операцiями купiвлi-продажу');
  exception
    when dup_val_on_index then 
      update tts
         set tt='CV9', name='CV9-Конвертацiя безготiвк. БМ за операцiями купiвлi-продажу', dk=1, nlsm=null, kv=null, nlsk=null, kvk=null, nlss=null, nlsa=null, nlsb=null, mfob=null, flc=0, fli=0, flv=1, flr=0, s=null, s2=null, sk=null, proc=null, s3800='#(nbs_ob22 (''3800'',''09''))', rang=null, flags='1000100000000000000000000000000000010000000000000000000000010000', nazn='Конвертація безготiвк. БМ за операцiями купiвлi-продажу'
       where tt='CV9';
  end;
  --------------------------------
  ----------- Реквизиты ----------
  ...
  exception
    ...
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (ps_tts: ''3640'', ''CV9'', 0) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  begin
  ...
  exception
    ...
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (ps_tts: ''3640'', ''CV9'', 1) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  --------------------------------
