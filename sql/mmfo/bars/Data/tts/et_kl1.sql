...
prompt Наименование операции: KL1 Внут.М/В платежi по "Клiєнт-Банк"
declare
  cnt_  number;
begin
  --------------------------------
  ...
  begin
    insert into tts(tt, name, dk, nlsm, kv, nlsk, kvk, nlss, nlsa, nlsb, mfob, flc, fli, flv, flr, s, s2, sk, proc, s3800, rang, flags, nazn)
    values ('KL1', 'KL1 Внут.М/В платежi по "Клiєнт-Банк"', 1, null, null, '#(bpk_get_transit5(''19'',#(NLSB),#(KVB),#(REF)))', null, null, null, null, null, 1, 0, 0, 0, null, null, null, null, null, null, '0100100000000000000000000001000000000000000000000000000000000000', null);
  exception
    when dup_val_on_index then 
      update tts
         set tt='KL1', name='KL1 Внут.М/В платежi по "Клiєнт-Банк"', dk=1, nlsm=null, kv=null, nlsk='#(bpk_get_transit5(''19'',#(NLSB),#(KVB),#(REF)))', kvk=null, nlss=null, nlsa=null, nlsb=null, mfob=null, flc=1, fli=0, flv=0, flr=0, s=null, s2=null, sk=null, proc=null, s3800=null, rang=null, flags='0100100000000000000000000001000000000000000000000000000000000000', nazn=null
       where tt='KL1';
  end;
  --------------------------------
  ----------- Реквизиты ----------
