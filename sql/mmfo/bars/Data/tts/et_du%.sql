...
begin
  ...
  begin
    insert into tts(tt, name, dk, nlsm, kv, nlsk, kvk, nlss, nlsa, nlsb, mfob, flc, fli, flv, flr, s, s2, sk, proc, s3800, rang, flags, nazn)
    values ('DU%', 'DU% Нарахування відсотків', 1, '#(dpu.get_nls4pay(-2,#(NLSA),#(KVA)))', null, null, null, null, null, null, null, 1, 0, 1, 0, null, null, null, null, '#(nbs_ob22_nls(''3800'',''03'',null))', 0, '0000100000000000000000000000000000000100000000000000000000000000', '#{DPU.GET_INTDETAILS(#(NLS),#(KV))} за пер_од з #(DAT1) по #(DAT2) вкл.');
  exception
    when dup_val_on_index then 
      update tts
