...
prompt Наименование операции: CDP (доч.CAP) Комісія агента за прийом переказу по сист."RIA"(Інші кра
declare
  cnt_  number;
begin
  --------------------------------
...
prompt Наименование операции: K54 (доч.CAP) Комісія за прийом переказу по системі "RIA"(Інші країни)
declare
  cnt_  number;
begin
  --------------------------------
...
begin
  ...
  exception
    ...
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (op_rules: ''REZID'', ''CAP'', ''M'', 1, 6, null, null) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  --------------------------------
  ...
  exception
    ...
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (chklist_tts: 1, ''CAP'', 2, null, ''f_universal_box2(USERID)=1'', null) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  begin
