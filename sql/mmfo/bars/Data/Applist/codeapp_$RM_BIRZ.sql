PROMPT ===================================================================================== 
PROMPT *** Run *** ========== Scripts /Sql/Bars/Data/Applist/codeapp_$RM_BIRZ.sql =========*
PROMPT ===================================================================================== 


PROMPT *** Create/replace  ARM  $RM_BIRZ ***
  declare
    l_application_code varchar2(10 char) := '$RM_BIRZ';
	...
	d integer := 0;
begin
     DBMS_OUTPUT.PUT_LINE(' $RM_BIRZ створюємо (або оновлюємо) АРМ АРМ Біржеві операції (ЗАГАЛЬНИЙ) ');
    ...
    DBMS_OUTPUT.PUT_LINE( chr(13)||chr(10)||' ********** Створюємо функцію ZAYP. Перекриття сальдо торгових рахунків ********** ');
          --  Створюємо функцію ZAYP. Перекриття сальдо торгових рахунків
      l := l +1;
      l_function_ids.extend(l);
      l_function_ids(l)   :=   abs_utils.add_func(
                                                  p_name     => 'ZAYP. Перекриття сальдо торгових рахунків',
                                                  p_funcname => '/barsroot/barsweb/dynform.aspx?form=frm_zay_p',
                                                  p_rolename => 'BARS_ACCESS_DEFROLE' ,
                                                  p_frontend => l_application_type_id
                                                  );
    ...
    DBMS_OUTPUT.PUT_LINE( chr(13)||chr(10)||' ********** Створюємо функцію ZAYB. Повернення прострочених заявок на купівлю-продаж валюти ********** ');
          --  Створюємо функцію ZAYB. Повернення прострочених заявок на купівлю-продаж валюти
      l := l +1;
      l_function_ids.extend(l);
      l_function_ids(l)   :=   abs_utils.add_func(
                                                  p_name     => 'ZAYB. Повернення прострочених заявок на купівлю-продаж валюти',
                                                  p_funcname => '/barsroot/ndi/referencebook/GetRefBookData/?accessCode=1&sPar=[PROC=>p_zay_datt_check(0)]',
                                                  p_rolename => 'BARS_ACCESS_DEFROLE' ,
                                                  p_frontend => l_application_type_id
                                                  );
   ...
   DBMS_OUTPUT.PUT_LINE(chr(13)||chr(10)||'  Прикріпляємо ресурси функцій до даного АРМу ($RM_BIRZ) - АРМ Біржеві операції (ЗАГАЛЬНИЙ)  ');
   ...
   commit;
umu.add_report2arm(288,'$RM_BIRZ');
umu.add_report2arm(4012,'$RM_BIRZ');
umu.add_report2arm(4015,'$RM_BIRZ');
umu.add_report2arm(4016,'$RM_BIRZ');
commit;
end;
/
...
PROMPT ===================================================================================== 
PROMPT *** End *** ========== Scripts /Sql/Bars/Data/Applist/codeapp$RM_BIRZ.sql =========**
PROMPT ===================================================================================== 
