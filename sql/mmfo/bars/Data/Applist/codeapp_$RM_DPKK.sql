...
begin
    ...
    DBMS_OUTPUT.PUT_LINE( chr(13)||chr(10)||' ********** Створюємо функцію DPT. Архів депозитів ФО ********** ');
      ...
      l_function_ids(l)   :=   abs_utils.add_func(
                                                  ...
                                                  p_funcname => '/barsroot/ndi/referencebook/GetRefBookData/?tableName=V_DPT_ARCHIVE&accessCode=1&sPar=[PAR=>:A(SEM=Дата,TYPE=D),:B(SEM=Вид депозиту-пусто-всі,TYPE=N,REF=DPT_VIDD),:C(SEM=Код підрозділу-пусто-всі,TYPE=С,REF=OUR_BRANCH)][PROC=>DPT_RPT_UTIL.SET_ARCHV_CD(:A,:B,:C)][EXEC=>BEFORE][CONDITIONS=> VIDD_ID = nvl(DPT_RPT_UTIL.GET_VIDD_CD, VIDD_ID )  and BRANCH = nvl(DPT_RPT_UTIL.GET_BRANCH_CD, BRANCH)]',
                                                  p_rolename => '' ,    
                                                  p_frontend => l_application_type_id
                                                  );
