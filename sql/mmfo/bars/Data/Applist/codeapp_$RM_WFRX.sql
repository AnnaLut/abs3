PROMPT ===================================================================================== 
PROMPT *** Run *** ========== Scripts /Sql/Bars/Data/Applist/codeapp_$RM_WFRX.sql =========*
PROMPT ===================================================================================== 


PROMPT *** Create/replace  ARM  $RM_WFRX ***
  declare
    l_application_code varchar2(10 char) := '$RM_WFRX';
    l_application_name varchar2(300 char) := 'АРМ Forex';
	...
	d integer := 0;
begin
     DBMS_OUTPUT.PUT_LINE(' $RM_WFRX створюємо (або оновлюємо) АРМ АРМ Forex ');
    ...
    DBMS_OUTPUT.PUT_LINE( chr(13)||chr(10)||' ********** Створюємо функцію N 3. Фін.результат за період по похідним фін.інструментам ********** ');
      ...
      l_function_ids(l)   :=   abs_utils.add_func(
                                                  ...
                                                  p_funcname => '/barsroot/ndi/referencebook/GetRefBookData/?tableName=V_FOREX_ANI34&accessCode=2&sPar=[PROC=>P_ANI34(34,:B,:E,:R)][PAR=>:B(SEM=Дата_з,TYPE=D),:E(SEM=Дата_по,TYPE=D),:R(SEM=Вн_№/0,TYPE=N)][EXEC=>BEFORE]',
                                                  p_rolename => '' ,
                                                  p_frontend => l_application_type_id
                                                  );


    DBMS_OUTPUT.PUT_LINE( chr(13)||chr(10)||' ********** Створюємо функцію N 3`. Фінансовий результат за період по "коротким" Форекс-угодам ********** ');
          --  Створюємо функцію N 3`. Фінансовий результат за період по "коротким" Форекс-угодам
      l := l +1;
      l_function_ids.extend(l);
      l_function_ids(l)   :=   abs_utils.add_func(
                                                  p_name     => 'N 3`. Фінансовий результат за період по "коротким" Форекс-угодам',
                                                  p_funcname => '/barsroot/ndi/referencebook/GetRefBookData/?tableName=V_FOREX_ANI35&accessCode=1&sPar=[PROC=>P_ANI34(35,:B,:E,:R)][PAR=>:B(SEM=Дата_з,TYPE=D),:E(SEM=Дата_по,TYPE=D),:R(SEM=Вн_Ном/0,TYPE=N)][EXEC=>BEFORE]',
                                                  p_rolename => 'BARS_ACCESS_DEFROLE' ,
                                                  p_frontend => l_application_type_id
                                                  );
    ...
    DBMS_OUTPUT.PUT_LINE( chr(13)||chr(10)||' ********** Створюємо функцію SALDO_DSW: Таблиця СТАРТ-даних по ДЕПО-СВОПАМ ********** ');
      ...
      l_function_ids(l)   :=   abs_utils.add_func(
                                                  p_name     => 'SALDO_DSW: Таблиця СТАРТ-даних по ДЕПО-СВОПАМ',
                                                  p_funcname => '/barsroot/ndi/referencebook/GetRefBookData/?tableName=V_SALDO_DSW&accessCode=2',
                                                  p_rolename => 'bars_access_defrole' ,
                                                  p_frontend => l_application_type_id
                                                  );
   ...
   DBMS_OUTPUT.PUT_LINE(chr(13)||chr(10)||'  Прикріпляємо ресурси функцій до даного АРМу ($RM_WFRX) - АРМ Forex  ');
   ...
   commit;
umu.add_report2arm(772,'$RM_WFRX');
umu.add_report2arm(773,'$RM_WFRX');
umu.add_report2arm(774,'$RM_WFRX');
umu.add_report2arm(775,'$RM_WFRX');
commit;
end;
/
...
PROMPT ===================================================================================== 
PROMPT *** End *** ========== Scripts /Sql/Bars/Data/Applist/codeapp$RM_WFRX.sql =========**
PROMPT ===================================================================================== 
