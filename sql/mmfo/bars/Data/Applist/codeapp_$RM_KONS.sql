...
begin
    ...
    DBMS_OUTPUT.PUT_LINE( chr(13)||chr(10)||' ********** Створюємо функцію Макети юридичних осіб ********** ');
    ...
    DBMS_OUTPUT.PUT_LINE( chr(13)||chr(10)||' ********** Створюємо функцію Рах 2600 (Крим) для  перенесення на 2903 та закриття ********** ');
          --  Створюємо функцію Рах 2600 (Крим) для  перенесення на 2903 та закриття
      l := l +1;
      l_function_ids.extend(l);
      l_function_ids(l)   :=   abs_utils.add_func(
                                                  p_name     => 'Рах 2600 (Крим) для  перенесення на 2903 та закриття',
                                                  p_funcname => '/barsroot/ndi/referencebook/GetRefBookData/?accessCode=2&sPar=V_NLS_KRM[NSIFUNCTION][showDialogWindow=>false][EDIT_MODE=>MULTI_EDIT]',
                                                  p_rolename => '' ,
                                                  p_frontend => l_application_type_id
                                                  );
   ...
   commit;
umu.add_report2arm(5060,'$RM_KONS');
commit;
end;
/
