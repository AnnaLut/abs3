...
begin     
   l_reports.name := 'Ф601 Частина 4 Інформація про боржника  - ЮО';
   l_reports.id := '6008';
 ...
 {"Name":"p_sFdat2","Type":"Date","Value":"","Label":"По"}]';
    ...
    l_reports.SQLPREPARE          := 'select '||nlchr||
                           ' pu.rnk rnk,'||nlchr||
                           ' pu.NAMEUR Найменування,'||nlchr||
                           ' pu.CODEDRPOU Ідентифікаційний_код,'||nlchr||
                           ' pu.ISREZ Резидентність,'||nlchr||
                           ' pu.COUNTRYCODNEREZ Код_країни_реєстрац_нериз,'||nlchr||
                           ' pu.REGISTRYDAY || '' '' || pu.NUMBERREGISTRY as Дата_та_номер_держ_реєстр,'||nlchr||
                           ' pu.K110 Вид_економічної_діяльності,'||nlchr||
                           ' pu.EC_YEAR Період_визн_вид_економ_діяльн,'||nlchr||
                           ' uo.SALES Показн_діяльн_SALES,'||nlchr||
                           ' uo.EBIT Показн_діяльн_EBIT,'||nlchr||
                           ' uo.EBITDA Показн_діяльн_EBITDA,'||nlchr||
                           ' uo.TOTALDEBT Показн_діяльн_TOTAL_NET_DEBT,'||nlchr||
                           ' gu.WHOIS Приналеж_до_груп_спільн_контр,'||nlchr||
                           ' gu.NAMEURGR Перелік_з_групи_спільн_контр,'||nlchr||
                           ' gu.CODEDRPOUGR Коди_ЄДРПОУ_під_спіл_контрол,'||nlchr||
                           ' pu.ISCONTROLLER Статус_участі_в_групі,'||nlchr||
                           ' uos.SALESGR Показн_діяльн_повяз_груп_SALES,'||nlchr||
                           ' uos.EBITGR Показн_діяльн_повяз_груп_EBIT,'||nlchr||
                           ' uos.EBITDAGR Показн_діяльн_пов_груп_EBITDA,'||nlchr||
                           ' uos.TOTALDEBTGR Показн_діяльн_п_груп_TOTAL_N_D,'||nlchr||
                           ' pru.NAMEURPR Перел_до_груп_з_спіл_еконм_риз,'||nlchr||
                           ' pru.CODEDRPOUPR Коди_ЄДРПОУ_з_спіл_еконм_риз,'||nlchr||
                           ' FINU.SALES Пока_діяльн_груп_SALES,'||nlchr||
                           ' FINU.EBIT Пока_діяльн_груп_EBIT,'||nlchr||
                           ' FINU.EBITDA Пока_діяльн_груп_EBITDA,'||nlchr||
                           ' FINU.TOTALDEBT Пока_діяльн_груп_TOTAL_N_D,'||nlchr||
                           ' pu.ISAUDIT Факт_проход_аудиту_або_звітн,'||nlchr||
                           ' pu.K060 Код_типу_повяз_з_банк_особи,'||nlchr||
                           ' pu.ISKR Озн_використ_даних_для_КР_НБУ'||nlchr||
                           ' from'||nlchr||
                           'branch b,'||nlchr||
