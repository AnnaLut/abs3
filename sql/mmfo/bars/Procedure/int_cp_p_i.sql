...
PROMPT ===================================================================================== 
PROMPT *** Run *** ========== Scripts /Sql/BARS/Procedure/INT_CP_P_I.sql =========*** Run **
PROMPT ===================================================================================== 


PROMPT *** Create  procedure INT_CP_P_I ***

  CREATE OR REPLACE PROCEDURE BARS.INT_CP_P_I (p_metr   IN     INT, -- код методики
...
IS
   ...
   BEGIN
    ...
    l_int := 0;
     ...
     EXCEPTION
        WHEN NO_DATA_FOUND
        THEN bars_audit.info(G_TITLE|| 'По указанному АСС = %s не найдено пакетов ЦБ' || to_char(p_acc));
             RETURN;
     END;
   ...
   BEGIN
     ...
     IF p_Dat2 < p_Dat1
     THEN                                                /* явная глупость */
        bars_audit.info(G_TITLE ||' p_Dat2 < p_Dat1 : ' || to_char(p_Dat2) || '<'|| to_char(p_Dat1));
        RETURN;
     END IF;
     l_dat_begin   := p_dat1;
     l_dat_end     := p_dat2;
     ...
     THEN
        bars_audit.info(G_TITLE|| ' Не актуальний купонный период: ' || to_char(l_cprow.dok, 'dd.mm.yyyy')|| ' - ' || to_char(l_cprow.dnk, 'dd.mm.yyyy'));
        RETURN;
     END IF;
   END;
...
--------------------------------------------------------------------------------------------------------------------------------
BEGIN
  bars_audit.info(G_TITLE|| ' ===> Старт с параметрами: '
 ...
 ------------------------------------------------------------------------------------------------------
    ...
    LOOP
      ...
      LOOP
        bars_audit.info(g_title||' acc = '|| to_char(cursor_acc.acc)
           ...
           then
                ...
                then
                 if (cursor_fdat.fdat - l_dat_tmp-1) > 0
                  then
                  bars_audit.info(g_title||'  работаем по графику проспекта эмиссии: период = '|| to_char(l_dat_tmp,'dd.mm.yyyy')|| ' - ' || to_char(cursor_fdat.fdat,'dd.mm.yyyy'));
                 end if;
                    ...
                    for dates in (
                        ...
                        from (
                          SELECT greatest(dok-1,l_dat_tmp-1) d1,
                                 ...
                                 case when l_cprow.pr1_kup = 2
                                      then round(kup * ((cursor_fdat.fdat)-(dok-1))/(least(dnk, cursor_fdat.fdat) - greatest(dok-1,l_dat_tmp-1) ),0)
                                      else       kup * ((cursor_fdat.fdat)-(dok-1))/(least(dnk, cursor_fdat.fdat) - greatest(dok-1,l_dat_tmp-1) )
                                 end k1,
                                 case when l_cprow.pr1_kup = 2
                                      then round(kup * ((l_dat_tmp-1)-(dok-1))/(least(dnk, cursor_fdat.fdat) - greatest(dok-1,l_dat_tmp-1) ),0)
                                      else       kup * ((l_dat_tmp-1)-(dok-1))/(least(dnk, cursor_fdat.fdat) -greatest(dok-1,l_dat_tmp-1) )
                                 end k2,
                                 kup
                            FROM (
                     ...
                     loop
                      ...
                      bars_audit.info(G_TITLE|| '       (' ||to_char(dates.d1,'dd.mm.yyyy')|| '-'|| to_char(dates.d2,'dd.mm.yyyy')
                ...
                then
                  bars_audit.info(g_title||' работаем по процентной ставке ');
                else
                  bars_audit.info(g_title||' нет ни графика ни ставки.... ');
                  return;
                end if;

           elsif l_cprow.kv = 36
           then
              bars_audit.info(g_title||' kv = 36 !!! работаем по графику проспекта эмиссии: период = '|| to_char(l_dat_tmp,'dd.mm.yyyy')|| ' - ' || to_char(cursor_fdat.fdat,'dd.mm.yyyy'));
              for dates in ( select distinct d1, d2, real_days, k1, k2,
                              ...
                              from (SELECT greatest(dok-1,l_dat_tmp-1) d1,
                                           ...
                                           case when cursor_acc.metr = 515
                                                then round(100*(l_cprow.ir/100*365) * ((cursor_fdat.fdat)-(dok-1))/(least(dnk, cursor_fdat.fdat) - greatest(dok-1,l_dat_tmp-1)),3)
                                                when cursor_acc.metr in (8,23)
                                                then round(100*(l_cprow.ir/100*l_cprow.ky) * ((cursor_fdat.fdat)-(dok-1))/(least(dnk, cursor_fdat.fdat) - greatest(dok-1,l_dat_tmp-1)),3)
                                           end k1,
                                           case when cursor_acc.metr = 515
                                                then round(100*(l_cprow.ir/100*365) * ((l_dat_tmp-1)-(dok-1))/(least(dnk, cursor_fdat.fdat) - greatest(dok-1,l_dat_tmp-1)),3)
                                                when cursor_acc.metr in (8,23)
                                                then round(100*(l_cprow.ir/100*l_cprow.ky) * ((l_dat_tmp-1)-(dok-1))/(least(dnk, cursor_fdat.fdat) - greatest(dok-1,l_dat_tmp-1)),3)
                                           end  k2
                                      FROM CP_DAT_V d
                                     WHERE id = l_cprow.id
                                       AND least(dnk, cursor_fdat.fdat) - greatest(dok-1,l_dat_tmp-1) > 0
      ...
      END LOOP;
      p_int := p_int + l_int;
      bars_audit.info(g_title||' Cуммарно купона = ' || to_char(p_int));
    END LOOP;

    IF p_mode = 0
    THEN
      bars_audit.info(g_title||' ИГРОВОЙ РЕЖИМ Cуммарно купона = ' || to_char(p_int));
    ELSIF  p_mode = 1
    THEN
       CHECK_DATES (p_dat1,
                    p_dat2,
                    l_dat_begin,
                    l_dat_end); -- тут, если неактуальный купонный период - выкинет нахрен. В игровом режиме - нахрен
      IF p_int != 0
       THEN
        ...
        INSERT INTO acr_intN (acc, id, fdat, tdat, acrd, ir, br, osts, nazn, remi)
             VALUES (p_acc, p_id, p_dat1, p_Dat2, p_Int, l_cprow.ir, 0, l_acrrow.ost, l_nazn, 0);
      END IF;
    END IF;
   bars_audit.info(G_TITLE|| ' ==============================> Финиш '|| sqlerrm);
END;
/
show err;
...
PROMPT ===================================================================================== 
PROMPT *** End *** ========== Scripts /Sql/BARS/Procedure/INT_CP_P_I.sql =========*** End **
PROMPT ===================================================================================== 
