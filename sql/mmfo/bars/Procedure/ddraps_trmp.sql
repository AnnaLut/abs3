...
PROMPT ===================================================================================== 
PROMPT *** Run *** ========== Scripts /Sql/BARS/Procedure/DDRAPS_TRMP.sql =========*** Run *
PROMPT ===================================================================================== 


PROMPT *** Create  procedure DDRAPS_TRMP ***

  CREATE OR REPLACE PROCEDURE BARS.DDRAPS_TRMP 
( dat_    in      DATE
...
BEGIN
  ...
  THEN
    OPEN x FOR
        'select /*+ parallel(8) leading(a) */ a.KF
           ...
           join ( SELECT sa1.KF,
                    ...
                    FROM saldoa sa1,
                        ( select KF, ACC, max(FDAT) as FDAT
                            from SALDOA
                           where KF = :kf_ and 
                                 FDAT <= :1
                           group by KF, ACC
    ...
    USING dat_, dat_, dat_, l_kf, dat_,l_kf, l_kf, dat_, dat_;
  ...
  ELSE -- ( l_mode = 1 )
    ...
    OPEN x FOR
          'SELECT a.KF, a.acc, a.rnk, a.nbs, a.nls, a.kv, a.tip
            ...
            join ( select nvl(c.KF,   b.KF ) as KF,
                     ...
                     from ( select * 
                              from SNAP_BALANCES
                             where KF = :p_kf and FDAT = :1
                          ) b
                     full 
                     join ( select * 
                              from SALDOA
                             where KF = :p_kf and FDAT = :2
                          ) c 
    ...
    USING l_kf,dat#,l_kf,dat_,l_kf,dat_,dat_;
  ...
  BARS_UTL_SNAPSHOT.set_table_scn( 'SALDOA', dat_, dbms_flashback.get_system_change_number() );
...
--    END IF;
  ...
  FOR v IN ( SELECT /*+ LEADING(vp b0) FULL(vp) INDEX(b1 UK_SNAP_BALANCES_EXCHANGE) */
               ...
               FROM VP_LIST vp
               JOIN SNAP_BALANCES_EXCHANGE b0
                 on ( b0.FDAT = dat_   and b0.KF = vp.KF  and b0.ACC = vp.ACC3800 )
               left
               join SNAP_BALANCES_EXCHANGE b1
  ...
  LOOP
    ...
    VALUES
      ...
      , 'Переоцінка валютної позиції '||v.nls3800, null, null );
  ...
  -- -- полный доступ
  -- BC.SET_CONTEXT;
  -- BC.SET_POLICY_GROUP(BC.GROUP_WHOLE);
...
PROMPT ===================================================================================== 
PROMPT *** End *** ========== Scripts /Sql/BARS/Procedure/DDRAPS_TRMP.sql =========*** End *
PROMPT ===================================================================================== 
