...
PROMPT *** Create  procedure P_PROTOCOL_KOVBASA ***
  ...
  CREATE OR REPLACE PROCEDURE BARS.P_PROTOCOL_KOVBASA (p_day_before NUMBER)
--------------------------------------------------------------------------
  --  P_PROTOCOL_KOVBASA
  --
...
BEGIN
  WHILE l_numb > 0
  LOOP
    SELECT to_char(MAX(fdat), 'DD/MM/YYYY')
      INTO l_bankdate
      FROM (SELECT fdat
    ...
    bars_zvtdoc.nest_report_table(to_date(l_bankdate, 'DD/MM/YYYY'));
    l_numb := l_numb - 1;
  END LOOP;
  COMMIT;
