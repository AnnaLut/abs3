   ...
   if sum_ = 0 then     -- Check if the document acknowleged
      ...
      begin  -- Clear document
        ...
        begin
            ...
            exception
                          when no_data_found then L_U_ID:=2009401;
        end;
         insert into oper_visa (ref, dat, userid, groupid, status)
               values (ref_, sysdate, L_U_ID, grp_,    1);
         update oper set id_o='******',
                         sign='0123',
                         ref_a= case when length(ref_a)>9 then to_char(to_number(substr(ref_a, -9))) else ref_a end
          where ref=ref_
            and ( id_o is null or
                  sign is null or
                  ref_a is null);
...
grant EXECUTE                                                                on VISA_BATCH_OW6  to BARS_ACCESS_DEFROLE;
