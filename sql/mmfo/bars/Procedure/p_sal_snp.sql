    ...
    select a.branch,
      ...
      from accounts a  --      , specparam_int s      COBUSUPABS-6984 ОБ22 переїхало в accounts
         , ( select m.acc,
               ...
               from ( select acc,
           (decode(FDAT, dat1_, ost+dos-kos-cudos+cukos, 0)) ost,
           (decode(FDAT, dat1_, ostq+dosq-kosq-cudosq+cukosq, 0)) ostq,
           (decode(FDAT, dat2_, ost -crdos +crkos, 0))  osti,
           (decode(FDAT, dat2_, ostq-crdosq+crkosq, 0)) ostiq,
           (dos + crdos- cudos)  dos ,
