...
*/
 ...
 ri_     varchar2(100)    ;
 kf      varchar2(9):=sys_context('bars_context','user_mfo');
 -------------------------
BEGIN
...
if nAlg_ =   0 then
   ...
   begin
     execute immediate 'alter table isPRO_'||kf||' add (ref int, stmt int)';
  ...
  'select i.DEBET, i.KREDIT, i.s, i.nazn, substr(d.nms,1,38), substr(c.nms,1,38)
   from accounts d, accounts c,
       (select  DEBET, KREDIT, sum(SUMMA*100) s, min(NAZN) nazn
        from isPRO_'||kf||' group by DEBET, KREDIT) i
   ...
   LOOP
     ...
     OPEN c1 FOR
       'select SUMMA*100, BRANCH, rowid from isPRO_'||kf||  ' where DEBET=' || nlsa_ || ' and KREDIT=' || nlsb_ ;
     LOOP
        ...
        execute immediate
         'update isPRO_'||kf||' set ref=' || gl.aref || ', stmt=' || gl.astmt ||
         ' where rowid ='''||ri_ ||'''';
