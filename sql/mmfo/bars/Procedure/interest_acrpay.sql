...
--  raise_application_error(-20001,'l_sql='||l_sql );
  ...
  loop
   ...
   if (l_mfo_b is null) then
       raise_application_error(-20000, 'Не вказані параметри отримувача відсотків');
   end if;

   l_purpose := case when l_vpid.nazn is null and l_vpid.ttb = 'PS1' then
                          'Виплата відсотків по рахунку ' || account_utl.get_account_number(l_vpid.acc)
                     else l_vpid.nazn
                end;

   if (l_purpose is null) then
       raise_application_error(-20000, 'Не вказано призначення платежу для виплати відсотків');
   end if;
