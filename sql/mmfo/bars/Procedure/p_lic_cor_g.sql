...
BEGIN
  --ГРН выписка по счетам за последний день месяца с кор. проводками
  ...
  --Модиф. даты
  d_last:=LAST_DAY( p_s );               --Последний рабочий день этого месяца
  d_next_mb:=d_last+1;                   --Первый день след. месяца
  d_next_me:=LAST_DAY( d_next_mb );      --Последний день след. месяца
