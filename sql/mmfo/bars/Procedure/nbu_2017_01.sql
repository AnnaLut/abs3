...
PROMPT ===================================================================================== 
PROMPT *** Run *** ========== Scripts /Sql/BARS/Procedure/NBU_2017_01.sql =========*** Run *
PROMPT ===================================================================================== 


PROMPT *** Create  procedure NBU_2017_01 ***

  CREATE OR REPLACE PROCEDURE BARS.NBU_2017_01 ( p_okpo varchar2) is
 tt TEST_NBU_2017%rowtype;
 l_acc number;
 l_ref number;
 p_KF varchar2(6) := '300465';
 l_dat2 date := to_date('30-09-2017','dd-mm-yyyy');
   ...
   loop tt.kv := aa.kv;     tt.nls := aa.nls;     tt.nms := substr(aa.nms,1,50); tt.branch := aa.branch; 
        ...
        loop tt.fdat := ss.fdat; tt.vx := ss.ostf ;  tt.dos := ss.dos; tt.kos := ss.kos; tt.ix := ss.ostf - ss.dos + ss.kos;
             ...
             for oo in (select * from bars.opldok             where fdat = ss.fdat and acc =  ss.acc and sos = 5
              union all select * from bars.opldok@mgrs.grc.ua where fdat = ss.fdat and acc =  l_acc  and sos = 5
                       ) -- order by ref, dk, s
             loop tt.tt := oo.tt ;  tt.ref := oo.ref; if oo.dk = 0 then tt.dd := oo.s ; tt.kk := 0 ; else tt.dd  := 0 ; tt.kk := oo.s ; end if;
                  ...
                  exception when no_data_found then                  -- l_ref := trunc ( oo.ref /100);
                     begin select CASE WHEN nlsa = aa.nls and kv = aa.kv and mfoa = aa.kf THEN nlsb else nlsa end NLSB ,
                                  CASE WHEN nlsa = aa.nls and kv = aa.kv and mfoa = aa.kf THEN mfob else mfoa end  MFOB  , nazn
                           into tt.nlsb, tt.mfob , tt.nazn                     from bars.oper@mgrs.grc.ua where ref = oo.ref ;
...
PROMPT ===================================================================================== 
PROMPT *** End *** ========== Scripts /Sql/BARS/Procedure/NBU_2017_01.sql =========*** End *
PROMPT ===================================================================================== 
