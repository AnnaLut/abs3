...
PROMPT ===================================================================================== 
PROMPT *** Run *** ========== Scripts /Sql/BARS/Procedure/FPB1.sql =========*** Run *** ====
PROMPT ===================================================================================== 


PROMPT *** Create  procedure FPB1 ***

  CREATE OR REPLACE PROCEDURE BARS.FPB1 (BAKOD1_ varchar2, GGGGMM varchar2 ) IS
/*%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% DESCRIPTION :    Процедура формирования файла PB_1 (табл. PB_1)
% COPYRIGHT   :    Copyright UNITY-BARS Limited, 1999.All Rights Reserved.
% VERSION     : 28/03/2017 (06/01/2015, 22/08/2014)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
--28.03.2017   остатки на начало и конец периода а также суммы док-тов 
--             будут формироваться в копейках а не в тыс. как было ранее
...
-- МФО "родителя"
   ...
   IF mfou_ = 300465 and mfou_ != mfo_g THEN    --322498 THEN 
      ...
      for k in (select p.ref ref
       ...
       loop
          ...
          if k.nlsd LIKE '2600%' 
             ...
             where a.tag='KOD_N' 
               and (trim(a.value) is null OR trim(a.value)<>'2312002') 
               and a.ref=k.ref;
          end if;
          ...
          if k.nlsd LIKE '100%' 
             ...
             where a.tag='KOD_N' 
               and (trim(a.value) is null OR trim(a.value)<>'2312003') 
               and a.ref=k.ref;
          end if;
          ...
          if k.nlsd LIKE '2600%' 
             and k.nlsk LIKE '100%' 
             and (k.name_a like '%'||UPPER('Укрпошт')||'%' OR k.name_a like UPPER('Укрпошт')||'%' OR 
    ...
    -------------------------------------------------------------------------      
          ...
          if k.nlsd LIKE '100%' and k.nlsk LIKE '2809%' and 
             ob22_ in ('32','33','34','28','31','35') and k.tt=k.tt1 then 
          ...
          -- прийнято готiвку за переказом UNISTREAM
          if k.nlsd LIKE '100%' and k.nlsk LIKE '2909%' and ob22_ in ('69') then 
             update operw a set a.value='8428001' 
             where a.tag='KOD_N' 
               and (trim(a.value) is null OR trim(a.value)<>'8428001') 
               and a.ref=k.ref;
          ...
          if k.nlsd LIKE '2909%' and k.nlsk LIKE '100%' and ob22_ in ('60','64') then 
             update operw a set a.value='8428002' 
             where a.tag='KOD_N' 
               and (trim(a.value) is null OR trim(a.value)<>'8428002') 
               and a.ref=k.ref;
          ...
          -- видано готiвку за переказом UNISTREAM
          if k.nlsd LIKE '2909%' and k.nlsk LIKE '100%' and ob22_ in ('69') then 
             update operw a set a.value='8428002' 
             where a.tag='KOD_N' 
               and (trim(a.value) is null OR trim(a.value)<>'8428002') 
               and a.ref=k.ref;
          ...
          -- видано готiвку за переказом UNISTREAM
          if nlsd1_ LIKE '2809%' and nlsk1_ LIKE '100%' and 
             ob22_ in ('29') and (k.tt=k.tt1 or tt_ is not null) then 
   ...
   FOR g in (select decode(a.kv,810,decode(a.pos,2,' N  ',' K  '),'    ') KOR,
                ...
                fdos(a.acc,dat1_,dat1_) -
                    fkos(a.acc,dat1_,dat1_) ), 0)                     S1,
                NVL(abs(fost(a.acc,dat2_)), 0)                        S2
             ...
             select 'к  ',  --'Є   ','k   ',
                    ...
                    a.nls,
                            NVL(abs(fost(a.acc,dat1_) +
                                fdos(a.acc,dat1_,dat1_) -
                                fkos(a.acc,dat1_,dat1_)), 0),
                            NVL(abs(fost(a.acc,dat2_)), 0)
...
-- значна сума  (виключаемо нейтральнi операцii код 8444 )
         if gl.p_icurval(g.KV, k.S,     k.fdat) >=
            ...
            substr(g.NLS,1,2)<>'10' AND KOD_<>'8444'  THEN
...
-- deb.trace( 15, 'DECL_', DECL_ );
               ...
               elsif gl.amfo<>300001 and sNBSk_ in ('16','39' ) then
...
PROMPT *** Create  grants  FPB1 ***
grant EXECUTE                                                                on FPB1            to BARS_ACCESS_DEFROLE;
grant EXECUTE                                                                on FPB1            to PB1;



PROMPT ===================================================================================== 
PROMPT *** End *** ========== Scripts /Sql/BARS/Procedure/FPB1.sql =========*** End *** ====
PROMPT ===================================================================================== 
