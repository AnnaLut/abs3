...
begin
  ...
  LOOP
     If    p_mode = 34 then FETCH C_34 into Z_KOD, Z_tag1, Z_noga, Z_dat, Z_ntik, Z_rnk, Z_dat_M, Z_dat_X, Z_KVA, Z_suma1, Z_suma2, Z_KVb, Z_sumb1, Z_sumb2;  EXIT WHEN c_34%NOTFOUND;
     ElsIf p_mode = 35 then FETCH C_35 into Z_KOD, Z_tag1,         Z_dat, Z_ntik, Z_rnk, Z_dat_M, Z_dat_X, Z_KVA, Z_suma1,          Z_KVb, Z_sumb1, z_Ref  ;  EXIT WHEN c_35%NOTFOUND;
     else  RETURN ;
     end if;
...
--6204	20	нереалўзований фўнансовий результат за операцўями FOREX-звичайний, СВОП (короткў)
     ...
     If fl_3800 = 1    then
        If p_mode = 34 then
           ...
           select l_dat1, l_dat2, decode(z_noga,1,'Форвард', 2,'Вал.своп','Депо-своп') G01, z_tag1 G02, z_ntik g03, z_rnk g04, substr(nmk,1,70) g05,
                  z_dat g06,      decode(z_noga,1,null,z_dat_m) g07, z_dat_X g08, z_KVA g08a, z_KVb g08b,
