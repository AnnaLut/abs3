 ...
 logger.trace ('PAY_CCK: CC_ID='||CC_ID_);
  ...
  begin
     ...
     IF DAT1_ is not null then
        ...
        select acc, rnk, nd, tobo, nls,limit
        INTO ACC_,RNK_,ND_,TOBO_, nls_SG, l_limit
        from (
           select a.acc, a.rnk, n.nd, a.tobo, a.nls,d.limit
...
', sum11_=' ||sum11_ ||' VOB='||to_char(VOB_)
);


  --------------------------- ПЕНЯ ------------------------------------
   --нам все равно какую валюту вернула get_info_cck_ext
   -- исользуем ту что прописана  в спец параметрах
 ...
 logger.trace ('PAY_CCK: KV_SN8 = '||KV_SN8||' SN8 = '||SN8_);
  ...
  If nvl(sn8_,0) >0 then
     begin
       ...
       where t.tobo=TOBO_
         and t.tag='CC_6397' and a.kv=n980_ and t.val=a.nls and a.dazs is null;
     EXCEPTION  WHEN OTHERS THEN
 ...
 logger.trace ('PAY_CCK: 4');
  ...
  INSERT INTO operw (ref,tag,value) VALUES (REF_,'FIO', NMK_ );
     ...
     update oper set vob=vob_ where ref=REF_  and vob is null;
...
bars_audit.trace('PAY_CCK_UPB: D='||sum1D_||', M='||sum1M_||', T='||sum1T_ ||', B='||Sum1B_ );
  ...
  If L_ALLSUM_FOR_VP >0 and kv_<>gl.baseval and l_del_vp!=0 then
     ...
     update opldok set txt='Викуп нерозмінної частини валюти по курсу купівлі' where stmt=gl.aSTMT and ref=ref_;
     end if;
  end if;



  If kv_ <>gl.baseval then null;
     --в инвалюте
  ...
  update oper set vob=vob_ where ref=REF_;
