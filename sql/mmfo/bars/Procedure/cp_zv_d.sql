...
PROMPT ===================================================================================== 
PROMPT *** Run *** ========== Scripts /Sql/BARS/Procedure/CP_ZV_D.sql =========*** Run *** =
PROMPT ===================================================================================== 


PROMPT *** Create  procedure CP_ZV_D ***

  CREATE OR REPLACE PROCEDURE BARS.CP_ZV_D 
...
-- ***  v.3.10 21/07-16  20/07-16   ***

/****
 21/07    Exception - множинна вибірка з CP_DAT
 20/07    Exception щодо док-в  init_ref за роки < 2010
  5/07-16 Проблеми обробки STIKET
...
--------------
 ...
 --n_p2 number;
 l_tiket varchar2(32000);    ---blob;  --long raw;
 l_tiket_7 varchar2(32000);
...
if l_dat_k2 is not null then
 ...
 if k.e_op=3 and l_init_ref is not null then
 begin null;
 ...
 tr.cena0:=cena_init;
 exception when NO_DATA_FOUND then NULL; S_INIT:=0; l_dat_bay:=null; sn_init:=0;
 logger.error(l_pref||'! НЕ знайдено док-т init_ref='||l_init_ref);
 end;
 end if;
...
--
 ...
 --  ????
    ...
    n2_p:=sn_pr;
    sumb_pr:=0; op_pr:=null; kl_p2:=null; cena_p2:=null; cena_p2q:=null;
  --n2_p:=null;
    n3_p:=null;
 end if;   -- 31
...
--
 ...
 --- !!! Розробити алгоритм
  ...
  tr.ost_v:=l_init_ref; tr.dat_4:=l_dat_bay;
...
-- end;
 ...
 --LOG(l_pref||'a ref='||k.ref||'/'||l_ref2||' i9='||i9,'INFO',0);
  ...
  if l_ref_pr is not null and l_dat_pr is not null
   ...
   l_tiket:=null;
   if k.id not in (280,296,465) or nvl(l_op_ar,2) not in (20,22)
            then
...
LOG(l_pref||k.cp_id||'  '||k.ref||' sale '||l_ref_pr||' TR '||s_63,'INFO');
...
PROMPT *** Create  grants  CP_ZV_D ***
grant EXECUTE                                                                on CP_ZV_D         to BARS_ACCESS_DEFROLE;
grant EXECUTE                                                                on CP_ZV_D         to START1;



PROMPT ===================================================================================== 
PROMPT *** End *** ========== Scripts /Sql/BARS/Procedure/CP_ZV_D.sql =========*** End *** =
PROMPT ===================================================================================== 
