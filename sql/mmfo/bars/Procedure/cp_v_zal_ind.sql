...
/* raise_application_error(-20001,'START '||'l_ref= ' ||l_ref ||' p_acc= '||p_acc||' l_id= '||l_id
  ...
  /*Diver: 14.12.2017
    Зміна та історізація велась через таблицю accountsp в розрізі acc та дати застави (велось val - кількість в заставі). 
    При з`явленні нової хотілки в ще в розрізі rnk (COBUMMFO-4020) такий механізм непідходить. 
    Для нових цілей була розширена сама таблиця cp_zal (+ створена історична таблиця cp_zal_update)
