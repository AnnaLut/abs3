...
--- Корректирующие проводки для счетов отсутствующих в конце отчетного года ---
CURSOR SALDOOGK IS
...
--- обороты отчетного года (обработка счета корреспондента)
CURSOR PROVODKI IS
...
--- остатки на 1 января предыдущего года
CURSOR SALDOPGN IS
   SELECT a.acc, a.nls, a.kv, b.fdat, NVL(LTRIM(RTRIM(k.ddd)),'00'),
...
--- Корректирующие проводки для счетов отсутствующих на 1 января
--- предыдущего года ---
CURSOR SALDOPGKN IS
   SELECT s.acc, s.nls, s.kv, s.daos, s.nbs, NVL(LTRIM(RTRIM(k.ddd)),'00'),
          NVL(SUM(DECODE(a.dk, 1, 1, -1)*a.s), 0)
...
CLOSE SALDOPGN;
-----------------------------------------------------------------------------
--- Остатки сформиров. по корр.проводкам для сч. отсутств. на 1 января  ---
--- предыдущего года
OPEN SALDOPGKN;
   LOOP
