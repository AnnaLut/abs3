...
PROMPT ===================================================================================== 
PROMPT *** Run *** ========== Scripts /Sql/BARS/Procedure/CP_ZV8N.sql =========*** Run *** =
PROMPT ===================================================================================== 


PROMPT *** Create  procedure CP_ZV8N ***

  CREATE OR REPLACE PROCEDURE BARS.CP_ZV8N 
...
-- ***  v.3.2   21/11-14    28/07-14  10/07-14     ***
...
l_frm:=trim(p_frm);
l_frm2:=substr(l_frm,2,1);  if l_frm2='k' then l_frm2:='Q'; end if;
l_dat0101 := trunc(l_dat2,'Y');

if l_frm2 = 'Y' then
  --- !  уточнити період дат для повного року
 l_dat1 := trunc(l_dat2,'Y');
 l_dat1131 := l_dat1 - 1;
...
logger.info('CP_ZV start: дати '||l_dat1||' '||l_dat2||' p_id='||p_id||' frm='||l_frm||' u_id='||l_userid);
...
if l_frm like '8%' and k.nbs1 in (1404, 1410, 1412, 1420, 1430, 1435, 1440)
   then  null;
elsif l_frm like '7%' and k.nbs1 in (1411,1413,1414,1421,1422,1423,1424, 3010, 3011, 3012, 3013, 3014, 3110, 3111, 3112, 3113, 3114, 3210, 3211, 3212, 3213, 3214)
   then null;
   ...
   then null;
        ...
        --elsif l_frm like '8%' and k.nbs1 in (1300) then null;  -- !! otl
else goto EX1;
end if;
...
--LOG('CP_ZV: ISIN '||k.cp_id||'/'||k.kv||' dl1='||r_dl1.acc||' '||r_dl1.accd||' '||r_dl1.accr,'INFO',0);
...
tr:=null;
tr.userid:=l_userid; i9:=0;   l_days:=null;
...
--LOG('CP_ZV 1: ID='||k.id||' '||k.cp_id||'/'||k.kv
--                  ||' '||k.dat_k||' ref='||k.ref,'INFO',0);
-- LOG('CP_ZV 1: ISIN='||k.cp_id||'/'||k.kv||' ref='||k.ref,'ERROR',0);
...
-- !! l_dat_k, l_dat_k2 потрібно уточнення по аналізу на  VOB=096
 ...
 LOG('CP_ZV 1: ID='||k.id||' '||k.cp_id||' '||l_kv||' bay date '||k.dat_k||' ref='||k.ref||' Sn_bay='||n_bay,'INFO',0);
 LOGGER.INFO('CP_ZV 1: ID='||k.id||' '||k.cp_id||' '||l_kv||' bay date '||k.dat_k||' ref='||k.ref||' Sn_bay='||n_bay);
...
--
 ...
 if sn_pr is not null then
  ...
  if l_kv!=980 then cena_pr:=gl.p_icurval(l_kv,cena_pr*100,l_dat_pr)/100; end if;
     -- op_pr   2 - sale  3 - move
...
/*
 ...
 where o.acc in (k.accr,k.accr2) and o.dk=1
       ...
       and o.tt in ('FX7','FX8');
  if r_otr !=0 then
  logger.info('CP_ZV: погаш. '||k.cp_id||' '||'/'||k.ref||' ref пог. '||l_ref_g||' купон='||r_otr);
  end if;
  end;
  ...
  r_otrq:=r_otr;
  if l_kv!=980 then  NULL;
     r_otrq:=gl.p_icurval(l_kv,r_otr*100,t_dat)/100;
  end if;
     ***/
...
--
 ...
 if  OKN_K != SN_31 - SN_ then   DF_:= OKN_K - (SN_31 - SN_);  null;
  ...
  for i5 in
             (select o.ref, null,
              ...
              and o.dk=1
              and o.ref=p.ref
              and o.fdat  between l_dat1 and l_dat2
              and o.sos=5
                    order by 1)
  loop
             -- всі реф-си угод продажу
 ...
 LOG('CP_ZV: ISIN='||k.cp_id||' '||k.dat_k||'/'||k.ref||' SN='||SN_
               ||' sale '||l_dat_pr||'/'||l_ref2||' SN_pr='||sn_pr||' i9='||i9,'INFO');
 logger.info('CP_ZV: прод. '||k.cp_id||' '||k.dat_k||'/'||k.ref||' SN='||SN_
...
-- end;
 ...
 into   r_sale, r_saleq
 from opldok o      --, OPER P
 where o.acc in (r_dl1.accr, r_dl1.accr2) and o.dk=1  --and o.ref=p.ref
 ...
 --LOG('CP_ZV:a ref='||k.ref||'/'||l_ref2||' i9='||i9,'INFO',0);
  ...
  if nvl(cena_p2,0)=0 then cena_p2:=1;  end if;
  ...
  if l_ref_pr is not null and l_dat_pr is not null
   ...
   l_tiket:=null;
   if k.id not in (280,296) then
   select  stiket  into l_tiket
   from cp_arch where ref=nvl(l_ref_pr,0);
   end if;
 ...
 if sn_pr is not null then
    ...
    cena_pr:=round(nvl(sumb_pr,s_pr_op)/nvl(kl_p2,1),2);
    if l_kv!=980 then cena_pr:=gl.p_icurval(l_kv,cena_pr*100,l_dat_pr)/100; end if;
             -- ?! Не точна при частк. продажі
             -- замість sumb_pr брати суму продажу щодо пакету
             -- по складових рах-х пакету ...)
     -- op_pr   2 - sale  3 - move
    tr.g044:='№ б/у'; --tr.g045:='*pk';
 end if;
 ...
 ---  torg. rez-t   *****+++++++++
   if k.kv=980 and 1=1 then          -- для kv!=980 через VP  доробити
      ...
      where o.acc = acc_3739
            ...
            and o.sos=5
            and rownum=1;
--logger.info('CP_ZV: '||' '||k.ref||' sale '||l_ref_pr||' TR '||s_63);
LOG('CP_ZV: '||k.cp_id||'  '||k.ref||' sale '||l_ref_pr||' TR '||s_63,'INFO');
      ...
      where o.acc = acc_3739
          ...
          --  and o.stmt=k.stmt     -- вал. номіналу
            ...
            and o.sos=5
            and rownum=1;
LOG('CP_ZV:* '||k.cp_id||' '||k.ref||' sale '||l_ref_pr
              ||' TR '||s_63||' '||round(s_63*l_kf_pr,2)||' kf_pr='||l_kf_pr,'INFO');
      exception when NO_DATA_FOUND then NULL;  s_63:=0;
      end;
...
--    or k.nbs1='1420' then          -- 44
 null;
  -- LOG('CP_ZV:c ref='||k.ref||'/'||l_ref2||' i9='||i9,'INFO',0);
        --*******
                    --888888888888
 begin

--LOG('CP_ZV:d '||l_ref2||' i9='||i9,'INFO',0);
-- LOG('CP_ZV:d ref='||k.ref||'/'||l_ref2||' i9='||i9,'INFO',0);
...
--- .....
   ...
   tr.g035:=null; tr.g036:=null; tr.g055:=null; tr.g060:=null; tr.g069:=null;
   ...
   tr.g053:=null; tr.g054:=null; tr.g056:=null;
   tr.g062:=null;
 ...
 begin
 tr.nls_p1:='sale'; tr.nls:=i9;
 insert into tmp_cp_zv  values tr;
  exception when others then NULL;
 LOG('CP_ZV: error* sale N ID='||k.cp_id||' ref='||k.ref||' i9='||i9,'ERROR',0);
 end;
 end if;
                 --******
 exception when others then NULL;
 ...
 insert into tmp_cp_zv  values tr;
  exception when others then NULL;
 LOG('CP_ZV: error9 N ISIN='||k.cp_id||' '||k.dat_k||' ref='||k.ref,'ERROR',0);
 end;
 end if;
...
<<EX2>> null;
LOG('CP_ZV: finished ','INFO',0);
logger.info('CP_ZV: finished ');
--commit;
end;  -- cp_zv8n
/
show err;

PROMPT *** Create  grants  CP_ZV8N ***
grant EXECUTE                                                                on CP_ZV8N         to BARS_ACCESS_DEFROLE;
grant EXECUTE                                                                on CP_ZV8N         to START1;



PROMPT ===================================================================================== 
PROMPT *** End *** ========== Scripts /Sql/BARS/Procedure/CP_ZV8N.sql =========*** End *** =
PROMPT ===================================================================================== 
