...
BEGIN
   ...
   nn_        := 22;
   if mode_ = 0 THEN
      z23.to_log_rez (user_id , nn_ , dat01_ ,'Начало Проводки - Реальные (новый план счетов) '||'nal='||nal_);
   else
      z23.to_log_rez (user_id , nn_ , dat01_ ,'Начало Проводки - МАКЕТ  (новый план счетов)'||'nal='||nal_);
   end if;
   dat31_ := gl.bdate;  --Dat_last_work(dat01_-1); -- Последний рабочий день месяца
   ...
   rasform_nazn           := 'Згортання резерву за ' || dat31_; -- ||name_mon_||' '||year_ ;
      ...
      loop
         ...
         if k.f7_acc is null then
            if REZPROV_ = 1 then
               ...
               nls_ := '373909999903';
               nls_ := vkrzn( substr(gl.aMfo,1,5), NLS_);
               nms_ := 'Транзитний для згортання-розгортання резервів';
               k.f7_nls := nls_;
