...
PROMPT *** Create  procedure PUL_DAT ***
  ...
  CREATE OR REPLACE PROCEDURE BARS.PUL_DAT ( sFdat1 varchar2,  sFdat2 varchar2 ) is

/*
 20-07-2016 По согласованию с Суховой убрала запись в таблицу + переприсвоение sFdat2, если оно не дата
 29.06.2016 Сухова  Добавлено запоминание переменных и в таблицу.
 25.04.2013 01 число месяца за sFdat1. + Ид єтой даты
 11-01-2013 Дата корр оборотыв
 21-01-2011 если пар = пусто    если в переменных пусто, то = bank-дате   иначе        = перепеннаЯ
*/
l_dat   varchar2(35);
l_dat2  date;
dat01_  date;
...
if   sFdat1 is null then   l_dat := pul.Get_Mas_Ini_Val('sFdat1');
  If l_dat  is null then PUL.Set_Mas_Ini( 'sFdat1', to_char(gl.bdate,'dd.mm.yyyy'), 'sар.sFdat1' );  end if;
...
if sFdat2 is null then   l_dat := pul.Get_Mas_Ini_Val('sFdat2');
  If l_dat is null then
   ...
   begin
      select to_date(pul.Get_Mas_Ini_Val('sFdat2')) into l_dat2 from dual;
      PUL.Set_Mas_Ini( 'sFdat2', sFdat2, 'Пар.sFdat2' );
   exception when others then
      PUL.Set_Mas_Ini( 'sFdat2', to_char(gl.bdate,'dd.mm.yyyy'), 'sар.sFdat2' );
   end;
end if;
-------------------------------------------------------------------------
--delete from T_SFDAT where isp = gl.auid;
--insert into T_SFDAT (isp, b,e,z) values (gl.auid, NVL (TO_DATE (pul.get_mas_ini_val ('sFdat1'), 'dd.mm.yyyy'), gl.bd),
--                                                  NVL (TO_DATE (pul.get_mas_ini_val ('sFdat2'), 'dd.mm.yyyy'), gl.bd),
--                                                  NVL (TO_DATE (pul.get_mas_ini_val ('zFdat1'), 'dd.mm.yyyy'), gl.bd)
--                                      );
------------------------------------------------------------------------------------------------------
end PUL_DAT;
/
...
grant EXECUTE                                                                on PUL_DAT         to RCC_DEAL;
