...
% VERSION     : 12.04.2012 (10.04.12,08.11.11,12.07.11,06.04.11,03.03.11,
%             :             11.02.11,25.01.11,20.01.11,19.01.11,22.04.10)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
параметры: Dat_ - отчетная дата

12.04.2012 для показателя 113 дополнительно формируем показатель 103 с
           противоположным кодом dk_ и сумма показателя отрицательная
10.04.2012 с 30.03.2012 введены новые коды которые занесены в классификатор
           По счетам резерва (ЮЛ и ФЛ - 2400,2401) выполненна разбивка
           по параметру CUSTTYPEю Если CUSTTYPE<>3 то это ЮЛ,
           если CUSTTYPE=3 то это ФЛ.
           Дополнительно для ФЛ выполняется проверка параметра K150
           (тип контрагента).
           Если CUSTTYPE<>3 and K150='3' то это счет ФЛ
...
CURSOR SALDOOG IS
   SELECT s.acc, s.nls, s.kv, s.fdat, s.nbs, s.ost, s.ostq,
          ...
          a.tobo, a.nms, nvl(trim(p.k150),'2')
   FROM  otcn_saldo s, accounts a, specparam p, customer c, kl_k070 k, specparam_int sp
   WHERE s.acc=a.acc
     and s.rnk=c.rnk
     and s.acc=p.acc(+)
     and c.ise=k.k070(+)
...
LOOP
   FETCH SALDOOG INTO acc_, nls_, kv_, data_, nbs_, Ostn_, Ostq_,
                      ...
                      custtype_, k072_, sed_, ob22_, tobo_, nms_, k150_;
      ...
      if mfou_ = 300465 then
         IF nbs_ in ('1518','1519','1526','1527','1528','1529','1819',
            ...
            IF nbs_='3590' and ob22_ not in ('01','02') THEN
               ddd_:='109';
            END IF;
            IF nbs_='3599' and ob22_ in ('08','09','10') THEN
               ddd_:='116';
            END IF;
         END IF;
      end if;
...
--      END IF;
      ...
      if ddd_ in ('109','113','115','117','119','130','132','512') then   ---('512','514')
         kod_11_ := '11';
      else
         kod_11_ := '10';
      end if;
      ...
      -- с 30.03.2012 для счетов резервов ЮЛ новый код 130 который внесен в кл-р KOD_R020_B5, а для ФЛ новый код 132
      IF dat_ >= to_date('30032012','ddmmyyyy') and ddd_='130' THEN
         -- резерви фiзичних осiб з 30.03.2012 окремий код 132
         if custtype_ = 3 then
            ddd_ := '132';
         end if;
         if custtype_ <> 3 and k150_ = '3' then
            ddd_ := '132';
         end if;
      end if;
      ...
      INSERT INTO rnbu_trace (nls, kv, odate, kodp, znap, comm) VALUES
                             (nls_, kv_, data_, kodp_, znap_, comm_);

      -- с 30.03.2012 для счетов ФЛ новый код 131
      /*
      IF ddd_='120' THEN
...
--      END IF;
      ...
      -- в код 103 формируем сумму резерва с противоположным знаком
      IF ddd_ in ('113','130','132','115','117','109') THEN
...
--      END IF;
      ...
      -- c 30.03.2012 все в одном коде 506
      /*
      IF nbs_='5100' THEN
         --BEGIN
         --   SELECT NVL(r011,'0') INTO r011_ FROM specparam
