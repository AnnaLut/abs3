...
BEGIN
   ...
   IF (:old.dazs IS NOT NULL) AND (:new.dazs IS NOT NULL)
   THEN
      RETURN;
   END IF;

   -- Nothing changes, do nothing
   IF     ( (:new.vid = :old.vid) OR (:new.vid IS NULL AND :old.vid IS NULL))
      AND (   (:new.dazs = :old.dazs)
           OR (:new.dazs IS NULL AND :old.dazs IS NULL))
   THEN
      RETURN;
   END IF;

   -- Accounts type not interested TaxPolice
   IF    (:new.vid = :old.vid AND :new.vid = 0)
      OR (:old.vid IS NULL AND :new.vid IS NULL)
      OR (:new.vid <> :old.vid AND :new.vid <> 0 AND :old.vid <> 0)
   THEN
      RETURN;
   END IF;
   ...
   -- Account resurected!
   IF (:old.dazs IS NOT NULL) AND (:new.dazs IS NULL)
   THEN
      DELETE FROM ree_tmp
            WHERE     nls = :old.nls
                  AND ot = 3
   ...
   IF l_acc_blkid > 0 AND :old.blkd = l_acc_blkid AND :new.blkd = l_acc_blkid
   THEN
      RETURN;
   ELSIF :old.blkd = l_acc_blkid AND :new.blkd <> l_acc_blkid
   THEN
   ...
   ELSE
      ...
      THEN
         SELECT NVL (MIN (VALUE), 3)
           INTO ot_
           FROM accountsw
          WHERE acc = :old.acc AND tag = 'DPAOTYPE';
      ELSE
         IF (:new.vid <> :old.vid OR :old.vid IS NULL)
         THEN
            IF :new.vid = 0
            THEN
               ot_ := 3;
   ...
   IF l_custtype = 3 AND SUBSTR (okpo_, 1, 5) IN ('99999', '00000')
   THEN
      BEGIN
   ...
   THEN
      ...
      INSERT INTO ree_tmp (mfo,
           ...
           VALUES (mfo_,
                   ...
                   ot_,
                   nls_,
                   coalesce(l_bankdate, gl.bdate, trunc(sysdate)),
                   kv_,
                   rez_out,
                   nmk_,
                   nmk_,
