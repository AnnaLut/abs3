...
PROMPT ===================================================================================== 
PROMPT *** Run *** ========== Scripts /Sql/BARS/Trigger/TAIUD_SHTFSCALE_UPDATE.sql =========
PROMPT ===================================================================================== 


PROMPT *** Create  trigger TAIUD_SHTFSCALE_UPDATE ***

  CREATE OR REPLACE TRIGGER BARS.TAIUD_SHTFSCALE_UPDATE 
after insert or update or delete on BARS.SH_TARIF_SCALE
for each row
declare
  l_rec  SH_TARIF_SCALE_UPDATE%rowtype;
  ---
  ...
  begin
    ...
    then
        ...
        l_rec.SUM_LIMIT := :old.SUM_LIMIT; l_rec.SUM_TARIF := :old.SUM_TARIF;
        l_rec.PR        := :old.PR;        l_rec.KF        := :old.KF;
        l_rec.SMIN      := :old.SMIN;      l_rec.SMAX      := :old.SMAX;
    else
        l_rec.IDS       := :new.IDS;       l_rec.KOD       := :new.KOD;
        l_rec.SUM_LIMIT := :new.SUM_LIMIT; l_rec.SUM_TARIF := :new.SUM_TARIF;
        l_rec.PR        := :new.PR;        l_rec.KF        := :new.KF;
        l_rec.SMIN      := :new.SMIN;      l_rec.SMAX      := :new.SMAX;
    end if;
    l_rec.IDUPD         := bars_sqnc.get_nextval('s_shtfscale_update', l_rec.KF);
    ...
    insert into BARS.SH_TARIF_SCALE_UPDATE values l_rec;
...
begin
  case
    ...
    then
      case
        when (:old.KOD <> :new.KOD) or (:old.SUM_LIMIT <> :new.SUM_LIMIT) or (:old.IDS <> :new.IDS) or (:old.KF <> :new.KF) -- !!! analize changing PRIMARY KEY - columns
...
end TAIUD_SHTFSCALE_UPDATE;
/
ALTER TRIGGER BARS.TAIUD_SHTFSCALE_UPDATE ENABLE;


PROMPT ===================================================================================== 
PROMPT *** End *** ========== Scripts /Sql/BARS/Trigger/TAIUD_SHTFSCALE_UPDATE.sql =========
PROMPT ===================================================================================== 
