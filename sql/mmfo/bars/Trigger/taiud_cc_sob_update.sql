...
declare
  ...
  l_KF   CC_SOB_UPDATE.KF%type:=null;
begin
  if deleting then
     l_chgaction   := 3;
     l_nd   := :old.nd;
     ...
     if    inserting
     then l_chgaction := 1;
     elsif :old.OTM <> :new.OTM or :old.FACT_DATE <> :new.FACT_DATE
     then l_chgaction := 2;
     end if;
  ...
  if l_chgaction > 0 then
     l_idupd := bars_sqnc.get_nextval('s_CC_SOB_UPDATE', l_KF);
     insert into CC_SOB_UPDATE (nd,FDAT,ID,ISP,TXT,OTM,FREQ,PSYS, FACT_DATE,
                                chgdate, chgaction, doneby, idupd, kf)
                        values (l_nd,l_FDAT,l_ID,l_ISP,l_TXT,l_OTM,l_FREQ,l_PSYS, l_FACT_DATE,
                                sysdate, l_chgaction,(select logname from staff$base where id = user_id), l_idupd, l_KF
                               );
  end if;
