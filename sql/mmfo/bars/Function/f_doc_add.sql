    ...
    When 4 Then
      ...
      BEGIN
        SELECT -- to_date(dat_begin, 'dd.mm.yyyy') d1, to_date(dat_end, 'dd.mm.yyyy') d2,
               ...
               --add_months(to_date(dat_begin, 'dd.mm.yyyy'),trunc(months_between (to_date(dat_end, 'dd.mm.yyyy'), to_date(dat_begin, 'dd.mm.yyyy')),0)) d3,
               TRIM(f_sumpr(to_date(d.dat_end, 'dd.mm.yyyy') - add_months(to_date(d.dat_begin, 'dd.mm.yyyy'),
               trunc(months_between (to_date(d.dat_end, 'dd.mm.yyyy'), to_date(d.dat_begin, 'dd.mm.yyyy')),0)), null,'M'))||'  дн. '
          INTO l_str
          FROM dpt_deposit d,
               dpt_agreements a
         WHERE d.deposit_id = a.dpt_id
