 ...
 PROMPT ===================================================================================== 
 PROMPT *** Run *** ========== Scripts /Sql/BARS/function/f_pop_otcn_n_acc.sql =========*** R
 PROMPT ===================================================================================== 
 
  CREATE OR REPLACE FUNCTION BARS.F_POP_OTCN_N_ACC (Dat_ DATE, type_ NUMBER,
                                       sql_acc_ VARCHAR2,
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
10/07/2012  - работает по перечню аналитических счетов (асс)
...
16.01.2011 - для файлов #25,#81 в облуправлениях СБ отчетная дата
             '0101GGGG' (т.к. сворачивание выполняется всегда '0101')
             и поэтому конец периода корректирующих ограничил окончанием
             месяца - Dat2_ = TRUNC(Dat_ +28)
12.01.2011 - добавил отбор оборотов перекрытия и для 01 месяца кроме 12
             т.к. в облуправлениях СБ перекрытие выполняется 01.01.YYYY.
...
IF datp_ IS NULL THEN -- дата начала не задано, по умолчанию - месяц
   ...
   IF to_char(Dat_,'DDMM') = '0101' THEN
      Dat2_ := TRUNC(Dat_ +28);
   END IF;
...
/
 ...
 PROMPT ===================================================================================== 
 PROMPT *** End *** ========== Scripts /Sql/BARS/function/f_pop_otcn_n_acc.sql =========*** E
 PROMPT ===================================================================================== 
