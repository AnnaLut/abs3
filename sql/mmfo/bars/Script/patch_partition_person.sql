...
begin
  execute immediate 'drop index I_PERSON_PSN';
...
declare
  ...
  pragma exception_init( E_IDX_NOT_EXISTS, -01418 );
begin
  execute immediate 'drop index I1_PERSON';
  dbms_output.put_line( 'Index dropped.' );
exception
  when E_IDX_NOT_EXISTS
  then null;
...
begin
  execute immediate 'alter table PERSON drop constraint PK_PERSON cascade drop index';
...
begin
  ...
  then -- ÿêùî PARTITIONS â³äñóòí³
    ...
    begin
      execute immediate q'[alter table PERSON add kf varchar2(6)]';
      dbms_output.put_line( 'Table altered.' );
    exception
    ...
    execute immediate 'alter table PERSON enable all triggers';
...
begin
  execute immediate q'[create index I_PERSON_PSN on PERSON (KF, PASSP, TRANSLATE(UPPER(SER),'ABCEHIKMOPTXY','ÀÂÑÅÍ²ÊÌÎÐÒÕÓ'), UPPER(NUMDOC))
  tablespace BRSBIGI
  pctfree 10
  initrans 2
  maxtrans 255
