...
begin 
  ...
  bc.go('/');
  for r in (select * from mv_kf) loop --r
  bc.go(r.kf);
     ...
     for i in (
         -- —чета 2620, которые открыты, но по которым 22.02.2018 были закрыты депозиты
           ...
           exception when others then
             ...
             -- записываем в табличку лога
             l_s := 'Error. ќшибка восстановлени€ депозита ID = '||to_char(i.deposit_id)||' и счета '||to_char(i.acc)|| '('||i.nls||')';
             l_len := 190 - length(l_s);
             l_s := l_s ||substr(sqlerrm,1,l_len);
             insert into test_recovery2620_220218 values 
