prompt ===================================== 
prompt == Виписки по рахунках Клієнта
prompt ===================================== 
...
begin     
   l_reports.name := 'Виписки по рахунках Клієнта';
   l_reports.id := '3003';
 ...
 {"Name":"~p_rnk","Type":"Decimal","Value":"0","Label":"РНК клієнта"},
 ...
 {"Name":"~p_kv","Type":"Text","Value":"0","Label":"Валюта","Control":{"Type":"select","Data":[{"Value":"0","Text":"Всі валюти"},{"Value":"980","Text":"Нац.валюта"},{"Value":"-980","Text":"Ін.валюта"}]}}
    ...
    l_reports.RESULT_FILE_NAME     := '''VP''||case when ~p_kv = 980 then ''GRN'' when ~p_kv = ''-980'' then ''VAL'' else ''_'' end||''_''||~p_rnk||''_''||substr(dwh_cbirep.frmt_date(p_sFdat1,''DMYY''),1,6)||''.rar''';
...
for c0 in (select rnk, nls, kv 
             from accounts a
            where rnk like ~p_rnk and nls like ~p_nls and case when ~p_kv = 0                     then 1
...
begin   
    ...
    l_codeoper := operlist_adm.add_new_func
                   (p_name      =>     'Формування звітів',   
                    p_funcname  =>     '/barsroot/dwh/report/index?moduleId='||l_codearm,                                
                    p_frontend  =>      1 );                      
