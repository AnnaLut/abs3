...
select /*+ parallel( 24 ) */ ]' || l_col_lst || q'[
  from ]'|| l_tab_nm;
    ...
    --select * from sec_audit where rec_message like 'stmt=%' and rec_date > sysdate - 1/(24 *4) order by rec_id desc 
    
    bars_audit.info('stmt='||l_tab_stmt);
...
begin
  BPA.ALTER_POLICIES( 'CASH_LASTVISA' );
end;
/
