MERGE INTO BARS.OW_OUT_FILES_SOURCE A USING
 (SELECT
  0 as ID,
  'Way4. Формування файла поповнень/списань IIC_Documents*.xml для Way4' as FUNCNAME,
...
MERGE INTO BARS.OW_OUT_FILES_SOURCE A USING
 (SELECT
  2 as ID,
  'Way4. Формування файла IIC_Documents*.xml по док. регул.плат.' as FUNCNAME,
  'bars_ow.web_out_files' as PROC,
  0 as TYPE
  FROM DUAL) B
ON (A.id = B.id)
...
MERGE INTO BARS.OW_OUT_FILES_SOURCE A USING
 (SELECT
  3 as ID,
  'Way4. Формування файла повернення платежів OIC*LOCPAY*.xml' as FUNCNAME,
  'bars_ow.web_out_files' as PROC,
  1 as TYPE
  FROM DUAL) B
ON (A.id = B.id)
