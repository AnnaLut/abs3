...
PROMPT /Sql/BARS/Table/CC_ACCP.sql
...
PROMPT Create index UK_CC_ACCP
begin
    execute immediate q'[create unique index BARS.UK_CC_ACCP on BARS.CC_ACCP(KF, ACC, ACCS) tablespace brsmdli compute statistics]';
exception
    when others then
        if sqlcode = -955 then null; else raise; end if;
end;
