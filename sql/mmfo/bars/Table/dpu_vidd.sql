...
COMMENT ON COLUMN BARS.DPU_VIDD.FREQ_V IS 'Периодичность выплаты %%';
...
COMMENT ON COLUMN BARS.DPU_VIDD.COMPROC IS 'Флаг капитализации %%';
...
COMMENT ON COLUMN BARS.DPU_VIDD.IRREVOCABLE IS 'Безвідкличний депозитний договір (заборонено дострокове вилучення)';
...
PROMPT *** Create  constraint CC_DPUVIDD_SROK_NN ***
begin   
 execute immediate '
  ALTER TABLE BARS.DPU_VIDD MODIFY (SROK CONSTRAINT CC_DPUVIDD_SROK_NN NOT NULL ENABLE NOVALIDATE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
begin   
 execute immediate '
  ALTER TABLE BARS.DPU_VIDD MODIFY (IRREVOCABLE CONSTRAINT CC_DPUVIDD_IRREVOCABLE_NN NOT NULL ENABLE NOVALIDATE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
begin   
 execute immediate '
  ALTER TABLE BARS.DPU_VIDD ADD CONSTRAINT CC_DPUVIDD_IRREVOCABLE CHECK ( IRREVOCABLE = 0 OR IRREVOCABLE = 1 ) ENABLE NOVALIDATE';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
