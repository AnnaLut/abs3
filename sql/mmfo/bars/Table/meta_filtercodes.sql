...
BEGIN 
        execute immediate  
          'begin  
               bpa.alter_policy_info(''META_FILTERCODES'', ''CENTER'' , null, null, null, null);
...
COMMENT ON COLUMN BARS.META_FILTERCODES.BRANCH IS 'Hierarchical Branch Code';
...
PROMPT *** Create  constraint CC_METAFILTERCODES_BRANCH_NN ***
begin   
 execute immediate '
  ALTER TABLE BARS.META_FILTERCODES MODIFY (BRANCH CONSTRAINT CC_METAFILTERCODES_BRANCH_NN NOT NULL ENABLE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
