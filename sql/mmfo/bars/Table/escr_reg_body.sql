...
BEGIN 
        execute immediate  
          'begin  
               bpa.alter_policy_info(''ESCR_REG_BODY'', ''FILIAL'' , ''M'', ''M'', ''M'', ''M'');
...
COMMENT ON TABLE BARS.ESCR_REG_BODY IS 'Портфель енергозберігаючих кредитів (специфікація)';
COMMENT ON COLUMN BARS.ESCR_REG_BODY.ID IS '';
COMMENT ON COLUMN BARS.ESCR_REG_BODY.DEAL_ID IS 'Ідентифікатор кредитного договору';
COMMENT ON COLUMN BARS.ESCR_REG_BODY.DEAL_ADR_ID IS 'Ідентифікатор адреси';
COMMENT ON COLUMN BARS.ESCR_REG_BODY.DEAL_REGION IS 'Дані кредиту: область';
COMMENT ON COLUMN BARS.ESCR_REG_BODY.DEAL_FULL_ADDRESS IS 'Дані кредиту: повна адреса';
COMMENT ON COLUMN BARS.ESCR_REG_BODY.DEAL_BUILD_ID IS 'Дані кредиту: тип будинку';
COMMENT ON COLUMN BARS.ESCR_REG_BODY.DEAL_EVENT_ID IS 'Ідентифікатор енергоефективного заходу';
COMMENT ON COLUMN BARS.ESCR_REG_BODY.KF IS '';
