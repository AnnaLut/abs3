...
begin
  select count(1) into v_num
    ...
    where table_name = 'TELLER_ATM_STATUS'
      and column_name = 'BLOCKED';
  if v_num = 0 then
    execute immediate 'alter table TELLER_ATM_STATUS add BLOCKED number';
  end if;
end;
/
...
COMMENT ON COLUMN BARS.TELLER_ATM_STATUS.EQUIP_CODE IS 'Вид устройства';
COMMENT ON COLUMN BARS.TELLER_ATM_STATUS.EQUIP_IP IS 'Адрес устройства';
COMMENT ON COLUMN BARS.TELLER_ATM_STATUS.WORK_DATE IS 'Банковская дата';
COMMENT ON COLUMN BARS.TELLER_ATM_STATUS.AMOUNT IS 'информация об остатках';
COMMENT ON COLUMN BARS.TELLER_ATM_STATUS.LAST_USER IS 'кто обновлял';
COMMENT ON COLUMN BARS.TELLER_ATM_STATUS.LAST_DT IS 'когда обновлял';
COMMENT ON COLUMN BARS.TELLER_ATM_STATUS.STATUS IS 'статус АТМ (от Глори)';
COMMENT ON COLUMN BARS.TELLER_ATM_STATUS.STATUS_DESC IS 'Описание статуса ';
COMMENT ON COLUMN BARS.TELLER_ATM_STATUS.OCCUPY_USER IS 'Кто блокирует';
COMMENT ON COLUMN BARS.TELLER_ATM_STATUS.AMOUNT_TIME IS 'Время обновления статуса и остатка';
COMMENT ON COLUMN BARS.TELLER_ATM_STATUS.BLOCKED IS 'Признак блокировки';
