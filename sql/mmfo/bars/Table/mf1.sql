...
begin   
 execute immediate '
  ALTER TABLE BARS.MF1 ADD CONSTRAINT PK_MF1 PRIMARY KEY (ID)
...
begin    execute immediate '
  CREATE UNIQUE INDEX BARS.PK_MF1 ON BARS.MF1 (ID) 
  PCTFREE 10 INITRANS 2 MAXTRANS 255 COMPUTE STATISTICS 
  TABLESPACE BRSDYND ';exception when others then  if  sqlcode=-955  then null; else raise; end if; end;/
prompt add KF on MF1 and recreate PK
begin
  bars.bc.go(f_ourmfo_g);
  execute immediate 'alter trigger tbiu_mf1 disable';
  execute immediate 'alter table bars.mf1 add kf varchar2(6) default sys_context(''bars_context'', ''user_mfo'')';
  execute immediate 'alter trigger tbiu_mf1 enable';
exception
  when others then
...
/
begin
  execute immediate 'alter table bars.mf1 drop constraint PK_MF1';
exception
  when others then
...
/
begin
  execute immediate 'drop index bars.PK_MF1';
exception
  when others then
...
/
begin
  execute immediate 'create unique index bars.pk_mf1 on bars.mf1(id, kf) tablespace brsdyni';
exception
  when others then
    if sqlcode = -955 then null; else raise; end if;
end;
/
begin
  execute immediate 'alter table bars.mf1 add constraint pk_mf1 primary key(id, kf) using index bars.pk_mf1';
exception
  when others then
