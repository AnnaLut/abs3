begin
  execute immediate 'begin bpa.alter_policy_info(''ow_inst_sub_t_hist'', ''WHOLE'',  null, ''E'', ''E'', ''E''); end;';
exception when others then
  if sqlcode = -06550 then null; else raise; end if;
end;
/
 begin
  execute immediate 'begin bpa.alter_policy_info(''ow_inst_sub_t_hist'', ''FILIAL'', ''M'', ''M'', ''M'' , ''M''); end;';
exception when others then
  if sqlcode = -06550 then null; else raise; end if;
end;
/ 
begin
  execute immediate 'begin bpa.alter_policy_info(''ow_inst_sub_t_hist'', ''CENTER'', null, ''E'', ''E'' , ''E''); end;';
exception when others then
  if sqlcode = -06550 then null; else raise; end if;
end;
/
BEGIN
execute immediate 'create table ow_inst_sub_t_hist (
chain_idt number,
plan_num number,
type varchar2(30),
code varchar2(30),
...
KF varchar2(6 BYTE) default sys_context(''bars_context'',''user_mfo'') CONSTRAINT CC_ow_inst_sub_t_hist_KF_NN NOT NULL) 
tablespace BRSMDLD
PARTITION BY LIST (KF) 
 ...
 END;
/
COMMENT ON TABLE BARS.ow_inst_sub_t_hist IS 'Графік сплати Інстолмент в розрізі складових';

COMMENT ON COLUMN BARS.ow_inst_sub_t_hist.CHAIN_IDT IS 'ІД графіка';

COMMENT ON COLUMN BARS.ow_inst_sub_t_hist.TYPE IS 'Тип боргу: 
PRINCIPAL - основний борг
FEE - винагорода';

COMMENT ON COLUMN BARS.ow_inst_sub_t_hist.CODE IS 'Тип боргу: 
PRINCIPAL - основний борг
FEE - комісія
INT - проценти';

COMMENT ON COLUMN BARS.ow_inst_sub_t_hist.STATUS IS 'Сдатус графіка: 
...
COMMENT ON COLUMN BARS.ow_inst_sub_t_hist.TOTAL_AMOUNT IS 'Загальна сума складової графіку';

COMMENT ON COLUMN BARS.ow_inst_sub_t_hist.AMOUNT_TO_PAY IS 'Сума до сплати';

COMMENT ON COLUMN BARS.ow_inst_sub_t_hist.WRITTEN_OFF_AMOUNT IS 'Сума списання';

COMMENT ON COLUMN BARS.ow_inst_sub_t_hist.OVERDUE_AMOUNT IS 'Сума прострочки';

COMMENT ON COLUMN BARS.ow_inst_sub_t_hist.KF IS 'МФО';
/
begin
  execute immediate 'begin BARS_POLICY_ADM.ALTER_POLICIES(''ow_inst_sub_t_hist''); end;';
exception when others then
  if sqlcode = -06550 then null; else raise; end if;
end;
/
begin   
 execute immediate 'ALTER TABLE BARS.ow_inst_sub_t_hist ADD 
CONSTRAINT PK_ow_inst_sub_t_hist
 PRIMARY KEY (CHAIN_IDT, PLAN_NUM, CODE)
 ENABLE
 VALIDATE';
exception when others then
...
begin   
 execute immediate ' 
ALTER TABLE BARS.ow_inst_sub_t_hist ADD 
CONSTRAINT FK_ow_inst_sub_t_hist
...
/
grant all on ow_inst_sub_t_hist to bars_access_defrole;
/
grant all on ow_inst_sub_t_hist to bars;
/