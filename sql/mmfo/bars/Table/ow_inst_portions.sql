begin
  execute immediate 'begin bpa.alter_policy_info(''ow_inst_portions'', ''WHOLE'',  null, ''E'', ''E'', ''E''); end;';
exception when others then
  if sqlcode = -06550 then null; else raise; end if;
end;
/
 begin
  execute immediate 'begin bpa.alter_policy_info(''ow_inst_portions'', ''FILIAL'', ''M'', ''M'', ''M'' , ''M''); end;';
exception when others then
  if sqlcode = -06550 then null; else raise; end if;
end;
/ 
begin
  execute immediate 'begin bpa.alter_policy_info(''ow_inst_portions'', ''CENTER'', null, ''E'', ''E'' , ''E''); end;';
exception when others then
  if sqlcode = -06550 then null; else raise; end if;
end;
/ 
BEGIN
execute immediate 'create table ow_inst_portions (
chain_idt number,
status varchar2(30),
eff_date date,
due_date date,
...
KF varchar2(6 BYTE) default sys_context(''bars_context'',''user_mfo'') CONSTRAINT CC_OW_INST_POTRION_KF_NN NOT NULL) 
tablespace BRSMDLD
PARTITION BY LIST (KF) 
 ...
 END;
 /
COMMENT ON TABLE BARS.OW_INST_PORTIONS IS 'Графік сплати Інстолмент в розрізі порцій';

COMMENT ON COLUMN BARS.OW_INST_PORTIONS.CHAIN_IDT IS 'ІД графіка';

COMMENT ON COLUMN BARS.OW_INST_PORTIONS.STATUS IS 'Сдатус графіка:  
...
COMMENT ON COLUMN BARS.OW_INST_PORTIONS.EFF_DATE IS 'Дата виставления до погашення';

COMMENT ON COLUMN BARS.OW_INST_PORTIONS.DUE_DATE IS 'Кінцевий термін погашення';

COMMENT ON COLUMN BARS.OW_INST_PORTIONS.REP_DATE IS 'Дата погашення';

COMMENT ON COLUMN BARS.OW_INST_PORTIONS.SEQ_NUMBER IS 'Номер порції';

COMMENT ON COLUMN BARS.OW_INST_PORTIONS.TOTAL_AMOUNT IS 'Загальна сума порції';

COMMENT ON COLUMN BARS.OW_INST_PORTIONS.AMOUNT_TO_PAY IS 'Сума до сплати';

COMMENT ON COLUMN BARS.OW_INST_PORTIONS.WRITTEN_OFF_AMOUNT IS 'Сума списання';

COMMENT ON COLUMN BARS.OW_INST_PORTIONS.OVERDUE_AMOUNT IS 'Сума прострочки';

COMMENT ON COLUMN BARS.OW_INST_PORTIONS.KF IS 'МФО';
/
begin
  execute immediate 'begin BARS_POLICY_ADM.ALTER_POLICIES(''ow_inst_portions''); end;';
exception when others then
  if sqlcode = -06550 then null; else raise; end if;
end;
/
begin   
 execute immediate 'ALTER TABLE BARS.OW_INST_PORTIONS ADD 
CONSTRAINT OW_INST_PORTIONS_PK
 PRIMARY KEY (CHAIN_IDT, SEQ_NUMBER)
 ENABLE
 VALIDATE';
exception when others then
...
begin   
 execute immediate ' 
ALTER TABLE BARS.OW_INST_PORTIONS ADD 
CONSTRAINT OW_INST_PORTIONS_FK
...
/
grant all on ow_inst_portions to bars_access_defrole;
/
grant all on ow_inst_portions to bars;
/