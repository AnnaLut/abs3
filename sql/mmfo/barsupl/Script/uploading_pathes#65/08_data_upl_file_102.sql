...
begin
   ...
   dbms_output.put_line(':   SQL # 1102');
end;
/
-- ***************************************************************************
-- Оптимизация ежедневной выгрузки счетов
-- 
-- ***************************************************************************
...
-- UPL_SQL
-- ***********************
delete from BARSUPL.UPL_SQL where SQL_ID IN (1102);
       ...
       (select *
         ...
         where idupd in (
                         select max(idupd) idupd
                          ...
                          where au21.idupd in (
                                                select /*+ INDEX (au22 IDX_ACCOUNTSUPD_GLD_EFFD)*/ max(idupd)
                                                ...
                                                select max(idupd)  --по отдельным балансовым всегда текущее состояние только для ГРЦ
                                                 ...
                                                 group by a.acc
                                                union all
                                                select /*+ INDEX (au23 IDX_ACCOUNTSUPD_GLD_EFFD)*/ max(idupd) --открытые задним числом и реанимированые
...
Insert into UPL_SQL (sql_id, sql_text, before_proc, after_proc, descript, vers)
 Values
   (1102, l_clob, 'begin execute immediate ''begin barsupl.bars_upload_usr.tuda; end;''; exception when others then if sqlcode = -6550 then null; else raise; end if; end;', NULL, 'Счета банка', '7.11');
