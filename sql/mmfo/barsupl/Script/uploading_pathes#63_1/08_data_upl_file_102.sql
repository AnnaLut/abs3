...
/
-- ***************************************************************************
-- всгда полная выгрузка на дату для груп счетов '300%', '301%'
-- COBUPRVNIXIII-25  При вивантаженні дельти рахунків до DWH, забезпечити повне вивантаження груп рахунків 300% та 301%
-- ***************************************************************************
...
Insert into UPL_SQL (sql_id, sql_text, before_proc, after_proc, descript, vers)
 Values (102, l_clob, 'begin execute immediate ''begin barsupl.bars_upload_usr.tuda; end;''; exception when others then if sqlcode = -6550 then null; else raise; end if; end;', NULL, 'Счета банка', '7.10');
...
Insert into UPL_SQL (sql_id, sql_text, before_proc, after_proc, descript, vers)
 Values (1102, l_clob, 'begin execute immediate ''begin barsupl.bars_upload_usr.tuda; end;''; exception when others then if sqlcode = -6550 then null; else raise; end if; end;', NULL, 'Счета банка', '7.10');
                         ...
                         select max(idupd) idupd
                          ...
                          where au21.idupd in (
                                                select max(idupd)
                                                 ...
                                                 group by acc
                                                union all
                                                select max(idupd)  --по отдельным балансовым всегда полная выгрузка на дату
                                                  from bars.accounts_update au25, bars.accounts a, dt
                                                 where (a.nls like ''14%''   or a.nls like ''31%''  or a.nls like ''32%'' or
                                                        a.nls like ''33%''   or a.nls like ''41%''  or a.nls like ''42%'' or
                                                        a.nls like ''3541%'' or a.nls like ''300%'' or a.nls like ''301%'')
...
Insert into UPL_SQL (sql_id, sql_text, before_proc, after_proc, descript, vers)
 Values
   (6102, l_clob, 'begin execute immediate ''begin barsupl.bars_upload_usr.tuda; end;''; exception when others then if sqlcode = -6550 then null; else raise; end if; end;',
    NULL, 'Счета банка от глобальной БД -3 дня до глобальной БД (для IFRS9)', '2.1');
