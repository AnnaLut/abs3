PROMT --Дані про структуру активів та пасивів за строками

Declare
	Newfunc  number;
begin
	Newfunc  := BARS.OPERLIST_ADM.ADD_NEW_FUNC  (
	p_name=>'Вивантаження A7', p_funcname=>'/barsroot/ndi/referencebook/GetRefBookData/?accessCode=0=[PROC=>BARSUPL.BARS_UPLOAD_USR.create_interface_job(''GR'',18,1,0,:date_to)][PAR=>:date_to(SEM=За дату,TYPE=D)][EXEC=>BEFORE] [showDialogWindow=>false][CONDITIONS=> KF = sys_context(''bars_context'',''user_mfo'')]',
	p_usearc=> 1, 
	p_frontend=>1, 
	p_forceupd=>1,
	p_runnable=> 1,
	p_parent_function_id=>1868);
commit;
end;
/
...
--bars.bars_audit.error(l_trace||'333_ bars.gl.kf=' || nvl(bars.gl.kf, 'NULL') );
   ...
   barsupl.bars_upload_usr.routine_for_job( '||l_bnkdtstr||' , '||l_groupid||','||l_usearc||','||l_useftp||','||l_usecopy||','||p_sheduled||','''||l_kf||''');
    ...
    --
    -----------------------------------------------------------------
    procedure create_interface_job ( p_type       varchar2,
									 p_group_id   number,
                                     p_enabled    number,
                                     p_sheduled   number,
                                     p_bankdate   date default null
    ...
    begin
       ...
       begin
         ...
         where VAL.PARAM = 'GROUPID' and VAL.VALUE = 18;
       exception when others then
           l_jobname:=null;
       end;
       if (l_jobname is not null) then
           l_groupid  :=p_group_id;
           -- bars_upload.get_job_param(p_jobname, 'GROUPID');
           l_usearc   := bars_upload.get_job_param(l_jobname, 'USE_ARCH');
           l_usecopy  := bars_upload.get_job_param(l_jobname, 'USE_COPY');
           l_useftp   := bars_upload.get_job_param(l_jobname, 'USE_FTP');
           l_kf       := bars_upload.get_param('KF');
           l_bnkdtstr := case when p_bankdate is null then 'null' else 'to_date('''||to_char(p_bankdate,'dd/mm/yyyy')||''',''dd/mm/yyyy'')' end;
           ...
           if p_sheduled = 1 then
              l_interval := 'FREQ=DAILY;BYDAY='||bars_upload.get_job_param(l_jobname, 'WHEN_DAYLIST')||';'||
                                               'BYHOUR='  ||bars_upload.get_job_param(l_jobname, 'WHEN_HOUR')||';'||
                                               'BYMINUTE='||bars_upload.get_job_param(l_jobname, 'WHEN_MINUTE')||';BYSECOND=0';
              l_real_jobname := l_jobname;
           else
             l_interval := null;
             l_real_jobname := l_jobname||'_onetime';
           end if;
