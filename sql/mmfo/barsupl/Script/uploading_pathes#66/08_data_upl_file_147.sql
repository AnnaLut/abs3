...
/
-- ***************************************************************************
-- TSK-0000786 UPL - выгрузить данные по тэгу NUMKF в таблицу SRC_CCVALS
-- TSK-0001369 UPL - добавить файлы в  99 группу для иништиал выгрузки 
-- ***************************************************************************
...
l_clob:= to_clob('with dt  as ( select /*+ materialize */ bars_upload_usr.get_last_work_date(to_date (:param1, ''dd/mm/yyyy'')) + 1 dt1, to_date (:param1, ''dd/mm/yyyy'') dt2 from dual ),
  ...
  tg_c_f as ( select /*+ materialize */ unique trim(tag) tag, ref_id from barsupl.upl_tag_lists where tag_table in (''CC_TAGS'', ''CD_TAGS'') and coalesce(ref_id, 0) = 0 and isuse = 1
                                                                                                  and tag in (''NUMKF'')),
    txt  as ( select /*+ materialize */ tu.nd, tu.tag, tu.txt, tu.kf, decode(chgaction, 2, ''U'', 3, ''D'', ''I'') as chgaction
               ...
               where tu.idupd in (select max(idupd)
                                   ...
                                   group by u.nd, u.tag)),
      h  as ( select /*+ materialize */ kf, nd, case vidd when 10 then 10 when 110 then 10 when 26 then 19 else 3 end as nd_type
               ...
               where cu.idupd in (select max(u.idupd) idupd
                                   ...
                                   where u.effectdate <= dt.dt2
                                     and u.nd = txt.nd
                                   group by u.nd)),
  txt_f  as ( select /*+ materialize */  tu.nd, tu.tag, tu.txt, tu.kf, decode(chgaction, 2, ''U'', 3, ''D'', ''I'') as chgaction
                from bars.nd_txt_update tu
               where tu.idupd in (select max(idupd)
                                    from dt, bars.nd_txt_update u
                                   ...
                                   where u.effectdate <= dt.dt2
                                   group by u.nd, u.tag)),
    h_f  as ( select /*+ materialize */ kf, nd, case vidd when 10 then 10 when 110 then 10 when 26 then 19 else 3 end as nd_type
                from bars.cc_deal_update cu
               where cu.idupd in (select max(u.idupd) idupd
                                   ...
                                   group by u.nd)
               union
              select /*+ materialize */ distinct kf, nd, 10 as nd_type from bars.acc_over_update )
--delta
...
Insert into BARSUPL.UPL_SQL (SQL_ID, SQL_TEXT, BEFORE_PROC, AFTER_PROC, DESCRIPT, VERS)
 Values (2147, l_clob, 'begin execute immediate ''begin barsupl.bars_upload_usr.tuda; end;''; exception when others then if sqlcode = -6550 then null; else raise; end if; end;',
 NULL, 'Значення додаткових реквізитів угод банку (delta+full по тегу NUMKF)', '1.1');
end;
/
