 ...
 PROMPT ===================================================================================== 
  ...
  CREATE OR REPLACE PACKAGE BARSUPL.BARS_UPLOAD is
    ...
    (  colcnt      number,       -- кол-во колонок
       ...
       prefun      vrchr_array   -- массив кодов функций предварительной подготовки данных
    );
    ...
    procedure add_table_to_upload(
                        ...
                        p_endline     varchar2     default '10' , --'13||10'
...
--    end;
   ...
   --    trunc_pk_kf
   --
   --    Функция отрезания последних двух символов в PK ММФО
   --
   --    p_data - обрабатываемые данные
   -----------------------------------------------------------------
   function trunc_pk_kf(p_data varchar2) return varchar2
   is
   begin
      --не обрезать если длинна поля меньше 3-х символов, иначе на выходе получаем NULL
      if lower(trim(p_data)) = 'null' or length(trim(p_data)) < 3 or trim(p_data) = '' or p_data is null then return(trim(p_data)); end if;
      return substr(trim(p_data), 1, length(trim(p_data))-2);
   end;

   -----------------------------------------------------------------
   --    PREFUN
   --
   --    Функция дополнительной обработки полей при выгрузке
   --
   --    p_data - обрабатываемые данные
   --    p_prefun - код фенкции из UPL_PREFUN
   -----------------------------------------------------------------
   function prefun(p_data varchar2, p_prefun varchar2) return varchar2
   is
   begin
      case p_prefun
      when 'TRUNC_E2' then
           return bars_upload.trunc_pk_kf (p_data);
      else
           return p_data;
      end case;
   end;
...
/*
    ...
    */
    ...
    is
       ...
       l_format        varchar2(100);
       l_nullval       varchar2(10);
       l_endline       upl_files.endline%type;
    ...
    begin
       ...
       l_endline := ''; --парсим символ конца строки
       if p_endline is not null
       then
           for m in ( select column_value val from table(bars.gettokens( replace(p_endline, '||', ',') ) ) )
            loop
                l_endline := l_endline ||chr(to_number(m.val));
            end loop;
        else
           l_endline := chr(10);
        end if;
       ...
       while dbms_sql.fetch_rows(p_cur) != 0 loop
          ...
          for k in 1..p_cdesc.colcnt loop
             ...
             if ( nvl(get_param('USE_PREFUN'), '0') = '1' and p_cdesc.prefun is not null ) then
               l_tmp := bars_upload.prefun(l_tmp, p_cdesc.prefun(k));
             end if;

             if length(l_data) + nvl(length(p_delim),0) + nvl(length(l_tmp),0) + nvl(length(l_endline), 0) > G_BUFF_LENGTH - 1  then
                case when G_UPLOAD_METHOD = 'BUFF' then
                          i:= i+1;
                          utl_file.put(l_fileh, l_data);
                          utl_file.fflush(l_fileh);
    ...
    --  get_nextFileNumber
    --
    --    получить последовательный номер выгрузки для формирования имени файла
    --
    --    p_fileid      --  Код файла из справочника (UPL_FILES)
    --    p_param1      --  Текстовое значение параметра 1 (данные парметры будут использованы для bind переменных в запросах на выгрузку)
    --    p_param2      --  Текстовое значение параметра 2
    -----------------------------------------------------------------
    function get_nextFileNumber(p_fileid    in number,
    ...
    begin
      ...
      select cnt + 1 into l_nextnmbr
       ...
       where file_id = p_fileid
         ...
         for update of upl_file_counters.cnt
       wait 10;
    ...
    exception
      when no_data_found then
        ...
        values (p_fileid, to_date(p_param1,'dd/mm/yyyy'), 1);
        commit;
        return 1;
      when others then
        rollback;
...
--log_str('upload_file_data  upl_params   (p_fileid => ' || p_fileid || ', "UPL_METHOD")=' || l_uplmethod);
      ...
      if l_sqltext is not null then
          ...
          for c in ( select col_name, col_type, col_length, col_scale, col_format, repl_chars_with, prefun, count(*) over() cnt
                       from upl_columns
                      where file_id = p_fileid
                      order by col_id )
          loop
...
--      end if; --      if l_sql_before is not null then
--      if l_sqltext is not null then
          ...
          dbms_sql.close_cursor(l_cur);
      end if; --      if l_sqltext is not null then
      -- выполнить действие 'ПОСЛЕ'
      if l_sql_after is not null then
        begin
    ...
    begin
      ...
      log_start_process( p_start_time  => sysdate,
                         ...
                         p_parentid    => null,
                         p_sqlid       => l_sqlid,
                         p_params      => 'p1='||p_param1||' p2='||p_param2, --nvl( p_param2, to_char(l_uplfile_id, 'FM9999999')),
      ...
      upload_file_data( p_fileid       => l_uplfile_id,
                        p_sqlid        => l_sqlid,
                        p_param1       => p_param1,
                        p_param2       => p_param2, --nvl( p_param2, to_char(l_uplfile_id, 'FM9999999')),
                        p_rowsupl      => l_rowsupl,
                        p_filename     => l_filename);
  ...
  begin
    log_start_process( p_start_time => sysdate,
                       ...
                       p_parentid   => p_parentid,
                       p_sqlid      => p_sqlid,
                       p_params     => 'p1='||p_param1||' p2='||nvl( p_param2, to_char(p_filegroup, 'FM9999999')), --p_param2,
                       p_bankdate   => to_date(p_param1,'dd/mm/yyyy'),
                       p_id         => l_statfl_id );
    ...
    upload_file_data( p_fileid   => p_fileid,
                      p_sqlid    => p_sqlid,
                      p_param1   => p_param1,
                      p_param2   => nvl( p_param2, to_char(p_filegroup, 'FM9999999')), --p_param2,
                      p_rowsupl  => l_rowsupl,
                      p_filename => l_filename );
...
--log_str('upload_stat_file_2 p_fileid => ' || l_fileid || ', group=' || p_filegroup || ', group_param=' || get_group_param(p_filegroup, 'UPL_METHOD') || ', file_param=' || get_file_param(l_fileid, 'UPL_METHOD'));
       ...
       l_statfl_id := upload_file(
                      ...
                      p_defsqlid   => 0,
                      p_param1     => p_param1,
                      p_param2     => to_char(p_filegroup, 'FM9999999'), -- null,
                      p_forcestop  => 0,
                      p_fileid     => l_fileid,
                      p_sqlid      => l_sqlid,
                      p_critical   => 1
   ...
   is
      ...
      l_out   number;
   begin
      bars_upload_utl.check_kf(0, l_out); --пользователь долженбыть на уровне МФО
      if l_out = 0 then
         bars.bars_error.raise_nerror(G_MODULE,'USER_NOT_LOGING');
      end if;
      ...
      if G_PARAMS_LIST.count  = 0 or p_force = 1 then
             for c in (select trim(param) param, value, kf from upl_params)
             loop
                 G_PARAMS_LIST(c.param) := c.value;
                 if c.param = 'REGION_PRFX' then --
                    G_PARAMS_LIST('KF') := c.kf;
                    bars_audit.info(l_trace||'KF'||'-'||c.kf);
                 end if;
      ...
      if not G_PARAMS_LIST.exists('KF')  then
         bars.bars_error.raise_nerror(G_MODULE,'USER_NOT_LOGING');
      end if;
...
--log_str('init_file_params   G_FILEPARAM_LIST(' || c.file_id || ')(' || c.param || ') ' || G_FILEPARAM_LIST(c.file_id)(c.param));
   ...
   --
   procedure init_params(p_force number) is
        l_out number;
   begin
      bars_upload_utl.check_kf(0, l_out); --пользователь долженбыть на уровне МФО
      if l_out = 0 then
