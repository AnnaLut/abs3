...
is
  ...
  function body_version return varchar2;
    ...
    -----------------------------------------------------------------
    --  CHECK_KF ()
    --  Проверка на заполнение контекста
    --  p_force - 1/0 генерить/не генерить ошибку
    --  p_out   -   0     - контекст не заполнен
    --             -1     - значение bars.gl.kf не соответствует значению из параметров настройки
    --              1     - контекст заполнен
    -----------------------------------------------------------------
...
is
    ...
    begin
       ...
       -- содать
       bars.bars_audit.info(l_trace||'создание джоба '||l_jobname||' для выполнения '||l_batpathname);
