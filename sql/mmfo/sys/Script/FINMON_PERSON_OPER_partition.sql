prompt Скрипт для секционирования таблицы finmon.person_oper

prompt create interim table
create table FINMON.TEST_PERSON_OPER
tablespace FMNTBS
  pctfree 10
  initrans 1
  maxtrans 255
...
as select * from finmon.person_oper
;
...
prompt start redef
BEGIN
  DBMS_REDEFINITION.START_REDEF_TABLE('FINMON','PERSON_OPER','TEST_PERSON_OPER');
END;
/
prompt copy deps
DECLARE
...
BEGIN
  DBMS_REDEFINITION.COPY_TABLE_DEPENDENTS ('FINMON',  'PERSON_OPER', 'TEST_PERSON_OPER',
...
prompt sync
BEGIN
  DBMS_REDEFINITION.SYNC_INTERIM_TABLE ('FINMON', 'PERSON_OPER', 'TEST_PERSON_OPER');
END;
/
prompt finish redef
BEGIN
  DBMS_REDEFINITION.FINISH_REDEF_TABLE ('FINMON', 'PERSON_OPER', 'TEST_PERSON_OPER');
END;
/
prompt drop interim table
drop table FINMON.test_person_oper cascade constraints purge;
prompt return fgac
BEGIN
