...
-- Create table
begin
    execute immediate 'create table ADR_SETTLEMENTS
(
...
comment on table ADR_SETTLEMENTS
  is 'Довідник населених пунктів';
-- Add comments to the columns 
comment on column ADR_SETTLEMENTS.settlement_id
  is 'Ідентифікатор населеного пункту (equal to "SPIU.SUMMARYSETTLEMENTS.SUMMARYSETTLEMENTID"';
comment on column ADR_SETTLEMENTS.settlement_name
  is 'Назва населеного пункту';
comment on column ADR_SETTLEMENTS.settlement_name_ru
  is 'Назва населеного пункту';
comment on column ADR_SETTLEMENTS.settlement_type_id
  is 'Ідентифікатор типу населеного пункту';
comment on column ADR_SETTLEMENTS.postal_code_min
  is 'Мінімальний індекс для населеного пункту';
comment on column ADR_SETTLEMENTS.postal_code_max
  is 'Максимальний індекс для населеного пункту';
comment on column ADR_SETTLEMENTS.region_center_f
  is 'Чи є населений пункт обласним центром';
comment on column ADR_SETTLEMENTS.area_center_f
  is 'Чи є населений пункт районним центром';
comment on column ADR_SETTLEMENTS.region_id
  is 'Ід. області (для міст обласного підпорядкування)';
comment on column ADR_SETTLEMENTS.area_id
  is 'Ід. району';
comment on column ADR_SETTLEMENTS.koatuu
  is 'Код населеного пункту згідно класифікатора об`єктів адміністративно-територіального устрою України (КОАТУУ)';
comment on column ADR_SETTLEMENTS.terrstatus
  is 'Спеціальний статус території (Зона АТО, Тимчасово окупована територія - Крим)';
comment on column ADR_SETTLEMENTS.eff_dt
  is 'The date from which an instance of the entity is valid.';
comment on column ADR_SETTLEMENTS.end_dt
  is 'The date after which an instance of the entity is no longer valid.';
comment on column ADR_SETTLEMENTS.settlement_pid
  is 'Previous Ідентифікатор населеного пункту';
/
begin
-- Create/Rebegin
    execute immediate 'create index I_ADRSETTLEMENTS_UNAME on ADR_SETTLEMENTS (UPPER(SETTLEMENT_NAME))
  tablespace BRSSMLI
  pctfree 10
  initrans 2
  maxtrans 255
...
begin
    execute immediate 'create index I_ADRSETTLEMENTS_UNAMERU on ADR_SETTLEMENTS (UPPER(SETTLEMENT_NAME_RU))
  tablespace BRSSMLI
  pctfree 10
  initrans 2
  maxtrans 255
...
begin
  ...
  references ADR_PHONE_CODES (PHONE_CODE_ID)';
 exception when others then 
...
begin
  ...
  references ADR_SETTLEMENT_TYPES (SETTLEMENT_TP_ID)';
 exception when others then 
    if sqlcode = -2275 then null; else raise; 
    end if; 
end;
