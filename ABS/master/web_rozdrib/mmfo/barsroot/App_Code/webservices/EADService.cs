  ...
  FROM (SELECT d.id,
    ...
    public struct UClient
    {
        ...
        public static UClient GetInstance(String ObjID, OracleConnection con)
        {
            ...
            cmd.CommandText = @"select    t1.rel_rnk as rnk
                                      ...
                                      where t1.rnk = :p_rnk
                                            and t1.rel_id > 0
                                            and t1.REL_INTEXT = 1";//лише клієнти банку
            cmd.Parameters.Clear();
            cmd.Parameters.Add("p_rnk", OracleDbType.Int64, Convert.ToInt64(ObjID), ParameterDirection.Input);
    ...
    public struct Third_Persons_Clients
    {
        ...
        public Third_Persons_Clients(Int64 Rnk, Int16 PersonStateID, DateTime? BDate, DateTime? EDate)
        {
            this.Rnk = Rnk;
        ...
        public static Agr GetInstance(String ObjID, OracleConnection con)
        {
            ...
            switch (AgrType)
            {
                                  ...
                                  FROM (SELECT *
                                       ...
                                       (SELECT *
                                          FROM (  SELECT *
                                                ...
                                                ORDER BY idupd DESC)
                                         WHERE ROWNUM = 1) t2
                                 WHERE t1.rnk <> t2.rnk";//в выборку не должны попадать дублирующие записи.
                ...
                case "WAY":
                    ...
                    cmd.CommandText = @"SELECT t1.nd AS agr_code,
                                 t3.rnk,
                                 T1.CHGDATE AS changed,
                                 T3.DAOS AS created,
                                 t3.branch AS branch_id,
                                 t2.logname AS user_login,
                                 t2.fio AS user_fio,
                                 'bpk_fo' AS agr_type,
                                 CASE
        ...
        public static UAgr GetInstance(String ObjID, OracleConnection con)
        {
            ...
            switch (AgrType)
            {
                ...
                case "ACC":
                    ...
                    cmd.CommandText = @"SELECT sp.nkd AS agr_code,
                                               a.rnk AS rnk,
                                               ...
                                               sb.fio AS user_fio,
                                               'pr_uo' AS agr_type,
                                               CASE WHEN a.dazs IS NULL OR a.dazs > SYSDATE THEN 1 ELSE 0 END
                                                  AS agr_status,
                                               sp.nkd AS agr_number,
