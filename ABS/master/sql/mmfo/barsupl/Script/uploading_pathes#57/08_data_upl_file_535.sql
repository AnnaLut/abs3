...
select BARS.GL.KF, 4 as ND_TYPE, b1.ND,
       case
         when b1.TAG = ''VNCRR''   then 9
         when b1.TAG = ''BUS_MOD'' then 14
         when b1.TAG = ''SPPI''    then 15
         when b1.TAG = ''IFRS''    then 16
         when b1.TAG = ''INTRT''   then 17
         when b1.TAG = ''ND_REST'' then 18
         else null
       end as PAR_ID,
       b1.VALUE as PAR_VAL
 ...
 where b1.IDUPD in ( select max(IDUPD)
                      ...
                      where TAG in ( ''VNCRR'', ''BUS_MOD'', ''SPPI'', ''IFRS'', ''INTRT'', ''ND_REST'' )
                        and EFFECTDATE between dt.dt1 and dt.dt2
                      group by ND, TAG )
 UNION ALL
select KF, ND_TYPE, ND,
...
select KF, ND_TYPE, ND,
  ...
  from SCRTS
 where IR is Not Null
 UNION ALL  --параметры по ЦБ
select BARS.GL.KF, 9 as ND_TYPE, sd.ND, p.param_id, cw.VALUE as PAR_VAL
  from SCRTD sd
...
Insert into BARSUPL.UPL_SQL (sql_id, sql_text, before_proc, after_proc, descript, vers)
 Values
   (535, l_clob, 'begin execute immediate ''begin barsupl.bars_upload_usr.tuda; end;''; exception when others then if sqlcode = -6550 then null; else raise; end if; end;',
     NULL, 'Змінені параметри договорів (для FineVare)', '2.3');
end;
/
...
Insert into UPL_CONSTRAINTS (file_id, constr_name, priority, fk_fileid) Values (535, 'agrmchg0(ND_TYPE,ND,KF)_$_credkz(TYPE,ND,KF)', 1, 161);

-- ***********************
-- UPL_CONS_COLUMNS
-- ***********************
delete from BARSUPL.UPL_CONS_COLUMNS where FILE_ID IN (&&sfile_id);
Insert into UPL_CONS_COLUMNS (file_id, constr_name, fk_colid, fk_colname) Values (535, 'agrmchg0(ND_TYPE,ND,KF)_$_credkz(TYPE,ND,KF)', 1, 'ND_TYPE');
Insert into UPL_CONS_COLUMNS (file_id, constr_name, fk_colid, fk_colname) Values (535, 'agrmchg0(ND_TYPE,ND,KF)_$_credkz(TYPE,ND,KF)', 2, 'ND');
Insert into UPL_CONS_COLUMNS (file_id, constr_name, fk_colid, fk_colname) Values (535, 'agrmchg0(ND_TYPE,ND,KF)_$_credkz(TYPE,ND,KF)', 3, 'KF');
