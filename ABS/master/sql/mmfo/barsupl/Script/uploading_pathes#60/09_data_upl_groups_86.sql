...
--define sgroup_id = 86
--define sautojob  = 'UPDATE_TABLE_SYNC'
--define sjobdesc  = 'Узгодження данних історичних таблиць'
begin
   dbms_output.put_line(to_char(sysdate(), 'dd.mm.yyyy hh24:mi:ss') ||': Process for GROUP_ID: 86');
   dbms_output.put_line(':   GROUP # UPDATE_TABLE_SYNC  Узгодження данних історичних таблиць');
end;
/
-- ***************************************************************************
...
declare
  ...
  l_sql             varchar2(500);
begin
  l_job_desc := 'Узгодження данних історичних таблиць';
  l_group_id := 86;

  --UPL_GROUPS
  delete from barsupl.upl_groups WHERE group_id = l_group_id;
  insert into barsupl.upl_groups(group_id, descript)    values(l_group_id,'Узгодження данних історичних таблиць');

  for l_kf in (select kf from bars.mv_kf)
  loop
      dbms_output.put_line(to_char(sysdate(), 'dd.mm.yyyy hh24:mi:ss') ||': Process for UPL_GROUPS.GROUP_ID: 86');
      bars.bc.go(l_kf.kf);
...
select * from barsupl.upl_groups WHERE group_id = 86;

select * from barsupl.upl_autojobs WHERE job_name like 'UPDATE_TABLE_SYNC%' order by job_name;

select * from barsupl.upl_autojob_param_values WHERE job_name like 'UPDATE_TABLE_SYNC%' order by job_name;
