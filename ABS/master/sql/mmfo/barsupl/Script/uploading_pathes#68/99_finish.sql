...
begin
  execute immediate 'alter table UPL_FILE_COUNTERS   enable constraint FK_UPLFILECOUNTS_FILEID';
  exception when OTHERS then if sqlcode=-02431 or sqlcode=-00942 then null; else raise; end if;
end;
/

begin
  execute immediate 'alter table UPL_CONSTRAINTS     enable constraint FK_UPLCONSTRAINTS_FILEID';
  exception when OTHERS then if sqlcode=-02431 or sqlcode=-00942 then null; else raise; end if;
end;
/

begin
  execute immediate 'alter table UPL_CONSTRAINTS     enable constraint FK_UPLCONSTRAINTS_FKFILEID';
  exception when OTHERS then if sqlcode=-02431 or sqlcode=-00942 then null; else raise; end if;
end;
/

begin
  execute immediate 'alter table UPL_STATS           enable constraint FK_UPLSTATSFILEID';
  exception when OTHERS then if sqlcode=-02431 or sqlcode=-00942 then null; else raise; end if;
end;
/
...
-- CONSTRAINTS
begin
  execute immediate 'alter table UPL_CONS_COLUMNS    enable constraint FK_UPLCONSCOLUMNS_FILEID';
  exception when OTHERS then if sqlcode=-02431 or sqlcode=-00942 then null; else raise; end if;
end;
/
...
-- COLUMNS
begin
  execute immediate 'alter table UPL_CONS_COLUMNS    enable constraint FK_UPLCONSCOLUMNS_FILEIDCOLN';
  exception when OTHERS then if sqlcode=-02431 or sqlcode=-00942 then null; else raise; end if;
end;
/
...
-- FILEGROUPS_RLN
begin
  execute immediate 'alter table UPL_FILEGROUPS_RLN  enable constraint FK_GROUPID';
  exception when OTHERS then if sqlcode=-02431 or sqlcode=-00942 then null; else raise; end if;
end;
/
...
-- TAG_LISTS
begin
  execute immediate 'alter table UPL_TAG_LISTS enable constraint FK_UPLTAGLISTS_TAGTABLE';
  exception when OTHERS then if sqlcode=-02431 or sqlcode=-00942 then null; else raise; end if;
end;
/

begin
  execute immediate 'alter table UPL_TAG_LISTS enable constraint FK_UPLTAGLISTS_UPLTAGREF';
  exception when OTHERS then if sqlcode=-02431 or sqlcode=-00942 then null; else raise; end if;
end;
/
...
-- SOURCES_TRIGGER
begin
  execute immediate 'alter table BARSUPL.UPL_SOURCES_TRIGGER enable constraint FK_UPL_SOURCES_TRIGGER';
  exception when OTHERS then if sqlcode=-02431 or sqlcode=-00942 then null; else raise; end if;
end;
/
...
--
begin
  for l_kf in (select kf from bars.mv_kf where kf in ('300465', '322669'))
  loop
    bars.bc.go(l_kf.kf);
