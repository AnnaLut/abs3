...
create or replace type body t_core_pledge is
    ...
    begin
        ...
        if (p_core_object is of (t_core_pledge)) then
            ...
            l_equals := bars.tools.equals(l_core_pledge.customer_id        , customer_id        ) and
    ...
    begin
        ...
        l_attributes(14) := json_utl.make_json_value('real6income', real6income, p_mandatory => false);
        l_attributes(15) := json_utl.make_json_value('noreal6income', noreal6income, p_mandatory => false);

        l_attributes(16) := json_utl.make_json_value('flagInsurancePledge', nvl(flaginsurancepledge, 'false'), p_mandatory => true);
        ...
        if (deposit is not null and deposit is not empty) then
            ...
            l_attributes(17) := json_utl.make_json_value('deposit', '[' || bars.tools.words_to_string(l_deposit_attributes, p_splitting_symbol => ', ', p_ignore_nulls => 'Y') || ']');
        end if;
