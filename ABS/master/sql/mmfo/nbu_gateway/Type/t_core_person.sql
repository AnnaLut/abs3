...
create or replace type body t_core_person is
    ...
    begin
        ...
        if (documents is not null and documents is not empty) then
            ...
            l_attributes(6) := json_utl.make_json_value('document', '[' || bars.tools.words_to_string(l_document_attributes, p_splitting_symbol => ', ', p_ignore_nulls => 'Y') || ']');
        end if;
        ...
        l_attributes(9) := json_utl.make_json_value('profit', '{ ' ||
