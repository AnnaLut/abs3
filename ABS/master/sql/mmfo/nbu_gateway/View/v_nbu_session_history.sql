...
select s.id,
      ...
      case when o.object_type_id in (1,2) then 'clientregister/registration.aspx?readonly=1'||chr(38)||'rnk='||c.core_customer_id
           when o.object_type_id=3 then (select 'CreditUi/provide/Index/?id='||nd as pledge_link from 
                                                                                             (select acc,nd,request_id,max(request_id) over (partition by acc,nd) max_req from nbu_gateway.core_pledge_dep) 
                                                                                               where request_id=max_req and acc=pl.core_pledge_id) 
           when o.object_type_id=4 then (select case when cc.type_credit=1 and cc.check_person=1 then 'CreditUi/NewCredit/?custtype=3'||chr(38)||'nd='||cc.nd
                                                     ...
                                                     when cc.type_credit=3 then 'ndi/referencebook/GetRefBookData/?code=V1_OVRN'||chr(38)||'jsonSqlParams=[{"Name":"ND","Type":"N","Value":'||cc.nd||'}]'
