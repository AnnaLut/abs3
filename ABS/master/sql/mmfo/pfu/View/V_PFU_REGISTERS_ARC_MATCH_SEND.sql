...
CREATE OR REPLACE VIEW v_pfu_registers_arc_match_send (
   arcdate,
   arcdateuser,
   id,
   receiver_mfo,
...
SELECT f.arcdate,
       ...
       (SELECT COUNT (1)
        FROM   pfu_file_records r
        WHERE      r.file_id = f.id
               AND r.state = (CASE f.state WHEN 'IN_PAY' THEN 20 ELSE 0 END)
...
-- Grants for View
GRANT SELECT ON v_pfu_registers_arc_match_send TO bars
/
GRANT SELECT ON v_pfu_registers_arc_match_send TO bars_access_defrole
/
GRANT SELECT ON v_pfu_registers_arc_match_send TO upld
/
GRANT SELECT ON v_pfu_registers_arc_match_send TO barsreader_role
/
