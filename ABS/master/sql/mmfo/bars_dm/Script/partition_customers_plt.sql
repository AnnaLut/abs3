...
prompt partition script:
declare
l_is_partitioned varchar2(10);
begin
    select partitioned into l_is_partitioned from user_tables t where t.table_name = 'CUSTOMERS_PLT';
    if l_is_partitioned = 'NO' then
        dbms_output.put_line('CREATE INTERIM TABLE');
        begin
            execute immediate '
        ...
        execute immediate 'alter table bars_dm.test_customers_plt modify merried varchar2(500)';
        execute immediate 'alter table bars_dm.test_customers_plt modify ADR_WORK_ADR VARCHAR2(100)';
        dbms_output.put_line('CREATE PARTITIONS');
        begin
        ...
        dbms_output.put_line('FILL TABLE IN PARALLEL');
        declare
        ...
        dbms_output.put_line('EXCHANGE TABLES'); 
        declare
        l_orig_cnt number;
        l_tmp_cnt number;
        begin
            select count(*) into l_orig_cnt from bars_dm.CUSTOMERS_PLT where  per_id>=1192;
            execute immediate 'select count(*) from bars_dm.test_CUSTOMERS_PLT' into l_tmp_cnt;
