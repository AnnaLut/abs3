...
PROMPT *** Create  trigger TBU_CCDEAL_K23 ***
  ...
  before update of FIN23, OBS23  ON BARS.CC_DEAL   for each row
   WHEN (new.FIN23 >0 and new.OBS23 >0 ) declare
...
begin
  ...
  If  :old.vidd like ('150%') then l_cus := 5; -- Для коррахунків
  else
  ...
  case WHEN substr(:new.prod,1,2) = '21' THEN
            l_cus   := 4;
            l_fin23 := least(l_fin23, 4);   -- бюджетні установи від 1 до 4 COBUPRVN-166

            if :new.FIN23 > 4 and nvl(:old.FIN23,0) <=4 then
               raise_application_error(-(20000+111),'\' ||'Заборонено встановлювати значення класу позичальника вище 4 для державних установ ',TRUE);
            end if;
       ELSE l_cus := l_cus;
  END CASE;
  ...
  Case l_flag when  1 then   -- приміняємо ручне коригування
