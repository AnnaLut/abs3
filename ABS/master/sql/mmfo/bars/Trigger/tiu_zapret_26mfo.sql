...
CREATE OR REPLACE TRIGGER TIU_ZAPRET_26MFO
...
BEGIN
   ...
   IF length(:new.tobo) = 8 and :new.tip <> 'W4I' and (:new.tip not in ('W4A', 'W4G') and :new.nbs is null )THEN
      erm := 'Заборонено відкриття/модифікація рахунків 26-ої групи на рівень МФО!';
      RAISE err;
   END IF;

   if length(:new.tobo) <= 15 and :new.tip <> 'W4I' and :new.tip like 'W4%'  and (:new.tip  not in ('W4A', 'W4G') and :new.nbs is null) then
