...
PROMPT *** Create  trigger TU_OPLDOK_SQ ***
  ...
  CREATE OR REPLACE TRIGGER BARS.TU_OPLDOK_SQ BEFORE UPDATE OF sos ON OPLDOK FOR EACH ROW
     WHEN (OLD.sos < 5 AND NEW.sos >=5) declare KV_ int;  acc_ number ; s_ number ; l_vob oper.vob%type;
BEGIN
...
/
ALTER TRIGGER BARS.TU_OPLDOK_SQ ENABLE;
