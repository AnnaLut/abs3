...
PROMPT ===================================================================================== 
PROMPT *** Run *** ========== Scripts /Sql/BARS/Procedure/CP_ZV8.sql =========*** Run *** ==
PROMPT ===================================================================================== 


PROMPT *** Create  procedure CP_ZV8 ***

  CREATE OR REPLACE PROCEDURE BARS.CP_ZV8 
...
--------------
 ...
 l_fl_p_7 int;               vob7 int;     R_sale number; r_int number;
...
l_userid := user_id;
LOG('CP_ZV: дата '||p_dat2||' p_id='||p_id||' frm='||p_frm||' u_id='||l_userid,'INFO',0);
logger.info('CP_ZV: дата '||p_dat2||' p_id='||p_id||' frm='||p_frm||' u_id='||l_userid);

begin
select acc,nls into acc_3739, nls_3739          -- ! редагувати
...
l_frm:=trim(p_frm);
--delete from tmp_cp_obu where 1=1 and userid=l_userid;   --id=p_id;
...
for k in (select e.ID, e.RYN,           
                 ...
                 ar.op, ar.ref_main,      -- rnbu/bars11  -bars1
...
if l_frm='8' and k.nbs1 in (1404, 1410, 1411, 1412, 1413, 1414, 1420, 1421, 1422, 1423, 1424, 1430, 1435, 1440)
   then  null;
elsif l_frm='7' and k.nbs1 in (3010, 3011, 3012, 3013, 3014, 3110, 3111, 3112, 3113, 3114, 3210, 3211, 3212, 3213, 3214)
   then null;
elsif l_frm='9' and k.nbs1 in (3002, 3003, 3005, 3006, 3102, 3103, 3105, 3106, 3122, 3123, 3125, 3132, 3133, 3135, 4102, 4103, 4105, 4202, 4203, 4205)
   then null;
...
tr.g066:=1;   tr.frm:=p_frm;
tr.g009:='Ні';
if p_frm='8' then tr.g009:='Так'; end if;
--- ......
tr_f:=tr;
    ...
    s_63:=0; tr_31:=0;  r_sale:=null; r_int:=null; 
    sumb_pr_7:=null; s_pr_op_7:=null;
    n2_k:=0; n2_p:=0;  n3_p:=null;  n2_p_7:=null;
...
l_kv:=k.kv;                          
s_pr := 0;            ------l_ref_mv := k.ref_new;    -- RNBU/OBU 
s_pr_op:=0;
...
logger.info('CP_ZV 1: ID='||k.id||' '||k.cp_id||' '||k.ref);
...
from oper where ref=l_ref_pr_9                       and sos=5; -- RNBU
...
---------
 ...
 tr.ref2:=nvl(l_ref_pr,l_ref_pr_9);
  ...
  kl_p_7:=round(sn_PR_7/nvl(cena7,1),2);  -- ?
...
if SD_ != 0 then
tr.g020:=SD_;  -- tr.g020:=SD_Q;
end if;
tr.g055:=SD_31;  -- tr.g055:=SD_31Q;
...
if SP_ != 0 then
tr.g020:=SD_+SP_;   --tr.g020:=SD_Q+SP_Q;
end if;
tr.g055:=SD_31+SP_31;   --tr.g055:=SD_31Q+SP_31Q;
...
if SR_ != 0 then
tr.g021:=SR_;   --tr.g021:=SR_Q;
end if;

tr.g056:=SR_31;   --tr.g056:=SR_31Q;
...
if SR2_ != 0 then
tr.g022:=SR2_;   --tr.g022:=SR2_Q;
end if;
tr.g057:=SR2_31;   --tr.g057:=SR2_31Q;
...
if SS_ != 0 then
tr.g025:=SS_;   --tr.g025:=SS_Q;
tr.s1:=SS_;
end if;
tr.g060:=SS_31;   --tr.g060:=SS_31Q;
                    tr.S31:=SS_31;
 ...
 -- r_int:=0;
     /****
  begin                  
 select sum(o.s)/100        -- по R/R2 при нарахуванні на 605 
 into   r_int 
 from opldok o   --, OPER P
 ...
 cena_p2:=cena_pr;
   ---
 if (k.dat_k < l_dat1 and  SN_ !=0) or k.dat_k >= l_dat1  
    or k.nbs1='1420' then
 begin
--
...
PROMPT *** Create  grants  CP_ZV8 ***
grant EXECUTE                                                                on CP_ZV8          to BARS_ACCESS_DEFROLE;
grant EXECUTE                                                                on CP_ZV8          to START1;



PROMPT ===================================================================================== 
PROMPT *** End *** ========== Scripts /Sql/BARS/Procedure/CP_ZV8.sql =========*** End *** ==
PROMPT ===================================================================================== 
