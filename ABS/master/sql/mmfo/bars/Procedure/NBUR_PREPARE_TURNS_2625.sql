CREATE OR REPLACE PROCEDURE BARS.NBUR_PREPARE_TURNS_2625 (
                                                       p_kf            varchar2
                                                       , p_report_date date
                                                    )
is
BEGIN
    ...
    select 'OLD' ACC_TYPE, t.fdat, t.kf, t.rnk, t.acc, t.nlsalt nls, t.kv, t.ob22alt ob22,
       ...
       nvl(sum(s.dos), 0) + decode(sgn, 1, t.ostf, 0) dos_repm,
       nvl(sum(s.dosq), 0) + decode(sgn, 1, t.ostqf, 0) dosq_repm,
       nvl(sum(s.kos), 0) + decode(sgn, -1, -t.ostf, 0) kos_repm,
       nvl(sum(s.kosq), 0) + decode(sgn, -1, -t.ostqf, 0) kosq_repm
    from temp_sel t
    left outer join snap_balances s
    on (t.acc = s.acc and
