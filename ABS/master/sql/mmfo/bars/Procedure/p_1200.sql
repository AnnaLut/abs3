...
/* Версия 3.2   04-09-2017  18-07-2017  07-02-2017  01-02-2017  
...
l_ead    rez_cr.ead%type     ; l_eadq   rez_cr.eadq%type     ; l_VNCRR    rez_cr.VKR%type    ;
l_bv     number; l_bvq number; l_idf    number; l_pd   number; l_dat31  date;
...
begin
   ...
   for k in ( select p_dat01 fdat,c.custtype,substr(c.nmk,1,35) nmk, 'NEW/' || acc id, - ost_korr(a.acc,l_dat31,null,a.nbs) bv, 1 fin, 1 kat, 
                     c.OKPO, DECODE (NVL (c.codcagent, 1), '2', 2, '4', 2, '6', 2, 1) RZ, case when d.tipa in (12, 30, 93) then 1 else 0 end PD_0,
                     d.tipa, a.*, d.tipa_FV  
   ...
   LOOP
      ...
      if k.tipa = 92 THEN  
         ...
         if       k.custtype = 1                THEN l_idf := 80; 
            elsif k.custtype = 1 and k.RZ = 1   THEN l_idf := 80; l_tip_fin := 1;  
      ...
      INSERT INTO NBU23_REZ ( ob22   , tip   , acc    , FDAT     , branch    , nls       , nmk   , RNK   , NBS   , KV    , ND    , ID    , 
                              ...
                              ead    , eadq  , fin_351, s080     , DDD_6B    , OKPO      , pd_0  , tipa)                                                                       
                     values ( k.ob22 , k.tip , k.acc  , p_dat01  , k.branch  , k.nls     , k.nmk , k.rnk , k.nbs , k.kv  , k.acc , k.id  , 
                              l_BV   , l_BVQ , l_fin  , k.kat    , k.daos    , k.custtype, 0     , 0     , 0     , 0     , 0     , 0     , 
                              l_ead  , l_eadq, l_fin  , l_s080   , l_DDD_6B  , k.OKPO    , k.pd_0, k.tipa_fv)    ;
      INSERT INTO REZ_CR    ( fdat   , RNK   , NMK    , ND       , KV        , NLS       , ACC   , EAD   , EADQ  , FIN   , PD    , CR    , 
                              CRQ    , bv    , bvq    , LGD      , CUSTTYPE  , nbs       , tip   , sdate , RZ    , s080  , ob22  , pd_0  , 
                              ccf    , istval, wdate  , ddd_6B   , tip_fin   , tipa      )                                            
                  VALUES    ( p_dat01, k.RNK , k.NMK  , k.acc    , k.kv      , k.nls     , k.acc , l_ead , l_eadq, l_fin , l_pd  , 0     , 
                              0      , l_bv  , l_bvq  , 1        , k.CUSTTYPE, k.nbs     , k.tip , k.daos, k.RZ  , l_s080, k.ob22, k.pd_0, 
                              l_ccf  , 0     , k.mdate , l_ddd_6B, l_tip_fin , k.tipa    ) ;  
