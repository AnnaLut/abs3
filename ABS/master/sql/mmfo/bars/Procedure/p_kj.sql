...
PROMPT ===================================================================================== 
PROMPT *** Run *** ========== Scripts /Sql/BARS/Procedure/P_KJ.sql =========*** Run *** ====
PROMPT ===================================================================================== 


PROMPT *** Create  procedure P_KJ ***

  CREATE OR REPLACE PROCEDURE BARS.P_KJ (id_ SMALLINT, sdat_ varchar2, sdat2_ varchar2 ) IS
...
CURSOR a0 IS
  SELECT acc,kv,nls||' '||nms FROM accounts WHERE tip='KAS';
...
CURSOR O1 IS
SELECT p.fdat,p.s,o.userid,p.ref,p.dk,o.nd,decode(p.tt,o.tt,o.sk,t.sk),p.tt,p.STMT
FROM OPER O, OPLDOK P, tts t
WHERE
    p.acc=acc_ AND p.fdat BETWEEN dat_ AND dat2_ AND
...
SELECT nls  FROM opl WHERE
    ref=ref_ and tt=tt_ AND dk=1-DK_ AND fdat=fdat_ and sos=5 AND kv=kv_ AND stmt=stmt_ AND
...
BEGIN
 ...
 OPEN O1; LOOP FETCH O1 INTO fdat_,s_,ud_,ref_,dk_,nd_,sk_,tt_,stmt_;
...
PROMPT ===================================================================================== 
PROMPT *** End *** ========== Scripts /Sql/BARS/Procedure/P_KJ.sql =========*** End *** ====
PROMPT ===================================================================================== 
