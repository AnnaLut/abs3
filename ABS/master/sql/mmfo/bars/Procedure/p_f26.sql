...
%
% VERSION     :   v.16.002     20.02.2017 (01.11.2016)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
CURSOR SALDO IS
   SELECT a.acc, a.nls, a.kv, a.fdat, a.nbs, c.country, c.codcagent,
          ...
          a.ostf-a.dos+a.kos, NVL(trim(sp.r013),'0')
   ...
   THEN
      ...
      if nbs_='9200' and Dat_ < to_date('07092009','ddmmyyyy') and 
         ...
         if k.s - NVL(sump_,0) > 0 then
            ...
            VALUES  (k.nlsd, k.kv, dat_, kodp_, to_char(k.s-sump_), acc_);
            ...
            if sq_ - sumpq_ > 0 then   
               ...
               INSERT INTO rnbu_trace
                       (nls, kv, odate, kodp, znap, acc)
               VALUES  (k.nlsd, k.kv, dat_, kodp_, to_char(sq_-sumpq_), acc_); 
         ...
         if k.s - NVL(sump_,0) > 0 then
            ...
            INSERT INTO rnbu_trace (nls, kv, odate, kodp, znap, acc) VALUES
                             (k.nlsd, k.kv, data_, kodp_, invk_, acc_);

            kodp_:= '98' || LPAD(to_char(cs1_),3,'0') || kb1_ || '00000000' ;
            INSERT INTO rnbu_trace (nls, kv, odate, kodp, znap, acc) VALUES
                                   (k.nlsd, k.kv, data_, kodp_, nb1_, acc_);

            kodp_:= '99' || LPAD(to_char(cs1_),3,'0') || kb1_ || '00000000' ;
            INSERT INTO rnbu_trace (nls, kv, odate, kodp, znap, acc) VALUES
                             (k.nlsd, k.kv, data_, kodp_, ' ', acc_);
        ...
        if se_ < 0 then
          ...
          if l_o_sum !=0  then
             ...
             INSERT INTO rnbu_trace
               ...
               VALUES (nls_, kv_, data_, kodp_, znap_, acc_, comm_);
      ...
      if se_ <> 0 or (kv_<> 980 and sn_ <> 0) then
         ...
         INSERT INTO rnbu_trace (nls, kv, odate, kodp, znap, acc) VALUES
                                (nls_, kv_, data_, kodp_, invk_, acc_);
         ...
         INSERT INTO rnbu_trace (nls, kv, odate, kodp, znap, acc) VALUES
                                (nls_, kv_, data_, kodp_, nb_, acc_);

         kodp_:= '99' || LPAD(to_char(cs_),3,'0') || kb_ || '00000000' ;
         INSERT INTO rnbu_trace (nls, kv, odate, kodp, znap, acc) VALUES
                                (nls_, kv_, data_, kodp_, rb_, acc_);
      end if;
