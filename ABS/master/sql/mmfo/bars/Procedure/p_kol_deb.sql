...
/* Версия 7.7  10-05-2018  29-01-2018  28-12-2017  25-10-2017  25-09-2017  18-09-2017  04-08-2017  11-07-2017   26-04-2017  09-03-2017
   Визначення Кількості днів прострочки та фін. стану по дебіторці
   p_deb = 0 - Звичайна дебіторка 
           1 - Нова хоз.дебіторка 
   ------------------------------------
15) 10-05-2018(7.7) - замена S080 по счетам 2805, 2806 с К на А,М (по типу контрагента) COBUMMFO-7717
   ...
   begin 
      ...
      loop
         ...
         if k.bv > 0 THEN
            ...
            --if l_tip  = 0 THEN  
            l_fin := f_rez_kol_fin_pd((case when k.deb = 3 THEN 2 else k.deb end), 1, l_kol);
            if l_tip <> 0 THEN 
               l_fin_cls := fin_nbu.zn_p_nd('CLS', l_f, p_dat01, l_nd, k.rnk);

               if    l_tip = 1 and l_fin = 2 THEN l_fin :=  5;
               elsif l_tip = 2 and l_fin = 2 THEN l_fin := 10;
               else                               l_fin := l_fin_cls;
               end if;   
