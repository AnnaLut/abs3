...
begin
   ...
   for k in (select substr(nls,1,4) nbs_n, 9 tipa, a.*, - A.OSTC OST -- -ost_korr(acc,l_dat31,null,nbs) ost
             from accounts a
             where  substr(nls,1,4) in ('1415','3115') and nbs is null and -a.ostc <>0 and tip='SR')
   LOOP
      l_pd_0 := 0;
      ...
      begin 
         select kk.emi into l_emi  from cp_deal d, cp_kod kk where ref = l_nd and  D.ID = KK.ID;
      EXCEPTION WHEN NO_DATA_FOUND THEN l_emi := null;     
      end;
      begin
      ...
      l_fin      := f_rnk_maxfin(p_dat01, F_RNK_gcif (cust.okpo, cust.rnk), l_tip_fin, l_nd, 1);
      ...
      begin
         insert into nbu23_rez  (fdat   , acc   , kv   , nbs    , bv   , bvq   , rez   , rezq    , rnk          , nd    , nls  , fin  , kat, dd  , ddd  , okpo     , nmk     ,
                                 rz     , ob22  , tip  , tipa   , vkr  , id    , s080  , ddd_6b  , custtype     , pd_0  )
                         values (P_dat01, k.acc , k.kv , k.nbs_n, k.ost, l_ostq, 0     , 0       , k.rnk        , l_nd  , k.nls, l_fin, 1  , l_dd, l_ddd, cust.okpo, cust.nmk,
                                 l_rz   , k.ob22, k.tip, k.tipa , l_vkr, l_id  , l_s080, l_ddd_6b, cust.custtype, l_pd_0);
