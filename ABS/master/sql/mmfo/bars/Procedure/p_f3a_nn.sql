...
% DESCRIPTION : Процедура формирования #3A для КБ (универсальная) с 01.06.2009
% COPYRIGHT   : Copyright UNITY-BARS Limited, 1999.  All Rights Reserved.
% VERSION     : 13/03/2017 (20/10/2016, 30/09/2016)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
15/08/2014 - в протокол формирования файла и протокол формирования контрольных
             ...
             для банка Петрокоммерц включаем в файл "похідні фін.інструменти"
...
23/07/2014 - для Демарка будем включать обороты с 2630,2635 на 2620 если
             разные хвосты
...
10/09/2013 - Розрахунок середньозваженої процентної ставки по кредитах, у
             яких можуть бути різні процентні ставки на протязі життя кредиту +
             для Демарка будемо включати обороти по кредитах/депозитах, що
             видаються та гасяться протягом одного банківського дня
06/09/2013 - Розрахунок середньозваженоъ процентноъї ставки по депозитах, у
             яких можуть бути різні процентні ставки на протязі життя вкладу
02/09/2013 - зміна структури показника
16.08.2013 - виключаємо обороти д.3739 - к. 2630,2635 з призначенням
             "перенесено залишки", "%зачислено%в связи с закрытием%"
30.04.2013 - для показателя процентной ставки изменен формат с '990D0000'
             на  '99990D0000' (проявилось в Крыму СБ)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%*/
   kodf_      VARCHAR2 (2)   := '3A';
   ...
   IS
      SELECT a.acc, a.nls, a.kv, a.FDAT,
        ...
        FROM (SELECT /*+parallel(AA)*/
                     s.acc, s.nls, s.kv, s.mdate, AA.FDAT, s.nbs, AA.ostf,
...
-- МФО "родителя"
   ...
   if flag_blk = 0 then
    ...
    --  Поточн_ рахунки ф_зичних ос_б
             ...
             -- так требует НБУ (например, в KOD_D6) и ПКБ
             if nbs_ IN ('2600', '2620', '2650') and se_ > 0 and
                mfo_ = 300120 and nvl(r011_, '0') in ('0', '2', '9')
             then
                s180new_ := '1';
                s181new_ := '1';
             end if;
    ...
    ----------------------------------------------------------------------------
       ...
       LOOP
          ...
          if nbs_ IN ('2600', '2620', '2650') and se_ > 0 and
             nvl(r011_, '0') in ('0', '2', '9') and 
             300120 in (mfo_, mfou_) 
          then
             s180_ := '1';
          end if;
          ...
          IF flag_ THEN
            ...
            -- для Сбербанка вычитаем из Кт оборотов капитализацию процентов
                ...
                IF (skos_ > 0 AND r050_ = '22' and se_ >= 0) OR
                   (skos_ > 0 and nbs_ = '2600' and r013p_ in ('1','7','8','A')) OR
                   (skos_ > 0 and nbs_ = '2605' and r013p_ in ('1','3')) OR
                   (skos_ > 0 and nbs_ = '2655' and r013p_ = '3') OR
                   (skos_ > 0 and nbs_ = '2650' and r013p_ in ('1','3','8'))
                THEN
                ...
                IF (s_prol_ > 0 AND r050_ = '22'  and se_ > 0) OR
                   (s_prol_ > 0 and nbs_ = '2600' and r013p_ in ('1','7','8','A')) OR
                   (s_prol_ > 0 and nbs_ = '2605' and r013p_ in ('1','3')) OR
                   (s_prol_ > 0 and nbs_ = '2655' and r013p_ = '3') OR
                   (s_prol_ > 0 and nbs_ = '2650' and r013p_ in ('1','3','8'))
                THEN
    ...
    --- если остаток на конец дня и на начало дня Дебетовый и разница больше нуля
       ...
       LOOP
          ...
          if  nd1_ = nd2_ then
              ...
              if se_ <> 0 then
                  spcntp_ := Acrn.FPROC(acc_, datp_ - 1);
                  spcntc_ := Acrn.FPROC(acc_, dat_);
              else
                  spcntp_ := 0;
                  spcntc_ := 0;
              end if;
