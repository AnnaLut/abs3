...
PROMPT ===================================================================================== 
PROMPT *** Run *** ========== Scripts /Sql/BARS/Procedure/P_FD5_NN.sql =========*** Run *** 
PROMPT ===================================================================================== 
...
%
% VERSION     : v.17.003      05.07.2017 (24.05.2017, 09.03.2017)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 10     Y          K072 код сектора економiки
 11     9          (K081)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 25/11/2013 - еквівалент по визнаним процентним доходам по валюті будуть 
 ...
 09/08/2013 - по просьбе Демарка для табл. ACC_OVER не выбираем гр. 22
 18/04/2013 - по просьбе Демарка для бал.счета 8026 добавил бал.счет 
              для корреспонденции 8029  
 19/02/2013 - доопрацювала блок вирівнювання резерву з балансом та блок
              формування резерву по овердрафту
 18/02/2013 - доопрацювала по зауваженнях від ГОУ
 04/02/2013 - коректне формування параметра K111 із довідника KL_K110
 27/01/2013 - для выбора категории качества из V_TMP_REZ_RISK добавил
              условие ROWNUM = 1
 21/01/2013 - дополнительно ищем в V_TMP_REZ_RISK только ACC счета
              без номера договора
 18/01/2013 - убрал обработку бал.счета 1592 и исключаем при расшифровке
              счетов резерва балюсчет 1500
 17/01/2013 - добавлена обрабока бал.счета 1592.
 16/01/2013 - разбивка счетов резерва выполняетмя по V_TMP_REZ_RISK
              VIEW по таблице NBU23_REZ.
              Категорiя якостi також формується iз V_TMP_REZ_RSK
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%*/
kodf_     VARCHAR2(2):='D5';
S080_     VARCHAR2(1);
S080_os   VARCHAR2(1);
...
BEGIN
   ...
   -- на 01.04.2016 новый код в структуре показателя
   --if dat_ >= dat_izm4
   --then 
   --   if substr(nbs_, 4, 1) in ('8','9') or nbs_ in ('2607','2627','2657')
   --   then
   --      s190_ := f_get_s190 (dat_, acc_, nd_, acc_ );
   --   else
   --      s190_ := f_get_s190 (dat_, acc_, nd_, null );
   --   end if;
   --end if;
   ...
   if p_type_ = 1 and dat_ <= dat_izm3 then
       begin
          ...
          if 353575 NOT IN (mfo_, mfou_) and
             300465 NOT IN (mfo_, mfou_) and
             se_ < 0 and kom_ <> 0
          then
              old_prc_ := spcnt_;

              if 333432 IN (mfo_, mfou_) then  -- для Ровно Укоопспилка
                 ...
                 -- вводится ежедневная коммисия
                 spcnt_ := ROUND (spcnt_ + b_yea * kom_, 4);
          ...
          elsif 353575 IN (mfo_, mfou_) or 300465 IN (mfo_, mfou_)
          then
...
-- проверка на наличие счета в БПК и корректировка S180 
   ...
   IF SUBSTR(nbs_,1,3) IN ('260','261','206','207','208') AND
      k051_ IN ('34','91') AND k072_<>'L' THEN
      k072_:='L';
   END IF;

   IF re_<>0 AND SUBSTR(nbs_,1,3) IN ('260','261','206','207','208') AND
      k051_ IN ('34','91') AND k111_='65' AND k072_<>'9' THEN
      k072_:='9';
   END IF;

   IF re_<>0 AND SUBSTR(nbs_,1,3) IN ('260','261','206','207','208') AND
      k051_ IN ('34','91') AND k111_='66' AND k072_<>'F' THEN
      k072_:='F';
   END IF;

   IF re_<>0 AND SUBSTR(nbs_,1,3) IN ('260','261','206','207','208') AND
      k051_ IN ('34','91') AND k111_='67' AND k072_<>'C' THEN
      k072_:='C';
   END IF;

   IF SUBSTR(nbs_,1,3) IN ('220','221','222','223','262','263') AND
      k051_ IN ('34','91') AND k072_<>'N' THEN
      k072_:='N';
      k051_:='00';
      k111_:='00';
   END IF;

   IF 380623 in (mfo_, mfou_) and SUBSTR(nbs_,1,3) IN ('262','263') AND k072_='N' THEN
      k051_:='00';
      k111_:='00';
      s260_ := '06';
   END IF;

   IF mfou_ = 353575 and nls_ like '8025%' and nbs_ = '8025' THEN
      nbs_:='2625';
   END IF;
   ...
   VALUES
      (nls_, kv_, data_, kodp1_, znap_,nbuc_,isp_,rnk_,acc_, comm_, nd_);
...
BEGIN
    ...
    if 353575 not in (mfo_, mfou_) then 
        ...
        if prnk_ is null then
            sql_acc_ := 'select a.ACC, a.NLS, a.KV, a.NBS, a.RNK, a.DAOS, a.DAPP, a.ISP, a.NMS, a.LIM, a.PAP, a.TIP, ';
            sql_acc_ := sql_acc_ || 'a.VID, a.MDATE, a.DAZS, a.ACCC, a.TOBO from accounts a where nbs in ';
            sql_acc_ := sql_acc_ || '(select r020 from kod_r020 where trim(prem)=''КБ'' and a010=''D5'' and r020 not like ''6%'' ';
            sql_acc_ := sql_acc_ || 'and d_open<=to_date('''||to_char(dat_kl_, 'ddmmyyyy')||''',''ddmmyyyy'') ';
            sql_acc_ := sql_acc_ || 'and (d_close is null or d_close>to_date('''||to_char(dat_kl_, 'ddmmyyyy')||''',''ddmmyyyy''))) ';
            sql_acc_ := sql_acc_ || 'and (a.nls, a.kv) not in (select b.bal_nls, c.kv from nsmep_sparams b, accounts c ';
            sql_acc_ := sql_acc_ || 'where b.acc = c.acc and b.bal_nls is not null and substr(b.bal_nls,1,4) in (''2625'',''2627'')) ';
            sql_acc_ := sql_acc_ || 'and a.nls not in (''262561118'',''262771018'')  ';
            sql_acc_ := sql_acc_ || 'union all ';
            sql_acc_ := sql_acc_ || 'select a.ACC, a.NLS, a.KV, substr(c.bal_nls,1,4) NBS, a.RNK, a.DAOS, a.DAPP, a.ISP, a.NMS, a.LIM, a.PAP, a.TIP, ';
            sql_acc_ := sql_acc_ || 'a.VID, a.MDATE, a.DAZS, a.ACCC, a.TOBO from accounts a, nsmep_sparams c ';
            sql_acc_ := sql_acc_ || 'where a.acc = c.acc and substr(a.nls,1,4) in (''8025'',''8026'') ';
    ...
    LOOP
       ...
       if nbs_ not in ('1590', '2400', '2401') then
    ...
    ------------------------------------------------------------------------------
    -- формирование показателей для счетов резерва 1590, 2400, 2401
    if dat_ >= dat_izm1 then
       ...
       if kol_ = 0 then
          ...
          OPEN SALDO ;
             LOOP
                ...
                if nbs_ in ('1590', '2400', '2401') then
          ...
          for k in (select t.nls, t.kv, t.dat, t.szq, t.sz, gl.p_icurval(t.kv, t.sz, dat_) sz1,
                    ...
                    where t.dat=Dat23_
                      and t.acc = r.acc
                      and substr(r.kodp,3,3) not in ('159','240') and
                      substr(r.kodp,3,4) not in ('1500', '2607', '2627')
                      and t.nls = r.nls
                      ...
                      and m.ost - m.crdos + m.crkos <> 0                            
                        union all
                     select t.nls, t.kv, t.dat, t.szq, t.sz, gl.p_icurval(t.kv, t.sz, dat_) sz1,
                           t.s080, t.rnk, t.nd nd, t.tobo,
                           r.acc, r.odate, r.kodp, r.comm, r.nd ndr, t.id, nvl(s.r013, '0') r013
                    from v_tmp_rez_risk t, rnbu_trace r, specparam s
                    where t.dat=Dat23_
                      and substr(r.kodp,3,4) in ('2607', '2627')
                      and t.nls = r.nls
                      and t.kv = r.kv
                      and r.kodp like '1%' || (case when dat_ > dat_izm3 then '2' else '' end) || (case when dat_ >= dat_izm4 then '_' else '' end)
                      and t.acc = s.acc(+)
          ...
          loop
             ...
             k072_ := substr(k.kodp,10,1);
             s183_ := substr(k.kodp,12,1);
             re_ := substr(k.kodp,13,1);
             k051_ := substr(k.kodp,14,2);
             s032_ := substr(k.kodp,16,1);
             country_ := substr(k.kodp,20,3);
             s080_ := substr(k.kodp,23,1);
             s260_ := substr(k.kodp,24,2);
             s190_ := substr(k.kodp,27,1);
             ...
             IF typ_>0 and mfou_ <> 353575 THEN
                nbuc_ := NVL(F_Codobl_Tobo(acc_,typ_),nbuc1_);
             ELSE
                nbuc_ := nbuc1_;
             END IF;
             ...
             if nbs_ = '1590' then
                if nls_ like '15%' then
                   r011_ := '1';
                elsif nls_ like '16%' then
                   r011_ := '2';
                else
                   r011_ := '9'; 
                end if;    
             ...
             if se_ <> 0 and nbs_ not in ('1592', '3590') then
    ...
    if dat_ > dat_izm3 then
       ...
       begin
                 ...
                 from
                   (select s.acc, a.nls, a.kv, a.rnk, a.isp, a.accc, a.tip, sum(s.dos) dos
                    ...
                    a.nbs in ('1528','1607','2018', '2028', '2038', '2068', '2078', '2088', '2607', '2657',
                              '2108', '2118', '2128', '2138', '2208', '2218', '2228', '2238', '2627', '8026') and
           ...
           from (
                 ...
                 from
                   (select s.acc, a.nls, a.kv, a.rnk, a.isp, a.accc, a.tip, sum(s.dos) dos
                    ...
                    s.fdat between datr1_ and dat_kor_ and
                    s.dos <> 0 and
                    a.nbs in ('1509', '1519', '1529', '2029', '2039', '2069', '2079', '2089',
                              '2109', '2119', '2129', '2139', '2209', '2219', '2229', '2239') and
                    (prnk_ is null or a.rnk = prnk_)
                    group by s.acc, a.nls, a.kv, a.rnk, a.isp, a.accc, a.tip) a
                    left outer join int_accn i
                    on (a.acc = i.acra and
                 ...
                 from
                   (select s.acc, a.nls, a.kv, a.rnk, a.isp, a.accc, a.tip, sum(s.dos) dos
                    ...
                    ---------
                      union
                    ---------
                    select s.acc, s.nls, s.kv, s.rnk, s.isp, s.accc, s.tip, 0 dos
                    from sal s
                    where
                    s.fdat = dat_ and
                    s.nbs in ('1526', '2016','2026','2036','2066','2076','2086','2106','2116','2126',
                        '2136','2206','2216','2226','2236','3600') and
           ...
           where s.fdat between datr1_ and dat_kor_ and
               ...
               p.dk = 1 and
               p.acc = a.acc and
               (a.nbs in ('6014', '6015', '6016', '6017', '6018', '8029')
                or a.nbs like '602%'
                or a.nbs like '603%'
           ...
           where s.fdat between datr1_ and dat_kor_ and
               ...
               exists (select 1
                       ...
                       where o1.ref = o.ref and
                            ...
                            p1.acc = a1.acc and
                            p1.dk = 1 and
                           (a1.nbs in ('6014', '6015', '6016', '6017', '6018', '8029')
                            or a1.nbs like '602%'
                            or a1.nbs like '603%'
                            or a1.nbs like  '604%'));
           ...
           for k in (select 
                     ...
                     where b.CNT = 1 and
                        b.acc = c.acc(+) and
                        b.iacc = a.acc and
                        (a.nls not like '8%' or mfo_ = 353575 and a.nls like '802%') and 
                        a.nls not like '9%' and
                        substr(a.nls,1,4) not in ('1607','2607', '2627', '2657','8026') and
                  ...
                  if k1.dob <> 0 then
                     fl_ := 1;
                      ...
                      if k.kodp is not null then
                         if k1.nbs in ('6014', '6015') then
                            s183_ := 'B';
                         elsif k1.nbs in ('6020', '6040') then
                            s183_ := '1';
                         else
                            s183_ := substr(k.kodp, 12, 1);
                         end if;
           ...
           loop
              ...
              if k.s1 <> 0 then
                  ...
                  if k.kodp is not null then
                     if k.nbs in ('6014', '6015') then
                        s183_ := 'B';
                     elsif k.nbs in ('6020', '6040') then
                        s183_ := '1';
                     else
                        s183_ := substr(k.kodp, 12, 1);
                     end if;
                  ...
                  if kodp_ is not null then
                     INSERT INTO rnbu_trace
                      ...
                      select s_rnbu_record.NEXTVAL recid,
                             ...
                             '10' || substr(kodp,3,23) || '5' || decode(Dat_, dat_izm5, 'G', substr(kodp,25,1)), 
                             to_char(k.sq), acc,
                             rnk, isp, mdate, k.ref,
...
PROMPT *** Create  grants  P_FD5_NN ***
grant EXECUTE                                                                on P_FD5_NN        to BARS_ACCESS_DEFROLE;



PROMPT ===================================================================================== 
PROMPT *** End *** ========== Scripts /Sql/BARS/Procedure/P_FD5_NN.sql =========*** End *** 
PROMPT ===================================================================================== 
