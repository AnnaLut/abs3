...
PROMPT *** Create  procedure PK_2600 ***
  ...
  l_dazs    date ;
/*
 14.11.2016 Sta Обход триггера TIU_ZAPRET_26MFO по ош
 ORA-20000: \ Заборонено відкриття/модифікація рахунків 26-ої групи на рівень МФО!
...
--- Ищем/Открываем счет 2608* на РНК этого же Клиента:
    ...
    BEGIN Select ACC, dazs  into acc_2608, l_dazs from Accounts  where NBS=nbs_2608 and KV=k.KV  and  RNK=k.RNK  and rownum=1;
          If l_dazs is not null then
             ...
             Update Accounts set NMS  = substr('Нарах.%% '||k.NMS,1,70), ISP=k.ISP, tobo  = k.branch where ACC=acc_2608;
          end if;
    EXCEPTION WHEN OTHERS then   nls_2608 := VKRZN(SUBSTR(gl.aMFO,1,5),nbs_2608||SUBSTR(k.NLS,5));
       -- OP_REG(99,0,0,NULL, tmp_, k.RNK, nls_2608, k.KV, substr('Нарах.%% '||k.NMS,1,70),'ODB',k.ISP,acc_2608);
          op_reg_exfl
            ( mod_ => 99,    --      INTEGER,   -- Opening mode : 0, 1, 2, 3, 4, 9, 99, 77
             ...
             nms_  => substr('Нарах.%% '||k.NMS,1,70), --      VARCHAR2,  -- Account name
