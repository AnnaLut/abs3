...
PROMPT *** Create  procedure P_F39SB ***
  ...
  CREATE OR REPLACE PROCEDURE BARS.P_F39SB (Dat_ DATE)  IS
/*%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% DESCRIPTION :	Процедура формирование файла @39 для Ощадного Банку
% COPYRIGHT   :	Copyright UNITY-BARS Limited, 1999.All Rights Reserved.
% VERSION     : 16.12.2010 (09.12.10,18.12.09,24.07.09,11.06.09,21.04.09,
%                           02.04.09,19.03.09,23.02.09,17.11.08,12.11.08)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
CURSOR Basel IS
   ...
   FROM (SELECT a.kodp KODP,
                a.znap ZNAP, '0' ZNAP_PR
         FROM RNBU_TRACE a
         WHERE SUBSTR (a.kodp, 1, 1) = '1'
         UNION ALL
         SELECT '1'||substr(a.kodp,2,6) KODP,
      ...
      if kurs_ is not null then
         ...
         exception when others then
            if sqlcode=-6502 then
               raise_application_error(-20001,'Помилка: введений курс м_стить не числове значення (acc='||acc_||', kurs='''||kurs_||''')');
            else
...
-- покупка
   IF sum1_>0 AND sun1_>0 AND sum1_ <> sun1_ THEN
      ...
      INSERT INTO rnbu_trace (nls, kv, odate, kodp, znap, ref, comm) VALUES
                             (nls_, kv_, datd_, a_, b_, ref_, comm_) ;
   ...
   LOOP
      ...
      IF sum0_<>0 then
         ...
         INSERT INTO tmp_irep
              (kodf, datf, kodp, znap)
         VALUES
              (kodf_, Dat_, '4'||substr(kodp_,2,6), b_) ;
      end if;
