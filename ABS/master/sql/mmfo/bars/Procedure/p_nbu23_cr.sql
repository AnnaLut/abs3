...
BEGIN
   ...
   if sys_context('bars_context','user_mfo') = '300465' THEN
      for k in (select  r.rowid ri , nd, REPLACE( ( select ConcatStr(KOD) from nd_oznaka_351 where nd = r.nd) ,', ',';') kol_26 
                from rez_cr r 
                where r.fdat = p_dat01 and r.tipa in (5, 6)
               )
      LOOP
   ...
   for k in (select   FDAT  , RNK    , ACC    , KV      , NLS   , NBS , ND      , VIDD, FIN  , bv02q , sum( BV  ) bv  , sum( BVQ  ) bvq , sum( EAD ) ead,
                      ...
                      RZ    , FIN_Z  , CCF    , PD_0    , istval, rpb , CC_ID   , s250, TIP  ,         sum( RC  ) rc  , nvl(sum( ZAL    ),0) zal   ,
             ...
             group by FDAT  , RNK    , ACC    , KV      , NLS   , nbs   , ND     , VIDD   , FIN , VKR , KOL  , FIN23, kpz   , NMK, SDATE, wdate, TIPA,
                      LGD   , bv02   , bv02q  , OVKR    , P_DEF , OVD   , OPD    , CCF    , PD_0, RZ  , FIN_Z, cc_id, ISTVAL, RPB, S250 , TIP  , 
   ...
   LOOP
      ...
      if k.tipa= 15 THEN l_nd_cp := null;
      else               l_nd_cp := k.nd;
      end if;
