...
/* Версия 2.2  12-07-2018  14-09-2017  31-07-2017
   BV по 351 постанове = БАЛАНСУ
   -------------------------------------
 3) 12-07-2018  - EAD по ('SDI','SDA','SDM','SDF') - не надо!
...
begin
   ...
   for k in (select  a.ostc - a.bv del, a.*
             ...
             where bv<>ostc)
   LOOP
      for r in (select rowid rw, bv, kv, ead, eadq, cr, crq, nvl(pd,0) pd, decode(tipa,17,1,21,1,nvl(lgd,0)) lgd, tipa, ccf, tip 
                from rez_cr  where  fdat = p_dat01  and acc = k.acc order by fin,bv)
      LOOP
         l_bv  := greatest(r.bv + k.del,0);
         l_bvq := p_icurval(r.kv,l_bv*100,l_dat31)/100;
         k.del := k.del + (r.bv - l_bv);
         if r.tip in ('SDI','SDA','SDM','SDF')  THEN r.ead := 0; r.eadq := 0;
         else 
         ...
         end if;         r.cr   := round(r.ead * r.pd *r.LGD,2); r.crq  := p_icurval(r.kv,r.cr*100,l_dat31)/100;
