...
% VERSION     :  07/03/2017 (08/02/2017, 07/02/2017)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    параметры: Dat_ - отчетная дата
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
07/03/2017 возвратил блок выравнивания остатков по счетам резерва с #02
...
then
   ...
   for k in (select a.nbs, a.kv, nvl(l.k041,'1') K041,
               'A' S080,  --NVL(s.s080,'A') S080,  
               '3' K150,  --NVL(s.k150,'3') K150, 
   ...
   loop
      ...
      BEGIN
         select NVL(sum(to_number(r.znap)),0)
            into sn_
         from rnbu_trace r
         where r.kodp like '2' || k.nbs || '_' || '_' || k.k041 || k.kv || k.k150 || '%';
      EXCEPTION WHEN NO_DATA_FOUND THEN 
         select NVL(sum(to_number(r.znap)),0)
            into sn_
         from rnbu_trace r
         where r.kodp like '2' || k.nbs || '_' || '_' || k.k041 || k.kv || '_' || '%';
      END;
