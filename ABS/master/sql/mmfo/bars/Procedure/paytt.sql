...
IS -- Автооткрытие счетов      (flg = флаги 23 24 minus 1)
...
IS -- Вычисление номера счета по формуле
  S  varchar2(254);
BEGIN
...
BEGIN
  ...
  --
  -- Начало обработки документов Реестра вкладчиков ССР (ЦРВ)
  --
  --------------
...
*/
     declare
         ...
         s_source    number;         -- сумма первичного документа
         kv_source   number;         -- валюта первичного документа
         nazn_source oper.nazn%type; -- назначение первичного документа
         ...
         l_manual boolean;  -- флаг ручного разбора
     -- ссылка на первоначальный реф платежа
     begin
        bars_audit.trace('PAYTT-' || ref_ || '. USSR');

        --
        -- читаем номер документа и назначение платежа типа ТК*
        select nd, nazn into p_ND, p_nazn
        from oper where ref= ref_;
        bars_audit.trace('PAYTT-' || ref_ || '. p_nd=>'||p_nd);

        if p_ND='DBF' then
            -- возврат в картотеку DBF 3902? никаких проверок и возврат на общий котел вместо областных
            NULL;
        else
            -- номер документа возвращаемого платежа должен равняться референсу исходного
            begin
               p_REF := to_number(p_ND);
            EXCEPTION  WHEN OTHERS THEN
               bars_audit.trace('PAYTT-' || ref_ || '. Ош.№ док(не число) ='|| p_ND);
               RAISE_APPLICATION_ERROR (-20001, 'Ош.№ док(не число) ='|| p_ND);
            end;
            bars_audit.trace('PAYTT-' || ref_ || '. p_ref=>'||p_ref);

            -- читаем сумму и валюту исходного(выплаченного реестром) док-та
            -- + МФО получателя платежа и его код региона (РУ)
            ...
            -- пытаемся найти этот возврат в таблице crv_back_refs(соответствие возвратов и первичных выплат)
            -- в эту таблицу вносят непринятые автоматически возвраты, для которых потом вручную ищут первичную выплату
            begin
                select * into l_br from crv_back_refs where payment_ref=p_ref;
                l_manual := true;  -- ручной разбор
            ...
            if l_manual then
                ...
                If L_NSC <1 OR L_NSC >19 then
                    bars_audit.trace('PAYTT-' || ref_ || '. Ош.№ счета');
                ...
                If tt_ in ('TK ','TK3')  then
                    ...
                    If L_DBCODE <1 OR L_DBCODE >11 then
                        bars_audit.trace('PAYTT-' || ref_ || '. Ош.DBCODE');
                ...
                If s_source <> sa_ then
                   bars_audit.trace('PAYTT-' || ref_ || '. Ошибка суммы: выплачено '||s_source/100||', возвращают '||sa_/100);
                   raise_application_error (-20001, 'Ошибка суммы: выплачено '||s_source/100||', возвращают '||sa_/100);
                end if;

                if kv_source <> kva_ then
                   bars_audit.trace('PAYTT-' || ref_ || '. Ошибка валюты: '|| kv_source ||'(вылюта выплаты) HE= '||kva_||'(валюта возврата)');
                   raise_application_error (-20001, 'Ошибка валюты: '|| kv_source ||'(вылюта выплаты) HE= '||kva_||'(валюта возврата)');
                end if;

                if ( l_tip <> 'TK3' and substr(p_nazn,1,1) =  'П' ) or
                   ( l_tip =  'TK3' and substr(p_nazn,1,1) <> 'П' ) then
                   bars_audit.trace('PAYTT-' || ref_ || '. Ош. несответствие счета(TK3)-назначения платежа(П)');
                   RAISE_APPLICATION_ERROR (-20001, 'Ош. несответствие счета(TK3)-назначения платежа(П)');
                end if;

            end if;

            -- подменяем счет Б на счет регонального управления, откуда пришел платеж
            ...
            exception when no_data_found then
                raise_application_error (-20001, 'Не найден счет Б: '||l_nls2||'('||kva_||')');
            end;
            -- меняем сторону Б в oper
            update oper set nlsb=l_nls2, nam_b=l_namb where ref=ref_;
            --
            If tt_ in ('TK ','TK3')  then
               -- возврат обычной выплаты (1000 грн или эквивалент)
               execute immediate
  ...
  --------------
  --
  -- перехват операций C14 со счета 2906401201
  --
  ...
  then
      declare
        l_ref_ssr     integer;
      begin
        -- ND(номер документа) документа C14 должен совпадать с референсом SSR док-та
