...
%
% VERSION     :  v.19.44   21/05/2019 (02/05/2019)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%/%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
---------------------------------------
          ...
          THEN
             ...
             if exist_cp = 1 and
                SUBSTR (nls_, 1, 3) IN ('140', '141', '142', '143', '144', '145', 
                          '301', '311', '321', '330',  '331', '332',  '333', '338')
             then
                IF SUBSTR (nls_, 1, 4) IN ('3110','3111','3112','3113','3114',
                ...
                comm_ := comm_ || (case when cp_id_ <> 'YYYYYYYYYY' then ' CPID=' || cp_id_ || ' ' else '' end);
             else
                exist_cp_acc := 0;
             end if;
   ...
   loop
      ...
      if k.szq <>0 then
          if TP_SND then
             ...
             comm_ := SUBSTR (' резерв під '||(case when k.tip in ('SPN', 'XPN') then 'прострочені ' else '' end)||'відсотки '||' sumc='||to_char(sumc_) , 1, 200);
          else
             kodp_ := k.sign_rez||nbs_||r011_||r013_||s181_||s242_||k.rz||substr(k.kodp, 11,4);
...
---------------------------------------
      ...
      if k.szq <> 0 then
          if TP_SND then
             ...
             if sakt_ = 0 then
                comm_ := SUBSTR (k.tobo || ' перевищення резерву під відсотки (залишок = 0) ', 1, 200);
             else
                ...
                from (
                   ...
                   join
                    ...
                    on (a.nd = b.nd and
                        ...
                        a.kv = b.kv)) s
                where substr(s.nls, 1, 3) not in ('140', '141', '142', '143', '144', '145', 
                          '301', '311', '321', '330',  '331', '332',  '333', '338'))
      loop
         if k.rnum = 1 then
         ...
         if k.rnum = 1 then
            ...
            if k.rnum = k.cnt then
               ...
               if kodp_ is not null then
                   comm_ := substr(k.comm || ' похибка округлення при розбивці дисконту ',1,200);
...
-----------------
        ...
        for k in (select nvl(a.nbuc, b.nbuc) nbuc, nvl(a.t020, b.t020) t020,
                 ...
                 from (select nbuc, rez, t020, nbs, kv,
                     ...
                     (select nbuc,
                         ...
                         from rnbu_trace r
                         where substr(kodp, 2, 4) in ('1410','1412','1415','1416','1417','1418','1419',
                                           '1420','1422','1425','1426','1427','1428','1429',
                                           '1490','1491','1492','1493','1590','1592','1890',
                                           '2400','2401','2600','2890','3190','3290','3590','3599','3690','3692',
                                           '9010','9015','9030','9031','9036','9500','1419','1429','1509','1519',
   ...
   if dat_ <to_date('20171226','yyyymmdd')  then
    ...
    IF pdat_ >= zm_date5_ THEN
       ...
       update rnbu_trace
          set kodp =substr(kodp,1,8)||'N'||substr(kodp,10)
        where substr(kodp,9,1) in ('6');

       update rnbu_trace
          set kodp =substr(kodp,1,8)||'O'||substr(kodp,10)
        where substr(kodp,9,1) in ('7');

       update rnbu_trace
          set kodp =substr(kodp,1,8)||'P'||substr(kodp,10)
        where substr(kodp,9,1) in ('8','A','B');
...
--------------------------------------------------
   ...
   if pmode_ <> 2 and pnd_ is null then
      ...
      group by acc, (case when substr(kodp,9,1)='Z' then '2' else '1' end), substr(kodp, 2, 4), 
        (case when substr(kodp, 2, 4) = '2049' then sign(znap) else 1 end);
   end if;
