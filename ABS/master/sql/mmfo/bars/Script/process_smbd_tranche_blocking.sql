...
begin
    l_process_type_id := process_utl.corr_proc_type('SMBD'
                                                    ,'TRANCHE_BLOCKING'
                                                    ,'Блокування депозитного траншу ММСБ'
                                                    ,p_can_create => 'smb_deposit_proc.can_create_tranche_blocking'  -- перевірка можливості блокування траншу
                                                    ,p_on_create  => 'smb_deposit_proc.create_tranche_blocking'      -- створення процесу для блокування траншу
                                                    ,p_is_active  => 'Y');
    ...
    l_back_office_confirm_act_id := process_utl.corr_proc_flow(l_process_type_id
                                                              ...
                                                              ,'N'
                                                              ,'N'
                                                              ,p_on_run   => 'smb_deposit_proc.run_tranche_blocking' 
                                                              ,p_is_active => 'Y');
