    ...
    loop
       ...
       BEGIN
          ...
          Insert into KL_F00$LOCAL (POLICY_GROUP, BRANCH, KODF, A017, UUU, 
                                    ZZZ, PATH_O, DATF, NOM, KF)
          Values
                                   ('FILIAL', branch_, kodf_, a017_, uuu_, 
                                    zzz_, path_o_, TO_DATE('28/02/2018','DD/MM/YYYY'), '1', z.KF);
...
BEGIN 
  ...
  NBUR_FILES.SET_FILE (P_FILE_ID, '#3V', 'C', '1', 'Дані про боржників банку - великих, середніх і малих підприємств', 'TXT', 
    '03', '04', 'Y', '1', NULL, 0, 'C', 'V_NBUR_#3V', 1);
  ...
  NBUR_FILES.SET_FILE_PROC (P_PROC_ID, P_FILE_ID, 'O', 'Y', 'BARS',  'NBUR_P_F3V_NC', 
        '#3V схема С рiчний', 'P_F3V v.18.001', TO_DATE('12/29/2017', 'MM/DD/YYYY'), NULL); 
        
  NBUR_FILES.SET_FILE_STC (P_FILE_ID, 1, 'Код~LL', 'substr(kodp,1,2)', '1', 3);
  NBUR_FILES.SET_FILE_STC (P_FILE_ID, 2, '1-велике,среднє~2-мале під-во', 'substr(kodp,3,1)','1', 2);
  NBUR_FILES.SET_FILE_STC (P_FILE_ID, 3, 'Код~ZZZZZZZZZZ', 'substr(kodp,4,10)', '1', 1);
  NBUR_FILES.SET_FILE_STC (P_FILE_ID, 4, 'Значення~показника', 'znap', '0', NULL);        
