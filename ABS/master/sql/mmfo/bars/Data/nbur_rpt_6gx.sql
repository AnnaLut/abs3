...
BEGIN 
  ...
  , p_file_nm          => '(xml) 6GX Данi про концентрацію ризиків за активними операціями банку'
  , p_file_fmt         => 'XML'
  ...
  ( p_proc_id => l_proc_id
   ...
   , p_proc_active => 'Y'
   , p_scheme => 'BARS'
   , p_proc_name => 'NBUR_P_F6GX_NC'
   , p_description => '(xml) 6G XML-формат'
   , p_version => 'v.18.001'
...
begin
    delete from BARS.NBUR_REF_PREPARE_XML WHERE FILE_CODE = '#6G'; 
    Insert into NBUR_REF_PREPARE_XML
       ( DATE_START, FILE_CODE, DESC_XML, ATTR_NIL )
     Values
       (TO_DATE('01/01/2018 00:00:00', 'MM/DD/YYYY HH24:MI:SS'), '#6G',
'select EKP, K020, K021, Q003_2, Q003_3, to_char(Q007,'''||'dd.mm.yyyy'||''') as Q007, B040, T070_1
    from nbur_log_f6GX t
    where  report_date = :p_rpt_dt and
           kf = :p_kf
    order by K020 ,Q003_2',null
       );
