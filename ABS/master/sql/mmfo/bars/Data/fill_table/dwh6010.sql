prompt ===================================== 
prompt == Ф601 Частина 6 Інформація про кредитну операцію боржника  - ЮО
prompt ===================================== 
...
begin     
   l_reports.name := 'Ф601 Частина 6 Інформація про кредитну операцію боржника  - ЮО';
   l_reports.id := '6010';
 ...
 {"Name":"p_sFdat2","Type":"Date","Value":"","Label":"По"}]';
    ...
    l_reports.RESULT_FILE_NAME     := '''601p6_''||SUBSTR(dwh_cbirep.frmt_date(p_sFdat2,''DDMMYY.rar''),1,10)';
    l_reports.SQLPREPARE          := 'select'||nlchr||
                           ' ROWNUM N,'||nlchr||
                           ' c.TYPECREDIT Вид_кредиту_чи_зобовязання,'||nlchr||
                           ' c.NUMDOG Номер_договору,'||nlchr||
                           ' c.DOGDAY Дата_укладання_договору,'||nlchr||
                           ' c.ENDDAY Кінцева_дата_погашення,'||nlchr||
                           ' c.SUMZAGAL Загальна_сума_фін_зобовязання,'||nlchr||
                           ' c.R030 Код_валюти,'||nlchr||
                           ' c.PROCCREDIT Процентна_ставка,'||nlchr||
                           ' c.SUMPAY Розмір_платежів_за_КО,'||nlchr||
                           ' c.PERIODBASE Періодичність_сплати_осн_боргу,'||nlchr||
                           ' c.PERIODPROC Періодичність_сплати_процентів,'||nlchr||
                           ' c.SUMARREARS Заборгованість_за_кред_опер,'||nlchr||
                           ' c.ARREARBASE Прострочена_заборгов_осн_боргу,'||nlchr||
                           ' c.ARREARPROC Прострочена_заборгов_процентів,'||nlchr||
                           ' c.DAYBASE Кільк_днів_простр_заборг_за_ОБ,'||nlchr||
                           ' c.DAYPROC Кільк_днів_простр_заборг_проц,'||nlchr||
...
XLSX_BUILDER_PKG.set_column_width(1, 15);  
XLSX_BUILDER_PKG.set_column_width(2, 41);
XLSX_BUILDER_PKG.set_column_width(3, 35);
XLSX_BUILDER_PKG.set_column_width(4, 35);
XLSX_BUILDER_PKG.set_column_width(5, 35);
XLSX_BUILDER_PKG.set_column_width(6, 41);
XLSX_BUILDER_PKG.set_column_width(7, 25);
XLSX_BUILDER_PKG.set_column_width(8, 35);
XLSX_BUILDER_PKG.set_column_width(9, 38);
XLSX_BUILDER_PKG.set_column_width(10, 43);
XLSX_BUILDER_PKG.set_column_width(11, 41);
XLSX_BUILDER_PKG.set_column_width(12, 41);
XLSX_BUILDER_PKG.set_column_width(13, 45);
XLSX_BUILDER_PKG.set_column_width(14, 45);
XLSX_BUILDER_PKG.set_column_width(15, 41);
XLSX_BUILDER_PKG.set_column_width(16, 41);
XLSX_BUILDER_PKG.set_column_width(17, 41);
XLSX_BUILDER_PKG.set_column_width(18, 39);
XLSX_BUILDER_PKG.set_column_width(19, 25);
XLSX_BUILDER_PKG.set_column_width(20, 41); 
  ...
  Values  (p_cbirep_queries_id, l_file_name, DBMS_LOB.GETLENGTH (l_blob), l_blob);
    ...
    l_reports.stmt                := '';
    l_reports.FILE_NAME           := '''601p6_''||SUBSTR(dwh_cbirep.frmt_date(p_sFdat2,''DDMMYY.xlsx''),1,11)';
