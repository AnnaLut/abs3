prompt ===================================== 
prompt == Ф601 Част.1 Інформація щодо власників істотної участі боржника-ЮО
prompt ===================================== 
...
begin     
   l_reports.name := 'Ф601 Част.1 Інформація щодо власників істотної участі боржника-ЮО';
   l_reports.id := '6005';
 ...
 {"Name":"p_sFdat2","Type":"Date","Value":"","Label":"По"}]';
    ...
    l_reports.SQLPREPARE          := 'select'||nlchr||
                           '       ROWNUM  N,'||nlchr||
                           '       c.lastname || '' '' || c.firstname || '' '' || c.middlename as Прізвище_імя_по_батькові,'||nlchr||
                           '       c.birthday as Дата_народження,'||nlchr||
                           '       d.seriya||'' ''||d.nomerd as Серія_та_номер_паспорта,'||nlchr||
                           '       d.dtd as Дата_видачі_паспорта,'||nlchr||
                           '       null as Освіта,'||nlchr||
                           '       c.inn Ідентифікаційний_код_боржника,'||nlchr||
                           '       f.codregion Код_регіону_реєстрації,'||nlchr||
                           '       trim(f.area||'' ''||f.zip||'' ''||f.city||'' ''||f.streetAddress||'' ''||f.houseNo||'' ''||f.adrKorp||'' ''||f.flatno) as Адреса_реєстрації,'||nlchr||
                           '       c.isrez Резидентність,'||nlchr||
                           '       c.countrycodnerez Країна_реєстр_ФО_нерезидента,'||nlchr||
                           '       o.namew Місце_роботи_найменування,'||nlchr||
                           '       o.codedrpou Місце_роботи_ЄДРПОУ,'||nlchr||
                           '       p.real6month Дохід_підтверджений,'||nlchr||
                           '       p.noreal6month Дохід_непідтверджений,'||nlchr||
                           '       fam.status_f Сімейний_стан,'||nlchr||
                           '       fam.members Кількість_осіб_на_утриманні,'||nlchr||
                           '       c.k060 Код_типу_повяз_з_банком_особи,'||nlchr||
                           '       c.iskr Ознака_викор_даних_для_КР_НБУ '||nlchr||
                           'from '||nlchr||
