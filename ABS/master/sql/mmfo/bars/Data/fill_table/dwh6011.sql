...
begin     
   l_reports.name := 'Ф601 Част.7 Інформація щодо забезпечення за КО з боржником-ЮО';
   l_reports.id := '6011';
 ...
 {"Name":"p_sFdat2","Type":"Date","Value":"","Label":"По"}]';
    ...
    l_reports.RESULT_FILE_NAME     := '''601p7_''||SUBSTR(dwh_cbirep.frmt_date(p_sFdat2,''DDMMYY.rar''),1,10)';
    l_reports.SQLPREPARE          := 'select'||nlchr||
                           '  ROWNUM N,'||nlchr||
                           '  P.NUMBERPLEDGE Номер_договору_застави_чи_інш,'||nlchr||
                           '  c.NUMDOG Номер_кредитного_договору,'||nlchr||
                           '  P.PLEDGEDAY Дата_укладання,'||nlchr||
                           ...
                           '  P.SUMGUARANTEE Сума_гарантії,'||nlchr||
                           '  c.FLAGINSURANCE Страхування_кредиту,'||nlchr||
                           '  P.FLAGINSURANCEPLEDGE Страхування_забезпечення'||nlchr||
                           'from'||nlchr||
                           'branch b,'||nlchr||
...
XLSX_BUILDER_PKG.set_column_width(3, 37);
XLSX_BUILDER_PKG.set_column_width(4, 35);
XLSX_BUILDER_PKG.set_column_width(5, 41);
XLSX_BUILDER_PKG.set_column_width(6, 35);
XLSX_BUILDER_PKG.set_column_width(7, 41);
XLSX_BUILDER_PKG.set_column_width(8, 41);
XLSX_BUILDER_PKG.set_column_width(9, 41);
XLSX_BUILDER_PKG.set_column_width(10, 30);
XLSX_BUILDER_PKG.set_column_width(11, 41);
XLSX_BUILDER_PKG.set_column_width(12, 42);
XLSX_BUILDER_PKG.set_column_width(13, 35);
XLSX_BUILDER_PKG.set_column_width(14, 35);
XLSX_BUILDER_PKG.set_column_width(15, 35);
XLSX_BUILDER_PKG.set_column_width(16, 35);
  ...
  Values  (p_cbirep_queries_id, l_file_name, DBMS_LOB.GETLENGTH (l_blob), l_blob);
    ...
    l_reports.stmt                := '';
    l_reports.FILE_NAME           := '''601p7_''||SUBSTR(dwh_cbirep.frmt_date(p_sFdat2,''DDMMYY.xlsx''),1,11)';
