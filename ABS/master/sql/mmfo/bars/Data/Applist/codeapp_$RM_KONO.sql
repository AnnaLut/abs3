...
*/
    DBMS_OUTPUT.PUT_LINE( chr(13)||chr(10)||' ********** Розрахунки з Казнач.ЦА за КЗ ********** ');
          --  Створюємо функцію KP рах. 2603-Різні
      l := l +1;
      l_function_ids.extend(l);      
      l_function_ids(l)   :=   abs_utils.add_func(
...
---------------------------------------------------------------------------------------------------------------------
	DBMS_OUTPUT.PUT_LINE( chr(13)||chr(10)||' ********** Довідники NEW ********** ');
	umu.add_func2arm_bypath('/barsroot/referencebook/referencelist/', l_application_code, 1 ); 


	DBMS_OUTPUT.PUT_LINE( chr(13)||chr(10)||' ********** Довідник DICT_UZ_SPLITTING ********** ');
	umu.add_reference2arm_bytabname('DICT_UZ_SPLITTING',l_application_code,2,1);


	DBMS_OUTPUT.PUT_LINE( chr(13)||chr(10)||' ********** Розщеплення коштів по рахунку ПАТ «Укрзалізниця» ********** ');

    l := l +1;
    l_function_ids.extend(l); 
	l_function_ids(l):= abs_utils.add_func(
											p_name     => 'Розщеплення коштів по рахунку ПАТ «Укрзалізниця»',
											p_funcname => '/barsroot/ndi/referencebook/GetRefBookData/?accessCode=1'||chr(38)||'sPar=[PROC=> PAC_UZ_SPLITTING.tr_sum(:Par0)][PAR=>:Par0(SEM=Попередня банківська дата <dd.mm.yyyy>,TYPE=D)][EXEC=>BEFORE][MSG=>Операцію завершено.]',
											p_rolename => '' ,    
											p_frontend => 1
											);
---------------------------------------------------------------------------------------------------------------------												  
