...
begin
    ...
    DBMS_OUTPUT.PUT_LINE( chr(13)||chr(10)||' ********** Створюємо функцію Згортання рах 6-7 класу (ребранчинг) ********** ');
      ...
      l_function_ids(l)   :=   abs_utils.add_func(
                                                  ...
                                                  p_funcname => '/barsroot/ndi/referencebook/GetRefBookData/?accessCode=1&sPar=NEW_REBRANCH[NSIFUNCTION][PROC=>pul.put(''BRO'',:O);pul.put(''BRN'',:N);pul.put(''NZN'',:Z)][PAR=>:O(SEM=Бранс_old,TYPE=C,REF=BRANCH2),:N(SEM=Бранс_New,TYPE=C,REF=BRANCH2),:Z(SEM=Признач_пл,TYPE=C)][EXEC=>BEFORE][showDialogWindow=>false]',
