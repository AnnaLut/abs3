...
PROMPT *** Create/replace  ARM  OPFU ***
  declare
    ...
    l_application_name varchar2(300 char) := 'АРМ для роботи з реєстрами ПФУ';
	...
	d integer := 0;
begin
     DBMS_OUTPUT.PUT_LINE(' OPFU створюємо (або оновлюємо) АРМ АРМ для роботи з реєстрами ПФУ ');
    ...
    DBMS_OUTPUT.PUT_LINE( chr(13)||chr(10)||' ********** Створюємо функцію Перегляд та оплата реєстрів ПФУ ********** ');
      ...
      l_function_ids(l)   :=   abs_utils.add_func(
          ...
          --  Створюємо функцію 'Перегляд реєстрів ПФУ що пернесені до архіву'
      l := l +1;
      l_function_ids.extend(l);
      l_function_ids(l)   :=   abs_utils.add_func(
                                                  p_name     => 'Перегляд реєстрів ПФУ що перенесені до архіву',
                                                  p_funcname => '/barsroot/pfu/pfu/Archives',
                                                  p_rolename => '' ,
                                                  p_frontend => l_application_type_id
                                                  );
   ...
   DBMS_OUTPUT.PUT_LINE(chr(13)||chr(10)||'  Прикріпляємо ресурси функцій до даного АРМу (OPFU) - АРМ для роботи з реєстрами ПФУ  ');
   ...
   commit;
umu.add_report2arm(5710,'OPFU');
umu.add_report2arm(5711,'OPFU');
commit;
end;
/
