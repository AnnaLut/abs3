...
begin
    ...
    DBMS_OUTPUT.PUT_LINE( chr(13)||chr(10)||' ********** Видаляємо функцію ФМ. Перевірка ПЕП БАЗОВИЙ ********** ');
    declare
    l_c number;
    begin
    ...
    DBMS_OUTPUT.PUT_LINE( chr(13)||chr(10)||' ********** Створюємо функцію ФМ. Перевірка ПЕП ********** ');
          --  Створюємо функцію ФМ. Перевірка ПЕП
      l := l +1;
      l_function_ids.extend(l);
      l_function_ids(l)   :=   abs_utils.add_func(
                                                  p_name     => 'ФМ. Перевірка ПЕП',
