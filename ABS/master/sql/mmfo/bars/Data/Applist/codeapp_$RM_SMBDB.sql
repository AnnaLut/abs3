...
PROMPT *** Create/replace  ARM  $RM_SMBDB ***
  declare 
    ...
    l_application_name varchar2(300 char) := 'АРМ Депозити ММСБ (бек-офіс)';
    ...
    d integer := 0;
begin
     DBMS_OUTPUT.PUT_LINE(' $RM_SMBDB створюємо (або оновлюємо) АРМ Депозити ММСБ (бек-офіс) ');
     ...
     DBMS_OUTPUT.PUT_LINE( chr(13)||chr(10)||' ********** Створюємо функцію ММСБ: Портфель депозитів ММСБ (бек-офіс) ********** ');
          --  
      l := l +1;
      l_function_ids.extend(l);
      l_function_ids(l)   :=   abs_utils.add_func(
                                                  p_name     => 'Портфель депозитів ММСБ (бек-офіс)',
                                                  p_funcname => '/barsroot/gda/gdaback/index',
                                                  p_rolename => 'BARS_ACCESS_DEFROLE' ,
                                                  p_frontend => l_application_type_id
                                                  );
                                            
   DBMS_OUTPUT.PUT_LINE(chr(13)||chr(10)||'  Прикріпляємо ресурси функцій до даного АРМу ($RM_SMBDB) АРМ Депозити ММСБ (бек-офіс)');
