...
prompt Наименование операции: SC2 Повернення суми вкладу в нац.валюті (внутр.)
declare
  cnt_  number;
begin
  --------------------------------
  ...
  begin
    ...
    values ('SC2', 'SC2 Повернення суми вкладу в нац.валюті (внутр.)', 1, null, 980, null, 980, null, null, null, null, 0, 0, 0, 0, null, null, null, null, null, null, '0000100000000000000000000000000000010000000000000000000000000000', 'Повернення коштів згідно договору № #{dpt_social.f_nazn(''U'',#(ND))}');
  exception
    when dup_val_on_index then 
      update tts
         set tt='SC2', name='SC2 Повернення суми вкладу в нац.валюті (внутр.)', dk=1, nlsm=null, kv=980, nlsk=null, kvk=980, nlss=null, nlsa=null, nlsb=null, mfob=null, flc=0, fli=0, flv=0, flr=0, s=null, s2=null, sk=null, proc=null, s3800=null, rang=null, flags='0000100000000000000000000000000000010000000000000000000000000000', nazn='Повернення коштів згідно договору № #{dpt_social.f_nazn(''U'',#(ND))}'
       where tt='SC2';
  end;
  --------------------------------
  ----------- Реквизиты ----------
