...
prompt Наименование операции: SC3 Повернення суми вкладу в нац.валюті (міжбанк)
declare
  cnt_  number;
begin
  --------------------------------
  ...
  begin
    ...
    values ('SC3', 'SC3 Повернення суми вкладу в нац.валюті (міжбанк)', 1, null, 980, '#(get_proc_nls(''T00'',#(KVA)))', 980, null, null, null, null, 0, 1, 0, 0, null, null, null, null, null, null, '0300100000000000000000000000000000010000000000000000000000000000', 'Повернення коштів згідно договору №  #{dpt_social.f_nazn(''U'',#(ND))}');
  exception
    when dup_val_on_index then 
      update tts
         set tt='SC3', name='SC3 Повернення суми вкладу в нац.валюті (міжбанк)', dk=1, nlsm=null, kv=980, nlsk='#(get_proc_nls(''T00'',#(KVA)))', kvk=980, nlss=null, nlsa=null, nlsb=null, mfob=null, flc=0, fli=1, flv=0, flr=0, s=null, s2=null, sk=null, proc=null, s3800=null, rang=null, flags='0300100000000000000000000000000000010000000000000000000000000000', nazn='Повернення коштів згідно договору №  #{dpt_social.f_nazn(''U'',#(ND))}'
       where tt='SC3';
  end;
  --------------------------------
  ----------- Реквизиты ----------
