...
prompt Наименование операции: DMF +Викуп нерозмінної частини Банківських металів
declare
  cnt_  number;
begin
  --------------------------------
  ...
  begin
    ...
    values ('DMF', 'DMF +Викуп нерозмінної частини Банківських металів', 1, null, null, null, 980, null, '#(tobopack.GetTOBOParam(''CASH11''))', '#(tobopack.GetToboCASH)', null, 0, 0, 1, 0, 'round(MOD(#(S),DECODE(#(KVA),959,20/31.1034807*1000,961,100/31.1034807*1000,1)))', 'eqv_obs(#(KVA),round(MOD(#(S),DECODE(#(KVA),959,20/31.1034807*1000,961,100/31.1034807*1000,0))),BANKDATE,0)', 56, null, '#(nbs_ob22 (''3800'',''09''))', 0, '0000100001010000000000000000000000010000000000000000000000000000', 'Викуп нерозмінного залишку при поверненні коштів згідно договору #{DPT_WEB.F_NAZN(''U'',#(ND))}');
  exception
    when dup_val_on_index then 
      update tts
         set tt='DMF', name='DMF +Викуп нерозмінної частини Банківських металів', dk=1, nlsm=null, kv=null, nlsk=null, kvk=980, nlss=null, nlsa='#(tobopack.GetTOBOParam(''CASH11''))', nlsb='#(tobopack.GetToboCASH)', mfob=null, flc=0, fli=0, flv=1, flr=0, s='round(MOD(#(S),DECODE(#(KVA),959,20/31.1034807*1000,961,100/31.1034807*1000,1)))', s2='eqv_obs(#(KVA),round(MOD(#(S),DECODE(#(KVA),959,20/31.1034807*1000,961,100/31.1034807*1000,0))),BANKDATE,0)', sk=56, proc=null, s3800='#(nbs_ob22 (''3800'',''09''))', rang=0, flags='0000100001010000000000000000000000010000000000000000000000000000', nazn='Викуп нерозмінного залишку при поверненні коштів згідно договору #{DPT_WEB.F_NAZN(''U'',#(ND))}'
       where tt='DMF';
  end;
  --------------------------------
  ----------- Реквизиты ----------
