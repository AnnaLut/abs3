...
begin     
   ...
   exception when no_data_found then                       
    ...
    l_zpr.create_stmt  := 'DATE_REP [10,'' '',L]; Name_RU [50,'' '',L] ; MFO_DEP  [6,'' '',R]; BRANCH [30,'' '',L]; count_05 [12,'' '',R]; count_06 [12,'' '',R] ; count_07 [12,'' '',R]; count_08 [12,'' '',R]; count_09 [12,'' '',R]; count_10 [12,'' '',R]; count_11 [12,'' '',R]; count_12 [12,'' '',R]; count_13 [12,'' '',R]; count_14 [12,'' '',R]; count_15 [12,'' '',R]; count_16 [12,'' '',R]; count_17 [12,'' '',R]; count_18 [12,'' '',R]; count_19 [12,'' '',R]; count_20 [12,'' '',R]; count_21 [12,'' '',R]; count_22 [12,'' '',R]; count_23 [12,'' '',R]; count_24 [12,'' '',R]; count_25 [12,'' '',R]; Delim=''њ''';
    l_zpr.rpt_template := '';
    l_zpr.form_proc    := '';
    l_zpr.default_vars := '';
    l_zpr.bind_sql     := '';
...
nvl(
(select count(*)  from accounts where nbs like  ''25%'' and nbs!=''2546'' and not (nbs=''2525'' and  fost(acc,nvl(to_date(:sFdat1,''dd/mm/yyyy''), gl.bd))>=0) 
and daos< nvl(to_date(:sFdat1,''dd/mm/yyyy''), gl.bd)
and (dazs is null or dazs>nvl(to_date(:sFdat1,''dd/mm/yyyy''), gl.bd))
and kv=980
and branch=b.branch
...
(select count(*)  from accounts where nbs like  ''25%'' and nbs!=''2546'' and not (nbs=''2525'' and  fost(acc,nvl(to_date(:sFdat1,''dd/mm/yyyy''), gl.bd))>=0)
and daos< nvl(to_date(:sFdat1,''dd/mm/yyyy''), gl.bd)
and (dazs is null or dazs>nvl(to_date(:sF';
    l_zpr.xsl_data     := '';
    l_zpr.xsd_data     := '';
