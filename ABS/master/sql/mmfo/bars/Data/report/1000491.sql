prompt ===================================== 
prompt == Количество счетов по классам (rahun.dbf) (super_new)
prompt ===================================== 
...
begin     
   l_zpr.name := 'Количество счетов по классам (rahun.dbf) (super_new)';
   l_zpr.pkey := '\OLD\***\***\178';
...
FROM
  ...
  (SELECT COUNT(*) CNT FROM ACCOUNTS WHERE NBS =''2625'' AND KV=980 AND (DAZS IS NULL or dazs>:Param0) AND DAOS<=:Param0) C7,
  (SELECT COUNT(*) CNT FROM ACCOUNTS WHERE NBS =''2625'' AND KV<>980 AND (DAZS IS NULL or dazs>:Param0) AND DAOS<=:Param0) C8,
