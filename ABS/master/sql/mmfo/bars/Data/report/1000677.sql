...
begin     
   ...
   exception when no_data_found then                       
    ...
    l_zpr.txt          := 'select s.tobo "В_дд_лення", 
       s.br_name "Назва",   
...
(select b.tobo, 
     ...
     from rnbu_trace r, accounts a, tobo b
     where r.nls = a.nls and r.kv = a.kv and r.kodp like ''1%''and 
           ...
           and
           rtrim(substr(replace(a.tobo||''/'',''//'',''/000000/''),1,instr(replace(a.tobo||''/'',''//'',''/000000/''),''/'',1,3)-1),''/'')||''/'' = b.tobo(+) 
 group by b.tobo, b.name , substr(r.nls,4,1), replace(substr(kodp,6,1),''4'',''3''), 
            substr(kodp,7,2), 
            (case when substr(kodp,7,2) = ''08'' then
                      (case when substr(r.nls,4,1)=''8'' or substr(r.nls,1,4) in (''2607'', ''2627'', ''3570'') 
                                  then 7 
...
group by tobo, br_name
) s,
(select tobo, sum(decode(pr,0,szq,0)) szq, sum(decode(pr,1,szq,0)) szq_3579, sum(szq) szq_all, 
        sum(decode(pr,0,sm_jur,0)) sm_jur, sum(decode(pr,0,sm_phys,0)) sm_phys 
 from
    ( select b.tobo, b.name br_name,sum(szq)/100 szq,decode(substr(r.nls,1,4),''3570'',1,''3578'',1,0) pr 
             ,sum(decode(r.custtype,2,szq/100,0)) sm_jur 
             ,sum(decode(r.custtype,3,szq/100,0)) sm_phys
      from tmp_rez_risk r, tobo b
      where r.s080 = 9 and r.dat = to_date(:dat_ ,''dd.mm.yyyy'') and r.id = nvl(:p_user, user_id)
          and rtrim(substr(replace(r.tobo||''/'',''//'',''/000000/''),1,instr(replace(r.tobo||''/'',''//'',''/000000/''),''/'',1,3)-1),''/'')||''/'' = b.tobo(+)
      group by b.tobo, b.name,decode(substr(r.nls,1,4),''3570'',1,''3578'',1,0) 
    )
 group by tobo                
) ss,
(select tobo, 
...
(select  b.tobo, sum(decode(a.nbs,''2400'', gl.p_icurval (a.kv, (rez.ostc96 (a.acc, to_date(:dat_ ,''dd.mm.yyyy''))), to_date(:dat_ ,''dd.mm.yyyy''))/100,0)) rest,
                 sum(decode(a.nbs,''3599'', gl.p_icurval (a.kv, (rez.ostc96 (a.acc, to_date(:dat_ ,''dd.mm.yyyy''))), to_date(:dat_ ,''dd.mm.yyyy''))/100,0)) rest_3579
    from accounts a, specparam_int s, tobo b
    where a.nbs in (''2400'',''3599'') and a.acc = s.acc and
          ...
          and rtrim(substr(replace(a.tobo||''/'',''//'',''/000000/''),1,instr(replace(a.tobo||''/'',''//'',''/000000/''),''/'',1,3)-1),''/'')||''/'' = b.tobo(+)
    group by b.tobo
) sss
where s.tobo = ss.tobo(+) and s.tobo = sss.tobo(+) and s.tobo = ss_o.tobo(+)
