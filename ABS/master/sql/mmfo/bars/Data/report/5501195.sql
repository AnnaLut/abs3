prompt ===================================== 
prompt == погашение по телу физики на дату(проблемн)
prompt ===================================== 
...
begin     
   l_zpr.name := 'погашение по телу физики на дату(проблемн)';
   l_zpr.pkey := '\zap\SBER\CCK\17';

   l_message  := ' люч запроса: '||l_zpr.pkey||'  '||nlchr;
   ...
   exception when no_data_found then                       
    ...
    l_zpr.name         := 'погашение по телу физики на дату(проблемн)';
    l_zpr.namef        := '';
    l_zpr.bindvars     := ':sFdat1='''',:sFdat2='''',:sFdat3=''дата пересчета валюты ''';
    l_zpr.create_stmt  := '';
    l_zpr.rpt_template := '';
    l_zpr.form_proc    := '';
    l_zpr.default_vars := '';
...
and  O.DATD  < to_date(:sFdat2,''dd.mm.yy'') ) order by o.kv,D.ND,o.datd';
    ...
    if l_isnew = 1 then           
       ...
       l_message:=l_message||'ƒобавлен новый кат.запрос є'||l_zpr.kodz||'.'; 
    else                           
       ...
       l_message:=l_message||' ат.запрос c таким ключем уже существует под є'||l_zpr.kodz||', его параметры изменены.'; 
