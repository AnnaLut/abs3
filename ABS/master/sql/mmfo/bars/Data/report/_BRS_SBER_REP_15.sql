...
begin
  ...
  l_zpr.create_stmt  := '';
  l_zpr.rpt_template := 'BZ001S.qrp';
  l_zpr.form_proc    := ''; -- P_BAL_SNP( 0, to_date(:sFdat1,''dd-mm-yyyy''),'''' ,'''', '''' )
  l_zpr.default_vars := '';
  l_zpr.bind_sql     := '';
  l_zpr.xml_encoding := 'CL8MSWIN1251';
  l_zpr.txt          := 'select ACG_TP  as B1'  ||nlchr||
                        '     , KL      as K'   ||nlchr||
                        '     , RAZD    as R'   ||nlchr||
                        '     , GR      as G'   ||nlchr||
                        '     , R020    as NBS' ||nlchr||
                        '     , TVR_DB  as DOS' ||nlchr||
                        '     , TVR_CR  as KOS' ||nlchr||
                        '     , BAL_DB  as OSTD'||nlchr||
                        '     , BAL_CR  as OSTK'||nlchr||
                        '     , KL_NM   as SK'  ||nlchr||
                        '     , RAZD_NM as SR'  ||nlchr||
                        '     , GR_NM   as SG'  ||nlchr||
                        '     , ACG_NM  as SB1' ||nlchr||
                        '     , RPT_DT  as DAT' ||nlchr||
                        '     , case row_number() over ( partition by ACG_TP order by R020 )'||nlchr||
                        '       when 1 then 0 else 1 end as TITL'||nlchr||
                        '  from table( RPT_UTL.GET_BALANCES( to_date(:sFdat1,''dd-mm-yyyy'') ) )';
  l_zpr.xsl_data     := '';
  l_zpr.xsd_data     := '';
  ...
  then
    ...
    begin
      select idf into l_repfolder from reportsf where idf = 11;
    exception
      when no_data_found then
        l_repfolder := null;
    end;
