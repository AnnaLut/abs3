...
begin
    delete from BARS.NBUR_REF_PREPARE_XML WHERE FILE_CODE = '26X'; 
    Insert into NBUR_REF_PREPARE_XML
       (FILE_CODE, DESC_XML, DATE_START)
     Values
       ('26X', 'select EKP, T020, R020, R011, R013, R030, 
       K040, Q001, K020, K021, K180, K190, S181, S245, 
       S580, F033, abs(sum(T070)) as T070, abs(sum(T071)) as T071
from nbur_log_f26X 
where report_date = :p_rpt_dt
           and kf = :p_kf    
group by EKP, T020, R020, R011, R013, R030, 
       K040, Q001, K020, K021, K180, K190, S181, S245, 
       S580, F033
having  sum(T070)<>0
order by 1,9,3,2', TO_DATE('01/01/2018 00:00:00', 'MM/DD/YYYY HH24:MI:SS'));
    COMMIT;
end;
/