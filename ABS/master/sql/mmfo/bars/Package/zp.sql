...
begin
  ...
  -- bars_audit.info('zp.pay_payroll.start_pay'||to_char(sysdate,'DD.MM.YYYY HH24:MI:SS') );
   ...
   ---подписываем ведомость
                  ...
                  when no_data_found then
                    ...
                    begin
                      select *
                        into l_acc_b_rec
                        from bars.accounts
                       where nlsalt = l_oper_zp.nlsb
                         and kv = l_accounts_2909.kv
                         and dazs is null;
                    exception
                      when no_data_found then
                        raise_application_error(-20000, 'Не знайдено альтернативний рахунок - '||l_oper_zp.nlsb||', або рахунок закрито.');
                    end;
                  when others then
                    raise_application_error(-20000, 'Помилка пошуку рахунка '||l_oper_zp.nlsb||'. '||dbms_utility.format_error_backtrace || ' ' || sqlerrm);
                end;
    ...
    exception when others
      then  rollback to sp_payroll;
          bars_audit.info('zp.pay_payroll pay_doc error '||dbms_utility.format_error_backtrace || ' ' || sqlerrm);
