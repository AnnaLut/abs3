...
begin
  ...
  if  p_tag = 'NOHOP' then -- cobummfo-10192, портфель ЮО, для субдоговоров параметр должен дублироваться с основного договора
    select ndg, vidd
      into v_ndg, v_vidd
      from cc_deal 
      where nd = p_nd;
    if v_vidd <5 and v_ndg is not null then
      if v_ndg is null then -- не субдоговор, нет генерального, проходим дальше
        ...
        when matched then update  -- если найден параметр для субдоговора, то он устанавливается из пришедшего по генеральному
          set n.txt = p_TXT
        ...
        merge into nd_txt n
          using (select nd as s_nd
                   ...
                   where nd = p_nd) sub
        on (n.nd = sub.s_nd and n.tag = 'NOHOP')
        when matched then update  -- если найден параметр для субдоговора, то он устанавливается из вычитанного по генеральному
          set n.txt = v_value
        when not matched then insert
          (nd,tag,txt)
