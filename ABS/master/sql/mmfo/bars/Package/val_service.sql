...
IS
 ...
 g_body_version   CONSTANT VARCHAR2 (64) := 'version 1.20 10.03.2017';
 ...
 G_TRACE                   CONSTANT varchar2(100) := 'val_service.';
 ...
 IS
 BEGIN
     RETURN 'Package header val_service '|| g_header_version;
 END header_version;
 ...
 IS
 BEGIN
    RETURN 'Package body BARS.val_service '|| g_body_version;
 ...
 IS
   ...
   IS
      ...
      l_message    VARCHAR2 (4000);
      l_clob       CLOB;
      l_trace      VARCHAR2 (1000) := G_TRACE||'set_eq: ';
      l            varchar2(2):= chr(13)||chr(10);
   BEGIN
      BARS_AUDIT.INFO(l_trace||'передаваемые параметры документа: '||'vdat_='    ||  vdat_ || l ||
                                                                     'datb_='    ||   datb_|| l ||
                                                                     'pdat_='    || pdat_  || l ||
                                                                      'vidd_='   ||vidd_   || l ||
                                                                      'serd_='   ||serd_   || l ||
      ...
      --добавить параметры
      soap_rpc.ADD_PARAMETER (l_request,'vdat_',   TO_CHAR (vdat_, 'dd/mm/yyyy HH24:MI:SS'));
      soap_rpc.ADD_PARAMETER (l_request,'datb_',   TO_CHAR (datb_, 'dd/mm/yyyy'));
      soap_rpc.ADD_PARAMETER (l_request,'pdat_',   TO_CHAR (pdat_, 'dd/mm/yyyy HH24:MI:SS'));
   ...
   is
      ...
      l_kurb    number (20, 8);
      l_drday   varchar2 (14);
      l_trace   varchar2 (1000) := G_TRACE||'set_val:';
   begin
      for c in (select kf from mv_kf) loop
         bc.subst_mfo (c.kf);
         for k in (select op.*, (select ov.dat from oper_visa ov  where ov.ref = v.ref and ov.status = 2) datvisa, v.tt vtt, nvl(p.passp,1) vidd,
                       ...
                       nvl ( (select value
                            ...
                            'Ќе заповнено')
                          pib,
                       (select to_number (replace (value, ',', '.'),'9999999.999999999999')
                          from operw
                         where     ref = op.ref
                               and tag =
                                      case
            ...
            then
              ...
              /* bars_audit.info('VAL_SERVICE l_vidd = '||l_vidd||
                ...
                ' l_limeq = '||l_limeq);*/
                  ...
                  bars_audit.info(l_trace||'Ќачало передачи докумнета '||k.ref||' в ÷Ѕƒ по валютообмену');

                  if   l_sq is null then  -- выт€нули что-то не то
                       delete from val_queue  where ref = k.ref and tt = k.vtt;
                       continue;
                  end if;

                  val_service.set_eq (nvl(k.datvisa,trunc(sysdate)) ,            --vdat_    varchar2,
               ...
               exception
                  ...
                  then
                     ...
                     then
                        ...
                        bars_error.raise_error(dbms_utility.format_error_stack() || chr(10) ||
