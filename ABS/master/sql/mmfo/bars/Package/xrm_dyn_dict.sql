prompt create package bars.xrm_dyn_dict
   ...
   begin
      for c in (select tag, regexp_replace(lower(substr (browser, 12, length (browser) - 13)),'select ','select '''||tag||''' tag, ',1,0) sql_stmt --обрезать поля
                 ...
                 where     browser is not null
					   ...
					   and regexp_substr(upper(browser), q'$(FROM)([[:blank:]])+([^ "\(\)']+)$', 1, 1, null, 3) not in ('SW_BANKS', 'KOD_DZ', 'RC_BNK') -- большие / часто используемые справочники вынесены из общего "котла"
					   )
      loop
