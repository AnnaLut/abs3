  ...
  BEGIN
    UPDATE dwh_log
       SET package_status = g_parse_result.status,
           ...
           parse_begin = decode(g_parse_result.status, 'PROCESSING', sysdate, parse_begin),
           parse_end = decode(g_parse_result.status, 'PARSED', sysdate, parse_end)
...
--                                        ,l_bankdate
          ...
          WHEN '1' THEN
            -- Депозити
            bars.attribute_utl.set_value(l_rnk, 'CUSTOMER_PRDCT_AMNT_DPT', udf_to_number(l_pt));
          WHEN '2' THEN
            -- Кредити (під заставу)
            bars.attribute_utl.set_value(l_rnk, 'CUSTOMER_PRDCT_AMNT_CREDITS', udf_to_number(l_pt));
          WHEN '3' THEN
            -- Кредити с БПК
            bars.attribute_utl.set_value(l_rnk, 'CUSTOMER_PRDCT_AMNT_CRDCARDS', udf_to_number(l_pt));
          WHEN '4' THEN
            -- Кредити під поруку
            bars.attribute_utl.set_value(l_rnk, 'CUSTOMER_PRDCT_AMNT_CRD_GARANT', udf_to_number(l_pt));
          WHEN '5' THEN
            -- Кредити Енергоефективність
            bars.attribute_utl.set_value(l_rnk, 'CUSTOMER_PRDCT_AMNT_CRDENERGY', udf_to_number(l_pt));
          WHEN '6' THEN
            -- БПК
            bars.attribute_utl.set_value(l_rnk, 'CUSTOMER_PRDCT_AMNT_CARDS', udf_to_number(l_pt));
          WHEN '7' THEN
            -- Поточні рахунки
            bars.attribute_utl.set_value(l_rnk, 'CUSTOMER_PRDCT_AMNT_ACC', udf_to_number(l_pt));
          WHEN '8' THEN
            -- Індивідуальні сейфи
...
--    dbms_xmlparser.parseclob(l_parser, l_clob);
    ...
    -- Какойто CustomerW trigger ругаецо
    bc.go(l_dwhlog_row.kf);
...
PROMPT *** Create  grants  SEGMENTATION_PACK ***
grant EXECUTE                                                                on SEGMENTATION_PACK to BARS_ACCESS_DEFROLE;

 
 
 PROMPT ===================================================================================== 
 PROMPT *** End *** ========== Scripts /Sql/BARS/package/segmentation_pack.sql =========*** E
 PROMPT ===================================================================================== 
