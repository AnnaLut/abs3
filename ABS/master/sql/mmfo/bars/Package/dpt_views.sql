               ...
               lst AS (SELECT dc.rnk,
                       ...
                       (SELECT type_name
                         ...
                         WHERE vidd = DC.VIDD)
                          VIDD_NAME,*/
                       0 AS rate,
                       NVL (dpt.f_dptw (dc.deposit_id, ''NCASH''), ''0'') DPT_NOCASH, 0 AS DPT_STATUS, DC.COMMENTS DPT_COMMENTS, max(DC.archdoc_id) archdoc_id, max(dc.wb) wb
                       FROM cust c, accmainlst ac, dpt_deposit_clos dc, dpt_vidd v
                       WHERE c.rnk = dc.rnk AND v.vidd = dc.vidd and dc.acc = ac.acc
          GROUP BY dc.rnk,dc.acc,dc.branch,dc.kv,v.amr_metr,DC.DEPOSIT_ID,DC.ND,dc.datz/*,DC.VIDD*/, NVL (dpt.f_dptw (dc.deposit_id,''NCASH''), ''0''),DC.COMMENTS'
                       ||
