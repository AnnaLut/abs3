     ...
     if (l_SMS_ACC_TEMPLATES.ID = 8)  -- якщо смс шаблон по теплому кредиту то перевіряємо чи з рахунку відшкодування прийшли кошти
       THEN
         ...
         if  (l_warm_nls = p_nlsa )
            THEN
            logger.trace (title || 'warm_credit_sms for acc:' || to_char(p_acc) );--відповідно до заявки: якщо було відшкодування по теплому кредиту - смс по даному рахунку відключаємо
            /*update accounts t1
               ...
               EXCEPTION
                  ...
                  THEN
                     logger.trace (title||
                           'EVERYDAY SMS. Для счета ACC='
                        || c.acc
                        || ' не указаны номера мобильных телефонов или отсутсвует балансовый');
               END;
            END IF;
         END LOOP;
      --
