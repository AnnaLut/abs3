CREATE OR REPLACE PACKAGE KWT_2924 IS
 ...
 G_HEADER_VERSION  CONSTANT VARCHAR2(64)  :=  'ver.4/ 07.08.2017';
...
begin suda ; ff.tag := 'KWT_2924' ;    ff.name := 'Старт-залишок розшифровано вручну'; ff.USE_IN_ARCH  := 0;
      update ACCOUNTS_FIELD set name = ff.name where tag = ff.tag  ;  
      if SQL%rowcount=0 then insert into ACCOUNTS_FIELD Values ff ; end if;
      COMMIT;
end;
/
...
CREATE OR REPLACE PACKAGE BODY KWT_2924 IS                               	
 G_BODY_VERSION  CONSTANT VARCHAR2(64)  :=   'ver.4. 08.08.2017';
/*
 07.08.2017 Sta ATM
 28.07.2017 Sta Формирование снимка 31 числа - удаление всего . но вставка по АСС

 13.06.2017 Sta Поле KF в таблице A_KWT_2924 +  T_KWT_2924
...
*/
 ...
 n_err number      := -20203 ;
---================================================================
procedure CLS_ATM ( p_DAT date ) is
  oo oper%rowtype;    l_DAT date ; l_OST number;   l_dk1 int  ; l_dk2 int ;   l_D1  number; l_D2   number;                    
begin
  l_dat := NVL(p_DAT, add_months( gl.bdate, -1) ) ;

  for k in (select a.* from accounts a where  a.tip in ('AT7', 'AT8') and a.dazs is null and kv = gl.baseval)
  loop
     If k.ostc = 0 then
        ...
        loop
           select         s     into l_D1 from opldok where ref =        x.ref1                                     and acc = k.acc and dk =  l_dk1 ;
           ...
           If oo.s <> 0 then 
              ...
              begin select substr(nms,1,38) into oo.nam_b from accounts where kv = gl.baseval and nls = oo.nlsb;
                    gl.ref (oo.REF);
                    oo.nd := trim (Substr( '          '||to_char(oo.ref) , -10 ) ) ;
                    gl.in_doc3 (ref_=>oo.REF ,  tt_=>'PS1', vob_=>6,    nd_ =>oo.nd, pdat_=>SYSDATE , vdat_=>gl.bdate, dk_ =>oo.dk,
                    ...
                    gl.payv(0, oo.ref, gl.bdate,'PS1', oo.dk, k.kv, k.nls, oo.s, k.kv,  oo.nlsb, oo.s);
...
procedure INS_ATM2 ( p_ACC number, p_REF1 number, p_REF2 number) is  
                     l_acc number; l_ref1 number; l_dk1 int   ;  l_dk2 int ; 
...
begin l_ACC  := NVL ( p_acc, to_number( pul.get('ATM_ACC' ))) ;   
      ...
      insert into atm_ref2 (ref1, ref2) select l_ref1, p_ref2 from opldok o where o.acc = l_acc and o.ref = p_ref2 and o.dk = l_DK2 and o.s <= l_Del; 
end INS_ATM2;
...
begin
  ...
  if  p_mode = 1 then  
      select  nvl(min(ref ),0) -1  into l_ref  from tmp_RT_KWT_2924 ;
      insert  into tmp_RT_KWT_2924 (acc,  ref, fdat, s )  values (l_acc, l_ref, l_dat01, l_s ) ;  

  elsif p_mode = 2 then  
     update tmp_RT_KWT_2924 set s= l_s where  ref = p_ref ;

  elsif p_mode = 3 then  
     delete from tmp_RT_KWT_2924       where  ref = p_ref ;

  elsif p_mode = 0 then  
     select  sum(s) - bars.fost(l_acc, l_dat31 ) into l_ss from tmp_RT_KWT_2924 where  acc = l_acc ;
     If l_ss <> 0 then  
        raise_application_error(n_err,'НЕ збалансовано залишок та суму докуметів' )  ; 
     end if ;

     insert into T_KWT_2924 (ACC,FDAT,REF,S) select acc,fdat,ref,s from tmp_RT_KWT_2924 where  acc = l_acc ;

     delete from tmp_RT_KWT_2924  where  acc = l_acc   ;
     insert into A_KWT_2924 ( acc, branch, daos, nls, kv, ob22, dat_KWT , KF)
...
PROCEDURE MAX_DAT  is -- определение ьфлсимальной даты квитовки и установка  ее в пул
  l_dat date ;
begin
  select max(dat_kwt) into l_dat from A_KWT_2924 ;
  PUL.put('DAT_KWT_2924', to_char( l_dat, 'dd.mm.yyyy') ) ;
end ;


PROCEDURE KWT_DAT  ( p_mode int, p_bDAT date) is  -- обавити в календар по банк- дату
   l_dat_sys date;
begin
   select max(dat_sys) into l_dat_sys from D_KWT_2924 ;
   If  l_dat_sys is null then l_dat_sys := trunc( sysdate) ; end if;

   insert into D_KWT_2924 (Dat_sys , Dat_kwt  ) 
...
begin 
  If p_dat is null then   
     ...
     begin select * into dd from D_KWT_2924  where  DAT_SYS = dd.DAT_SYS;
...
begin 
  ...
  If KWT_2924.DAT31 ( l_dat_end ) = 1  then
     ...
     delete from  T31_KWT_2924 ;
     delete from  A31_KWT_2924 ;
   end if;
...
If l_Dat_VZ < to_date('01-03-2017','dd-mm-yyyy') then 
...
---------------------------------------------------------------
  ...
  begin select   dat_kwt + 1   into  l_dat_beg  from A_KWT_2924 where acc = aa.acc;
  EXCEPTION WHEN NO_DATA_FOUND THEN  l_dat_beg := l_Dat_VZ;
        insert into A_KWT_2924 (acc, branch, daos, nls, kv, ob22, KF ) values (aa.acc, aa.branch, aa.daos, aa.nls, aa.kv, aa.ob22, aa.KF );
  end;
...
--l_ost :=  Fost(aa.acc, l_dat_end);
  ...
  If    l_dapp < l_dat_Beg  then  ------------------------------------------------ Протокол старый. без изменений
        update A_KWT_2924 set dat_KWT = l_dat_end where acc= aa.acc;

  ElsIf l_ost =  0       then        delete from t_KWT_2924 where acc = aa.acc ; ---- Протокол НУЛЕВОЙ = пустой
        update A_KWT_2924 set dapp=l_DAPP, datvz=l_dat_VZ, dat_KWT=l_dat_end, IXD = 0, IXK = 0 where acc= aa.acc;

  Else  EXECUTE   IMMEDIATE  ' truncate  TABLE TMP_KWT_2924 '; 
     If l_Dat_VZ >= l_dat_beg then   delete from t_KWT_2924 where acc = aa.acc ;       l_dat_beg :=  l_dat_VZ ; ---- Протокол вновь созданный     
     else  Insert into TMP_KWT_2924( s,fdat,ref,tt2) select s,fdat,ref,tt2 from t_KWT_2924 where acc = aa.acc ; ---- Протокод приращенный 
                                     delete from t_KWT_2924 where acc = aa.acc ;
     end if;
     ...
     for oo in (select o.tt, o.ref, o.fdat, (2*dk-1) * o.s  S            from opldok o, saldoa s 
      ...
      loop   
/*
скитувати в 2 кроки з таким пріоритетом
1)	Спочатку «Сума+РЕФ»
2)	Потім  «Тільки Сума»
*/
      ...
      insert into  T_KWT_2924  (acc,fdat,s,ref,tt2) select aa.acc,fdat,s,ref,tt2  from TMP_KWT_2924;
      If l_Ost > 0 then l_krd :=   l_Ost ; 
      else              l_deb := - l_Ost ; 
      end if;

      update A_KWT_2924 set dapp=l_DAPP, datvz = l_dat_VZ, dat_KWT = l_dat_end, IXD= l_deb/100, IXK= l_krd/100 where acc= aa.acc;

      If KWT_2924.DAT31 ( l_dat_end ) = 1  then
         -- это месячная отчетная дата. сделаем копию 
         insert into  T31_KWT_2924 ( acc, fdat, s, ref,tt2) select aa.acc,fdat,s,ref,tt2  from TMP_KWT_2924;
         insert into  A31_KWT_2924 ( ACC, BRANCH , NLS, KV, OB22, DAOS, DAPP, DATVZ, DAT_KWT, IXD, IXK)
                              select ACC, BRANCH , NLS, KV, OB22, DAOS, DAPP, DATVZ, DAT_KWT, IXD, IXK
                              from A_KWT_2924 where acc= aa.acc and dat_KWT = l_dat_end   ;
      end if;
...
begin update T_KWT_2924 set rkw = p_RKW where rowid = p_RI ; end RKW1;

PROCEDURE OK1      ( p_acc number ) is -- Виконати ручну ЗБАЛАНСОВАНУ квитовку 
 l_acc T_KWT_2924.acc%type ;
 l_S   T_KWT_2924.s%type   ;

begin 
  l_ACC := NVL(p_acc, to_number (PUL.GET('ACC') ) );
  select sum(s) into l_s  from T_KWT_2924 where acc = l_ACC and rkw = 1 ;
  If l_s <> 0  then raise_application_error(-20000, 'KWT_2924: НЕ збалансовані відмічені для квитовки суми !' );   end if ;
  delete                  from T_KWT_2924 where acc = l_ACC and rkw = 1 ;
end OK1;
---------------
