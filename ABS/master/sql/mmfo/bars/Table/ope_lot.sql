...
begin    execute immediate '
  ALTER TABLE BARS.OPE_LOT ADD CONSTRAINT PK_OPELOT PRIMARY KEY (OB22)
...
begin    execute immediate '
  CREATE UNIQUE INDEX BARS.PK_OPELOT ON BARS.OPE_LOT (OB22) 
  PCTFREE 10 INITRANS 2 MAXTRANS 255 COMPUTE STATISTICS 
  TABLESPACE BRSDYND ';exception when others then  if  sqlcode=-955  then null; else raise; end if; end;/
prompt add KF on OPE_LOT and recreate PK
begin
  bars.bc.go(f_ourmfo_g);
  execute immediate 'alter table bars.ope_lot add kf varchar2(6) default sys_context(''bars_context'', ''user_mfo'')';
exception
  when others then
...
/
begin
  execute immediate 'alter table bars.ope_lot drop constraint pk_opelot';
exception
  when others then
...
/
begin
  execute immediate 'drop index bars.pk_opelot';
exception
  when others then
...
/
begin
  execute immediate 'create unique index bars.pk_opelot on bars.ope_lot(ob22, kf) tablespace brsdyni';
exception
  when others then
    if sqlcode = -955 then null; else raise; end if;
end;
/
begin
  execute immediate 'alter table bars.ope_lot add constraint pk_opelot primary key(ob22, kf) using index bars.pk_opelot';
exception
  when others then
