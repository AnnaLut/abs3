...
BEGIN 
        execute immediate  
          'begin  
               ...
               bpa.alter_policy_info(''KF91'', ''FILIAL'' , ''M'', ''M'', ''M'', ''M'');
...
PROMPT *** Create  index I1_KF91 ***
begin   
 execute immediate '
  CREATE INDEX BARS.I1_KF91 ON BARS.KF91 (KF) 
  PCTFREE 10 INITRANS 2 MAXTRANS 255 COMPUTE STATISTICS 
  TABLESPACE BRSDYND ';
exception when others then
  if  sqlcode=-955  then null; else raise; end if;
...
grant SELECT                                                                 on KF91            to BARSREADER_ROLE;
