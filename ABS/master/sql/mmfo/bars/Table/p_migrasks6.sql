...
BEGIN 
        execute immediate  
          'begin  
               bpa.alter_policy_info(''P_MIGRASKS6'', ''CENTER'' , null, null, null, null);
               bpa.alter_policy_info(''P_MIGRASKS6'', ''FILIAL'' , null, null, null, null);
               bpa.alter_policy_info(''P_MIGRASKS6'', ''WHOLE'' , null, null, null, null);
               null;
           end; 
          '; 
END; 
...
COMMENT ON TABLE BARS.P_MIGRASKS6 IS 'Справочник импорта СКАРБ6(ф)';
COMMENT ON COLUMN BARS.P_MIGRASKS6.ACTION IS 'Действие';
COMMENT ON COLUMN BARS.P_MIGRASKS6.PROCNAME IS 'Наименование процедуры импорта';
COMMENT ON COLUMN BARS.P_MIGRASKS6.ERRMASK IS 'Маска ошибок';
COMMENT ON COLUMN BARS.P_MIGRASKS6.ORDNUNG IS 'Порядок сортировки';



PROMPT *** Create  grants  P_MIGRASKS6 ***
grant SELECT                                                                 on P_MIGRASKS6     to BARSREADER_ROLE;
grant SELECT                                                                 on P_MIGRASKS6     to BARS_DM;
grant SELECT                                                                 on P_MIGRASKS6     to UPLD;
