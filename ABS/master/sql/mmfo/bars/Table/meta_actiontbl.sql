...
BEGIN 
        execute immediate  
          'begin  
               bpa.alter_policy_info(''META_ACTIONTBL'', ''CENTER'' , null, null, null, null);
               bpa.alter_policy_info(''META_ACTIONTBL'', ''FILIAL'' , null, null, null, null);
...
PROMPT *** Create  constraint CC_METAACTIONTBL_BRANCH_NN ***
begin   
 execute immediate '
  ALTER TABLE BARS.META_ACTIONTBL MODIFY (BRANCH CONSTRAINT CC_METAACTIONTBL_BRANCH_NN NOT NULL ENABLE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
