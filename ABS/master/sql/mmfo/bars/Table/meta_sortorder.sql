...
BEGIN 
        execute immediate  
          'begin  
               bpa.alter_policy_info(''META_SORTORDER'', ''CENTER'' , null, null, null, null);
...
COMMENT ON COLUMN BARS.META_SORTORDER.BRANCH IS 'Hierarchical Branch Code';
...
PROMPT *** Create  constraint CC_METASORTORDER_BRANCH_NN ***
begin   
 execute immediate '
  ALTER TABLE BARS.META_SORTORDER MODIFY (BRANCH CONSTRAINT CC_METASORTORDER_BRANCH_NN NOT NULL ENABLE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
grant SELECT                                                                 on META_SORTORDER  to BARSREADER_ROLE;
grant DELETE,INSERT,SELECT,UPDATE                                            on META_SORTORDER  to BARS_ACCESS_DEFROLE;
grant SELECT                                                                 on META_SORTORDER  to BARS_DM;
grant SELECT                                                                 on META_SORTORDER  to START1;
grant SELECT                                                                 on META_SORTORDER  to UPLD;
