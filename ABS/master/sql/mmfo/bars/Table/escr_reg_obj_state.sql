...
BEGIN 
        execute immediate  
          'begin  
               bpa.alter_policy_info(''ESCR_REG_OBJ_STATE'', ''FILIAL'' , ''M'', ''M'', ''M'', ''M'');
...
COMMENT ON TABLE BARS.ESCR_REG_OBJ_STATE IS 'Таблиця статусів обєктів реєстру';
COMMENT ON COLUMN BARS.ESCR_REG_OBJ_STATE.ID IS 'Ідентифікатор';
COMMENT ON COLUMN BARS.ESCR_REG_OBJ_STATE.OBJ_ID IS 'Ідентифікатор обєкта, для якого записується статус';
COMMENT ON COLUMN BARS.ESCR_REG_OBJ_STATE.OBJ_TYPE IS 'Тип вхідного  обєкта (1 -реєстр,0-кредит)';
...
COMMENT ON COLUMN BARS.ESCR_REG_OBJ_STATE.KF IS '';
