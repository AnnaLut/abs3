...
BEGIN 
        execute immediate  
          'begin  
               bpa.alter_policy_info(''BPK_CREDIT_DEAL'', ''CENTER'' , null, ''E'', ''E'', ''E'');
...
COMMENT ON COLUMN BARS.BPK_CREDIT_DEAL.PC_TYPE IS 'Тип проценсінгового центру (BPK/W4)';
COMMENT ON COLUMN BARS.BPK_CREDIT_DEAL.CREATE_DT IS 'Дата створення запису';
COMMENT ON COLUMN BARS.BPK_CREDIT_DEAL.KF IS 'Код фiлiалу (МФО)';
...
PROMPT *** Create  constraint CC_BPKCREDITDEAL_KF_NN ***
begin   
 execute immediate '
...
PROMPT *** Create  index UK_BPKCREDITDEAL_DEALND ***
begin   
 execute immediate '
  CREATE UNIQUE INDEX BARS.UK_BPKCREDITDEAL_DEALND ON BARS.BPK_CREDIT_DEAL (KF, DEAL_ND) 
