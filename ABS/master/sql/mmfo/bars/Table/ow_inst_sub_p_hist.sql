begin
  execute immediate 'begin bpa.alter_policy_info(''ow_inst_sub_p_hist'', ''WHOLE'',  null, ''E'', ''E'', ''E''); end;';
exception when others then
  if sqlcode = -06550 then null; else raise; end if;
end;
/
 begin
  execute immediate 'begin bpa.alter_policy_info(''ow_inst_sub_p_hist'', ''FILIAL'', ''M'', ''M'', ''M'' , ''M''); end;';
exception when others then
  if sqlcode = -06550 then null; else raise; end if;
end;
/ 
begin
  execute immediate 'begin bpa.alter_policy_info(''ow_inst_sub_p_hist'', ''CENTER'', null, ''E'', ''E'' , ''E''); end;';
exception when others then
  if sqlcode = -06550 then null; else raise; end if;
end;
/
BEGIN
execute immediate 'create table ow_inst_sub_p_hist (
chain_idt number,
plan_num number,
idp number,
type varchar2(30),
...
KF varchar2(6 BYTE) default sys_context(''bars_context'',''user_mfo'') CONSTRAINT CC_ow_inst_sub_p_hist_KF_NN NOT NULL) 
tablespace BRSMDLD
PARTITION BY LIST (KF) 
 ...
 END;
/
COMMENT ON TABLE BARS.ow_inst_sub_p_hist IS 'Графік сплати Інстолмент в розрізі складових порції';

COMMENT ON COLUMN BARS.ow_inst_sub_p_hist.CHAIN_IDT IS 'ІД графіка';

COMMENT ON COLUMN BARS.ow_inst_sub_p_hist.IDP IS 'ІД порції';

COMMENT ON COLUMN BARS.ow_inst_sub_p_hist.TYPE IS 'Тип боргу: 
PRINCIPAL - основний борг
FEE - винагорода';

COMMENT ON COLUMN BARS.ow_inst_sub_p_hist.CODE IS 'Тип боргу: 
PRINCIPAL - основний борг
FEE - комісія
INT - проценти';

COMMENT ON COLUMN BARS.ow_inst_sub_p_hist.STATUS IS 'Сдатус порції: 
...
COMMENT ON COLUMN BARS.ow_inst_sub_p_hist.TOTAL_AMOUNT IS 'Загальна сума складової порції';

COMMENT ON COLUMN BARS.ow_inst_sub_p_hist.AMOUNT_TO_PAY IS 'Сума до сплати';

COMMENT ON COLUMN BARS.ow_inst_sub_p_hist.WRITTEN_OFF_AMOUNT IS 'Сума списання';

COMMENT ON COLUMN BARS.ow_inst_sub_p_hist.OVERDUE_AMOUNT IS 'Сума прострочки';

COMMENT ON COLUMN BARS.ow_inst_sub_p_hist.KF IS 'МФО';
/
begin
  execute immediate 'begin BARS_POLICY_ADM.ALTER_POLICIES(''ow_inst_sub_p_hist''); end;';
exception when others then
  if sqlcode = -06550 then null; else raise; end if;
end;
/
begin   
 execute immediate 'ALTER TABLE BARS.ow_inst_sub_p_hist ADD 
CONSTRAINT PK_ow_inst_sub_p_hist
 PRIMARY KEY (CHAIN_IDT, PLAN_NUM, IDP, CODE)
 ENABLE
 VALIDATE';
exception when others then
...
begin   
 execute immediate ' 
ALTER TABLE BARS.ow_inst_sub_p_hist ADD 
CONSTRAINT FK_ow_inst_sub_p_hist
 FOREIGN KEY (CHAIN_IDT, PLAN_NUM, IDP)
 REFERENCES BARS.OW_INST_PORTIONS_HIST (CHAIN_IDT, PLAN_NUM, SEQ_NUMBER)
 ENABLE
 VALIDATE';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
...
/
grant all on ow_inst_sub_p_hist to bars_access_defrole;
/
grant all on ow_inst_sub_p_hist to bars;
/