...
begin  EXECUTE IMMEDIATE 'CREATE TABLE RECLASS9( FROM_PROD CHAR(6), TO_K9 int, TO_PROD CHAR(6)  ) ' ;
...
begin EXECUTE IMMEDIATE 'ALTER TABLE RECLASS9 add  CONSTRAINT XPK_RECLASS9  PRIMARY KEY  (FROM_PROD, TO_K9 ) ' ;
exception when others then   if SQLCODE = -02260 then null;   else raise; end if;   -- ORA-02260: table can have only one primary key
end;
/

COMMENT ON TABLE  RECLASS9           IS '"Вектори" перекласифікації Активів по МСФЗ-9';
COMMENT ON COLUMN RECLASS9.FROM_PROD IS 'FROM:~З якого~Продукту ?';
COMMENT ON COLUMN RECLASS9.TO_K9     IS 'TO:~ В яку~"Корзину" ?'  ;
COMMENT ON COLUMN RECLASS9.TO_PROD   IS 'TO:~ В який~Продукт ?'  ;
