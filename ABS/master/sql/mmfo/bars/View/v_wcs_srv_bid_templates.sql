...
PROMPT *** Create  view V_WCS_SRV_BID_TEMPLATES ***
  ...
  CREATE OR REPLACE FORCE VIEW BARS.V_WCS_SRV_BID_TEMPLATES ("SRV", "BID_ID", "TEMPLATE_ID", "TEMPLATE_NAME", "PRINT_STATE_ID", "PRINT_STATE_NAME", "SCAN_QID", "WS_ID", "WS_NUM", "DOCEXP_TYPE_ID", "IMG", "IS_SCAN_REQUIRED", "ENABLED", "VISIBLE") AS 
  select decode(s.id,
              'SECURITY_SERVICE',
              'ss',
              'LAW_SERVICE',
...
grant SELECT                                                                 on V_WCS_SRV_BID_TEMPLATES to BARSREADER_ROLE;
grant SELECT                                                                 on V_WCS_SRV_BID_TEMPLATES to BARS_ACCESS_DEFROLE;
grant SELECT                                                                 on V_WCS_SRV_BID_TEMPLATES to UPLD;
