...
PROMPT ===================================================================================== 
PROMPT *** Run *** ========== Scripts /Sql/BARS/View/V_CCK_ZU.sql =========*** Run *** =====
PROMPT ===================================================================================== 


PROMPT *** Create  view V_CCK_ZU ***

  CREATE OR REPLACE FORCE VIEW BARS.V_CCK_ZU ("ISP", "ND", "CC_ID", "VIDD", "RNK", "KV", "S", "GPK", "DSDATE", "DWDATE", "PR", "OSTC", "SOS", "NAMK", "ACC8", "DAZS", "BRANCH", "CUSTTYPE", "PROD", "SDOG", "NDI", "TR") AS 
  SELECT x.ISP, x.ND, x.CC_ID, x.VIDD, x.RNK, x.KV, x.S, x.GPK,
       ...
       x.PROD, x.SDOG, x.NDI,  CASE WHEN VIDD in (2,3) and PR_TR='1' THEN  1 else  0  END TR
from ( SELECT d.user_id ISP, d.ND, d.CC_ID, d.VIDD, d.RNK, a8.KV, d.LIMIT S, a8.vid GPK, d.sdate DSDATE, D.wdate DWDATE,
       ...
       WHERE n.nd=d.nd AND C.RNK=D.RNK AND c.RNK=a8.rnk AND n.acc=a8.acc AND a8.tip='LIM'
         AND d.vidd IN (1,2,3         )     AND d.sos > 13
   ) x;

PROMPT *** Create  grants  V_CCK_ZU ***
grant SELECT                                                                 on V_CCK_ZU        to BARSREADER_ROLE;
grant SELECT                                                                 on V_CCK_ZU        to BARS_ACCESS_DEFROLE;
grant SELECT                                                                 on V_CCK_ZU        to UPLD;



PROMPT ===================================================================================== 
PROMPT *** End *** ========== Scripts /Sql/BARS/View/V_CCK_ZU.sql =========*** End *** =====
PROMPT ===================================================================================== 
