 ...
 PROMPT ===================================================================================== 
 PROMPT *** Run *** ========== Scripts /Sql/BARS/function/f_pop_otcn_gl.sql =========*** Run 
 PROMPT ===================================================================================== 
 
  CREATE OR REPLACE FUNCTION BARS.F_POP_OTCN_GL (Dat_ DATE, type_ NUMBER,
                                       sql_acc_ VARCHAR2,
...
% VERSION     : 12.06.2009 (29.02.08,18.01.08,08.02.07,16.02.06)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
параметры:
  Dat_ - отчетная дата
...
29.02.2008 - для переменной Dat3_ берем последний рабочий месяца начала
             ...
             vob in (96, 196))
12.06.2009 - изменил условие для SALDOA C
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%*/
...
sql_doda_ := 'insert into OTCN_ACC (ACC, NLS, KV, NBS, RNK, DAOS, DAPP, ISP, NMS, LIM, PAP, TIP, VID, MDATE, DAZS, ACCC, TOBO) '||
             ...
             'from (select * from v_gl where nbs ';
...
FROM (SELECT AA.FDAT,s.acc,s.nls,s.kv,s.nbs,s.rnk,AA.ostf,AA.dos,AA.kos
      ...
      WHERE t.kv<>980                     AND
           ...
           (s.nls IN (t.s0000,t.s3800,t.s3801,t.s0009) OR
            ...
            (s.acc,AA.FDAT) in (SELECT c.acc,MAX(c.FDAT)
                                FROM  SALDOB c
                                WHERE c.FDAT <= Dat_
                                GROUP BY c.acc)) a;
...
-- (файлы #81, #A4)
   IF type_ = 4 THEN
      P_Populate_Kor_GL(Dat1_,Bankdate,sql_doda_,type_kor_);
   ELSE
      P_Populate_Kor_GL(Dat1_,Dat2_,sql_doda_,type_kor_);
   END IF;
...
--         UPDATE OTCN_SALDO a
--          SET  a.vost=NVL(Fost(a.acc, datv_), 0), -- вх_дний залишок
--               a.vostq=DECODE(a.kv, 980, 0, NVL(Fostq_gl(a.acc, datv_), 0)), -- екв. вх_дного залишку
--               a.dos=Fdos(a.acc, Dat1_, Dat_),
--               a.dosq=DECODE(a.kv, 980, 0, Fdosq_gl(a.acc, Dat1_, Dat_)),
--               a.kos=Fkos(a.acc, Dat1_, Dat_),
--               a.kosq=DECODE(a.kv, 980, 0, Fkosq_gl(a.acc, Dat1_, Dat_)) ;
...
PROMPT *** Create  grants  F_POP_OTCN_GL ***
grant EXECUTE                                                                on F_POP_OTCN_GL   to BARS_ACCESS_DEFROLE;
grant EXECUTE                                                                on F_POP_OTCN_GL   to RPBN001;
grant EXECUTE                                                                on F_POP_OTCN_GL   to WR_ALL_RIGHTS;

 
 
 PROMPT ===================================================================================== 
 PROMPT *** End *** ========== Scripts /Sql/BARS/function/f_pop_otcn_gl.sql =========*** End 
 PROMPT ===================================================================================== 
