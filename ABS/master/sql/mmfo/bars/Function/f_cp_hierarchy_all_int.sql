 ...
 PROMPT ===================================================================================== 
 PROMPT *** Run *** ========== Scripts /Sql/BARS/function/f_cp_hierarchy_all_int.sql ========
 PROMPT ===================================================================================== 
 
  CREATE OR REPLACE FUNCTION BARS.F_CP_HIERARCHY_ALL_INT (p_id in cp_kod.id%type, p_date_start date, p_date_finish date, p_CP_ACCTYPE cp_accounts.CP_ACCTYPE%type, p_level int) return number
is
l_res number;
begin
 ...
 for times in (
    ...
    select GREATEST (cph.fdat, p_date_start) ds,
     ...
     where (cph.fdat between p_date_start and p_date_finish
       ...
       where H_ID = p_level)
 loop        
       l_res := nvl(l_res,0) + nvl( f_cp_hierarchy_int(p_id =>p_id,
...
return nvl(l_res,0);
end f_cp_hierarchy_all_int;
/
 show err;
 
PROMPT *** Create  grants  F_CP_HIERARCHY_ALL_INT ***
grant EXECUTE                                                                on F_CP_HIERARCHY_ALL_INT to BARS_ACCESS_DEFROLE;

 
 
 PROMPT ===================================================================================== 
 PROMPT *** End *** ========== Scripts /Sql/BARS/function/f_cp_hierarchy_all_int.sql ========
 PROMPT ===================================================================================== 
