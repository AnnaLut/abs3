 ...
 PROMPT ===================================================================================== 
  ...
  CREATE OR REPLACE FUNCTION BARS.BPK_TIP_PSYS (acc_ NUMBER, acc_tip_ VARCHAR2, acc_kv_ NUMBER, mode_ NUMBER)
...
BEGIN
  ...
  IF mode_ = 2 THEN -- возвращаем наименование платежной системы
    ...
    END CASE;
    IF acc_tip_ LIKE 'W4%' THEN
      SELECT UPPER(SUBSTR(name,1,InStr(name,' ')-1)) INTO Result_
        FROM w4_subproduct WHERE code = (SELECT sub_code FROM w4_card WHERE code=
                                          (SELECT card_code FROM w4_acc WHERE acc_pk=acc_));
    END IF;
  END IF;
  IF mode_ = 4 THEN
    case acc_tip_
