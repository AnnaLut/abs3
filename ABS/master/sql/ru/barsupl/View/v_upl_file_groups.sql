...
PROMPT *** Create  view V_UPL_FILE_GROUPS ***
  ...
  CREATE OR REPLACE FORCE VIEW BARSUPL.V_UPL_FILE_GROUPS ("JOB_NAME", "GROUP_ID", "GROUP_NAME", "FILE_ID", "FILE_CODE", "FILE_NAME", "SQL_ID") AS 
  select p.JOB_NAME,
       fg.group_id,
       g.descript,
       fg.file_id,
  ...
  join (select JOB_NAME, to_number(value) as GROUP_ID
          from BARSUPL.UPL_AUTOJOB_PARAM_VALUES
         where PARAM = 'GROUPID' ) p on (p.group_id = g.group_id)
 where f.ISACTIVE = 1;
