...
COMMENT ON TABLE BARSUPL.UPL_CONSTRAINTS IS 'Информация о fk констрейнтах';
COMMENT ON COLUMN BARSUPL.UPL_CONSTRAINTS.FILE_ID IS 'Код файла';
COMMENT ON COLUMN BARSUPL.UPL_CONSTRAINTS.CONSTR_NAME IS '';
COMMENT ON COLUMN BARSUPL.UPL_CONSTRAINTS.PRIORITY IS 'Приоритетность проверки ключа';
COMMENT ON COLUMN BARSUPL.UPL_CONSTRAINTS.FK_FILEID IS 'Сcылочнаяч таблдица, содержащая перичный ключ, на который ссылается ссылка';
