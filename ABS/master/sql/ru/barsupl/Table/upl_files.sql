...
COMMENT ON TABLE BARSUPL.UPL_FILES IS 'Информация о файлах выгрузки';
COMMENT ON COLUMN BARSUPL.UPL_FILES.FILE_ID IS 'Код файла выгрузки';
COMMENT ON COLUMN BARSUPL.UPL_FILES.SQL_ID IS 'Ссылка на код запроса';
COMMENT ON COLUMN BARSUPL.UPL_FILES.FILE_CODE IS 'Кодовое наименование файла';
COMMENT ON COLUMN BARSUPL.UPL_FILES.FILENAME_PRFX IS 'Префикс для построения имени файла (наприм. АССОUNTS)';
COMMENT ON COLUMN BARSUPL.UPL_FILES.EQVSPACE IS ' (=1)Значения в колонках - одной ширины, (=0) - значения в колоках обрезаются(тримаются)';
COMMENT ON COLUMN BARSUPL.UPL_FILES.DELIMM IS 'Символы разделителя колонок, например: 09||124 (это конкатенация табуляции и пробела) или null - если разделителя небудет';
COMMENT ON COLUMN BARSUPL.UPL_FILES.DEC_DELIMM IS 'Разделитель дробной части';
COMMENT ON COLUMN BARSUPL.UPL_FILES.ENDLINE IS 'Символ конца строки, например: 09||124 (это конкатенация табуляции и пробела)';
COMMENT ON COLUMN BARSUPL.UPL_FILES.HEAD_LINE IS 'Символы разделителя колонок, например: 09||124 (это конкатенация табуляции и пробела) или null - если разделителя небудет';
COMMENT ON COLUMN BARSUPL.UPL_FILES.DESCRIPT IS 'Краткое описание содержимого файла';
COMMENT ON COLUMN BARSUPL.UPL_FILES.ORDER_ID IS 'Последовательный № выгрузки';
COMMENT ON COLUMN BARSUPL.UPL_FILES.NULLVAL IS 'Текстовое поле для подстановки в null значение';
COMMENT ON COLUMN BARSUPL.UPL_FILES.DATA_TYPE IS 'Тип данных в файле -  дельта или полностью справочник';
COMMENT ON COLUMN BARSUPL.UPL_FILES.DOMAIN_CODE IS 'Домен информации для файла';
COMMENT ON COLUMN BARSUPL.UPL_FILES.ISACTIVE IS 'Активность выгрзки (=1 - активен, =0 - неактивен)';
COMMENT ON COLUMN BARSUPL.UPL_FILES.SEQ_CASHE IS 'Размер кеша для формировния сиквенса для GK';
COMMENT ON COLUMN BARSUPL.UPL_FILES.GK_INDICATOR IS 'Ключ в DWH: 0 - дорівнює ключу АБС (для довідників), 1 - генерується свій (KF + ACC, KF + RNK, і т.д.)';
COMMENT ON COLUMN BARSUPL.UPL_FILES.MASTER_CKGK IS '';
COMMENT ON COLUMN BARSUPL.UPL_FILES.CRITICAL_FLG IS 'Признак критических данных для выгрузки. При 1 - сбой выгрузки файла или 2 файл пуст - выгрузка преклащается и принимает статус ERROR';
