...
is
    ...
    g_header_version  constant varchar2(64)  := 'version 3.4.1 17/08/2018';--БПК2620+gcif
...
CREATE OR REPLACE PACKAGE BODY DM_IMPORT is
  ...
  g_body_version constant varchar2(64) := 'Version 3.4.1 17/08/2018';--БПК2620+gcif
  g_body_defs    constant varchar2(512) := null;
  G_TRACE        constant varchar2(20) := 'dm_import.';

  --G_GOUCMFO constant varchar2(6) := '300465';
...
begin
    ...
    bars.bars_audit.trace('mfo=%s, pcode=%s', g_kf, g_phonecode);
...
begin
    ...
    is
        ...
        q_str_main  varchar2(32000) :=
                   ' select
                       ...
                       p.organ,
                       c.okpo  as okpo,
                       dm_import.add38phone(substr(mpno,1,20)) as mpno,
                       dm_import.add38phone(p.teld) teld,
                       dm_import.add38phone(p.telw) telw,
    ...
    begin
        ...
        loop
          begin
            fetch c into l_row.id, l_row.rnk,l_row.branch,l_row.last_name,l_row.first_name,l_row.middle_name,
...
-- 20.03.2017  COBUSUPABS-5659  and not (C.ise in ('14100', '14200', '14101','14201') and C.sed ='91') --фильтруем ФОПов
     ...
     loop
       ...
       select --+ leading(na a b)
         sum(b.ost)/100 into l_row.borg_sy
         from bars.nd_acc na, bars.accounts a, bars.agg_monbals b
        where na.ND = cur.nd and na.ACC=a.acc and a.tip in ('SS ','SP ','SL ')
          and a.acc=b.acc and b.fdat = trunc(trunc(p_dat,'YEAR')-1,'MONTH')
          and (a.dazs is null or a.dazs>trunc(p_dat,'YEAR')) and a.daos <= trunc(p_dat,'YEAR');

       l_row.borg_sy := abs(nvl(l_row.borg_sy, 0));

       -- Сума залишку заборгованості за відсотками на початок року, грн.
       select --+ leading(na a b)
         sum(b.ost)/100 into l_row.borgproc_sy
         from bars.nd_acc na, bars.accounts a, bars.agg_monbals b
        where na.ND = cur.nd and na.ACC=a.acc and a.tip in ('SN ', 'SPN', 'SLN')
          and a.acc=b.acc and b.fdat = trunc(trunc(p_dat,'YEAR')-1,'MONTH')
          and (a.dazs is null or a.dazs>trunc(p_dat,'YEAR')) and a.daos <= trunc(p_dat,'YEAR');
    ...
    loop
      begin
        ...
        select min(to_date(nt.TXT,'dd/mm/yyyy'))
          ...
          from bars.nd_txt nt
         where tag='NOHOP' and nt.ND=cur.nd;
...
begin
    ...
    loop
      begin
        fetch c into l_row.rnk, l_row.branch, l_row.nmk, l_row.nmkk, l_row.ruk,
...
begin
    ...
    loop
      begin
        fetch c into
         l_row.rnk, l_row.rel_id, l_row.rel_rnk, l_row.rel_intext, l_row.name, l_row.okpo, l_row.vaga1, l_row.custtype,
    ...
    procedure store_customer_segments_data(
        ...
        p_errors_count out integer)
    is
        --type t_customer_segments is table of customers_segment%rowtype;
        l_customer_segments T_CUST_SEGMENTS;--t_customer_segments;
    ...
    begin
        ...
        loop
            ...
            begin
                ...
                /*forall i in 1 .. l_customer_segments.count save exceptions
                    insert into customers_segment
                    values l_customer_segments(i);*/
            exception
    ...
    begin
        ...
        open l_cursor for
        select /*+ parallel(4) leading(k) */ bars.t_attribute_value(d.attribute_id, d.object_id, nh.value, null, null, null, null, null, null, null, null, null) as R
        from   (select h.attribute_id, h.object_id, max(h.id) as last_history_id
                from bars.attribute_history h
                ...
                where  k.attribute_code in ('CUSTOMER_SEGMENT_ACTIVITY', 'CUSTOMER_SEGMENT_FINANCIAL',
                                            ...
                                            'CUSTOMER_PRDCT_AMNT_CARDS', 'CUSTOMER_PRDCT_AMNT_ACC')
                   and h.OBJECT_ID in (select OBJECT_ID from bars.attribute_history where SYS_TIME between p_date_from and p_date_to)
                group by h.attribute_id, h.object_id) d
        left join bars.attribute_number_history nh on nh.id = d.last_history_id;
    ...
    begin
        ...
        bars_audit.info('Сегментація клієнтів для XRM - початок підготовки даних за дату ' || p_snapshot_value_date);
           ...
           open l_cursor for
        with data as (select h.attribute_id, h.object_id, min(h.id) keep (dense_rank last order by h.id) last_history_id
                      from   bars.attribute_history h
                      join   bars.attribute_kind k on k.id = h.attribute_id
                      where  k.attribute_code in ('CUSTOMER_SEGMENT_ACTIVITY', 'CUSTOMER_SEGMENT_FINANCIAL',
                                                  'CUSTOMER_SEGMENT_BEHAVIOR', 'CUSTOMER_SEGMENT_TRANSACTIONS',
                                                  ...
                                                  'CUSTOMER_PRDCT_AMNT_CREDITS', 'CUSTOMER_PRDCT_AMNT_CRD_GARANT',
                                                  'CUSTOMER_PRDCT_AMNT_CRDENERGY', 'CUSTOMER_PRDCT_AMNT_CRDCARDS',
                                                  'CUSTOMER_PRDCT_AMNT_CARDS', 'CUSTOMER_PRDCT_AMNT_ACC') --and
                             --h.value_date <= p_snapshot_value_date
                      group by h.attribute_id, h.object_id)
     select /*+parallel(4) leading(K)*/ bars.t_attribute_value(a.attribute_id, a.object_id, a.value, null, null, null, null, null, null, null, null, null)
        from   bars.customer c
        left join (select d.attribute_id, d.object_id, nh.value
                   from   data d
                   left join bars.attribute_number_history nh on nh.id = d.last_history_id) a on a.object_id = c.rnk
        where c.custtype = 3 and
              -- перевіряємо наявність даних за таким rnk в person
              exists (select 1
                      from   bars.person p
                      where  p.rnk = c.rnk);
         ...
         (select distinct rnk from bars.customer_address_update cau where cau.chgdate between trunc(p_dat) and trunc(p_dat)+0.99999)
                 ...
                 (select distinct rnk from bars.person_update pu where  pu.chgdate between trunc(p_dat) and trunc(p_dat)+0.99999)
                  ...
                  select
                       ...
                       c.okpo  as okpo,--ІНН
                       dm_import.add38phone(substr(mpno,1,20)) as telm,--мобільний
                       dm_import.add38phone(p.teld) teld,--домашній телефон
                       dm_import.add38phone(p.telw) telw,--робочий телефон
...
--                       ww.STMT, 06.12.2016 [COBUSUPABS-5030]  замена на c.STMT
    ...
    BEGIN
       ...
       THEN
          l_phone := '38' || g_phonecode || l_phone;
       END IF;
    ...
    begin
        ...
        loop
        begin
          /*fix for deposits policies*/
        if cur.OBJ_NAME = 'DEPOSITS' or cur.OBJ_NAME = 'DEPOSITS_PLT' then
          for i in (select t.branch from bars.branch t where t.branch like '/______/')   --TODO: another way?
          loop
              bars.bc.subst_branch(i.branch);
