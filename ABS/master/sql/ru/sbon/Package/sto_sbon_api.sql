...
CREATE OR REPLACE PACKAGE BODY SBON.STO_SBON_API as
  ...
  IS
  BEGIN
     RETURN 'Package header sto_sbon_api ' || G_HEADER_VERSION;
  END header_version;
  ...
  IS
  BEGIN
     RETURN 'Package body sto_sbon_api ' || G_BODY_VERSION;
  END body_version;
    procedure add_provider(
        p_id_dog in integer,
        p_nomdog in varchar2,
    ...
    is
    begin
        bars.sto_payment_utl.decline_payment_by_sbon(bars.bars_sqnc.rukey(p_id_plat), p_description);
    end;
    ...
    is
    begin
        bars.sto_payment_utl.set_payment_amount_by_sbon(bars.bars_sqnc.rukey(p_id_plat), p_suma_borg, p_suma_op, p_suma_kom);
    end;
    ...
    is
    begin
        bars.sto_payment_utl.set_fee_amount_by_sbon(bars.bars_sqnc.rukey(p_id_plat), p_suma_kom);
    end;
    ...
    is
    begin
        bars.sto_payment_utl.set_payment_purpose_by_sbon(bars.bars_sqnc.rukey(p_id_plat), p_payment_purpose);
    end;
    ...
    is
    begin
        bars.sto_payment_utl.close_payment_by_sbon(bars.bars_sqnc.rukey(p_id_plat), p_mfo, p_rozrah, p_kodorg, p_provider_purpose);
    end;
