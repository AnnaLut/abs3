...
PROMPT *** Create  constraint FK_AQ_SUBSCRIBERS_ACC_ACC ***
begin   
 execute immediate '
  ALTER TABLE BARSAQ.AQ_SUBSCRIBERS_ACC ADD CONSTRAINT FK_AQ_SUBSCRIBERS_ACC_ACC FOREIGN KEY (ACC)
...
begin   
 execute immediate '
  ALTER TABLE BARSAQ.AQ_SUBSCRIBERS_ACC ADD CONSTRAINT FK_AQ_SUBSCRIBERS_ACC_NAME FOREIGN KEY (NAME)
	  REFERENCES BARSAQ.AQ_SUBSCRIBERS (NAME) ENABLE';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
