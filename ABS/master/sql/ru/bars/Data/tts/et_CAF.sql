...
begin
  ...
  begin
    insert into op_rules(TAG, TT, OPT, USED4INPUT, ORD, VAL, NOMODIFY)
    values ('REZID', 'CAF', 'M', 1, 6, null, 1);
  exception
    when dup_val_on_index then null;
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (op_rules: ''REZID'', ''CAF'', ''M'', 1, 6, null, 1) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  --------------------------------
  ...
  begin
    insert into chklist_tts(idchk, tt, priority, f_big_amount, sqlval, f_in_charge)
    values (1, 'CAF', 2, null, '(f_teller_staff (userid,s,kv)=1 and branch_edit.get_branch_parameter_ex(branch, ''NOT2VISA'', ''0'') = ''0'')', null);
  exception
    when dup_val_on_index then null;
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (chklist_tts: 1, ''CAF'', 2, null, ''(f_teller_staff (userid,s,kv)=1 and branch_edit.get_branch_parameter_ex(branch, ''''NOT2VISA'''', ''''0'''') = ''''0'''')'', null) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  begin
