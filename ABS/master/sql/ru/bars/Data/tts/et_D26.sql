...
prompt Наименование операции: Списано комісію за зарахування коштів
declare
  cnt_  number;
begin
  --------------------------------
  ...
  begin
    ...
    values ('K26', 'Списано комісію за зарахування коштів', 0, null, 980, '#(nbs_ob22 (''6110'',''17''))', 980, null, null, '#(nbs_ob22 (''6110'',''17''))', null, 0, 0, 0, 0, 'F_TARIF(70, #(KVA), #(NLSB), #(S) )', null, null, null, null, null, '0000100000000000000010000000000000000100000000000000000000000000', 'Списано комісію за зарахування коштів');
  exception
    when dup_val_on_index then 
      update tts
         set tt='K26', name='Списано комісію за зарахування коштів', dk=0, nlsm=null, kv=980, nlsk='#(nbs_ob22 (''6110'',''17''))', kvk=980, nlss=null, nlsa=null, nlsb='#(nbs_ob22 (''6110'',''17''))', mfob=null, flc=0, fli=0, flv=0, flr=0, s='F_TARIF(70, #(KVA), #(NLSB), #(S) )', s2=null, sk=null, proc=null, s3800=null, rang=null, flags='0000100000000000000010000000000000000100000000000000000000000000', nazn='Списано комісію за зарахування коштів'
       where tt='K26';
  end;
  --------------------------------
  ----------- Реквизиты ----------
  ...
  exception
    ...
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (ps_tts: ''6110'', ''K26'', 1) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  --------------------------------
