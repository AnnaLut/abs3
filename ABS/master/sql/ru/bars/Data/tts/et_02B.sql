...
prompt Наименование операции: Прийм. гот. плат. за послуги (комунал,інтернет,охорона,тощо) (з коміс)
declare
  cnt_  number;
begin
  --------------------------------
  ...
  exception
    ...
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (ps_tts: ''2902'', ''02B'', 1) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  --------------------------------
  ...
  exception
    ...
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (tts_vob: 154, ''02B'', null) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  --------------------------------
  ...
  exception
    ...
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (chklist_tts: 1, ''02B'', 2, null, ''f_teller_staff (userid,s,kv)=1'', null) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  begin
