...
prompt Наименование операции: Internet-Banking: Списання з карт.рахунку міжбанк
declare
  cnt_  number;
begin
  --------------------------------
  ...
  begin
    insert into tts(tt, name, dk, nlsm, kv, nlsk, kvk, nlss, nlsa, nlsb, mfob, flc, fli, flv, flr, s, s2, sk, proc, s3800, rang, flags, nazn)
    values ('IB4', 'Internet-Banking: Списання з карт.рахунку міжбанк', 1, '#(bpk_get_transit(''20'',#(NLSB),#(NLSA),#(KVA)))', null, '#(get_proc_nls(''T00'',#(KVA)))', null, null, null, null, null, 1, 1, 0, 0, null, null, null, null, null, null, '0301000000001000000000000000000000110000000000000000000000000000', null);
  exception
    when dup_val_on_index then 
      update tts
         set tt='IB4', name='Internet-Banking: Списання з карт.рахунку міжбанк', dk=1, nlsm='#(bpk_get_transit(''20'',#(NLSB),#(NLSA),#(KVA)))', kv=null, nlsk='#(get_proc_nls(''T00'',#(KVA)))', kvk=null, nlss=null, nlsa=null, nlsb=null, mfob=null, flc=1, fli=1, flv=0, flr=0, s=null, s2=null, sk=null, proc=null, s3800=null, rang=null, flags='0301000000001000000000000000000000110000000000000000000000000000', nazn=null
       where tt='IB4';
  end;
  --------------------------------
  ----------- Реквизиты ----------
  ...
  exception
    ...
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (ps_tts: ''2605'', ''IB4'', 0) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  begin
  ...
  exception
    ...
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (ps_tts: ''2625'', ''IB4'', 0) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  begin
  ...
  exception
    ...
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (ps_tts: ''2655'', ''IB4'', 0) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  --------------------------------
  ...
  exception
    ...
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (tts_vob: 1, ''IB4'', null) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  begin
  ...
  exception
    ...
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (tts_vob: 6, ''IB4'', null) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  --------------------------------
  ...
  exception
    ...
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (folders_tts: 25, ''IB4'') - первичный ключ не найден!');
      else raise;
      end if;
  end;
end;
