...
prompt Наименование операции: W4. Зарахування коштів на картрах. фіз.осіб після обов"язкового продаж
declare
  cnt_  number;
begin
  --------------------------------
  ...
  begin
    ...
    values ('W4P', 'W4. Зарахування коштів на картрах. фіз.осіб після обов"язкового продаж', 1, null, 980, '#(bpk_get_transit('''',#(NLSA),#(NLSB),#(KVA)))', 980, null, null, null, null, 0, 0, 0, 0, null, null, null, null, null, null, '1000100000000000000000000001000000010000000000000000000000000000', 'Зарахування коштів на карткові рахунки фізичних осіб після обов"язкового продажу на МВРУ ');
  exception
    when dup_val_on_index then 
      update tts
         set tt='W4P', name='W4. Зарахування коштів на картрах. фіз.осіб після обов"язкового продаж', dk=1, nlsm=null, kv=980, nlsk='#(bpk_get_transit('''',#(NLSA),#(NLSB),#(KVA)))', kvk=980, nlss=null, nlsa=null, nlsb=null, mfob=null, flc=0, fli=0, flv=0, flr=0, s=null, s2=null, sk=null, proc=null, s3800=null, rang=null, flags='1000100000000000000000000001000000010000000000000000000000000000', nazn='Зарахування коштів на карткові рахунки фізичних осіб після обов"язкового продажу на МВРУ '
       where tt='W4P';
  end;
  --------------------------------
  ----------- Реквизиты ----------
  ...
  begin
    insert into op_rules(TAG, TT, OPT, USED4INPUT, ORD, VAL, NOMODIFY)
    values ('REZID', 'W4P', 'O', 1, 4, null, 1);
  exception
    when dup_val_on_index then null;
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (op_rules: ''REZID'', ''W4P'', ''O'', 1, 4, null, 1) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  begin
