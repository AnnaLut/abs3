...
prompt Создание / Обновление операции IB5
prompt Наименование операции: Internet-Banking: Поповнення карт.рахунку внутрішнє
declare
  cnt_  number;
begin
  --------------------------------
  ...
  begin
    ...
    values ('IB5', 'Internet-Banking: Поповнення карт.рахунку внутрішнє', 1, null, null, '#(bpk_get_transit(''19'',#(NLSA),#(NLSB),#(KVA)))', null, null, null, null, null, 0, 0, 0, 0, null, null, null, null, null, null, '0101000000000000000000000000000000010000000000000000000000000000', null);
  exception
    when dup_val_on_index then 
      update tts
         set tt='IB5', name='Internet-Banking: Поповнення карт.рахунку внутрішнє', dk=1, nlsm=null, kv=null, nlsk='#(bpk_get_transit(''19'',#(NLSA),#(NLSB),#(KVA)))', kvk=null, nlss=null, nlsa=null, nlsb=null, mfob=null, flc=0, fli=0, flv=0, flr=0, s=null, s2=null, sk=null, proc=null, s3800=null, rang=null, flags='0101000000000000000000000000000000010000000000000000000000000000', nazn=null
       where tt='IB5';
  end;
  --------------------------------
  ----------- Реквизиты ----------
  ...
  begin
    insert into chklist_tts(idchk, tt, priority, f_big_amount, sqlval, f_in_charge)
    values (30, 'IB5', 2, null, 'bpk_visa30(ref, 1)=1', null);
  exception
    when dup_val_on_index then null;
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (chklist_tts: 30, ''IB5'', 2, null, ''bpk_visa30(ref, 1)=1'', null) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  --------------------------------
  ...
  exception
    ...
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (folders_tts: 25, ''IB5'') - первичный ключ не найден!');
      else raise;
      end if;
  end;
end;
