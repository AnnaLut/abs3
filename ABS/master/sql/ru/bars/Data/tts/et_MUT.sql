...
begin
  ...
  begin
    insert into tts(tt, name, dk, nlsm, kv, nlsk, kvk, nlss, nlsa, nlsb, mfob, flc, fli, flv, flr, s, s2, sk, proc, s3800, rang, flags, nazn)
    values ('MUT', 'MUT Виплата коштів ІВ з пот/рах ФО (від "Claims conference ceef")', 1, null, 978, '#(tobopack.GetToboCASH)', 978, null, null, '#(tobopack.GetToboCASH)', null, 1, 0, 0, 0, null, null, 61, null, null, null, '0000100000000000000000000000000000010000000000000000000000000000', 'Виплата коштів ІВ з пот/рах ФО (від "Claims conference ceef") (з комісією)');
  exception
    when dup_val_on_index then 
      update tts
         set tt='MUT', name='MUT Виплата коштів ІВ з пот/рах ФО (від "Claims conference ceef")', dk=1, nlsm=null, kv=978, nlsk='#(tobopack.GetToboCASH)', kvk=978, nlss=null, nlsa=null, nlsb='#(tobopack.GetToboCASH)', mfob=null, flc=1, fli=0, flv=0, flr=0, s=null, s2=null, sk=61, proc=null, s3800=null, rang=null, flags='0000100000000000000000000000000000010000000000000000000000000000', nazn='Виплата коштів ІВ з пот/рах ФО (від "Claims conference ceef") (з комісією)'
       where tt='MUT';
  end;
  --------------------------------
  ----------- Реквизиты ----------
  ...
  begin
    insert into op_rules(TAG, TT, OPT, USED4INPUT, ORD, VAL, NOMODIFY)
    values ('REZID', 'MUT', 'M', 1, 5, null, 1);
  exception
    when dup_val_on_index then null;
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (op_rules: ''REZID'', ''MUT'', ''M'', 1, 5, null, 1) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  --------------------------------
