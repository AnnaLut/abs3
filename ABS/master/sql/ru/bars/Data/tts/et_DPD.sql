...
prompt Наименование операции: Поповнення в ін.валюті готівкою
declare
  cnt_  number;
begin
  --------------------------------
  ...
  begin
    insert into tts(tt, name, dk, nlsm, kv, nlsk, kvk, nlss, nlsa, nlsb, mfob, flc, fli, flv, flr, s, s2, sk, proc, s3800, rang, flags, nazn)
    values ('DPD', 'Поповнення в ін.валюті готівкою', 0, null, null, null, null, null, null, '#(tobopack.GetToboCASH)', null, 1, 0, 0, 0, null, null, null, null, null, 0, '0000100000000000000000000010000000010000000000000000000000000000', 'Поповнення згідно договору #{DPT_WEB.F_NAZN(''U'',#(ND))}');
  exception
    when dup_val_on_index then 
      update tts
         set tt='DPD', name='Поповнення в ін.валюті готівкою', dk=0, nlsm=null, kv=null, nlsk=null, kvk=null, nlss=null, nlsa=null, nlsb='#(tobopack.GetToboCASH)', mfob=null, flc=1, fli=0, flv=0, flr=0, s=null, s2=null, sk=null, proc=null, s3800=null, rang=0, flags='0000100000000000000000000010000000010000000000000000000000000000', nazn='Поповнення згідно договору #{DPT_WEB.F_NAZN(''U'',#(ND))}'
       where tt='DPD';
  end;
  --------------------------------
  ----------- Реквизиты ----------
  ...
  begin
    insert into op_rules(TAG, TT, OPT, USED4INPUT, ORD, VAL, NOMODIFY)
    values ('DJNR ', 'DPD', 'O', 1, 99, null, null);
  exception
    when dup_val_on_index then null;
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (op_rules: ''DJNR '', ''DPD'', ''O'', 1, 99, null, null) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  begin
  ...
  begin
    insert into op_rules(TAG, TT, OPT, USED4INPUT, ORD, VAL, NOMODIFY)
    values ('KOD_B', 'DPD', 'O', 1, null, '6', null);
  exception
    when dup_val_on_index then null;
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (op_rules: ''KOD_B'', ''DPD'', ''O'', 1, null, ''6'', null) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  begin
    insert into op_rules(TAG, TT, OPT, USED4INPUT, ORD, VAL, NOMODIFY)
    values ('KOD_G', 'DPD', 'O', 1, null, '804', null);
  exception
    when dup_val_on_index then null;
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (op_rules: ''KOD_G'', ''DPD'', ''O'', 1, null, ''804'', null) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  begin
    insert into op_rules(TAG, TT, OPT, USED4INPUT, ORD, VAL, NOMODIFY)
    values ('KOD_N', 'DPD', 'O', 1, null, '8446001', null);
  exception
    when dup_val_on_index then null;
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (op_rules: ''KOD_N'', ''DPD'', ''O'', 1, null, ''8446001'', null) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  begin
  ...
  begin
    ...
    values (1, 'DPD', 3, null, '(f_teller_staff (userid,s,kv)=1 and (branch_edit.get_branch_parameter_ex(branch, ''NOT2VISA'', ''0'') = ''0'') AND (EBP.HAS_EBP_DEPOSIT_ACCOUNTS(ref) = 0))', null);
  exception
    when dup_val_on_index then null;
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (chklist_tts: 1, ''DPD'', 3, null, ''(f_teller_staff (userid,s,kv)=1 and (branch_edit.get_branch_parameter_ex(branch, ''''NOT2VISA'''', ''''0'''') = ''''0'''') AND (EBP.HAS_EBP_DEPOSIT_ACCOUNTS(ref) = 0))'', null) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  begin
