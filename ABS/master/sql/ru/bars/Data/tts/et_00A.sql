...
prompt Наименование операции: 00A  Видача по ЧЕКУ з 2604 мат/забезп (соц.страх) з комiс. з 2600,2650
declare
  cnt_  number;
begin
  --------------------------------
  ...
  begin
    insert into op_rules(TAG, TT, OPT, USED4INPUT, ORD, VAL, NOMODIFY)
    values ('REZID', '00A', 'O', 1, 13, null, 1);
  exception
    when dup_val_on_index then null;
    when others then
      if ( sqlcode = -02291 ) then
        dbms_output.put_line('Не удалось добавить запись (op_rules: ''REZID'', ''00A'', ''O'', 1, 13, null, 1) - первичный ключ не найден!');
      else raise;
      end if;
  end;
  begin
