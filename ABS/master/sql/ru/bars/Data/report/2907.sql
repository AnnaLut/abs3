prompt ===================================== 
prompt == Звіт про зміни в статистичній звітності
prompt ===================================== 
...
begin     
   l_zpr.name := 'Звіт про зміни в статистичній звітності';
   l_zpr.pkey := '\BRS\SBM\DPT\102';
...
from (select DAT,decode (kod,1,kol,0) UAH_2620, decode (kod,2,kol,0) NER_2620,   
      ...
      FROM ( SELECT  KOD , SUM(KOL)KOL, DAT 
              FROM (select nbs NBS, decode(nbs,''2620'', decode(kv, 980, 1, 5), 
                                           ...
                                           )KOD, count(*) KOL,:sFdat1 DAT
                      from accounts   
                     where (nbs in (''2620'',  ''2630'', ''2635'') or (nbs=''2625'' and nls not like ''8%'')) 
                   ...
                   select nbs , decode(i.ob22, ''08'', 2, ''09'', 2, ''11'', 2, ''12'',2,''20'',3,''21'',4) ,
                    ...
                    where a.nbs  = ''2620''
                      and a.acc  = i.acc
                      and (a.dazs is null or a.dazs > to_date(:sFdat1,''dd.mm.yyyy''))
                      and i.ob22 in (''08'', ''09'', ''11'', ''12'', ''20'', ''21'') 
                    group by nbs, decode(i.ob22, ''08'', 2, ''09'', 2, ''11'', 2, ''12'',2,''20'',3,''21'',4) 
                    union all 
                   select nbs , 9 , count(*) ,:sFdat1 DAT
                     from accounts a, specparam_int i
                    where a.nbs  = ''2630'' and i.ob22 in (''11'', ''12'', ''13'', ''14'',''B6'',''B7'',''B8'',''B9'') 
                      and a.acc  = i.acc
                      and (a.dazs is null or a.dazs > to_date(:sFdat1,''dd.mm.yyyy''))
                    group by nbs
                ) group by  KOD, DAT
