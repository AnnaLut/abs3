...
SET DEFINE OFF 
PROMPT ===================================================================================== 
PROMPT *** Run *** ========== Scripts /Sql/Bars/Data/codeapp_AVVV.sql =========*** Run *** =
PROMPT ===================================================================================== 


PROMPT *** Create/replace  ARM  AVVV ***
  declare
    l_application_code varchar2(10 char) := 'AVVV';
    l_application_name varchar2(300 char) := 'АРМ Вiдкриття внутрiшнiх рахункiв';
	...
	d integer := 0;
begin
     DBMS_OUTPUT.PUT_LINE(' AVVV створюємо (або оновлюємо) АРМ АРМ Вiдкриття внутрiшнiх рахункiв ');
    ...
    DBMS_OUTPUT.PUT_LINE( chr(13)||chr(10)||' ********** Створюємо функцію Рах. для облiку РФ бранч-2 по 1 вал ********** ');
          --  Створюємо функцію Рах. для облiку РФ бранч-2 по 1 вал
      l := l +1;
      l_function_ids.extend(l);
      l_function_ids(l)   :=   abs_utils.add_func(
                                                  p_name     => 'Рах. для облiку РФ бранч-2 по 1 вал',
                                                  p_funcname => 'FunNSIEdit("[PROC=>OP_BR_REZ(:Br,:Kv)][PAR=>:Br(SEM= Бранч-2>,TYPE=S,REF=BRANCH2),:Kv(SEM= Вал>,TYPE=N,REF=TABVAL)][MSG=>OK РФ !]")',
                                                  p_rolename => 'CUST001' ,
                                                  p_frontend => l_application_type_id
                                                  );
    ...
    DBMS_OUTPUT.PUT_LINE( chr(13)||chr(10)||' ********** Створюємо функцію Перегляд та оновлення вiдкритих рахункiв ********** ');
          --  Створюємо функцію Перегляд та оновлення вiдкритих рахункiв
      l := l +1;
      l_function_ids.extend(l);
      l_function_ids(l)   :=   abs_utils.add_func(
                                                  p_name     => 'Перегляд та оновлення вiдкритих рахункiв',
                                                  p_funcname => 'ShowAccList(0, AVIEW_ALL, AVIEW_Linked | AVIEW_Limit | AVIEW_Financial | AVIEW_Interest | AVIEW_Access | AVIEW_Special, "a.acc in (select acc from CCK_AN_TMP)" )',
                                                  p_rolename => 'CUST001' ,
                                                  p_frontend => l_application_type_id
                                                  );


   DBMS_OUTPUT.PUT_LINE(chr(13)||chr(10)||'  Прикріпляємо ресурси функцій до даного АРМу (AVVV) - АРМ Вiдкриття внутрiшнiх рахункiв  ');
...
PROMPT ===================================================================================== 
PROMPT *** End *** ========== Scripts /Sql/Bars/Data/Applist/codeappAVVV.sql =========*** En
PROMPT ===================================================================================== 
