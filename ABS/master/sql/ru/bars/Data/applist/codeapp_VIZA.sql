...
PROMPT *** Create/replace  ARM  VIZA ***
  declare
    ...
    l_application_name varchar2(300 char) := 'АРМ Контролер підрозділу';
	...
	d integer := 0;
begin
     DBMS_OUTPUT.PUT_LINE(' VIZA створюємо (або оновлюємо) АРМ АРМ Контролер підрозділу ');
    ...
    DBMS_OUTPUT.PUT_LINE( chr(13)||chr(10)||' ********** Створюємо функцію ФМ. Вiдбiр документiв для Фiн. Монiторiнгу [ДОКУМЕНТИ ПIДРОЗДIЛУ] ********** ');
          --  Створюємо функцію ФМ. Вiдбiр документiв для Фiн. Монiторiнгу [ДОКУМЕНТИ ПIДРОЗДIЛУ]
      l := l +1;
      l_function_ids.extend(l);
      l_function_ids(l)   :=   abs_utils.add_func(
                                                  p_name     => 'ФМ. Вiдбiр документiв для Фiн. Монiторiнгу [ДОКУМЕНТИ ПIДРОЗДIЛУ]',
    ...
    DBMS_OUTPUT.PUT_LINE( chr(13)||chr(10)||' ********** Створюємо функцію Перегляд рахунків(кущові сховища) ********** ');
          --  Створюємо функцію Перегляд рахунків(кущові сховища)
      l := l +1;
      l_function_ids.extend(l);
      l_function_ids(l)   :=   abs_utils.add_func(
                                                  p_name     => 'Перегляд рахунків(кущові сховища)',
   ...
   DBMS_OUTPUT.PUT_LINE(chr(13)||chr(10)||'  Прикріпляємо ресурси функцій до даного АРМу (VIZA) - АРМ Контролер підрозділу  ');
