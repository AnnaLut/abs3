...
BEGIN 
        execute immediate  
          'begin  
               bpa.alter_policy_info(''FDAT'', ''FILIAL'' , ''M'', ''M'', ''M'', ''M'');
...
COMMENT ON COLUMN BARS.FDAT.KF IS '';




PROMPT *** Create  constraint FK_FDAT_KF ***
begin   
 execute immediate '
  ALTER TABLE BARS.FDAT ADD CONSTRAINT FK_FDAT_KF FOREIGN KEY (KF)
...
begin   
 execute immediate '
  ALTER TABLE BARS.FDAT ADD CONSTRAINT PK_FDAT PRIMARY KEY (KF, FDAT)
...
PROMPT *** Create  constraint CC_FDAT_KF_NN ***
begin   
 execute immediate '
  ALTER TABLE BARS.FDAT MODIFY (KF CONSTRAINT CC_FDAT_KF_NN NOT NULL ENABLE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
begin   
 execute immediate '
  CREATE UNIQUE INDEX BARS.PK_FDAT ON BARS.FDAT (KF, FDAT) 
