...
BEGIN 
        execute immediate  
          'begin  
               ...
               bpa.alter_policy_info(''SV_OWNER'', ''WHOLE'' , null, ''E'', ''E'', ''E'');
               null;
           end; 
          '; 
END; 
...
PROMPT *** Create  constraint FK_SVPERSON_SVOZN ***
begin   
 execute immediate '
  ALTER TABLE BARS.SV_OWNER ADD CONSTRAINT FK_SVPERSON_SVOZN FOREIGN KEY (OZN)
	  REFERENCES BARS.SV_OZN (ID) ENABLE';
exception when others then
...
begin   
 execute immediate '
  ALTER TABLE BARS.SV_OWNER MODIFY (OZN CONSTRAINT CC_SVOWNER_OZN_NN NOT NULL ENABLE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
begin   
 execute immediate '
  ALTER TABLE BARS.SV_OWNER ADD CONSTRAINT CC_SVOWNER_TYPE CHECK (type in (0,1,2)) ENABLE';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
grant SELECT                                                                 on SV_OWNER        to RPBN002;
