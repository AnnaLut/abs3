...
BEGIN 
        execute immediate  
          'begin  
               bpa.alter_policy_info(''FIN_CALCULATIONS'', ''FILIAL'' , null, null, null, null);
               bpa.alter_policy_info(''FIN_CALCULATIONS'', ''WHOLE'' , null, null, null, null);
               null;
           end; 
          '; 
END; 
...
PROMPT *** Create  constraint SYS_C002313657 ***
begin   
 execute immediate '
  ALTER TABLE BARS.FIN_CALCULATIONS MODIFY (DATD NOT NULL ENABLE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
