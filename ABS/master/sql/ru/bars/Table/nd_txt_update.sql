...
BEGIN 
        execute immediate  
          'begin  
               bpa.alter_policy_info(''ND_TXT_UPDATE'', ''FILIAL'' , ''F'', ''F'', ''F'', null);
               bpa.alter_policy_info(''ND_TXT_UPDATE'', ''WHOLE'' , ''C'', ''C'', ''C'', null);
               null;
           end; 
          '; 
END; 
...
grant DELETE,INSERT,SELECT,UPDATE                                            on ND_TXT_UPDATE   to BARS_ACCESS_DEFROLE;
grant SELECT                                                                 on ND_TXT_UPDATE   to BARS_SUP;
