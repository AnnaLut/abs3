...
PROMPT *** Create  constraint FK_SWIOPERLIST_TTS_PA ***
begin   
 execute immediate '
  ALTER TABLE BARS.SWI_OPER_LIST ADD CONSTRAINT FK_SWIOPERLIST_TTS_PA FOREIGN KEY (TT_PA)
...
PROMPT *** Create  constraint FK_SWIOPERLIST_TTS ***
begin   
 execute immediate '
  ALTER TABLE BARS.SWI_OPER_LIST ADD CONSTRAINT FK_SWIOPERLIST_TTS FOREIGN KEY (TT)
	  REFERENCES BARS.TTS (TT) ENABLE';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
...
PROMPT *** Create  constraint FK_SWIOPERLIST_TTS_980 ***
begin   
 execute immediate '
  ALTER TABLE BARS.SWI_OPER_LIST ADD CONSTRAINT FK_SWIOPERLIST_TTS_980 FOREIGN KEY (TT_980)
	  REFERENCES BARS.TTS (TT) ENABLE';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
...
grant DELETE,FLASHBACK,INSERT,SELECT,UPDATE                                  on SWI_OPER_LIST   to BARS_ACCESS_DEFROLE;
grant DELETE,INSERT,SELECT,UPDATE                                            on SWI_OPER_LIST   to START1;
grant FLASHBACK,SELECT                                                       on SWI_OPER_LIST   to WR_REFREAD;
