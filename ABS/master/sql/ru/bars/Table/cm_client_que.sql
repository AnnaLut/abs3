...
begin   
 execute immediate '
  ALTER TABLE BARS.CM_CLIENT_QUE ADD CONSTRAINT CC_CMCLIENTQUE_OPERSTATUS CHECK (oper_status in (1,2,3,10)) ENABLE';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
begin   
 execute immediate '
  ALTER TABLE BARS.CM_CLIENT_QUE MODIFY (ID CONSTRAINT CC_CMCLIENTQUE_ID_NN NOT NULL ENABLE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
-- Add/modify columns 
begin
    execute immediate 'alter table cm_client_que add shortnameowner varchar2(105)';
 exception when others then 
    if sqlcode = -1430 then null; else raise; 
    end if; 
end;
/
alter table CM_CLIENT_QUE
...
begin
  ...
  check (oper_status in (1,2,3,10,99))';
 exception when others then 
