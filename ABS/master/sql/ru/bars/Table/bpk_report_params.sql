...
begin   
 execute immediate '
  ALTER TABLE BARS.BPK_REPORT_PARAMS ADD CONSTRAINT CC_BPKREPORTPARAMS_TYPE CHECK (type in (''C'',''N'')) ENABLE';
exception when others then
...
begin   
 execute immediate '
  ALTER TABLE BARS.BPK_REPORT_PARAMS MODIFY (CODE CONSTRAINT CC_BPKREPORTPARAMS_CODE_NN NOT NULL ENABLE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
