...
PROMPT *** Create  constraint FK_RKOTARIF_RKOORGAN ***
begin   
 execute immediate '
  ALTER TABLE BARS.RKO_TARIF ADD CONSTRAINT FK_RKOTARIF_RKOORGAN FOREIGN KEY (ORGAN)
	  REFERENCES BARS.RKO_ORGAN (ID) ENABLE';
exception when others then
...
PROMPT *** Create  constraint FK_RKOTARIF_RKOINDPAR ***
begin   
 execute immediate '
  ALTER TABLE BARS.RKO_TARIF ADD CONSTRAINT FK_RKOTARIF_RKOINDPAR FOREIGN KEY (INDPAR)
	  REFERENCES BARS.RKO_INDPAR (ID) ENABLE';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
