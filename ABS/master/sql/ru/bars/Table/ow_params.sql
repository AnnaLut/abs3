...
BEGIN 
        execute immediate  
          'begin  
               bpa.alter_policy_info(''OW_PARAMS'', ''FILIAL'' , null, null, null, null);
               bpa.alter_policy_info(''OW_PARAMS'', ''WHOLE'' , null, null, null, null);
               null;
           end; 
          '; 
END; 
...
begin   
 execute immediate '
  ALTER TABLE BARS.OW_PARAMS ADD CONSTRAINT PK_OWPARAMS PRIMARY KEY (PAR)
...
begin   
 execute immediate '
  ALTER TABLE BARS.OW_PARAMS MODIFY (PAR CONSTRAINT CC_OWPARAMS_PAR_NN NOT NULL ENABLE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
begin   
 execute immediate '
  CREATE UNIQUE INDEX BARS.PK_OWPARAMS ON BARS.OW_PARAMS (PAR) 
