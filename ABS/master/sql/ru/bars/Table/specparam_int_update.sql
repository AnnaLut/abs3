...
begin   
 execute immediate '
  ALTER TABLE BARS.SPECPARAM_INT_UPDATE MODIFY (KF CONSTRAINT CC_SPECPARAMINTUPD_KF_NN NOT NULL ENABLE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
begin   
 execute immediate '
  ALTER TABLE BARS.SPECPARAM_INT_UPDATE MODIFY (IDUPD CONSTRAINT CC_SPECPARAMINTUPD_IDUPD_NN NOT NULL ENABLE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
begin   
 execute immediate '
  ALTER TABLE BARS.SPECPARAM_INT_UPDATE MODIFY (USER_NAME CONSTRAINT CC_SPECPARAMINTUPD_USERNAME_NN NOT NULL ENABLE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
begin   
 execute immediate '
  ALTER TABLE BARS.SPECPARAM_INT_UPDATE MODIFY (FDAT CONSTRAINT CC_SPECPARAMINTUPD_DATE_NN NOT NULL ENABLE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
PROMPT *** Create  index XAI_SPECPARAM_INT_UPDATE_ACC ***
begin   
 execute immediate '
  CREATE INDEX BARS.XAI_SPECPARAM_INT_UPDATE_ACC ON BARS.SPECPARAM_INT_UPDATE (ACC) 
  PCTFREE 10 INITRANS 2 MAXTRANS 255 COMPUTE STATISTICS 
  TABLESPACE BRSBIGD ';
exception when others then
  if  sqlcode=-955  then null; else raise; end if;
