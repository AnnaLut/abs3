...
PROMPT *** Create  constraint FK_RKOMETHOD_RKOMETHODRATE ***
begin   
 execute immediate '
  ALTER TABLE BARS.RKO_METHOD ADD CONSTRAINT FK_RKOMETHOD_RKOMETHODRATE FOREIGN KEY (ID_RATE)
	  REFERENCES BARS.RKO_METHOD_RATE (ID) ENABLE';
exception when others then
...
PROMPT *** Create  constraint FK_RKOMETHOD_RKOMETHODTARIF ***
begin   
 execute immediate '
  ALTER TABLE BARS.RKO_METHOD ADD CONSTRAINT FK_RKOMETHOD_RKOMETHODTARIF FOREIGN KEY (ID_TARIF)
	  REFERENCES BARS.RKO_METHOD_TARIF (ID) ENABLE';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
