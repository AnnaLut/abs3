...
PROMPT *** Create  constraint SYS_C002755386 ***
begin   
 execute immediate '
  ALTER TABLE BARS.STO_SBON_ORDER_CONTR MODIFY (ID NOT NULL ENABLE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
grant SELECT                                                                 on STO_SBON_ORDER_CONTR to BARSUPL;
grant SELECT                                                                 on STO_SBON_ORDER_CONTR to BARS_ACCESS_DEFROLE;
grant SELECT                                                                 on STO_SBON_ORDER_CONTR to BARS_DM;
grant SELECT                                                                 on STO_SBON_ORDER_CONTR to SBON;
