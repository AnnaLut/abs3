...
BEGIN 
        execute immediate  
          'begin  
               bpa.alter_policy_info(''REZ_CR'', ''FILIAL'' , null, null, null, null);
               bpa.alter_policy_info(''REZ_CR'', ''WHOLE'' , null, null, null, null);
               null;
           end; 
          '; 
END; 
...
begin
 execute immediate   'alter table REZ_CR add (Z number) ';
exception when others then
  -- ORA-01430: column being added already exists in table
  if SQLCODE = - 01430 then null;   else raise; end if; 
end;
/
COMMENT ON COLUMN REZ_CR.Z IS 'Інтегральний показник';

begin
 execute immediate   'alter table REZ_CR add (FIN_KOL integer) ';
exception when others then
  -- ORA-01430: column being added already exists in table
  if SQLCODE = - 01430 then null;   else raise; end if; 
end;
/
COMMENT ON COLUMN REZ_CR.FIN_KOL IS 'Клас боржника-юридичної особи (визначений на підставі оцінки фінансового стану) з урахуванням своєчасності сплати ним боргу';

begin
 execute immediate   'alter table REZ_CR add (RNK_SK integer) ';
exception when others then
  -- ORA-01430: column being added already exists in table
  if SQLCODE = - 01430 then null;   else raise; end if; 
end;
/
COMMENT ON COLUMN REZ_CR.RNK_SK IS 'Боржник належить до групи юридичних осіб під спільним контролем';

begin
 execute immediate   'alter table REZ_CR add (FIN_SK integer) ';
exception when others then
  -- ORA-01430: column being added already exists in table
  if SQLCODE = - 01430 then null;   else raise; end if; 
end;
/
COMMENT ON COLUMN REZ_CR.FIN_SK IS 'Скоригований клас боржника з урахуванням належності до групи під спільним контрорлем';

begin
 execute immediate   'alter table REZ_CR add (FIN_PK integer) ';
exception when others then
  -- ORA-01430: column being added already exists in table
  if SQLCODE = - 01430 then null;   else raise; end if; 
end;
/
COMMENT ON COLUMN REZ_CR.FIN_PK IS 'Скоригований клас боржника з урахуванням належності до групи належності до групи пов"язаних контрагентів';
