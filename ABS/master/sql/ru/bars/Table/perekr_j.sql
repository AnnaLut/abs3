...
begin   
 execute immediate '
  ALTER TABLE BARS.PEREKR_J ADD CONSTRAINT FK_PEREKRJ_ACCOUNTS FOREIGN KEY (KF, ACCS)
...
begin   
 execute immediate '
  ALTER TABLE BARS.PEREKR_J ADD CONSTRAINT FK_PEREKRJ_ACCOUNTS3 FOREIGN KEY (KF, ACC)
	  REFERENCES BARS.ACCOUNTS (KF, ACC) ENABLE';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
...
grant DELETE,INSERT,SELECT,UPDATE                                            on PEREKR_J        to BARS_ACCESS_DEFROLE;
