...
COMMENT ON COLUMN BARS.MSG_SUBMIT_DATA.PAYEDREF IS 'REF плати за СМС інформування';
...
PROMPT *** Create  constraint PK_MSGSUBMIT ***
begin   
 execute immediate '
  ALTER TABLE BARS.MSG_SUBMIT_DATA ADD CONSTRAINT PK_MSGSUBMIT PRIMARY KEY (MSG_ID)
...
PROMPT *** Create  constraint CC_MSGSUBMIT_ENCODE_CC ***
begin   
 execute immediate '
  ALTER TABLE BARS.MSG_SUBMIT_DATA ADD CONSTRAINT CC_MSGSUBMIT_ENCODE_CC CHECK (encode in (''cyr'',''lat'')) ENABLE';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
grant SELECT                                                                 on MSG_SUBMIT_DATA to BARS_ACCESS_DEFROLE;
