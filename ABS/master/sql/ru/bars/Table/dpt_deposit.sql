...
BEGIN 
        execute immediate  
          'begin  
               bpa.alter_policy_info(''DPT_DEPOSIT'', ''FILIAL'' , null, null, null, null);
               bpa.alter_policy_info(''DPT_DEPOSIT'', ''WHOLE'' , null, null, null, null);
               null;
           end; 
          '; 
END; 
...
grant ALTER,DEBUG,DELETE,FLASHBACK,INSERT,ON COMMIT REFRESH,QUERY REWRITE,SELECT,UPDATE on DPT_DEPOSIT     to BARS_ACCESS_DEFROLE;
...
grant SELECT                                                                 on DPT_DEPOSIT     to BARS_SUP;
