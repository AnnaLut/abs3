...
begin   
 execute immediate '
  ALTER TABLE BARS.WCS_PARTNER_TYPES ADD CONSTRAINT CC_PARTNERTYPES_NAME_NN CHECK (NAME IS NOT NULL) ENABLE';
exception when others then
...
PROMPT *** Create  constraint CC_PARTNERTYPES_ID_NN ***
begin   
 execute immediate '
  ALTER TABLE BARS.WCS_PARTNER_TYPES ADD CONSTRAINT CC_PARTNERTYPES_ID_NN CHECK (ID IS NOT NULL) ENABLE';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
