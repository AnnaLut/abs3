...
COMMENT ON COLUMN BARS.MV_WCS_SUR_GRP_QUEST_RELATION.SURVEY_ID IS '';
COMMENT ON COLUMN BARS.MV_WCS_SUR_GRP_QUEST_RELATION.SGROUP_ID IS '';
COMMENT ON COLUMN BARS.MV_WCS_SUR_GRP_QUEST_RELATION.QUESTION_ID IS '';




PROMPT *** Create  constraint SYS_C003177453 ***
begin   
 execute immediate '
  ALTER TABLE BARS.MV_WCS_SUR_GRP_QUEST_RELATION MODIFY (QUESTION_ID NOT NULL ENABLE)';
exception when others then
...
PROMPT *** Create  constraint SYS_C003177452 ***
begin   
 execute immediate '
  ALTER TABLE BARS.MV_WCS_SUR_GRP_QUEST_RELATION MODIFY (SGROUP_ID NOT NULL ENABLE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
PROMPT *** Create  constraint SYS_C003177451 ***
begin   
 execute immediate '
  ALTER TABLE BARS.MV_WCS_SUR_GRP_QUEST_RELATION MODIFY (SURVEY_ID NOT NULL ENABLE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
