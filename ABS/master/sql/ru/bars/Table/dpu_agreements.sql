...
BEGIN 
        execute immediate  
          'begin  
               bpa.alter_policy_info(''DPU_AGREEMENTS'', ''FILIAL'' , null, null, null, null);
               bpa.alter_policy_info(''DPU_AGREEMENTS'', ''WHOLE'' , null, null, null, null);
               null;
           end; 
          '; 
END; 
...
begin   
 execute immediate '
  ALTER TABLE BARS.DPU_AGREEMENTS ADD CONSTRAINT FK_DPUAGRMNTS_DPUAGRMNTTYPES FOREIGN KEY (AGRMNT_TYPE)
	  REFERENCES BARS.DPU_AGREEMENT_TYPES (ID) ENABLE';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
grant SELECT                                                                 on DPU_AGREEMENTS  to BARS_ACCESS_DEFROLE;
