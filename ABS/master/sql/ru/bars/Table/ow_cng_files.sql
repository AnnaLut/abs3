...
BEGIN 
        execute immediate  
          'begin  
               bpa.alter_policy_info(''OW_CNG_FILES'', ''FILIAL'' , null, null, null, null);
...
begin   
 execute immediate '
  ALTER TABLE BARS.OW_CNG_FILES MODIFY (FILE_DATE CONSTRAINT CC_OWCNGFILES_FILEDATE_NN NOT NULL ENABLE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
begin   
 execute immediate '
  ALTER TABLE BARS.OW_CNG_FILES MODIFY (FILE_NAME CONSTRAINT CC_OWCNGFILES_FILENAME_NN NOT NULL ENABLE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
begin   
 execute immediate '
  ALTER TABLE BARS.OW_CNG_FILES MODIFY (ID CONSTRAINT CC_OWCNGFILES_ID_NN NOT NULL ENABLE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
