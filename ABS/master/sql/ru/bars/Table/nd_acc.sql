...
PROMPT *** Create  index I1_ND_ACC ***
begin   
 execute immediate '
  CREATE INDEX BARS.I1_ND_ACC ON BARS.ND_ACC (ACC) 
  PCTFREE 10 INITRANS 2 MAXTRANS 255 COMPUTE STATISTICS 
  TABLESPACE BRSMDLI ';
exception when others then
  if  sqlcode=-955  then null; else raise; end if;
...
grant ALTER,DELETE,INSERT,SELECT,UPDATE                                      on ND_ACC          to BARS_ACCESS_DEFROLE;
grant SELECT                                                                 on ND_ACC          to BARS_DM;
grant SELECT                                                                 on ND_ACC          to BARS_SUP;
...
PROMPT *** Create SYNONYM  to ND_ACC ***

  CREATE OR REPLACE SYNONYM BARSDWH_ACCESS_USER.ND_ACC FOR BARS.ND_ACC;
