...
BEGIN 
        execute immediate  
          'begin  
               bpa.alter_policy_info(''CUSTOMER'', ''FILIAL'' , null, null, null, null);
               bpa.alter_policy_info(''CUSTOMER'', ''WHOLE'' , null, null, null, null);
               null;
           end; 
          '; 
END; 
...
begin   
 execute immediate '
  ALTER TABLE BARS.CUSTOMER ADD CONSTRAINT FK_CUSTOMER_SPRREG FOREIGN KEY (C_REG, C_DST)
	  REFERENCES BARS.SPR_REG (C_REG, C_DST) ENABLE';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
PROMPT *** Create  constraint UK_CUSTOMER_SAB ***
begin   
 execute immediate '
  ALTER TABLE BARS.CUSTOMER ADD CONSTRAINT UK_CUSTOMER_SAB UNIQUE (SAB)
...
PROMPT *** Create  constraint CC_CUSTOMER_BRANCH_NN ***
begin   
 execute immediate '
  ALTER TABLE BARS.CUSTOMER MODIFY (BRANCH CONSTRAINT CC_CUSTOMER_BRANCH_NN NOT NULL ENABLE)';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
...
PROMPT *** Create  index UK_CUSTOMER_SAB ***
begin   
 execute immediate '
  CREATE UNIQUE INDEX BARS.UK_CUSTOMER_SAB ON BARS.CUSTOMER (SAB) 
  PCTFREE 10 INITRANS 2 MAXTRANS 255 COMPUTE STATISTICS 
  TABLESPACE BRSBIGI ';
exception when others then
  if  sqlcode=-955  then null; else raise; end if;
...
grant SELECT                                                                 on CUSTOMER        to BARS;
...
grant SELECT                                                                 on CUSTOMER        to BARS_SUP;
...
grant SELECT                                                                 on CUSTOMER        to FINMON;
...
PROMPT *** Create SYNONYM  to CUSTOMER ***
  ...
  CREATE OR REPLACE SYNONYM BARSDWH_ACCESS_USER.CUSTOMER FOR BARS.CUSTOMER;
