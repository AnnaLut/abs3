...
COMMENT ON COLUMN BARS.SKRYNKA_STAFF.USERID IS 'код пользовател€';
COMMENT ON COLUMN BARS.SKRYNKA_STAFF.TIP IS 'тип исполнител€';
...
COMMENT ON COLUMN BARS.SKRYNKA_STAFF.MFO IS 'ћ‘ќ';
...
COMMENT ON COLUMN BARS.SKRYNKA_STAFF.MODE_TIME IS '';
COMMENT ON COLUMN BARS.SKRYNKA_STAFF.WEEKEND IS '';
COMMENT ON COLUMN BARS.SKRYNKA_STAFF.APPROVED IS '';
COMMENT ON COLUMN BARS.SKRYNKA_STAFF.ID IS '';
COMMENT ON COLUMN BARS.SKRYNKA_STAFF.ACTIV IS '';
...
begin   
 execute immediate '
  ALTER TABLE BARS.SKRYNKA_STAFF ADD CONSTRAINT FK_SKRYNKA_STAFF_TIP FOREIGN KEY (TIP)
	  REFERENCES BARS.SKRYNKA_STAFF_TIP (TIP) ENABLE';
exception when others then
  if  sqlcode=-2260 or sqlcode=-2261 or sqlcode=-2264 or sqlcode=-2275 or sqlcode=-1442 then null; else raise; end if;
 end;
/
