...
PROMPT *** Create  view WEB_SAL ***
  ...
  CREATE OR REPLACE FORCE VIEW BARS.WEB_SAL ("ACC", "NLS", "KV", "LCV", "DIG", "DENOM", "NMS", "OST", "DOS", "KOS", "OSTC", "OSTB", "OSTF", "DAPP", "DAOS", "DAZS", "TOBO", "TIP", "PAP", "BLKK", "BLKD", "MDATE", "POS", "NBS", "ISP", "RNK", "KF", "OB22", "NLSALT", "LIM", "OSTQ", "DOSQ", "KOSQ", "NBS2", "VID", "TRCN", "ACCC", "SEC", "BRANCH") AS 
  SELECT s.ACC,
       s.NLS,
       s.KV,
       v.LCV,
       ...
       s.TIP,
       s.PAP,
       p.BLKK,p.BLKD,p.MDATE,p.POS,p.NBS,p.ISP,p.RNK,
       p.KF,
       p.OB22,
       p.NLSALT,
       p.LIM,
       ...
       , p.branch
FROM saldo p,
    tabval$global v, sal s /* именно sal а не sal_branch, т.к. пользователь может видеть счета чужих бранчей */
WHERE p.acc=s.acc and s.kv=v.kv and s.fdat=bankdate;
