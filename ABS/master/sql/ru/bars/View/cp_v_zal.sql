...
PROMPT *** Create  view CP_V_ZAL ***
  ...
  SELECT x.B,
          ...
          x.KOL_ALL,
          x.kolz,
          decode(x.kolz,null,null,F_GET_FROM_ACCOUNTSPV_DAT2(x.spid,x.acc,x.b)) datz,
     ...
     FROM cp_zal z,
          ...
          (SELECT d.B, 0.01 KF,
                  ...
                  ROUND(-fost (e.acc, d.B) / 100 / f_cena_cp(k.id,d.B,0),5) KOL_ALL,
             to_number(trim(F_GET_FROM_ACCOUNTSPV(sp.spid,e.acc,d.b))) KOLZ,
                  e.accd,
                  e.accp,
                  e.accr,
                  e.accr2,
             ...
             FROM cp_deal e,
                  cp_kod k,
                  accounts a,
                  (SELECT NVL (TO_DATE (pul.get_mas_ini_val ('sFdat1'),'dd.mm.yyyy'),gl.bd) B  FROM DUAL) d,
