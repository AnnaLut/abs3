...
PROMPT *** Create  view V_REZ_2605 ***
  ...
  SELECT q.ACC, q.B,q.rnk, q.tobo, q.acc, q.kv, q.NBS, q.nls, q.daos,q.mdate,f.obs,f.fin,f.kat,f.k,
             q.BV               / 100 BV ,         q.BV*(1-f.k)       / 100 PV ,
   GREATEST( q.BV*f.k -q.ZAL, 0)/ 100 REZ,
             q.ZAL              / 100 ZAL
from acc_fin_obs_kat f,
   (select v.B, a.acc, a.rnk, a.tobo, a.kv, a.nls, a.nbs, a.daos, a.mdate, -rez1.ostc96(a.acc, Dat_last (v.b-4,v.b)) BV,
          NVL ( (SELECT SUM(s) FROM tmp_rez_obesp23 WHERE dat = v.B AND accs = a.acc),0) zal
    FROM v_gl a, V_SFDAT v WHERE a.nbs='2605' AND rez1.ostc96(a.acc,Dat_last (v.b-4,v.b))<0
    ) q
WHERE  q.acc = f.acc(+);
