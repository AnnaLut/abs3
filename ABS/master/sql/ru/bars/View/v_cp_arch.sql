...
PROMPT *** Create  view V_CP_ARCH ***
  ...
  CREATE OR REPLACE FORCE VIEW BARS.V_CP_ARCH ("REF_REPO", "REF", "ID", "CP_ID", "KV", "NAME", "DAT_UG", "DAT_OPL", "TIP", "IR", "DAT_ROZ", "SUMB", "T", "TQ", "N", "D", "P", "R", "S", "Z", "VD", "VP", "STR_REF", "OP", "SOS", "SN", "REF_MAIN", "NOM", "ND") AS 
  SELECT a.REF_REPO,
          a.REF,
          a.ID,
          k.CP_ID,
     ...
     FROM CP_ARCH a,
          ...
          (SELECT id,
             ...
             FROM cp_kod
            WHERE id > 0) k
    WHERE k.id = a.ID AND a.REF = o.REF AND NVL (o.sos, 3) > 0;
