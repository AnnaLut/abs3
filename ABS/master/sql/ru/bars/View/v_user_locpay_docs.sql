...
PROMPT *** Create  view V_USER_LOCPAY_DOCS ***
  ...
  CREATE OR REPLACE FORCE VIEW BARS.V_USER_LOCPAY_DOCS ("COLOR1", "COLOR2", "VDAT", "REF", "TT", "NLSA", "NLSB", "MFOB", "NB_B", "S", "S_", "DK", "SK", "LCV1", "DIG1", "USERID", "CHK", "NAZN", "LCV2", "DIG2", "S2", "S2_", "ND", "NEXTVISAGRP", "KV", "KV2", "FLAGS", "DEAL_TAG", "DATD", "PDAT", "PRTY", "SOS", "NAM_B", "MFOA", "NB_A", "DATP", "VOB", "NAM_A", "BRANCH", "ID_A", "ID_B") AS 
  SELECT SIGN (a.vdat - bankdate) AS color1,
    ...
    WHERE     v1.kv = a.kv
          ...
          AND a.tt = tt.tt
          and a.tt in ('WO5','WO6')
          AND a.nextvisagrp = '02'            -- 02  группа визирования
          AND a.mfoa = ba.mfo
          AND a.mfob = bb.mfo
;
