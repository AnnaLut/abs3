...
PROMPT *** Create  view STO_WEB ***
  ...
  CREATE OR REPLACE FORCE VIEW BARS.STO_WEB ("NMK", "NLSA", "ORD", "MFOB", "NLSB", "POLU", "OKPO", "DAT1", "DAT2", "NAZN", "FSUM", "IDD", "CALEND") AS 
  SELECT c.NMK,
          d.NLSA,
          d.ORD,
          d.MFOB,
          ...
          d.FSUM,
          d.idd,
          '<a href="'|| '/barsroot/tools/sto/sto_calendar.aspx?IDD='|| d.idd||'&mode=RO'|| '" onclick="window.open(this.href, '''', ''width=950, height=800, resizable=yes,scrollbars=yes,status=yes''); return false">'|| 'Календар подій' || '</a>'
           calend
     FROM STO_LST l,
          STO_DET d,
          customer c,
...
grant DELETE,INSERT,SELECT,UPDATE                                            on STO_WEB         to BARS_ACCESS_DEFROLE;
