  ...
  SELECT DISTINCT
      ...
      WHERE     w.product_code = k.product_code
       AND k.sub_code in (select code from w4_subproduct where flag_kk = 2)
            ...
            and nvl (w.date_open, bankdate) <= bankdate
            and nvl (w.date_close, bankdate + 1) > bankdate  
            
            and nvl (k.date_open, bankdate) <= bankdate
            and nvl (k.date_close, bankdate + 1) > bankdate              
   ORDER BY w.code;
