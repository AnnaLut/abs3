...
*/
  ...
  begin
    select 0 into ro.ref  from kaz_zobt where  DATD = VDAT_ and rownum=1;
     raise_application_error(-20100,'Повторне виконання за '||to_char(VDAT_,'dd.mm.yyyy') );
  EXCEPTION WHEN NO_DATA_FOUND THEN
    delete from KAZ_ZOBT;
  end;
...
grant EXECUTE                                                                on KAZ_ZOBP        to PYOD001;
