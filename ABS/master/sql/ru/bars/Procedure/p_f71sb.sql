...
% VERSION     : 10.08.2012 (09.12.2009,24.11.09,26.10.09)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%10.08.2012 - убрал удаление данных из табл. OTCN_F71_HISTORY которая
              используется при формировании файла #D8
              (осталось случайно т.к. создавалась процедура из P_FD8_nn)
   ...
   IS
      SELECT   *
          FROM (SELECT a.acc, a.nls, c.nmk, c.rnk, TRIM (f.k081),
                       ...
                       2 - MOD (c.codcagent, 2), c.country, c.c_reg, c.ved,
...
--                            WHERE datf = dat2_);
      ...
      txt_sql :=
         ...
         || 'c.country, c.c_reg, c.ved, DECODE (c.prinsider, NULL, 2, 0, 2, 99, 2, 1) prins, '
         || 'a.nbs, a.daos, a.mdate, '
         ...
         SELECT rnk, nmk, okpo, codcagent, country, c_reg, ved, prins,
           ...
           FROM (SELECT   /*+index(v.s) index(v.s1)*/
                    ...
                    WHERE s.fdat <= dat_
                      ...
                      AND k.kf = '71'
                      AND v.nls LIKE k.r020 || '%'
--                      AND (v.nls,v.kv) not in (SELECT nls,kv
--                                               FROM kf91
...
--                                                                         where kf='71'))
                      AND s.acc = ca.acc
                      AND c.rnk = ca.rnk
                      AND (our_okpo_ = '0' or ltrim(c.okpo, '0') <> our_okpo_)
                      AND c.codcagent < 7
                      AND (   (    SUBSTR (v.nls, 1, 4) NOT IN
                                      ('1600',
                                       '2600',
...
--         || '                                                   where kf=''71'')) '
         ...
         || '   AND ca.rnk = r.rnk '
         || '   AND r.fs = f.k080(+) '
         || '   AND (:our_okpo_ = ''0'' or ltrim(r.okpo, ''0'') <> :our_okpo_) '
...
--------------------------------------------------------------------------
   ...
   ELSE
      OPEN saldo FOR txt_sql USING dat_, dat_, dat_, dat1_, dat_, our_okpo_, our_okpo_, prnk_,
      prnk_;
   END IF;
...
PROMPT *** Create  grants  P_F71SB ***
grant EXECUTE                                                                on P_F71SB         to BARS_ACCESS_DEFROLE;
grant EXECUTE                                                                on P_F71SB         to RPBN002;
