create or replace procedure AMOVE9 (p_ND number)  IS -- АВТО-ПЕРЕКЛАССИФИКАЦИЯ КД согласно доп.рекв. по вектору: от... до 01.01.2018 . далее - помесячно до 01.хх.2018
  ...
  procedure AMOVE1( p_ND_FROM number, p_TO_PROD varchar ) is -- процедура переклассификации актива
      ...
      NBS_SS char(4)        ;
      ACC_SP number         ;
      procedure OB22(a2  accounts%rowtype ) is   sb sb_ob22%rowtype;
            GL_bdate        date; gl_aMFO varchar2(6); 
      begin GL_bdate := gl.Bdate; gl_Amfo := gl.Amfo ; 
            BC.go ('/') ;
         begin select * into sb from sb_ob22 where  r020||ob22 = a2.nbs||a2.ob22 ;
               If sb.D_OPEN > gl.bdate   then update sb_ob22 set D_OPEN  = GL_BDATE where  r020||ob22 = a2.nbs||a2.ob22 ;     end if ;
               If sb.d_close is not null then update sb_ob22 set d_close = null, 
                                                                 txt = (select name from ps where nbs = a2.NBS )  
                                                                                    where  r020||ob22 = a2.nbs||a2.ob22 ;     end if ;
         exception when no_data_found    then insert into sb_ob22 (r020,ob22,txt)   select nbs, a2.ob22, name from ps where nbs = a2.NBS;
         end;
         BC.go (gl_Amfo)  ;
      end ;
  ...
  begin 
     ...
     loop
         If a1.tip not in ('SS ','SP ','SN ','SPN','SDI','SDF','SDM','SDA',  'SRR', 'SNO' ) then       goto NEXT_;     end if; -- доп.счет
...
--2) Установить ему об22
            If a2.ob22 is not  null then   OB22(a2) ;  Accreg.setAccountSParam(a2.acc, 'OB22', a2.ob22 ) ;   end if;
            ...
            If a1.tip in ('SS ','SP ')   then  
               ...
               loop ii.acc := a2.acc; 
                     ...
                     insert into int_RATN (acc,    id,    Bdat , IR ) 
                                 select a2.acc, ii.id, gl.Bdate, acrn.Fprocn ( a1.acc, ii.ID, gl.bdate) 
                                 from int_Ratn where acc = a1.acc and id = ii.id and rownum = 1 ;
               end loop;
            end if ;
  
            If a1.tip in ('SN ', 'SPN' )  then

               Update int_accn set acra = a2.ACC   ----,        acrb = cc_o_nls_ext(NBS_SS, null, null, null, 'SD0',null, null)   
                  where acc in (ACC_SS, ACC_SP) and acra = a1.ACC;
            end if ;
...
-------------------- 
begin  If gl.aMfo <> '300465' then RETURN; end if;
  -------------------------------------------
             ...
             From RECLASS9 r,  
                 (select d.ND, d.prod, t.txt IFRS,  NVL( (select decode (txt, 'Так',1, '1',1,0)  from nd_txt  where tag = 'POCI' and nd = d.nd), 0) POCI
                  from (select * from nd_txt  where tag = 'IFRS') t,
                       (select * from cc_deal where vidd in (1) and sos >=10 and sos<=14 and p_ND in ( 0, nd)  ) d ----and sdate < p_DAT01 and ( P_DAT01 = G_Dat01 or sdate >= add_months(p_dat01,-1)  ) )  d 
                  where d.nd = t.nd    
                  ) x   
