...
% VERSION     : 13/01/2016 (17/01/2014, 17/01/2012)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
--- обороты по перекрытию 6,7 классов на 5040,5041
   IF Doszg_ > 0 THEN
      p_ins(data_, '5', nls_, nbs_, kv_, k041_, TO_CHAR(Doszg_));
   END IF;

   IF Koszg_ > 0 THEN
      p_ins(data_, '6', nls_, nbs_, kv_, k041_, TO_CHAR(Koszg_));
   END IF;
...
--- корректирующие обороты за месяц по перекрытию 6,7 классов на 5040,5041
   IF Dos96zg_ > 0 THEN
      p_ins(data_, '7', nls_, nbs_, kv_, k041_, TO_CHAR(Dos96zg_));
   END IF;

   IF Kos96zg_ > 0 THEN
      p_ins(data_, '8', nls_, nbs_, kv_, k041_, TO_CHAR(Kos96zg_));
   END IF;
