...
PROMPT *** Create  procedure DELOIT ***
  ...
  CREATE OR REPLACE PROCEDURE BARS.DELOIT 
(mode_ INT,  -- режит формирования
 s_DAT1 VARCHAR2,
 s_DAT2 VARCHAR2)
 IS
...
begin
  ...
  If s_DAT2 is not null then     l_caldt_date := trunc ( l_dat2, 'MM');
     begin  select caldt_id into l_caldt_id from ACCM_CALENDAR where caldt_date =l_caldt_date ;
     EXCEPTION WHEN NO_DATA_FOUND THEN raise_application_error(-20100,'В ACCM_CALENDAR вiдсутня дата '||to_char(l_caldt_date,'dd.mm.yyyy') ) ;
     end;
  end if;

  IF mode_ not in ( 6,1)  THEN
    begin      EXECUTE IMMEDIATE 'drop   table '|| Tabn_  ;
...
loop
 ...
 begin
   select c.custtype, nvl(substr(replace (c.nmk,'''','`') ,1,38), '???' ) ,
          nvl(substr(c.okpo,1,10), '???' ) ,   nvl(c.PRINSIDER, 99),
 ...
 begin
    ...
    where i.id = 1 and i.acc=k.acc  and m.acc= i.acra and m.caldt_id = l_caldt_id;
    ...
    If k.tip = 'DEP'   THEN
       ...
       If ( l_cust = 2 OR l_SED='91')   then
          ...
          from ( select nvl(d.nd,to_char(d.Dpu_ID)) ND,
                 ...
                 where acc = k.acc and d.DATEU <= l_dat2
       ...
       ElsIf l_cust = 3 then
          ...
          where d.acc=k.acc
            and (d.idupd) =
                (SELECT MAX(idupd) FROM dpt_deposit_clos
                 WHERE bdate <= l_dat2 and deposit_id=d.deposit_id
                 );
       end if;

    ElsIf k.tip like 'PK_'    then

       --БПК
       select to_char(d.nd), nvl(d.product_id,0)
       into l_ND, l_VIDD
       from bpk_acc d where acc_pk=k.acc;
 ...
 If  l_mdate <= l_dat2 then l_IR := 0;
 else                       l_IR := nvl( acrn.fprocn( k.acc,1,l_dat2), 0);
 end if;
 ...
 If l_MDATE is not null then
    ...
    begin
      ...
      select NVL(Trim(S240),'0'), nvl(trim(s180),'0')    into l_240, l_180
 ...
 IF nvl(l_240,'0') = '0'  then  l_240 := fs180_def(k.NBS, 2); end if;
 IF nvl(l_180,'0') = '0'  then  l_180 := fs180_def(k.NBS, 2); end if;


 l_Sql :=  'insert into ' || Tabn_  || '  (
 G01, G02, G03, G04, G05, G06, G07, G08, G09, G10, G11, G12, G13, G14, G15, G16,
 G17, G18, G19, G20, G21, G22, G23, G24, G25 ) values ('''||
...
--begin
  EXECUTE IMMEDIATE l_Sql;
--exception when others then   logger.info(l_sql);   raise;
--end;
