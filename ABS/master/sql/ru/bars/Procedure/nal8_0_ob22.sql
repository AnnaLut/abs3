...
bars_audit.info('nal_01 '||nls_8099_01);
   ...
   FOR k in (
         SELECT s.NLS nls, s.OSTF ostf,i.ob22,i.p080 ,substr(s.nms,1,38) NMS,  s.ACC  -- все ob22,p080
         ...
         union all
         SELECT s.NLS, s.OSTF,i.ob22,i.p080 ,substr(s.nms,1,38) NMS,  s.ACC   -- только отдельные P080
          FROM  sal s,specparam_int i,
         ...
         union all
         SELECT s.NLS, s.OSTF,i.ob22,i.p080 ,substr(s.nms,1,38) NMS,  s.ACC   -- только отдельные OB22
          FROM  sal s,specparam_int i,
                (select nbs_b,ob22_b,p080_b from SB_ZGOD_2009  where dk=1 and nbs_a='8099' and ob22_a='01') t
          WHERE     s.kv=980                  and   s.fdat=dat_                 and s.ostf<0
              and  s.nbs =t.nbs_b            and   s.acc=i.acc
...
bars_audit.info('nal_02 '||nls_8099_02);
   ...
   FOR k in (
         SELECT s.NLS nls, s.OSTF ostf,i.ob22,i.p080 ,substr(s.nms,1,38) NMS,  s.ACC
          FROM  sal s,specparam_int i,
