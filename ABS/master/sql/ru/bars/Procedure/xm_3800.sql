...
PROMPT ===================================================================================== 
PROMPT *** Run *** ========== Scripts /Sql/BARS/Procedure/XM_3800.sql =========*** Run *** =
PROMPT ===================================================================================== 


PROMPT *** Create  procedure XM_3800 ***
...
*/
  ern CONSTANT POSITIVE := 333;   erm VARCHAR2(280);
  MFO_ varchar2 (6)    ;
  nls3800_ varchar2(15); nms3800_ varchar2(38); acc3800_ int  ;
  nls3801_ varchar2(15); nms3801_ varchar2(38); acc3801_ int  ;
  p4_            int   ; acc6204_        int  ; acc_rrd_ int  ;  acc_RRS_       int  ;
  nls6_ varchar2(15)   ; nms6_ varchar2(38)   ; OB62_  char(2);  DK_            int  ;
  ref_           int   ;  s_          number  ; OB22_  char(2);  RNK_           int  ;
  val_  varchar2(10)   ;  brG_  varchar2(15)  ; BRANCH_ varchar2(30);  absadm_  int  :=20094;
  mfo5_   varchar2(5)  ;  BBBB_      char(4)  ; nms1_ varchar2(30)  ;
begin
 tuda ;
 MFO_ := Nvl(p_MFO, gl.aMFO);
 If GL_BDATE is not null then
    -- установить нужную дату, как банковскую
 ...
 brG_ := '/' || MFO_ || '/000000/'; mfo5_:= substr(gl.aMfo,1,5);

for o in (select ob22, substr(txt,1,38) NMS from sb_ob22  where r020='3800' and ob22 in ('09','03') )
loop
  ...
  loop
    ...
    --найти нужный РНК
    begin
       select val into val_ from BRANCH_PARAMETERS    where branch=b.branch and tag='RNK';   RNK_:= to_number(val_);
    ...
    loop
      ...
      begin
        select acc into acc3800_  from accounts where nls=nls3800_ and kv=k.kv and dazs is not null;
      ...
      if SQL%rowcount = 0 then insert into specparam_int(acc,ob22) values(acc3800_, o.ob22);
      end if;
      ...
      acc6204_ := null;
      if o.ob22 = '16' then ob62_ :='18'; else  ob62_ := '15';  end if;
      ...
      If acc6204_ is null then
         ...
         update accounts  set tobo=b.branch, daos = daos-10 where acc=
                                                                  acc6204_ ;
         update specparam_int set ob22=OB62_ where acc=           acc6204_ ;
         if SQL%rowcount = 0 then
            insert into specparam_int(ob22,acc) values (ob62_,    acc6204_);
         end if;
      end if;

      --2 ) HKP-2) найти нужный 6204 -2
      acc_rrd_ := null;
      if o.ob22 = '16' then ob62_ :='18'; else  ob62_ := '07'; end if;
      OP_BS_OB1 (b.branch,'6204' ||ob62_ ) ;
      begin
         nls6_:=Nvl( nbs_ob22_null('6204',ob62_,b.branch),  nbs_ob22_null('6204',ob62_,brG_)      );
      ...
      If acc_rrd_ is null then
         ...
         if SQL%rowcount = 0 then
            insert into specparam_int(ob22,acc) values (ob62_,    acc_rrd_);
         end if;
      end if;

      -- 3) PKP-3 найти нужный 6204 -3
      acc_rrs_ := null;
      if o.ob22 = '16' then ob62_ :='18'; else  ob62_ := '06';  end if;
      OP_BS_OB1 (b.branch,'6204'||ob62_ ) ;
      begin
      ...
      If acc_rrs_ is null then
         ...
         if SQL%rowcount = 0 then
            insert into specparam_int(ob22,acc) values (ob62_,    acc_rrs_);
         end if;
      end if;
...
PROMPT ===================================================================================== 
PROMPT *** End *** ========== Scripts /Sql/BARS/Procedure/XM_3800.sql =========*** End *** =
PROMPT ===================================================================================== 
