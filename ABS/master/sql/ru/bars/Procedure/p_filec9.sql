-- консолидация для файлу #C9
-- если значение показателя 20NNN меньше равно 1000.00 $ 
-- то выполняем консолидацию
CREATE OR REPLACE PROCEDURE BARS.p_filec9 (Dat_ DATE )  IS
   ...
   loop
      ...
      then
         ...
         INSERT INTO v_banks_report91         
               (nbuc, kodf, datf, kodp, znap) 
         VALUES
               (k.nbuc, 'C9', Dat_, '10' || k.NNN, k.KV); 

         INSERT INTO v_banks_report91         
               (nbuc, kodf, datf, kodp, znap) 
         VALUES
               (k.nbuc, 'C9', Dat_, '31' || k.NNN, decode(k.CUSTTYPE, '1', '006', '0')); 

         INSERT INTO v_banks_report91         
               (nbuc, kodf, datf, kodp, znap) 
         VALUES
               (k.nbuc, 'C9', Dat_, '20' || k.NNN, k.SUMP); 

         INSERT INTO v_banks_report91         
               (nbuc, kodf, datf, kodp, znap) 
         VALUES
               (k.nbuc, 'C9', Dat_, '35' || k.NNN, '0'); 

         INSERT INTO v_banks_report91         
               (nbuc, kodf, datf, kodp, znap) 
         VALUES
               (k.nbuc, 'C9', Dat_, '42' || k.NNN, '00'); 

         INSERT INTO v_banks_report91         
               (nbuc, kodf, datf, kodp, znap) 
         VALUES
               (k.nbuc, 'C9', Dat_, '40' || k.NNN, '00');

         INSERT INTO v_banks_report91         
               (nbuc, kodf, datf, kodp, znap) 
         VALUES
               (k.nbuc, 'C9', Dat_, '62' || k.NNN, '000');

         INSERT INTO v_banks_report91         
               (nbuc, kodf, datf, kodp, znap) 
         VALUES
               (k.nbuc, 'C9', Dat_, '99' || k.NNN, 'Консолідований'); 
...
begin
    execute immediate 'DROP PUBLIC SYNONYM p_filec9';
exception
    when others then null;
end;
/    

create public synonym p_filec9 for bars.p_filec9;
grant execute on p_filec9 to rpbn002;
/
