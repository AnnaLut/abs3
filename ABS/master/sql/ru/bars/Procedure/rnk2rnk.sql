        ...
        loop
          ...
          if dazs_ is null then
            begin
              ...
              if nbs_ = '2625' then
                  for cur in -- и неважно, открыт или закрыт deal. Все равно забираем счет и присоединяем к другому, ведь счет открыт
                  ...
                  l_dkbo_acc_list  := bars.attribute_utl.get_number_values(p_object_id      => cur.id
                                                                          ,p_attribute_code => lc_acc_list);

                  l_acc_list_union := l_dkbo_acc_list MULTISET EXCEPT DISTINCT number_list(acc_);

                  bars.attribute_utl.set_value(p_object_id      => cur.id
                                              ,p_attribute_code => lc_acc_list
