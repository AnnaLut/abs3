...
-- МФО "родителя"
   ...
   IF mfou_ = 300465 and mfou_ != mfo_g THEN
      for k in (select unique o.ref
       ...
       for k in (select p.pdat pdat, p.fdat fdat, p.ref ref, p.tt tt,
                        ...
                        p.nazn, p.ptt tt1, p.rnkd, p.rnkk, p.ob22d, p.ob22k,
                        nvl(substr(trim(w.value),1,1), '1') rezid_o
                   from provodki_otc p, operw w
                   where p.ref in (select o.ref
       ...
       loop
          ...
          if k.nlsd not like '100%' then
             ob22_ := nvl(trim(k.ob22d),'00');

             BEGIN
                select 2-mod(c.codcagent,2)
                  into rezid_
                from customer c
                where c.rnk = k.rnkd;
          ...
          if k.nlsd LIKE '2600%'
             ...
             set a.value='2312002'
             where a.tag='KOD_N'
               and (trim(a.value) is null OR trim(a.value) = '0000000')
               and a.ref=k.ref;
          end if;
...
--                       and trim (p.id_b) = trim (c.okpo);
               ...
               END IF;
            EXCEPTION
                WHEN NO_DATA_FOUND THEN DECL_:=null;
                WHEN others THEN dbms_output.put_line(k.ref);
            end;
...
grant EXECUTE                                                                on FPB1            to START1;
grant EXECUTE                                                                on FPB1            to WR_ALL_RIGHTS;
