...
PROMPT *** Create  procedure DE_SAL ***
  ...
  CREATE OR REPLACE PROCEDURE BARS.DE_SAL (acc_ NUMBER,dat_ DATE,s_ DECIMAL,sq_ DECIMAL) IS
...
BEGIN
   ...
   UPDATE saldoa
      SET dos =dos - s_, kos =kos - s_
    WHERE acc=acc_ AND fdat=dat_;

   SELECT dos, kos
     INTO dos_,kos_
     FROM saldoa WHERE acc=acc_ AND fdat=dat_;
   ...
   IF dos_=0 AND kos_=0 THEN
      ...
      UPDATE saldoa SET pdat=pdat_ WHERE acc=acc_ AND
             fdat=( SELECT MIN(fdat)
             FROM saldoa WHERE acc=acc_ AND fdat>dat_);
      DELETE FROM saldoa WHERE acc=acc_ AND fdat=dat_ AND pdat is not null;
