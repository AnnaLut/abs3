...
*/
 ...
 l_02 number; l_03 number; l_83 number; l_79 number;
...
begin
 ...
 If Mode_ = 2 then
    ...
    from accounts a
    where a.dazs is null and a.NBS ='9819' AND a.OB22 IN ('02','79','83','03') ;
...
ElsIf Mode_ = 0 then
    -- 0.Первинне (до)Наповнення даними з КП
    insert into nd_9819 ( nd, k_02, k_03, k_83, k_79)
               --Кредитнi~Угоди~02 = 1
               --      Страховi~Угоди~03 = null
               --            Iпотечнi~Угоди~83 = kolI
               --                      Iншi~Угоди~79     = kolz+kolp
    select d.nd, 1,   null, i.koli,  z.kolz+p.kolp
    from cc_deal d,
    ...
    where d.nd   = i.nd (+)
      ...
      and d.sos >= 10
      and d.sos  < 15
      and not exists (select 1 from nd_9819 where nd=d.ND);
  ...
  elsIf Mode_ = 3 then
    ...
    begin
      ...
      from accounts
      where nbs ='9819' and ob22 in ('02','03','83','79')
...
----       where nbs ='9819'  and ob22 in ('02',     '83','79')
    ...
    begin
      select r_02, r_03, r_83, r_79
      into   l_02, l_03, l_83, l_79
      from VR_9819
      where branch = l_branch
        and ( r_02<>0 or r_03<>0 or r_83<>0 or r_79<>0) ;
...
exception  when others then
  ...
  if sqlcode = -00001 then   null;  else      raise;     end if;
    ...
    for k in (select nls, ob22, substr(nls,1,38) NMS, -ostc S, isp
              from accounts
              where kv=gl.baseval
                and nbs ='9819' and ob22 in ('02','03','83','79')
---             and nbs ='9819' and ob22 in ('02',     '83','79')
    ...
    loop
       ...
       for z in (select nd, nmk, decode(k.ob22,
                 ...
                 from vv_9819 where branch like l_branch || '%' )
