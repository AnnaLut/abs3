...
PROMPT ===================================================================================== 
PROMPT *** Run *** ========== Scripts /Sql/BARS/Procedure/P_F67_NN.sql =========*** Run *** 
PROMPT ===================================================================================== 


PROMPT *** Create  procedure P_F67_NN ***

  CREATE OR REPLACE PROCEDURE BARS.P_F67_NN (Dat_ DATE)  IS
/*%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% DESCRIPTION :	Процедура формирование файла @67 для КБ
...
%            спецпараметры для отбора в 67 файл
%            убрала "старые" хинты
...
---Остатки и Обороты (по 6,7 кл.) для счетов в том числе отсут. в конце месяца
--CURSOR Saldo6_7 IS
...
SELECT count(*) into f67k_
FROM tmp_irep
WHERE kodf='67' and datf=Dat_ ;
...
LOOP
   ...
   SELECT count(*) INTO f67_ FROM sb_p0853 WHERE r020=nbs_ ;
--   SELECT count(*) INTO f67_ FROM sb_p085d3 WHERE r020=nbs_ ;
---   f67_:=1 ;

   IF f67_ >0 and Ostn8_<>0 THEN
      comm_ := '';
...
-- имеются дочерние счета активно-пассивиные, а остаток необ-мо иметь один
...
--               f67k_:=0;
--               SELECT count(*) INTO f67k_ FROM SB_P0853
...
--   SELECT count(*) INTO f67_ FROM sb_p085d3 WHERE r020=nbs_ ;
   ...
   IF f67_>0 THEN    ---and (Dosn_ > 0 OR Kosn_ > 0) THEN
      comm_ := '';
...
-------------------------------------------------------------------------
---------------------------------------------------
DELETE FROM tmp_nbu where kodf='67' and datf= dat_;
---------------------------------------------------
OPEN BaseL;
LOOP
...
PROMPT *** Create  grants  P_F67_NN ***
grant EXECUTE                                                                on P_F67_NN        to BARS_ACCESS_DEFROLE;
grant EXECUTE                                                                on P_F67_NN        to RPBN002;



PROMPT ===================================================================================== 
PROMPT *** End *** ========== Scripts /Sql/BARS/Procedure/P_F67_NN.sql =========*** End *** 
PROMPT ===================================================================================== 
