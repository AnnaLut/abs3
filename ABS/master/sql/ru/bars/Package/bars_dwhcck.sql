...
is
 ...
 where status in ('NEW_ALL','NEW_DAY')
 and not exists
 ...
 insert into dwh_tmp_dblcredits(g00, gt, gr,g59, g19,g21, acc, cnt, maxacc, maxost)
 select g00, gt, gr, g59 , g19, g21, acc, cnt, maxacc, maxost
 from (
 select g00, gt, gr, g59, g19, g21, acc, count(*) over ( partition by gt, g59) cnt,
 max(g21) over ( partition by gt, g59) maxost,
 max(acc) over ( partition by gt, g59) maxacc
 from inv_cck_fl
 where g19 in ('2202','2203','2233') and gr = 'C'
 and g00 = p_date
 )
 where cnt > 1;

 update dwh_tmp_dblcredits d set nbs = ( select g19 from dwh_tmp_dblcredits where g00 = d.g00 and g59 = d.g59 and gt = d.gt
 and ( (maxost<>0 and g21 = maxost) or -- выбираем по макс. остатку
 (maxost = 0 and acc = maxacc) -- выбираем по макс. позднему открытому счету
 )
 and rownum = 1
 ) ;
