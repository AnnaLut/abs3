...
g_header_version  constant varchar2(64)  := 'version 4.285 14/08/2016';
...
-- constants
--
g_body_version    constant varchar2(64)  := 'version 6.016 15/09/2017';
...
-- глобальні параметри функціональності
g_enable_mkk number;
--
--  types
--
...
type t_insurance is record(nd         w4_acc.nd%type,
                           ...
                           ext_code   ins_mapping_purpose.ext_code%type,
                           haveins    number,
                           haveoldins number,
                           ins_ukr_id number,
                           ins_wrd_id number,
...
begin
  ...
  begin
     select nvl(trim(val), 0) into g_enable_mkk from ow_params where par = 'ENABLEMKK';
  exception when no_data_found then
     g_enable_mkk := 0;
  end;
...
begin
  ...
  select min(p.nls), min(p.mfo), min(im.ext_code), min(c.okpo), min(p.name)
    into l_insurance.ins_nls, l_insurance.ins_mfo, l_insurance.ext_code, l_insurance.ins_okpo, l_insurance.ins_name
    from ins_partners p
    join customer c
      on p.rnk = c.rnk
    join ins_mapping_purpose im
      on im.okpo_ic = c.okpo and im.ext_id = l_insurance.ext_id;
...
begin
  ...
  begin
     ...
     if p_mode = 0 then
        select a.acc, a.nls,  a.kv, q.dk, o.tt, decode(o.kv, o.kv2, o.s, o.s2), o.sos, o.currvisagrp, o.nextvisagrp, nvl2(trim(wt.tt),1,0), nvl2(ow.ref,1,0), decode(o.mfoa, o.mfob, 1, 0), o.s2
        ...
        select a.acc, a.nls,  a.kv, 0, o.tt, decode(o.kv, o.kv2, o.s, o.s2), o.sos, o.currvisagrp, o.nextvisagrp, nvl2(trim(wt.tt),1,0), decode(o.mfoa, o.mfob, 1, 0), o.s2
        ...
        select a.acc, a.nls,  a.kv, t.dk, o.tt, decode(o.kv, o.kv2, o.s, o.s2), o.sos, o.currvisagrp, o.nextvisagrp, nvl2(trim(wt.tt),1,0), decode(o.mfoa, o.mfob, 1, 0), o.s2
          into l_acc, l_nlsb, l_kv, l_dk, l_tt, l_s, l_sos, l_currvisagrp, l_nextvisagrp, l_tt_w4_flag, l_isourmfo,l_s2
...
is
  ...
  err    exception;
  pragma exception_init(err, -6502);
...
begin
  ...
  execute immediate 'truncate table ow_cng_data_txt';

  bars_audit.info(h || 'Truncated.');
...
exception when err
then   bars_audit.info(h || 'invalid number on row - ' || l_data);
raise;
...
begin
  ...
  begin
     select file_name, file_n into l_iic_filename, l_iic_n
       from ow_iicfiles
      where upper(file_name) = upper(substr(p_filename,3));
  exception when no_data_found then
     l_filestatus := 3;
...
function send_request_insurance(p_body in clob) return wsm_mgr.t_response is
  l_url         params$global.val%type := getglobaloption('OWEWAURL');
  l_wallet_path varchar2(256) := getglobaloption('OWEWAWALLETPATH');
  l_wallet_pwd  varchar2(256) := getglobaloption('OWEWAWALETPWD');
...
begin
  ...
  if l_filetype is null then
     ...
     select count(*) into i from ow_files where file_name = l_filename;
...
begin
  ...
  begin
     select a.acc, a.nls, a.tip, nvl(g_cm_branch,a.branch), a.dazs,
            ...
            trim(decode(g_enable_mkk, 1, regexp_replace(upper(c.nmk),'(ФОП)|(П-ЦЬ)|(ПП)|(ФІЗИЧНА ОСОБА ?-ПІДПРИЄМЕЦЬ)|(ФЕРМЕРСЬКЕ ГОСПОДАРСТВО)', ''), ''))
  ...
  if p_opertype = 12 and g_enable_mkk = 1 then
  ...
  -- person
  if l_cmclient.clienttype = 2 or (p_opertype = 12 and g_enable_mkk = 1)  then
     begin
  ...
  -- corps
     ...
     if p_opertype <> 12 or g_enable_mkk = 0 then
        l_cmclient.birthdate       := null;
        l_cmclient.birthplace      := null;
        l_cmclient.gender          := null;
        l_cmclient.phonenumber_dod := null;
  ...
  if l_grpcode = 'SALARY' then
     begin
        ...
        exception when no_data_found then
           ...
           if p_opertype not in ( 3, 7) -- додано тип 7 у виключення -- http://jira.unity-bars.com.ua:11000/browse/COBUSUPABS-6284
       then 
  ...
  if l_cmclient.clienttype = 1 then
     ...
     if l_sed = '91' and p_opertype = 12 and g_enable_mkk = 1 then
  ...
  l_cmclient.oper_status := case when p_wait_confirm = 1 and g_enable_mkk = 1 then 99 else 1 end;
                                              ...
                                              xmlelement("ObjectFor",
                                                         xmlelement("ClientIDT",
                                                                    xmlelement("ClientInfo",
                                                                               xmlforest(g_w4_branch||lpad(to_char(p_cmclient.rnk), 8, '0') as "ClientNumber",
...
function add_query_to_log(p_rnk in number,
                          p_nd  in number) return number is
  l_reqid number;
begin
...
begin
  ...
  if l_tmp > 0 then
    ...
    update ow_query_log t
       set t.respcode = l_respcode,
           t.resptext = l_resptext,
           t.reqbody = l_body,
           t.respbody = l_response.cdoc
     where t.reqid = l_reqid;
  else
...
exception
  when others then
    update ow_query_log t
       set t.err_text = substr(dbms_utility.format_error_stack() || chr(10) ||
...
/*     if not regexp_like(p_cobrandid, '^[A-Z]{4}[A-Z0-9]{7}+$') then
  ...
  add_deal_to_cmque(
     ...
     p_wait_confirm => case when l_ctype = 2 and g_enable_mkk = 1 then 1 else null end,
     p_cmclient   => l_cmclient);
  if l_ctype = 2 and g_enable_mkk = 1 then
    update w4_acc_instant t
  ...
  if l_opertype = 6 and l_iscrm = '0' then
     ...
     l_isok := pers_instant_card(l_nd, l_cmclient);
     if l_isok then
...
begin
  ...
  if nvl(p_project.type_doc,0) = 1 and
     (not check_digit(p_project.paspnum) or length(p_project.paspnum) <> 6) then
...
begin
  ...
  for z in ( select c.okpo, c.okpo_n, trim(c.org_name) org_name, trim(replace(c.product_code, chr(9), '')) product_code, id id_cm
              ...
              where not exists (select 1 from bpk_proect where okpo = c.okpo and nvl(okpo_n,0) = nvl(c.okpo_n,0) )
                ...
                and exists ( select 1 from w4_product where code = trim(replace(c.product_code, chr(9), '')) ) )
  loop
     select min(id + 1) into l_id
       from bpk_proect b
      where not exists ( select id from bpk_proect where id = b.id + 1 );
...
begin
  select o.ref bulk collect
    into l_reflist
    from oper o
    join arc_rrp a
      on o.ref = a.ref and o.tt in ('OW6', 'WO6') and a.blk = -1 and
         o.pdat >= trunc(sysdate)
    join ow_locpay_match om
      on a.ref = om.ref and om.state = 0 and om.revflag = 0;

  forall idx in indices of l_reflist
    update ow_locpay_match t
       set t.revflag = 2,
           t.state   = 0,
           t.rev_message = 'Сторно платежа'
     where t.ref = l_reflist(idx);
