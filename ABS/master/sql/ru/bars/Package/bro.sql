...
procedure BONUS (p_nd   IN  number  ,
                 ...
                 p_nmsb IN  varchar2,
                 p_REF  OUT number  ) ;
-- капитализация начисленных бонусных процентов
------------------------------------------------------
procedure CLOS_ALL (p_dat IN  date);
--оптовое закрытие договоров, срок которым наступил.
  ...
  ElsIf p_mode=10 then
     ...
     If p_ir > l_IR  then                      -- Установка ставки + Окончательный ввод в действие
        ...
        l_int := calp ( s_     => l_lim,         -- сумма капитала
                        int_   => (p_ir - l_ir), -- бонусная проц.ставка
                        dat1_  =>dd.sdate+1    , -- дата "С"  исключительно
                        dat2_  =>dd.wdate      , -- дата "ПО" включительно
                        basey_ =>ii.basey        -- код базы начисления
                       );
        delete from nd_txt where nd = p_ND and tag in ('KNL');
        insert into nd_txt ( nd, tag, txt ) values ( p_nd, 'KNL', to_char(l_int) ); -- столько всего причитaется бонуса
...
procedure BONUS (p_nd   IN  number  ,
  ...
  sRKO_D varchar2(100)   ;
  sRKO_N varchar2(100)   ;
  sErr_  varchar2 (100)  := 'BRO-05.BONUS, реф.дог='|| p_nd ||'   '||to_char(gl.bdate,'dd.mm.yyyy')||'  ';
  sErr1_ varchar2 (100)  ;

begin

  begin
     sErr1_:='не знайдено дог (vidd=26 and wdate< gl.bd and sos=10)';  SELECT d.* into dd from cc_deal d  where d.nd = p_nd and d.vidd=26 and d.wdate < gl.bdate and d.sos= 10;
...
procedure CLOS_ALL (p_dat IN  date) is
 --оптовое закрытие договоров, срок которым наступил. Использовать на финише дня.
 oo oper%rowtype;
begin
