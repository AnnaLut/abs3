...
--***************************************************************--
  ...
  взят из операции "RKO" (  конкретный  или  из INI-файла, если в операции
...
-- ---------------------------------------------------
  ...
  n_tarpak    INTEGER; -- № тарифного пакета
...
BEGIN
   ...
   If mode_<0 then
      ...
      --  За НАЧАЛЬНАУЮ дату расчетного периода принимаем дату
      --  "ПО"+1 (DAT0B+1) для первого встретившегося 2560/03
      --  с НЕпустым DAT0B :
...
IF  n_tarpak >= 38 THEN        ----     П А К Е Т Ы  >=  38    ----
      ...
      IF MODE_<0 THEN           ---<--  Для 412 отчета
         ...
         INSERT INTO CCK_AN_TMP ( reg, NLS, acc, N1, PR, N2 )
          ----------------------------------------------------------
          -- 1 --   По ДЕБЕТУ / только IB1,IB2        -- 412 --
          ----------------------------------------------------------
      ...
      ELSE       ------------  Р е а л ь н ы й   р а с с ч е т :
        ...
        (
          ---------------------------------------------------------
          -- 1 --     По ДЕБЕТУ / только IB1,IB2    -- Реал.расч.--
          ---------------------------------------------------------
               ...
               (SELECT nls, S, SDOK, ref  FROM
                    (SELECT a.NLS nls,
                     ...
                     WHERE a.acc=acc_
                       and a.acc=d.acc
                       and d.ref=o.ref
                       and t.TT like 'IB%'   ---<  только IB1,IB2 !
                       and d.sos=5
                       and d.fdat >=fdat_
                       and d.fdat < dat_+1
                       and d.tt = t.tt
          ...
          Group by NLS
                                 UNION ALL
          --------------------------------------------------------
          -- 2 --     По ДЕБЕТУ / кроме IB1,IB2    -- Реал.расч.--
          --------------------------------------------------------
          SELECT NLS , sum(S) SUMS, COUNT(*) CNT, sum(sdok) SUMDOK
          FROM
             (Select NLS, S, SDOK, REF, rownum R  FROM
               (SELECT nls, S, SDOK, ref  FROM
                    (SELECT a.NLS nls,
                     ...
                     WHERE a.acc=acc_
                       and a.acc=d.acc
                       and d.ref=o.ref
                       and t.TT not like 'IB%'   ---<  кроме IB1,IB2 !
                       and d.sos=5
                       and d.fdat >=fdat_
                       and d.fdat < dat_+1
                       and d.tt = t.tt
...
PROCEDURE pay(mode_ VARCHAR2, dat_ DATE,filt_ VARCHAR2 DEFAULT NULL) IS
-- ---------------------------------------------------
...
-- =============    Начало цикла по счетам RKO_LST:   ================
  ...
  'SELECT r.acc FROM rko_lst r,accounts a
   ...
   FETCH c0 INTO acc_; EXIT WHEN c0%NOTFOUND;
      ...
      -- Определяем NLS,RNK,NMS основного счета  acc_ (для записи в oper.NAZN):
      BEGIN                    ----------------------
      ...
      --  Определяем все по счету-плательщику  accd_ :
                           -------------------------
      SELECT a.nls,a.kv, a.ostc+nvl(a.lim,0), a.isp,a.grp,cu.rnk,
      ...
      WHERE a.acc=accd_ AND a.acc=cu.acc AND cu.rnk=c.rnk;
...
---------------------------------------------------------------------
      IF INSTR(mode_,'0') > 0 AND s0_>0 AND ostc_>0 AND ostc_>=s0_ THEN  -- Попробуем начисленную плату взыскать
         ...
         BEGIN
            ...
            VALUES ( ref_,tt_,6,ref_,1,SYSDATE,  gl.bDATE,gl.bDATE, gl.bDATE,
            ...
            IF acc1_ is NULL  or  acc1_zakr=0  THEN
               ...
               BEGIN  ------  Подбор номера 3570*
                ...
                where  rownum<2;
                   ...
                   --   Cчет 3570<хвост 2600> уже есть, но он:
                   --    -  НА ДРУГОМ RNK   и л и   с OB22<>'03'    и л и   он ЗАКРЫТ !
                   --    -  этот 3570 привязан к счету из списка "Только нач. на 3570"
               ...
               OP_REG(99,0,0, grp_,tmp_, rnk_, nlsc_,kva_,
                       substr('Нарах.РКО '||nam_a_,1,70),'ODB',isp_,acc1_);
     ...
     ---------------------------------------------
       ...
       ----------------------------------------
         ...
         IF s1_ > 0 AND ostc_ > 0  AND  blkd_ = 0  THEN
            ...
            ---  Если Даты "з" и "по" старые (больше 2-х мес.назад),
            ---  то их НЕ печатаем.

                 if (dat1a_ < gl.bDATE - 62 or dat1a_ is null) and
                    (dat1b_ < gl.bDATE - 62 or dat1b_ is null) then
...
----------------------------------------------------------------------------
      ...
      IF INSTR(mode_,'3') > 0 AND s1_ > 0 THEN -- переносим на просрочку
         ...
         BEGIN
            ...
            IF acc2_ IS NULL  or  acc2_zakr=0  THEN
               ...
               substr('Простр.РКО до 31д. '||nam_a_,1,70),'ODB',isp_,acc2_);
