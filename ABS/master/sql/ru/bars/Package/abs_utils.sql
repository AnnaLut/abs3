 ...
 PROMPT ===================================================================================== 
  ...
  CREATE OR REPLACE PACKAGE BARS.ABS_UTILS is
   ...
   G_HEADER_VERSION       constant varchar2(64)  := 'version 3.0 03.07.2013';
...
CREATE OR REPLACE PACKAGE BODY BARS.ABS_UTILS as
  ...
  G_BODY_VERSION    constant varchar2(64)  := 'version 4.2 24.07.2013';
  ...
  begin
     ...
     exception when OTHERS then
        if sqlcode = -1921 then null; --role name '...' conflicts with another user or role name
        else raise;
        end if;
     end;
     ...
     begin
        ...
        execute immediate 'insert into roles$base(role_name) '||
                          'values (:1)' using p_rolename;
     exception when OTHERS then
        case (sqlcode)
             when -1   then null; --  such role exists
             when -942 then null; --  no roles$base  table
             else raise;
        end case;
     end;
  ...
  is
  begin
     execute immediate ' insert into policy_table(table_name, policy_group, owner) '||
                       ' values(:1, :2, :3) '
                       using upper(p_tabname), p_group, p_owner;
  exception when OTHERS then
     case (sqlcode)
          when -1   then null; --  such policy exists
          when -942 then null; --  no policy_table
          else raise;
     end case;
  end;
  ...
  exception  when dup_val_on_index then    null;
   ...
   begin
      ...
      begin
         insert into list_funcset (rec_id, set_id, func_id, func_activity, func_position
               , kf
         )
         values(l_newid, p_type, p_codeoper, p_checked, l_pos
               , f_ourmfo_g
         );
      exception when dup_val_on_index then
         null;
