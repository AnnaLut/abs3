...
G_HEADER_VERSION  CONSTANT VARCHAR2(64)  := 'version 3.65 06/12/2012';
...
procedure make_data4visa(p_refs in tt_num_array, p_grp in integer,
  p_key in varchar2, p_visa_array out tt_visa_array);
/**
...
G_BODY_VERSION  CONSTANT VARCHAR2(100)  := '$Ver: 3.50 2016-05-16 ';
...
BEGIN
   ...
   logger.trace('chk.get_visa_params(): currvisagrp='||curr_||',nextvisagrp='||onext_||',nnext='||nnext_);
  ...
  for i in p_refs.first..p_refs.last loop
    ...
    if p_grp=self_grp_ then
      ...
      if l_uid=uid_ then
         ...
         begin
           select * into l_staff from staff s where s.id = uid_;
