 ...
 PROMPT ===================================================================================== 
  ...
  -- ===============================================================================================
  g_header_version constant varchar2(64) := 'version 3.0 27/10/2016';
  ...
  */
  ...
  procedure set_null_2_hided_cd_quests(p_bid_id wcs_bids.id%type, -- Идентификатор заявки
                                       p_subproduct_id wcs_subproducts.id%type, -- Идентификатор субпродукта
                                       p_group_id wcs_creditdata_groups.id%type -- Идентификатор групы
                                       );
  ...
  */
  ...
  function get_sources_of_payment(p_bid_id wcs_bids.id%type,
                                  p_type varchar2, -- Тип расчета скор балла (STANDART - стандартный, OBU - постановка ОБУ)
                                  p_ws_id wcs_workspaces.id%type,
                                  p_ws_number number) return number;
...
CREATE OR REPLACE PACKAGE BODY BARS.WCS_UTL is
  ...
  -- ===============================================================================================
  g_body_version constant varchar2(64) := 'version 4.0 16/11/2016';
            ...
            when 'PRD_NAME' then
              select p.name
               ...
               where s.product_id = p.id
                 and s.id = l_b_row.subproduct_id;
            when 'BID_INTEREST' then
              if nvl(wcs_utl.get_creditdata(p_bid_id,'BASE_RATES'),0) = 0 then
                l_replace_txt := wcs_utl.get_creditdata(p_bid_id, 'INTEREST_RATE');
              elsif to_date(wcs_utl.get_creditdata(p_bid_id,'RATE_WORKER_DATE1'),'dd.mm.yyyy') <= trunc(sysdate) and nvl(wcs_utl.get_creditdata(p_bid_id,'BASE_RATES'),0) = 1 then
                l_replace_txt := wcs_utl.get_creditdata(p_bid_id, 'RATE_WORKER_BANK1');
              else
                l_replace_txt := wcs_utl.get_creditdata(p_bid_id, 'RATE_WORKER_BANK');
              end if;
            when 'CUR_WS_ID' then
    ...
    exception
      when zero_divide then
        ...
        bars_audit.trace(g_pack_name || l_proc_name ||
                         'Process. zero_divide');
        when others then
        -- RAISE_APPLICATION_ERROR('-20005',l_calc_text_);
        null;
    end;
  ...
  begin
    ...
    if (l_plsql_bool = '1' or l_plsql_bool = 'TRUE') then
      return 1;
    elsif (l_plsql_bool is null or l_plsql_bool = '0' or l_plsql_bool = 'FALSE') then
      return 0;
    end if;
...
RAISE_APPLICATION_ERROR('-20005','Не можливо виконати неіменований блок для  питання'|| p_question_id);
  end;
  end calc_sql_bool;
    ...
    begin
     ...
     begin
        ...
        select mt.tabname
          into l_tabname
          from meta_tables mt
         where mt.tabname = l_qrp_row.tab_name;--mt.tabid = l_qrp_row.tab_name;
  ...
  begin
    case p_type
      ...
      when 'DECIMAL' then
        l_res := trim(to_char(to_number(p_val), '9999999999999999990D999'));
      when 'DATE' then
      ...
      when 'REFER_QUEST' then
        ...
        begin
         select *
           ...
           where qrp.question_id = p_question_id;
        exception when no_daTA_found then
             raise_application_error(-20010,'Не вдалось визначити параметри довідника для питання '||p_question_id);
          END;
      ...
      when 'REFER_MAC' then
        ...
        begin
          ...
          select mt.tabname
            into l_tabname
            from meta_tables mt
           where mt.tabname = l_mrp_row.tab_name;
  ...
  procedure set_answ(p_bid_id      wcs_answers.bid_id%type, -- Идентификатор заявки
                     ...
                     p_val         varchar2, -- Значение
                     p_ws_id       wcs_answers.ws_id%type default g_cur_ws_id, -- Идентификатор рабочего пространства
                     p_ws_number   wcs_answers.ws_number%type
                      default g_cur_ws_num -- Номер рабочего пространства
                     ) is
    l_type_id varchar2(100);
  begin
    -- берем тип параметра
   ...
   exception
    when others then
     raise_application_error (-20000,'Некоректний тип даних для питання '|| p_question_id);
    end;
      when ('NUMB') then
      begin
 ...
 when ('DATE') then
  ...
  function calc_kzb(p_bid_id wcs_answers.bid_id%type) return number
    is
      l_mzS number(13,3);
      l_mzSQ number(13,3);
      l_vzSAuto number(13,3);
      l_vzSAutoQ number(13,3);
      l_vzSGuar number(13,3);
      l_vzSGuarQ number(13,3);
      l_vzSInsu number(13,3);
      l_vzSInsuQ number(13,3);
      l_vzS number(13,3);
      l_vzSQ number(13,3);
      l_res number(13,3);
  begin
    /*Місцезнаходження об`єкта застави/іпотека*/
    select nvl(round(sum(wcs_utl.get_answ(bg.bid_id, 'MZ', bg.ws_id, bg.garantee_num) * grt_cost),3),0),
           nvl(round(sum(grt_cost),3),0)
      into l_mzSQ, l_mzS
    ...
    select nvl(round(sum((case
                  when wcs_utl.get_answ_list(bid_id,'GRT_3_4_0',ws_id,garantee_num) = 1 then 8
                  when wcs_utl.get_answ_list(bid_id,'GRT_3_4_0',ws_id,garantee_num) = 0 and wcs_utl.get_answ_list(bid_id,'GRT_3_1',ws_id,garantee_num) = 1 then 16
                  when wcs_utl.get_answ_list(bid_id,'GRT_3_4_0',ws_id,garantee_num) = 0 and wcs_utl.get_answ_list(bid_id,'GRT_3_1',ws_id,garantee_num) = 0 then 18
                  else 0
                end)*grt_cost),3),0),
           nvl(round(sum(grt_cost),3),0)
      into l_vzSAutoQ,l_vzSAuto
      from v_wcs_bid_garantees
     where garantee_id = 'VEHICLE'
       and bid_id = p_bid_id;

    /*Фінансова порука*/
    select nvl(round(sum(12*bg.grt_cost),3),0),
           nvl(round(sum(bg.grt_cost),3),0)
      into l_vzSGuarQ,l_vzSGuar
      from v_wcs_bid_garantees bg,
     ...
     where bg.bid_id = b.id
       ...
       and bg.bid_id = p_bid_id
       and wcs_utl.get_answ_list(bg.bid_id,'GRT_2_19',bg.ws_id,bg.garantee_num) = 0
       and wcs_utl.get_score(bg.bid_id,ss.scoring_id,'KPP',bg.ws_id,bg.garantee_num) > 1.3
       and wcs_utl.get_score(bg.bid_id,ss.scoring_id,'P4DP',bg.ws_id,bg.garantee_num) > 1;
    ...
    declare
      ...
      l_sumInsu decimal;
    begin
      l_sumInsuQ := nvl(round(wcs_utl.get_answ_decimal(p_bid_id, 'INSURANCE_SUM') * 19, 3), 0);
      l_sumInsu := nvl(round(wcs_utl.get_answ_decimal(p_bid_id, 'INSURANCE_SUM'),3), 0);
      select nvl(round(sum(sum * 19), 3), 0), nvl(round(sum(sum), 3), 0)
        into l_vzSInsuQ, l_vzSInsu
        from (select bid_id, insurance_id, sum
                from v_wcs_bid_grt_insurances
              union all
              select bid_id, insurance_id, sum
                from v_wcs_bid_insurances)
       where bid_id = p_bid_id
         and insurance_id = 4;

      l_vzSQ := round(l_vzSAutoQ + l_vzSGuarQ + l_vzSInsuQ + l_sumInsuQ, 3);
      l_vzS  := round(l_vzSAuto + l_vzSGuar + l_vzSInsu + l_sumInsu, 3);

      l_res := case nvl(l_mzS, 0) + nvl(l_vzS, 0)
                 when 0 then 0
                 else round((nvl(l_mzSQ, 0) + nvl(l_vzSQ, 0)) / (nvl(l_mzS, 0) + nvl(l_vzS, 0)), 3)
               end;
    end;
    return l_res;
  end calc_kzb;
  ...
  function get_general_score(p_bid_id     wcs_answers.bid_id%type, -- Идентификатор заявки
   ...
   is
    l_score            number(13, 3) := 0;
    l_balls_ojective   number(13, 3);
    l_balls_subjective number(13, 3);
    l_balls_total      number(13, 3);

    l_4ps    number(13, 3); --Частка погашення кредиту в чистому сукупному доході позичальника (Чпс)
    l_p4dp   number(13, 3); --Показник сукуного чистого доходу Позичальника(Пчдп)
    l_p4ds   number(13, 3); --Показник сукуного чистого доходу сім`ї Позичальника(Пчдс)
    l_rb     number(13, 3); --Накопичення на рахунках у банку
    l_zs     number(13, 3); --Загальний стаж роботи(Зс)
    l_mp     number(13, 3); --Місце проживання(Мп)
    l_pj     number(13, 3); --Професійні якості(Пя)
    l_vmaina number(13, 3); --Вид майна (Вм)
    l_dp     number(13, 3); --Наявність джерел погашення кредиту (Дп)
    l_kpp    number(13, 3); --Коефіцієнт платоспроможності позичальника (Кпп)
    l_kps    number(13, 3); --Коефіцієнт платоспроможності сім’ї позичальника (Кпс)
    l_4p     number(13, 3); --Частка погашення кредиту в чистому сукупному доході позичальника (Чп)
    l_pr     number(13, 3); --Наявність постійної роботи (Пр)
    l_sm     number(13, 3); --Сімейний стан та місце проживання (См)
    l_vpk    number(13, 3); --Вік позичальника на момент погашення кредиту (Впк)
    l_r      number(13, 3); --Наявність та вид рахунку в Банку (Р)
    l_kzb    number(13, 3) := 0; --
    l_cnt    number := 0;
  begin
    select count(1) into l_cnt from v_wcs_bid_garantees where bid_id = p_bid_id and wcs_utl.get_answ_list(bid_id,'GRT_2_20',ws_id,garantee_num) = 1;
    ...
    l_4ps := wcs_utl.get_score(p_bid_id,
                               p_scoring_id,
                               '4PS',
                               p_ws_id,
                               p_ws_number);
    if p_ws_id = 'MAIN' then
      l_dp := get_sources_of_payment(p_bid_id, p_type, p_ws_id, p_ws_number);
    else
      l_dp := nvl(wcs_utl.get_answ_decimal(p_bid_id, 'DP_ALL'),
                  wcs_utl.get_score(p_bid_id,
                                    p_scoring_id,
                                    'DP',
                                    p_ws_id,
    ...
    l_p4dp := wcs_utl.get_score(p_bid_id,
                                p_scoring_id,
                                'P4DP',
                                p_ws_id,
                                p_ws_number);

    l_p4ds   := wcs_utl.get_score(p_bid_id,
                                  p_scoring_id,
                                  'P4DS',
                                  p_ws_id,
                                  p_ws_number);*/
    l_rb     := wcs_utl.get_score(p_bid_id,
                                  p_scoring_id,
                                  'RB',
                                  p_ws_id,
                                  p_ws_number);
    l_kzb    := wcs_utl.get_score(p_bid_id,
                                  p_scoring_id,
                                  'KZB',
                                  p_ws_id,
                                  p_ws_number);
    l_vmaina := wcs_utl.get_score(p_bid_id,
                                  p_scoring_id,
                                  'VMAINA',
                                  p_ws_id,
                                  p_ws_number);
    l_pr     := wcs_utl.get_score(p_bid_id,
                                  ...
                                  p_ws_id,
                                  p_ws_number);
    l_zs     := wcs_utl.get_score(p_bid_id,
                                  p_scoring_id,
                                  'ZS',
                                  p_ws_id,
                                  p_ws_number);
    l_sm     := wcs_utl.get_score(p_bid_id,
                                  ...
                                  p_ws_id,
                                  p_ws_number);
    l_mp     := wcs_utl.get_score(p_bid_id,
                                  p_scoring_id,
                                  'MP',
                                  p_ws_id,
                                  p_ws_number);
    ...
    l_r   := wcs_utl.get_score(p_bid_id,
                               ...
                               p_ws_id,
                               p_ws_number);
    l_pj  := wcs_utl.get_score(p_bid_id,
                               p_scoring_id,
                               'PJ',
                               p_ws_id,
                               p_ws_number);
    case
      when p_type = 'OBU' then
        ...
        --if wcs_utl.get_answ_bool(p_bid_id,'CL_34_1',p_ws_id,p_ws_number) = 1 then
        if l_cnt > 0 then
          l_balls_ojective := round(l_kpp + l_kps + l_4ps + l_dp + l_p4ds + l_rb,3);
        else
          l_balls_ojective := round(l_kpp + l_4p + l_dp + l_p4dp + l_rb,3);
        end if;

        l_balls_subjective := round(l_vmaina + l_pr + l_zs + l_sm + l_mp +
                                    l_vpk + l_r + l_pj + l_kzb,
                                    3);

        l_balls_total := l_balls_ojective + l_balls_subjective;

        l_score := l_balls_total;
        --set_answ(p_bid_id,'C1',l_score,p_ws_id,p_ws_number);
        l_score := l_score /
                   wcs_utl.get_answ_decimal(p_bid_id,
                                            'PVF',
                                            p_ws_id,
                                            p_ws_number) * 100;
      else
        ...
        if p_ws_id = 'MAIN' then
          ...
          l_dp := nvl(wcs_utl.get_answ_decimal(p_bid_id, 'DP_ALL_C'),
                      wcs_utl.get_score(p_bid_id,
                                        p_scoring_id,
                                        'DP_C',
                                        p_ws_id,
        ...
        if (l_balls_subjective / l_balls_total) > (3 / 10) then
          l_balls_total := (l_balls_ojective * 100 / 70);
          l_score := l_balls_total;
        else
          l_score := l_balls_total;
        end if;
        --set_answ(p_bid_id,'C2',l_score,p_ws_id,p_ws_number);
        l_score := l_score /
                   wcs_utl.get_answ_decimal(p_bid_id,
                                            'PVB',
                                            p_ws_id,
  ...
  */
  ...
  begin
    ...
    case (l_q_row.type_id)
      ...
      when 'LIST' then
        if l_qp_row.list_sid_default is not null then
        ...
        p_text_width       := l_qp_row.text_width;
      when 'REFER' then
        if l_qp_row.refer_sid_default is not null then
      ...
      when 'BOOL' then
        if l_qp_row.bool_val_default is not null then
  ...
  begin
    ...
    for cur in (select *
      ...
      if (regexp_like(cur.dnshow_if, l_search_pattern) or (cur.is_checkable = 1 and regexp_like(cur.check_proc, l_search_pattern))) then
        return 1;
      end if;
      -- ищем зависимоcть в базовых параметрах вопроса
      select *
  ...
  --Обнуляет непоказываемые вопросы групы даных кредита
  procedure set_null_2_hided_cd_quests(p_bid_id wcs_bids.id%type, -- Идентификатор заявки
                                       p_subproduct_id wcs_subproducts.id%type, -- Идентификатор субпродукта
                                       p_group_id wcs_creditdata_groups.id%type -- Идентификатор групы
                                       ) is
  begin
    for cur in (select sc.question_id
                  from wcs_creditdata_groups cg, wcs_subproduct_creditdata sc
                 where cg.id = sc.group_id
                   and cg.id = p_group_id
                   and sc.subproduct_id = p_subproduct_id
                   and ((cg.dnshow_if is not null and
                       1 = wcs_utl.calc_sql_bool(p_bid_id ,
                                                   cg.dnshow_if)) or
                       (sc.dnshow_if is not null and
                       1 = wcs_utl.calc_sql_bool(p_bid_id ,
                                                   sc.dnshow_if))))
      loop
        wcs_pack.answ_del(p_bid_id, cur.question_id);
      end loop;
  end set_null_2_hided_cd_quests;
  ...
  is
    ...
    v_n_months number;
    l_prev_rate decimal;
    l_is_first_day decimal := 0;
    l_next_rate number;
    l_next_r_date date;
  begin
    -- начало действия ставки должно быть 01 число
/*    for x in ( select * from table (p_credit_rate_tab) where to_char(rate_date_start,'dd') <> '01')
    loop
      return; -- выход
    end loop;*/
    ...
    for x in (select a.*,
                    (select rate from table(p_credit_rate_tab) where TRUNC(add_months(a.platej_data,-1), 'MM') between rate_date_start and nvl(rate_date_end, v_max_date) ) month_rate,
                    (select rate from table(p_credit_rate_tab) where TRUNC(a.platej_data, 'MM') between rate_date_start and nvl(rate_date_end, v_max_date) ) next_month_rate,
                    (select rate_date_start from table(p_credit_rate_tab) where TRUNC(add_months(a.platej_data,-1), 'MM') between rate_date_start and nvl(rate_date_end, v_max_date) ) start_month_rate,
                    (select rate_date_start from table(p_credit_rate_tab) where TRUNC(a.platej_data, 'MM') between rate_date_start and nvl(rate_date_end, v_max_date) ) next_start_month
              from (select level
      ...
      loop
        logger.info('WCS GPK: level='||to_char(x.level-1)||' rate='||x.month_rate||' pay_date='||x.platej_data||' rate_date='||x.start_month_rate);
        if x.level = 1 then
        ...
        if x.level <> 1 and x.level <> v_n_months then
          ...
          if x.month_rate = x.next_month_rate then
            ...
            v_VP1 := round(((v_Z1* (x.month_rate/100)) / v_qty_days_year) * v_N1, 2);
            v_VP2 := round(((v_Z2* (x.month_rate/100)) / v_qty_days_year) * v_N2, 2);
          else
            if x.next_start_month < add_months(x.platej_data, -1) then
              v_N1 := ((x.next_start_month - 1) - trunc(add_months(x.platej_data, -1),'mm')) + 1;
              v_N2 := ((add_months(x.platej_data, -1) - 1) - x.next_start_month) + 1;
              v_N3 := (last_day(add_months(x.platej_data, -1)) - add_months(x.platej_data, -1)) + 1;

              v_VP1 := round(((v_Z1* (x.month_rate/100)) / v_qty_days_year) * v_N1, 2);
              if v_N2 > 0 then
                v_VP2 := round(((v_Z1 * (x.next_month_rate/100)) / v_qty_days_year) * v_N2, 2);
              else
                v_VP2 := 0;
              end if;
              v_VP3 := round(((v_Z2* (x.next_month_rate/100)) / v_qty_days_year) * v_N3, 2);

              v_VP1 := v_VP1 + v_VP2;
              v_VP2 := v_VP3;
            elsif x.next_start_month > add_months(x.platej_data, -1) then
              v_N1 := ((add_months(x.platej_data, -1) - 1) - trunc(add_months(x.platej_data, -1),'mm')) + 1;
              v_N2 := ((x.next_start_month - 1) - add_months(x.platej_data, -1)) + 1;
              v_N3 := (last_day(add_months(x.platej_data, -1)) - x.next_start_month) + 1;
              ...
              v_N1 := (add_months(x.platej_data, -1) - trunc(x.start_month_rate, 'mm'));
              v_N2 := (last_day(x.start_month_rate) - add_months(x.platej_data, -1))+1;
  ...
  function build_gpk(p_bid_id in wcs_bids.id%type -- Идентификатор заявки
    ...
    l_is_affiliate number := nvl(wcs_utl.get_answ_bool(p_bid_id,'AFFILIATE_PRG'),0);
    l_affiliate_rate number(5,3) := wcs_utl.get_answ_decimal(p_bid_id,'AFFILIATE_PRG_RATE');
  ...
  begin
    ...
    case
      when l_repayment_method in (2, 4) and nvl(l_mac_crd_type, 0) = 0 and l_is_employee <> 1 and l_is_base_rate = 0 and l_is_affiliate = 0 then
      ...
      when l_repayment_method = 0 and l_mac_crd_type = 1 and l_is_employee <> 1 and l_is_base_rate = 0 and l_is_affiliate = 0 then
      ...
      when l_repayment_method in (2, 4) and l_mac_crd_type = 1 and l_is_employee <> 1 and l_is_base_rate = 0 and l_is_affiliate = 0 then
        -- 2 - рівними частинами (база факт/факт)/ 4 - ануітет (база 30/360), стандартний кредит
        l_pl1 := cck.f_pl1(p_nd   => null, -- для тебя константа
                           p_lim2 => l_credit_sum, -- сумма кредита (в коп.)
                           p_gpk  => l_repayment_method, -- 4-Ануитет. 2 - Класс
      ...
      when l_is_employee = 1 and l_repayment_method = 4 and l_is_base_rate = 1 and l_is_affiliate = 0 then
      ...
      when l_is_employee = 1 and l_repayment_method = 2 and l_is_base_rate = 1 and l_is_affiliate = 0 then
        for c in (select * from V_BRATES where br_id = 9999 order by dat)
          loop
            ...
            logger.info('WCS GPK: rate='||c.rate||' sdate='||c.dat||' edate='||l_next_date);
          end loop;
        v_credit_rate_tab.extend(1);
        v_credit_rate_tab(v_credit_rate_tab.count) := t_credit_rate(l_interest_rate,l_date_rate_begin,null);
        logger.info('WCS GPK: rate='||l_interest_rate||' sdate='||l_date_rate_begin||' edate=');
...
/*        if to_number(l_repayment_day) > to_number(extract(DAY from l_deal_date)) then
  ...
  --      end if;
      ...
      when l_is_affiliate = 1 then
        v_credit_rate_tab.extend(1);
        v_credit_rate_tab(v_credit_rate_tab.count) := t_credit_rate(l_affiliate_rate,trunc(l_deal_date,'MM'),add_months(l_deal_date,12)-1);
        logger.info('WCS 1 rate = '||to_char(l_affiliate_rate)||' date1 = '||to_char(l_deal_date,'dd.mm.yyyy')||' date2 = '||to_char(add_months(l_deal_date,12),'dd.mm.yyyy'));
        v_credit_rate_tab.extend(1);
        v_credit_rate_tab(v_credit_rate_tab.count) := t_credit_rate(l_interest_rate,add_months(l_deal_date,12),null);
        logger.info('WCS 2 rate = '||to_char(l_interest_rate)||' date1 = '||to_char(add_months(l_deal_date,12),'dd.mm.yyyy')||' date2 = ');
        l_gpk_rec.bid_id         := p_bid_id;
        l_gpk_rec.build_date     := l_deal_date;
        l_gpk_rec.pmt_id         := 0;
        l_gpk_rec.pmt_date       := trunc(l_deal_date);
  ...
  function get_sources_of_payment(p_bid_id wcs_bids.id%type,
                                  ...
                                  p_ws_number number) return number
    is
    l_sumSP number(13,3) := 0;
    l_sumSZ number(13,3) := 0;
    l_sumFP number(13,3) := 0;
    l_SZ number(13,3) := 0;
    procedure set_dp(p_bid_id wcs_bids.id%type,
                     p_question_id wcs_questions.id%type,
                     p_sumSP number) is
      pragma autonomous_transaction;
    begin
      wcs_utl.set_answ(p_bid_id,p_question_id,p_sumSP);
      commit;
    end;
  begin
    if p_ws_id = 'MAIN' then
      select nvl(sum(grt_cost),0)
        into l_sumSP
        from v_wcs_bid_garantees
       where bid_id = p_bid_id
         and wcs_utl.get_answ_list(bid_id,'GRT_2_20',ws_id,garantee_num) = 0
         and wcs_utl.get_answ_decimal(bid_id,'KPP',ws_id,garantee_num) >= 1.3
         and wcs_utl.get_answ_decimal(bid_id,'P4DP',ws_id,garantee_num) >= 1
         and (select ord from cck_rating where code = wcs_utl.get_answ_list_text(p_bid_id,'CRISK_OBU',ws_id,garantee_num)) <= 13
         and garantee_id = 'GUARANTOR';
    else
    /*сума порук*/
      select nvl(sum(grt_cost),0)
        into l_sumSP
    ...
    /*сума фінансових порук*/
    /*l_sumFP !!!!!!!!!!!!!!!!!!!!! доробити !!!!!!!!!!!!!!!!!!!*/
    if wcs_utl.has_answ(p_bid_id,'INSURANCE_SUM') = 1 then
      l_sumFP := wcs_utl.get_answ_decimal(p_bid_id,'INSURANCE_SUM');
    else
      select nvl(sum(sum),0)
        into l_sumFP
        from (select bid_id, insurance_id, sum from v_wcs_bid_grt_insurances
              union all
              select bid_id, insurance_id, sum from v_wcs_bid_insurances)
       where bid_id = p_bid_id
         and insurance_id = 4;
    end if;
    if p_type = 'OBU' then
      l_SZ := round((l_sumSP + l_sumSZ + l_sumFP)/wcs_utl.get_answ_decimal(p_bid_id,'PK',p_ws_id,p_ws_number),3);
    else
      l_SZ := round((l_sumSP + l_sumSZ + l_sumFP)/wcs_utl.get_answ_decimal(p_bid_id,'PK_C',p_ws_id,p_ws_number),3);
    end if;
    if p_ws_id = 'MAIN' and p_type = 'OBU' then
      set_dp(p_bid_id,'DP_ALL',l_SZ);
    elsif p_ws_id = 'MAIN' and p_type != 'OBU' then
      set_dp(p_bid_id,'DP_ALL_C',l_SZ);
    end if;
    return l_SZ;
  end get_sources_of_payment;
  -- Регистрирует мужа/жену как поручителя если выбрана опция "Враховувати доходи/витрати чоловіка/дружини у доходах/витратах сім`ї?"
  procedure register_spouse_as_guarantor(p_bid_id wcs_bids.id%type -- идентификатор заявки
                                         ) is
    -- Враховувати доходи/витрати чоловіка/дружини у доходах/витратах сім`ї?
  ...
  begin
    ...
    -- смотрим есть ли в списке обеспечения субпродукта "Порука"
    select count(1)
      into l_cnt_guarant
      from wcs_bids b,
     ...
     where b.subproduct_id = sg.subproduct_id
       and b.id = p_bid_id
       and sg.garantee_id = 'GUARANTOR';
    if l_cnt_guarant = 0 then
      return;
    end if;
    ...
    wcs_pack.answ_text_set(p_bid_id,
                           'CL_1',
                           wcs_utl.get_answ_text(p_bid_id, 'CL_0_176'),
                           l_ws_id,
                           l_ws_number); -- Прізвище
    wcs_pack.answ_text_set(p_bid_id,
                           'CL_2',
                           wcs_utl.get_answ_text(p_bid_id, 'CL_0_221'),
                           l_ws_id,
                           l_ws_number); -- Ім`я
    wcs_pack.answ_text_set(p_bid_id,
                           'CL_3',
                           wcs_utl.get_answ_text(p_bid_id, 'CL_0_222'),
                           l_ws_id,
                           l_ws_number); -- По батькові
    wcs_pack.answ_list_set(p_bid_id,
                           'CL_71',
                           (case when wcs_utl.get_answ_list(p_bid_id,'CL_71') = 1 then 2 else 1 end),
                           l_ws_id,
                           l_ws_number); -- Стать
    wcs_pack.answ_text_set(p_bid_id,
                           'CL_123_R',
    ...
    wcs_pack.answ_list_set(p_bid_id,
                           'FAMILY_RELATIOSHIPS',
                           wcs_utl.get_answ_list(p_bid_id, 'FAMILY_RELATIOSHIPS'),
                           l_ws_id,
                           l_ws_number); -- Статус сімейних відносин
    wcs_pack.answ_text_set(p_bid_id,
                           'CL_0_176',
                           wcs_utl.get_answ_text(p_bid_id, 'CL_1'),
                           l_ws_id,
                           l_ws_number); -- Прізвище чоловіка/дружини
    wcs_pack.answ_text_set(p_bid_id,
                           'CL_0_221',
                           wcs_utl.get_answ_text(p_bid_id, 'CL_2'),
                           l_ws_id,
                           l_ws_number); -- Імя чоловіка/дружини
    wcs_pack.answ_text_set(p_bid_id,
                           'CL_0_222',
                           wcs_utl.get_answ_text(p_bid_id, 'CL_3'),
                           l_ws_id,
                           l_ws_number); -- По батькові чоловіка/дружини
    wcs_pack.answ_text_set(p_bid_id,
                           'CL_0_176_1',
    ...
    wcs_pack.answ_text_set(p_bid_id,
                          'CL_0_285',
                          wcs_utl.get_answ_text(p_bid_id, 'CL_0_285'),
                          l_ws_id,
                          l_ws_number); -- Місце роботи
/*    wcs_pack.answ_dec_set(p_bid_id,
                          'CL_0_70',
