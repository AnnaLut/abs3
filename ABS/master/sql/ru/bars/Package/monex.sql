    ...
    for i in 0 .. l_length - 1 loop
      ...
      If l_FL = 0 then
         ...
         declare
            nTmp_ int;
         begin
           select 1 into nTmp_ from  MONEXR where FDAT = l_FDAT and kod_nbu = l_nbu;
           raise_application_error(-20100,
      ...
      l_Branch := substr   (MONEX.getChildNodeValue(l_child, 'BarsPointCode'), 1, 22);
...
PROCEDURE monexD (p_dat1 date, p_dat2 date)  is
begin
  ...
  loop
     ...
     for k in (select distinct fdat from monexr where fdat >= p_dat1 and fdat <= p_dat2 and KOD_NBU = s.KOD_NBU ) loop
     monex.monexp   ( p_kod_NBU => s.KOD_NBU, p_dat => k.fdat );                     commit ;   end loop;

     -- 2) клиринг за период  дат на межбанке по СВИФТУ по всем валютаи системы
     monex.monex_MB ( p_kod_NBU => s.KOD_NBU, p_dat1 => p_dat1, p_dat2 => p_dat2) ;  commit ;

  end loop ; --s по системам за период

 -- Выполнить расчеты  по ЮЛ ( сальдо счета)  
--  monex.monex_KL (  p_UL =>0 ) ; -- , p_dat1 => p_dat1, p_dat2 => p_dat2 ) ; 
...
begin
  ...
  loop                                     
     ...
     If k.S_2909 > 0 and k.rS_2909 is null then  
        ...
        oo.S    := k.S_2909; 
        oo.S2   := oo.S;  
        oo.nlsb := monex.NLSM_ext ( UU.UO, UU.OB22_2909, k.branch );
     ...
     --* тега <SENTFEE> зачислить в соответствующее ТОБО в виде комиссии за отправленый перевод
        ...
        If k.kv <> gl.baseval  then  gl.payv(0, MR.rk_2909, gl.bdate, 'D06', 1, k.kv, X0.nlst, abs(k.k_2909), 980 , X0.nlst, oo.s );
           If monex.l_flag = 1 then  gl.pay (2, MR.rk_2909, gl.bDATE);    end if;
        end if;
     end if;
     ...
     If k.S_2809 > 0 and k.rs_2809 is null then    
        ...
        oo.S    := k.S_2809; 
        oo.S2   := oo.S;  
        oo.nlsb := monex.NLSM_ext (  UU.UO, UU.OB22_2809, k.branch );
...
------------------------------------------------------------------------
     ...
     If k.k_2809 > 0 and k.rk_2809 is null then   
        ...
        If k.c_kv <> gl.baseval then
           ...
           If monex.l_flag = 1 then  gl.pay (2, MR.rk_2809, gl.bDATE);    end if;
        end if;
     end if;
---------------------------------------------------------------------------------------------------------------
     ...
     If k.s_0000 > 0 and k.rs_0000 is null then  
        ...
        If p_kod_nbu = '96' then  oo.nlsb := X0.nlsb;
        else                    oo.nlsb := monex.NLSM_ext (  UU.UO, UU.OB22_2809, k.branch );
        end if;
     ...
     If UU.UO > 0 then -- Это не ОБ, а другие ЮЛ. У них есть ТРИ дополнительных поля по комиссии
        ...
        If k.KOMB2 > 0 then   --- Деб Тр  по СТП вал на сумму KOMB2  кред Комм.ЮЛ в грн 
           ...
           oo.s  := k.KOMB2 ;
           oo.s2 := gl.p_icurval ( oo.kv, oo.s, gl.bdate );  
           oo.nlsb := monex.NLSM_ext (  UU.UO, UU.ob22_kom, k.branch );
