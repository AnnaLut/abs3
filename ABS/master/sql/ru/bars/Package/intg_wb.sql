...
procedure header_mode (gmode in int) is
  ...
  function get_type_interests(p_type_id int, p_kv int, p_period varchar2) return varchar2 is
    ...
    /* "earlyClose": {
          "description": {
            "ru": "ѕо истечении 181 дн€ - 7% в рубл€х и 1,5% годовых в долларах",
    ...
    begin
      /*"withdrawal": {
          "description": {
            "ru": "Ќе более 30% от первоначальной суммы вклада",
    ...
    begin
     ...
     l_freq_k :=CASE
                  ...
                  ELSE 5-- ѕо умолчанию ежемес€чно
                END;
   ...
   BEGIN
      SELECT val
       ...
       WHERE key = par;
      RETURN TRIM (l_res);
   EXCEPTION WHEN NO_DATA_FOUND THEN raise_application_error (-20000, 'Ќе найден KEY=' || par || ' в таблице web_barsconfig!');
   END;
