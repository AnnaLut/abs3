...
procedure p_ob_sal1012(dat1 date
					  ,dat2 date
					  ,nbs_list varchar2
					  ,p_id number   );
...
procedure p_doc_per2(  p_dir varchar2
                      ,dat1 date
					  ,dat2 date);
procedure p_doc_per3(   p_dir varchar2
                      ,dat1 date
                      ,dat2 date);
...
procedure p_doc3 (    p_dir varchar2 default 'AUDIT_NBU'
                     ,p_dat date
                    );
end;
/
...
begin
  ...
  where a.nbs = p.nbs
    and a.rnk = c.rnk
	and c.rnk in (select to_number(column_value) nbs from table(gettokens(listrnk_)))
    and a.daos < dat3_
    and nvl(a.dazs,to_date('01-01-4000','dd-mm-yyyy'))> dat1_
    and c.codcagent=d.codcagent;
...
cursor c0 is    select REF, BDAT, PDAT, S, SQ, KVA, MFOA, NLSA, NAMA, KVB, MFOB, NLSB, NAMB, NAZN, REFL, SOS, TT
				 from test_oper_p67 t, accounts a
				where a.nls = case when substr(t.nlsa,1,4) = '2809' then nlsa else nlsb end
				  and a.kv = case when substr(t.nlsa,1,4) = '2809' then kva else kvb end
				  and nbs = '2809' and ob22 in ('15','17','18' ,'19','20','23','27','28' ,'29','30','32','33','34','35','36' );

--cursor c0 is	select * from test_oper_p67;
t_c0  c0%rowtype;
...
execute immediate 'truncate table test_oper_p67';
 ...
 sql_ :=
     ...
     --P67
                  when sos = 5  and (
                       (regexp_like(nlsa,''^100[1,2]'') and regexp_like(nlsb,''^2[8]09''))  or
                       (regexp_like(nlsb,''^100[1,2]'') and regexp_like(nlsa,''^2[8]09''))
					   ) then   into test_oper_p67
                select * from (
                 select /*+ parallel(10) */
                       ref,
...
commit;
    ...
    open c0;
	delete from  test_oper_p67;
    loop
        fetch c0 into t_c0;
        exit when c0%notfound;

	    insert into test_oper_p67  values t_c0 ;

	end loop;

    open c67;
    p_filename := 'D_P67p_'||f_ourmfo_g||'_'||to_char(trunc(p_dat,'q'),'yyyymmdd')||'.csv';
    f := UTL_FILE.FOPEN(p_dir, p_filename,'W');
    loop
       fetch c67 into t_c67;
       exit when c67%notfound;
       ...
       l_kv :=  coalesce(nullif(t_c67.KVA,980),nullif(t_c67.KVB,980),980);
       --l_sq :=  case when l_kv = 980 then t_c67.s else pg_cur.p_icurval(l_kv,t_c67.s,t_c67.bdat) end/100;
	   l_fio := f_dop(t_c67.ref,'KURS ');
       l_sq  := t_c67.s*to_number(l_fio)/100;
       --CONTINUE WHEN l_sq <150000;
       --CONTINUE WHEN t_c67.mfoa!=t_c67.mfob;
      -- CONTINUE WHEN l_fio is null and  t_c67.nlsb like '2902%';
