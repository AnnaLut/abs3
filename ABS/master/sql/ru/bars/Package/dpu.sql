 ...
 PROMPT ===================================================================================== 
 PROMPT *** Run *** ========== Scripts /Sql/BARS/package/dpu.sql =========*** Run *** =======
 PROMPT ===================================================================================== 
 
  CREATE OR REPLACE PACKAGE BARS.DPU 
is

g_header_version  constant varchar2(64)  := 'version 42.37 12.10.2016';
...
END DPU;
/
CREATE OR REPLACE PACKAGE BODY BARS.DPU 
is
  --
  -- глобальные переменные и константы
  --
  g_body_version  constant varchar2(64)          := 'version 43.88 13.02.2017';
  ...
  -- Types
  ---
  type t_accrec is record ( id   accounts.acc%type
...
begin
  return 'Package header DPU '||g_header_version||'.'||chr(10)||
...
begin
  ...
  RETURN 'Package  body  DPU '||g_body_version||'.'||chr(10)||
           'PLSQL_CCFLAGS => '||l_result;
...
procedure get_accparams
  (p_dealid    in  dpu_deal.dpu_id%type,
   ...
   p_mfo       in  varchar2,
   p_dpuline   in  number,
   p_termtype  in  dpu_vidd.dpu_type%type,
   p_depacc    out t_accrec,
   p_intacc    out t_accrec)
is
  title      constant varchar2(60) := 'dpu.getaccparams:';
  l_dealnum  number(38);
begin
  ...
  p_depacc.name := case
                   when p_dpuline  = 1 then bars_msg.get_msg(modcode, 'FNLS_NMS_MASK2_FLEXT1')
                   when p_termtype = 1 then bars_msg.get_msg(modcode, 'FNLS_NMS_MASK2_NBS2610')
                   when p_termtype = 2 then bars_msg.get_msg(modcode, 'FNLS_NMS_MASK2_NBS2615')
...
exception
  when others then
    bars_error.raise_nerror( modcode, 'EXPENSACC_NOT_FOUND',
                             to_char(p_dptype), p_branch, sqlerrm );
end get_expacc;
...
begin
      ...
      begin
        select r011
         ...
         where prem     = 'КБ'
           ...
           and r020r011 is null;
      exception
        when others then p_intr011 := null;
      end;
    when others then p_intr011 := null;
  end;
  ...
  -- r013 for deposit/interest accounts
  p_depr013 := case
...
begin
  ...
  begin
    insert into specparam (acc, s180, s181, r011, r013, d020)
    values (p_depaccid, l_s180, l_s181, l_depr011, l_depr013, p_d020);
  exception
...
$end
  ...
  -- OB22
  bars.Accreg.setAccountSParam( l_depacc.id, 'OB22', l_depob22 );
  ...
  -- OB22
  bars.Accreg.setAccountSParam( l_intacc.id, 'OB22', l_intob22 );
...
procedure iopen_agraccs
 (p_agrnum    in  dpu_deal.dpu_add%type,  -- № доп.соглашения
  ...
  p_depacc    out t_accrec,               -- параметры деп.счета 8-го класса
  p_intacc    out t_accrec)               -- параметры проц.счета 8-го класса
is
...
begin
  ...
  bars_audit.trace('%s entry, № %s, custid %s, dputype %s, accgrp %s', title,
                   p_dealnum, to_char(p_custid), to_char(p_dputype), to_char(p_accgrp));
...
$end
  ...
  begin
    insert into int_accn
      ( id, acc, acra, acrb, stp_dat, tt,
        metr, io, basey, freq,
        kvb, nlsb, mfob, namb )
    values
      ( 1, l_depacc.id, l_intacc.id, l_typerow.acc7, l_stpdat, l_typerow.tt,
...
is
  title  constant  varchar2(30) := 'dpu.datevalidate:';
  l_errmsg         varchar(1000);
  l_s181           varchar2(1);
  l_bsd            dpu_vidd.bsd%type;
...
--    l_ret := 0;
--  else
--    l_ret := p_num - trunc(p_num);
--    loop
--      l_ret := l_ret * 10;
--      exit when (l_ret - trunc(l_ret)) = 0;
--    end loop;
--  end if;
--  RETURN l_ret;
--end upper2int;
...
BEGIN
  ...
  begin
    select distinct
           v.BSD, v.SROK, v.TERM_TYPE, v.TERM_MIN, v.TERM_MAX, n.S181 --, v.DPU_TYPE
      into l_bsd, l_srok, l_term_type, l_term_min, l_term_max, l_s181
      from BARS.DPU_VIDD v
      join BARS.DPU_NBS4CUST n
        on ( n.NBS_DEP = v.BSD )
     where v.vidd = p_vidd;
  exception
    when NO_DATA_FOUND then
  ...
  bars_audit.trace( '%s BSD = %s, SROK = %s, TERM_TYPE = %s, S181 = %s.'
                  , title, l_bsd, to_char(l_srok), to_char(l_term_type), l_s181 );

  if ( l_term_type Is Not Null AND
       l_term_min  Is Not Null )
  then
    ...
    then -- діапазон
      ...
      --   when l_s181 = 1
      --   then
      --     l_term_min := least(l_term_min,12);
      --     l_term_max := least(nvl(l_term_max,12),l_term_max);
      --   when l_s181 = 2
      --   then
      --     l_term_min := greatest(l_term_min,12.01);
      --     l_term_max := greatest(nvl(l_term_max,12.01),l_term_max);
      --   else
      --     l_term_max := nvl(l_term_max,36);
      -- end case;
      -- ---< TEMP >---
    ...
    l_months := trunc(l_srok);               -- к-ть місяців
    l_days   := ( l_srok - l_months ) * 100; -- к-ть днів
    ...
    case l_s181
      when 0
      then  -- для поточних термін має бути строго рівним вказному терміну
        if p_datend = F_DURATION( p_datbeg, l_months, l_days )
        then
          l_errmsg := null;
        elsif l_srok = 0
        then -- довільним якщо не задано термін
          l_errmsg := null;
        else
          l_errmsg := 'Термін не відповідає допустимому ('||to_char(l_months)||' міс. '||to_char(l_days)||' днів)!';
        end if;
      when 1
      then  -- для короткострокових l_srok = min. термін депозиту
        if ( p_datend > add_months( p_datbeg, 12)                ) or
           ( p_datend < F_DURATION( p_datbeg, l_months, l_days ) )
        then
          l_errmsg := 'Термін депозиту має бути в межах від '||to_char(l_months)||' міс. '||to_char(l_days)||' днів до 12 міс.!';
        else
          l_errmsg := null;
        end if;
      when 2 then  -- для довгострокових   l_srok = max. термін депозиту
        if ( p_datend < add_months( p_datbeg, 12)                ) or
           ( p_datend > F_DURATION( p_datbeg, l_months, l_days ) )
        then
          l_errmsg := 'Термін депозиту має бути в межах від 12 міс. до '||to_char(l_months)||' міс. '||to_char(l_days)||' днів!';
        else
          l_errmsg := null;
        end if;
      else         -- у всіх інших випадках
        l_errmsg := 'Недопустимий термін депозиту!';
    end case;
...
begin
  ...
  bars_audit.trace('%s dpu_num = %s, cust_id = %s, dpu_type = %s', title,
...
begin
  ...
  begin
    ...
    l_nls8600 := vkrzn( substr(gl.amfo, 1, 5), '86000000000000' );
...
$end
  ...
  loop
      l_tt := tt_rec.tt;
  exit
      when (tt_cur%rowcount = 1 or tt_cur%notfound or tt_cur%notfound is null);
  end loop;
...
begin
  ...
  ElsIf ((SubStr(p_nls, 1, 2) = '70') Or (SubStr(p_nls, 1, 4) = '6399')) then
...
is
  l_tt       oper.tt%type;
  l_okpo     oper.id_b%type;       -- для ОщадБанку (SWIFT)
    ...
    loop
      ...
      then
        ...
        l_val := case w.TAG
                   ...
                   then '/' || l_rec_A.acc_num                        || chr(10) -- Номер рахунку Платника
                            ...
                            ||        '6/UA/' || l_rec_A.cust_idcode             -- Код країни + Код ЄДРПОУ Платника
      ...
      then
        ...
        dpu.fill_operw( p_ref, w.TAG, l_val );
...
$if DPU_PARAMS.SBER
$then
    if ( l_rec_A.acc_cur != gl.baseval ) and
       ( p_mfo_B          = '300465'   ) and
       ( p_nls_B          = '191992'   )
    then
      l_okpo := '00032129';
...
$end
    ...
    begin
      ...
      gl.in_doc3( ref_    => p_ref,                           mfoa_   => gl.amfo,
                  ...
                  d_rec_  => null,                            id_b_   => coalesce(l_okpo, p_okpo_B, l_rec_A.cust_idcode),
...
BEGIN
  ...
  then -- для автопролонгації договорів
    select dpu_id, nd, acc, datz, dat_begin, dat_end, datv, d.comproc,
           branch, nvl(cnt_dubl, 0), dpu_gen, v.fl_extend
      bulk collect
      into l_dpulist
      from dpu_deal d
      join dpu_vidd v
...
$if DPU_PARAMS.SBER
$then
           , SUM       = bars.fost(l_dpulist(i).acc, l_bdat)
$end
...
$then
  ...
  if SubStr(l_rec_A.acc_num, 1, 1) = '8' then
    select gen_acc
      into l_acccA
      from V_DPU_RELATION_ACC
  ...
  if SubStr(l_rec_B.acc_num, 1, 1) = '8' then
    select gen_acc
      into l_acccB
      from V_DPU_RELATION_ACC
...
$end
  ...
  -- якщо мультивалютна
  if ( l_rec_A.acc_cur != l_rec_B.acc_cur ) then
    l_vob  := 16;
  else
    l_vob  :=  6;
  end if;
...
( p_dpuid    in   dpu_deal.dpu_id%type,  -- ідент. депозитного договору
  ...
  type t_rec4payments is record ( acc_dep      dpu_deal.acc%type
                                ...
                                , irrevocable  dpu_vidd.irrevocable%type
...
BEGIN
  ...
  select d.ACC, d.DAT_BEGIN, d.DAT_END, d.mfo_d, d.nls_d, nms_d, d.branch, d.MIN_SUM,
         v.MIN_SUMM, -- v.IRREVOCABLE,
         case when ( v.IRREVOCABLE = 1 and d.ID_STOP = С_IRVC_STPID ) then 1 else 0 end,
...
begin
  ...
  l_bnk_dt := coalesce( p_rpt_dt, GL.bDATE, GL.gbDATE );
  ...
  begin
    select s.USERNAME || ' (' || s.MACHINE || '/' || s.OSUSER || ')'
     ...
     where s.TYPE   = 'USER'
       ...
       and c.VALUE  = sys_context('BARS_CONTEXT','USER_MFO');
  exception
    when NO_DATA_FOUND then
      l_errmsg := null;
  end;
...
begin
  ...
  for d in ( select d.dpu_id as dpt_id,     d.nd    as dpt_num,    d.datz  as dpt_dat,
                    ...
                    i.id     as int_id,    'DU%'    as int_tt,     i.acr_dat, nvl(i.stp_dat, l_bdate - 1) as stp_dat,
               ...
               join BARS.TABVAL    t on ( ad.kv  = t.kv   )
              where d.closed = 0
                and d.datV   = l_datret )
  loop
    ...
    begin
      ...
      then
        ...
        then
          ...
          update BARS.INT_ACCN i
             set i.APL_DAT = l_datret
...
begin
  ...
  then -- Нарахування по всьому портфелю
    ...
    if (p_bdate is Null)
    then -- Щомісячне нарахування %%

      l_bdate := glb_bankdate;

      -- расчет даты выполняения и граничной даты начисления процентов в конце месяца
      dpt_web.get_mnthintdates( p_bnkdate  => l_bdate,    -- текущая банк.дата
                                ...
                                p_mode     => 0 );        -- режим запуска функции

      if ( l_bdate != l_valdate )
      then
         -- нарушен регламент начисления процентов в конце месяца
         bars_audit.info( bars_msg.get_msg('DPT','AUTOMAKEINTMNTH_DENIED') );
         -- return;
         bars_error.raise_error( modcode, 666, bars_msg.get_msg('DPT','AUTOMAKEINTMNTH_DENIED') );

      end if;

    else -- Щоденне нарахування %%
    ...
    /* уніфікувати щоденне (макросом):
       - або по всьому портфелю,
       - або по тих які закінчилися
    */
    insert into INT_QUEUE
      ( kf, branch, deal_id, deal_num, deal_dat, cust_id, int_id,
        acc_id, acc_num, acc_cur, acc_nbs, acc_name, acc_iso,
...
exception
  when others then
    ...
    Else
      ...
      bars_error.raise_error( modcode, 999, SubStr(title||' '||dbms_utility.format_error_stack(), 1, 254) );
    End If;
  ...
  if ( l_nd > 0) then
    ...
    EXCEPTION
      when NO_DATA_FOUND then
        ...
        --     join BARS.ACCOUNTS a on ( a.acc = o.acc )
        --    where o.ND  = l_nd
        --      and o.acc = o.acco
        --      and a.dazs is Null;
        --
        ...
        --
        --   -- пошук суми простроченого боргу (2067) + (2069)
        --   select sum(a.OSTB)
        --     into l_crd_sum
        --     from BARS.OVR_ACCOUNTS oa
        --     join BARS.ACCOUNTS      a on (a.acc = oa.accid)
        --    where oa.OVRID = l_nd
...
begin
  ...
  l_date := greatest( trunc(SYSDATE), BARS.GL.BD );
...
PROMPT *** Create  grants  DPU ***
grant EXECUTE                                                                on DPU             to BARS_ACCESS_DEFROLE;
grant EXECUTE                                                                on DPU             to DPT_ROLE;
grant EXECUTE                                                                on DPU             to WR_ALL_RIGHTS;
grant EXECUTE                                                                on DPU             to WR_DEPOSIT_U;

 
 
 PROMPT ===================================================================================== 
 PROMPT *** End *** ========== Scripts /Sql/BARS/package/dpu.sql =========*** End *** =======
 PROMPT ===================================================================================== 
