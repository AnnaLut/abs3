...
is
  ...
  -- Получаем код филиала из кода отделения
    ...
    begin
        ...
        l_mfo := null;
        l_slash := instr(l_branch, '/', 2);

        if (l_slash > 0) then
            l_mfo := substr(l_branch, 2, l_slash - 2);
        end if;
  ...
  -- чистим атрибут selected_branch, тем самым делаем рабочим отделением самое верхнее
        ...
        sys.dbms_session.set_context(CONTEXT_CTX, CTXPAR_SELECTED_BRANCH, p_branch, client_id => sys_context('userenv', 'client_identifier'));
        -- сохраняем выбранный бранч в staff
        save_current_branch(p_branch);
        -- делаем запись в журнал, т.к. это важное действие
        bars_audit.info('Вибрано відділення '||p_branch);
        --
