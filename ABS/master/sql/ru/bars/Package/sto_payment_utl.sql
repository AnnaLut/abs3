 ...
 G_HEADER_VERSION  CONSTANT VARCHAR2(64)  :=  'version 2.3  16.01.2017';
...
CREATE OR REPLACE PACKAGE BODY BARS.STO_PAYMENT_UTL as
  ...
  IS
  BEGIN
     RETURN 'Package header sto_payment_utl ' || G_HEADER_VERSION;
  END header_version;
  ...
  IS
  BEGIN
     RETURN 'Package body sto_payment_utl ' || G_BODY_VERSION;
  END body_version;
    ...
    begin
        ...
        values (sto_payment_seq.nextval,
                p_order_id,
        ...
        returning id into l_payment_id;
         bars_audit.info('create_payment: l_payment_id=' || to_char(l_payment_id));
    ...
    begin
        ...
        exception   when dup_val_on_index
                    then p_errormsg := 'Платіж '||to_char(p_payment_id)||' вже в черзі';
    ...
    begin
        begin
         ...
         if sql%rowcount = 0
         then p_errormsg := 'Платіж '||to_char(p_payment_id)||' не знайдено в черзі платежів';
              BARS_AUDIT.INFO('create_payment_utl.create_payment_que:'||p_errormsg);
         else p_errormsg := 'Ok';
         end if;
        end;
    ...
    PROCEDURE cancel_payment (p_payment_id IN INTEGER, p_errormsg OUT VARCHAR2)
    IS
    BEGIN
       begin
          update sto_payment p
           ...
           where p.id = p_payment_id
             and p.state in (sto_payment_utl.STO_PM_STATE_NEW,sto_payment_utl.STO_PM_STATE_READY_TO_WITHDRAW);
       end;
       if sql%rowcount = 0
       then p_errormsg := 'Відміна за SMS:Платіж '||to_char(p_payment_id)||' не знайдено в черзі платежів';
       else track_payment_state(p_payment_id, sto_payment_utl.STO_PM_STATE_CANCEL_BY_CLIENT, 'Canceled by SMS');
            p_errormsg := 'Ok';
       end if;
    END;
...
/*
        l_new_state := case when l_payment_row.purpose is not null then sto_payment_utl.STO_PM_STATE_READY_TO_WITHDRAW
                            else sto_payment_utl.STO_PM_STATE_SBON_AMOUNT_GOT
                       end;
*/
    ...
    begin
        ...
        if (l_payment_amount > 0) then
            ...
            if (l_account_row.dazs is null or l_account_row.dazs >=gl.bDATE)then
                  l_customer_row := customer_utl.read_customer(l_account_row.rnk);
                  l_transit_account_number := GetGlobalOption(sto_payment_utl.TRANS_ACCOUNT_PARAMETER);
