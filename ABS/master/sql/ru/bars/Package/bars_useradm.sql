...
is
    ...
    VERSION_HEADER       constant varchar2(64)  := 'version 1.09 23.05.2011';
...
is
    ...
    VERSION_BODY      constant varchar2(64)  := 'version 2.30 05.05.2014';
...
--    l_gtwpasswd   varchar2(128);
    ...
    begin
        ...
        if (l_licpermfree = 0) then
            ...
            l_reclickey := imake_tkey(l_usrid, p_reclogname, l_reclicexp);
        ...
        l_usrstmtcr := 'create user ' || p_reclogname || ' identified by ' || p_recpasswd ||
        ...
        if (p_usrusegtw != 1) then
            ...
            execute immediate 'alter user ' || p_reclogname || ' quota unlimited on ' || l_usrdefts;
            bars_audit.trace('%s: qouta granted.', p);

            -- даем право на соединение
            execute immediate 'grant create session to ' || p_reclogname;
            bars_audit.trace('%s: create session granted.', p);

            if (p_recdefrole is not null) then
                -- выдаем умолчательную роль
                execute immediate 'grant ' || p_recdefrole || ' to ' || p_reclogname;
                bars_audit.trace('%s: default role %s granted.', p, p_recdefrole);

                -- устанавливаем умолчат. роль
                execute immediate 'alter user ' || p_reclogname || ' default role ' || p_recdefrole;
            ...
            if (p_recappauth is not null) then
                ...
                execute immediate 'alter user ' || p_reclogname || ' grant connect through "' || p_recappauth || '"';
        ...
        begin
            ...
            values (l_usrid, p_usrfio, p_reclogname, p_usraccown, l_usrtabn,
		...
		--Вставка в WEB_USERMAP
         if (p_recappauth is not null) then
            web_utl_adm.set_webuser(
									webuser_   => lower(p_reclogname),
									dbuser_    => upper(p_reclogname),
									webpass_   => null,
									blocked_   => 0,
									errmode_   => 1,
            ...
            if (p_usrusegtw = 1) then
                ...
                values (l_usrtabn, l_gtwtaskid, p_reclogname, p_gtwpasswd );
        ...
        exception
            when OTHERS then
                rollback;
                if (p_usrusegtw != 1) then
                    execute immediate 'drop user ' || p_reclogname;
                end if;
                raise;
        end;
        ...
        -- формируем сообщение
        if (l_licpermfree = 0) then
            g_lastmsg := bars_msg.get_msg(MODCODE, 'TEMPORARY_USER_CREATED', p_reclogname);
        else
            g_lastmsg := bars_msg.get_msg(MODCODE, 'USER_CREATED', p_reclogname); -- 'Наташенька-золотце');
        end if;
    ...
    begin
        ...
        if (p_usrusegtw = 1) then
            update sec_logins set secid = p_usrtabn where oralogin = l_usrlogname;
            bars_audit.trace('%s: user gateway login parameters stored.', p);
        end if;
...
-- NONONO   bars_lic.revalidate_lic;
    ...
    begin
        ...
        select s.logname into l_usrlogname
          from staff$base s
         where s.id         = p_userid
           and s.active     = 1;
        bars_audit.trace('%s: logname is %s', p, l_usrlogname);
...
grant EXECUTE                                                                on BARS_USERADM    to OIM;
grant EXECUTE                                                                on BARS_USERADM    to OIM_APPROVE;
