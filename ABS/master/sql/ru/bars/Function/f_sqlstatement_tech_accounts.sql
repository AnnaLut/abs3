    ...
    if GetGlobalOption('NUMMODEL')='3' then 
            ...
            --З НБУ файли і СТП
             if l_accounts.tip in('N00','N99') then
                l_stmt:=l_stmt||'SELECT decode(ARC_RRP.dk,0,ARC_RRP.s,0)/100 s0, decode(ARC_RRP.dk,1,ARC_RRP.s,0)/100 s1,'|| 
                         l_sel||' ARC_RRP.nlsB, ARC_RRP.nam_B';
             --з дирекції файли і СТП
             elsif l_accounts.tip='L00' then
                l_stmt:=l_stmt||'SELECT decode(ARC_RRP.dk,0,ARC_RRP.s,0)/100 s0,decode(ARC_RRP.dk,1,ARC_RRP.s,0)/100 s1,' ||
                         l_sel|| 'decode(ARC_RRP.mfob,'''||l_mfo||''',ARC_RRP.nlsB, ARC_RRP.nlsA) NLSB,
                                    decode(ARC_RRP.mfob,'''||l_mfo||''',ARC_RRP.nam_B,ARC_RRP.nam_A) NAM_B'; 
             end if;
         ...
         if l_accounts.tip in('N00','TNB','L00', 'TUR','TUD') then             
             ...
             if l_accounts.tip in('N00','TNB') then
                l_stmt:=l_stmt||'SELECT decode(ARC_RRP.dk,1,ARC_RRP.s,0)/100 s1, decode(ARC_RRP.dk,0,ARC_RRP.s,0)/100 s0,'||
                              l_sel||' ARC_RRP.nlsA, ARC_RRP.nam_A';  
             --на дирекцію                 
             elsif  l_accounts.tip in('L00','TUR','TUD') then 
                l_stmt:=l_stmt||'SELECT decode(ARC_RRP.dk,1,ARC_RRP.s,0)/100 s1, decode(ARC_RRP.dk,0,ARC_RRP.s,0)/100 s0,'||
                l_sel||'decode(ARC_RRP.mfoa,'''||l_mfo||''',ARC_RRP.nlsA, ARC_RRP.nlsB) NLSA,
                        decode(ARC_RRP.mfoa,'''||l_mfo||''',ARC_RRP.nam_A,ARC_RRP.nam_B) NAM_A';                
             end if;
        ...
        l_stmt:=l_stmt||'
                        ...
                        from (SELECT decode(o.dk,1,arc_rrp.s,0)/100 S1, decode(o.dk,0,arc_rrp.s,0)/100 S0,
                                   decode('''||l_mfo||''',arc_rrp.mfoa,arc_rrp.nlsa, arc_rrp.mfob,arc_rrp.nlsb ,'') NLSA,
                                   decode('''||l_mfo||''',arc_rrp.mfoa,arc_rrp.nam_a,arc_rrp.mfob,arc_rrp.nam_b,'') NAMA,
                        ...
                        SELECT decode (o.dk,1,arc_rrp.s,0)/100 s1,decode(o.dk,0,arc_rrp.s,0)/100 s0,
                               decode('''||l_mfo||''',arc_rrp.mfoa,arc_rrp.nlsa, arc_rrp.mfob,arc_rrp.nlsb,'') NLSA,
                               decode('''||l_mfo||''',arc_rrp.mfoa,arc_rrp.nam_a,arc_rrp.mfob,arc_rrp.nam_b,'') NAM_A,
...
/
  GRANT EXECUTE ON "BARS"."F_SQLSTATEMENT_TECH_ACCOUNTS" TO "BARS_ACCESS_DEFROLE";
