 ...
 PROMPT ===================================================================================== 
 PROMPT *** Run *** ========== Scripts /Sql/BARS/function/f_pop_otcn_n.sql =========*** Run *
 PROMPT ===================================================================================== 
 
  CREATE OR REPLACE FUNCTION BARS.F_POP_OTCN_N (Dat_ DATE, type_ NUMBER,
                                       sql_acc_ VARCHAR2,
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
31.07.2009 - оптимизация запросов
15.05.2009 - для сальдовок за период type_=5 изменили блок вычисления
             оборотов и входящих остатков
23.10.2008 - внесены изменения для хинта: вместо /*+ INDEX(a) указали
             /*+ INDEX(a XPK_SALDOA)
17.10.2008 - добавлен хинт INDEX(a) для SALDOA строка 220
29.02.2008 - для переменной Dat3_ берем последний рабочий день предыду-
             щего месяца заданного начала периода и для корректирующих
             прошлого месяца включаем и корректирующие перекрытия
             (vob=196)  вместо vob=96 берем  vob in (96, 196))
...
ELSE
   ...
   -- кiнець перiоду коригуючих для вих. залишків
   Dat2_ := ADD_MONTHS(Dat_, 1);

   -- кiнець перiоду коригуючих для вх. залишків
...
---обработка корректирующих проводок
      ...
      IF type_ = 5 THEN -- для сальдовок за період
...
--         UPDATE OTCN_SALDO a
--          SET  a.vost=NVL(Fost(a.acc, datv_), 0), -- вхідний залишок
--               a.vostq=DECODE(a.kv, 980, 0, NVL(Fostq(a.acc, datv_), 0)), -- екв. вхідного залишку
--               a.dos=Fdos(a.acc, Dat1_, Dat_),
--               a.dosq=DECODE(a.kv, 980, 0, Fdosq(a.acc, Dat1_, Dat_)),
--               a.kos=Fkos(a.acc, Dat1_, Dat_),
--               a.kosq=DECODE(a.kv, 980, 0, Fkosq(a.acc, Dat1_, Dat_)) ;

--        merge into OTCN_SALDO d
--        using (
        for k in (
            select acc,
...
--                   set d.vost=s.vost, -- вх_дний залишок
--                       d.vostq=s.vostq, -- екв. вх_дного залишку
--                       d.dos=s.dos,
--                       d.dosq=s.dosq,
--                       d.kos=s.kos,
--                       d.kosq=s.kosq;
      ELSE
         --- наполнение оборотов за месяц
--         UPDATE OTCN_SALDO a
--          SET  a.dos=Fdos(a.acc, Dat1_, Dat_),
--               a.kos=Fkos(a.acc, Dat1_, Dat_),
--               a.dosq=DECODE(a.kv, 980, 0, Fdosq(a.acc, Dat1_, Dat_)),
--               a.kosq=DECODE(a.kv, 980, 0, Fkosq(a.acc, Dat1_, Dat_)) ;
        for k in (
...
/
 ...
 PROMPT ===================================================================================== 
 PROMPT *** End *** ========== Scripts /Sql/BARS/function/f_pop_otcn_n.sql =========*** End *
 PROMPT ===================================================================================== 
