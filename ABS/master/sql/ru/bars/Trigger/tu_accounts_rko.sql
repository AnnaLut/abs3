...
PROMPT *** Create  trigger TU_ACCOUNTS_RKO ***
  ...
  CREATE OR REPLACE TRIGGER BARS.TU_ACCOUNTS_RKO AFTER UPDATE OF DAZS ON accounts
FOR EACH ROW
BEGIN
  IF :NEW.DAZS is not null THEN
    DELETE FROM rko_lst WHERE acc=:NEW.ACC ;
  END IF;
END;
/
ALTER TRIGGER BARS.TU_ACCOUNTS_RKO disable;
