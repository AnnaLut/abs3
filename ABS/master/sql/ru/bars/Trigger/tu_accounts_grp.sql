...
BEFORE UPDATE OF GRP ON accounts
FOR EACH ROW
BEGIN
IF :NEW.grp<16 THEN
   :NEW.sec:=
   UTL_RAW.BIT_OR
    ( NVL(:OLD.sec,'0000'),
      UTL_RAW.SUBSTR(
      UTL_RAW.CAST_FROM_BINARY_INTEGER(power(2,16-MOD(:NEW.grp,16)-1)),3,2));

ELSIF :NEW.grp>=16 AND :NEW.grp<512 THEN
   :NEW.sec:=
   UTL_RAW.BIT_OR
    ( NVL(:OLD.sec,'0000'),
     UTL_RAW.OVERLAY(
     UTL_RAW.CAST_FROM_BINARY_INTEGER(power(2,16-MOD(:NEW.grp,16)-1)),'0000',
