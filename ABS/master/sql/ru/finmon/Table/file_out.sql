...
begin   
 execute immediate '
  ALTER TABLE FINMON.FILE_OUT ADD CONSTRAINT XPK_FILE_OUT PRIMARY KEY (ID)
...
begin   
 execute immediate '
  CREATE UNIQUE INDEX FINMON.XAK_FILE_OUT_NAME ON FINMON.FILE_OUT (OUT_NAME) 
...
begin   
 execute immediate '
  CREATE UNIQUE INDEX FINMON.XPK_FILE_OUT ON FINMON.FILE_OUT (ID) 
  PCTFREE 10 INITRANS 2 MAXTRANS 255 COMPUTE STATISTICS 
  TABLESPACE USERS ';
exception when others then
  if  sqlcode=-955  then null; else raise; end if;
