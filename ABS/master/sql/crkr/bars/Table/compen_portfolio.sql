...
/ 
begin
    execute immediate 'create index IDX_COMPEN_PORTFOLIO_BRANCH on COMPEN_PORTFOLIO (SUBSTR(BRANCH,2,6))
  tablespace BRSBIGD';
 exception when others then 
    if sqlcode = -955 or sqlcode = -1408 then null; else raise; 
    end if; 
...
*/
begin
    execute immediate 'create index IDX_COMPEN_PORTFOLIO_branch_a on COMPEN_PORTFOLIO (substr(branchact,2,6))
  tablespace BRSBIGD';
 exception when others then 
    if sqlcode = -955 or sqlcode = -1408 then null; else raise; 
    end if; 
...
begin
    execute immediate 'alter table COMPEN_PORTFOLIO add type_person number(1)';
 exception when others then 
    if sqlcode = -1430 then null; else raise; 
    end if; 
end;
/ 
alter table COMPEN_PORTFOLIO modify type_person default 0;
/
