...
is
  ...
  -- constants
  --
  g_body_version  constant varchar2(64) := 'version 1.06  2017.04.13';
  g_dbgcode       constant varchar2(32) := 'ebkc_wforms_utl';
...
, p_new_m_rnk      in     number
  ...
  l_trace  varchar2(500) := g_dbgcode || '.change_master_card: ';
begin
 ...
 delete EBKC_DUPLICATE_GROUPS 
  ...
  begin
    ...
    bars_audit.trace( '%s.merge_2rnk: Entry with ( p_rnkfrom=%s, p_rnkto=%s ).'
                    , g_dbgcode, to_char(p_rnkfrom), to_char(p_rnkto) );
    ...
    bars_audit.trace( '%s.merge_2rnk: Exit.', g_dbgcode );
