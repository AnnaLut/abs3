...
IS
  ...
  G_BODY_VERSION  CONSTANT VARCHAR2(64)  := 'version 45.6 від 10.01.2018';
...
BEGIN
  ...
  loop
    ...
    begin
      select F_ELT_nd(k.nls3) into ELT_nd_ from dual;
    exception
      when OTHERS then
        null;
        logger.error( 'ELT.OPL  Ошибка выполнения ф-ии '||int_Func_);
--    FL_Y:=0;
--    erm:='incorrect SELECT expression with '||int_Func_;
--    raise err;
    end;
...
---------
   elsif MODE_ =3 and DAT1_ <= k.WDATE then

--  declare  err EXCEPTION;

 begin
      l_nlsp:=k.nls2; l_nmsp:=k.nms2; l_tip:=k.tip2;
      if k.nls_p is not null then
         l_nlsp := k.nls_p; l_nmsp := k.nms_p; l_tip :=k.tipp;
      end if;
...
------------
    ...
    exception  when NO_DATA_FOUND THEN pdv5:=0;
      ...
      If    MODE_= 7 and S_>k.OST3  and k.ost3=k.ost3p    -- факт=план
            and DAT1_ > k.WDATE
            and k.nls3 like '3600%'      -- AGIO
            and DEB_26_ = 'Y' then
            S_:= S_- k.OST3;
            datlit_:=' '||F_Month_lit(dat1_,1,4);
      elsif MODE_= 7 and k.OST3<0 and k.ost3=k.ost3p      -- факт=план
            and DAT1_ > k.WDATE
...
---------
 ...
 --    logger.trace('ELT.BORG 8.1 '||k.nls8||' '||k.ost8);
     ...
     -- стягнення заборгованостi з р/р клiента
      If    MODE_= 8 and k.OST8<0 and k.OST8=k.OST8P and l_ostp>0
            and (k.nls_p != k.nls3 or k.nls_p is NULL)
            and DEB_26_ = 'Y' then
