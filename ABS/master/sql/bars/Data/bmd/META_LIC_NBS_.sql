...
begin
  ...
  l_tabsemantic := 'Бал рах для виписки по Казначейству';
  l_tablinesdef := '10';
  l_tabname     := 'LIC_NBS_DKSU';
  ...
  bars_metabase.add_column(l_tabid, 1, 'NBS', 'C', 'БАЛ.СЧЕТ', null, 4, null, 0, 1, 0, 1, '', '', 0, 0, '', 0, 0, 0, '', '', 0);

  -- добавляем описание сложного поля
  l_newtabid := bars_metabase.get_tabid('PS');
  l_newcolid := bars_metabase.get_colid(l_newtabid, 'NBS');
  if (l_newtabid is not null and l_newcolid is not null) then
    bars_metabase.add_extrnval(l_tabid, 1, l_newtabid, l_newcolid, 'P', '', '', '');
  else
    dbms_output.put_line('В БМД не описана таблица PS для описания сложного поля таблицы LIC_NBS_DKSU.');
  end if;

  -- добавляем описание для условий фильтра
  l_newtabid := bars_metabase.get_tabid('PS');
  l_newcolid := bars_metabase.get_colid(l_newtabid, 'NAME');
  l_varcolid := bars_metabase.get_colid(l_newtabid, 'NAME');
  if (l_newtabid is not null and l_newcolid is not null) then
    bars_metabase.add_browsetbl(l_tabid, l_newtabid, '', '1', l_newcolid, l_varcolid, 'Назва бал. рахунку');
  else
    dbms_output.put_line('В БМД не описана таблица PS для описания условий фильтра таблицы LIC_NBS_DKSU.');
  end if;
...
prompt -- Добавление справочников

declare
  l_id   meta_tables.tabid%type;
  l_name meta_tables.tabname%type := 'LIC_NBS_DKSU';
begin
  begin
     select tabid into l_id from meta_tables where tabname = l_name;
  exception when no_data_found then  
  dbms_output.put_line('tabname = LIC_NBS_DKSU not found');
  end;
...
begin 
  execute immediate 
    ' Insert into BARS.LIC_NBS_DKSU    (NBS)'||
    '  Values    (''2513'')';
