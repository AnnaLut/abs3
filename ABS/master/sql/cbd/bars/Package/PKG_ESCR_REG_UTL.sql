...
CREATE OR REPLACE PACKAGE BODY BARS.pkg_escr_reg_utl IS
  ...
  g_body_version   CONSTANT VARCHAR2(64) := 'VERSION 8.5.3 16/08/2017';
  g_header_version CONSTANT VARCHAR2(64) := 'VERSION 8.2.7 04/05/2017';
      ...
      BEGIN
        ...
        SELECT t.deal_id BULK COLLECT
         ...
         WHERE t.deal_id IN (SELECT * FROM TABLE(all_deal_list))
           AND t.credit_status_id NOT IN (9, 5) or t.credit_status_id is null;
      EXCEPTION
        WHEN no_data_found THEN
          NULL;
      END;
