...
function ExecSync(method, args) {
    ...
    request.set_body(JSON.stringify(args));
    request.invoke();
...
function Check_MainRekv() {
    ...
    if (obj_Parameters['CUSTTYPE'] == 'person') {
        if (tmpSel == '1') {
            ...
            if (!fioMask.test(gE(curTab, 'ed_FIO_FN_NR').value)) {
                addClass(gE(curTab, 'ed_FIO_FN_NR'), 'err');
                addClass(gE(curTab, 'ed_NMK'), 'err');
                if (curElement == '') curElement = 'ed_FIO_LN';
                valid = false;
            }
            if (!fioMask.test(gE(curTab, 'ed_FIO_MN_NR').value)) {
                addClass(gE(curTab, 'ed_FIO_MN_NR'), 'err');
                addClass(gE(curTab, 'ed_NMK'), 'err');
                if (curElement == '') curElement = 'ed_FIO_MN_NR';
                valid = false;
            }
            if (!fioMask.test(gE(curTab, 'ed_FIO_4N_NR').value)) {
                addClass(gE(curTab, 'ed_FIO_4N_NR'), 'err');
                addClass(gE(curTab, 'ed_NMK'), 'err');
                if (curElement == '') curElement = 'ed_FIO_4N_NR';
                valid = false;
            }
        }
    }
...
function Check_RekvNalogoplat() {
    ...
    if (gE(curTab, 'ckb_main').checked) {
        ...
        if (obj_Parameters['TGR'] != '3' && isEmpty(gE(curTab, 'ed_DATET')) && doCheck) {
            isEmptyCheck(gE(curTab, 'ed_DATET'));
            return false;
        }
        if (obj_Parameters['TGR'] != '3' && isEmpty(gE(curTab, 'ed_DATEA')) && doCheck) {
...
function Check_EkonomNorm() {
    ...
    if (gE(curTab, 'ckb_main').checked) {
        ...
        else if (isEmpty(gE(curTab, 'ed_FS'))) {
...
function Check_ClientRekvPerson() {
    ...
    if (gE(curTab, 'ckb_main').checked) {
        ...
        if (gE(curTab, 'ddl_PASSP').selectedIndex == 0) {
        ...
        if (ClientRekv && Check_DopInf() && ValidateClient()) {
            ...
            if (tmp == '1') {
                ...
                var RegisterResult = ExecSync('Register', {
                    ...
                    DOV: obj_Parameters['DOV'],
                    BDOV: obj_Parameters['BDOV'],
                    EDOV: obj_Parameters['EDOV'],
