...
<body onload="InitObjects();">
    <form id="form1" runat="server">
        <div style="padding-left: 10px; padding-top: 10px;">
            <asp:Menu ID="mRecvTabs" runat="server" DynamicEnableDefaultPopOutImage="false" Orientation="Horizontal"
                ForeColor="Black" OnMenuItemClick="mRecvTabs_MenuItemClick">
                <StaticSelectedStyle BorderStyle="Groove" BackColor="Gray" VerticalPadding="2px"
                    BorderColor="ActiveBorder" HorizontalPadding="2px" ForeColor="White" BorderWidth="2px" />
                <StaticMenuItemStyle BorderStyle="Outset" BorderWidth="2px" BorderColor="InactiveBorder"
            ...
            <div style="border: 1px solid grey">
                <div style="padding-left: 20px; padding-top: 10px; height: 30px" class="simpleTextStyle">
                    <input type="checkbox" id="chCheckReq" checked />
                    <label for="chCheckReq">
                        Перевіряти заповнення обов'язкових реквізитів</label>
                </div>
                <asp:Panel runat="server" ID="pnMain">
                    <bars:BarsSqlDataSourceEx ID="dsMain" runat="server" AllowPaging="False" FilterStatement=""
                    ...
                    <bars:BarsGridViewEx ID="gvMain" DataSourceID="dsMain" runat="server" AutoGenerateColumns="False"
                        ...
                        CaptionType="Cool" RefreshImageUrl="/common/images/default/16/refresh.png" ShowPageSizeBox="False"
                        meta:resourcekey="gvMainResource1" GridLines="Horizontal" Width="99%" AllowSorting="true" OnRowDataBound="gvMain_RowDataBound">
                        <columns>
                        ...
                        <asp:TemplateField HeaderText="Значение" meta:resourcekey="TemplateFieldResource3">
                            <ItemTemplate>
                                <input type="text" runat="server" style='<%# "width: 250px;background-color:" + ((Convert.ToString(Eval("OPT"))=="1")?("pink"): ("white")) %>'
                                    tag='<%# Eval("TAG") %>' opt='<%# Eval("OPT") %>' isp='<%# Eval("ISP") %>' tagtype='<%# Eval("TYPE") %>' colname='<%# Eval("TABCOLUMN_CHECK") %>' tabname='<%# Eval("TABNAME") %>' value='<%# Eval("VALUE") %>'
                                    id="edEdVal" onchange="addToSaveTags(this)" /></ItemTemplate>
                            <ItemStyle Width="250px" HorizontalAlign="Center"></ItemStyle>
                        </asp:TemplateField>
                        <asp:TemplateField HeaderImageUrl="/Common/Images/default/16/reference.png" meta:resourcekey="TemplateFieldResource4">
                            <ItemTemplate>
                                <asp:ImageButton runat="server" ID="imgEdHelp" Width="16px" ToolTip="Справочник"
                                    Height="16px" Visible='<%# ( Convert.ToString(Eval("TABNAME")) == "" ? false : true ) %>'
                                    ImageUrl="/Common/Images/default/16/reference_open.png" OnClientClick=<%# "return ShowHelp(this, '" + Eval("TABNAME") + "')" %>
                                    meta:resourcekey="imgEdHelpResource1"></asp:ImageButton></ItemTemplate>
