...
function OnCOUNTRYCdChange() {
    ...
    for (i = 0; i < getEl('ddl_COUNTRY').options.length; i++) {
...
function checkOKPO(edit,validFromBase) {
    ...
    if (custType == 'person' && rezId == 1) {
        ...
        if (strOKPO.substr(strOKPO.length - 2, 1) != '0' && parent.document.frames['Tab3'].document.getElementById('ckb_main').checked) {
            var res=parseInt(strOKPO.substr(strOKPO.length - 2, 1)) % 2;
            if ((res == 0 && sex != 2) || (res == 1 && sex != 1) ) {
...
function OnTOBOCdChange() {
    ...
    for (i = 0; i < $get('ddl_TOBO').options.length; i++) {
...
function ShowfullADR() {
    ...
    /*if (result != null) {
        ...
        $('#ed_ADR').val(result[1]); //result.type1.locality + ', ' + result.type1.address;
    }*/
    if (result != null) {
        parent.obj_Parameters['fullADR'] = result;
        $get('ed_ADR').value = result.type1.locality + ', ' + result.type1.address;
    }
}
