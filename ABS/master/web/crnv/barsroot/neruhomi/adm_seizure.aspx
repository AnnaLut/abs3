 <%@ Page Language="C#" AutoEventWireup="true" CodeFile="adm_seizure.aspx.cs" Inherits="adm_seizure" %>
    ...
    <form id="formOperationList" runat="server">
        ...
        <script type="text/javascript">
               ...
               function open_seizure_immobile(key) {
                   var myWindow = window.showModalDialog("/barsroot/neruhomi/seizure_immobile_adm.aspx?key=" + key, "", "dialogHeight:525px;dialogWidth:600px;center:yes;edge:sunken;scroll:no;help:no;status:no;");
                   if (myWindow == true) {
        ...
        <BarsEx:BarsGridViewEx ID="gvMain" runat="server" AllowPaging="True" AllowSorting="True"
            ...
            <Columns>
                  <asp:TemplateField>
                    <ItemTemplate>
                        <asp:Image runat="server" src="/Common/Images/default/16/form_green.png"  Visible='<%#String.Format("{0}", Eval("fl"))=="-5"||String.Format("{0}", Eval("fl"))=="13"||String.Format("{0}", Eval("fl"))=="8"||(String.Format("{0}", Eval("fl"))=="10" && Convert.ToDecimal(String.Format("{0}", Eval("ost")))==0)||String.Format("{0}", Eval("fl"))=="-3"||String.Format("{0}", Eval("fl"))=="9"||String.Format("{0}", Eval("fl"))=="-4"||(String.Format("{0}", Eval("fl"))=="11" && Convert.ToDecimal(String.Format("{0}", Eval("ost")))==0) ?(false):(true) %>' title="Накласти арешт"  onclick='<%#"open_seizure_immobile(" + Eval("KEY") + ")" %>'/>
                    </ItemTemplate>
                </asp:TemplateField>
