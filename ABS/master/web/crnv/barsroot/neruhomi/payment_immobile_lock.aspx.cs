...
public partial class payment : Bars.BarsPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            SelectCommand += "where 1=1 and fl !=6 ";
        }
        ...
        {
            SelectCommand += " and fl in(-3,5,6,7) ";
        }
        if (rbWaits.Checked)
        {
            SelectCommand += " and fl in (8, 10,13) ";
            
        }
        if (rbSender.Checked)
        {
            SelectCommand += " and (fl in(9) or fl in(11) and OST=f_part_sum_immobile(key)) ";
        }
        if(rbPartPay.Checked)
        {
            SelectCommand += " and fl in(11) and OST>f_part_sum_immobile(key) ";
        }
        if (rbSeizure.Checked)
        {
            SelectCommand += " and key in(select key from part_pay_immobile where status=2) ";
        }
        if (rbErr.Checked)
        {
            SelectCommand += " and fl in(-4, -5) ";
        }
