...
public partial class DepositClosePayIt : Bars.BarsPage
{
    ...
    private void Page_Load(object sender, System.EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            Deposit dpt = new Deposit(Convert.ToDecimal(dpt_id.Value), other);
            ...
            /// Якщо ми вернулися з вибраною операцією по ви
                     ...
                     BankType.GetCurrentBank() == BANKTYPE.SBER) &&
                    Convert.ToString(Request["dest"]) == "closep")
            {
    ...
    /// </summary>
    private void FillControls(bool getShtraf, decimal sum2pay, decimal perc2pay)
    {
        if (Request["dpt_id"] == null)
        ...
        Deposit dpt = new Deposit(Convert.ToDecimal(Convert.ToString(Request["dpt_id"])), other);
            ...
            try
            {
                ...
                if (Request["agr_id"] == null && getShtraf && dpt.EndDate != DateTime.MinValue)
                {
                    ...
                    cmdGetShtrafInfo.CommandText = "begin " +
                        ...
                        ":p_penalty, :p_commiss, :p_commiss2, :p_dptrest, :p_intrest); " +
                    "end;";
                    ...
                    ShtrafSum.ValueDecimal = p_penalty;
                    ShtrafPercentSum.ValueDecimal = p_intrest;
    ...
    private void btShtraf_ServerClick(object sender, System.EventArgs e)
    {
        ...
        try
        {
            ...
            cmdShtraf.CommandText = "begin dpt_web.penalty_payment(:dpt_id,:sum, :dpt_rest, :perc_rest); end;";
            ...
            FillControls(false, sum2pay, perc2pay);
    ...
    private void btPayPercent_Click(object sender, System.EventArgs e)
    {
        ...
        else
        {
            ...
            if (mSum > 0)
            {
                DBLogger.Info("Пользователь выплатил проценты при досрочном расторжении вклада на странице выплаты депозита до завершения. Номер депозита " + Convert.ToString(Request["dpt_id"]),
                    "deposit");
    ...
    private void CheckIfEnabled()
    {
        ...
        try
        {
            ...
            cmdInfo.CommandText = "SELECT to_char(d.dat_end,'dd/mm/yyyy'),v.br_wd, NVL(v.min_summ*t.denom,0),to_char(bankdate,'dd/mm/yyyy'), t.denom " +
                "FROM dpt_deposit d, dpt_vidd v, tabval t WHERE d.deposit_id = :dpt_id AND d.vidd = v.vidd and d.kv = t.kv";
