...
public partial class DepositContract : Bars.BarsPage
{
    ...
    private void Page_Load(object sender, System.EventArgs e)
    {
        ...
        // Проверяем установлены ли переменные сессии. Они
        // устанавливаются первой страницей и содержат информацию
        // о клиенте. Если переменные не установлены, значит
        // пришли напрямую -- отправляем на первую страницу
    ...
    private void InitControls(bool reinit_types)
    {
        ...
        try
        {
            ...
            if (reinit_types)
            {
                ...
                cmdTypes.CommandText = "select type_id, type_name from dpt_types where FL_ACTIVE = 1 order by sort_ord";
    ...
    private void FillContractDependControls()
    {
        ...
        try
        {
            ...
            if (listContractType.SelectedValue != String.Empty)
            {
                ...
                dpt_bonus.estimate_bonus(:p_custid,:p_vidd), 
                v.currency_code, v.dpt_brateid, dpt.get_forecast_int (:p_vidd, :p_dptamount * t.denom)/t.denom,    
                getbrat(sysdate, v.dpt_brateid, v.currency_code, :p_dptamount * t.denom), 
                dv.term_type, t.denom
                from v_dpt_type v, dpt_vidd dv, tabval t where dpt_type = :dpt and dv.vidd = v.dpt_type and v.currency_code = t.kv";
                ...
                cmdSelectContractParams.Parameters.Add("dpt", OracleDbType.Decimal, contractTypeCode, ParameterDirection.Input);
    ...
    private int CheckFormData()
    {
        ...
        // якщо вклад в металах то min.сума вкладу вказана в грамах
        if (kv.Value == "959" || kv.Value == "961" || kv.Value == "962")
        {
