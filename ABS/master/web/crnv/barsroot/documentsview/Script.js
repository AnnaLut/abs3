...
function printSelDocum() {
    if (arrayForPrint.length > 0)
        for (var i = 0; i < arrayForPrint.length; i++) {
            document.all.webService.Doc.callService(onPrint, "GetFileForPrint", arrayForPrint[i], document.getElementById("cbPrintTrnModel").checked);
        }
    return false;
}
...
function getError(result, modal) {
 ...
 function OpenDoc(ref)
 {
	...
	window.open("/barsroot/documentview/default.aspx?ref="+ref, "_self", true);
 }
//--рефреш
