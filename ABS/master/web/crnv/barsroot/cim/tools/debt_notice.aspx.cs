...
public partial class cim_tools_debt_notice : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            try
            {
                ...
                oraCmd.CommandText = "select name, adr from cim_journal_num where branch = nvl(sys_context('bars_context', 'user_branch'), f_ourmfo)";
        ...
        dsDebtNotice.SelectCommand = @"select * from v_cim_borg_message where delete_date is null and (" + cb1 + @"=1 or file_name is null) and
                                    ...
                                    and (" + cb2 + @"=1 or branch = nvl(sys_context('bars_context', 'user_branch'), f_ourmfo) )
                                    order by file_name desc, id desc";

        ScriptManager.GetCurrent(this).Scripts.Add(new ScriptReference("/barsroot/cim/tools/script/debt_notice.js?v1.5"));
    }
    protected void gvDebtNotice_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            rowdata += string.Format(parMaskStr, "nb", Convert.ToString(DataBinder.Eval(e.Row.DataItem, "NAME_BANK")));
            rowdata += string.Format(parMaskStr, "ab", Convert.ToString(DataBinder.Eval(e.Row.DataItem, "ADR_BANK")));
            ...
            rowdata += string.Format(parMaskStr, "nk", Convert.ToString(DataBinder.Eval(e.Row.DataItem, "NAME_KL")));
            rowdata += string.Format(parMaskStr, "ak", Convert.ToString(DataBinder.Eval(e.Row.DataItem, "ADR_KL")));
    ...
    public static void DeleteDebt(decimal id, string filename)
    {
        ...
        try
        {
            ...
            if (string.IsNullOrEmpty(filename))
            {
                ...
                oraCmd.CommandText = "update cim_borg_message set delete_date=sysdate, delete_uid=user_id where id=:id";
                oraCmd.ExecuteNonQuery();
            }
            else
            {
    ...
    protected void btGenFile_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            string templatePath = Path.Combine(System.Web.HttpContext.Current.Server.MapPath("templates"), "borg_message.frx");
            ...
            cmd.CommandText = (@"declare
                                     p_txt  clob  := null; 
                                 begin
                                    :p_txt :=f_cim_borg_message(:p_file_name);
                                 end;");

            cmd.Parameters.Add("p_txt", OracleDbType.Varchar2, "".PadRight(4000), ParameterDirection.InputOutput);
