...
function fnLoadGeneral() {
    ...
    if (data[48].text == "2")
        dd_data["ddTobo"] = url_dlg_mod + "our_branch&tail=''";
    else
...
function fnSelectNbs() {
    var result = window.showModalDialog("ListNbs.aspx", null, "dialogWidth:600px;dialogHeight:600px;center:yes;edge:sunken;help:no;status:no;");
    if (result != null) {
...
function fnGetNbs() {
    webService.Acc.callService(onGetNbs, "getNbs", document.all.tbNbs.value);
}
