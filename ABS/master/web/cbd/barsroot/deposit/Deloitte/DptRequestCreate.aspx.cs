...
public partial class DptClientRequestCreate : Bars.BarsPage
{
    ...
    /// </summary>
    private Int32 cust_id
    {
        get
        {
            return Convert.ToInt32(Request["cust_id"]);
        }
    }
    ...
    {
        DBLogger.Info("Користувач натиснув кнопку " + btnReturn.Text + " на сторінці \"Формування запиту на доступ через «БЕК-офіс»\".", "deposit");
        ...
        if (gvContracts.DataKeys.Count > 0)
        {
            foreach (GridViewRow row in gvContracts.Rows)
            {
                ...
                if (cbSelect.Checked)
                {
                    ...
                    AccessRow.DPT_ID = Convert.ToInt32(gvContracts.DataKeys[row.DataItemIndex]["DPT_ID"]);
                    
                    DBLogger.Info("Користувач вибрав депозитний договір №" + Convert.ToString(AccessRow.DPT_ID), "deposit");
    ...
    protected void btPrintRequest_Click(object sender, EventArgs e)
    {
        ...
        if (req_id.HasValue)
        {
            ...
            DptRequest = new DepositRequest(Convert.ToInt32(req_id.Value), AccessList);
    ...
    protected void btPrintSign_Click(object sender, EventArgs e)
    {
        ...
        Int64 p_rnk = Convert.ToInt64( Request.QueryString["cust_id"]);
        DBLogger.Info("Користувач роздрукував картку зразків підписів для кліента rnk =" + Convert.ToString(p_rnk), "deposit");
        if (p_rnk>0)
        {
            // Друк Заяви
            FrxParameters pars = new FrxParameters();
            pars.Add(new FrxParameter("p_rnk", TypeCode.Int64, p_rnk));
    ...
    {
        DBLogger.Info("Користувач почав формування запиту на доступ №" , "deposit");
		if (req_id.HasValue)
        {
            DepositRequest.InitRequest(req_id.Value);

            DBLogger.Info("Користувач завершив формування запиту на доступ №" + Convert.ToString(req_id.Value), "deposit");
            ...
            Decimal? ClientApplicationID = ep.MSG_CREATE("CLIENT", cust_id.ToString());            
            ...
            else
            {
                for (int i = 0; i < AccessList.Count; i++)
                {
                    ...
                    Decimal? DealApplicationID = ep.MSG_CREATE("AGR", AccessList[i].DPT_ID.ToString());
                ...
                {//створення повідомлень для працівників бекофісу
					...
					DBLogger.Info(url, "deposit");
					DepositRequest.SendMessageToBackOffice(url);
                }
                catch (Exception ex)
                {
                    DBLogger.Info("Exception text = " + ex.Message, "deposit");
    ...
    /// 
    /// </summary>
    private void FillGrid(String TrusteeType)
    {
        ...
        // РНК власника
        dsContracts.SelectParameters.Add("Trustee_ID", TypeCode.Decimal, Request.QueryString["cust_id"]);
        DBLogger.Info(strQuery, "deposit");
