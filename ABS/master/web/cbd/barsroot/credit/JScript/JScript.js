/* Файл содержащий скрипты для библиотеки контролов */
...
function doNumOnly(e) {
    var key = e.keyCode;
    if (key > 31 && (key < 48 || key > 57) && (key < 96 || key > 105)) return false;
}
...
function ToTxt(id) {
    ...
    var exp = Math.pow(10, controls[id].decimals);
    ...
    // alert('beforedot = ' + beforedot);
    // alert('afterdot = ' + afterdot);
    ...
    // alert('new_beforedot = ' + new_beforedot);

    if (controls[id].groupSeparator != '' && new_beforedot.indexOf("-" + controls[id].groupSeparator) != -1) new_beforedot = new_beforedot.substring(0, 1) + new_beforedot.substring(2);

    // alert('new_beforedot + afterdot = ' + new_beforedot + afterdot);
