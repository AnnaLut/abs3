...
$(function () {
  ...
  $('#tableYear').jungGridView({
    updateTableUrl: '/barsroot/kaznaEstimate/yearlist/',
...
function addYear() {
  ...
  //newYearRow.find('input[name="krk"]').val($('#krk-kod').val());

  newYearRow.find('input[name^="s"]').maskMoney({ allowZero: true, allowNegative: true, thousands: ' ' });


  newYearRow.find('input[name="datd"]')/*.datepicker({
    changeMonth: true,
    changeYear: true,
...
function saveYear(elem) {
  ...
  if (validNewYear(thisTr)) {
    $('#tabsEstimateGroup-1').loader();
    $.post('/barsroot/kaznaEstimate/yearsave/',
        {
          ID_: thisTr.data('id'),
        ...
        function (data, textStatus) {
          ...
          $('#tabsEstimateGroup-1').loader('remove');
        }, 'json');
  }
  else {
...
/*function saveEstimate(elem) {
    $('#tabsEstimateGroup-1').loader();
    var thisTr = $(elem).parentsUntil('tr').parent();
    if (validNewLegal(thisTr)) {
        $.post('/barsroot/kaznaEstimate/legaladd/',
                {
                    ID: null,
                    idu: null,
...
function removeYear() {
  if (selectedYear != null) {
    barsUiConfirm('Показник буде видалено', function () {
      $('#tabsEstimateGroup-1').loader();
      $.post('/barsroot/kaznaEstimate/yearremove/',
...
function visaYear() {
  ...
  if (check.length == 0) {
    ...
    $('#tabsEstimateGroup-1').loader();
    $.post('/barsroot/kaznaEstimate/yearvisa/',
        { id: check.arr },
        function (data, textStatus) {
          $('#tabsEstimateGroup-1').loader('remove');
          if (textStatus == 'success') {
