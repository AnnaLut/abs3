...
$(function () {
  ...
  $('#tableGeneral').jungGridView({
    updateTableUrl: '/barsroot/kaznaallocation/generallist/',
    ...
    trClickFunk: function () { selectGeneral(); },
    sort: 'ID',
    sortDir: 'DESC'
  });
});
...
function addGeneral() {
  ...
  if (selectedGroup != null) {
    ...
    newGeneralRow.find('input[name="datd"]').mask('99/99/9999');
...
function saveGeneral(elem) {
  ...
  if (validNewGeneral(thisTr)) {
    ...
    var post = $.post('/barsroot/kaznaallocation/generalsave/',
        {
          ID_: thisTr.data('id'),
...
function removeGeneral() {
  var selectedGeneral = $('#tableGeneral').jungGridView('selectedrow');
  if (selectedGeneral != null) {
    barsUiConfirm('Показник буде видалено.', function () {
      $('#tabsAllocationGroup-2').loader();
      $.post('/barsroot/kaznaallocation/generalremove/',
...
function visaGeneral() {
  ...
  if (check.length == 0) {
    ...
    $('#main').loader();
    $.post('/barsroot/kaznaallocation/generalvisa/',
        { id: check.arr /*selectedGeneral.data('id')*/ },
        function (data, textStatus) {
          $('#main').loader('remove');
          if (textStatus == 'success') {
            if (data.status == 'ok') {
...
function editGeneral() {
  ...
  if (selectedGeneral != null) {
    ...
    if (ref != '') barsUiAlert('Заборонено редагування взятого на облік показника', 'Помилка', 'error');
      ...
      newGeneralRow.find('input[name="kvk"]').val(kpk);      
