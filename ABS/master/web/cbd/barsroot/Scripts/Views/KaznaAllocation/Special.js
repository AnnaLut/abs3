...
$(function () {
  ...
  $('#tableSpecial').jungGridView({
    updateTableUrl: '/barsroot/kaznaallocation/speciallist/',
    ...
    trClickFunk: function () { selectSpecial(elem); },
    sort: 'ID',
    sortDir: 'DESC'
  });
});
...
function addSpecial() {
  ...
  if (selectedGroup != null) {
    ...
    newSpecialRow.find('input[name="datd"]').mask('99/99/9999');
...
function saveSpecial(elem) {
  ...
  if (validNewSpecial(thisTr)) {
    ...
    $.post('/barsroot/kaznaallocation/specialsave/',
        {
          ID_: thisTr.data('id'),
...
function removeSpecial() {
  var selectedSpecial = $('#tableSpecial').jungGridView('selectedrow');
  if (selectedSpecial != null) {
    barsUiConfirm('Показник буде видалено.', function () {
      $('#tabsAllocationGroup-3').loader();
      $.post('/barsroot/kaznaallocation/specialremove/',
...
function visaSpecial() {
  ...
  if (check.length == 0) {
    ...
    $('#main').loader();
    $.post('/barsroot/kaznaallocation/specialvisa/',
        { id: check.arr },
        function (data, textStatus) {
          $('#main').loader('remove');
          if (textStatus == 'success') {
