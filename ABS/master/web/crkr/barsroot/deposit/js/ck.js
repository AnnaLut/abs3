...
function ckNumber()
{
	...
	var type = docType.options[docType.selectedIndex].value;
  
	var validator = GetNumDocValidator(type);

	if (!validator.regExp.test(val) || val.length > validator.length)
	{
...
function CheckDocNumber(element) {
    // ENTER пускаємо далі
    if (event.keyCode == 13) {
        return true;
    }
    ...
    var select = document.getElementById("listDocType");
