...
namespace Bars.WebServices.Skrynka
{
    ...
    public class AddType
    {
         ...
         Int64 CellCount)
        {
            ...
            _dbLogger.Info("Addtype(" + O_SK.ToString() + "," + Branch + "," + Name + "," + EtalonID.ToString() + "," + CellCount.ToString() + ");", "Skrynka");
            ...
            {
                _dbLogger.Info("Помилка при " + e.Message, "Skrynka");
                throw e;
            }
            finally
            {
    ...
    public class AddND
    {
        ...
        {
            _dbLogger = DbLoggerConstruct.NewDbLogger();
            _dbLogger.Info("AddND(" + O_SK.ToString() + "," + Branch + "," + ND.ToString() + "," + OpenDate + "," + CloseDate + "," + RenterName + "," + SOS.ToString() + ");", "Skrynka");
            OracleConnection con = OraConnector.Handler.IOraConnection.GetUserConnection();
            OracleCommand cmd = con.CreateCommand();
            try
            {
                ...
                cmd.Parameters.Add("p_sos", OracleDbType.Decimal, SOS, ParameterDirection.Input);
    ...
    public class SkrynkaService : Bars.BarsWebService
    {
         ...
         Int64 CellCount)
        {
            ...
            {
                _dbLogger.Info(e.Message, "Skrynka");
                throw e;
         ...
         Int64 SOS)
        {
            try
            {
                ...
                AddND nd = new Skrynka.AddND(O_SK, Branch, ND, OpenDate, CloseDate, RenterName, SOS);
            }
            catch (System.Exception e)
            {
                _dbLogger.Info(e.Message, "Skrynka");
