...
public class ListSetRepository : IListSetRepository
{
    ...
    private void InitListfuncsetQuery(decimal setId)
    {
        ...
        {
            SqlText = string.Format(@"select f.FUNC_ID, (select name from operlist where codeoper = f.FUNC_ID) FUNCNAME, f.FUNC_ACTIVITY, F.FUNC_COMMENTS, F.FUNC_POSITION, F.KF, F.REC_ID, F.SET_ID
