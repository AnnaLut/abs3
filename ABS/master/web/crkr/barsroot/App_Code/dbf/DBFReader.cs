...
namespace DotNetDBF
{
    public class DBFReader : DBFBase, IDisposable
    {
        ...
        {
            return NextRecord(65001);
        }

        public Object[] NextRecord(int codePage)
        {
            if (isClosed)
            {
                ...
                for (int i = 0; i < header.FieldArray.Length; i++)
                {
                    switch (header.FieldArray[i].DataType)
                    {
                        case NativeDbType.Char:
                            ...
                            {
                                //Other code pages
                                recordObjects[i] = Encoding.GetEncoding(codePage).GetString(b_array);
                            }
                            break;
                        ...
                        case NativeDbType.Numeric:
                            ...
                            try
                            {
                                ...
                                if (t_numeric.Length > 0
                                    && !tParsed.Contains(DBFValue.Unknown))
                                {
                                    recordObjects[i] = Decimal.Parse(
                                        tParsed.Replace(".", CultureInfo.CurrentCulture.NumberFormat.NumberDecimalSeparator).Replace(",", CultureInfo.CurrentCulture.NumberFormat.NumberDecimalSeparator)
                                    );
                                }
                                else
                                {
        ...
        public Object[] NextRecord(bool isDosEncoding = false)
        {
            if (isDosEncoding)
            {
                return NextRecord(866);
            }
            return NextRecord();
        }
    }
