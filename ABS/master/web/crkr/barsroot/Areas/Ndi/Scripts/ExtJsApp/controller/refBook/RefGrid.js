Ext.define('ExtApp.controller.refBook.RefGrid', {
            ...
            if (col.NOT_TO_SHOW != 1) {
                ...
                if (cellIndex == IterIndex && col.WEB_FORM_NAME != null && col.WEB_FORM_NAME != "" && col.WEB_FORM_NAME != undefined) {
                    ...
                    if (href.indexOf("barsroot/ndi/referencebook") < 0) {
                        ...
                        if (col.WEB_FORM_NAME.indexOf("sParColumn=[") > 0) {
            ...
            { tableId: thisController.controllerMetadata.tableInfo.TABID, tableName: thisController.controllerMetadata.tableInfo.TABNAME, parameters: pars, filterName: filterNameParam.value, saveFilter: saveInBD, clause: clause },
              function (status, msg) {
                  ...
                  if (saveInBD == 1)
                      title = 'збереження';
                  if (saveInBD == 0)
                      title = 'додано до динамічних фильтрів';
                  //обработка при удачном запросе на сервер
                  Ext.MessageBox.hide();
    ...
    showBeforeFilterDialog: function (metadata, referenceGrid) {
        ...
        ////если есть информация о начальных фильтрах
        //controllerMetadata.SympleFilters = ExtApp.utils.RefBookUtils.configFormFields(controllerMetadata.filtersMetainfo.FilterColumns);
        ...
        if (controllerMetadata.filtersMetainfo.ShowFilterWindow != 'false' || hasCustomFilter || hasSystemFilter) {
            ...
            //{
            //    title: 'звичайні',
            //    items: Ext.create('ExtApp.view.refBook.SimpleFilterPanel', { SympleFilters: controllerMetadata.SympleFilters })
                    ...
                    handler: function (btn) {
                        ...
                        if (true) {
                            ...
                            if (params && params.length > 0)
                                thisController.sendToServer(
                                    "/barsroot/ReferenceBook/InsertFilter",
                                    { tableId: thisController.controllerMetadata.tableInfo.TABID, tableName: thisController.controllerMetadata.tableInfo.TABNAME, parameters: pars, saveFilter: 0 },
                                      function (status, msg, controller) {
                                          ...
                                          if (status == 'ok') {
                                              ...
                                              if (clause && clause != '') {
                                                  thisController.addDynamicFilterHandler('', clause);
                                              }
                                              Ext.MessageBox.hide();
                                              myWindow.close();
                                              controllerMetadata.tabPanel = tab;
    ...
    writeFilter: function (saveInBD, items, whereClause) {
    ...
    addDynamicFiltersTub: function (row, clause) {
        var thisController = this;
        var controllerMetadata = thisController.controllerMetadata;
        controllerMetadata.thisController = thisController;
        var tab = Ext.getCmp('tabFilterPanel');
        ...
        if (filterGrid && filterGrid.store)
            thisController.insertDynamicFilterInGrid(row, clause);
    },

    addDynamicFilterHandler: function (row, clause) {
        var thisController = this;
        var filterGrid = Ext.getCmp('DynamicFiltersGridId');
        if (filterGrid && filterGrid != '' && filterGrid.store) {
            this.insertDynamicFilterInGrid(row, clause);
        }
        else {
            this.addDynamicFiltersTub(row, clause);
        }

    },

    insertDynamicFilterInGrid: function (row, clause) {
        var thisController = this;
        var filterGrid = Ext.getCmp('DynamicFiltersGridId');
        if (!filterGrid)
            return false;
        ...
        if (clause && clause != '') {
            ...
            if (!item) {
                filterGrid.store.insert(0, { IsApplyFilter: true, FilterName: 'фільтр', Where_clause: clause });
            }
        }
    ...
    executeCurrentSqlFunction: function (callbackFunc) {
        ...
        Ext.each(func.params, function (param) {
            ...
            { codeOper: func.CodeOper, tableId: func.tableId, funcId: func.funcId, jsonFuncParams: Ext.JSON.encode(param.rowParams), procName: func.funcName, random: Math.random, jsonSqlProcParams: window.jsonSqlProcParams },
            ...
            function (status, msg, controller) {
                ...
                if (func.currentCallIndex == func.allFuncCallCount) {
                    ...
                    if (func.linkWebFormName && func.linkWebFormName != '' && !func.errorLog.length > 0) {
                        ...
                        Ext.MessageBox.show({
                            title: func.infoDialogTitle,
                            msg: "Виконання процедур закінчено. <br/>Успішно " + func.successCallCount + " з " + func.allFuncCallCount + "<br/>" + resMsg + '</br> </br>',
                            buttons: Ext.MessageBox.OK
                        });
                    }
