Ext.define('ExtApp.view.refBook.FilterConstructorGrid', {
extend: 'Ext.grid.Panel',
    alias: 'widget.CustomFiltersreferenceGrid',
margin: 10,
height: 200,
width: 400,
columns: [],
...
initComponent: function () {
        ...
        listeners: {
            beforerender: function (formPanel) {
                ...
                if (form.getRecord().phantom) {
                    var notToEditCols = Ext.Array.filter(referenceGrid.metadata.filtersMetainfo.FiltersMetaColumns, function (col) { return col.NOT_TO_EDIT == 1; });
    ...
    //Ext.create('ExtApp.view.refBook.RefGridFilterExt', referenceGrid.metadata.extFilters, this.store),
    {
        ...
        items: [
            ...
            },
            {
                ...
                iconCls: 'excel',
                menu: [
                    { xtype: 'Ext.Button', text: 'Усі сторінки', allPages: true },
                    { xtype: 'Ext.Button', text: 'Поточна сторінка', allPages: false }
                ]
            }
        ]
    }];
    ...
    if (referenceGrid.metadata.tableMode != null && referenceGrid.metadata.tableMode != undefined) {

        var isDisabled;
        isDisabled = !referenceGrid.metadata.canInsert();
        {
