...
function separateMoney(text, precision) {
  ...
  if (arr.length > 1) {
      if (!arr[1]) arr[1] += '00';
      if (1 == arr[1].length) arr[1] += '0';
    result += arr[1].substring(0, precision||2);
  } else {
    result += '00';
  }
  return result;
...
function barsUiAlert(options) {
  ...
  if (typeof(options) !== 'object') {
    ...
    var id = options.id || 'barsUiAlertDialog';
  ...
  options = $.extend({ text: "", func: null, title: "", winType: "", width: '400', height: '150' }, options);
    ...
    alert.dialog({
      bgiframe: true,
      dialogClass: '',
      buttons: options.buttons || [{ text: 'ok', click: function () { alert.dialog('close'); } }],
      autoOpen: true,
      position: { at: 'center' },
      title: options.title,
      title_html: true,
  ...
  var methods = {
    init: function(options) {
      ...
      var make = function() {
        ...
        var divPopup = $('<div class="popupBox" />').css(options.css);
