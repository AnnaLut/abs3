    ...
    <form id="form1" runat="server">
        ...
        <StaticHoverStyle CssClass="hover"/>   
            ...
            <div style="border: 1px solid #c5d0dc;padding: 10px">
                    ...
                    <bars:BarsGridViewEx 
                        ...
                        <asp:TemplateField HeaderText="Значение" meta:resourcekey="TemplateFieldResource3">
                            <ItemTemplate>
                                <input type="text" 
                                    runat="server" 
                                    style='<%# "width: 250px;background-color:" + ((Convert.ToString(Eval("OPT"))=="1")?("pink"): (Convert.ToString(Eval("NOT_TO_EDIT"))=="1")?("#ddd"): ("white")) %>'
                                    tag='<%# Eval("TAG") %>' 
                                    opt='<%# Eval("OPT") %>' 
                                    isp='<%# Eval("ISP") %>' 
                                    tagtype='<%# Eval("TYPE") %>' 
                ...
                <asp:Panel runat="server" ID="pnCats" Visible="false">
                    <bars:BarsSqlDataSourceEx ID="dsCats" runat="server" AllowPaging="False" FilterStatement=""
                        PageButtonCount="10" PagerMode="NextPrevious" PageSize="10" PreliminaryStatement=""
                        ProviderName="barsroot.core" SortExpression="" SystemChangeNumber="" WhereStatement="">
                    </bars:BarsSqlDataSourceEx>
                    <bars:BarsGridViewEx ID="gvCats" DataSourceID="dsCats" runat="server" AutoGenerateColumns="False"
                        CaptionText="" CssClass="barsGridView" DateMask="dd.MM.yyyy" ExcelImageUrl="/common/images/default/16/export_excel.png"
                        FilterImageUrl="/common/images/default/16/find.png" MetaFilterImageUrl="/common/images/default/16/filter.png"
                        MetaTableName="" DataKeyNames="CATEGORY_ID" RefreshImageUrl="/common/images/default/16/refresh.png"
                        ShowFooter="True" ShowPageSizeBox="False" GridLines="Horizontal" Width="99%">
                        <columns>
                        <asp:BoundField DataField="CATEGORY_ID" HeaderText="Код категорії"></asp:BoundField>
                        <asp:TemplateField HeaderText="Встановити">
                            <ItemTemplate>
                                <input type="checkbox" runat="server" id="Checkbox1" onclick="addToSaveCategory(this)"
                                    CatId='<%# Eval("CATEGORY_ID") %>' checked='<%#( Convert.ToString(Eval("VALUE")) == "1" ? true : false ) %>' />
                            </ItemTemplate>
                            <ItemStyle Width="65px" HorizontalAlign="Center"></ItemStyle>
                        </asp:TemplateField>
                        <asp:TemplateField HeaderText="Назва особової категорії">
                            <ItemTemplate>
                                <asp:Label runat="server" ID="Label3" Text='<%# (Convert.ToString(Eval("CATEGORY_NAME")).Length > 130) ? (Convert.ToString(Eval("CATEGORY_NAME")).Substring(0, 130) + " ..."):(Eval("CATEGORY_NAME")) %>'
                                    ToolTip='<%# Eval("CATEGORY_NAME") %>'></asp:Label>
                            </ItemTemplate>
                        </asp:TemplateField>
                    </columns>
                    </bars:BarsGridViewEx>
