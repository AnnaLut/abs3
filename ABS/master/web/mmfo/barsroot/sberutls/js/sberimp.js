...
function skSelect(target, dk_control, nlsa_control, nlsb_control) {
    ...
    var result = window.showModalDialog('dialog.aspx?type=metatab&tabname=sk&role=BASIC_INFO&tail=" ' + ' sk like \'' + target.value + '%\' and ' + sql + ' __bktOp__d_close is null or d_close > gl.bd__bktCl__ order by sk"', "", "dialogWidth:600px;center:yes;edge:sunken;help:no;status:no;");
...
$(document).ready(function () {
    ...
    if ($("input[name='fv_SKTextBox_text']").val()=="")
        if (getUrlVars(document.URL)['skp']) {
            $("input[name='fv_SKTextBox_text']").val(getUrlVars(document.URL)['skp']);
        }
