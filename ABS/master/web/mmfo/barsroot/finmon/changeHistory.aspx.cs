...
public partial class finmon_changeHistory : System.Web.UI.Page
{
    ...
    private void FillGrid()
    {
        ...
        changeHistoryDataSource.SelectCommand = @"SELECT f.mod_date, f.mod_type, t.mod_name as NAME, f.user_id, s.FIO as user_name, substr(f.mod_value, 1, 1000) as OLD_VALUE
                                                    FROM finmon_que_modification f, finmon_que_modtype t, staff$base s
                                                    WHERE f.mod_type = t.mod_type AND f.id =:p_id AND f.USER_ID = s.ID";
    }
}