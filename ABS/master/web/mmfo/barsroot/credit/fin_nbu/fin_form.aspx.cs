...
public partial class credit_fin_form : Bars.BarsPage
{
        ...
        try
        {
            ...
            gvMain.DataSource = SQL_SELECT_dataset(@"select idf, fm, fdat, ord, pob, name, kod, s, okpo
                                                from (SELECT  f.IDF, ' ' FM,
                                                       ...
                                                       NVL (f.ord, 0) AS ord,
                                                       f.pob,
                                                       case when f.pob > 2 then f.name else '(' || f.kod || ')  ' || f.name end name,
                                                       f.kod,
                                                       nvl(FIN_RNK.s,0) S,
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Request["frm"] != null)
        {
            ...
            if (Request["frm"] == "2" || Convert.ToDecimal(Request["rnk"]) <= 0) 
            {
                tb_kol.Visible = false;
                lb_kol.Visible = false;
                ReqFVal1.Visible = false;
                ReqEVal1.Visible = false;
                tb_kved.Visible = true;
                lb_kved.Visible = true;
                Reqtb_kved.Visible = true;
                btKVED.Visible = true;
                Lb_n_ved.Visible = true;
        ...
        if (!IsPostBack)
            if (Convert.ToString(Request["dat"]).Substring(0, 5) == "01/01")
            {
    ...
    protected void lock_form()
    {
        ...
        try
        {
            ...
            if (rdr.Read())
            {
                ...
                {
                    get_k140(); //Розраховуємо і зберігаемо показник  К140 заявка http://10.10.11.3:11000/browse/COBUSUPABS-6154
...
//            Response.Write(par_read);
    ...
    protected void Tb_kved()
    {
        ...
        try
        {
            ...
            if (!rdr.Read())
            {
                ...
                {
                    Lb_n_ved.Text = "Заповніть обсяг реалізованої продукції";
                    Lb_n_ved.ForeColor = System.Drawing.Color.Blue;
                    par_kved = "0";
                }
            }
            else
    ...
    protected void btKVED_Click(object sender, EventArgs e)
    {
        ScriptManager.RegisterStartupScript(this, this.GetType(), "send_success", "location.replace('/barsroot/credit/fin_nbu/fin_kved.aspx?okpo=" + tbOKPO.Text + "&rnk=" + Convert.ToString(Request["rnk"]) + "&dat=" + "01/01/" + tbFDat.Text.Substring(6, 4) + "&datp="  + tbFDat.Text + "', 'aaaa', 'width=1050, height=400, resizable=yes,scrollbars=yes,status=yes" + "')", true);
        //ScriptManager.RegisterStartupScript(this, this.GetType(), "send_success", "window.open('/barsroot/credit/fin_nbu/fin_kved.aspx?OKPO=" + tbOKPO.Text + "&RNK=" + Convert.ToString(Request["rnk"]) + "&DAT=" + tbFDat.Text + "', 'aaaa', 'width=1050, height=400, resizable=yes,scrollbars=yes,status=yes" + "')", true);
    }

    protected void get_k140()
    {
        try
        {
            CultureInfo cinfo = CultureInfo.CreateSpecificCulture("en-GB");
                ...
                if (tbFrm.Value == 2)
                {
                    SQL_NONQUERY(@"declare
                                  ...
                                  begin
                                    ...
                                    if trunc(l_dat,'YYYY') = l_dat AND trunc(sysdate,'YYYY') =  trunc(l_dat,'YYYY')  then 

                                    select avg(rate_o)/100
                                      into l_cur
                                      from cur_rates$base
                                     where kv = 978
                                       and branch like '/______/' 
                                       and vdate between trunc(l_dat-1,'YYYY') and trunc(l_dat,'YYYY');
                                     
                                     l_2000 := fin_nbu.ZN_F2(KOD_ => 2000, GR_ => 3, DAT_ => l_dat, OKPO_ => l_okpo)*1000/l_cur;
                                     
                                     case 
                                        when l_2000> 50000000. then l_k140 := 1;
                                        when l_2000> 10000000. then l_k140 := 2;
                                        when l_2000> 2000000.  then l_k140 := 3;
                                        when l_2000> 500000.   then l_k140 := 4;
                                        when l_2000> 50000.    then l_k140 := 5;
                                        when l_2000> 0.        then l_k140 := 6;
                                                               else l_k140 := 9;
                                     end case; 

                                    -- проставляємо к140 customerw K140
                                       kl.setCustomerElement(l_rnk, 'K140', l_k140, 0);
