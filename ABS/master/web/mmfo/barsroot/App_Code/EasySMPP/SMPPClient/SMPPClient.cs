    ...
    public class SMPPClient
    {
        ...
        private void logMessage(int logLevel, string pMessage)
        {
            try
            {
                ...
                if (logLevel >= LogLevels.LogErrors)
                {
        ...
        private void connectCallback(IAsyncResult ar)
        {
            ...
            {
                string socketError = string.Empty;
                if (ex is SocketException)
                    socketError = (ex as SocketException).SocketErrorCode.ToString();
                logMessage(LogLevels.LogExceptions, "connectCallback | [" + socketError + "]" + ex.ToString());
                tryToReconnect();
            }
        }//connectCallback
        ...
        private void receiveCallback(IAsyncResult ar)
        {
            try
            {
                ...
                {
                    logMessage(LogLevels.LogSteps | LogLevels.LogWarnings, "IsIncoming network buffer from SMSC is empty.");
            ...
            catch (Exception ex)
            {
                ...
                if (ex is SocketException)
                    socketError = (ex as SocketException).SocketErrorCode.ToString();
                logMessage(LogLevels.LogExceptions, "receiveCallback | [" + socketError + "]" + ex.ToString());
                unBind();
            }
