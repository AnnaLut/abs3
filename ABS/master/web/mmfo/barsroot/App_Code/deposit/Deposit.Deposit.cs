...
public class Deposit
{
    ...
    private void WriteContract(OracleConnection connect)
    {
        ...
        cmdCreateContract.CommandText =
           ...
           " :DptID, gl.bdate, :p_duration, :p_duration_days, :p_wb);   " +
           " end; ";
        ...
        cmdCreateContract.Parameters.Add("p_wb", OracleDbType.Char, this.wb, ParameterDirection.Input);
        // Выполняем
    ...
    public void ReadFromDatabaseExt(bool active, bool other, bool all)
    {
        ...
        try
        {
            DBLogger.Info("Чтение из базы депозитного договора №" + this.ID.ToString() + " началось.",
                "deposit");
            DBLogger.Info("active" + active + "other" + other + "all" + all + this.ID.ToString() + " началось.",
                "deposit");
            ...
            //DBLogger.Info(cmdDepositInfo.CommandText + this.ID);
    ...
    public bool AddPaymentEnabled()
    {
        ...
        try
        {
            ...
            cmd.CommandText = @"select nvl(disable_add,0) from dpt_vidd dv, DPT_DEPOSIT d
                where dv.vidd = d.vidd and d.deposit_id = :dpt_id";
