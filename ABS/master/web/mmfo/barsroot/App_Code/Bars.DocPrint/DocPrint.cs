...
namespace Bars.DocPrint
{
    public class cDocPrint
    {
            ...
            if (s.StartsWith("[REF="))
            {
                ...
                if (rez.Length > 85)
                {
                    ...
                    foreach (var word in rez.Split(new[] { ' ' }))
                    {
                        ...
                        var lword = word;
                        if(word.Length > nTextWidth - 1)
                            lword = lword.Insert(nTextWidth - 1, Environment.NewLine);
                        if (word.Length > (nTextWidth - 1)  * 2)
                            lword = lword.Insert((nTextWidth) * 2, Environment.NewLine);
                        tmpRez += lword + " ";
                    }
                    rez = tmpRez;
                }
        ...
        public bool DocPrintTest(decimal Ref)
        {
            OracleCommand cmd = this.con.CreateCommand();
        ...
        public cDocPrint(OracleConnection con, decimal Ref, string TemplPath) :
            this(con, Ref, TemplPath, GetRandomFileName(Ref.ToString()))
        {
        }
        public cDocPrint(OracleConnection con, decimal Ref, string TemplPath, string FnDoc) :
            this(Ref, TemplPath, FnDoc, "")
        {
        }
        public cDocPrint(decimal Ref, string TemplPath) :
            this(Ref, TemplPath, GetRandomFileName(Ref.ToString()), "")
        {
        }
        public cDocPrint(decimal Ref, string TemplPath, string DBLink) :
            this(Ref, TemplPath, GetRandomFileName(Ref.ToString()), DBLink)
        {
        }
        public cDocPrint(OracleConnection con, decimal Ref, string TemplPath, bool printTrnModel)
        {
            this.InRef = Ref;
        ...
        public cDocPrint(decimal Ref, string TemplPath, string FnDoc, string DBLink)
        {
