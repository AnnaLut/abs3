...
namespace BarsWeb.Controllers
{
    ...
    public class BoardController : ApplicationController
    {
        ...
        public ActionResult Add(string title, string text)
        {
            ...
            string sql = "begin bms.set_bars_board(:p_msg_title, :p_msg_text, bars.user_name); end;";
            object[] parameters =
            {
                new OracleParameter("p_msg_title", OracleDbType.Varchar2).Value = title,
                new OracleParameter("p_msg_text", OracleDbType.Varchar2).Value = text
            };
        ...
        public ActionResult Edit(decimal id, string title, string text)
        {
            ...
            string sql = "begin bms.set_bars_board(:p_msg_title, :p_msg_text, bars.user_name, :p_id); end;";
            object[] parameters =
            {
        ...
        public ActionResult Delete(decimal[] id)
        {
            string status = "ok";
            string message = "Зміни збережені";
            string sql = "begin bms.del_bars_board(:p_id); end;";
            try
            {
                using (_entities = new EntitiesBarsCore().NewEntity())
                {
