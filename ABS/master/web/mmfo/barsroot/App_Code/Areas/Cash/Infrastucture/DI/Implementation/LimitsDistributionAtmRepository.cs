...
namespace BarsWeb.Areas.Cash.Infrastructure.Repository.DI.Implementation
{
    public class LimitsDistributionAtmRepository : ILimitsDistributionAtmRepository
    {
        ...
        public UpdateDbStatus UploadFile(DateTime date, byte[] file)
        {
            ...
            _entities.ExecuteStoreCommand(@"begin clim_lim.load_atmxls(:p_acc_id ,:p_startdate, :p_status,:p_message, :p_idsession);end;", parameters);
        ...
        public List<AtmProtocolModel> GetAtmProtocolData(decimal sessionId)
        {
            var sql = @"select ID_ATM, LIM_MAX, NAME_RU, NAME_ACC, STATUS, TEXT as ERROR
                                from V_CLIM_LOG_LOADXLS_ATMLIM 
            ...
            {
                return connection.Query<AtmProtocolModel>(sql, p).ToList();
            }
        }
    }
}
