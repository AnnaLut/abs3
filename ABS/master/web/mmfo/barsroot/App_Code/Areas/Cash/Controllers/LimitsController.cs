...
namespace BarsWeb.Areas.Cash.Controllers
{
    ...
    public class LimitsController : ApplicationController
    {
            ...
            string limType = null)
        {
            ...
            List<AccountLimit> readyList = new List<AccountLimit>();
            foreach (AccountLimit item in listToDataSource.Data)
            {
                readyList.Add(item);
            }

            var excelRows = readyList.Select(i => new AccountLimitExcel
            {
                Mfo = i.Mfo,
                MfoName = i.MfoName,
                Branch = i.Branch,
