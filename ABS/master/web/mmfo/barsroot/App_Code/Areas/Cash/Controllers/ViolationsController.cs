...
namespace BarsWeb.Areas.Cash.Controllers
{
    ...
    public class ViolationsController : ApplicationController
    {
        ...
        //        var list = viewRecords.ToDataSourceResult(request);
            ...
            decimal kv)
        {
            ...
            var list = rows.ToDataSourceResult(request);
            return Json(list, JsonRequestBehavior.AllowGet);
        }
            ...
            List<string> mfoList = null)
        {
            ...
            var reData = row.ToDataSourceResult(request).Data;
            IQueryable<V_CLIM_ACCOUNT_LIMIT_MD> qRes = reData.Cast<V_CLIM_ACCOUNT_LIMIT_MD>().AsQueryable();

            var header = "Звіт про порушення в розрізі рахунків станом на " + DateTime.Now.ToShortDateString();
            var exel = new ExcelHelpers<V_CLIM_ACCOUNT_LIMIT_MD>(qRes, true, header);
        ...
        //    //    dbRecords = dbRecords.Where(x => mfoList.Contains(x.KF));
        //    //}
        //    var list = dbRecords.ToDataSourceResult(request);
        //    return Json(list, JsonRequestBehavior.AllowGet);
        //}
        ...
        public ActionResult GetCashViolationsBranchKvConsolidateFile([DataSourceRequest] DataSourceRequest request,
            List<string> mfoList = null)
        {
            ...
            var reData = excelData.ToDataSourceResult(request).Data;
            IQueryable<V_CLIM_CASH_VIOLATION_EXCEL> qRes = reData.Cast<V_CLIM_CASH_VIOLATION_EXCEL>().AsQueryable();

            var header = "Звіт про порушення в розрізі рахунків станом на " + DateTime.Now.ToShortDateString();
            var exel = new ExcelHelpers<V_CLIM_CASH_VIOLATION_EXCEL>(qRes, true, header);
        ...
        public ActionResult GetCashBranchViolationsFile(
            [DataSourceRequest] DataSourceRequest request,
            List<string> mfoList = null,
            string datefr = null, 
            string datet= null)
        {
            ...
            IEnumerable<V_CLIM_CASHBRANCH_VIOLATION> dbRecords =
                        _limitRepository.GetCashBranchViolations();

            var reData = dbRecords.ToDataSourceResult(request).Data;
            IEnumerable<V_CLIM_CASHBRANCH_VIOLATION> qRes = reData.Cast<V_CLIM_CASHBRANCH_VIOLATION>().AsQueryable();

            if (mfoList != null)
            {
                qRes = dbRecords.Where(x => mfoList.Contains(x.KF));
            }
            if (datefr != null && datet != null)
            {
                DateTime datefrom = Convert.ToDateTime(datefr);
                DateTime dateto = Convert.ToDateTime(datet);
                qRes = dbRecords.Where(x => x.BDATE >= datefrom && x.BDATE <= dateto);
            }
            else
            {
                if (datefr == null)
                {
                    DateTime dateto = Convert.ToDateTime(datet);
                    qRes = dbRecords.Where(x => x.BDATE <= dateto);
                }
                else if (datet == null)
                {
                    DateTime datefrom = Convert.ToDateTime(datefr);
                    qRes = dbRecords.Where(x => x.BDATE >= datefrom);
                }
            }
            var header = "Звіт про порушення по каса - ТВБВ на " + DateTime.Now;

            var excelData = qRes.Select(i => new CashBranchViolation
            {
                Colour = i.COLOUR,
                Mfo = i.KF,
                RuName = i.RU_NAME,
        ...
        //        }
        //    }
        //    var list = new[] { limit }.ToDataSourceResult(request, ModelState);
        //    return Json(list);
        //}

        public ActionResult GetCashViolGrpMfoKv(
            [DataSourceRequest] DataSourceRequest request,
            List<string> mfoList = null)
        {
            ...
            List<string> mfoList = null)
        {
            ...
            if (mfoList != null)
            {
                    dbRecords = dbRecords = dbRecords.Where(x => mfoList.Contains(x.KF));
            }
            ...
            var reData = rows.ToDataSourceResult(request).Data;
            IQueryable<V_CLIM_MFOLIM_MD> qRes = reData.Cast<V_CLIM_MFOLIM_MD>().AsQueryable();

            var header = "Порушення Каса - МФО станом на " + DateTime.Now.ToShortDateString();
            var exel = new ExcelHelpers<V_CLIM_MFOLIM_MD>(qRes, true, header);
