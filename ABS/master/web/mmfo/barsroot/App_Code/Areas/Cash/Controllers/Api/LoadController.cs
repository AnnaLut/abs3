...
namespace BarsWeb.Areas.Cash.Controllers.Api
{
    ...
    {
        private readonly IRegionAccountRepository _regionAccountRepository;// = new AccountRepository(new CashModel());

        public LoadController(IRegionAccountRepository repository)
        {
            _regionAccountRepository = repository;
        }
        ...
        public BankDatesResponse BankDates() // mfo
        {
            ...
            try
            {
                IQueryable<V_CLIM_FDAT> dbRecords = _regionAccountRepository.GetBankDates();
