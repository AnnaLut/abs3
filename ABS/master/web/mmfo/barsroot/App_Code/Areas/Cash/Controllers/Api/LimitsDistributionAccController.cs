...
namespace BarsWeb.Areas.Cash.Controllers.Api
{
    ...
    public class LimitsDistributionAccController : ApiController
    {
        ...
        public HttpResponseMessage Get([ModelBinder(typeof(WebApiDataSourceRequestModelBinder))] DataSourceRequest request, string date, string type)
        {
            ...
            if (type.ToUpper() == "XLSUPLOAD")
            {
                ...
                var list = limDist.ToDataSourceResult(request);
                List<LimitsDistributionAccExcelForUpload> resultList = new List<LimitsDistributionAccExcelForUpload>();
                foreach (LimitsDistributionAccExcelForUpload l in list.Data)
                {
                    resultList.Add(l);
                }
                var exel = new ExcelHelpers<LimitsDistributionAccExcelForUpload>(resultList, true);
