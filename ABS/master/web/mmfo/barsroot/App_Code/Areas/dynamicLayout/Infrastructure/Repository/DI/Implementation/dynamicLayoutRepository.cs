...
namespace BarsWeb.Areas.dynamicLayout.Infrastructure.DI.Implementation
{
    public class dynamicLayoutRepository : IdynamicLayoutRepository
    {
        readonly dynamicLayoutModel _dynamicLayout;
        ...
        public dynamicLayoutRepository(IKendoSqlTransformer sqlTransformer, IKendoSqlCounter kendoSqlCounter, IParamsRepository globalData)
        {
            _dynamicLayout = new dynamicLayoutModel(EntitiesConnection.ConnectionString("dynamicLayoutModel", "dynamicLayout"));
        ...
        public void CreateDynamicLayout(decimal? pMode, decimal? pDk, string pNls, string pBs, string pOb, decimal? pGrp)
        {
            using (var connection = OraConnector.Handler.UserConnection)
            {
                var p = new DynamicParameters();
        ...
        public void CalculateDynamicLayout(decimal? pId)
        {
            using (var connection = OraConnector.Handler.UserConnection)
            {
                var p = new DynamicParameters();
        ...
        public IEnumerable<T> SearchGlobal<T>(DataSourceRequest request, BarsSql searchQuery)
        {
            ...
            var item = _dynamicLayout.ExecuteStoreQuery<T>(query.SqlText, query.SqlParams);
            return item;
        }
        public decimal CountGlobal(DataSourceRequest request, BarsSql searchQuery)
        {
            ...
            ObjectResult<decimal> res = _dynamicLayout.ExecuteStoreQuery<decimal>(query.SqlText, query.SqlParams);
            decimal count = res.Single();
            return count;
        }
        public IEnumerable<T> ExecuteStoreQuery<T>(BarsSql searchQuery)
        {
            return _dynamicLayout.ExecuteStoreQuery<T>(searchQuery.SqlText, searchQuery.SqlParams);
        }

        public int ExecuteStoreCommand(string commandText, params object[] parameters)
        {
            return _dynamicLayout.ExecuteStoreCommand(commandText, parameters);
        }
