...
namespace BarsWeb.Areas.MetaDataAdmin.Infrastructure.Repository.Helpers
{
    ...
    {
        public static string PushRowsFilterList(List<BarsWeb.Areas.Ndi.Infrastructure.Repository.Helpers.FilterRowInfo> filterRowList, string filterStructure, int tableId, string filterName, int saveFilter, string whereClause)
        {
            //var functionInfoList = new FilterInfoList();
            //functionInfoList.FilterInfoArray = filterRowList.ToArray();

            using (var context = new BarsWeb.Areas.Ndi.Models.OracleDbModel())
            {
                try
                {
