...
using BarsWeb.Areas.MetaDataAdmin.Infrastructure.Repository.DI.Implementation;
using System;
using System.Data;
using BarsWeb.Areas.MetaDataAdmin.Models.DbModels;
using BarsWeb.Areas.MetaDataAdmin.Infrastructure.Helpers.BarsWeb.Areas.MetaDataAdmin.Infrastructure.Helpers;
...
namespace BarsWeb.Areas.MetaDataAdmin.Infrastructure
{
    ...
    public static class SqlStatementParamsParser
    {
        ...
        public static string GetMetaTypeTypeName(string typeAsString)
        {
            switch (typeAsString)
            {
                case "System.Double":
        ...
        public static void BuilFunctionParams(List<CallFunctionMetaInfo> callFunctions, MetaTable tableInfo = null)
        {
            foreach (var func in callFunctions)
            {
                ...
                if (func.PROC_EXEC == "SYNC_COLUMNS")
                {
                    AdminFuncParamsParser.BuildSyncParamsFuncInfo(func, paramsInfo);
                }
                ...
                if (!string.IsNullOrEmpty(func.GenerateParams))
                {
                    func.GenerateParamsInfo = GetSqlFuncCallParamsDescription<GenerateParamInfo>(func.PROC_NAME, func.GenerateParams);
                    if (func.GenerateParamsInfo != null && func.GenerateParamsInfo.Count() > 0)
                    {
                        if (paramsInfo != null && paramsInfo.Count() > 0)
                            paramsInfo.RemoveAll(x => func.GenerateParamsInfo.Find(y => y.ColName == x.ColName) != null);
                ...
                {
                    List<ComplexParams> complexParams = GetSqlFuncCallParamsDescription<ComplexParams>(func.PROC_NAME, func.ConvertParams);
                    func.ConvertParamsInfo = complexParams.Where(x => x.Kind == "CONVERT_PARAMS" && x is ConvertParams).Select(c => c as ConvertParams).ToList();
                    func.ConvertParamsInfo.ForEach(x =>
                        {
                            func.RowParamsNames.ForEach(t => { if (t == x.ColName && x.GetFrom == "GRID_COLUMN") t = x.SrcFrom; });
                            paramsInfo.ForEach(b => { if (b.IsInput == false && b.ColName == x.ColName && x.GetFrom == "GRID_COLUMN") b.ColName = x.SrcFrom; });
                        }
                       );
                }

                if (!string.IsNullOrEmpty(func.GetParams))
                {
                    func.GetParamsInfo = GetSqlFuncCallParamsDescription<GetParamsInfo>(func.PROC_NAME, func.GetParams);
                    if (func.GetParamsInfo != null && func.GetParamsInfo.Count() > 0)
                    {
                        func.HasFileResult = true;
                    }
                }
        ...
        public static void ExecOptionsFromDictionary<T>(T paramMetaInfo, Dictionary<string, string> options) where T : ParamMetaInfo, new()
        {
            ...
            else if (paramMetaInfo is GenerateParamInfo)
                AddOptionsFromDictionary(paramMetaInfo as GenerateParamInfo, options);
            else if (paramMetaInfo is GetFileFromTable)
                AddOptionsFromDictionary(paramMetaInfo as GetFileFromTable, options);
            else if (paramMetaInfo is GetParamsInfo)
                AddOptionsFromDictionary(paramMetaInfo as GetParamsInfo, options);
            ...
            foreach (var option in options)
            {
                switch (option.Key)
                {
                    case "GET_FROM":
                        ...
                        break;
                    case "SRC_NAME_FROM":
                        paramMetaInfo.SrcFrom = option.Value;
                        break;
                    default:
                        break;
                }
        ...
        private static void AddOptionsFromDictionary(GenerateParamInfo paramMetaInfo, Dictionary<string, string> options)
        {
            foreach (var option in options)
            {
                switch (option.Key)
                {
                    case "SRC_VALUE":
                        paramMetaInfo.SrcValue = option.Value;
                        break;
                    case "GET_FROM":
                        paramMetaInfo.GetFrom = option.Value;
                        break;
                    case "POSTFIX":
                        paramMetaInfo.PostFix = option.Value;
                        break;
                }
            }
        }
        ...
        private static void AddOptionsFromDictionary(GetParamsInfo paramMetaInfo, Dictionary<string, string> options)
        {
            ...
            foreach (var option in options)
            {
                switch (option.Key)
                {
                    ...
                    case "SRC_FILE_NAME":
                        ...
                        break;
                    case "SRC_FILE":
                        paramMetaInfo.SrcFile = option.Value;
                        break;
                    case "GET_FROM":
                        paramMetaInfo.GetFrom = option.Value;
                        break;
        ...
        private static void AddOptionsFromDictionary(GetFileFromTable paramMetaInfo, Dictionary<string, string> options)
        {
            foreach (var option in options)
            {
                switch (option.Key)
                {
                    ...
                    case "SRC_FILE_NAME":
                        paramMetaInfo.SrcFileName = option.Value;
                        break;
                    case "COL_NAME_GET_FROM":
                        paramMetaInfo.ColNameGetFrom = option.Value;
                        break;
                    case "TABLE_NAME_FROM":
                        paramMetaInfo.TableNameFrom = option.Value;
                        break;
                    case "EXEC":
                        paramMetaInfo.ExecSelect = option.Value;
                        break;
                    case "TYPE":
                        paramMetaInfo.ColType = option.Value;
                        break;
        ...
        public static string GetFileNameParam(List<GetParamsInfo> outparams, List<FieldProperties> funcParams, bool withExtention)
        {
            string fileName = string.Empty;
            GetParamsInfo getFileParam = outparams.FirstOrDefault(x => x.ColType == "CLOB" || x.Kind == "GET_FILE");
        ...
        public static void BuildFunction(CallFunctionMetaInfo function)
        {
            ...
            if (!string.IsNullOrEmpty(function.WEB_FORM_NAME) && function.WEB_FORM_NAME.Contains("sPar="))
            {
                ...
                if (!string.IsNullOrEmpty(function.WEB_FORM_NAME) && function.WEB_FORM_NAME.Contains("/"))
            {
