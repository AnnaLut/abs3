...
namespace BarsWeb.Areas.CRSOUR.Controllers
{
    ...
    public class DealMonitorController : ApplicationController
    {
        ...
        /// <summary>
        /// Угода кредитних ресурсів
        /// </summary>
        /// <returns></returns>
        public ViewResult CreditAgreement()
        {
            return View();
        }
        ...
        {
            IQueryable<ViddList> session = _dealsRepository.GetVidd();
            return Json(session, JsonRequestBehavior.AllowGet);
        }

        public ActionResult GetCurrency()
        {
            IQueryable<CurrencyList> session = _dealsRepository.GetCurrency();
            return Json(session, JsonRequestBehavior.AllowGet);
        }

        public ActionResult GetBasey()
        {
            IQueryable<BaseyList> session = _dealsRepository.GetBasey();
            return Json(session, JsonRequestBehavior.AllowGet);
        }

        public ActionResult GetDealNls(decimal productId, decimal currencyId)
        {
            DealNls session = _dealsRepository.GetDealNls(productId, currencyId);
            return Json(session, JsonRequestBehavior.AllowGet);
        }

        public decimal GetRateSum(RateParams param)
        {
            decimal session = _dealsRepository.GetRateSum(param);
            return session;
        }

        public ActionResult GetMfoName(string mfo)
        {
            MfoName session = _dealsRepository.GetMfoName(mfo);
            return Json(session, JsonRequestBehavior.AllowGet);
        }

        public ActionResult GetMfo(decimal rnk)
        {
            string session = _dealsRepository.GetMfo(rnk);
            return Json(session, JsonRequestBehavior.AllowGet);
        }

        public ActionResult GetDeal(decimal nd)
        {
            DealParams session = _dealsRepository.GetDeal(nd);
            return Json(session, JsonRequestBehavior.AllowGet);
        }

        public ActionResult SaveDeal(SaveDeal param)
        {
            string session = _dealsRepository.SaveDeal(param);
            return Json(session, JsonRequestBehavior.AllowGet);
        }

        public ActionResult UpdateDeal(decimal dealId, DateTime expiryDate, decimal dealAmount, decimal interestRate, string partnerMainAccount, string partnerInterestAccount)
        {
            string session = _dealsRepository.UpdateDeal(dealId, expiryDate, dealAmount, interestRate, partnerMainAccount, partnerInterestAccount);
            return Json(session, JsonRequestBehavior.AllowGet);
        }
    }
}