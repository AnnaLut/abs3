...
namespace BarsWeb.Areas.Pfu.Infrastructure.Repository.DI.Implementation
{
    ...
    public class SqlCreator
    {
        ...
        public static BarsSql KvitDeath(string state)
        {
            return new BarsSql()
            {
                SqlText = @"select * from pfu.V_PFU_DEATH_KVIT t where t.state = :P_STATE",
                SqlParams = new object[] 
                {
                    new OracleParameter("P_STATE", OracleDbType.Varchar2) { Value = state },
                }
            };
        }
        public static BarsSql KvitLackMoveRecords(string id)
        {
            return new BarsSql()
            {
                SqlText = string.Format(@"select * from pfu.V_PFU_NO_TURNOVER where id_file = {0}", id)
            };
        }
        public static BarsSql KvitLackMove(string state)
        {
            const string add_sql = " t where t.state = :P_STATE";
            var barssql = new BarsSql();
            barssql.SqlText = state != "" ? @"select * from pfu.V_PFU_NO_TURNOVER_LIST" + add_sql : @"select * from pfu.V_PFU_NO_TURNOVER_LIST";
            barssql.SqlParams = state != "" ? new object[] { new OracleParameter("P_STATE", OracleDbType.Varchar2) { Value = state } } : null;
            return barssql;
        }
