...
{
    /// <summary>
    /// Summary description for CdmLegalRepositoryV2
    /// </summary>
    public class CdmLegalRepositoryV2 : CdmLegalRepository
    {
        protected const PersonType LegalPerson = (PersonType) 1;
        ...
        public override int SavePersonCardsFast(RequestFromEbkV2 request, int allowedCardsPerRequest)
        {
            ...
            if (request.Entries.Count <= allowedCardsPerRequest) return InternalSavePersonCards(request);
            var errorMsg =
                string.Format(
        ...
        public override decimal PackAndSendClientCards(int? cardsCount, int packSize, string kf)
        {
            ...
            var apiUrl = ConfigurationManager.AppSettings["ebk.ApiUriLegal"] +
                         ConfigurationManager.AppSettings["ebk.LoadCardsMethodLegal.v2"];
            ...
            try
            {
                ...
                List<LegalPerson> packBody = packPlaneBody.Select(MapLegalData).ToList();
                ...
                if (HttpStatusCode.OK == httpResponse.StatusCode)
                {
                    ...
                    packBody.ForEach(card => QueueRemoveCard(card.Rnk, card.Kf));
        ...
        public override ActionStatus PackAndSendSingleCard(decimal rnk)
        {
            ...
            var apiUrl = ConfigurationManager.AppSettings["ebk.ApiUriLegal"] +
                         ConfigurationManager.AppSettings["ebk.LoadCardMethodLegal.v2"];

            string err;
            try
            {
                LegalData plainCard = QueueGetCard(rnk);
                ...
                LegalPerson card = MapLegalData(plainCard);

                card.RelatedPersons = ExtractRelatedPersonList(rnk);

                decimal packNum = GetNextPackNumber();

                var simpleCard = new LegalCard(card.Kf, packNum.ToString(Culture),
                    HomeRepo.GetUserParam().USER_FULLNAME, card);
                ...
                if (HttpStatusCode.OK == httpResponse.StatusCode)
                {
                    ...
                    else if (ResponseStatus.ERROR.ToString() == ebkResponse.Status)
                    {
                        ...
                        {
                            result.Status = ActionStatusCode.Error;
                            result.Message = string.Format("Сервіс відхилив запит. Причина: {0}", ebkResponse.Message);
                        }
            ...
            catch (Exception e)
            {
                err = string.Format("{0} Невідома помилка надсилання картки РНК={2} онлайн:  {1}",
                    _logMessagePrefix, (e.InnerException != null ? e.InnerException.Message : e.Message), rnk);
                Logger.Error(err);
        ...
        public override string SendCloseCard(string kf, decimal rnk, string dateOff)
        {
            ...
            {
                kf = _entities
                    .ExecuteStoreQuery<string>("SELECT kf FROM customer where rnk = " + Convert.ToString(rnk, Culture),
                        null)
                    .FirstOrDefault();
            ...
            var serviceUrl = ConfigurationManager.AppSettings["ebk.ApiUriLegal"] +
                             ConfigurationManager.AppSettings["ebk.CloseMethodLegal.v2"] + "?kf=" + kf + "&rnk=" + rnk +
            ...
            try
            {
                HttpWebResponse httpResponse = MakeRequest(serviceUrl, new byte[0], "PUT");
        ...
        public override string SynchronizeCard(string kf, decimal rnk)
        {
            ...
            string serviceUrl = ConfigurationManager.AppSettings["ebk.ApiUriLegal"] +
                                ConfigurationManager.AppSettings["ebk.SyncCardMethodLegal.v2"] + "?kf=" + kf + "&rnk=" +
                                rnk;
            var status = "ERROR";
            string error;
            try
            {
                ...
                if (HttpStatusCode.OK == httpResponse.StatusCode)
                {
                    ...
                    {
                        error = string.Format(
                            "{0} Помилка синхронізації картки клієнта ЮО з віддаленим сервісом ЄБК. <br />Помилка: {1}",
                            _logMessagePrefix, ebkResponse.Message);
                        Logger.Error(error);
                    }
                    else
            ...
            {
                error = string.Format("{0} Невідома помилка синхронізації картки ЮО РНК={2}  онлайн:  {1}",
                    _logMessagePrefix,
                    (ex.InnerException != null ? ex.InnerException.Message : ex.Message), rnk);
                Logger.Error(error);
                return status;
        ...
        private LegalData QueueGetCard(decimal rnk)
        {
            var sqlParams = new object[]
            {
                new OracleParameter("p_rnk", OracleDbType.Decimal) {Value = rnk}
            };
            return _entities.ExecuteStoreQuery<LegalData>(
                    "select * from V_EBKC_QUEUE_UPDCARD_LEGAL where rnk = bars.ebkc_wforms_utl.cut_rnk(:p_rnk)",
                    sqlParams)
                .SingleOrDefault();
        }

        private List<LegalData> QueueGetCards(int packSize, string kf)
        {
            var sqlParams = new object[]
            {
            ...
            return _entities.ExecuteStoreQuery<LegalData>(
                    "select * from V_EBKC_QUEUE_UPDCARD_LEGAL where rownum <= :p_Size and kf = :p_kf",
                    sqlParams)
                .ToList();
        }
        ...
        private void QueuePutCard(decimal rnk)
        {
            ...
            _entities.ExecuteStoreCommand(
                "begin bars.ebkc_wforms_utl.add_rnk_queue(:p_rnk); end;",
                sqlParams);
        }
