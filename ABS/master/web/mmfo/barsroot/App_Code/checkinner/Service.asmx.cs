    ...
    public class Service : Bars.BarsWebService
    {
        ...
        public object[] GetData(string[] data)
        {
            ...
            try
            {
                ...
                object[] MainResult = BindTableWithNewFilter(@" /*+ leading(ref_que oper) */ a.COLOR1,	
																a.COLOR2,	
																...
																a.NEXTVISAGRP,
                                                                a.prty,
                                                                a.sos", Params.Table + " a", filter, data);
                ...
                DataSet dsAggs = GetFullDataSetForTable("/*+ leading(ref_que oper) */ count(1) as cnt, sum(a.s_) as s_", Params.Table + " a", filter, data);
        ...
        [WebMethod(EnableSession = true)]
        public decimal[] GetAccountBalance(decimal Ref) {
            ...
            try
            {
                ...
                cmd.CommandText = "select a.acc, t.denom, a.ostc/t.denom, a.ostb/t.denom, a.ostf/t.denom  from oper o, accounts a, tabval$global t where o.ref=:p_ref and a.nls=o.nlsa and a.kv=o.kv and t.kv=a.kv";
                OracleDataReader rdr = cmd.ExecuteReader();
                if (rdr.Read()) {
                    result[0] = rdr.GetDecimal(0);
                    result[1] = rdr.GetDecimal(1);
                    result[2] = rdr.GetDecimal(2);
                    result[3] = rdr.GetDecimal(3);
                }
                rdr.Dispose();
            }
            finally {
        ...
        [WebMethod(EnableSession = true)]
        public VisaResult PutVisas(String XmlData, String Type, DocumentInfo[] StpDocs)
        {
            ...
            {
                // СТП
                if (StpDocs != null && StpDocs.Length > 0)
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    for (int i = 0; i < StpDocs.Length; i++)
                    {
                        if (StpDocs[i].Edited)
                        {
                            cmd.Parameters.Clear();
                            cmd.Parameters.Add("p_ref", OracleDbType.Decimal, StpDocs[i].Ref, ParameterDirection.Input);
                            cmd.Parameters.Add("p_prty", OracleDbType.Decimal, StpDocs[i].Priority, ParameterDirection.Input);
                            cmd.CommandText = "chk.put_prty";
                            cmd.ExecuteNonQuery();
                        }
                    }
                }
        ...
        [WebMethod(EnableSession = true)]
        public VisaResult PutVisasExt(String XmlData, String Type, DocumentInfo[] StpDocs)
        {
            VisaResult Result = new VisaResult();
            String XmlDataDecoded = HttpUtility.UrlDecode(XmlData);
