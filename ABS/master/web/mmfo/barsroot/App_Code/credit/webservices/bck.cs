...
public class pvbki : bki
{
    ...
    private String _reportEndpointName;
    private String _reportEndpointKeyPath;
    ...
    protected override void loadConfig()
    {
        ...
        _reportEndpointName = ConfigurationManager.AppSettings["pvbki.ReportEndpointName"];
        _reportEndpointKeyPath = ConfigurationManager.AppSettings["pvbki.ReportEndpointKeyPath"];
    ...
    {
        byte[] res = null;
        FileStream fs = new FileStream(HttpContext.Current.Server.MapPath(_reportEndpointKeyPath), FileMode.Open);
        try
        {
    ...
    public byte[] GetReport(PvbkiClientType ClientType, String Identification)
    {
        ...
        CigReports tmp = new CigReports();
        CigWsHeader header = new CigWsHeader();

        tmp.Url = _reportServiceUrl;
        header.UserName = _reportServiceUserName;
        header.Password = _reportServicePassword;
        header.Version = "1_0";

        tmp.CigWsHeaderValue = header;

        XmlNode resp = tmp.GetReport(Identification, ((int)ClientType).ToString(), report_id);

        String buffer = resp.InnerXml;

        StringBuilder sbXslOutput = new StringBuilder();
        
        XslCompiledTransform transformer = new XslCompiledTransform();
        transformer.Load("h:\\st\\STANDARD_IND.xslt");

        using (XmlWriter xslWriter = XmlWriter.Create(sbXslOutput, transformer.OutputSettings))
        {
            XsltArgumentList args = new XsltArgumentList();

            XmlDataDocument doc = new XmlDataDocument();
            //doc.LoadXml("<?xml version=\"1.0\" encoding=\"windows-1251\"?><CigResult Version=\"2.0.0\" xmlns=\"\">" + resp.InnerXml + "</CigResult>");
            doc.Load("h:\\st\\standard report IND.xml");
        ...
        return Encoding.UTF8.GetBytes(dataSetHtml);
    ...
    /// <param name="Identification">Ид.код клиента</param>
    /// <returns>HTML отчета</returns>
    public byte[] GetReservReport(String Identification)
    {
        PVBKIReverseWebService service = new PVBKIReverseWebService();
        service.Url = _reportServiceUrl;
        ...
        try
        {
            ...
            bool isWork = testDescription.Contains("работа в штатном режиме");
            result = Encoding.Default.GetBytes(testDescription);

            AuthCredentialHeader header = new AuthCredentialHeader();
            header.UserName = _reportServiceUserName;
            header.Password = _reportServicePassword;

            AuthIdentityHeader identity = new AuthIdentityHeader();
            identity.Name = _reportEndpointName;
            identity.Key = getEndpointKey();

            service.AuthenticationCredential = header;
            service.AuthenticationIdentity = identity;
            ...
            if (isWork)
            {
                try
                {
                    ...
                    string temporaryPath = System.Environment.GetEnvironmentVariable("TEMP") + "\\Convert\\";
                    byte[] responce = service.Statement(Identification);

                    xmlFilePath = temporaryPath + "responce" + Convert.ToString(random.Next(100000)) + ".xml";
                    htmlFilePath = temporaryPath + "responce" + Convert.ToString(random.Next(100000)) + ".html";
                    FileStream fileStream = new FileStream(xmlFilePath, FileMode.Create, FileAccess.Write);
                    fileStream.Write(responce, 0, responce.Length);
                    fileStream.Close();
                    /***************************************************************************************/

                    string xmlStr = File.ReadAllText(xmlFilePath);

                    XmlDocument doc_iner = new XmlDocument();
                    doc_iner.LoadXml(xmlStr);

                    XmlNodeList locIdAddr = doc_iner.SelectNodes("/Statement/Address");
                    XmlNodeList locIdClltrl = doc_iner.SelectNodes("/Statement/Collateral");

                    List<string> locIdList = new List<string>();
                    foreach (XmlNode item in locIdAddr)
                    {
                        locIdList.Add(item.SelectSingleNode("locationId").InnerText);
                    }

                    foreach (XmlNode item in locIdClltrl)
                    {
                        locIdList.Add(item.SelectSingleNode("locationId").InnerText);
                    }
                    ...
                    String OutXml = "<?xml version=\"1.0\" encoding=\"windows-1251\"?><Territory>";
                    try
                    {
                        foreach (var item in locIdList)
                        {
                            ...
                            cmd.CommandText = "select * from bars.territory where id = " + item;
                            OracleDataReader reader = cmd.ExecuteReader();
                            while (reader.Read())
                            {
                                Place place = new Place();
                                place.locationId = reader["id"].ToString();
                                place.town = reader["city"].ToString();
                                place.district = reader["district"].ToString();
                                place.region = reader["region"].ToString();
                                XmlWriterSettings sttngs = new XmlWriterSettings(){ Indent = false, OmitXmlDeclaration = true };

                                var stream = new MemoryStream();
                                using (XmlWriter xw = XmlWriter.Create(stream, sttngs))
                                {
                                    XmlSerializerNamespaces ns = new XmlSerializerNamespaces(
                                                       new[] { XmlQualifiedName.Empty });
                                    XmlSerializer x = new XmlSerializer(place.GetType(), "");
                                    x.Serialize(xw, place, ns);
                                }
                                StreamReader streader = new StreamReader(stream);
                                streader.BaseStream.Position = 0;
                                OutXml += streader.ReadToEnd();
                            }
                        }
                    ...
                    OutXml += "</Territory>";
                    var byteTerritory = Encoding.Default.GetBytes(OutXml);
                    var pathTerritory = HttpContext.Current.Server.MapPath("/barsroot/ExternalBin/stosch/Vocabulary[cid-territory-cyr].xml");
                    FileStream fileStreamTerritory = new FileStream(pathTerritory, FileMode.Create, FileAccess.Write);
                    fileStreamTerritory.Write(byteTerritory, 0, byteTerritory.Length);
                    fileStreamTerritory.Close();

                    /***************************************************************************************/
                    var path = HttpContext.Current.Server.MapPath("/barsroot/ExternalBin/stosch/defaultStatementUK.xslt");
                    ...
                    result = File.ReadAllBytes(htmlFilePath);

                    File.Delete(xmlFilePath);
                    File.Delete(htmlFilePath);
                    File.Delete(pathTerritory);

                }
                catch (SoapException e)
                {
                    var message = service.BackError.FaultMessage;
                    result = Encoding.Default.GetBytes("Answer from service: " + message + "; Exception: " + e.Message);
                }
            }
    ...
    /// </summary>
    /// <param name="Identification">Составной ключ(Ид.код клиента)</param>
    /// <returns>HTML отчета</returns>
    public string GetReservReportString(String Identification)
    {
        return Encoding.Default.GetString(GetReservReport(Identification));
    }
...
public class mvbki : bki
{
    ...
    public byte[] GetReservReport(String Identification)
    {
        ...
        try
        {
            byte[] request = service.Statement(Identification);
            return request;
        }
        finally
        {
    ...
    public string GetReservReportString(String Identification)
    {
        return Encoding.UTF8.GetString(GetReservReport(Identification));
    }
...
public class bck : System.Web.Services.WebService
{
    ...
    [WebMethod]
    public string GetReportMVBKI(String ReportType, String TaxId, String PSerial, String PNumber)
    {
        mvbki svc = new mvbki();
        PvbkiClientType type;
        if (ReportType == "12")
    ...
    [WebMethod]
    public string GetStatment(String ForId)
    {
        pvbki svc = new pvbki();
        return svc.GetReservReportString(ForId);
    }
}
