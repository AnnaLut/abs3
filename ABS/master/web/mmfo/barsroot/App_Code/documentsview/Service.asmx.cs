	...
	public class Service : BarsWebService
	{
		...
		public object[] GetData(string[] data)
		{
			try
			{
                ...
                object[] MainResult = BindTableWithNewFilter(@"	a.REF as REF, 
												a.TT as TT, 
												a.USERID as USERID, 
												a.ND as ND, 
												a.MFOA as MFOA, 
                ...
                res[1] = MainResult[1];
                ClearParameters();
                DataSet dsAggs = GetFullDataSetForTable("count(1) as cnt, sum(a.s_) as s_", data[11] + " a", dateFilter, data);
                res[2] = Convert.ToDecimal(dsAggs.Tables[0].Rows[0]["CNT"]);
                string s = dsAggs.Tables[0].Rows[0]["S_"].ToString();
                res[3] = String.Format("{0:### ### ### ### ### ### ### ### ### ##0.00}", dsAggs.Tables[0].Rows[0]["S_"]);
                return res;
			}
			catch(Exception e)
			{
