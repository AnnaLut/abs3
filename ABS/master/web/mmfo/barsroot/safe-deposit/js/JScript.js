...
function BacktoPortfolio() {
    var DPT_ID = document.getElementById('DEAL_REF').value;
    var url = "safeportfolio.aspx?dpt_id=" + DPT_ID;
    location.replace(encodeURI(url));
    return true;
}
function BacktoPortfolioAll() {   
    var url = "safeportfolio.aspx?";
    location.replace(encodeURI(url));
    return true;
}
...
function validateParams()
{
    ...
    if (document.getElementById('KEY_NUM')) {
        var NUMPAR = document.getElementById('KEY_NUM').value;
        if (isEmpty(KEYS_GIVEN)) {
            alert('Необходимо ввести реквизит "номер ключа"!');
            focusControl('KEY_NUM');
            return true;
        }
    } 

    if (document.getElementById('KEYS_GIVEN')) {
        var NUMPAR = document.getElementById('KEYS_GIVEN').value;
        if (isEmpty(KEYS_GIVEN)) {
            alert('Необходимо ввести реквизит "ключів отримано"!');
            focusControl('KEYS_GIVEN');
            return true;
        }
    }
...
//        return true;
    ...
    "dialogWidth:1000px;dialogHeight:600px;center:no;edge:sunken;help:no;status:no;");    
...
function CalcDate()
{
    ...
    if (term.value > 365) { alert('Термін аренди не може бути більшим за 365 днів ( введено ' + term.value + '), змінюємо на 365!'); term.value = 365; }
    ...
    else if (event.srcElement == dat_end)    
    {
        ...
        document.getElementById('TERM').value = 
            Math.ceil((fnDateParse(dat_end.value).getTime() - 
             fnDateParse(dat_begin.value).getTime() + 0)/(one_day)+1);
        PageMethods.CkDate(fnDateParse(dat_end.value), OnSucceeded, OnFailed);	   
    }
...
{
    var arr = new Array();
    arr[0] = { code: 'DEAL_NUM', name: 'Номер договору' };
    arr[1] = { code: 'DAT_BEGIN', name: 'Дата початку' };
    arr[2] = { code: 'DEAL_DATE', name: 'Дата договору' };
    arr[3] = { code: 'TERM', name: 'Термін' };
    arr[4] = { code: 'DAT_END', name: 'Дата закінчення' };
    arr[5] = { code: 'FIO', name: 'ПІБ' };
    arr[6] = { code: 'OKPO', name: 'ІНН' };
    arr[7] = { code: 'JOKPO', name: 'ЗКПО' };
    arr[8] = { code: 'DOC', name: 'Документ' };
    arr[9] = { code: 'ISSUED', name: 'Виданий' };
    arr[10] = { code: 'NMK', name: 'Найменування' };
    arr[11] = { code: 'ADDRESS', name: 'Адреса' };
    ...
    for (var id in arr)
    {
        var elem = document.getElementById(arr[id].code);
        if (elem != null)
        {
            if (isEmpty(elem.value))
            {
                alert('Заполните обязательное поле! (' + arr[id].name+')');
                focusControl(arr[id].code);
                return true;
            }
        }
    }
...
function OnSucceeded(result, userContext, methodName) {
    if (methodName == 'GetNls') {
        ...
        if (fnDateParse(result) > fnDateParse(dat_end.value)) {
            if (term < 365) {
                var one_day = 1000 * 60 * 60 * 24;
                term =  Math.ceil(( ( fnDateParse(result).getTime() -  fnDateParse(dat_begin.value).getTime() + 1) / (one_day)));
                if (confirm('Дата закінчення договору випала на вихідний день, встановити дату закінчення на наступний робочий день (' + result  + ')?')) {
                    strDatEnd = result;
                ...
                var dateend = fnDateParse(result);
                dateend.setDate(dateend.getDate() - fnDateParse(result).getDay() - 2);
                var strDatEnd = fnDateToString(dateend);                
                if (confirm('Дата закінчення договору випала на вихідний день, встановити дату закінчення на попередній робочий день (' + strDatEnd + ')?')) {
                    window[dat_end_string].SetValue(strDatEnd);
                    window[dat_end_string].Initialize();
                    document.getElementById('TERM').value = term - fnDateParse(result).getDay() - 1 ;
                }
            }
        }
    }
