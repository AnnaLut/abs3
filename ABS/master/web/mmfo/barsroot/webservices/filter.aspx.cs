...
namespace Bars.WebServices
{
	...
	public partial class Filter : BarsPage
	{
		...
		private void Page_Load(object sender, EventArgs e)
		{
			if(!IsPostBack)
			{
				try
				{
                    ...
                    List<int> indexes4Skip = new List<int>();
                    Dictionary<int, List<int>> duplicates = FindDuplicates();

                    for (int i = 0; i < tags.Count; i++)
                    {
                        if (!indexes4Skip.Contains(i) && tags[i] is Tag)
                        {
                            Tag tag = ((Tag)tags[i]);
                            ...
                            if (tag.sHColName != null)
                                item.Value += ";" + tag.sQTab + ";" + tag.sHColName + ";" + tag.sRColName;

                            if (duplicates.ContainsKey(i))
                            {
                                for (int j = 0; j < duplicates[i].Count; j++)
                                {
                                    var index = duplicates[i][j];
                                    Tag tag_ = (Tag)tags[index];
                                    item.Value += ";" + tag_.sHColName + ";" + tag_.sRColName;
                                    indexes4Skip.Add(index);
                                }
                            }
            ...
            if (tags != null)
            {
                for (var i = 0; i < tags.Count; i++)
                {
                    if (tags[i] is Tag)
                    {
                        ...
                        {
                            if (i != j && tags[j] is Tag)
                            {
                                Tag tag_ = (Tag)tags[j];
                                if (tag.sQCol == tag_.sQCol && tag.sQTab == tag_.sQTab)
                                {
		...
		private void dgUser_ItemDataBound(object sender, DataGridItemEventArgs e)
		{
			...
			{
				DataGridItem row = e.Item;
				if(usrIs.Value != "1" && row.Cells[1].Text != "-1") usrIs.Value = "1";
				...
				string row_id = "u_" + row.Cells[1].Text;
				row.Attributes.Add("id", row_id);
				row.Style.Add("cursor","hand");
				row.Attributes.Add("onclick", "Sel_Usr('"+row.Cells[1].Text+"')");
				row.Attributes.Add("ondblclick", "Apl_Usr('"+row.Cells[1].Text+"')");
				row.Attributes.Add("cw",row.Cells[3].Text);

                TableCell ckbCell = row.Cells[0];
                ckbCell.Attributes.Add("id", "ckb_" + row.Cells[1].Text);
                ckbCell.Attributes.Add("onclick", "Sel_Usr_Ckb('" + row.Cells[1].Text + "')");
			}
		}
