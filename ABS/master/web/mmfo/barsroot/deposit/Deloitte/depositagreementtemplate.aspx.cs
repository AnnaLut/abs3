...
public partial class DepositAgreementTemplate : Bars.BarsPage
{
    ...
    private void Page_Load(object sender, System.EventArgs e)
    {
        ...
        try
        {
            ...
            _dbLogger.Info("Выбор шаблона для доп. соглашения тип=" + agr_id + 
    ...
    private void btForm_ServerClick(object sender, System.EventArgs e)
    {
        ...
        Decimal dpt_id = Convert.ToDecimal(Request["dpt_id"]);
        Int64 agr_id = Convert.ToInt64(Request["agr_id"]);
        string template_id = Convert.ToString(Template_id.Value);

        _dbLogger.Info("Пользователь выбрал шаблон " + template_id + " для доп. соглашения тип=" + agr_id +
            " по договору №" + dpt_id, "deposit");
        ...
        switch (agr_id)
        {
            ...
            case 19:
                {
                    destination = ("/barsroot/deposit/deloitte/DepositChangeFrequency.aspx?dpt_id=" + dpt_id.ToString() +
                        "&agr_id=" + agr_id.ToString() + "&template=" + template_id);
                    break;
                }
            case 20:  // Додаткова угода про закладання депозиту в заставу по кредиту --Yourchenko
                {
                    _dbLogger.Info("Пользователь выбрал депозит в якості застави по кредиту"+
                        " по договору №" + dpt_id, "deposit");
                     destination = ("/barsroot/deposit/deloitte/DepositEditAccount.aspx?next=print&dpt_id=" + dpt_id
                        + "&agr_id=" + agr_id + "&template=" + template_id);
                    break;
            ...
            case 35:  // Додаткова угода про дострокове розірвання невідкличного вкладу
            case 36:  // Заява про дострокове розірвання невідкличного вкладу
            case 37:  // Додатковий договір до депозитного договору на вклад “Класичний” на ім’я фізичної особи для передачі вкладником у заставу банку майнових прав з метою забезпечення виконання зобов’язань корпоративних клієнтів за операціями з надання банківських гарантій
            case 38:  // ощад онлайн24
                {
                    destination = ("/barsroot/deposit/deloitte/depositagreementprint.aspx?dpt_id=" + dpt_id +
                        "&agr_id=" + agr_id + "&template=" + template_id);
                    if (Request["inherit_irr_id"] != null)
                        destination += ("&inherit_irr_id=" + Convert.ToString(Request["inherit_irr_id"]));
                    break;
                }
            default:
                {
