...
public partial class DepositPercentPay : Bars.BarsPage
{
	...
	private void Page_Load(object sender, System.EventArgs e)
	{
		...
		Deposit dpt = new Deposit(Convert.ToDecimal(Request.QueryString["dpt_id"]), other);
	...
	{
		_dbLogger.Info("Пользователь нажал на кнопку \"Выплата\" (процентов) на странице выплаты процентов по договору # " + 
    ...
    protected void btAuthorizationByBPK_ClientIdentified(object sender, Bars.UserControls.ClientIdentifiedEventArgs e)
    {
        ...
        {
            _dbLogger.Info("Користувач виконав Авторизацію виплати відсотків за допомогою БПК клієнта (rnk=" + 
           ...
           /* String url = "/barsroot/deposit/deloitte/DepositContractInfo.aspx?dpt_id=" + Request.QueryString["dpt_id"] +
            ...
            ClientScript.RegisterStartupScript(this.GetType(), "PercentPay", "PercentPay(); location.replace('" + url + "');", true);
        }
        else
        {
    ...
    /// <summary>
    /// Перевірка суми відсотків до виплати
    /// </summary>
    /// <returns></returns>
    //private Boolean CheckSum()
    //{        
    //    if (Convert.ToDecimal(textSum.Value) <= 0)
    //    {
    //        // "Невозможно выполнить операцию." + "Нулевая сумма!"
    //        Response.Write("alert(LocalizedString('Mes6') \n LocalizedString('Mes7'));");
    ...
    //    Decimal etalon = Convert.ToDecimal(textPercentSum.Value);

    //    if (Request["inherit_id"] != null)
    //        etalon = Convert.ToDecimal(InheritSum.Value);

    //    if (Convert.ToDecimal(textSum.Value) > etalon)
    //    {
    //        // "Сумма больше максимальной допустимой!"
    //        Response.Write("alert(LocalizedString('Mes9'));");
