...
{   
    ...
    private void Page_Load(object sender, System.EventArgs e)
    {
        ...
        {
            _dbLogger.Info("Пользователь зашел на страницу выбора типа депозитного договора без информации о клиенте и был перенаправлен на карточку клиента",
                "deposit");
    ...
    private void FillContractDependControls()
    {
        ...
        try
        {
            ...
            if (listContractType.SelectedValue != String.Empty)
            {
                ...
                _dbLogger.Info("p_custid = " + dpt.Client.ID.ToString() + " p_vidd =" + contractTypeCode.ToString() + " p_dptamount=" + textContractSum.Value.ToString(),"deposit");
                ...
                if (rdr.Read())
                {
                    ...
                    if (Convert.ToDecimal(rdr.GetValue(15)) == 2)
                    {
                        ...
                        dtContractBegin.Date = System.DateTime.Now; 
    ...
    /// 
    /// 
    private void FillThirdPersonRB(decimal type_id)
    {
        rbOnBeneficiary.Enabled = false; rbOnBeneficiary.Checked = false;
        rbOnChildren.Enabled = false; rbOnChildren.Checked = false;
        rbOnOwner.Enabled = false; rbOnOwner.Checked = false;
        rbOnBeneficiary.Text = "Вклад на бенефіціара";
        rbOnChildren.Text = "Вклад на користь малолітної особи";
        rbOnOwner.Text = "Вклад на ім'я малолітної особи";
         OracleConnection connect = new OracleConnection();
        try
        {
            ...
            cmd.CommandText = @"select f1||f2||f3 from ( select sum(case when flags =  5  then '1' else '0' end) f1, 
                                                                sum(case when flags =  26 then '1' else '0' end) f2,
                                                                sum(case when flags =  27 then '1' else '0' end) f3
                                                                from bars.dpt_vidd_scheme where vidd = :type_id) ";
            cmd.Parameters.Add("p_vidd", OracleDbType.Decimal, type_id, ParameterDirection.Input);            

            String result = Convert.ToString(cmd.ExecuteScalar());
            if (result.Substring(0, 1) == "1") rbOnBeneficiary.Enabled = true; else rbOnBeneficiary.Text = "Вклад на бенефіціара недоступний ( шаблон ДУ 5 не прив'язано до виду " + Convert.ToString(type_id) + ")";
            if (result.Substring(1, 1) == "1") rbOnChildren.Enabled = true; else rbOnChildren.Text = "Вклад на користь малолітної особи недоступний ( шаблон ДУ 26 не прив'язано до виду " + Convert.ToString(type_id) + ")";
            if (result.Substring(2, 1) == "1") rbOnOwner.Enabled = true; else rbOnOwner.Text = "Вклад на ім'я малолітної особи недоступний ( шаблон ДУ 27 не прив'язано до виду " + Convert.ToString(type_id) + ")";
    ...
    private void btnBack_ServerClick(object sender, System.EventArgs e)
    {
        ...
        {
            _dbLogger.Info("Пользователь нажал на кнопку \"Назад\" на странице выбора типа депозита и вернулся на карточку клиента",
                "deposit");
    ...
    {
        _dbLogger.Info("Пользователь нажал на кнопку \"Далее\" на странице выбора типа депозита и продолжил заведение депозитного договора." +
    ...
    protected void listTypes_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        _dbLogger.Info("tbTypeCode.Text = " + tbTypeCode.Text,"deposit");
        FillCurrencyList();
    ...
    protected void listContractType_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        FillThirdPersonRB(Convert.ToDecimal(listContractType.SelectedItem.Value));
    ...
    protected void FillTypeList()
    {
        ...
        dsType.SelectCommand = "select TYPE_ID, TYPE_NAME from DPT_TYPES t where FL_ACTIVE = 1 " +
            " and FL_DEMAND = 0 and exists (select 1 from DPT_VIDD v where v.TYPE_ID = t.TYPE_ID and v.FLAG = 1 ) order by SORT_ORD";
    ...
    protected void FillCurrencyList()
    {
        ...
        dsCurrency.SelectCommand = @"select KV as Currency_Code, NAME as Currency_Name from TABVAL$GLOBAL c
                                      where exists (select 1 from DPT_VIDD v where v.TYPE_ID = "+ tbTypeCode.Text +
                                      " and v.KV = c.KV and v.FLAG = 1 ) ";

        //dsCurrency.SelectParameters.Add("p_typeid", TypeCode.Int32, tbTypeCode.Text);
        _dbLogger.Info("dsCurrency.SelectCommand = " + dsCurrency.SelectCommand, "deposit");
    ...
    protected void FillContractTypeList()
    {
        ...
        dsContractType.SelectCommand = @"select vidd, type_name from V_DPT_VIDD_USER 
                    where type_id = "+ listTypes.SelectedValue+ " And kv = " +listCurrency.SelectedValue + "  order by vidd";

        //dsContractType.SelectParameters.Add("p_type", System.Data.DbType.Int32, listTypes.SelectedValue);
        //dsContractType.SelectParameters.Add("p_kv", System.Data.DbType.Int32, listCurrency.SelectedValue);
