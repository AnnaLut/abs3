...
public partial class DepositSelectTT : Bars.BarsPage
{
    ...
    private void Page_Load(object sender, System.EventArgs e)
	{
		...
		try
		{
			...
			if (action == "percent")
			{
				...
				_dbLogger.Info("Выбор операции для выплаты процентов по депозитному договору №" + dpt_id.ToString(),
					"deposit");
			}		
			else if (action == "close")
			{
				...
				_dbLogger.Info("Выбор операции для досрочного расторжения депозитного договора №" + dpt_id.ToString(),
					"deposit");
			}
            ...
            else if (action == "payout")
            {
                ...
                lbInfo.Text = Resources.Deposit.GlobalResources.h02;
                _dbLogger.Info("Вибір операції для виплати готівкою в день завершення депозиту №" + dpt_id.ToString(),
                    "deposit");
            }
            else if (action == "return")
			{
                lbInfo.Text = Resources.Deposit.GlobalResources.h02;
                //lbInfo.Text = "Допустимые операции для возврата депозита";
				_dbLogger.Info("Выбор операции для выплаты по завершению депозитного договора №" + dpt_id.ToString(),
					"deposit");
			}
	    ...
	    else if (action == "close")		
            {
                searchQuery += " (" +
                    ...
                    ((int)DPT_OP.OP_33).ToString() + ") " +
                    @"  and op_type not in ('EDP', 'DPE')";
                        //and (d.kv = 980                       //COBUMMFO-4641
                        //or(ebp.dpt_out_kv(d.deposit_id) != 1 and op_type in ('EDP', 'DPE'))
                        //or(ebp.dpt_out_kv(d.deposit_id) = 1 and(op_type not in ('EDP', 'DPE')))
                        //or(dpt_isirrevocable(d.deposit_id) = 0)
                        //or(d.vidd in (select vidd from dpt_vidd where type_cod = 'FLEX' )))";// inga 27/05/2014 BRSMAIN-2689  -включить "33" 28/10/2014 (Гуренко)
                        //                                                                     // inga 27/02/2017 COBUSUPABS-5521
            }
            else if (action == "closep")
            {
                searchQuery += " (" +
    ...
    private void btSelect_ServerClick(object sender, System.EventArgs e)
	{
		...
		if (action == "percent")
		{
			_dbLogger.Info("Пользователь выбрал для выплаты процентов по депозиту №" + dptid +
		...
		{
			_dbLogger.Info("Пользователь выбрал для досрочной выплаты по депозиту №" + dptid +
				" операцию " + _tt,"deposit");
        ...
        {
            _dbLogger.Info("Пользователь выбрал для досрочной выплаты(выплата процентов) по депозиту №" + dptid +
                " операцию " + _tt, "deposit");
        ...
        {
            // виплата вкладу готівкою в день завершення (ЕБП)
            _dbLogger.Info("Користувач вибрав для виплати в день завершення по депозиту №" + dptid +
		...
		{
			_dbLogger.Info("Пользователь выбрал для выплаты по завершения по депозиту №" + dptid +
				" операцию " + _tt,"deposit");
