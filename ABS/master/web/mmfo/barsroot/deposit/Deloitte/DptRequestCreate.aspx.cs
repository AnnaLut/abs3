...
public partial class DptClientRequestCreate : Bars.BarsPage
{
    ...
    protected void btCreateRequest_Click(object sender, EventArgs e)
    {
		...
		if (req_id.HasValue)
        {
            ...
            Decimal? ClientApplicationID = ep.MSG_CREATE("CLIENT", cust_id.ToString(), (Int64)cust_id, KF);
            ...
            else
            {
                for (int i = 0; i < AccessList.Count; i++)
                {
                    ...
                    Decimal? DealApplicationID = ep.MSG_CREATE("AGR", "DPT;" + AccessList[i].DPT_ID.ToString(), (Int64)cust_id, KF);
