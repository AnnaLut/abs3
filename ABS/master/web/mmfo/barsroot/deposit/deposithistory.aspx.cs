...
public partial class DepositHistory : Bars.BarsPage
{
    ...
    private void Page_Load(object sender, System.EventArgs e)
    {
        ...
        if (Request["dpt_id"] == null)
            Response.Redirect("DepositSearch.aspx?action=history&extended=0");
        else
            FillInForm(Convert.ToDecimal(Request.QueryString["dpt_id"]));
    }
    /// <summary>
    /// Инициализация информации на странице
    /// </summary>
    private void FillInForm(Decimal dpt_id)
    {
        ...
        if (Request["noext"] != null)
            ...
            dpt.ReadFromDatabaseExt(false, false, false);
        
        textDepositNumber.Text = dpt.Number;

        textClientName.Text = dpt.Client.Name;
        textClientPasp.Text = dpt.Client.DocTypeName + " " + dpt.Client.DocSerial + " " + dpt.Client.DocNumber;
    ...
    private void btAddAgreementHistory_Click(object sender, System.EventArgs e)
    {
        ...
        try
        {
            _dbLogger.Debug("Пользователь запросил историю доп.соглашений к договору №" + Request["dpt_id"].ToString() + " на странице просмотра истории договоров",
                "deposit");
    ...
    private void btShowFinancialHistory_Click(object sender, System.EventArgs e)
    {
        ...
        try
        {
            _dbLogger.Debug("Пользователь запросил финансовую историю к договору №" + Request["dpt_id"].ToString() + " на странице просмотра истории договоров",
                "deposit");
