...
public partial class DepositContractInfo : Page
{
    ...
    private void Page_Load(object sender, System.EventArgs e)
    {
        ...
        {    
            ...
            FillContractInfo();
            if (IS_FEE_PAYMENT_DONE(dpt))
            {
                btFirstPayment.Attributes["onclick"] = "javascript:FirstPayment();"; EnablePrint();
            }
            else
            {
                btFirstPayment.Attributes["onclick"] = "javascript:FeePayment();";
            }
...
//                    rnk.Value = dpt.Client.ID.ToString();
    ...
    private void btnNext_Click(object sender, System.EventArgs e)
    {
        ...
        _dbLogger.Info("Пользователь нажал на кнопку \"Новый договор\" на странице карточки вклада",
            "deposit");
    ...
    private void btFormDptText_ServerClick(object sender, System.EventArgs e)
    {
        ...
        _dbLogger.Info("Пользователь нажал на кнопку \"Подтвердить\"(первичный взнос) на странице карточки вклада","deposit");
    ...
    private bool IS_FEE_PAYMENT_DONE(Deposit dpt)
    {   bool res = false; 
        OracleConnection connect = new OracleConnection();
        try
        {
            ...
            cmdSetRole.CommandText = conn.GetSetRoleCommand("DPT_ROLE");
            cmdSetRole.ExecuteNonQuery();
            if (dpt.dpt_nls == String.Empty || dpt.dpt_nls == null)
            {
                res = true;
            }
            else
            {
                _dbLogger.Info("IS_FEE_PAYMENT_DONE: " + dpt.dpt_nls.Substring(0, 4) + "dpt.Type = " + Convert.ToString(dpt.Type), "deposit");
                if (dpt.dpt_nls.Substring(0, 4) == "2620" && dpt.Type != 1987)
                {
                    OracleCommand cmdDepositInfo = new OracleCommand();
                    cmdDepositInfo.Connection = connect;
                    cmdDepositInfo.CommandText = " select nvl(nvl((SELECT 1 FROM dpt_deposit d,accounts a WHERE a.nbs = '2620' " +
                                                 " and a.ob22 in ('20','21','33','39') AND d.deposit_id = :dpt_id AND d.acc = a.acc), " +
                    ...
                    Decimal sum = cmdDepositInfo.ExecuteScalar() == null ? 0 : Convert.ToDecimal(cmdDepositInfo.ExecuteScalar());

                    _dbLogger.Info("IS_FEE_PAYMENT_DONE: " + Convert.ToString(sum), "deposit");
                    if (Math.Abs(sum) > 0) { res = true; } else { res = false; }
                }
                else { res = true; }
            }            
        }
    ...
    private void btnSgnContract_ServerClick(object sender, System.EventArgs e)
    {
        ...
        _dbLogger.Info("Пользователь нажал на кнопку \"Подписать договор\" на странице карточки вклада",
            "deposit");
