    ...
    private void Page_Load(object sender, System.EventArgs e)
	{
		...
		try
		{
			...
			if (action == "percent")
			{
                ...
                _dbLogger.Info("Выбор операции для выплаты процентов по депозитному договору №" + dpt_id.Value,
					"deposit");
			}		
			else if (action == "close")
			{
                lbInfo.Text = Resources.Deposit.GlobalResources.h02;
                //lbInfo.Text = "Допустимые операции для возврата депозита при досрочном расторжении договора";
                _dbLogger.Info("Вибір операції для виплати депозиту при достроковому розірванні договору №" + dpt_id.Value,
					"deposit");
			}
            else if (action == "closep")
            {
                lbInfo.Text = Resources.Deposit.GlobalResources.h01;
                //lbInfo.Text = "Допустимые операции для возврата процентов при досрочном расторжении договора";
                _dbLogger.Info("Вибір операції для виплати відсотків при достроковому розірванні договору №" + dpt_id.Value,
                    "deposit");
            }
            else if (action == "return")
			{
                ...
                _dbLogger.Info("Выбор операции для выплаты тела по завершению депозитного договора №" + dpt_id.Value,
					"deposit");
			}
			else if (action == "returnp")
			{
                lbInfo.Text = Resources.Deposit.GlobalResources.h01;
                //lbInfo.Text = "Допустимые операции для выплаты процентов";
                _dbLogger.Info("Выбор операции для выплаты процентов по завершению депозитного договора №" + dpt_id.Value,
					"deposit");
			}
			else
			{
			...
			else if (action == "close")		
            {
                searchQuery += " (" +
                    ...
                    ((int)DPT_OP.OP_35).ToString() + "," +
                    ((int)DPT_OP.OP_33).ToString() + ") " +
                    @"  and op_type not in ('EDP', 'DPE')";  //COBUMMFO-4641
            }
            else if (action == "closep")
            {
                searchQuery += " (" +
