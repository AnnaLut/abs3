    ...
    public partial class DocInputPage : BarsPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                __IS_NEED_CHECK_TT.Value = IsNeedCheckTtBySms();
            ...
            __VALIDATE_MODE.Value = ConfigurationSettings.AppSettings["Module.Payment.ExtValidation"];
            ...
            foreach (string js_nam in js_list)
            {
                if (!ClientScript.IsClientScriptBlockRegistered(js_nam))
                    ClientScript.RegisterClientScriptBlock(Page.GetType(), js_nam, "<script language=\"javascript\" src=\"js/" + js_nam + ".js?v1.9.84\"></script>");
            }
        ...
        private void DRecsFill()
        {
            ...
            // 
            if (Request["attrMode"] == "1")
            {
                cbKeepAttribute.Visible = true;
                cbKeepAttribute.Checked = Convert.ToString(Session["ka_Enable"]) == "true";
                if (cbKeepAttribute.Checked)
                {
                    Nazn.Text = Convert.ToString(Session["ka_Nazn"]);
                }
            }
            ...
            try
            {
                ...
                cmd.Parameters.Clear();
                // COBUSUPABS-4686
                Hashtable validatedParams = new Hashtable();

                if (ConfigurationSettings.AppSettings["Module.Payment.ExtValidation"] == "1")
                {
                    cmd.Parameters.Add("TT", OracleDbType.Varchar2, TT, ParameterDirection.Input);
                    cmd.CommandText = "select * from v_params_prime_load where tt=:tt and prime = 1";
                    ...
                    while (oraReader.Read())
                    {
                        ...
                        validatedParams[paramName] = new Tuple<string, string, bool, bool>(paramName, paramValue, isParamEmpty, isParamEdit);
                    }
                }
                //
                cmd.Parameters.Clear();
                ...
                while (rdr.Read())
                {
                    ...
                    else if (ControlName == "reqv_TR_ID")
                    {
                        if (Request.Form["TransactionId"] != null)
                            valueDopReq = Convert.ToString(Request.Form["TransactionId"]);
                    }
                    else if (ControlName == "reqv_MTSC")
                    {
                        string mtiName = Convert.ToString(Session["SWI_MTI_NAME"]);
                        if (!string.IsNullOrEmpty(mtiName))
                        {
                            Session.Remove("SWI_MTI_NAME");
                            valueDopReq = mtiName;
                        }
                    }
                    else
                    {
                        ...
                        if (Request.Params[ControlName] != null)
                            valueDopReq = Convert.ToString(Request.Params[ControlName]);
                    }
                    ...
                    if (__ISSWI.Value == "1")
                    {
                        ...
                        // COBUSUPABS-4217
                        if (ControlName == "reqv_D#73" && Request.Form["reqv_DATT"] != null)
                        {
                            CultureInfo cinfo = CultureInfo.CreateSpecificCulture("en-GB");
                            cinfo.DateTimeFormat.ShortDatePattern = "dd.MM.yyyy";
                            cinfo.DateTimeFormat.DateSeparator = ".";
                            DateTime today = DateTime.Now;
                            DateTime datt = Convert.ToDateTime(Request.Form["reqv_DATT"], cinfo);
                            DateTime start = new DateTime(today.Year, today.Month, 1, 0, 0, 0);
                            DateTime finish = start.AddMonths(1).AddSeconds(-1);
                            if (datt >= start && datt <= finish)
                                valueDopReq = "000";
                            else
                                valueDopReq = "370";
                            nomodify = "1";
                    ...
                    if (cbKeepAttribute.Checked)
                    {
                        var key = "ka_" + ControlName.Replace("$", String.Empty).Replace("reqv_", "");
                        if (Session[key] != null)
                            valueDopReq = Convert.ToString(Session[key]);
                    }
                    ///
                    bool disableReq = false;
                    string jsValidateOnChangeFunc = string.Empty;
                    string jsReqDictionaryFunc = "selectDopReq";
                    if (validatedParams.ContainsKey(op_field))
                    {
                        var tuple = (Tuple<string, string, bool, bool>)validatedParams[op_field];
                        if (!string.IsNullOrEmpty(tuple.Item2))
                            valueDopReq = tuple.Item2;
                        drecs.Rows[i].Cells[0].ID = "lb_" + ControlName;
                        if (!tuple.Item3)
                        {
                            drecs.Rows[i].Cells[0].Style.Add("color", "red");
                            if (!Mand_Drecs_ids.Value.Contains("," + ControlName + ","))
                                Mand_Drecs_ids.Value += ControlName + ",";
                        }
                        disableReq = !tuple.Item4;
                        jsReqDictionaryFunc += "Ext";
                        if (ControlName.ToUpper() == "REQV_KOD_N")
                            jsValidateOnChangeFunc = "validateKodN();";
                    }

                    // если передали через строку вызова - приоритетней
                    if (ControlName.Replace("$", String.Empty).ToUpper() == "REQV_KOD_G")
                    {
                        if (Request["reqv_D6#70"] != null)
                            valueDopReq = Request["reqv_D6#70"];
                        if (Request["reqv_D6%2370"] != null)
                            valueDopReq = Request["reqv_D6%2370"];
                    }
                    ///
                    ...
                    field.Attributes["required"] = isRequired;
                    ...
                    if (rdr.GetOracleDecimal(8) == 1 && (nomodify != "1" || ControlName.ToUpper() == "REQV_PASPV" || ControlName.ToUpper() == "REQV_PASNR" || ControlName.ToUpper() == "REQV_NAMET" || ControlName.ToUpper() == "REQV_LCSDE" || ControlName.ToUpper() == "REQV_DJNR" || ControlName.ToUpper() == "REQV_META" || ControlName.ToUpper() == "REQV_EXCTX" || ControlName.ToUpper() == "REQV_REZID" || ControlName.ToUpper() == "REQV_EXCTN" || ControlName.ToUpper() == "REQV_DJER"))
                    {
                        field.Attributes["onfocusin"] = "ShowF12()";
                        field.Attributes["onfocusout"] = "HideF12()";
                        field.Attributes["onkeydown"] = jsReqDictionaryFunc + "(event,'" + ControlName + "')";
                        ...
                        bt.Value = "?";
                        bt.Attributes["onclick"] = jsReqDictionaryFunc + "(event,'" + ControlName + "',1)";
                    ...
                    //--- IDDO - Дійсний до (ID-картка)
                    if (ControlName.ToUpper() == "REQV_IDDO")
                    {
                        field.Attributes["onchange"] = "chkCustomReq('IDDO')";
                    }
                    if (!string.IsNullOrEmpty(jsValidateOnChangeFunc))
                        field.Attributes["onchange"] += jsValidateOnChangeFunc;
        ...
        private void GetGlobalData()
        {
            ...
            try
            {
                ...
                try
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add("tt", OracleDbType.Varchar2, TT, ParameterDirection.Input);
                    cmd.CommandText = "gl_ui.doc_input_check";
                    cmd.ExecuteNonQuery();
                }
                catch (OracleException oraEx)
                {
                    if (oraEx.Number != 6550)
                    {
                ...
                + "(select nvl(min(val),'') from params where par='OKPO') BANK_OKPO, "
                ...
                __BANKOKPO.Value = Convert.ToString(rdr["BANK_OKPO"]);
        ...
        /// <summary>
        /// признак чи потрібно робити перевірку по моб.телефону (Луганськ,Донецьк)
        /// </summary>
        private static string IsNeedCheckTtBySms()
        {
            string res = "0";
            OracleConnection con = Bars.Classes.OraConnector.Handler.IOraConnection.GetUserConnection();
            try
            {
                var select = @"select branch_attribute_utl.get_value('CHECK_CUSTOMER_WITH_SMS') from dual";
                using (OracleCommand cmd = new OracleCommand(select, con))
                {
                    string sRes = Convert.ToString(cmd.ExecuteScalar());
