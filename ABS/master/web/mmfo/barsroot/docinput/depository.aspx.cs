...
public partial class docinput_depository : Bars.BarsPage
{
    ...
    private void FillTts()
    {
        try
        {
            ...
            ddBranches.DataSource = SQL_SELECT_dataset("select branch, name from branch where branch like nvl(SYS_CONTEXT('bars_context','user_mfo_mask'),'%') and length(branch)=15 order by branch").Tables[0];
        ...
        foreach (GridViewRow row in DataDepository.Rows)
        {
                          ...
                          && !string.IsNullOrEmpty(HttpUtility.HtmlDecode(row.Cells[5].Text).Trim()))
            {
                ...
                try
                {
                    ...
                    ArrayList reader = SQL_reader("select d.nms,k.nms, to_char(nvl(web_utl.get_bankdate,sysdate), 'dd/MM/yyyy') BDATE, docsign.GetIdOper, f_ourokpo OKPO, f_ourmfo MFO from accounts d, accounts k where d.kv=980 and k.kv=980 and d.nls=:nlsd and k.nls=:nlsk");
                ...
                try
                {
                    ...
                    {
                        Ref = ourDoc.Ref;
                        _dbLogger.Financial("Ввод док Ref" + Ref + ": " + NlsA + "(" + KvA + ") " + SA.ToString("F0") + " -> " + BankB + " " + NlsB + "(" + KvB + ") " + SB.ToString("F0") + "[" + NamB + "],[" + Nazn + "] " + OperId);
