...
function Waiting(flag) {
    kendo.ui.progress($(".search-catalog"), flag);
}

function GetKvitHref(value) {
    if(value === "kvit1")
    {
        return bars.config.urlContent("/pfu/pfu/Receipts?type=envelop");
    }
    else{   //"kvit2"
        return bars.config.urlContent("/pfu/pfu/KvitTwo");
    }
}
...
function getIdByName(dropDownData) {
    for(var i=0; i<dropDownData.length; i++){
        dropDownData[i].STATE_NAME
    }
}
...
// fill any grid
function fillKendoGrid(gridId, srcDataSource, srcSettings, toolbarTemplate) {
    // debugger;
    var dstDataSource = {
        type: "aspnetmvc-ajax",
        pageSize: 12,
