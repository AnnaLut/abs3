...
function changeGridMaxHeight(koef, gridId) {
    ...
    $(selector).css("max-height", a4 * 0.9);
}
...
String.prototype.toDateTime = function (dateSeparator, timeSeparator, dtSeparator) {
    dtSeparator = dtSeparator || ' ';
    timeSeparator = timeSeparator || ':';
    dateSeparator = dateSeparator || '.';

    var tmp = this.split(dtSeparator);
    var _date = tmp[0].split(dateSeparator);
    var _time = tmp[1].split(timeSeparator);

    return new Date(+_date[2], +_date[1] - 1, +_date[0], +_time[0], _time[1], _time[2] || 0);
};
