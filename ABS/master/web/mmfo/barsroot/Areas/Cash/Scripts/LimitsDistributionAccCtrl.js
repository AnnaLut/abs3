angular.module('BarsWeb.Controllers').controller('Cash.LimitsDistributionAcc', ['$scope', '$http', function ($scope, $http) {
    ...
    var exportToExelUpload = function () {
        var datsource = $scope.limDistGrid.dataSource;
        ...
        if (filter && filter != null)
        {
            ...
            dataSourceRequest = dataSourceRequest + "&filter=";
        console.log(dataSourceRequest);
        document.location.href = dataSource.url + dataSourceRequest + '&date=' + $scope.date + '&type=XLSUPLOAD';
    }
                            ...
                            bars.ui.confirm({
                                ...
                                func: function () {
                                    ...
                                    dataSource.remove(dataItem.Id, $scope.date, function () {
                                        bars.ui.notify(
                                            'Успішно',
                                            'Розподіл за ' + $scope.date + ' видалено, вступив в дію попередній розподіл згідно історії',
                                            'success');
