angular.module("BarsWeb.Areas").controller("Credit.moreCreditParamsCtrl", ["$scope", "$http", "$rootScope", "$location", function ($scope, $http, $rootScope, $location) {
    ...
    $scope.tabDataSource = new kendo.data.DataSource({
        transport: {
            read: {
                url: bars.config.urlContent("/credit/newcredit/getTabList/"),
                dataType: "json"
            }
        }
    });
    ...
    $scope.getEditor = function (container, options) {
        switch (options.model.TYPE) {
            case "S": var editor = container.append('<input type="text" class="k-textbox" data-bind="value:' + options.field + '"/>'); break;
            case "N": var editor = container.append('<input kendo-numeric-text-box data-bind="value:' + options.field + '"/>'); break;
            case "D": var editor = container.append('<input kendo-date-picker k-format="dd.MM.yyyy" data-bind="value:' + options.field + '"/>'); break;
            case "R": {
    ...
    $scope.tabDataSource.fetch(function () {
        ...
        for (i = 0; i < tabs.length; i++) {
            ...
            var dataTxt;
            if (nd != null) {
                urlTxt = "/credit/newcredit/getNdTxtDeal/";
                dataTxt = { nd: nd, code: tabs[i].CODE };
            }
            else {
                urlTxt = "/credit/newcredit/getNdTxt/";
