app.controller("moneyFlowHomeController", ['$scope', 'paramsService', '$window', 'LS', moneyFlowHomeController]);


function moneyFlowHomeController($scope, paramsService, $window, LS) {
    $("#loader").hide();
    ...
    $scope.vm = {};
    paramsService.getMoneyFlowParams(window.REF).then(function (response) {
        $scope.vm = response;
        $scope.vm.REF = window.REF;
        $scope.vm.RB1 = 1;
        $scope.vm.RB2 = 0;
