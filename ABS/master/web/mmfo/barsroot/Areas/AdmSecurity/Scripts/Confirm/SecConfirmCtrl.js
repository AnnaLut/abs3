$(document).ready(function () {
    var confirmTabsDatasource = new kendo.data.DataSource({
        ...
        transport: {
            read: {
                type: "GET",
                dataType: "json",
                url: bars.config.urlContent("/admsecurity/confirm/GetConfirmTabs"),
    ...
    function getCheckedItems(selector, resCode, tabId) {
            ...
            for (i; i < checkedList.length; i++) {
                ...
                $("#save").kendoButton({
                    click: function () {
                        ...
                        $.ajax({
                            type: "GET",
                            url: bars.config.urlContent("/api/admsecurity/confirm/"),
                        ...
                        var gr = $("#confirmResource" + resCode + tabId).data("kendoGrid");
                        gr.dataSource.read();
                    },
                    enable: true
                });
    ...
    $("#ConfirmTabstrip").kendoTabStrip({
        ...
        select: function (e) {
            ...
            $("#SaveTab_" + data.ID).on("click", function () {
                getCheckedItems("#confirmResource" + data.RESOURCE_CODE + data.ID, data.RESOURCE_CODE, data.ID);
            });
        },
