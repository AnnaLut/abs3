...
CIM.contract_card_module = function () {
    ...
    var mode = null,
        ...
        ctrlBenef = null,
        ctrlCreditTerm = null,
        ctrlCreditBorrower = $('select[name$="ddCreditBorrower"]'),
        ctrlTrDeadline = $('select[name$="ddTrDeadline"]'),
        ...
        ctrlServiceBranch = $('select[name$="ddServiceBranch"]'),
    ...
    module.initialize = function (args) {
        ...
        $('#tbDateOpen').kendoDatePicker({ format: "dd/MM/yyyy" });
        $('#tbDateClose').kendoDatePicker({ format: "dd/MM/yyyy" });
        ...
        ctrlContType.on("change", checkContrType);
        ctrlContType.change();
        
        ctrlKv = $('select[name$="ddKv"]');
        ctrlSum = $('input[name$="tbSum"]');
        ctrlCreditTerm = $('select[name$="ddCreditTerm"]');
        ctrlCreditTerm.on("change", checkCreditType);
        ctrlCreditTerm.change();

        $('input[name$="btFormFile"]').on('click', checkDownloadReq);
        $('#cbOldBankConract').on('click', showOldBankReq);
        $('#cbOldDepConract').on('click', showOldDepReq);
        ...
        $('#btSelClient').on("click", function () { searchMode = 0; selectClient(); });
        $('#btSelClientByOkpo').on("click", function () { searchMode = 1; selectClient(); });
        $('#btSelectBenef').on("click", function () { selectBeneficiar(); });
        $('#btSelectBenefBicCode').on("click", function () { selectBeneficiarBicCode(); });

        ctrlOkpo = $('#tbClientOkpo');
        ctrlOkpo.on("change", function () { searchMode = 1; getClientInfo(null, $(this).val()); });

        ctrlRnk = $('#tbClientRnk');
        ctrlRnk.on("blur", function () { searchMode = 0; getClientInfo($(this).val(), null); });

        ctrlBenef = $('#tbBenefId');
        ctrlBenef.on("blur", function () { getBenefInfo($(this).val()); });

        ctrlBenefBicCode.on("blur", function () { getBenefBankInfo($(this).val()); });
        

        $('#btRefresh').on("click", CIM.reloadPage);
        $('#btSave').on("click", saveContract);
        $('#btCancel').on("click", cancelContract);
        $('#btApproveNbu').on("click", approveContract);
        $('#btDiscardNbu').on("click", discardContract);
        $('#btSetOwner').on("click", setOwner);
        $('#btChangeBranch').on("click", setBranch);
    ...
    var ContractClass = {
        ...
        StatusName: null, Comments: null, Bank_change: null, Branch: null, BranchName: null, ServiceBranch: null,
    ...
    var CreditContractClass = {
        ...
        F503_Reason: null, F503_State: null, F503_Note: null, F503_PercentType: null, F503_Base: null, F503_Margin: null,
        F503_Percent: null, F503_Purpose: null, F503_Base_Term: null, p_f503_change_info: null
    }
    ...
    function onPopulateContract(res) {
        ...
        if (res.ContrId) {
            ...
            // Відділення, за яким закріплено контракт
            ctrlBranch.val(res.Branch);

            // Відділення, відповідальне за прийом первинних документів
            ctrlServiceBranch.val(res.ServiceBranch);
            ...
            if (res.TradeContractInfo && res.TradeContractInfo.ContrId) {
                ...
                $('select[name$="ddPercentType"]').val(CreditContractClass.F503_PercentType);
                $('select[name$="ddBasePersentRate"]').val(CreditContractClass.F503_Base); 
                $('#ddF504Reason').val(CreditContractClass.F504_Reason);
                $('#tbF504Note').val(CreditContractClass.F504_Note); 
                $('#tbSizeMargin').val(CreditContractClass.F503_Margin); 
                $('#tbSizePersentRate').val(CreditContractClass.F503_Percent); 
                $('select[name$="ddPurpose"]').val(CreditContractClass.F503_Purpose);
                $('select[name$="ddBasePersentRateTerm"]').val(CreditContractClass.F503_Base_Term);
                $('select[name$="ddInfoAboutChanges"]').val(CreditContractClass.F503_Change_Info);
                // Діючий
    ...
    function checkContrType() {
        ...
        if (contrType == CTYPE.IMPORT || contrType == CTYPE.EXPORT) {
            ...
            actTabs.push(2, 4);
            if (ContractClass.StatusId != 1 && mode != "create" && accessFlag == '0')
                actTabs.push(5);
    ...
    function saveContract() {
        ...
        if ($("#aspnetForm").valid()) {
            ...
            ContractClass.Bank_change = $("#tbInfoOtherBank").val();
            ContractClass.ServiceBranch = ctrlServiceBranch.val();
            ...
            if (ctrlContType.val() == 2) {
                ...
                ContractClass.CreditContractInfo.F503_PercentType = $('select[name$="ddPercentType"]').val();
                ContractClass.CreditContractInfo.F503_Base = $('select[name$="ddBasePersentRate"]').val();
                ContractClass.CreditContractInfo.F503_Margin = $('#tbSizeMargin').val(); 
                ContractClass.CreditContractInfo.F503_Percent = $('#tbSizePersentRate').val(); 
                ContractClass.CreditContractInfo.F503_Purpose = $('select[name$="ddPurpose"]').val(); 
                ContractClass.CreditContractInfo.F503_Base_Term = $('select[name$="ddBasePersentRateTerm"]').val();
                ContractClass.CreditContractInfo.F503_Change_Info = $('select[name$="ddInfoAboutChanges"]').val();
            }
