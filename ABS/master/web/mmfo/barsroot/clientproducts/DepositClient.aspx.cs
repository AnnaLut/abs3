...
{
    protected OracleDataAdapter adapterCountry;
    protected DataSet dsCountry;
    protected OracleDataAdapter adapterClientCodeType;
    protected DataSet dsClientCodeType;
    protected DataSet dsDocType;
    protected OracleCommand cmdSelectDocType;
    protected OracleDataAdapter adapterDocType;
    ...
    private void Page_Load(object sender, System.EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (Request["info"] == null)
            {
                ...
                if (Request["scheme"] == "DELOITTE")
                {
                    ...
                    if (Request["rnk"] != null)
                    {
                        ...
                        textClientRnk.Text = cust_id.ToString();
                        // РНК для сканування документів
                        ScanIdentDocs.RNK = Convert.ToDecimal(cust_id);

                        // РНК для перегляду сканкопій документів
                        EADocsView.RNK = Convert.ToInt64(Request["rnk"].Substring(0, Request["rnk"].Length - 2)); // отсечь хвост ММФО
                        ...
                        */
                        ...
                        //if (Tools.Get_CellPhoneVerifiedState(cust_id))
                        //{                             
                        //    string rnklist = Tools.Get_CellPhoneExists("+38" + textCellPhone.Text, ((Deposit)Session["DepositInfo"]).Client.ID);
                        //    if (rnklist == string.Empty || rnklist == "")
                        //    {
                        //     btContracts.Enabled = true;
                        //     LCellPhone.Visible = false;
                        //}
                        //}
                        //else
                        //{
                        //    btContracts.Enabled = false;
                        //    LCellPhone.Visible = true;
                        //}         
                    }
                    ...
                    DisableControls(); //відключаємо всі контроли для редагування
                }
                # endregion
            ...
            else // info
            {
                ...
                if (Request["scheme"] == "DELOITTE")
                {
                    ...
                    EADocsView.RNK = Convert.ToDecimal(Request["rnk"].Substring(0, Request["rnk"].Length - 2)); // отсечь хвост ММФО
                    EADocsView.Visible = true;
        ...
        else // IsPostBack
        {
            ...
            {
                _dbLogger.Info("Пользователь получил информацию из страницы SearchResults", "deposit");
    ...
    /// </summary>
    private void InitEADControls(Decimal cust_id)
    {
        // РНК для перегляду сканкопій документів
        string rnkstr = Convert.ToString(cust_id);
        EADocsView.RNK = Convert.ToDecimal(rnkstr.Substring(0, rnkstr.Length - 2)); // отсечь хвост ММФО
        EADocsView.Visible = true;
    ...
    private void FillClassFromControls()
    {
        ...
        {
            throw new DepositException(ex.Message);
        }
    ...
    private void btSubmit_ServerClick(object sender, System.EventArgs e)
    {
        ...
        {
            //Client client = ((Deposit)Session["DepositInfo"]).Client;
            //string tcmd = string.Empty;
            //string rnklist = string.Empty;
            //if (Tools.Get_CellPhoneVerState(((Deposit)Session["DepositInfo"]).Client.CellPhone))
            //{
            //    LCellPhone.Visible = false;
            //    rnklist = Tools.Get_CellPhoneExists(((Deposit)Session["DepositInfo"]).Client.CellPhone, ((Deposit)Session["DepositInfo"]).Client.ID);
            //    DBLogger.Info("rnklist = " + rnklist);
            //    if (rnklist == string.Empty || rnklist == "")
            //    {
            ...
            //    else
            //    {
            //        rnklist = "З вказаним номером телефона вже існують клієнти в відділеннях: " + rnklist;
            //        tcmd = "<script>confirm(\"" + rnklist + "\");</script>";
            //        Response.Write(tcmd);
            //        Response.Flush();
            //    }
            //}
        ...
        {
            _dbLogger.Info("Пользователь подтвердил данные о клиенте на странице карточки вкладчика заведения депозитного договора" +
    ...
    private void btRegister_Click(object sender, System.EventArgs e) {
        ...
        _dbLogger.Info("Користувач натиснув кнопку \"Реєструвати\" на сторінці пошук клієнта.", "deposit");

        Response.Redirect("/barsroot/clients/customers/index/?custtype=person");
    }
    /// <summary>
    ...
    {
        _dbLogger.Info("textClientName.Value = " + textClientName.Value, "deposit");

        bool OnBeneficiary = false;//для сообщения о регистарции бенефициара при redirect(не ломает логику. true - бенефициар создан новим клиентом)
        ...
        if (Request.QueryString["scheme"] == "DELOITTE")
        {
            ...
            {
                // client.WriteToDatabase();
                _dbLogger.Info("Користувач оновив інформацію по клієнту №" + client.ID.ToString() +
            ...
            else
            {
                ...
                // client.WriteToDatabase();
                if (Convert.ToString(Session["OnBeneficiary"]) == "YES" || Request["agr_id"] == "12" || Request["agr_id"] == "8" || Request["agr_id"] == "26" || Request["agr_id"] == "27")
                {
                    _dbLogger.Info("Користувач зареєстрував бенефіціара/спадкоємця/малолітню особу/довірену особу/по клієнту №" + client.ID.ToString() +
                   " на сторінці \"Картка Клієнта\" в режимі швидкої реєстрації.", "deposit");
                    FillClassFromControls();
                    client.ClientFastRegistration();

                    OnBeneficiary = true;
                }
                else
                {
                    _dbLogger.Info("Користувач зареестрував клієнта №" + client.ID.ToString() + " на сторінці \"Картка Клієнта\".", "deposit");
                }
            }
        ...
        if (Request["agr_id"] != null)
        {
            _dbLogger.Info("Request[agr_id]= " + Request["agr_id"], "deposit");
            ...
            else
            {
                if (Request["dpt_id"] == null)
                {
                    if ((Session["OnTrustee"]) == "YES" && Request["agr_id"] == "12")
                    {
                        ...
                        {
                            _dbLogger.Info("Обрано клієнта для заведення депозиту по довіреності " + client.ID.ToString(), "deposit");
                ...
                try
                {
                    ...
                    if (TT != String.Empty)
                    {
                        _dbLogger.Info("За оформление доп. соглашения тип=" + Convert.ToString(agr_id) +
    ...
    protected void eadPrintChange_BeforePrint(object sender, System.EventArgs e)
    {
        ...
        string tcmd = string.Empty;
        string rnklist = string.Empty;
        if (Tools.Get_CellPhoneVerState("+38" + textCellPhone.Text))
        {            
            rnklist = Tools.Get_CellPhoneExists("+38" + textCellPhone.Text, ((Deposit)Session["DepositInfo"]).Client.ID);
            if (rnklist == string.Empty || rnklist == "")
            {
                // чистим таблицю 
    ...
    {
        _dbLogger.Info("Користувач поставив відмітку про наявність підпису клієнта (rnk=" + Request.QueryString["rnk"] +
        ...
        _dbLogger.Info("t_OldPhoneNumber =" + t_OldPhoneNumber, "deposit");
        ...
        {
            _dbLogger.Info("Користувач намагався обнулити картку клієнта (rnk=" + Request.QueryString["rnk"] + ").", "deposit");
        ...
        else
        {
            ...
            {              
                ...
                if (rnklist == string.Empty || rnklist == "")
                {
                    ...
                    ClientScript.RegisterStartupScript(this.GetType(), "EditClient_Done", "alert('Зміни в реквізитах клієнта збережені!'); f_DisableEditCliet(); ", true);
                }
                else
                {
                    rnklist = "З вказаним номером телефона вже існують клієнти в відділеннях: " + rnklist;
    ...
    protected void eadFinmonQuestionnaire1_BeforePrint(object sender, EventArgs e)
    {
        // Опитувальний лист фінмоніторингу
        Client client = ((Deposit)Session["DepositInfo"]).Client;
        Decimal RNK = Request.QueryString["rnk"] == null ? client.ID : Convert.ToDecimal(Request.QueryString["rnk"]);
        eadFinmonQuestionnaire1.RNK = RNK;
        eadFinmonQuestionnaire1.TemplateID = "DPT_FINMON_QUESTIONNAIRE";
        eadFinmonQuestionnaire1.EAStructID = 401;        
    }
    protected void eadFinmonQuestionnaire1_DocSigned(object sender, EventArgs e)
    {
        Client client = ((Deposit)Session["DepositInfo"]).Client;
        client = ((Deposit)Session["DepositInfo"]).Client;
    ...
    {
        _dbLogger.Info("Користувач натиснув кнопку \"Портфель договорів\" на сторінці Картка клієнта.", "deposit");
    ...
    {
        _dbLogger.Info("Користувач натиснув кнопку \"Запит на БЕК-офіс\" на сторінці Картка клієнта.", "deposit");
    ...
    protected void ScanIdentDocs_ScanIdDocsDone(object sender, System.EventArgs e)
    {
        ...
        _dbLogger.Info("Користувач завершив сканування ідентифікуючих документів клієнта з РНК = " + cust_id.ToString(), "deposit");
    ...
    private bool clientChanged()
    {
        ...
        if ((dpt.Client.Code != Client.DefaultOKPO()) && (dpt.Client.Code != Client.NonResidentOKPO()))
        {
            ...
            _dbLogger.Debug("Перевірка існування контрагента на сторінці \"Картка клієнта\". ІПН=" +
    ...
    public static String GetNameGenitive(String FirstName, String LastName, String Patronymic, String sex)
    {
        ...
        DbLoggerConstruct.NewDbLogger().Info("sex=" + sex + ",dsex=" + dsex.ToString() + ".", "deposit");
    ...
    public static void DocumentVerified()
    {
        ...
        DbLoggerConstruct.NewDbLogger().Info("Користувач встановив відмітку про перевірку реквізитів ідентифікуючого документу клієнта з РНК=" +
    ...
    /// <summary>
    /// Відключення всіх контролів сторінки для редагування
    /// </summary>
    ...
    /// <summary>
    /// Підключення необхідних контролів в окремих випадках для редагування
    /// </summary>
    protected void EnableControls()
    {
        if (Request["agr_id"] != null)
        {
            ...
            btSubmit.Visible = false;
            btUpdate.Visible = true;
            this.textClientLastName.Disabled = true;
            this.textClientFirstName.Disabled = true;
            this.textClientPatronymic.Disabled = true;
            this.textBirthDate.Enabled = false;
            /* в связи с переходом на регистрацию клиентов исключительно общей формой, отключаю, чтобы неповадно было
            switch (Convert.ToInt32(Convert.ToString(Request["agr_id"])))
            {
                case 5:
                    {
                ...
                case 26:
                    {
                        //lbInfo.Text = "Регистрация клиента (розпорядник по депозитах малолітніх осіб)";
                        this.textClientLastName.Disabled = false;
                        this.textClientFirstName.Disabled = false;
                        this.textClientPatronymic.Disabled = false;
                        this.textBirthDate.Enabled = true;
                        break;
                    }
                case 27:
                    {
