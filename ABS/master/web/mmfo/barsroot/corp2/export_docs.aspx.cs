...
public partial class corp2_export_docs : System.Web.UI.Page
{
    ...
    protected void btExport_Click(object sender, EventArgs e)
    {
        ...
        var queryStr = "select 'card_' || tt || '(' ||ref|| ').rtf', ref, doc_desc from bars.v_corp2_docs v where v.kv != 980 and v.pdat between :startDate and :finishDate + 1";
        if (rbCL.Checked) queryStr = "select 'card_' || tt || '(' ||ref|| ').doc', ref from bars.v_cl_docs v where v.kv != 980 and v.pdat between :startDate and :finishDate + 1";
            ...
            using (OracleCommand command = new OracleCommand())
            {
                ...
                using (var rdr = command.ExecuteReader())
                {
                    while (rdr.Read())
                    {
                        ...
                        var docRef = rdr.GetOracleDecimal(1).Value;
                        byte[] data = null;
                        //Corp2
                        if (rbC2.Checked)
                        {
                        ...
                        {
                            data = new FrxDocHelper(new FastReportModel
                            {
                                ResponseFileType = FrxExportTypes.Word2007,
                                FileName = "CL_SWIFT.frx",
                                Parameters = new FrxParameters
                                {
                                    new FrxParameter("p_doc_ref", TypeCode.Decimal, docRef)
                                }
                            }).GetFileInByteArray();
                        }
