...
<TD title="Дата руху" width="17%"><xsl:attribute name="onclick"><xsl:text>Sort('FDAT')</xsl:text></xsl:attribute>
<xsl:text>Дата руху</xsl:text></TD>

<TD title="Вхідний залишок" width="22%"><xsl:attribute name="onclick"><xsl:text>Sort('ch_OSTF')</xsl:text></xsl:attribute>
<xsl:text>Вхідн. залиш.</xsl:text></TD>

<TD width="18%"><xsl:attribute name="onclick"><xsl:text>Sort('ch_DOS')</xsl:text></xsl:attribute>
<xsl:text>Дебет</xsl:text></TD>

<TD width="19%"><xsl:attribute name="onclick"><xsl:text>Sort('ch_KOS')</xsl:text></xsl:attribute>
<xsl:text>Кредит</xsl:text></TD>

<TD title="Вихідний залишок" width="24%"><xsl:attribute name="onclick"><xsl:text>Sort('ch_OST')</xsl:text></xsl:attribute>
<xsl:text>Вихідн. залиш.</xsl:text></TD>
</TR>
...
<TR>
	...
	<TD align="center" title="Виписка по рахунку" style="CURSOR: hand; COLOR: blue; TEXT-DECORATION: underline">
    <xsl:attribute name="onclick"><xsl:text>OpenAccExtract('</xsl:text><xsl:value-of select="ACC" /><xsl:text>','</xsl:text><xsl:value-of select="CH_FDAT" /><xsl:text>')</xsl:text></xsl:attribute><xsl:value-of select="CH_FDAT" />
  </TD>
