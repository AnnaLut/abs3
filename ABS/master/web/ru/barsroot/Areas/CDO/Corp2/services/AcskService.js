        ...
        function ($http, $q) {
            ...
            var _getRules = function (registrationId) {
                ...
                if (signedData && signedData !== 'canceled') {
                    ...
                    $http.post(bars.config.urlContent('/api/cdo/corp2/UserCertificate/getRules'),
            ...
            var _enrollRequest = function (relCustId, subjectData) {
                ...
                var pkcs10;
                try {
                    pkcs10 = secret.enrollRequest(subjectData.Subject, false, false);
                } catch (e) {
                    state.isLoading = false;
                    deferred.reject({ Message: 'Помилка в VegaCrypto.enrollRequest'});
                    return deferred.promise;
                }
                //debugger;
                var enroll =
                ...
                if (signedData && signedData !== 'canceled') {
                    ...
                    $http.post(bars.config.urlContent('/api/cdo/corp2/UserCertificate/enrollRequest/' + relCustId),
                        { signedData: signedData })
                    .success(function (response) {
                        state.isLoading = false;
                        deferred.resolve(response);
            ...
            var _getSubject = function (registrationId, templateId, providerId) {
                ...
                if (signedData && signedData !== 'canceled') {
                    ...
                    $http.post(bars.config.urlContent('/api/cdo/corp2/UserCertificate/getSubject'),
                        { signedData: signedData })
                    .success(function (response) {
                        state.isLoading = false;
                        deferred.resolve(response);
            ...
            var _getCertificatesStatus = function (relCustId, userId, registrationId, sdo) {
                state.isLoading = true;
                ...
                if (signedData && signedData !== 'canceled') {
                    ...
                    $http.post(bars.config.urlContent('/api/cdo/corp2/UserCertificate/getSertificatesStatus/' + relCustId + '/' + userId),
                        { signedData: signedData })
                    .success(function (response) {
                        state.isLoading = false;
                        deferred.resolve(response);
            ...
            var _sendProfileToAcsk = function (relCustId, custId) {
                ...
                var deferred = $q.defer();
                $http.post(bars.config.urlContent('/api/cdo/corp2/UserCertificate/sendToAcsk/') + relCustId + '/' + custId)
                    .success(function (response) {
                        state.isLoading = false;
                        deferred.resolve(response);
                    }).error(function (response) {
            ...
            var _changeCertificateState = function (relCustId, registrationId, cerialNumber, newId, comment) {
                ...
                if (signedData && signedData !== 'canceled') {
                    ...
                    $http.post(bars.config.urlContent('/api/cdo/corp2/UserCertificate/changeState/' + relCustId),
                        { signedData: signedData })
                    .success(function (response) {
                        state.isLoading = false;
                        deferred.resolve(response);
            ...
            //    var deferred = $q.defer();
            //    $http.post(bars.config.urlContent('/CorpLight/home/parseBase64Crt'), { Base64crt: base64crt })
            //        .success(function (response) {
            //            state.isLoading = false;
            //            deferred.resolve(response);
            //        }).error(function (response) {
