...
public partial class corp2_export_docs : System.Web.UI.Page
{
    ...
    protected void btExport_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            {
                command.CommandText += " and ref=" + tbRef.Text;
            }
            if (!string.IsNullOrEmpty(tbKv.Text))
            {
                command.CommandText += " and kv=" + tbKv.Text;
            }
            if (!string.IsNullOrEmpty(tbUserId.Text))
            {
                command.CommandText += " and userid=" + tbUserId.Text;
            }
            if (!string.IsNullOrEmpty(tbNls.Text))
            {
                command.CommandText += " and nlsa=" + tbNls.Text;
            }
            ...
            while (rdr.Read())
            {
                ...
                try
                {
                    ...
                    var rdrDetail = command.ExecuteReader();
                    if (rdrDetail.Read())
                    {
                    ...
                    rdrDetail = command.ExecuteReader();
                    if (rdrDetail.Read())
                    {
                    ...
                    command.CommandText = "select to_char(dat, 'DD.MM.YYYY'), to_char(dat, 'HH24:MI'), s.fio, substr(s.branch, 2, 6), b.branch, b.name  from bars.oper_visa o, staff$base s, branch b where o.userid=s.id and  s.branch=b.branch and o.ref=:ref and o.status=2 order by dat";
                    rdrDetail = command.ExecuteReader();
                    if (rdrDetail.Read())
                    {
