...
<xsl:text>Номер документа</xsl:text></TD>
<TD width="5%"><xsl:attribute name="onclick"><xsl:text>Sort('OKPO')</xsl:text></xsl:attribute>
<xsl:text>ЕДРПОУ \ іден. код</xsl:text></TD>
<TD width="10%"><xsl:attribute name="onclick"><xsl:text>Sort('NAME')</xsl:text></xsl:attribute>
<xsl:text>Тип контрагента</xsl:text></TD>
<TD width="5%"><xsl:attribute name="onclick"><xsl:text>Sort('B')</xsl:text></xsl:attribute>
<xsl:text>Підпр.</xsl:text></TD>
<TD width="50%"><xsl:attribute name="onclick"><xsl:text>Sort('NMK')</xsl:text></xsl:attribute>
<xsl:text>Найменування</xsl:text></TD>
<TD width="10%"><xsl:attribute name="onclick"><xsl:text>Sort('dat_on')</xsl:text></xsl:attribute>
<xsl:text>Дата реєстрації</xsl:text></TD>
<TD width="10%"><xsl:attribute name="onclick"><xsl:text>Sort('dat_off')</xsl:text></xsl:attribute>
<xsl:text>Дата закриття</xsl:text></TD>
<TD width="10%"><xsl:attribute name="onclick"><xsl:text>Sort('branch')</xsl:text></xsl:attribute>
<xsl:text>Код безбаланс. відділення</xsl:text></TD>  
<TD width="10%"><xsl:text>Статус реквізитів</xsl:text></TD>    
</TR>
...
<xsl:if test="(DAT_ON!='')=true"><xsl:attribute name="style">color:#9933FF</xsl:attribute></xsl:if>  
...
<xsl:attribute name="DAT_ON"><xsl:value-of select="DAT_ON" /></xsl:attribute>  
...
<TD align="left"><xsl:value-of select="REQ_STATUS" /></TD>   
</TR>
</xsl:for-each>
</TABLE>
</xsl:template>
