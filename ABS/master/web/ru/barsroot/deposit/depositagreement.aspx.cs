...
public partial class DepositAgreementPage : Bars.BarsPage
{
    ...
    private void Page_Load(object sender, System.EventArgs e)
    {
        ...
        _dbLogger.Info("Пользователь зашел на страницу оформления доп. соглашений по договору №" +
    ...
    private bool CkUnique()
    {
        ...
        try
        {
            ...
            _dbLogger.Debug("Проверка взможности заключения доп. соглашения тип=" + f_id +
            ...
            if (!rdr.Read())
            {
                _dbLogger.Debug("Выбраное доп. соглашение№" + f_id.ToString() + " по договору №" + dpt_id.ToString() + " формировать можно",
                    "deposit");
            ...
            else
            {
                ...
                if (only_one != 1)
                {
                    _dbLogger.Debug("Выбраное доп. соглашение тип=" + f_id.ToString() + " по договору №" + dpt_id.ToString() + " формировать можно",
                        "deposit");

                    return true;
                }
                _dbLogger.Debug("Выбраное доп. соглашение по договору №" + dpt_id.ToString() + " формировать нельзя: оно уникально и уже сформировано",
                    "deposit");
    ...
    private bool CkCondition()
    {
        ...
        try
        {
            ...
            _dbLogger.Debug("Проверка взможности заключения доп. соглашения тип=" + Convert.ToString(agr_id.Value) +
                " по договору №" + dpt_id, "deposit");
            ...
            */
            ...
            if (rdr.Read())
            {
                if (!rdr.IsDBNull(0))
                {
                    ...
                    {
                        _dbLogger.Debug("Вибрану додп.угоду по договору №" + dpt_id.ToString() + " можна формувати", "deposit");
                        return true;
                    }
                    else
                    {
            ...
            {
                _dbLogger.Debug("Вибрану додп.угоду по договору №" + dpt_id.ToString() + " формувати не можна!", "deposit");
                return false;
            }
        }
        finally
    ...
    /// 1) угода по ф.190 не формується для вкладів без поповнення
       /// </summary>
    private bool CkAdditionCondition()
    {
        OracleConnection connect = new OracleConnection();
        OracleDataReader rdr = null;
        try
        {
            ...
            _dbLogger.Info("CkAdditionCondition Проверка взможности заключения доп. соглашения тип=" + Convert.ToString(agr_id.Value) +
                " по договору №" + dpt_id, "deposit");
            ...
            OracleCommand cmdCheck = connect.CreateCommand();
            cmdCheck.CommandText = @"select nvl(disable_add, 0) 
                                       from dpt_deposit d, dpt_vidd dv
                                      where d.vidd = dv.vidd
                                        and d.deposit_id = :dpt_id";

            cmdCheck.Parameters.Add("dpt_id", OracleDbType.Decimal, dpt_id, ParameterDirection.Input);
    ...
    private void btForm_ServerClick(object sender, System.EventArgs e)
    {
        if (CkUnique())
        {
            ...
            _dbLogger.Info("Пользователь выбрал для формирования доп. соглашение тип=" + agrm_id +
            ...
            if (agrm_id == "25") 
            {
                ...
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "alert_check_condition",
                        "alert('Заборонено формувати дану додаткову угоду для договору №" + dpt_id + ". Договір не передбачає поповнення.')", true);
                    return;
                }
            }
