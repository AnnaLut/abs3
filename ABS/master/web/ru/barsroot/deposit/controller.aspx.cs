...
public partial class Controller : Bars.BarsPage
{
	...
	private void CheckIfEnabled()
	{
		...
		try
		{
			_dbLogger.Debug("Начало проверки: можно ли просматривать эту страницу.",
				"deposit");
			...
			if (result != "1") 
			{
				_dbLogger.Debug("Пользователю просматривать эту страницу нельзя!",
					"deposit");
				Response.Redirect("DepositClient.aspx");
			}

			_dbLogger.Debug("Пользователю можно просматривать эту страницу.",
				"deposit");
		}
		finally	
		{
    ...
    private void FillGrid(bool btSearchPressed)
    {
        ...
        _dbLogger.Info("Пользователь выполнил поиск депозитного договора","deposit");
