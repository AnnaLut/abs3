...
function SetVisa()
{   var rnd = Math.random();
core$IframeBox({ url: "/barsroot/checkinner/default.aspx?type=0", width: 850, height: 600, title: "Візування операцій", callback: "ReloadPage();" });
}
...
function showTerritory()
{
    ...
    if (result != null && result[0] != null)
    {
        ...
        if (isEmpty(document.getElementById('textClientTerritoryReal').value))
            document.getElementById('textClientTerritoryReal').value = result[0];
    }
}

// Вибір коду території (фактична)
function showTerritoryReal() {
    var result = window.showModalDialog('dialog.aspx?type=metatab&tabname=TERRITORY&tail=""&role=WR_CUSTREG', 'dialogHeight:700px; dialogWidth:700px');
    if (result != null && result[0] != null) {
        if (result[0] != null) {
            document.getElementById('textClientTerritoryReal').value = result[0];
        ...
        if (result[1] != null) {
            document.getElementById('textFactSettlement').value = result[1];
        }
        if (result[3] != null) {
            document.getElementById('textFactRegion').value = result[3];
        }
        if (result[4] != null) {
            document.getElementById('textFactDistrict').value = result[4];
        }    
    }
}
...
function searchOwner(form) {
    ...
    alert("Депозит на ім'я РНК =" + document.getElementById("RNK").value + ", Вноситель РНК =" + document.getElementById("RNK_TR").value);
...
function Fill() 
{
  ...
  if (window.dialogArguments.textBirthDate)
    document.getElementById("textClientDate").value = window.dialogArguments.textBirthDate.value;
      ...
      if (window.dialogArguments && window.dialogArguments.dtBirthDate && window.dialogArguments.dtBirthDate.value) {
    document.getElementById("textClientDate").value = window.dialogArguments.dtBirthDate.value;
      }
  if (window.dialogArguments.textDocSerial) {
	document.getElementById("textClientSerial").value = window.dialogArguments.textDocSerial.value;
  }
  if (window.dialogArguments.textDocNumber) {
...
//
function SearchAccountsInt()
{
    var type = "CARDS";
    if (document.getElementById('rbPercentWay_BankACC').checked)
        type = "NOTCARDS";
...
function SearchAccountsRest() {
    debugger;
    var type = "CARDS";
    if (document.getElementById('RBDepoPayWay_BankACC').checked)
        type = "NOTCARDS";    
    var url = "DepositCardAcc.aspx?";
    url += "mfo=" + document.getElementById("MFO").value;
    url += "&okpo=" + document.getElementById("OKPO").value;
...
// Повторение символов в телефонном номере
function RepeatCellNumbers() {
    var user    = document.getElementById('userFIO').value;
    var cellstr = document.getElementById('textCellPhone').value;  
     if (cellstr.charAt(3) != "" && cellstr.charAt(3) == cellstr.charAt(4) && cellstr.charAt(3) == cellstr.charAt(5) && cellstr.charAt(3) == cellstr.charAt(6)
                && cellstr.charAt(3) == cellstr.charAt(7) && cellstr.charAt(3) == cellstr.charAt(8) && cellstr.charAt(3) == cellstr.charAt(9)) {
      var txt = 'Я, ' +  user + ', ' +
        ...
        if (r == false) {
            document.getElementById('textCellPhone').value = "";
            return true;
        }
        else { return false; }
    }
