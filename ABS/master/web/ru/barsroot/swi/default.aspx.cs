...
public partial class swi_default : Bars.BarsPage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {
                InitOraConnection();
                SQL_Reader_Exec("select id, name, cts_url, cts_img, params, SYS_CONTEXT('bars_context', 'user_branch'), docsign.getidoper, (select nvl(val,'00') from params where par='REGNCODE'), to_char(bankdate_g, 'DD.MM.YYYY') transactionBankdate from swi_cts_list order by id");
                int ctsCount = 0;
    ...
    private Table DrawCTSInfo(ArrayList reader)
    {
		...
		string transactionBankdate = Convert.ToString(reader[8]);
		...
		url.Append("&transactionBankdate=" + HttpUtility.UrlEncode(transactionBankdate));
