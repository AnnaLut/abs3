...
public partial class credit_crdsrv_bid_card : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (!IsPostBack)
            {
                // отображаем для ускореного рассмотрения заявки и только для ЦА
                if (1 == cmn.wu.HAD_BID_STATE(Master.BID_ID, "NEW_FT_DATAINPUT") && 1 == cmn.wu.HAS_BID_STATE(Master.BID_ID, "NEW_CA_CREDIT_S"))
                {
                    lbEditBid.Visible = true;
                }
                // отображаем только для ЦА
                if (1 == cmn.wu.HAS_BID_STATE(Master.BID_ID, "NEW_CA_CREDIT_S"))
                {
                    lbCheckService.Visible = true;
                }
                if (Master.SRV_HIERARCHY == "RU")
                {
                    lbPROCCESS.Visible = true;
                }
                if (cmn.wu.HAS_ANSW(Master.BID_ID, "REQUIRED_SRVS") != 1 || 1 != cmn.wu.HAS_BID_STATE(Master.BID_ID, "NEW_CA_CREDIT_S")) 
                {
                    pnlChoiceService.Visible = false;
                }
                // алгоритм для отображения выбраных служб 
                if (cmn.wu.HAS_ANSW(Master.BID_ID, "REQUIRED_SRVS") == 1 && 1 == cmn.wu.HAS_BID_STATE(Master.BID_ID, "NEW_CA_CREDIT_S"))
                {
                    StringBuilder value = new StringBuilder();
                    string[] listService = cmn.wu.GET_ANSW_TEXT(Master.BID_ID, "REQUIRED_SRVS").Split(',');
                    for (var i = 0; i < listService.Length; i++)
                    {
                        WcsRepository repo = new WcsRepository();
                        var service = repo.GetServiceList().GetEnumerator();
                        while(service.MoveNext())
                        {
                            if (service.Current.Id == listService[i])
                            {
                                value.AppendLine(service.Current.Name);
                            }
                        }
                    }
                    lblService.Text = value.ToString().Replace(Environment.NewLine,"<br />");
                }
    ...
    protected void lbEDITBID_Click(object sender, EventArgs e)
    {
        Response.Redirect(String.Format("/barsroot/credit/manager/bid_card.aspx?bid_id={0}&srvhr=CA", Master.BID_ID));
    }
    protected void lbCHKSERVICE_Click(object sender, EventArgs e)
    {
        Response.Redirect(String.Format("/barsroot/wcs/creditservice/CoiceOfServices/?bid_id={0}", Master.BID_ID));
    }
    ...
    protected void lbFinishCcAnalyse_Click(object sender, EventArgs e)
    {
        ...
        */
        ...
        switch (Convert.ToInt32(CCConclusion.Value))
        {
            ...
            case 4:
            case 5:
                # region 0 Прийнято, 4 Прийнято з відкладальною умовою, 5 Прийнято попереднє рішення
                // проверка довведенных данные если была доработка
                if (1 == cmn.wu.HAS_BID_STATE(Master.BID_ID, "NEW_DATAREINPUT_FINISHED"))
                {
                    cmn.wp.BID_STATE_CHECK_IN(Master.BID_ID, "NEW_DATAREINPUT_FINISHED", (String)null);
    ...
    private void CheckStateAccess()
    {
        ...
        switch (Master.SRV_HIERARCHY.ToUpper())
        {
            case "TOBO":
                ...
                break;
            case "RU":
                lbPROCCESS.Enabled = (1 == cmn.wu.HAS_BID_STATE(Master.BID_ID, "NEW_RU_CREDIT_S_PRC") || (1 == cmn.wu.HAS_BID_STATE(Master.BID_ID, "NEW_RU_CREDIT_S_SRVANALYSE"))) && !HasStateDataReInput;
                lbDOCS.Enabled = true;
            ...
            case "CA":
                ...
                lb2SrvsReInput.Enabled = (1 == cmn.wu.HAS_BID_STATE(Master.BID_ID, "NEW_CA_CREDIT_S_SRVANALYSE")) && !HasStateDataReInput && !HasStateSrvsReInput;
                lbEditBid.Enabled = (1 == cmn.wu.HAS_BID_STATE(Master.BID_ID, "NEW_CA_CREDIT_S_PRC"));
                lbCheckService.Enabled = (1 == cmn.wu.HAS_BID_STATE(Master.BID_ID, "NEW_CA_CREDIT_S_PRC"));
                break;
        }
    }
    private Boolean Validate(BbConnection con, Decimal? BID_ID)
    {
        ...
        List<VWcsCrdsrvBidTemplatesRecord> lstTemplates = (new VWcsCrdsrvBidTemplates(con)).SelectCrdsrvBidTemplates(BID_ID, "SRV_" + Master.SRV_HIERARCHY);
        ...
        if (cmn.wu.HAS_ANSW(Master.BID_ID, "REQUIRED_SRVS") != 1 && 1 == cmn.wu.HAS_BID_STATE(Master.BID_ID, "NEW_CA_CREDIT_S"))
        {
            Master.ShowError(String.Format(Resources.credit.StringConstants.text_not_select_service));
            return false;
        }
