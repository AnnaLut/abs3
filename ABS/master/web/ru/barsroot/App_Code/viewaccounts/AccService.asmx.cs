    ...
    public class AccService : Bars.BarsWebService
    {
				...
				try
				{
					...
					if (CodValutes != null)
					{
						...
						for (int i = 0; i < CodValutes.Length; i++)
						{
                            ...
                            //if (sp != null) UpdateSP(acc, sp, result);
                            var tacc = CreateAccountByKv(Convert.ToDecimal(acc),
				    ...
				    {
				        ValidateCustomer(acc, Convert.ToDecimal(gen[4]));
				    }
		...
		private object[] Percent(decimal acc, decimal id)
		{
			...
			try
			{
				...
				object[] reader = SQL_SELECT_reader(@"
					 SELECT metr,basem,basey,freq,stp_dat,acr_dat,apl_dat,tt,acrA,acrB,ttb,mfob,kvb,nlsb,namb,nazn,io,okpo 
        ...
        public void UpdatePercent(decimal acc, string[] data, string newAcc)
		{
			try
			{
				...
				if (count != 0)
					query = "UPDATE Int_accN SET acc=:nAcc,id=:nId,metr=:nMetr,basem=:nBaseM,basey=:nBaseY,freq=:nFreq,stp_dat=:dStpDat,acr_dat=:dAcrDat,apl_dat=:dAplDat,tt=:sTT1,acra=:nAcrA,acrb=:nAcrB,ttb=:sTT2,mfob=:sMFO,kvb=:nKvC,nlsb=:sNlsC,namb=:sNamC,nazn=:sNazn,io=:nIo,okpo=:okpo WHERE acc=:nAcc AND id=:nId";
				else
					query = "INSERT INTO Int_accN (acc,id,metr,basem,basey,freq,stp_dat,acr_dat,apl_dat,tt,acra,acrb,ttb,mfob,kvb,nlsb,namb,nazn,io,okpo) VALUES (:nAcc,:nId,:nMetr,:nBaseM,:nBaseY,:nFreq,:dStpDat,:dAcrDat,:dAplDat,:sTT1,:nAcrA,:nAcrB,:sTT2,:sMFO,:nKvC,:sNlsC,:sNamC,:sNazn,:nIo,:okpo)";
