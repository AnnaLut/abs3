...
namespace BarsWeb.Areas.Sep.Controllers
{
    ...
    public class SepTZController : ApplicationController
    {
        ...
        public ActionResult GetSepTZList([DataSourceRequest] DataSourceRequest request, AccessType accessType)
        {
            IQueryable<SepTZ> tzList = _repoTZ.GetSepTZList(request, accessType);
            decimal total = _repoTZ.GetSepTZCount(accessType, request);

            return Json(new { Data = tzList, Total = total }, JsonRequestBehavior.AllowGet);
        }
        public ActionResult DeleteRow(int rowREC)
        {
        ...
        public ActionResult GetZagA(string arcFn, string arcDate_a)
        {
            var result = new JsonResponse(JsonResponseStatus.Ok);
