...
namespace BarsWeb.Areas.Cdm.Infrastructure.DI.Implementation.PrivateEn
{
    public class CdmPrivateEnRepository : ICdmPrivateEnRepository
    {
        ...
        private const string _logMessagePrefix = "ЕБК-ФОП.";
        ...
        private List<RelatedPerson> ExtractRelatedPersonList(decimal? rnk)
        {
            var result = _entities.ExecuteStoreQuery<RelatedPerson>(
                "select * from V_EBKC_PRIVATE_ENT_REL where rnk = bars.ebk_wforms_utl.cut_rnk(:p_Rnk)", rnk).ToList();
            return result;
        }
        ...
        private PrivateEnPerson MapPrivateEnData(PrivateEnData planeBody)
        {
