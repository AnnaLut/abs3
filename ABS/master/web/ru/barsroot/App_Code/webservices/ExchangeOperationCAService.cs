...
public class ExchangeOperationCAService : BarsWebService
{
        ...
        string operid_nokk, decimal? req_type, string vdate_plan, decimal? custtype)
    {
        ...
        try
        {
            ...
            cmd.Parameters.Add(new OracleParameter("p_vdateplan", OracleDbType.Date) { Value = ParseDateFromString(vdate_plan, "dd.MM.yyyy") });
            cmd.Parameters.Add(new OracleParameter("p_custtype", OracleDbType.Decimal) { Value = custtype });
            cmd.ExecuteNonQuery();
    ...
    public Response<string> SetCurrency(string currentMfo, string currentDate, string xmlCurrencyIncome)
    {
        ...
        try
        {
            ...
            cmd.Parameters.Add(new OracleParameter("p_mfo", OracleDbType.Varchar2) { Value = currentMfo });
            cmd.Parameters.Add(new OracleParameter("p_date", OracleDbType.Varchar2) { Value = currentDate });  //OracleDbType.Date) { Value = ParseDateFromString(p_date, "dd.MM.yyyy H:mm:ss") });
            cmd.Parameters.Add(new OracleParameter("p_currency_clob", OracleDbType.Clob) { Value = xmlCurrencyIncome });
            cmd.ExecuteNonQuery();
            return new Response<string>
            {
                Status = "ok",
