...
namespace clientregister
{
    ...
    public class defaultWebService : Bars.BarsWebService
    {
        ...
        /// <returns></returns>
        [WebMethod(EnableSession = true)]
        public string CheckUserAcessMode()
        {
            string res = string.Empty;
            ...
            using (OracleConnection con = OraConnector.Handler.IOraConnection.GetUserConnection())
            {
                using (OracleCommand cmd = con.CreateCommand())
                {
                    try
                    {
                        cmd.CommandText = "kl.get_customer_field_access";
                        cmd.CommandType = CommandType.StoredProcedure;

                        cmd.Parameters.Add("p_return", OracleDbType.Varchar2, ParameterDirection.ReturnValue);
                        cmd.Parameters["p_return"].Size = 200;
                        cmd.Parameters.Add("p_field_code", OracleDbType.Varchar2, 100, "TEL_MOBILE", ParameterDirection.Input);

                        if (con.State != ConnectionState.Open) con.Open();
                        cmd.ExecuteNonQuery();

                        res = cmd.Parameters["p_return"].Value.ToString();
                    }
                    catch (Exception ex)
                    {
                        res = ex.Message;
       ...
       private void UpdateAccPhones(string oldPhone, string newPhone, decimal rnk)
        ...
        [WebMethod(EnableSession = true)]
        public int ClearKeysInSession(string[] keysForDel)
        {
            List<string> sl = new List<string>();
            int count = 0;
            foreach (string key in Session.Keys)
            {
                foreach (string keyForDel in keysForDel)
                {
                    if (key.ToString().StartsWith(keyForDel))
                    {
                        sl.Add(key);
                    }
                }
        ...
        {
            string photo = fromCache ? GetPhotoFromSession(rnk) : "";
        ...
        string GetPhotoFromSession(decimal rnk)
        {
            byte[] bytes = (byte[])Session[string.Format("BYTES_IMAGE_DATA_KEY_{0}", rnk.ToString())];
        ...
        public ValidationResult RegisterDbo(decimal rnk)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    cmd.Parameters["p_val"].Value = DateTime.Now.ToString("dd.MM.yyyy");
                    cmd.ExecuteNonQuery();
                }
            }
            catch (Exception ex)
        ...
        public void WriteImageToDatabase(string rnk)
        {
            ...
            if (imageId != null)
            {
                ...
                SaveImage(PhotoType, userPic, rnk);
                Session.Remove(imageId);
            }
            else if (imageByteId != null)
            {
                // get image from byte[]  (like uploaded in browser - jpg or png formats)                
                byte[] bytes = (byte[])Session[imageByteId];
                SaveImage(PhotoType, bytes, rnk);
                Session.Remove(imageByteId);
            }
        }
        ...
        private void SaveImage(string imgType, byte[] imgData, string rnk)
        {
            ...
            try
            {
                ...
                cmdDelete.Parameters.Add("p_rnk", OracleDbType.Int64, clientRnk, System.Data.ParameterDirection.Input);
                ...
                cmdInsert.Parameters.Add("p_rnk", OracleDbType.Int64, clientRnk, System.Data.ParameterDirection.Input);
                ...
                //Отправка данных на CM

                cmdSql.CommandType = CommandType.StoredProcedure;
                cmdSql.CommandText = "bars.ow_utl.get_nd";
                cmdSql.Parameters.Add("p_rnk", OracleDbType.Int64, clientRnk, ParameterDirection.Input);
        ...
        public void CheckRelationCustomer(string nmk = null, decimal? rnk = null, decimal? relRnk = null, decimal? relIntext = null, decimal? relId = null)
        {
            string publicFlagMessage = null;
            ...
            if (rnk != null)
            {
                ...
                OracleConnection con = OraConnector.Handler.IOraConnection.GetUserConnection();
                OracleCommand cmdSelectCustRel = con.CreateCommand();

                try
                {


                    if (relId != 32 || relId != 36 || relId != 37 || relId != 38 || relId != 39 || relId != 50)
                    {
                        cmdSelectCustRel.CommandText = @"select nvl(max(c.rel_id), 0) as res from customer_rel c
                                    where c.RNK = :RNK
                                    and c.REL_RNK = :REL_RNK
                                    and c.REL_INTEXT = :REL_INTEXT
                                    and c.REL_ID in (32, 36, 37, 38, 39, 50)";

                        cmdSelectCustRel.Parameters.Add("RNK", OracleDbType.Decimal, rnk, ParameterDirection.Input);
                        cmdSelectCustRel.Parameters.Add("REL_RNK", OracleDbType.Decimal, relRnk, ParameterDirection.Input);
                        cmdSelectCustRel.Parameters.Add("REL_INTEXT", OracleDbType.Decimal, relIntext, ParameterDirection.Input);


                        if (con.State != ConnectionState.Open) con.Open();

                        res = (decimal)cmdSelectCustRel.ExecuteScalar();

                        if (res == 0)
                        {
                            OracleCommand cmdSelectCustExtern = con.CreateCommand();
                            string relName;

                            cmdSelectCustExtern.CommandText = @"select c.NAME from CUSTOMER_EXTERN c where c.ID = :REL_RNK";
                            cmdSelectCustExtern.Parameters.Add("REL_RNK", OracleDbType.Decimal, relRnk, ParameterDirection.Input);

                            relName = (string)cmdSelectCustExtern.ExecuteScalar();

                            if (!String.IsNullOrEmpty(relName))
                                publicFlagMessage = GetPublicFlag(relName);
                        }
            ...
            {
                publicFlagMessage = GetPublicFlag(nmk);
            }

            if (!String.IsNullOrEmpty(publicFlagMessage))
                Session["publicFlagMessage"] = publicFlagMessage;
        }

        /// <summary>
        /// class for general name input info
        /// </summary>
