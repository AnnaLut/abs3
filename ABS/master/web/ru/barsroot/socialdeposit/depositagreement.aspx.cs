...
public partial class DepositAgreement : Bars.BarsPage
{
	...
	private void Page_Load(object sender, System.EventArgs e)
	{
		...
		_dbLogger.Info("Пользователь зашел на страницу оформления доп. соглашений по договору №" + dpt_id.ToString(),
			"SocialDeposit");
	...
	private bool CkUnique()
	{
		...
		try
		{
			...
			_dbLogger.Debug("Проверка взможности заключения доп. соглашения тип=" + f_id.ToString() +
			...
			if (!rdr.Read())
			{
                rdr.Close();
                rdr.Dispose();
				_dbLogger.Debug("Выбраное доп. соглашение№" + f_id + 
				...
				if (only_one != 1)
				{
					_dbLogger.Debug("Выбраное доп. соглашение тип=" + f_id + " по договору №" + 
                        dpt_id + " формировать можно","SocialDeposit");

					return true;	
				}
				_dbLogger.Debug("Выбраное доп. соглашение по договору №" + dpt_id + 
