...
CIM.pay_module = function () {
    ...
    function OnCheckForVisa(result) {
        if (result.Code > 0) {
            ...
            core$WarningBox(msg, "Візування документу", function () { checkReload(); });
        } else {
            core$IframeBox({ url: "/barsroot/checkinner/documents.aspx?type=1&grpid=" + visaId, width: 850, height: 400, title: "Візування документу ref=" + selRowData.rf, callback: "curr_module.CallbackSetVisa", afterloadFunc: "curr_module.SetCheckiner" });
        }
    }
    ...
    function OnCheckForVisaWithSend(result) {
        if (result.Message) {
            ...
            core$IframeBox({ url: "/barsroot/checkinner/documents.aspx?type=1&grpid=" + visaId, width: 900, height: 500, title: "Візування документу ref=" + selRowData.rf, callback: "curr_module.CallbackSetVisa", afterloadFunc: "curr_module.SetCheckiner" });
        }
        if (result.DataStr) {
            $("#ifDownload").remove();
    ...
    function OnDocBackVisa(result) {
        if (result.Code > 0) {
            ...
            core$IframeBox({ url: "/barsroot/checkinner/documents.aspx?type=1&grpid=" + visaId, width: 900, height: 550, title: "Повернення документу ref=" + selRowData.rf, callback: "curr_module.CallbackSetVisa", afterloadFunc: "curr_module.SetCheckinerBack" });
        }
    }

    function setCheckinerBack(iframeDocument) {
        if (iframeDocument && iframeDocument.getElementById("bt_PutVisa")) {
