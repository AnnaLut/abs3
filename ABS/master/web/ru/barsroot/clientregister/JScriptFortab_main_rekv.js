...
//check n set sex of custumer by ed_OKPO
function isMale(strOKPO) {
    return parseInt(strOKPO.substr(strOKPO.length - 2, 1)) % 2 !== 0;
}

function isPersonRez() {
    var rezId = $get('ddl_CODCAGENT').item($get('ddl_CODCAGENT').selectedIndex).value.substr(0, 1); //1-резидент; 2-нерезидент
    var custType = parent.obj_Parameters['CUSTTYPE'];//   person/corp
    return custType == 'person' && rezId == 1;
}
function isCreatingCust() {
    return getParamFromUrl('rnk', parent.document.location.href) === "";
}

function setSexByOKPO() {
    if (isCreatingCust()) { 
        if (isPersonRez()) {
            var ddlSex = gE(getParantFrame('Tab3'), 'ddl_SEX');
...
$(document).ready(function () {
    ...
    // перевірка заповненості По-Балькові користувача фо резидента
