...
<script type="text/javascript">
        $(document).ready(function () {
            var publicFlagMessage = '<%= Session["publicFlagMessage"] %>';
                                                            ...
                                                            <asp:View ID="v0" runat="server">
                                                                <table border="0" cellpadding="3" cellspacing="0">
                                                                    ...
                                                                    <tr>
                                                                        ...
                                                                        <td>
                                                                            <bars:TextBoxString ID="NAME" runat="server" Width="300px" SetFocusOnError="true" IsRequired="true" ValidationGroup="Params" Value='<%# Bind("NAME") %>' />
                                                                        </td>
                                                                    </tr>
                                                                    <tr>
                                                                        <td>
                                                                    ...
                                                                    <tr>
                                                                        ...
                                                                        <td>         
                                                                            <bars:TextBoxString ID="OKPO" runat="server" MaxLength="10" IsRequired="true" SetFocusOnError="true" ValidationGroup="Params"
                                                                                Value='<%# Bind("OKPO") %>' />
                                                                        </td>
                                                                    </tr>
                                                                    <tr>
    ...
    <script type="text/javascript">
        function remItemsFromDropdown(arrItems) {
            for (var i in arrItems) {
                ...
                $("option:contains(" + arrItems[i] + ")").remove();
            }
        }

        document.onmouseover = function () {
            if (document.location.href.indexOf('client=') >= 0) { 
                if (document.location.href.split('client=')[1].split("&")[0] == "corp") {            
                    //get ur of fiz id from table`s seleted row
                    var complid = $(".selectedRow")[0].children[2].innerHTML;
                    if (typeof complid !== 'undefined') {                    
                        if (complid == 2) { 
                            var rem_id = [
                                "Відокремлений підрозділ (філія)",//9,   
                                "Материнська компанія, холдинг, об'єднання, членом якого є клієнт (ЮО)",//8,  
                                "Представництво"]; //10]; 
                            remItemsFromDropdown(rem_id);
                        }
                        if (complid == 1) {  
                            var rem_id = [
                                "Член органу управління", // 31, 
                                "Керівник", // 40, 
                                "Контролер", // 29,  
                                "Контролер фізичної особи", // 34,  
                                "Посадова особа органу управління (ЮО)", // 26, 
                                "ФО, уповноважена представляти iнтереси акцiонерiв (учасників) клiєнта (ЮО)", // 7,   
                                "ФО, яка відкриває рахунок на ім'я клієнта (ФО)", // 35, 
                                "ФО, яка має право розпоряджатися рахунками і майном/уповноважена діяти від імені клієнта"]; // 20];
                            remItemsFromDropdown(rem_id);
                        }
                    }
                    // block Вид екон. дiяльностi (K110): 000000
                    if ($("#ctl00_body_fv_VED_tb").val() == '00000') {
                        alert('Недопустиме значення "Код виду економ. діяльності (К110) для ЮО" ');
                        $("#ctl00_body_fv_VED_tb").val("");
                    }
                }
            }
        };
