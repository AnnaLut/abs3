...
function Check_MainRekv() {
    ...
    if (obj_Parameters['CUSTTYPE'] === 'person' && !isCustomerSpd()) {
        ...
        //COBUSUPABS-4992 перевірка на обов'язковість повинна бути відключена
...
function Check_ClientRekvPerson() {
    ...
    if (gE(curTab, 'ckb_main').checked) {
        ...
        //COBUSUPABS-6007 поступні і цифри при введенні
        //if (isDocPassport && (organ.length < 10 || !/^[а-яА-Яа-яА-ЯіІїЇєЄ]{0,1}[а-яА-Яа-яА-ЯіІїЇєЄ\-\`\'\.\s]{1,255}$/.test(organ.toUpperCase()))) {
        if (isDocPassport && (organ.length < 10 || !/^[а-я0-9А-Яа-яА-ЯіІїЇєЄ]{0,1}[а-я0-9А-Яа-яА-ЯіІїЇєЄ\-\`\'\.\s]{1,255}$/.test(organ.toUpperCase()))) {
...
function Check_ClientRekvPhone(ignoreConfirmation) {
    if (gE(getFrame('Tab0'), 'ed_OKPO').value.length < 2) {
        alert("Відсутній ідентифікаційний код");
        return false;
    }
...
function Register() {
    locked = false;
      ...
      //При створенні 
    if (obj_Parameters['EditType'] == 'Reg' && obj_Parameters['CUSTTYPE'] == 'person' && getParamFromUrl('spd', document.location.href) != 1) {
       
        var rezId = getParamFromUrl('rezid', document.location.href);
        if (rezId == 1 && gE(getFrame('Tab2'), 'ed_ISE').value == '00000') {
            alert('Параметр К070 не може приймати значення "00000" для фізичної особи!');
            return false;
        }
    }

    //При редагуванні 
    if (obj_Parameters['EditType'] == 'ReReg') {
        if (gE(getFrame('Tab2'), 'ed_K050').value == '000' && obj_Parameters['CUSTTYPE'] == 'corp') {
            alert(' Заборонено змінювати значення для K050 000 для Юридичних осіб! Потрібно зареєструвати нового клієнта!');
            return false;
        }
       
    }

    //Заведення або редагування
    if (gE(getFrame('Tab2'), 'ed_VED').value == '00000' && obj_Parameters['CUSTTYPE'] != 'person') {
        alert('Параметр К110 не може приймати значення "00000" для НЕ фізичної особи!');
        return false;
    }

    if (gE(getFrame('Tab2'), 'ed_K050').value == '000' && obj_Parameters['CUSTTYPE'] != 'person') {
        alert('Параметр К050 не може приймати значення "000" для НЕ фізичної особи!');
        return false;
    }

    //Перевірка на закритого клієнта
    if (obj_Parameters['EditType'] == 'ReReg' && gE(getFrame('Tab0'), 'ed_DATE_OFF').value.length>0) {
        alert("Заборонено редагування закритого клієнта");
        return false;
    }
