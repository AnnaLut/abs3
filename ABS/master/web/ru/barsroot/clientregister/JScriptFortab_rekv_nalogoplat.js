...
function GetC_dstList() {
    var codRegElem = document.getElementById('ddl_C_REG');
    var codReg = codRegElem.selectedIndex;
	if (codReg != 0)
	{		
		$.ajax({
		    type: "GET",
		    contentType: 'application/json; charset=UTF-8',
		    url: '/barsroot/api/reference/handbook/SPR_REG?clause=c_reg=' + codRegElem.item(codReg).value,
		    async: false,
		    dataType: 'json',
        ...
        input.C_reg = codRegElem.item(codReg).value;
		input.val = new Array();
		input.txt = new Array();
