...
namespace Bars.DWH
{
    ...
    {
        [DataMember] public decimal PackageId;

        [DataMember] public decimal PackageType;
        ...
        public LogData(decimal PackageId, decimal PackageType, string BankDate, string PackageData,
            ref DWHResponse response, OracleConnection con)
        {
        ...
        public void Save(OracleConnection con, ref DWHResponse response)
        {
            ...
            try
            {
                ...
                cmd.BindByName = true;
                cmd.CommandText =
                    "insert into DWH_LOG(package_id, package_data, package_status, package_type, bank_date)" +
                    " values (:p_ID ,:p_DATA, :p_STATUS, :p_PACKAGETYPE, :p_BANKDATE)";
                cmd.Parameters.Add(new OracleParameter("p_ID", OracleDbType.Decimal, PackageId, ParameterDirection.Input));
                cmd.Parameters.Add(new OracleParameter("p_BANKDATE", OracleDbType.Varchar2, BankDate.Substring(0, 10),
                    ParameterDirection.Input));
                cmd.Parameters.Add(new OracleParameter("p_DATA", OracleDbType.Clob, clob, ParameterDirection.Input));
                cmd.Parameters.Add(new OracleParameter("p_STATUS", OracleDbType.Varchar2, PackageStatus,
                    ParameterDirection.Input));
                cmd.Parameters.Add(new OracleParameter("p_PACKAGETYPE", OracleDbType.Decimal, PackageType,
                    ParameterDirection.Input));
                cmd.ExecuteNonQuery();
                clob.Close();
                clob.Dispose();
    ...
    public class DWHService : BarsWebService
    {
                            ...
                            String DeliveryRegion*/, int MFO, string BankDate, string MD5, string Data)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(Data))
                {
                    var check = CheckData(PacketNumber, PreviousPacketNumber, PacketType, MFO, ref response);
                    if (check)
                    {
                        var log = new LogData(PacketNumber, Convert.ToDecimal(PacketType), BankDate, Data, ref response,
                            con);
