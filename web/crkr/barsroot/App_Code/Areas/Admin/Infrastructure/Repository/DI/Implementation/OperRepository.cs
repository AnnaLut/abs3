...
namespace BarsWeb.Areas.Admin.Infrastructure.Repository.DI.Implementation
{
    public class OperRepository : IOperRepository
    {
        ...
        public IEnumerable<TTS> OperData(DataSourceRequest request)
        {
            InitOperDataQuery();
        ...
        public void UpdateOperItem(TTS item)
        {
            ...
            var parameters = new object[]
            {
                ...
                new OracleParameter("p_kv", OracleDbType.Decimal) { Value = item.KV },
                new OracleParameter("p_nlsk", OracleDbType.Varchar2) { Value = item.NLSK },
                new OracleParameter("p_kvk", OracleDbType.Decimal) { Value = item.KVK },
                ...
                new OracleParameter("p_flags", OracleDbType.Varchar2) { Value = item.FLAGS },
        ...
        public IEnumerable<TTS_OpFlags> FlagsData(DataSourceRequest request, int[] code, string tt)
        {
            InitFlagsDataQuery(code, tt);
            var query = _sqlTransformer.TransformSql(_flagsDataQuery, request);
            var result = _entities.ExecuteStoreQuery<TTS_OpFlags>(query.SqlText, query.SqlParams);
            return result;
        }
        ...
        private void InitFlagsDataQuery(int[] code, string tt)
        {
            ...
            _flagsDataQuery = new BarsSql()
            {
                ...
                SqlText = string.Format(@"
                    ...
                    WHERE ( (a.fcode=b.code) ) AND (a.tt=:p_tt)
                    ORDER BY fcode"),
                SqlParams = new object[] { new OracleParameter("p_tt", OracleDbType.Varchar2) { Value = tt } }
            };
        }
        ...
        private void InitFlagsOutDataQuery(int[] code)
        {
            ...
            {
                SqlText = string.Format(@"select * from flags where CODE NOT IN ({0})", param),
                SqlParams = new object[] { }
            };
        }
        ...
        private void InitMonitoringGroupsQuery(string tt)
        {
            ...
            {
                SqlText = string.Format(@"
                    SELECT a.idchk IDCHK, b.name NAME, NVL(a.f_in_charge,4) F_IN_CHARGE, a.priority PRIORITY, a.sqlval SQLVAL, NVL(bitand(a.flags,1),0) FLAGS
