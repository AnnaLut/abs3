...
/// <summary>
	...
	public partial class DepositContract : Bars.BarsPage
	{
		...
		protected void btnBack_ServerClick(object sender, System.EventArgs e)
		{
            if (Request.Params.Get("dpt_id") != null)
            {
                _dbLogger.Debug("Користувач натиснув на кнопку \"Назад\" на сторінці карточки депозиту и повернувся на сторінку пошуку.",
                    "SocialDeposit");
                Response.Redirect("DepositSearch.aspx");
            }
            else
            {
                _dbLogger.Debug("Користувач натиснув на кнопку \"Назад\" на сторінці вибору типу депозиту и повернувся на карточку клієнта.",
                    "SocialDeposit");
		...
		protected void btnSubmit_ServerClick(object sender, System.EventArgs e)
		{
            ...
            if (Request.Params.Get("dpt_id") != null)
            {
                _dbLogger.Info("Користувач змінює данні депозитного договору пенсіонерів та безробітних." +
                    "Договір № " + dpt.ID,
                    "SocialDeposit");
            ...
            {
                _dbLogger.Info("Користувач починає відкивати депозитний договір пенсіонерів та безробітних." +
