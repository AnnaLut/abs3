...
public partial class ins_partner_type_attrs : System.Web.UI.Page
{
    ...
    protected void lv_ItemInserting(object sender, ListViewInsertEventArgs e)
    {
        ...
        var key = ip.SET_PARTNER_TYPE_ATTR((Decimal?)null, ATTR.SelectedValue, PARTNER.Value, TYPE.Value, IS_REQUIRED.Value);
        if (key == null) 
        {
            var message = @"barsUiError('Запис з такими параметрами вже існує!');";
            ScriptManager.RegisterStartupScript(this, GetType(), "ErrorPayMessage", message, true);
        }
        lv.InsertItemPosition = InsertItemPosition.None;
        lv.DataBind();
        e.Cancel = true;
