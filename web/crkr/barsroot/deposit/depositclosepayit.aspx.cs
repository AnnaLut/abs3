...
public partial class DepositClosePayIt : Bars.BarsPage
{
    ...
    private void Page_Load(object sender, System.EventArgs e)
    {
        ...
        if (Tools.DPT_IRREVOCABLE(dpt_id.Value) == 1)
            LabelIRR.Visible = true;
        else
            LabelIRR.Visible = false;
    ...
    private void FillControls(bool getShtraf, decimal sum2pay, decimal perc2pay, decimal p_int2pay_ing)
    {
        ...
        if (sum2pay < 0)
        {
            ...
            try
            {
                ...
                {
                    _dbLogger.Info("Договор расторгать(делать частичное снятие) пришло довереное лицо." +
       ...
       private void btPay_ServerClick(object sender, System.EventArgs e)
    {
        ...
        {
	    _dbLogger.Info("Переход на выбор операции по выплате процентов");
            btPay.Disabled = true;
            btPayPercent.Enabled = true;
            btCancel.Enabled = false;
            SumToPay.Enabled = false;
	...
	{
	    _dbLogger.Info("Выход, сумма процентов к выплате нуль");
            Response.Redirect("DepositCloseComplete.aspx?dpt_id=" + Convert.ToString(Request["dpt_id"]));
	}
    }
    /// <summary>
    ...
    {
        _dbLogger.Info("ВНИМАНИЕ!!!Пользователь выбрал расторжение депозитного договора №" +
