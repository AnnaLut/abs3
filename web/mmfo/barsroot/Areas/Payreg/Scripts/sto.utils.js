...
bars.utils.sto = bars.utils.sto || {
    ...
    detailInit: function (e) {
        ...
        $("<div/>").appendTo(e.detailCell).kendoGrid({
            ...
            columns: [
                ...
                {
                    field: "PAYMENT_STATE", title: "Статус обробки ", width: "100px",
                    headerAttributes: {
                        style: "white-space: normal;"
                    }
                }
    ...
    // inga    
    showOrderHist: function () {
        var grid = $('#cust_orders').data('kendoGrid');
        var row = grid.dataItem(grid.select());
        bars.utils.sto.currentOrderId = row.ID;
        var wnd = $("#wnd_order_hist").data("kendoWindow");
        bars.utils.sto.prepareHistWindow();
        wnd.center().open();
    },
    prepareHistWindow: function () {
        var self = bars.utils.sto;       
            $('#HistMasterGrid').kendoGrid({              
                dataBound: function () {
                    this.expandRow(this.tbody.find("tr.k-master-row").first());
                },
                dataSource: {
                    ...
                    schema: {
                        ...
                        model: {
                            fields: {
                                ...
                                SYS_TIME: { type: "string" }
                            }
                        }
                    },
                    transport: {
                        read: {
                            url: bars.config.urlContent('/Payreg/Payreg/GetOrderHist'),
                            data: self.getCurrentOrder
                        }
                    },                   
                },               
                columns: [                    
                            ...
                            {
                            field: "COMMENT_TEXT",
                            title: "Опис зміни ордера"
                            },
                            {
                            field: "SYS_TIME",
                            title: "Дата і час зміни",                           
                            width: "120px"
                            }
                ]
            });
...
bars.utils.sto.onSbonProviderSelectAll = function (extraData) {   
    var data = this.dataItem(this.select());
    if (data != null) {
        var currentSbonWorkMode = data.WORK_MODE_ID;
        bars.utils.sto.refreshSbonForm(currentSbonWorkMode);
...
bars.utils.sto.refreshSbonForm = function (workMode) {
    switch (workMode) {
        case 1:
            ...
            bars.utils.sto.refreshFiled('sbonSendSmsLi', true, true);
            break;
        case 2:
...
bars.utils.sto.getSepData = function () {
    ...
    return {
        ...
        SendSms: $sepFrm.find('input[name=sepSendSms]:checked').val()
    }
};
...
bars.utils.sto.getRecieverNMS = function () {    
        $.ajax({
            type: "POST",
            url: bars.config.urlContent('/Payreg/Payreg/GetRecieverAccountNMS'),
            data: { MFO: document.getElementById("sbonReceiverMfo").value, NLS: document.getElementById("sbonReceiverAccount").value},
            success: function (res) {
                if (res.status != 'ok') {
                    document.getElementById("sbonReceiverName").value = "";                    
                } else {                    
                    document.getElementById("sbonReceiverName").value = res.data;
                }
            },
            error: bars.utils.sto.errorFunc
        }); 
...
bars.utils.sto.getRecieverOKPO = function () {
    $.ajax({
        type: "POST",
        url: bars.config.urlContent('/Payreg/Payreg/GetRecieverAccountOKPO'),
        data: { MFO: document.getElementById("sbonReceiverMfo").value, NLS: document.getElementById("sbonReceiverAccount").value },
        success: function (res) {
            if (res.status != 'ok') {
                document.getElementById("sbonReceiverEdrpou").value = "";
            } else {
                document.getElementById("sbonReceiverEdrpou").value = res.data;
            }
        },
        error: bars.utils.sto.errorFunc
    });
...
bars.utils.sto.refreshOrdersToolbar = function () {
    ...
    bars.utils.sto.enableStoButton("view_order_hist", row );    
    bars.utils.sto.enableStoButton("preview_closed_order", row);
    bars.utils.sto.enableStoButton("confirm_order", row && row.ORD_STATE < 3);
