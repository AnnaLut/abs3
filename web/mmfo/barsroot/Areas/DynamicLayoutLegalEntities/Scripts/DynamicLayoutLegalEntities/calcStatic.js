...
function SearchAndUpdateHeaders() {
    $.ajax({
        type: "GET",
        url: bars.config.urlContent("/api/DynamicLayoutLegalEntities/DynamicLayoutLegalEntities/SearchTmpDynamicLayout"),
...
function initMainGrid() {
    ...
    var dataSourceObj = {
        ...
        transport: {
            read: {
                type: "GET",
                dataType: "json",
                url: bars.config.urlContent("/api/DynamicLayoutLegalEntities/DynamicLayoutLegalEntities/SearchStaticLayoutData")
            }
        },
        ...
        sort: ([
            ...
            {
                field: "DELTA", dir: "desc", compare: function (a, b) {
                    if (a.DELTA < 0 && b.DELTA > 0) return -1;
                    if (a.DELTA > 0 && b.DELTA < 0) return 1;

                    return a.DELTA === b.DELTA ? 0 : ((Math.abs(a.DELTA) > Math.abs(b.DELTA)) ? 1 : -1);
                }
            },
    ...
    var mainGridOptions = {
        ...
        columns: [
            ...
            { field: "NAMA", title: "Назва відправника", hidden: session.isA, width: "200px" },
            { field: "MFOB", title: "Код МФО-Б", width: "100px" },
            { field: "MFOB_NAME", title: "Назава МФО-Б", width: "200px" },
            { field: "NLS_B", title: "Рах.Б в МФО-Б", width: "120px" },
            { field: "NAMB", title: "Назва отримувача", width: "200px" },
...
function editForm(dataItem) {
            ...
            .find("#btnSave")
                .click(function () {
                    ...
                    if (!v.result) {
                        ...
                        $.ajax({
                            type: "POST",
                            url: bars.config.urlContent("/api/DynamicLayoutLegalEntities/DynamicLayoutLegalEntities/SaveDetails"),
    ...
    if (totalSum) {
        var numerictextbox = $("#summ_a_editor").data("kendoNumericTextBox");
        numerictextbox.enable(false);
    } else {
        kendoWindow.find("#helpMsg").hide();
    }
    ...
    if (!session.isA) {
        $("#kv_editor").kendoDropDownList({
            ...
            dataSource: {
                ...
                transport: {
                    read: {
                        url: bars.config.urlContent("/api/DynamicLayoutLegalEntities/DynamicLayoutLegalEntities/KvAC"),
                        dataType: "json"
                    }
                }
            }
...
function totalSumChange() {
    ...
    var zal = +$("#tbOstC").val();
    if (value > zal) {
        bars.ui.alert({ text: "Не можливо ввести суму більшу, ніж сума фактичного залишку на рахунку А." });
        this.value(0);
    }
};
...
function postDataForEditor(dataItem) {
    ...
    var sumA = +$("#summ_a_editor").val();
    var _nameA = dataItem.NAMA === undefined ? null : dataItem.NAMA;
    var _okpoA = dataItem.OKPOA === undefined ? null : dataItem.OKPOA;
...
function checkEditForm() {
    ...
    if (returnRes.result) {
        ...
        //var totalEnteredSum = 0;
        //var gridData = grid.dataSource.data();
        //var zal = +$("#tbOstC").val();
        //var summ_a = +$("#summ_a_editor").val();
        //summ_a = summ_a.toFixed(2);

        //$.each(gridData, function (index, value) {
        //    totalEnteredSum += +value.SUMM_A;
        //});

        //totalEnteredSum -= +dataItem.SUMM_A;

        //totalEnteredSum = +totalEnteredSum.toFixed(2);

        //var diff = (zal - totalEnteredSum).toFixed(2);

        //if (summ_a > diff) {
        //    returnRes.result = false;
        //    returnRes.errorMessage = "Загальна набрана сума не може перевищувати фактичний залишок на рахунку А."
        //                             + "<br/>Ви хочете додати суму <b>" + summ_a + "</b>"
        //                             + "<br/>Максимально допустима сума : <b>" + diff + "</b>";
        //}
    }
    return returnRes;
};
...
function deleteRow(selectedItem) {
    ...
    $.ajax({
        type: "POST",
        url: bars.config.urlContent("/api/DynamicLayoutLegalEntities/DynamicLayoutLegalEntities/DeleteStaticLayout"),
        data: postData,
        success: function (data) {
            bars.ui.loader('body', false);
            if (data.Result != "OK") {
...
function initLayout(urlParams) {
    ...
    $.ajax({
        type: "POST",
        url: bars.config.urlContent("/api/DynamicLayoutLegalEntities/DynamicLayoutLegalEntities/GetStaticLayout"),
        data: urlParams,
        success: function (data) {
            if (data.Result != "OK") {
                bars.ui.loader('body', false);
...
function initElements() {
    ...
    $("#tbTotalSum").kendoNumericTextBox(getNumericOptions({ change: totalSumChange }));
    ...
    $("#tbTypedSum").kendoNumericTextBox(getNumericOptions({ min: -999999999 }));
...
function calculate(e) {
    ...
    $.ajax({
        type: "POST",
        url: bars.config.urlContent("/api/DynamicLayoutLegalEntities/DynamicLayoutLegalEntities/UpdateAndCalculateDynamicLayout"),
        data: postData,
        success: function (data) {
...
function pay() {
    ...
    if (refs != "") {
        bars.ui.confirm(
            {
                text: "Увага, присутні уже оплачені рядки !<br/>РЕФ : <br/>" + refs
                        + "Вони будуть оплачені повторно !<br/>"
                        + "Продовжити ?"
            },
            payAll);
    } else {
...
function payAll() {
    ...
    $.ajax({
        type: "GET",
        url: bars.config.urlContent("/api/DynamicLayoutLegalEntities/DynamicLayoutLegalEntities/PayStaticLayout?pMak=" + 1),
        success: function (data) {
            bars.ui.loader('body', false);
            if (data.Result != "OK") {
                showBarsErrorAlert(data.ErrorMsg);
...
function paySelected() {
    ...
    if (dataItem != undefined && dataItem != null) {
        ...
        if (dataItem.SUMM_A > $("#tbOstC").val()) {
            bars.ui.alert({ text: "Сума проводки перевищує залишок на рахунку А.<br/>Операція не можлива." });
        } else {
            ...
            $.ajax({
                type: "POST",
                data: postData,
                url: bars.config.urlContent("/api/DynamicLayoutLegalEntities/DynamicLayoutLegalEntities/PaySelected"),
                success: function (data) {
                    bars.ui.loader('body', false);
                    if (data.Result != "OK") {
                        showBarsErrorAlert(data.ErrorMsg);
