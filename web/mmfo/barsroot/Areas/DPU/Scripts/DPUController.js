var app = angular.module('BarsWeb.Controllers', []).controller('DPU', ['$scope', '$http', function ($scope, $http) {
    ...
    $scope.GetDataSourceForDocGrid = function () {
        return {
            ...
            transport: {
                read: {
                    url: bars.config.urlContent('/api/DPU/DPUApi/GetDataForDocGrid')
                }
            },
            serverPaging: true,
    ...
    GetDataSourceForGrid = function () {
        ...
        if ($scope.firstid != undefined || $scope.firstid != null) {
            ...
            return {
                ...
                transport: {
                    read: {
                        url: bars.config.urlContent('/api/DPU/DPUApi/GetDataForGrid') + '?id=' + $scope.firstid
                    }
                },
                serverPaging: true,
                //serverFiltering: true,
    ...
    $scope.BeforeStart = function () {
        $.ajax({
            url: bars.config.urlContent("/api/DPU/DPUApi/BeforeStart"),
            method: "GET",
            async: false,
    ...
    $scope.DeleteRow = function () {
        ...
        if (selectedRef1 != null) {
            $http({
                url: bars.config.urlContent("/api/DPU/DPUApi/DeleteRow"),
                method: "POST",
                dataType: "json",
    ...
    $scope.PayBack = function () {
        ...
        if (selected != 0) {
            $http({
                url: bars.config.urlContent("/api/DPU/DPUApi/PayBack"),
                method: "POST",
                dataType: "json",
    ...
    $scope.CreditedAmount = function () {
        ...
        if (selected1 != null || selected1 != null) {
            $http({
                url: bars.config.urlContent("/api/DPU/DPUApi/CreditedAmount"),
                method: "POST",
                dataType: "json",
