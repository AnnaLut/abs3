Ext.define('ExtApp.controller.refBook.RefGrid', {
        ...
        Ext.MessageBox.confirm('Видалення рядка', 'Ви впевнені що хочете видалити рядок таблиці?', function (btn) {
            if (btn == 'yes') {
                ...
                thisController.sendToServer(
                    "/barsroot/ReferenceBookAdmin/DeleteData",
    ...
    removeAll: function (grid, plagin, rows) {
        ...
        Ext.MessageBox.confirm('Видалення ' + rowsCount + ' рядків', 'Ви впевнені що хочете видалити ' + rowsCount + '  рядка таблиці?', function (btn) {
            if (btn == 'yes') {
                ...
                Ext.MessageBox.show({ msg: 'Виконується оновлення даних, зачекайте...' + '</br></br>', wait: true, waitConfig: { interval: 200 }, iconCls: "save" });
                thisController.sendToServer(
                    "/barsroot/ReferenceBookAdmin/DeleteRows",
                    {
    ...
    saveFilterBtnHandler: function (filterParam, saveInBd, items, whereClause) {
        ...
        var url = filterParam.filterId && saveInBd
         ? "/barsroot/ReferenceBookAdmin/UpdateFilter"
         : "/barsroot/ReferenceBookAdmin/InsertFilter";
    ...
    updateFiltersInfo: function () {
        ...
        var thisController = this;
        Ext.Ajax.request({
            url: '/barsroot/metaDataAdmin/ReferenceBookAdmin/GetFiltersMetaInfo',
    ...
    syncAllCanges: function (btn) {
        ...
        Ext.MessageBox.show({ msg: 'Виконується оновлення даних, зачекайте...' + '</br></br>', wait: true, waitConfig: { interval: 200 }, iconCls: "save" });
        thisController.sendToServer(
            "/barsroot/ReferenceBookAdmin/InsertUpdateRows",
            {
    ...
    AddEditRecord: function (values, modified, isPhantom, isDirty) {
        ...
        if (isPhantom) {
            ...
            Ext.MessageBox.show({ msg: 'Виконується оновлення даних, зачекайте...' + '</br></br>', wait: true, waitConfig: { interval: 200 }, iconCls: "save" });
            this.sendToServer(
                "/barsroot/ReferenceBookAdmin/InsertData",
            ...
            if (isDirty) {
                ...
                Ext.MessageBox.show({ msg: 'Виконується оновлення даних, зачекайте...' + '</br></br>', wait: true, waitConfig: { interval: 200 }, iconCls: "save" });
                thisController.sendToServer(
                    "/barsroot/metaDataAdmin/ReferenceBookAdmin/UpdateData/?" + "tableId=" + referenceGrid.metadata.tableInfo.TABID + "&tableName=" +
    ...
    onExportToExcelBtnClick: function (menu, item) {
        ...
        window.open("/barsroot/ReferenceBookAdmin/ExportToExcel?" +
            "tableId=" + grid.metadata.tableInfo.TABID +
            "&tableName=" + grid.metadata.tableInfo.TABNAME +
    ...
    executeCurrentSqlFunction: function (callbackFunc) {
        ...
        if (func.sendByBatch)
            thisController.sendToServer(
            "/barsroot/ReferenceBookAdmin/CallFuncWithMultypleRows",
            {
 ...
 Ext.each(func.params, function (param) {
                thisController.sendToServer(
                "/barsroot/ReferenceBookAdmin/CallRefFunction",
                {
    ...
    executeCurrentSqlFunctionWithOutParams: function (callbackFunc) {
        ...
        var func = thisController.currentCalledSqlFunction;
        Ext.each(func.params, function (param) {
            thisController.getFileFromServer("/barsroot/ReferenceBookAdmin/CallFunctionWithResult/?" + 'tableId=' + func.tableId + '&funcId=' + func.funcId + '&codeOper=' + func.CodeOper +
 ...
 getUrlForCallFunc: function (func) {
        ...
        switch (exec) {
            case 'EACH':
                return "/barsroot/ReferenceBookAdmin/CallRefFunction";
            case 'BATCH':
                return "/barsroot/ReferenceBookAdmin/CallFuncWithMultypleRows";
            default:
                return false;
    ...
    executeCurrentSqlFunctionWithUploadFile: function (functi, formWindow) {
        ...
        if (fileParam && fileParam.Value && form.isValid()) {
            form.submit({
                url: '/barsroot/metaDataAdmin/ReferenceBookAdmin/UploadTemplateFile?fieldFileName=' + fileParam.Name + '&tableId=' + func.tableId + '&funcId=' + func.funcId + '&codeOper=' + func.CodeOper +
