var mainApp = angular.module(globalSettings.modulesAreas);

mainApp.controller("BaseRateDeposit", function ($controller, $scope, $timeout, $http,
    saveDataService, settingsService, modelService, validationService) {
    $controller('GdaBaseController', { $scope: $scope });     // Расширяем контроллер      

    $scope.placementTranche = modelService.initFormData("placementTranche");

    var activeOptions = [{ value: 1, text: "Активна" }, { value: 0, text: "Не активна" }];

    var saveGeneral = {
        type: "POST",
        url: bars.config.urlContent("/api/gda/gda/setgeneraloption"),
        data: function (data) {
            data.ValidFrom = kendo.toString(data.ValidFrom, "yyyy-MM-dd");
            data.ValidThough = kendo.toString(data.ValidThough, "yyyy-MM-dd");
            var grid = $("#generaloptions").data().kendoGrid;
            var selectedRow = grid.select();
            //
            if (selectedRow.length > 0) {
                $scope.sItem = grid.dataItem(grid.select()).Id;
            }
            return data;
        }
    };

    var saveCondition = {
        type: "POST",
        url: bars.config.urlContent("/api/gda/gda/setgeneralcondition"),
        data: function (data) {

            var grid = $("#generaloptions").data().kendoGrid;
            var selectedRow = grid.select();
            //
            if (selectedRow.length > 0) {
                $scope.sItem = grid.dataItem(grid.select()).Id;
            }
            //
            var selectedDataItem = grid.dataItem(selectedRow);
            data.InterestOptionId = selectedDataItem.Id;
        }
    };

    var GeneralOptionDataSource = $scope.createDataSource({
        type: "webapi",
        pageSize: 100,
        transport: {
            read: {
                url: bars.config.urlContent("/api/gda/gda/getgeneralinfo")
            },
            create: saveGeneral,
            update: saveGeneral
        },
        requestEnd: function (e) {
            bars.ui.loader("body", false);
            if (e.type == "create") {
                bars.ui.notify("Повідомлення", "Новий строк дії умов доданий", 'success', { autoHideAfter: 5 * 1000 });
            } else if (e.type == "update") {
                bars.ui.notify("Повідомлення", "Cтрок дії умов відредагований", 'success', { autoHideAfter: 5 * 1000 });
            } else {
                enableDisableButtons(['#cancelConditionBase', '#saveConditionBase','#generalconditionsEdit'], true);
                if (checkEdit('generalconditions', [], [], e) == true) {
                    disableTabsInEditMode('tabBase', true);
                } else {
                    return;
                }
            }
        },
        sort: { field: "ValidFrom", dir: "desc" },
        schema: {
            data: "Data",
            total: "Total",
            model: {
                id: "Id",
                fields: {
                    Conditions: [],
                    ValidFrom: { type: 'date' },
                    ValidThough: { type: 'date', validation: { nullable: true } },
                    IsActive: { type: 'number', validation: { required: true, min: 0, max: 1 } },
                    UserId: { type: 'string' },
                    SysTime: { type: 'string' },
                    Id: { type: 'string' }
                }
            }
        },
        serverFiltering: false, ////!!!!!
        serverPaging: false,
        serverSorting: false
    });

    var GeneralConditionsDataSource = $scope.createDataSource({
        type: "json",
        pageSize: 100,
        transport: {
            create: saveCondition,
            update: saveCondition
        },
        requestEnd: function (e) {
            bars.ui.loader('body', false);
            if (e.type == "create") {
                bars.ui.notify("Повідомлення", "Створена нова умова для обраного запису", 'success', { autoHideAfter: 5 * 1000 });
                var gridOpt = $("#generaloptions").data().kendoGrid;
                var gridCond = $('#generalconditions').data().kendoGrid;

                gridOpt.dataSource.read();
                gridCond.refresh();
            } else if (e.type == "update") {
                bars.ui.notify("Повідомлення", "Додаткова умова була відредагована.", 'success', { autoHideAfter: 5 * 1000 });
                var gridOpt = $("#generaloptions").data().kendoGrid;
                var gridCond = $('#generalconditions').data().kendoGrid;

                gridOpt.dataSource.read();
                gridCond.refresh();
            } else  {
                enableDisableButtons(['#cancelConditionBase', '#saveConditionBase'], false);
                if (checkEdit('generalconditions', [], [], e) == true) {
                    disableTabsInEditMode('tabBase', true);
                } else {
                    return;
                }
            }
        },
        schema: {
            model: {
                id: "Id",
                fields: {
                    Id: { type: 'string' },
                    InterestOptionId: { type: 'string' },
                    CurrencyId: { type: 'string' },
                    Currency: { type: 'string' },
                    TermFrom: { type: 'number', validation: { min: 0 } },
                    AmountFrom: { type: 'number', validation: { min: 0 } },
                    InterestRate: { type: 'number' }
                }
            }
        },
        serverFiltering: false,
        serverPaging: true,
        serverSorting: false
    });


    $scope.GeneralOptionGridOptions = $scope.createGridOptions({
        dataSource: GeneralOptionDataSource,
        dataBound: function (e) {
            $('#generalconditions').removeProp('disabled');
            if ($scope.sItem) {
                var dataItem = this.dataSource.get($scope.sItem);
                if (dataItem == null) {
                    return;
                } else {
                    this.select($('[data-uid=' + dataItem.uid + ']'));
                }
            }
            var rows = e.sender.tbody.children();
            for (var j = 0; j < rows.length; j++) {
                var row = $(rows[j]);
                var dataItem = e.sender.dataItem(row);

                if (dataItem.get("IsActive") == "1") {
                    row.addClass("classActive");
                } else {
                    row.addClass("classClose");
                }
            }
        },
        height: 300,
        selectable: true,
        editable: 'inline',
        filterable: true,
        sortable: true,
        excel: {
            fileName: "Export.xlsx",
            allPages: true,
            filterable: true,
            proxyURL: bars.config.urlContent("/GDA/GDA/ConvertBase64ToFile/")
        },
        excelExport: function (e) {
            var sheet = e.workbook.sheets[0];
            for (var i = 0; i < sheet.columns.length; i++) {
                sheet.columns[i].width = 150;
            }
        },
        columns: [
            {
                field: "ValidFrom",
                title: "Дата від",
                format: "{0:dd-MM-yyyy}"

            },
            {
                field: "ValidThough",
                title: "Дата до",
                format: "{0:dd-MM-yyyy}"
            },
            {
                field: "IsActive",
                title: "Статус",
                values: activeOptions,
                editor: function (container, options) {
                    var input = $('<input data-text-field="text" data-value-field="value" data-bind="value: ' + options.field + '" name="' + options.field + '"/>');
                    input.appendTo(container);
                    input.kendoDropDownList({
                        dataTextField: "text",
                        dataValueField: "value",
                        dataSource: activeOptions
                    });
                }
            }
        ],
        change: function (e) {
            var data = this.dataItem(this.select());
            if (data == null) {
                data = { Id: '', uid: '' };
                data.Id = '';
                data.uid = '';
            }

            if (data && data.Conditions != null) {
                var dataArray = $.map(data.Conditions, function (value, index) {
                    return [value];
                });
                //

                var editRow = this.dataItem('tr.k-grid-edit-row');
                if (!editRow) {
                    $('#generalconditions').data('kendoGrid').dataSource.data(dataArray);
                } else {

                }
                //
            } else {
                $('#generalconditions').data('kendoGrid').dataSource.data([]);
                $("#generalconditions").data("kendoGrid").refresh();
            }
            enableDisableButtons(['#addConditionBase', '#generaloptionsEdit'], false);
            //enableDisableButtons(['#cancelConditionBase', '#generalconditionsEdit', '#saveConditionBase'], true);
            var editRow = this.dataItem('tr.k-grid-edit-row');
            if (data == null || (data.Id != '' && data.SysTime != null && data.UserId != null)) {
                if (checkEdit('generaloptions', ['#saveConditionBase', '#cancelConditionBase'], ['#cancelOptionBase', '#saveOptionBase'], e) == true) {
                    bars.ui.alert({ text: "Завершіть редагування/додавання! <br> Щоб відмінити редагування/додавання натисніть <button class='btn btn-default'><i class='pf-icon pf-16 pf-delete'></i></button>" });
                    disableTabsInEditMode('tabBase', true);
                }
            } else if (data != null && data.Id != '') {
                if (data.SysTime == null && data.UserId == null) {
                    if (data == editRow) {
                        return;
                    } else {
                        if (checkEdit('generaloptions', ['#saveConditionBase', '#cancelConditionBase'], ['#cancelOptionBase', '#saveOptionBase'], e) == true) {
                            bars.ui.alert({ text: "Завершіть редагування/додавання! <br> Щоб відмінити редагування/додавання натисніть <button class='btn btn-default'><i class='pf-icon pf-16 pf-delete'></i></button>" });
                            disableTabsInEditMode('tabBase',true);
                        }
                    }
                } else {
                    return;
                }
            }


        }
    });

    function Cancel() {
        var grid = $("#generaloptions").data("kendoGrid");

        var data = grid.dataItem(grid.select());
        //$scope.sItem = data.Id;
        if (data) {
            if (data.Conditions != null) {
                $('#generalconditions').data('kendoGrid').dataSource.data(data.Conditions);
                $('#generalconditions').data('kendoGrid').refresh();
                $('#generaloptions').data('kendoGrid').dataSource.read();
            }
        }
    }

    $("#exportToExcelBase").click(function () {
        var grid = $("#generaloptions").data("kendoGrid");

        var selectedRow = grid.dataItem(grid.select());
        if (selectedRow == null) {
            bars.ui.alert({ text: "Щоб завантажити дані у Excel оберіть строк дії та ще раз натисніть цю кнопку" });
        } else {
            var rowsOpt = [{
                cells: [
                    { value: "Дата від" },
                    { value: "Дата до" },
                    { value: "Статус" },
                ]
            }];
            var rowsCond = [{
                cells: [
                    { value: "Валюта" },
                    { value: "Строк від" },
                    { value: "Сума від" },
                    { value: "Ставка" },
                ]
            }];

            if (grid.dataItem(grid.select()).Conditions != null) {
                rowsOpt.push({
                    cells: [
                        { value: grid.dataItem(grid.select()).ValidFrom },
                        { value: grid.dataItem(grid.select()).ValidThough },
                        { value: grid.dataItem(grid.select()).IsActive ? 'Активна' : 'Неактивна' }
                    ]
                });

                for (var i = 0; i < grid.dataItem(grid.select()).Conditions.length; i++) {
                    rowsCond.push({
                        cells: [
                            { value: grid.dataItem(grid.select()).Conditions[i].Currency },
                            { value: +grid.dataItem(grid.select()).Conditions[i].TermFrom },
                            { value: +grid.dataItem(grid.select()).Conditions[i].AmountFrom },
                            { value: +grid.dataItem(grid.select()).Conditions[i].InterestRate, format: "0.00" }
                        ]
                    });
                }
                var newArr = rowsOpt.concat(rowsCond);
            } else {
                rowsOpt.push({
                    cells: [
                        { value: grid.dataItem(grid.select()).ValidFrom },
                        { value: grid.dataItem(grid.select()).ValidThough },
                        { value: grid.dataItem(grid.select()).IsActive ? 'Активна' : 'Неактивна' }
                    ]
                });
                var newArr = rowsOpt;
            }



            var workbook = new kendo.ooxml.Workbook({
                sheets: [
                    {
                        columns: [
                            { autoWidth: true },
                            { autoWidth: true }
                        ],
                        title: "Базова ставка по депозиту ММСБ",
                        rows: newArr
                    }
                ]
            });
            kendo.saveAs({ dataURI: workbook.toDataURL(), fileName: "Базова ставка по депозиту ММСБ.xlsx", proxyURL: bars.config.urlContent("/GDA/GDA/ConvertBase64ToFile/") });
        }
    });

    $('#addOptionBase').click(function () {
        enableDisableButtons(['#cancelOptionBase', '#saveOptionBase'], false);
        enableDisableButtons(['#generaloptionsEdit', '#addConditionBase', '#generalconditionsEdit', '#saveConditionBase', '#exportToExcelBase'], true);
        var grid = $("#generaloptions").data("kendoGrid");
        grid.clearSelection();
        grid.addRow();
        disableTabsInEditMode('tabBase', true);
        $('#generalconditions').data('kendoGrid').dataSource.read();
    });
    $('#saveOptionBase').click(function () {
        enableDisableButtons(['#cancelOptionBase', '#saveOptionBase', '#saveConditionBase', '#addConditionBase'], true);
        enableDisableButtons(['#addOptionBase', '#generaloptionsEdit', '#generalconditionsEdit', '#exportToExcelBase'], false);
        var grid = $("#generaloptions").data("kendoGrid");
        grid.saveRow();

        $('#generaloptions').data('kendoGrid').dataSource.read();
        disableTabsInEditMode('tabBase', false);
    });
    $('#cancelOptionBase').click(function () {
        enableDisableButtons(['#cancelOptionBase', '#addConditionBase', '#saveConditionBase', '#cancelConditionBase', '#generaloptionsEdit', '#saveOptionBase'], true);
        enableDisableButtons(['#addOptionBase', '#generaloptionsEdit', '#exportToExcelBase'], false);
        var grid = $("#generaloptions").data("kendoGrid");
        grid.cancelRow();
        grid.dataSource.read();
        $('#generalconditions').data('kendoGrid').dataSource.data([]);
        $("#generalconditions").data("kendoGrid").refresh();
        disableTabsInEditMode('tabBase', false);
    });

    $('#addConditionBase').click(function (e) {
        enableDisableButtons(['#generalconditionsEdit', '#generaloptionsEdit', '#cancelOptionBase', '#addConditionBase'], true);
        enableDisableButtons(['#cancelConditionBase', '#saveConditionBase'], false);

        if (checkEdit('generaloptions', ['#saveConditionBase', '#cancelConditionBase'], ['#cancelOptionBase', '#saveOptionBase'], e) == true) {
            bars.ui.alert({ text: "Завершіть редагування/додавання! <br> Щоб відмінити редагування/додавання натисніть <button class='btn btn-default'><i class='pf-icon pf-16 pf-delete'></i></button>" });
            disableTabsInEditMode('tabBase', true);
        } else {
            var grid = $("#generalconditions").data("kendoGrid");
            var gridOpt = $("#generaloptions").data("kendoGrid");
            //
            $scope.sItem = gridOpt.dataItem(gridOpt.select()).Id;
            //
            grid.addRow();
            disableTabsInEditMode('tabBase', true);
        }

    });

    $('#generaloptionsEdit,#generalconditionsEdit').click(function (event) {
        var id = event.currentTarget.id.slice(0, -4);
        var grid = $("#" + id).data("kendoGrid");
        var selected = grid.select();
        var isGeneralOptions = id === 'generaloptions';
        if (selected.length == 0) {
            var toDisableIds = isGeneralOptions ? ['#cancelOptionBase'] : ['#cancelConditionBase'];
            var toEnableIds = isGeneralOptions ? ['#addOptionBase'] : ['#addConditionBase'];
            enableDisableButtons(toDisableIds, true);
            enableDisableButtons(toEnableIds, false);
            return;
        } else {
            var classList = selected[0].classList ? selected[0].classList : selected[0].className.split(' ');
            var inEdit = classList[classList.length - 1] === 'k-grid-edit-row';
            var toDisableIds = isGeneralOptions ? ['#cancelOptionBase', '#saveOptionBase'] : ['#addConditionBase', '#addOptionBase', '#generaloptionsEdit', '#saveOptionBase', '#cancelOptionBase', '#generalconditionsEdit', '#exportToExcelBase'];
            var toEnableIds = isGeneralOptions ? ['#addOptionBase', '#generalconditionsEdit', '#saveConditionBase', '#addConditionBase', '#exportToExcelBase', '#generaloptionsEdit'] : ['#cancelConditionBase', '#saveConditionBase'];

            if (isGeneralOptions) {
                if (checkEdit('generaloptions', ['#saveConditionBase', '#cancelConditionBase'], ['#cancelOptionBase', '#saveOptionBase'], event) == true) {
                    bars.ui.alert({ text: "Завершіть редагування/додавання! <br> Щоб відмінити редагування/додавання натисніть <button class='btn btn-default'><i class='pf-icon pf-16 pf-delete'></i></button>" });
                    disableTabsInEditMode('tabBase',true);
                } else {
                    enableDisableButtons(toDisableIds, inEdit);
                    enableDisableButtons(toEnableIds, !inEdit);
                    disableTabsInEditMode('tabBase', true);
                    grid.editRow(selected);
                }
            }
            else {
                if (checkEdit('generaloptions', ['#saveConditionBase', '#cancelConditionBase'], ['#cancelOptionBase', '#saveOptionBase'], event) == true) {
                    bars.ui.alert({ text: "Завершіть редагування/додавання! <br> Щоб відмінити редагування/додавання натисніть <button class='btn btn-default'><i class='pf-icon pf-16 pf-delete'></i></button>" });
                    disableTabsInEditMode('tabBase', true);
                } else {
                    enableDisableButtons(toDisableIds, !inEdit);
                    enableDisableButtons(toEnableIds, inEdit);
                    disableTabsInEditMode('tabBase', true);
                    grid.editRow(selected);

                }
            }

        }
    });
    $('#saveConditionBase').click(function () {
        //var gridOpt = $("#generaloptions").data("kendoGrid"),
        //    item = gridOpt.dataItem(gridOpt.select());
        //if (item != null) {
            enableDisableButtons(['#generaloptionsEdit', '#saveOptionBase', '#addConditionBase', '#generalconditionsEdit', '#cancelConditionBase', '#generaloptionsEdit', '#cancelOptionBase'], true);
            enableDisableButtons(['#addOptionBase', '#exportToExcelBase'], false);
            var grid = $("#generalconditions").data("kendoGrid");
            grid.saveRow();
            disableTabsInEditMode('tabBase', false);
        //} else {
        //    return;
        //}
    });
$('#cancelConditionBase').click(function () {
    //var gridOpt = $("#generaloptions").data("kendoGrid"),
    //    item = gridOpt.dataItem(gridOpt.select());
    //if (item != null) {
        enableDisableButtons(['#addOptionBase', '#addOptionBase', '#addConditionBase',], false);
        enableDisableButtons(['#cancelConditionBase', '#generalconditionsEdit', '#saveConditionBase'], true);
        var grid = $("#generalconditions").data("kendoGrid");
        grid.cancelRow();
        Cancel();
        disableTabsInEditMode('tabBase', false);
    //} else {
    //    return;
    //}
    });


    $scope.numberWithSpaces = function (x) {
        if (x == null) {
            return;
        } else {
            return x.toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ");
        }
    };
    $scope.GeneralConditionsGridOptions = $scope.createGridOptions({
        dataSource: GeneralConditionsDataSource,
        autoBind: false,
        height: 300,
        selectable: "row",
        editable: 'inline',
        sortable: true,
        columns: [
            {
                field: "CurrencyId",
                title: "Валюта",
                width: 180,
                template: "#=Currency# | #=CurrencyId# ",
                editor: function (container, options) {
                    var input = $('<input data-text-field="lcv" data-value-field="kv" data-bind="value: ' + options.field + '" name="' + options.field + '"/>');
                    input.appendTo(container);
                    //init drop
                    input.kendoDropDownList({
                        autoBind: true,
                        dataTextField: "lcv",
                        dataValueField: "kv",
                        filter: 'contains',
                        template: '#: kv # | #: lcv #',
                        dataSource: {
                            transport: {
                                read: {
                                    type: "GET",
                                    dataType: "json",
                                    url: bars.config.urlContent("/api/gda/gda/getcurrency")
                                }
                            },
                            schema: {
                                data: "Data",
                                model: {
                                    fields: {
                                        kv: { type: 'string' }
                                    }
                                }
                            }
                        },
                        filtering: function (ev) {
                            var filterValue = ev.filter != undefined ? ev.filter.value : "";
                            ev.preventDefault();
                            var intval = parseInt(filterValue);
                            this.dataSource.filter({
                                logic: "or",
                                filters: [
                                    { field: 'kv', operator: 'startswith', value: intval.toString() },
                                    { field: 'lcv', operator: 'contains', value: filterValue }
                                ]
                            });
                        },
                        optionLabel: {
                            lcv: "Оберіть валюту",
                            kv: null
                        }
                    });
                },
                sortable: {
                    compare: function (a, b) {
                        return a.CurrencyId - b.CurrencyId;
                    }
                }
            },
            {
                field: "TermFrom",
                headerAttributes: { style: "text-align:center" },
                title: "Строк від",
                sortable: {
                    compare: function (a, b) {
                        return a.TermFrom - b.TermFrom;
                    }
                },
                template: function (dataItem) {
                    if (dataItem.TermFrom < 1) {
                        return "<span>" + "0" + dataItem.TermFrom + "</span>";
                    } else {
                        return "<span>" + dataItem.TermFrom + "</span>";
                    }
                }
            },
            {
                field: "AmountFrom",
                headerAttributes: { style: "text-align:center" },
                title: "Сума від",
                sortable: {
                    compare: function (a, b) {
                        return a.AmountFrom - b.AmountFrom;
                    }
                },
                template: function (dataItem) {
                    if (dataItem.AmountFrom < 1) {
                        return "<span>" + "0" + dataItem.AmountFrom + "</span>";
                    } else {
                        return "<span>" + dataItem.AmountFrom + "</span>";
                    }
                }
                //template: function (dataItem) {
                //    return "<span>" + $scope.numberWithSpaces(dataItem.AmountFrom) + "</span>"
                //}
            },
            {
                field: "InterestRate",
                headerAttributes: { style: "text-align:center" },
                title: "Ставка",
                sortable: {
                    compare: function (a, b) {
                        return a.InterestRate - b.InterestRate;
                    }
                },
                template: function (dataItem) {
                    if (dataItem.InterestRate < 1) {
                        return "<span>" + "0" + dataItem.InterestRate + "</span>";
                    } else {
                        return "<span>" + dataItem.InterestRate + "</span>";
                    }
                }
            }
            //{ command: ["edit"], title: "&nbsp;", width: "250px" }
        ],
        save: function (e) {
            var model = e.model;
            var ddl = $("input[name=CurrencyId]").data("kendoDropDownList");
            model.Currency = ddl.text();
        },
        filterMenuInit: function (e) {
            var filterButton = $(e.container).find('.k-primary'),
                resetButton = $(e.container).find('.k-button');

            //очистити
            $(resetButton).click(function (e) {
                setTimeout("$('#generaloptions').data('kendoGrid').trigger('change')", 500);
            });

            //фільтрувати
            $(filterButton).click(function (e) {
                setTimeout("$('#generaloptions').data('kendoGrid').trigger('change')", 500);
            });
        },
        change: function (e) {

            var data = this.dataItem(this.select());
            var currency = data.Currency;
            if (currency === null || currency === '') {
                enableDisableButtons(['#saveOptionBase', '#cancelOptionBase'], false);
                enableDisableButtons(['#generaloptionsEdit'], true);
            } else {
                enableDisableButtons(['#generaloptionsEdit'], true);
                enableDisableButtons(['#generalconditionsEdit', '#addConditionBase'], false);
            }

            var editRow = this.dataItem('tr.k-grid-edit-row');
            if (data != null && data.Id != "") {
                if (data == editRow) {
                    return;
                } else {
                    if (checkEdit('generalconditions', ['#saveOptionBase', '#cancelOptionBase', '#generalconditionsEdit', '#exportToExcelBase', '#addOptionBase', "#generaloptionsEdit", '#addConditionBase'], ['#cancelConditionBase', '#saveConditionBase'], e) == true) {
                        bars.ui.alert({ text: "Завершіть редагування/додавання! <br> Щоб відмінити редагування/додавання натисніть <button class='btn btn-default'><i class='pf-icon pf-16 pf-delete'></i></button>" });
                        disableTabsInEditMode('tabBase', true);
                    } else if (checkEdit('generaloptions', ['#saveConditionBase', '#cancelConditionBase'], ['#cancelOptionBase', '#saveOptionBase'], e) == true) {
                        bars.ui.alert({ text: "Завершіть редагування/додавання! <br> Щоб відмінити редагування/додавання натисніть <button class='btn btn-default'><i class='pf-icon pf-16 pf-delete'></i></button>" });
                        disableTabsInEditMode('tabBase', true);
                    }
                }
            } else {
                return;
            }
        }
    });

    $("#generalconditions").on("mousedown", "a.k-link", function (e) {
        setTimeout("$('#generaloptions').data('kendoGrid').trigger('change')", 1);
    });
});

// disabled = true - заблокировать кнопку
// array - список ИД кнопок
function enableDisableButtons(array, disable) {
    if (Array && Array.isArray && Array.isArray(array) || Object.prototype.toString.call(array) === '[object Array]') {
        if (disable) {
            for (var i = 0; i < array.length; ++i)
                $(array[i]).prop('disabled', 'disabled');
        }
        else {
            for (var i = 0; i < array.length; ++i)
                $(array[i]).prop('disabled', false);
        }
    }
}

function checkEdit(grid, disableBtn, enableBtn, e) {

    var grid = $('#' + grid).closest(".k-grid");
    var editRow = grid.find(".k-grid-edit-row");

    if (editRow.length > 0) {
        enableDisableButtons(enableBtn, false);
        enableDisableButtons(disableBtn, true);
        e.preventDefault();
        return true;
    } else {
        return false;
    }

}

//action - true - disabled, false - enabled
function disableTabsInEditMode(tabActive,action) {
    var tabStrip = $("#tabStripDict").kendoTabStrip().data("kendoTabStrip");

    var index = {
        'tabBase': '0',
        'tabAction': '1',
        'tabPayment': '2',
        'tabCapital': '3',
        'tabProlong': '4',
        'tabPenalty': '5',
        'tabSum': '6',
        'tabTimeAdd': '7',
        'tabReplenishment': '8',
        'tabBlockRate': '9',
        'tabBonusProlong':'10'
    };
    delete index[tabActive];
    
    var count = 0;
    for (a in index) {
        if (index.hasOwnProperty(a)) {
            count++;
        }
    }
    for (var key in index) if (index.hasOwnProperty(key)) break;
    var f = index[key];

    for (i = f; i <= count; i++) {
        if (action == true) {
            tabStrip.disable(tabStrip.tabGroup.children().eq(i));
        } else {
            tabStrip.enable(tabStrip.tabGroup.children().eq(i));
        }
    }
}



