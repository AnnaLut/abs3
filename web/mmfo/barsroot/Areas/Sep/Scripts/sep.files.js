...
bars.sepfiles = bars.sepfiles || {
    ...
    recreateSepFile: function () {
        ...
        //////
        //Creating multiple selection version
        ...
        $.each(selected, function (idx, row) {
            ...
            docMultyParams.push(docParams);
        });
        //////
        //Changet to adjax and post method
        $.ajax({
            type: "POST",
            contentType: "application/json",
            url: bars.config.urlContent("/Sep/SepFiles/Recreate"),
            data: JSON.stringify(docMultyParams)
        }).done(function (data) {
            debugger;
            if (data.status == 'ok') {
                self.refreshGrid();
            ...
            $("#sepFilesGrid").kendoGrid({
                columns: [
                    ...
                    },
                    {
                        field: "SDE",
                        title: "Сума (дебет)",
                        template: "<div style='text-align:right;'>#=kendo.format('{0:0.00}',(SDE/100))#</div>", // kendo.toString((SDE / 100), self.numberFormat)
                        width: 100,
                        footerTemplate: "<div style='text-align:right;'>#=kendo.format('{0:0.00}',(sum/100))#</div>" // kendo.toString((sum/100), self.numberFormat)
                    },
                    {
                        field: "SKR",
                        title: "Сума (кредит)",
                        template: "<div style='text-align:right;'>#=kendo.format('{0:0.00}',(SKR/100))#</div>", // kendo.toString((SKR/100), self.numberFormat)
                        width: 100,
                        footerTemplate: "<div style='text-align:right;'>#=kendo.format('{0:0.00}',(sum/100))#</div>" // kendo.toString((sum/100), self.numberFormat)
                    },
                    {
            ...
            function () {
                ...
                $.each(selected, function (idx, row) {
                    ...
                    var docParams = {
                        ...
                        fileCreated: kendo.toString(record.DAT, "dd/MM/yyyy HH:mm"),
                        Incoming: incoming,
                        debitSum: item.SDE,   // debitSum: record.SDE, | debitSum: record.DebitSum * 100
                        kreditSum: item.SKR, // kreditSum: record.SKR, | kreditSum: record.KreditSum * 100
                        rowCount: item.N          // record.N | rowCount: record.RowCount 
                    }
                    docMultyParams.push(docParams);
                });
                //////
                //docMultyParams = JSON.stringify(docMultyParams);
                ...
                //    fileCreated: kendo.toString(record.DAT, "dd/MM/yyyy HH:mm"),
                //    Incoming: incoming,
                //    debitSum: record.SDE,   // debitSum: record.SDE, | debitSum: record.DebitSum * 100
                //    kreditSum: record.SKR, // kreditSum: record.SKR, | kreditSum: record.KreditSum * 100
                //    rowCount: record.N          // record.N | rowCount: record.RowCount 
                //}
                //$.get(bars.config.urlContent('/Sep/SepFiles/MatchSepFile'), docParams).done(function(data) { changed to multiple version
                //$.get(bars.config.urlContent('/Sep/SepFiles/MatchSepFile'), docMultyParams).done(function (data) {
                //    if (data.status == 'ok') {
                //        self.refreshGrid();
                //        bars.ui.alert({ text: data.message });
                //    } else {
                ...
                $.ajax({
                    type: "POST",
                    contentType: "application/json",
                    url: bars.config.urlContent("/Sep/SepFiles/MatchSepFile"),
                    data: JSON.stringify(docMultyParams)
                }).done(function (data) {
                    debugger;
                    if (data.status == 'ok') {
        ...
        bars.ui.confirm({ text: 'Видалити файл?' },
            function () {
                ...
                $.ajax({
                    type: "POST",
                    contentType: "application/json",
                    url: bars.config.urlContent("/Sep/SepFiles/DeleteSepFile"),
                    data: JSON.stringify(delMultyParams)
                }).done(function (data) {
                    debugger;
                    if (data.status == 'ok') {
                        self.refreshGrid();
        ...
        $("#selectBpReason").kendoButton({
            ...
            click: function () {
                ...
                $.each(selected, function (idx, row) {
                    ...
                    uncreateMultyParams.push(params);
                });
                //////


                /// Now need to use post + ajax to hava ability send arrays of data safely
                $.ajax({
                    type: "POST",
                    contentType: "application/json",
                    url: bars.config.urlContent("/Sep/SepFiles/UnCreateSepFile"),
                    data: JSON.stringify(uncreateMultyParams)
                }).done(function (data) {
                    debugger;
                    if (data.status == 'ok') {
                        self.refreshGrid();
