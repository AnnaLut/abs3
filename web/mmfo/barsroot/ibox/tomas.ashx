...
public class tomas : IHttpHandler, IRequiresSessionState
{
    ...
    public XmlDocument MoneyTransfer(XmlDocument XmlMessage)
    {
        ...
        String extRef = String.Empty, sum = String.Empty, fee = String.Empty;
        try
        {
            for (int i = 0; i < Transactions.Count; i++)
            {
                ...
                transaction_infos[i] = String.Concat(extRef, "@", sum, "@", fee);  // format of transaction_infos[i] is pay_id@pay_amount@fee_amount
