            ...
            <asp:UpdatePanel ID="upRepResults" runat="server" UpdateMode="Conditional">
                <ContentTemplate>
                    ...
                    <Bars:BarsGridViewEx
                        ...
                        <HeaderStyle CssClass="headerRow"></HeaderStyle><EditRowStyle CssClass="editRow"></EditRowStyle><PagerStyle CssClass="pagerRow"></PagerStyle><NewRowStyle CssClass=""></NewRowStyle><SelectedRowStyle CssClass="selectedRow"></SelectedRowStyle><AlternatingRowStyle CssClass="alternateRow"></AlternatingRowStyle><Columns>
                            <asp:TemplateField ShowHeader="False">
                                <ItemTemplate>
                                    <asp:ImageButton ID="ibPrint" runat="server" ToolTip="Друкувати звіт" CausesValidation="False"
                                                     Visible='<%# ((String) Eval("STATUS_ID") == "DONE" || (String) Eval("STATUS_ID") == "PRINTED" || (String) Eval("STATUS_ID") == "CREATEDFILE") %>'
                                                     CommandName="Print" ImageUrl="/Common/Images/default/16/print.png" CommandArgument='<%# Eval("QUERY_ID") %>'/>
                                </ItemTemplate><ItemStyle HorizontalAlign="Center" Width="25px"/>
                            </asp:TemplateField><asp:TemplateField ShowHeader="False">
                                <ItemTemplate>
                                    <asp:ImageButton ID="ibClear" runat="server" ToolTip="Видалити тимчасові дані" CausesValidation="False"
                                                     CommandName="Clear" ImageUrl="/Common/Images/default/16/garbage.png" CommandArgument='<%# Eval("QUERY_ID") %>'
                                                     OnClientClick="if (!confirm('Видалити дані?')) return false;"/>
                                </ItemTemplate><ItemStyle HorizontalAlign="Center" Width="25px"/>
                            </asp:TemplateField><asp:BoundField DataField="QUERY_ID" HeaderText="№" SortExpression="QUERY_ID"/><asp:TemplateField HeaderText="№ Звіту" SortExpression="REP_ID">
                                <ItemTemplate>
                                    <asp:Label ID="Label1" runat="server" Text='<%# Bind("REP_ID") %>' ToolTip='<%# Eval("REP_DESC") %>'></asp:Label>
                                </ItemTemplate>
                            </asp:TemplateField><asp:BoundField DataField="REP_DESC" HeaderText="Назва" SortExpression="REP_DESC"/><asp:BoundField DataField="CREATION_TIME" DataFormatString="{0:dd.MM.yy hh:mm}" HeaderText="Дата"
                                                                                                                                                   SortExpression="CREATION_TIME">
                                <ItemStyle HorizontalAlign="Center"/>
                            </asp:BoundField><asp:BoundField DataField="PROC_TIME" HeaderText="Час формування (хв.)" SortExpression="PROC_TIME">
                                <ItemStyle HorizontalAlign="Center"/>
                            </asp:BoundField><asp:TemplateField HeaderText="Статус" SortExpression="STATS_NAME">
                                <ItemTemplate>
                                    <asp:Label ID="Label2" runat="server" Text='<%# Bind("STATS_NAME") %>' ToolTip='<%# ((String) Eval("STATUS_ID") == "ERROR" ? "№ запису у журналі - " + Convert.ToString(Eval("STATS_COMM_ID")) + "\n" + (String) Eval("STATS_COMM") : "") %>'></asp:Label>
                                </ItemTemplate>
                            </asp:TemplateField><asp:TemplateField HeaderText="Параметри" SortExpression="XML_PARAMS">
                                <ItemTemplate>
                                    <asp:Label ID="lbParams" runat="server" Text='<%# Bind("XML_PARAMS") %>'></asp:Label>
                                </ItemTemplate>
                            </asp:TemplateField>
                        </Columns><RowStyle CssClass="normalRow"></RowStyle>
                    </Bars:BarsGridViewEx>
                </ContentTemplate>
            </asp:UpdatePanel>
        </asp:Panel>
        <asp:Panel ID="pnlRepQuery" runat="server" GroupingText="Заявка на формування звіту">
            <asp:UpdatePanel ID="upRepQuery" runat="server" UpdateMode="Conditional">
                <ContentTemplate>
                    ...
                    <asp:ImageButton runat="server" ID="btExportExcel" ImageUrl="/common/images/default/16/export_excel.png" OnClick="ClickToExportToExcel" style="margin-left: 3em; margin-top: 0.6em; position:absolute" />
                    <Bars:BarsGridViewEx ID="gvVCbirepReplist" runat="server" AllowPaging="false" AllowSorting="True" ShowExportExcelButton="False"
                                         ...
                                         OnRowCommand="gvVCbirepReplist_RowCommand" ShowPageSizeBox="false" PageSize="7">
                        <FooterStyle CssClass="footerRow"></FooterStyle><HeaderStyle CssClass="headerRow"></HeaderStyle><EditRowStyle CssClass="editRow"></EditRowStyle><PagerStyle CssClass="pagerRow"></PagerStyle><NewRowStyle CssClass=""></NewRowStyle><SelectedRowStyle CssClass="selectedRow"></SelectedRowStyle><AlternatingRowStyle CssClass="alternateRow"></AlternatingRowStyle><Columns>
                            <asp:TemplateField ShowHeader="False">
                                <ItemTemplate>
                                    <asp:ImageButton ID="ibQuery" runat="server" ToolTip="Відправити заявку на формування"
                                                     CausesValidation="False" CommandArgument='<%# Eval("REP_ID") %>' CommandName="Query"
                                                     ImageUrl="/Common/Images/default/16/options.png"/>
                                </ItemTemplate><ItemStyle HorizontalAlign="Center" Width="25px"/>
                            </asp:TemplateField><asp:BoundField DataField="REP_ID" HeaderText="№" SortExpression="REP_ID"/><asp:BoundField DataField="REP_DESC" HeaderText="Назва" SortExpression="REP_DESC"/>
                        </Columns><RowStyle CssClass="normalRow"></RowStyle>
                    </Bars:BarsGridViewEx>
                </ContentTemplate>
                <Triggers>
