...
public partial class sberutls_importex : Bars.BarsPage
{
    ...
    protected void btnLoad_Click(object sender, EventArgs e)
    {
        ...
        byte[] data_ = null;
        if (getCurrType() == 7) // COBUSUPABS-5229
        {
            string fGhostName = Path.GetFileNameWithoutExtension(Path.GetRandomFileName()); // for OLEDB (if file contain cyrillic)
            string fName = Path.GetFileName(fileUpload.PostedFile.FileName);

            fGhostName = string.Format("{0}{1}", fGhostName, Path.GetExtension(fName));
            string fGhostPath = Path.Combine(Path.GetTempPath(), fGhostName);

            using (FileStream fs = new FileStream(fGhostPath, FileMode.Create))
            {
                fs.Write(data, 0, data.Length);
            }
            FileInfo fi = new FileInfo(fGhostPath);
            try
            {
                DBFFile dbf = new DBFFile(fi, null, null);
                data_ = PaymentMessageManager.FillArray(fName, dbf.TableData.Rows);
            }
        ...
        try
        {
            try
            {
                ...
                try
                {
                    ...
                    if (getCurrType() == 7)
                    {
                        SetParameters("p_indoc", DB_TYPE.Clob, Encoding.GetEncoding(1251).GetString(data_), DIRECTION.Input);
                    }
                    else if (getCurrType() != -1)
                    {
