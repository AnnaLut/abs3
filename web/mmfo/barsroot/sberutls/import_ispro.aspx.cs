...
public partial class sberutls_import_ispro : Bars.BarsPage
{
    ...
    protected void btnLoad_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            SetParameters("EncodeFile", DB_TYPE.Varchar2, "DOS", DIRECTION.Input);
            ...
            String SelectCommand = @"select npp, debet, kredit, summa, nazn, branch, dt, null ref from ispro_" + kf;
    ...
    protected void btPay_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            SQL_NONQUERY(@"begin maket(0, gl.bd, gl.bd); end;");

            dsMain.ConnectionString = OraConnector.Handler.IOraConnection.GetUserConnectionString();
            String SelectCommand = @"select npp, debet, kredit, summa, nazn, branch, dt, ref from ispro_" + kf;

            dsMain.SelectCommand = SelectCommand;
            dsMain.DataBind();
            gvMain.DataBind();
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Увага", "alert('Оплата виконана успішно!');", true);
        }
        finally
        {
            DisposeOraConnection();
