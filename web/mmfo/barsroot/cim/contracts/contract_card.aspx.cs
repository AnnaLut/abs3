...
public partial class cim_contracts_other_contract_card : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            Master.SetPageTitle(Title + " №" + Request["contr_id"], true);
        ...
        if (!IsPostBack && Request["mode"] == "create")
        {
            var cimMgr = new CimManager(true);
            if(Session[Constants.StateKeys.AccessFlag] == "1")
                throw new ApplicationException("Заборонено створення контрактів по даному ТВБВ");
        }
        ...
        if (!ClientScript.IsStartupScriptRegistered(this.GetType(), "init"))
            ClientScript.RegisterStartupScript(this.GetType(), "init", "CIM.setVariables('" + Request["mode"] + "','" + Request["contr_id"] + "','" + Session[Constants.StateKeys.AccessFlag] + "'); ", true);
        ...
        dsBasePersentRate.ConnectionString = OraConnector.Handler.IOraConnection.GetUserConnectionString();
        dsBasePersentRateTerm.ConnectionString = OraConnector.Handler.IOraConnection.GetUserConnectionString();
        dsPurpose.ConnectionString = OraConnector.Handler.IOraConnection.GetUserConnectionString();
        dsInfoAboutChanges.ConnectionString = OraConnector.Handler.IOraConnection.GetUserConnectionString();
