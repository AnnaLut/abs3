        ...
        function SaveClick() {
            ...
            if (!csxi.ImageLoaded && imageCount == 0) {
                ...
                var thumbnails = $('#thumbnails object');
                    ...
                    if (window.name == 'DialogClientCard' || window.dialogArguments["windowName"] == 'DialogClientCard') {
                        var success = csxi.PostImage(UploadUrl, 'scanClientCard.pdf', '', UploadFormat); //передаем файл формата PDF на сервер
                        if (success == false) {
                            alert('Розмір фотографії перевищує 150 кб');
                            csxi.DeleteFile("scanClientCard.pdf", true);
                        }
                        else {
                            alert('Фотографію збереженно');
                            //$('#bt_reg', opener.document).prop("disabled", false);
                            CloseDialog({SaveBtnEnabled: true, Status: true, SelectedDevice: GetSelectedDevice() });
                        }
                    }
                    else {
