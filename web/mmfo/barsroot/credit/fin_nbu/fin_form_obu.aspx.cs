        ...
        if ((Request["rnk"] != null && Request["frm"] != null) && !IsPostBack)
        {
            ...
            try
            {
                ...
                cmd.CommandText = (@"    select c.nd, 
                                            ...
                                            c.DWDATE as wdate,
                                            c.cc_id,
                                            (case when vidd = 3 then (select acrN.FPROCN (a8.ACC, 0, '') from accounts a8, nd_acc nd where nd.nd = c.nd and nd.acc = a8.acc and a8.nls like '8999%' and a8.tip = 'LIM'  ) 
                                                  else pr
                                                  end  
                                            ) as pr,
                                            to_char(c.kv) as kv,
                ...
                if (rdr.Read())
                {
                    ...
                    String l_pr;
                    l_pr = rdr["pr"] == DBNull.Value ? (String) null : Convert.ToString( (Decimal)rdr["pr"]);
    ...
    public static String read_nd( String nd_, String rnk_, String kod_, String idf_,  String FDAT_)
{
  ...
  //вичитаєм внутрішній кредитний рейтинг і інтегральний показник        
    ...
    protected void forms_Click(object sender, EventArgs e)
    {
        ...
        FrxParameters pars = new FrxParameters();
        pars.Add(new FrxParameter("fdat", TypeCode.String, tb_datf.Text));
        pars.Add(new FrxParameter("rnk", TypeCode.String, Convert.ToInt64(tb_RNK.Text)));
    ...
    protected void Bt_next_Click(object sender, ImageClickEventArgs e)
    {
        ScriptManager.RegisterStartupScript(this, this.GetType(), "send_success", "location.replace('" + (String)ViewState["PREV_URL"] + "')", true);
        //ScriptManager.RegisterStartupScript(this, this.GetType(), "send_success", "location.replace('" + "/barsroot/barsweb/dynform.aspx?form=frm_fin2_kart_kl&rnk=" + tb_RNK.Text + "')", true);
    }
}
