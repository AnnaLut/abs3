...
<!--PSEM= Найменування спецпараметра;VALUE=Значення-->
...
<xsl:text>Найменування спецпараметра</xsl:text></TD>
<TD width="30%">
<xsl:attribute name="onclick"><xsl:text>Sort('VALUE')</xsl:text></xsl:attribute>
<xsl:text>Значення</xsl:text></TD>
<TD width="10%">
...
<TD>
  ...
  <xsl:attribute name="maxlength">
    <xsl:value-of select="MAX_CHAR" />
  </xsl:attribute>
  <xsl:attribute name="id">
    <xsl:value-of select="concat('VALUE_',position())" />
  </xsl:attribute>
