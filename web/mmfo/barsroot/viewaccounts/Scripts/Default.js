...
function onPopulate(result) {
    ...
    fnInitTab("webtab", array, 700, "onChangeTab");
...
function ForUpdateAccount() {
    ...
    for (key in sp_data) {
        switch (key) {
            case "72": result[27] = sp_data[key]; break;//код ТАРИФНОГО пакету
            case "2": result[28] = sp_data[key]; break;//Спеціальний параметр R013
            case "55": result[29] = sp_data[key]; break;//Номер договору
            case "4": result[30] = sp_data[key]; break;//Код сроку кред/деп рахунків (S180) S180
            case "9": result[31] = sp_data[key]; break;//Код сроку "до погашення" (S240) S240
        }
    }
    return result;
}
