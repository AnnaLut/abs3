...
public partial class docinput_editprops : BarsPage
{
    ...
    private void FillData()
    {
        ...
        string sql =
            "select OPER.ref, OPER.tt, OPER.userid, OPER.nlsa, OPER.s/v1.denom s, v1.lcv lcv_a, OPER.vdat, OPER.s2/v2.denom s2, v2.lcv lcv_b ,OPER.mfob, OPER.nlsb, OPER.dk, OPER.sk, OPER.datd from oper OPER, tabval$global v1, tabval$global v2 where OPER.kv=v1.kv AND OPER.kv2=v2.kv and OPER.branch LIKE SYS_CONTEXT ('bars_context','user_branch_mask') AND OPER.pdat >= :pdat {0}";
        ...
        if (string.IsNullOrEmpty(pDat))
            pDat = DateTime.Now.AddDays(-35).ToString("dd/MM/yyyy");
        ...
        {
            trDateFilter.Visible = true;
            sdsDocs.SelectCommand = string.Format(sql, " and userid=bars.user_id ", pDat);
            panelTitle = string.Format(panelTitle, "користувача ", pDat);
        }
        else if (Request["mode"] == "2")
        {
            sdsDocs.SelectCommand = string.Format(sql, " and vdat=bankdate ", pDat);
            panelTitle = "Перегляд документів за сьогодні";
        }
        else if (Request["ref"] != null)
        {
            sdsDocs.SelectCommand = string.Format(sql, " and ref=" + Request["ref"], pDat);
        ...
        if (Request["FilterSQL"] != null && Request["FilterSQL"] != "")
        {
            sdsDocs.SelectCommand += " and " + Request["FilterSQL"];
        }
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            deStartDate.Date = DateTime.Now.AddDays(-35);
