...
function cDocHand(n, form, noreload) {
    try {
        ...
        if (xmlDoc.parseError.errorCode != 0) {
            window.showModalDialog("dialog.aspx?type=err&r=" + Math.random(), "", "dialogWidth:800px;dialogHeight:768px;center:yes;edge:sunken;help:no;status:no;");
            return false;
        }
        ...
        if (n == 0) {
            form.__RNK_A.value = xmlDoc.getElementsByTagName("Rnk").item(0).text;
            if (!noreload) {
                if (document.getElementById("__FLAGS").value.substr(64, 1) == "1")
                    form.Nam_A.value = xmlDoc.getElementsByTagName("Nmk").item(0).text;
                else
            ...
            form.__RNK_B.value = xmlDoc.getElementsByTagName("Rnk").item(0).text;
            InitSumFields();
            if (isGetPerson) {
                getPersonData(xmlDoc);
            }
            ...
            var countryCode = xmlDoc.getElementsByTagName("CountryCode").item(0).text;
            if (countryCode && form.reqv_KOD_G && ",045,AA3,AA4,AA5,AA6,AA7,AA9,AAB,AAC,AAE,AAK,AAL,AAM,DPF".indexOf(form.__TT.value) > 0 ) {
                fillAndDisableReq("reqv_KOD_G", "CountryCode");
            }
            if (form.reqv_KURS)
...
function getPersonData(xmlDoc) {
    ...
    fillAndDisableReq("reqv_IDDO", "PersonIdActualDate");
}
...
function cDocHand_link(form, n, ctrl, ctrl_nms) {
    try {
        ...
        if (xmlDoc.parseError.errorCode != 0) {
            window.showModalDialog("dialog.aspx?type=err", "", "dialogWidth:800px;dialogHeight:768px;center:yes;edge:sunken;help:no;status:no;");
            return false;
        }
