...
window.onload = function () {
    ...
    if(location.search.indexOf("refDoc=") > 0 && document.getElementById("__FLAGS").value.substr(65, 1) == "1")
       cDocHand(5, document.forms[0]);
    ...
    if (document.getElementById("__FLAGS").value.substr(19, 1) == "1" && ("0" == document.getElementById("__DK").value || "2" == document.getElementById("__DK").value)) 
    {
        ...
        document.getElementById("SideB").style.top = document.getElementById('tdDebet').offsetTop;
        document.getElementById("SideA").style.top = document.getElementById('tdKredit').offsetTop;
    }
};
...
function GetBankName(evt) {
    ...
    var VK_F8 = 119;
    if (VK_F8 == charCode) {
        if (evt.srcElement.id === "Nam_A" || evt.srcElement.id === "Nam_B") {
            var side = evt.srcElement.id.substr(4);
            document.getElementById("Id_" + side).value = document.getElementById('__BANKOKPO').value;
            document.getElementById("Nam_" + side).value = document.getElementById('__BANKNAME').value;
        }
        else {
...
function InitSumFields() {
    ...
    init_numedit("CrossRat", ("" == document.getElementById("CrossRat").value) ? (0) : (document.getElementById("CrossRat").value), (document.getElementById("__FLAGS").value.substr(58, 1) == "1" || document.getElementById("__FLAGS").value.substr(14, 1) == "1") ? (2) : (12));
...
function callOplDoc(form, signData) {
    ...
    webService.useService("DocService.asmx?wsdl", "Doc");
    if (document.getElementById("__VALIDATE_MODE").value === "1") {
        prepareData.data = data;
        prepareData.tags = tags;
        prepareData.checkDocsList = checkDocsList;
        webService.Doc.callService(onValidateDoc, "ValidateDoc", data, tags, checkDocsList);
    }
    else
...
function onValidateDoc(result) {
    ...
    var messageType = result.value.MessageType;
    var goNext = true;
    if (messageType === "DIALOG") {
        goNext = Dialog(message, "confirm");
    }
    ...
    document.getElementById("btPayIt").disabled = goNext;
    if (goNext) {
        webService.Doc.callService(onPayDoc, "PayDoc", prepareData.data, prepareData.tags, prepareData.checkDocsList);
    }
}
...
// Перевірка TT - чи потрібно робити верифікацію за допомогою СМС
function CheckTTForPayVerification(tt) {
    var callObj = webService.createCallOptions();
    callObj.async = false;
    callObj.funcName = "PaymentVerificationTT";
    callObj.params = new Array();
    callObj.params.tt = tt;
...
function getError(result, modal) {
    if (result.error) {
        if (window.dialogArguments || parent.frames.length == 0 || modal) {
            window.showModalDialog("dialog.aspx?type=err&" + Math.random(), "", "dialogWidth:1024px;dialogHeight:768px;center:yes;edge:sunken;help:no;status:no;");
        }
        else
