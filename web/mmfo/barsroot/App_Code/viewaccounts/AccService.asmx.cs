...
namespace ViewAccounts
{
    ...
    public class AccService : Bars.BarsWebService
    {
        ...
        public IValidateAcctRepository _validRepo
        {
            get
            {
                ...
                return kernel.Get<IValidateAcctRepository>();
            }
        }
        ...
        private bool IsTemplateRequireSetMaxSum(string templateID)
        {
            InitOraConnection(Context);
            SetRole(base_role);
            ClearParameters();
            SetParameters("tpl_id", DB_TYPE.Varchar2, templateID, DIRECTION.Input);

            var required = SQL_SELECT_reader(@"select count(*) from DOC_DIALOGREQ where DOC_ID = :tpl_id");
            return Convert.ToInt16(required[0]) > 0;
        }

        private void SaveMaxSum(string acc, string maxSum)
        {
            InitOraConnection(Context);
            SetRole(base_role);
            ClearParameters();
            SetParameters("acc", DB_TYPE.Varchar2, acc, DIRECTION.Input);
            SetParameters("val", DB_TYPE.Varchar2, maxSum, DIRECTION.Input);
            SQL_NONQUERY(@"begin accreg.setAccountwParam(:acc, 'MAXCRSUM', :val); end;");
        }

        [WebMethod(EnableSession = true)]
        public decimal GetCreditMaxSum(string acc, string templateID)
        {
            decimal result = -1;
            if (IsTemplateRequireSetMaxSum(templateID))
            {
                ClearParameters();
                SetParameters("acc", DB_TYPE.Varchar2, acc, DIRECTION.Input);
                ...
                var maxSum =
                    SQL_SELECT_reader(
                        @"SELECT W.VALUE FROM accounts a, ACCOUNTSW w WHERE a.acc=W.ACC and W.TAG='MAXCRSUM' and A.ACC = :acc");
                result = Convert.ToDecimal((maxSum[0]));
        ...
        [WebMethod(EnableSession = true)]
        public string GetFileForPrint(string acc, string templateID, string maxSum)
        {
        ...
        private Customer GetCustomer(decimal customerId)
        {
            ClearParameters();
            SetParameters("rnk", DB_TYPE.Decimal, customerId, DIRECTION.Input);
            ...
            return new Customer
            {
                Name = Convert.ToString(customerParam[0]),
                Type = Convert.ToDecimal(customerParam[1]),
                Sed = Convert.ToString(customerParam[2]),
                IsSpd = Convert.ToString(customerParam[2]).Trim() == "91"
            };
        }
        ...
        public object[] Populate(decimal acc, string rnk)
        {
            ...
            try
            {
                ...
                var customerParam = SQL_SELECT_reader(@"select 
                                                        ...
                                                        where 
                                                            rnk=:rnk");
                var customer = GetCustomer(Convert.ToDecimal(rnk));
                result[56] = customer.Name; //customerParam[0].ToString();// SQL_SELECT_scalar("select nmk||'(РНК '||rnk||',НД '||nd||')' from customer where rnk=:rnk").ToString();
                ClearParameters();
                ...
                if (acc != 0)
                {
                    ...
                    result[63] = temp[30]; // _Ob22;  // 20.03.17 update
                    ...
                    result[61] = temp[29]; //признак доступності редагування тарифів
                    result[62] = (new Bars.Configuration.Module_Accounts().DisableEditBlkType
                        && (customer.Type == 2 //(Convert.ToString(customerParam[1]) == "2" 
                        || customer.IsSpd) /*Convert.ToString(customerParam[2]).Trim() == "91")*/) ? "0" : "1"; //признак можливості редагування параметррів блокування по Д/К
                }
                if (acc == 0)
                {
                    result[0] = "";
                ...
                //Ob22
                if (result[63] != null)
                    result[64] = SQL_SELECT_scalar("SELECT name FROM v_sb_ob22 WHERE ob22='" + result[63] + "' and r020='" + result[2] + "'");
                    ...
                    if (!string.IsNullOrEmpty(paramsList))
                    {
                        ...
                        if (count > 0)
                        {
                            ...
                            query = @"select 
                                        ...
                                        s.name, 
                                        s.semantic, 
                                        decode(:acc, '0', null,f_read_sp(s.spid,:acc)),
                                        s.max_char 
                                    from 
                                        sparam_list s, 
                                        ps_sparam p 
                                    where 
                                        ...
                                        and s.inuse = 1 
                                        and p.nbs=:nbs 
                                        and (opt=1 or s.name in " + paramsList + " )";
                        }
                        else
                        {
                            ClearParameters();
                        ...
                        while (SQL_Reader_Read())
                        {
                            ...
                            spr.MaxLength = Convert.ToDecimal(reader[4]);
            ...
            try
            {
                ...
                if (count != 0)
                {
                    ...
                    ds = SQL_SELECT_dataset(@"SELECT 
                                                ...
                                                l.name PName, 
                                                l.semantic PSem, 
                                                upper(l.tabname) PTab, 
                                                upper(rtrim(l.type)) PType, 
                                                upper(l.nsiname) PNSI, 
                                                upper(l.pkname) PNamePK,
                                                '' Value,
                                                '' HREF,
                                                s.opt Opt,
                                                s.sqlval,
                                                upper(l.nsisqlwhere) NsiSql, 
                                                l.sqlcondition, 
                                                l.tag, 
                                                l.tabcolumn_check,
                                                l.MAX_CHAR
                ...
                else
                {
                    ...
                    ds = SQL_SELECT_dataset(@"SELECT 
                                                ...
                                                semantic PSem, 
                                                upper(tabname) PTab, 
                                                upper(rtrim(type)) PType, 
                                                upper(nsiname) PNSI, 
                                                upper(pkname) PNamePK,
                                                '' Value,
                                                '' HREF,
                                                '' Opt, 
                                                '' SqlVal,
                ...
                try
                {
                    ...
                    if (CodValutes != null)
                    {
                        ...
                        for (int i = 0; i < CodValutes.Length; i++)
                        {
                            ...
                            //result = UpdateAccount(gen, acc);
                            //if (sp != null) UpdateSP(acc, sp, result);
                            var tacc = CreateAccountByKv(Convert.ToDecimal(result),
                                                         Convert.ToDecimal(CodValutes[i]));
                            if (tacc[1] != "null")
                            {
                                dubl_err += tacc[1] + " \n";
                            }
                        ...
                        gen[6] = old_kv;
                        if (dubl_err.Length > 0)
                        {
                            throw new Exception("CheckDbP::" + dubl_err + "::CheckDbP\n");
                        }
                    }


                    if (gen != null && gen[5].Trim().StartsWith("2"))
                    {
                        ValidateCustomer(Convert.ToDecimal(result), Convert.ToDecimal(gen[4]));
                    }


                    /*if (acc == 0)
				    {
				        var validCustomer = _validRepo.ValidateCustomer(Convert.ToDecimal(gen[4]));
				        if (validCustomer.Status == ValidateStatus.Error)
				        {
				            throw new Exception("NotValidCust:" + validCustomer.Message);
				        }
				    }*/
        ...
        private string[] CreateAccountByKv(decimal accm, decimal kv)
        {
            ...
            try
            {
                var message = "";
                ClearParameters();
                SetParameters("p_acc", DB_TYPE.Decimal, accm, DIRECTION.InputOutput);
                SetParameters("p_kv", DB_TYPE.Decimal, kv, DIRECTION.Input);
                SetParameters("p_errmsg", DB_TYPE.Varchar2, message, DIRECTION.Output);
                SQL_PROCEDURE("ACCREG.DUPLICATE_ACC");

                result[0] = Convert.ToString(GetParameter("p_acc"));
                result[1] = Convert.ToString(GetParameter("p_errmsg"));
            }
            finally { }
            return result;
        }

        private void ValidateCustomer(decimal acc, decimal rnk)
        {
            var customer = GetCustomer(rnk);
            if (acc == 0 && (customer.Type == 2 || customer.IsSpd))
            {
                var validCustomer = _validRepo.ValidateCustomer(rnk);
                if (validCustomer.Status == ValidateStatus.Error)
                {
                    throw new Exception("NotValidCust::" + validCustomer.Message + "::NotValidCust");
                }
            }
        }
        ...
        [WebMethod(EnableSession = true)]
        public string[] OpenAcc(string acc, string nls, string kv, string nms_new)//открытие точечного счета
        {
            string[] result = new string[2];
            try
            {
                ...
                SetRole(base_role);
                SetParameters("acc", DB_TYPE.Decimal, acc, DIRECTION.Input);
                ArrayList reader = SQL_reader("SELECT u.rnk,a.isp,a.nms,a.grp,a.ob22,a.branch FROM cust_acc u, accounts a WHERE a.acc=:acc AND a.acc=u.acc");
                ...
                string ob22 = Convert.ToString(reader[4]);
                ...
                SetParameters("mod_", DB_TYPE.Decimal, 99, DIRECTION.Input);
                SetParameters("p1_", DB_TYPE.Varchar2, 0, DIRECTION.Input);
                ...
                SetParameters("tip_", DB_TYPE.Varchar2, "ODB", DIRECTION.Input);
                ...
                SetParameters("pap_", DB_TYPE.Decimal, null, DIRECTION.Input);
                SetParameters("vid_", DB_TYPE.Decimal, null, DIRECTION.Input);
                SetParameters("pos_", DB_TYPE.Decimal, null, DIRECTION.Input);
                SetParameters("sec_", DB_TYPE.Decimal, 0, DIRECTION.Input);
                SetParameters("seci_", DB_TYPE.Decimal, null, DIRECTION.Input);
                SetParameters("seco_", DB_TYPE.Decimal, null, DIRECTION.Input);
                SetParameters("blkd_", DB_TYPE.Decimal, null, DIRECTION.Input);
                SetParameters("blkk_", DB_TYPE.Decimal, null, DIRECTION.Input);
                SetParameters("lim_", DB_TYPE.Decimal, null, DIRECTION.Input);
                SetParameters("ostx_", DB_TYPE.Varchar2, null, DIRECTION.Input);
                SetParameters("nlsalt_", DB_TYPE.Varchar2, null, DIRECTION.Input);
                SetParameters("tobo_", DB_TYPE.Varchar2, branch, DIRECTION.Input);
                int iresult = SQL_PROCEDURE("accreg.SetAccountAttr");

                result[1] = Convert.ToString(GetParameter("accR_"));
                //
        ...
        public object[] GetRates(string[] data)
        {
            ...
            try
            {
                ...
                if (isTarifScheme > 0)
                {
                    ...
                    if (reader.Count > 0)
                    {
                        tarifSchemeId = Convert.ToDecimal(reader[0]);
                        tarifSchemeName = Convert.ToString(reader[1]);
                    }

                    SetParameters("p_accn", DB_TYPE.Decimal, acc, DIRECTION.Input);
                    ds_list = SQL_SELECT_dataset(@"SELECT 
                                                        ...
                                                        a.kv_smin kv_smin,
                                                        a.kv_smax kv_smax,
														case when c.kod is null then 0 else 1 end diff
											        FROM 
                                                        v_acc_tarif a,
														acc_tarif c
											        WHERE 
        ...
        public object[] GetSob(string[] data)
        {
            ...
            try
            {
                ...
                //ds = SQL_SELECT_dataset(@"SELECT  a.id,    //-> EV_ID
                //      TO_CHAR(a.fdat,'DD/MM/YYYY') fdat,   //-> EV_DT->
                //                                  a.isp,   //->USR_ID
                //                                  s.fio,   //->USR_NM
                //                                  a.txt    //->EV_DSC
                ...
                //					 " order by " + order, startpos, pageSize);
                count = Convert.ToInt32(SQL_SELECT_scalar("SELECT count(*) from V_ACCOUNT_EVENTS WHERE  " + _acc));
                ds = SQL_SELECT_dataset(@"SELECT  EV_ID id,
                                                  to_char(EV_DT,'dd/mm/yyyy') fdat,
                                                  USR_ID isp,
                                                  USR_NM fio,
                                                  EV_DSC txt
									                FROM V_ACCOUNT_EVENTS
									                 WHERE " + _acc +
                                                     " order by " + order, startpos, pageSize);
...
//                string ostx = (Ostx == 0) ? ("") : (Ostx.ToString());
	        string ostx = ((Ostx != null) &&  (Ostx != 0))?  (Ostx.ToString()) :  ("NULL"); 
                ...
                if (ob22 == "" && GetOb22AmmForAcc(Nbs != "" ? Nbs : Nls.Substring(0, 4)) > 0)
                    throw new Exception("CheckGP::Не заповнено параметр ob22::CheckGP\n");
            ...
            try
            {
                ...
                for (int i = 0; i < data.Length - 2; i += 2)
                {
                    ...
                    if (reader.Count > 0)
                    {
                        ...
                        {
                            maxChar = Convert.ToDecimal(reader[5]);
                        }
                        string parameterName = Convert.ToString(reader[6]);
                        ...
                        if (!string.IsNullOrEmpty(spValue) && spValue.Length > maxChar)
                        {
                            throw new Exception("Довжина значення параметра " + parameterName +
                                " перевищує його допустипу довжину " + maxChar + " символів");
                        }
                        ...
                        else
                        {
                            ...
                            if (count == 0)
                            {
                                SQL_NONQUERY("insert into " + tabname + " (" + name + ", acc" +
                                             ((string.IsNullOrEmpty(tag)) ? ("") : (",tag")) + ") values (:val, :acc" +
                                             ((string.IsNullOrEmpty(tag)) ? ("") : (",:tag")) + ")");
                            }
                            else
                            {
                                SQL_NONQUERY("update " + tabname + " set " + name + "=:val where acc=:acc" +
                                             ((string.IsNullOrEmpty(tag)) ? ("") : (" and tag=:tag")));
                            }
                        }
                    }
                }
        ...
        [WebMethod(EnableSession = true)]
        public int GetOb22AmmForAcc(string nbs)
        {
            string sql = String.Format(@"select count(*) from v_sb_ob22  where R020 like '{0}%'", nbs);
            using (var connection = Bars.Classes.OraConnector.Handler.UserConnection)
            {
                return connection.Query<int>(sql).SingleOrDefault();
            }
        }

        /// <summary>
        /// Syncronize rates result datasets
        /// </summary>
        /// <param name="destination"></param>
        /// <param name="source"></param>
        private void FormatRatesResult(DataSet destination, DataSet source)
        {
            for (int i = 0; i < destination.Tables[0].Rows.Count; i++)
            {
                var row = destination.Tables[0].Rows[i];
                var row_list = source.Tables[0].Select("kod=" + row["kod"])[0];
                FormatRatesResultRows(row, row_list);
            }
            destination.Tables[0].AcceptChanges();
        }
        private void FormatRatesResultRows(DataRow destination, DataRow source)
        {
            foreach (var cell in GetRatesUpdateVals())
            {
                if (cell == "bdat" || cell == "edat") destination[cell] = source[cell];
                else if (source[cell].ToString() != "") destination[cell] = source[cell];
            }
        }
        private List<string> GetRatesUpdateVals()
        {
            var items = new List<string>();
            items.Add("tar");       //Сумма за 1 документ 
            items.Add("pr");        //% от суммы документа 
            items.Add("smin");      //минимальная сумма тарифа 
            items.Add("smax");      //максимальная сумма тарифа 
            items.Add("kv_smin");   //Валюта минимальной граничной суммы 
            items.Add("kv_smax");   //Валюта максиимальной граничной суммы 
            items.Add("bdat");      //Дата начала действия тарифа 
            items.Add("edat");      //Дата окончания действия тарифа 
            return items;
        }
