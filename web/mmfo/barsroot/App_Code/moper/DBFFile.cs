...
public class DBFFile
{
    ...
    public string IntTT = "I00";
    public string ExtTT = "I01";
            ...
            for (int i = 0; i < TableData.Columns.Count; i++)
            {
                if (TableData.Columns[i].DataType == typeof(string))
                    for (int j = 0; j < TableData.Rows.Count; j++)
                    {
                        ...
                        oldStr = ConvertEncoding(oldStr, Encoding.GetEncoding("IBM866"), Encoding.GetEncoding("Windows-1252"));
    ...
    private bool CheckSumCheck()
    {
        ...
        try
        {
                       cmd.CommandText = "SET ROLE bars_access_defrole";
            cmd.ExecuteNonQuery();
    ...
    public MOperErr ConvertDataToNewScheme()
    {
        ...
        // заполняем данные
        OracleConnection con = OraConnector.Handler.UserConnection;
        OracleCommand cmd = new OracleCommand("SET ROLE bars_access_defrole", con);
        try
        {
            cmd.ExecuteNonQuery();
            ...
            for (int i = 0; i < this.TableData.Rows.Count; i++)
            {
                ...
                // Счет А
                row["NLSA"] = Convert.ToString(TableData.Rows[i]["NLS"]);

                // МФО банка корреспондента
                row["MFOB"] = Convert.ToString(TableData.Rows[i]["MFO"]);

                // Счет Б
                row["NLSB"] = Convert.ToString(TableData.Rows[i]["NLSK"]);
                ...
                // Дата валютирования
                row["VDAT"] = (TableData.Rows[i]["VDATE"] == DBNull.Value) ? (((DateTime)row["DATD"]).Add(DateTime.Now.TimeOfDay)) : (Convert.ToDateTime(TableData.Rows[i]["VDATE"]));

                // Плательщик
                row["NAMEA"] = Convert.ToString(TableData.Rows[i]["PLAT"]);

                // Получатель
                row["NAMEB"] = Convert.ToString(TableData.Rows[i]["POLU"]);

                // Назначение платежа
                row["NAZN"] = Convert.ToString(TableData.Rows[i]["NAZ1"]);

                // ОКПО А
                row["OKPOA"] = Convert.ToString(TableData.Rows[i]["KOD"]);

                // ОКПО Б
                row["OKPOB"] = Convert.ToString(TableData.Rows[i]["KODB"]);
    ...
    private string CheckAccountAccess(string MFO, string NLS, int DK, OracleConnection Con) 
    {
        ...
        OracleCommand com = new OracleCommand("SET ROLE bars_access_defrole", Con);
        com.ExecuteNonQuery();
