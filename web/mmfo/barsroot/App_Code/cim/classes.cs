...
namespace barsroot.cim
{
    ...
    public class CimManager
    {
            ...
            if (readParams)
            {
                ...
                try
                {
                    ...
                    if (oraRdr.Read())
                    {
                        ...
                        string roMode = Convert.ToString(oraRdr["readonly_mode"]);

                        AccessFlag = (roMode == "1" && Branch.Length > 8) ? ("1") : ("0");
                        ...
                        HttpContext.Current.Session[Constants.StateKeys.AccessFlag] = AccessFlag;
                    }
                }
        ...
        public string AccessFlag = "0"; // 0 - полный доступ, 1 - readonly
    ...
    public class Contract : CimManager
    {
        ...
        public string Bank_change { get; set; }
        ...
        private void readContract()
        {
            ...
            if (this.ContrId.HasValue)
            {
                try
                {
                    ...
                    oraCmd.CommandText = "select contr_id,contr_type, contr_type_name, num, subnum, rnk,open_date,close_date,kv,s,benef_id,status_id,status_name,comments, bank_change, branch, branch_name, owner_uid, owner_name, bic, b010, service_branch, ea_url from v_cim_all_contracts where contr_id=:contrId";
                    ...
                    if (oraRdr.Read())
                    {
                        ...
                        this.Bank_change = Convert.ToString(oraRdr["bank_change"]);
                        ...
                        this.ServiceBranch = Convert.ToString(oraRdr["service_branch"]);
                        this.EaUrl = Convert.ToString(oraRdr["ea_url"]);
        ...
        public decimal SaveContract()
        {
            ...
            {
                oraCmd = oraConn.CreateCommand();
                if (DateClose.HasValue && DateOpen > DateClose)
                ...
                {
                    oraCmd.Parameters.Add("p_contr_id", OracleDbType.Decimal, ContrId, ParameterDirection.Input);
                    oraCmd.Parameters.Add("p_num", OracleDbType.Varchar2, Num, ParameterDirection.Input);
                    oraCmd.Parameters.Add("p_subnum", OracleDbType.Varchar2, SubNum, ParameterDirection.Input);
                    oraCmd.Parameters.Add("p_s", OracleDbType.Decimal, Sum, ParameterDirection.Input);
                    oraCmd.Parameters.Add("p_benef_id", OracleDbType.Decimal, BenefId, ParameterDirection.Input);
                    oraCmd.Parameters.Add("p_open_date", OracleDbType.Date, DateOpen, ParameterDirection.Input);
                    oraCmd.Parameters.Add("p_close_date", OracleDbType.Date, DateClose, ParameterDirection.Input);
                    oraCmd.Parameters.Add("p_comments", OracleDbType.Varchar2, Comments, ParameterDirection.Input);
                    oraCmd.Parameters.Add("p_bank_change", OracleDbType.Varchar2, Bank_change, ParameterDirection.Input);
                    ...
                    if (ContrType == 0 || ContrType == 1)
                    {
                        ...
                        oraCmd.Parameters.Add("p_spec_id", OracleDbType.Decimal, TradeContractInfo.SpecId, ParameterDirection.Input);
                        oraCmd.Parameters.Add("p_subject_id", OracleDbType.Decimal, TradeContractInfo.SubjectId, ParameterDirection.Input);
                        oraCmd.Parameters.Add("p_without_acts", OracleDbType.Decimal, TradeContractInfo.WithoutActs, ParameterDirection.Input);
                        oraCmd.Parameters.Add("p_deadline", OracleDbType.Decimal, TradeContractInfo.Deadline, ParameterDirection.Input);
                        oraCmd.Parameters.Add("p_txt_subject", OracleDbType.Varchar2, TradeContractInfo.SubjectText, ParameterDirection.Input);
                    }
                    else if (ContrType == 2)
                    {
                        ...
                        ":p_name,:p_add_agree,:p_percent_nbu_type,:p_percent_nbu_info,:p_r_agree_date,:p_r_agree_no,:p_prev_doc_key,:p_prev_reestr_attr,:p_ending_date_indiv," +
                        ":p_parent_ch_data,:p_ending_date,:p_f503_reason,:p_f503_state,:p_f503_note, :p_f504_reason, :p_f504_note, :p_f503_percent_type, :p_f503_percent_base, " +
                        ":p_f503_percent_margin, :p_f503_percent, :p_f503_purpose, :p_f503_percent_base_t, :p_f503_change_info";
                        ...
                        oraCmd.Parameters.Add("p_prev_doc_key", OracleDbType.Decimal, CreditContractInfo.CreditDocKey, ParameterDirection.Input);
                        oraCmd.Parameters.Add("p_prev_reestr_attr", OracleDbType.Varchar2, CreditContractInfo.CreditPrevReestrAttr, ParameterDirection.Input);
                        oraCmd.Parameters.Add("p_ending_date_indiv", OracleDbType.Date, CreditContractInfo.CrdIndEndDate, ParameterDirection.Input);
                        oraCmd.Parameters.Add("p_parent_ch_data", OracleDbType.Varchar2, CreditContractInfo.CreditCrdParentChData, ParameterDirection.Input);
                        oraCmd.Parameters.Add("p_ending_date", OracleDbType.Date, CreditContractInfo.CreditEndingDate, ParameterDirection.Input);
                        oraCmd.Parameters.Add("p_f503_reason", OracleDbType.Decimal, CreditContractInfo.F503_Reason, ParameterDirection.Input);
                        oraCmd.Parameters.Add("p_f503_state", OracleDbType.Decimal, CreditContractInfo.F503_State, ParameterDirection.Input);
                    ...
                    oraCmd.CommandText = @"begin cim_mgr.update_contract(:p_contr_id,
                                                                ...
                                                                :p_bic,
                                                                :p_b010,
                                                                :p_service_branch " + addParams + "); end;";
                    oraCmd.ExecuteNonQuery();
                }
                // Insert
                else
                {
                    ...
                    oraCmd.Parameters.Add("p_rnk", OracleDbType.Decimal, Rnk, ParameterDirection.Input);
                    oraCmd.Parameters.Add("p_num", OracleDbType.Varchar2, Num, ParameterDirection.Input);
                    oraCmd.Parameters.Add("p_subnum", OracleDbType.Varchar2, SubNum, ParameterDirection.Input);
                    oraCmd.Parameters.Add("p_s", OracleDbType.Decimal, Sum, ParameterDirection.Input);
    ...
    public class CreditContractClass : CimManager
    {
        ...
        public decimal? F503_Margin { get; set; }
        ...
        public string F503_Change_Info { get; set; }
        ...
        private void readCreditContract(bool buildCredGraph)
        {
            ...
            if (this.ContrId.HasValue)
            {
                try
                {
                    ...
                    oraCmd = oraConn.CreateCommand();
                    oraCmd.Parameters.Add("contrId", OracleDbType.Decimal, this.ContrId, ParameterDirection.Input);
                    oraCmd.CommandText = @"select contr_id,percent_nbu, s_limit, creditor_type, credit_type, credit_term, credit_prepay, name, add_agree, percent_nbu_type, " +
                        "percent_nbu_info, r_agree_date, r_agree_no, prev_ddoc_key, prev_reestr_attr, ending_date_indiv, parent_ch_data, ending_date, s_in_pl, s_out_pl, " +
                        "s_in_pl-s_out_pl, s_v_pr, z_pr, s_v_pr+z_pr,s_pr_nbu,s_dod_pl, f503_reason, f503_state, f503_note, s-s_in_pl, borrower_id, f504_reason, f504_note, " +
                        "f503_percent_type, f503_percent_base, f503_percent_margin, f503_percent, f503_purpose, f503_percent_base_t, f503_change_info from v_cim_credit_contracts where contr_id=:contrId";

                    oraRdr = oraCmd.ExecuteReader();
                    if (oraRdr.Read())
                    {
                        ContrId = (decimal?)(oraRdr["contr_id"]);
                        NbuPercent = (decimal)(oraRdr["percent_nbu"]);
                        if (oraRdr["s_limit"].ToString() != "")
                            CrdLimit = (decimal?)(oraRdr["s_limit"]);
                        if (oraRdr["creditor_type"].ToString() != "")
                            CreditorType = (decimal?)(oraRdr["creditor_type"]);
                        if (oraRdr["credit_type"].ToString() != "")
                            CreditType = (decimal?)(oraRdr["credit_type"]);
                        CreditTerm = (decimal)(oraRdr["credit_term"]);
                        if (oraRdr["credit_prepay"].ToString() != "")
                            CreditPrepay = (decimal?)(oraRdr["credit_prepay"]);
                        ...
                        CreditAgreeNum = oraRdr["r_agree_no"].ToString();
                        if (!oraRdr.IsDBNull(13))
                        ...
                        if (oraRdr["f503_change_info"].ToString() != "")
                            F503_Change_Info = oraRdr["f503_change_info"].ToString();
                    }
                    else
                        this.ContrId = null;
        ...
        public void UpdatePeriod(CreditPeriodClass cpc)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(cpc.RowId)) // insert
                {
                    ...
                    oraCmd.Parameters.Add("p_payment_day", OracleDbType.Decimal, cpc.PaymentDay, ParameterDirection.Input);
                    oraCmd.Parameters.Add("p_percent_day", OracleDbType.Decimal, cpc.PercentDay, ParameterDirection.Input);

                    oraCmd.CommandText = "begin cim_mgr.create_credgraph_period(:p_contr_id, :p_end_date, :p_cr_method, :p_payment_period,:p_z,:p_adaptive, :p_percent,:p_percent_nbu,:p_percent_base,:p_percent_period,:p_payment_delay,:p_percent_delay,:p_get_day,:p_pay_day, :p_payment_day, :p_percent_day); end;";
                    oraCmd.ExecuteNonQuery();
                }
                else // update
                {
                    ...
                    oraCmd.CommandText = "begin cim_mgr.update_credgraph_period(:p_contr_id, :p_row_id, :p_end_date, :p_cr_method, :p_payment_period,:p_z,:p_adaptive, :p_percent,:p_percent_nbu,:p_percent_base,:p_percent_period,:p_payment_delay,:p_percent_delay,:p_get_day,:p_pay_day, :p_payment_day, :p_percent_day); end;";
                    oraCmd.ExecuteNonQuery();
                }
            }
            catch (System.Exception ex)
