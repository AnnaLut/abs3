...
public class Tools
{
    ...
    public static Byte[] get_cliet_picture(Decimal CustomerID, String TypePicture)
    {
        ...
        try
        {
            ...
            if (rdr.Read())
          {
            img = rdr["image"] != System.DBNull.Value  ? (byte [])rdr["image"]
	    : null;
          }
    ...
    /// <summary>
    /// чи номер телефону складається з однакових цифр
    /// </summary>
    /// <param name="p_cell">номер телефону з форми</param>
    /// <returns></returns>
    public static String Get_CellPhoneExists(String p_cell, Decimal p_rnk)
    {
        ...
        try
        {
            ...
            cmdSQL.CommandText = "select mpno_ex( :p_cell, :p_rnk) from dual";
            cmdSQL.Parameters.Add("p_cell", OracleDbType.Varchar2, p_cell, ParameterDirection.Input);
            cmdSQL.Parameters.Add("p_rnk", OracleDbType.Varchar2, p_rnk, ParameterDirection.Input);

            res = Convert.ToString(cmdSQL.ExecuteScalar());
            return res;
    ...
    public static Decimal get_EADocID(decimal DepositID)
    {
        Decimal EAD_ID = 0;
        ...
        try
        {
            ...
            {
                if (!rdr.IsDBNull(0))
                    EAD_ID = Convert.ToDecimal(rdr.GetOracleDecimal(0).Value);
            }
        }
        finally
        {
    ...
    /// <summary>
    /// Створення запиту на відміну комісії
    /// </summary>
    /// <param name="DepositID">Рефененс депозитного договору</param>
    /// <param name="AgrmntID">Тип ДУ</param>
    /// <param name="commisRef">референс на відміну комісії</param>
    public static decimal CreateCommisRequest(Decimal DepositID, Decimal AgrmntID)
    {
        /*Decimal commisRef;
        commisRef = -13;*/
        OracleConnection connect = OraConnector.Handler.IOraConnection.GetUserConnection();
        ...
        try
        {
            ...
            cmd.CommandText = @"bars.dpt_web.create_commis_request";

            cmd.Parameters.Add("p_return", OracleDbType.Decimal, ParameterDirection.ReturnValue);
            cmd.Parameters.Add("p_dptid", OracleDbType.Decimal, DepositID, ParameterDirection.Input);
            cmd.Parameters.Add("p_agrmntid", OracleDbType.Decimal, AgrmntID, ParameterDirection.Input);
            
            cmd.ExecuteNonQuery();
           return ((OracleDecimal)cmd.Parameters["p_return"].Value).Value;
    ...
    public static String UserFIO() {
        string userFio;
        DbLoggerConstruct.NewDbLogger().Debug("Tools.UserFIO starts");
        OracleConnection connect = OraConnector.Handler.IOraConnection.GetUserConnection();
        try
        {            
            OracleCommand cmd = connect.CreateCommand();
            //передаємо на сторінку фіо користувача
            cmd.CommandText = @"select fio from staff$base where id = user_id";
            OracleDataReader rdr = cmd.ExecuteReader();
            if (rdr.Read())
            {
                userFio = rdr.GetOracleString(0).Value;
            }
            else { userFio = ""; }
        }
        finally
        ...
        DbLoggerConstruct.NewDbLogger().Debug("Tools.UserFIO end");
        return userFio;        
    }

    public static Decimal DPT_IRREVOCABLE(String DepositID)
    {
        Decimal is_irrevocable;
        var dbLogger = DbLoggerConstruct.NewDbLogger();
        OracleConnection connect = OraConnector.Handler.IOraConnection.GetUserConnection();
        try
        {
            OracleCommand cmd = connect.CreateCommand();           
            cmd.CommandText = @"select dpt_irrevocable(:DepositID) from dual";
            cmd.Parameters.Add("DepositID", OracleDbType.Varchar2, DepositID, ParameterDirection.Input);
            OracleDataReader rdr = cmd.ExecuteReader();
            if (rdr.Read())
            {
                is_irrevocable = rdr.GetOracleDecimal (0).Value;
            }
            else { is_irrevocable = 0; }
        }
        finally
        {
            if (connect.State != ConnectionState.Closed)
        ...
        dbLogger.Debug("Tools.DPT_IRREVOCABLE deposit_id = " + DepositID + ", res = " + is_irrevocable );
        return is_irrevocable;
    }


    public static Decimal DPT_HASNO35AGREEMENT(String DepositID)
    {
        Decimal HASNO35AGREEMENT;
        var dbLogger = DbLoggerConstruct.NewDbLogger();
        OracleConnection connect = OraConnector.Handler.IOraConnection.GetUserConnection();
        try
        {
            OracleCommand cmd = connect.CreateCommand();           
            cmd.CommandText = @"select dpt_hasno35agrmnt(:DepositID) from dual";
            cmd.Parameters.Add("DepositID", OracleDbType.Varchar2, DepositID, ParameterDirection.Input);
            OracleDataReader rdr = cmd.ExecuteReader();
            if (rdr.Read())
            {
                HASNO35AGREEMENT = rdr.GetOracleDecimal (0).Value;
            }
            else { HASNO35AGREEMENT = 0; }
        }
        finally
        {
            if (connect.State != ConnectionState.Closed)
        ...
        dbLogger.Debug("Tools.DPT_HAS35AGREEMENT DepositID = "+ DepositID +", res = "+HASNO35AGREEMENT);
        return HASNO35AGREEMENT;
    }
