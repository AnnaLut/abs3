...
namespace BarsWeb.Areas.Payreg.Infrastructure.Repository.DI.Abstract
{
    public interface IPayRegularRepository
    {
        ...
        IQueryable<V_STO_ORDER> GetSbonOrders(OrderParams orderparams);
        ...
        void SetNewOrderStatus(decimal orderId, decimal newStatus);
        string GetRecieverAccountNMS(string MFO, string NLS);
        string GetRecieverAccountOKPO(string MFO, string NLS);
        IQueryable<V_STO_PAY_MGRID> GetPayList(decimal orderId);
        IQueryable<STO_HIST> GetHistList(decimal orderId);
        IQueryable<V_STO_PAYMENTS> GetOrderPays();
    }
}