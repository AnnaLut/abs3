...
namespace BarsWeb.Areas.Payreg.Infrastructure.Repository.DI.Implementation
{
    public class PayRegularRepository : IPayRegularRepository
    {
        ...
        {
            const string query = @"select * from V_STO_ORDER where CUSTOMER_ID = :p_CUSTOMER_ID";
            var parameters = new object[] { 
                new OracleParameter("p_CUSTOMER_ID", OracleDbType.Decimal) { Value = customerId}
            };
            return _entities.ExecuteStoreQuery<V_STO_ORDER>(query, parameters).AsQueryable();
        }

        public IQueryable<V_STO_ORDER> GetSbonOrders(OrderParams orderparams)
        {
            DateTime from = new DateTime();
            DateTime to = new DateTime();
            if (orderparams.RegisteredFrom != null)
            {
                from = DateTime.ParseExact(orderparams.RegisteredFrom, "dd-MM-yyyy", null);
            }
            if (orderparams.RegisteredTo != null)
            {
                to = DateTime.ParseExact(orderparams.RegisteredTo, "dd-MM-yyyy", null);
            }
            

            var query = string.Format(@"select * from V_STO_ORDER where ORD_STATE = :p_status {0} {1}", 
                orderparams.RegisteredFrom == null ? "" : "and REG_ORD_DATE >= :p_from", 
                orderparams.RegisteredTo == null ? "" : "and REG_ORD_DATE <= :p_to");

            var parameters = new object[] { 
                new OracleParameter("p_status", OracleDbType.Decimal) { Value = orderparams.Status},
                new OracleParameter("p_from", OracleDbType.Date) { Value = from },
                new OracleParameter("p_to", OracleDbType.Date) { Value = to }
            };

            return _entities.ExecuteStoreQuery<V_STO_ORDER>(query, parameters).AsQueryable();

            /*
             return _entities.V_STO_ORDER.Where(o => o.ORD_STATE == orderparams.Status && 
                    (orderparams.RegisteredFrom == null || o.REG_ORD_DATE >= from) && 
                    (orderparams.RegisteredTo == null || o.REG_ORD_DATE <= to));
             */
        }
        ...
        public void SetNewOrderStatus(decimal orderId, decimal newStatus)
        {
            var sqlParams = new object[]
                    {
                        new OracleParameter("p_id", OracleDbType.Decimal)
                        {
                            Value = orderId 
                        },
                        new OracleParameter("p_state", OracleDbType.Decimal)
                        {
                            Value = newStatus 
                        }
                    };
            _entities.ExecuteStoreCommand("begin sto_ui.change_order_state(:p_id, :p_state); end;", sqlParams);
        }

        public string GetRecieverAccountNMS(string MFO, string NLS)
        {           
            var sqlParams = new object[]
                    {
                        new OracleParameter("p_MFO", OracleDbType.Varchar2)
                        {
                            Value = MFO 
                        },
                        new OracleParameter("p_NLS", OracleDbType.Varchar2)
                        {
                            Value = NLS 
                        }
                    };
         return _entities.ExecuteStoreQuery<string>("select sto_ui.get_reciever_nms (:p_MFO, :p_NLS) from dual", sqlParams).SingleOrDefault();         
        }
        public string GetRecieverAccountOKPO(string MFO, string NLS)
        {           
            var sqlParams = new object[]
                    {
                        new OracleParameter("p_MFO", OracleDbType.Varchar2)
            ...
            return _entities.ExecuteStoreQuery<string>("select sto_ui.get_reciever_okpo (:p_MFO, :p_NLS) from dual", sqlParams).SingleOrDefault();    
        }
        public IQueryable<V_STO_PAY_MGRID> GetPayList(decimal orderId)
        {
            return _entities.V_STO_PAY_MGRID.Where(p => p.ORDER_ID == orderId);
        }

   

        public IQueryable<V_STO_PAYMENTS> GetOrderPays()
        {
            return _entities.V_STO_PAYMENTS;
        }     
        
        public IQueryable<STO_HIST> GetHistList(decimal orderId)
        {
            const string query = @"select t.id, t.order_id, (select fio from staff$base where id = t.user_id) USER_ID, t.comment_text, to_char(t.sys_time,'dd.mm.yyyy hh:MI:ss') SYS_TIME from sto_order_tracking t where t.order_id = :p_orderid order by t.id";
            var parameters = new object[] { 
                new OracleParameter("p_orderid", OracleDbType.Decimal) { Value = orderId}
            };
            return _entities.ExecuteStoreQuery<STO_HIST>(query, parameters).AsQueryable();
        }
    }
}