using System;
using System.Linq;
using System.Collections.Generic;

namespace BarsWeb.Areas.CreditUi.Models
{
    public class CreateDeal
    {
        public decimal nRNK { get; set; }
        public string OKPO { get; set; }
        public string NMK { get; set; }
        public string CC_ID { get; set; }
        public decimal? ND { get; set; }
        public DateTime Dat1 { get; set; }
        public DateTime? Dat4 { get; set; }
        public DateTime Dat2 { get; set; }
        public DateTime Dat3 { get; set; }
        public decimal nKV { get; set; }
        public string nKVNAME { get; set; }
        public decimal nS { get; set; }
        public decimal nVID { get; set; }
        public string nVIDNAME { get; set; }
        public decimal nISTRO { get; set; }
        public string nISTRONAME { get; set; }
        public decimal nCEL { get; set; }
        public string MS_NX { get; set; }
        public string PRODNAME { get; set; }
        public decimal? nFIN { get; set; }
        public string nFINNAME { get; set; }
        public decimal? nOBS { get; set; }
        public string nOBSNAME { get; set; }
        public string sAIM { get; set; }
        public decimal? ID { get; set; }
        public string NLS { get; set; }
        public string nBANK { get; set; }
        public string nBANKNAME { get; set; }
        public string FLAGS { get; set; }
        public decimal nFREQ { get; set; }
        public string nFREQNAME { get; set; }
        public decimal dfPROC { get; set; }
        public decimal? nBasey { get; set; }
        public string nBaseyNAME { get; set; }
        public decimal dfDen { get; set; }
        public DateTime? DATNP { get; set; }
        public /*decimal?*/string nFREQP { get; set; }
        public string nFREQPNAME { get; set; }
        public decimal? nKom { get; set; }
        public string INIC { get; set; }
        public string AIMNAME { get; set; }
        public Int64? ACC8 { get; set; }
        public int? BASEM { get; set; }
        public decimal? S_SDI { get; set; }
        public decimal? RANG { get; set; }
        public string RANGNAME { get; set; }
        public decimal? METR { get; set; }
        public string METRNAME { get; set; }
        public decimal? METR_R { get; set; }
        public decimal? SN8 { get; set; }
        public decimal? SK4 { get; set; }
        public decimal? CR9 { get; set; }
        public decimal? I_CR9 { get; set; }
        public string I_CR9NAME { get; set; }
        public string DAYSN { get; set; }
        public DateTime? DATSN { get; set; }
        public string DAYNP { get; set; }
        public string DAYNPNAME { get; set; }
    }
}