...
namespace BarsWeb.Areas.Doc.Controllers.Api
{
    ...
    public class AdditionalReqvController : ApiController
    {
        ...
        public HttpResponseMessage GetChildGridInfo(decimal Ref, string KODF)
        {
            try
            {
                var data = _dInf.GetReqDependInfo(Ref,KODF);
                return Request.CreateResponse(HttpStatusCode.OK, data);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        [POST("api/doc/additionalreqv/deldocuments")]
        public HttpResponseMessage DelDocuments(decimal Ref, string KODF)
        {
            try
            {
                _dInf.DelDocuments(Ref, KODF);
