...
namespace BarsWeb.Areas.Doc.Infrastructure.Repository.DI.Implementation
{
    public class AddDocsInfo : IAddDocsInfo
    {
        ...
        public List<AddOtcnTraceChild> GetReqDependInfo(decimal Ref, string KODF)
        {
            string err = null;
            ...
            try
            {
                using (OracleConnection conn = Bars.Classes.OraConnector.Handler.UserConnection)
                {
                    ...
                    string sqlText = String.Format("select  KODP, ZNAP  from otcn_trace_70 where ref  = {0} and KODF = '{1}' order by KODP asc ", Ref, KODF);
                    using (OracleCommand cmd = new OracleCommand(sqlText, conn))
                    {
                        using (var reader = cmd.ExecuteReader())
                        {
        ...
        public void DelDocuments(decimal Ref, string KODF)
        {
            
                OracleConnection connection = OraConnector.Handler.UserConnection;
                try
                {
                    OracleCommand command = new OracleCommand("BARS.P_NBUR_TMP_DEL", connection);
                    command.CommandType = CommandType.StoredProcedure;
                    command.Parameters.Add("p_ref", OracleDbType.Decimal, Ref, ParameterDirection.Input);
                    command.Parameters.Add("p_kodf", OracleDbType.Varchar2, KODF, ParameterDirection.Input);
