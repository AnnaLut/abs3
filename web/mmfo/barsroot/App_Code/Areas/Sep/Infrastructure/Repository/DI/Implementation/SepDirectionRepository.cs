...
namespace BarsWeb.Areas.Sep.Infrastructure.Repository.DI.Implementation
{
    public class SepDirectionRepository : ISepDirectionRepository
    {
        ...
        private void InitSepDirectionSql(string answer)
        {
            ...
            var a = Convert.ToBase64String(sha.ComputeHash(Encoding.UTF8.GetBytes(pass)));
            ...
            switch (answer)
            {
                ...
                case "4": //Разблокировать ответные платежи
                    ...
                    break;
                case "5": //Блокировать все направления СЕП
                    _baseSepDirectionSql = new BarsSql()
                    {
                        SqlText = string.Format(
                    ...
                    break;
                case "6": //Разблокировать все направления СЕП
                    _baseSepDirectionSql = new BarsSql()
                    {
                        SqlText = string.Format(
                    ...
                    break;
                case "7": //Блокировать все направления ВПС
                    _baseSepDirectionSql = new BarsSql()
                    {
                        SqlText = string.Format(
                        @"UPDATE lkl_rrp SET blk=1
                            WHERE 0=blk and mfo in 
                            (select mfo from banks where mfop = {0} AND kodn=6)",
                        getBankMFO.Value),
                        SqlParams = new object[] { }
                    };
                    break;
                case "8": //Разблокировать все направления ВПС
                    _baseSepDirectionSql = new BarsSql()
                    {
                        SqlText = string.Format(
                        @"UPDATE lkl_rrp SET blk=0
