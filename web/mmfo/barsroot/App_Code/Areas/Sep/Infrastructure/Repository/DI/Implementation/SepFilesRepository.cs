...
namespace BarsWeb.Areas.Sep.Infrastructure.Repository.DI.Implementation
{
    public class SepFilesRepository : ISepFilesRepository
    {
        ...
        public object CreateFileRec(string save_type, string rec, string templateName, bool modelbuh = false)
        {
            string app_type = "";
            string file_type = "";
            ...
            using (var str = new MemoryStream())
            {
                ...
                return new { tempDir = TempPath, name = "" };
            }

        }
        ///<summary>
        ///формування тіла запиту в залежності від фільтра
        ///</summary>
        private BarsSql InitSepArcDocsSql(string filter)
        {
            BarsSql baseSepDocsSql = new BarsSql()
            {
                SqlParams = new object[]{
                },
                SqlText = string.Format(
                @"select * from V_ARCHIVE_DOCS {0}", filter == "" ? "where rownum <=500 " : "where " + filter + "")
            };
            return baseSepDocsSql;
        }
        ///<summary>
        ///Повертая кількість записів результату запиту
        ///</summary>
        public decimal GetSepArcDocsCount(DataSourceRequest request, string filter)
        {
            var filteredSql = _kendoSqlCounter.TransformSql(InitSepArcDocsSql(filter), request);
            var total = _entities.ExecuteStoreQuery<decimal>(filteredSql.SqlText, filteredSql.SqlParams).Single();
            return total;
        }
        ///<summary>
        ///Повертая записи архіву документів в залежності від фільтру
        ///</summary>
        public ObjectResult<SepArcDocuments> GetSepArcDocs(DataSourceRequest request, string filter)
        {
            var sql = _sqlTransformer.TransformSql(InitSepArcDocsSql(filter), request);
            return _entities.ExecuteStoreQuery<SepArcDocuments>(sql.SqlText, sql.SqlParams);
        }
    }
}