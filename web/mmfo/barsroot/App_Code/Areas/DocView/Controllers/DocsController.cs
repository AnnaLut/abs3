...
using BarsWeb.Areas.DocView.Infrastructure;
using Kendo.Mvc.UI;
using Kendo.Mvc.Extensions;

namespace BarsWeb.Areas.DocView.Controllers
{
    [AuthorizeUser]
    [CheckAccessPage]
    public class DocsController : ApplicationController
    {
        ...
        public DocsController(IDocumentsRepository repository)
        {
            _repository = repository;
        }

        public ActionResult ReasonsHandbook([DataSourceRequest] DataSourceRequest request)
        {
            var list = _repository.ReasonsHandbookData().ToList();
        ...
        {
            if (id == null) id = AccessDocType.User;
            switch (id)
            {
        ...
        public ActionResult Grid(AccessDocType id, string dStart,string dEnd,DirectionDocType dirType)
        {
            ...
            return PartialView(new GridMvc<BarsWeb.Models.Documents>(documents));
        }
        /// <summary>
        ...
        public ActionResult Item(decimal? id)
        {
            ...
            if (docum == null)
            {
                return Content("<span id=\"notFound\" style=\"color:red\">Документ з реф. " + id + " не знайдено!</span>");
            }
            // проверка на бранч (так так p_back_web разрешает только подчиненные документы сторнировать)
            var branch = Convert.ToString(_entities.ExecuteStoreQuery<string>("select sys_context('bars_context','user_branch_mask') from dual").FirstOrDefault()).Replace("%", "");
            if (!docum.BRANCH.StartsWith(branch))
            {
                return Content("<span id=\"notFound\" style=\"color:red\">Документ було створено на вищому рівні " + docum.BRANCH + ", сторнування неможливе !</span>");
            }
            object[] parameters =         
            { 
            ...
            if (!string.IsNullOrEmpty(docum.D_REC))
            {
            ...
            try
            {
                const string sql = @"SELECT 
                                    ...
                                    ORDER by 
                                        Counter, Sqnc";
                visa = _entities.ExecuteStoreQuery<V_VISALIST>
                    (sql, new OracleParameter("pref", OracleDbType.Decimal).Value = docum.REF).ToList();
            }
            catch (Exception e) { var t = e; }
            decimal pLevId=0;
        ...
        public ActionResult Storno(decimal? id)
        {
            ...
            return PartialView("_Storno",res);
        }
        ...
        public ActionResult DocumentDateFilter(int type)
        {
            ViewBag.type = type;
            ViewBag.Title = "Filter Doc Page";
            return View();
        }
    }
}   