...
{ 
    ...
    public class ArchivesController : ApplicationController
    {
        ...
        public ActionResult PutTbsOnline(string tableName)
        {
            var result = new JsonResponse(JsonResponseStatus.Ok, "Процедуру підключення розпочато");
            try
            {
                _repository.PutTbsOnline(tableName);
            }
            catch (Exception e)
            {
                result.status = JsonResponseStatus.Error;
