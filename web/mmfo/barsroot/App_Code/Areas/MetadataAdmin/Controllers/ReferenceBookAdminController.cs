...
namespace BarsWeb.Areas.MetaDataAdmin.Controllers
{
    [AuthorizeUser]
    public class ReferenceBookAdminController : Controller
    {
        ...
        string LoggerPrefix = "ReferenceBookAdminController";
        /// <summary>
        ...
        public ContentResult GetPatternData(string patternName)
        {
            try
            {
                if (string.IsNullOrEmpty(patternName)) throw new ArgumentNullException("data");
                // new GetDataModel();// FormatConverter.JsonToObject<GetDataModel>(data);
                GetDataResultInfo resultInfo = _repository.GetPatternData(patternName);
                var result = new
                {
        ...
        public ActionResult GetRefBookData(RequestMolel requestModel)
        {
            try
            {
                ...
                Logger.Debug(string.Format(" end GetRefBookData for: accessLevel: {0},tableName: {1}, jsonSqlParams: {2},sPar: {3},sParColumn: {4},nativeTabelId: {5}, RowParamsNames{6} ",
                    ...
                    requestModel.SparColumn.HasValue ? requestModel.SparColumn.Value.ToString() : "", requestModel.NativeTabelId.HasValue ? requestModel.NativeTabelId.Value.ToString() : "", requestModel.RowParamsNames), LoggerPrefix + "GetMetaData");
                return View("ReferenceGridAdmin", tableViwModel);
            }
            catch (Exception e)
            {
        ...
        /// <returns></returns>
        [HttpPost]
        [POST("MetadataAdmin/ReferenceBookAdmin/getrepodata")]
        ...
        public JsonResult InsertFilter(string insertUpdatetModel)
        {
            try
            {
                ...
                BarsWeb.Areas.Ndi.Models.InsertFilterModel filterModel = JsonConvert.DeserializeObject<BarsWeb.Areas.Ndi.Models.InsertFilterModel>(req);
                ...
                foreach (BarsWeb.Areas.Ndi.Infrastructure.Repository.Helpers.FilterRowInfo item in filterModel.FilterRows)
                {
        ...
        public JsonResult UpdateFilter(string insertUpdateModel)
        {
            try
            {
               ...
               EditFilterModel editFilterModel = FormatConverter.JsonToObject<EditFilterModel>(req);
                List < BarsWeb.Areas.Ndi.Infrastructure.Repository.Helpers.FilterRowInfo > filterRows = editFilterModel.FilterRows;
                ...
                foreach (BarsWeb.Areas.Ndi.Infrastructure.Repository.Helpers.FilterRowInfo item in filterRows)
                {
                    if (string.IsNullOrEmpty(item.Colname) && string.IsNullOrEmpty(item.LogicalOp) && string.IsNullOrEmpty(item.ReletionalOp) && string.IsNullOrEmpty(item.Value))
                        return Json(new { status = "error", msg = "строка не заповнена" });
                    if (item.Colname == null)
            ...
            string web_form_name = "", string sPar = "", string jsonSqlProcParams = "")
        {
            try
            {
                ...
                if (res.Result == "ok" && res.ContentResult != null && !string.IsNullOrEmpty(res.FileName))
                    return File(res.ContentResult, "text/html", HttpUtility.UrlEncode(res.FileName));
                else
