...
using System.Data;
using barsroot.Areas.MetaDataAdmin.Infrastructure.Helpers;
...
using BarsWeb.Areas.MetaDataAdmin.Models.Params;
using BarsWeb.Areas.MetaDataAdmin.Infrastructure.Constants;
using BarsWeb.Areas.MetaDataAdmin.Models;
using BarsWeb.Areas.MetaDataAdmin;
using Areas.MetadataAdmin.Models;
using BarsWeb.Areas.MetaDataAdmin.Models.DbTabelsInfoModels;

namespace BarsWeb.Areas.MetaDataAdmin.Infrastructure.Repository.DI.Implementation
{
    public class ReferenceBookRepository : IReferenceBookRepository
    {
        private readonly MetaDataAdminModel _entities;
        [Inject]
        public IDbLogger Logger { get; set; }
        private bool IsDebug = true;
        private string LoggerPrefix = "ReferenceBookAdminRepository.";
        ...
        {
            var connectionStr = EntitiesConnection.ConnectionString("MetaDataAdmin", "MetaDataAdmin");
            _entities = new MetaDataAdminModel(connectionStr);
        ...
        public bool EditData(int tableId, string tableName, EditRowModel editDataModel, bool multipleUse = false)
        {
            ...
            try
            {
                ...
                if (!substituationProcedureExecuted)
                {
                    ...
                    OracleCommand sqlUpdateCommand = GetOracleConnector.InstalAndCreateCommand;
             ...
             string funcText = "", string msg = "", string web_form_name = "", string jsonSqlProcParams = "", List<FieldProperties> addParams = null)
        {
            ...
            try
            {
                ...
                if (callFunction.PROC_EXEC == "SYNC_COLUMNS")
                    return CallSyncColumnsFunction(callFunction, funcParams);
                ...
                {
                    SqlStatementParamsParser.AddUploadParameters(callFunction, callFunctionCmd, funcParams, addParams);
        ...
        private string CallSyncColumnsFunction(CallFunctionMetaInfo callFunction, List<FieldProperties> fields)
        {
            try
            {
                if (fields == null)
                    throw new Exception("Немає назви таблиці та tabid");
                FieldProperties tabNameField = fields.FirstOrDefault(x => x.Name == "TABNAME");
                FieldProperties tabidField = fields.FirstOrDefault(x => x.Name == "TABID");
                if(tabNameField == null || tabNameField.Value == null)
                    throw new Exception("Немає назви таблиці та tabid");

                if (tabidField == null || tabidField.Value == null)
                    throw new Exception("Немає назви таблиці та tabname");

                decimal tabid = Decimal.Parse(tabidField.Value);

                OracleCommand command = GetOracleConnector.GetCommandOrCreate;
                command.CommandText = string.Format("select * from {0}", tabNameField.Value);
                OracleDataReader reader = command.ExecuteReader(CommandBehavior.SchemaOnly);
                DataTable schemaTable = reader.GetSchemaTable();

                IEnumerable<UserColComments> columnsWithComments = GetColumnComments(tabNameField.Value);
                List<MetaColumnsDbModel> metaColumns = GetDbNativeColumnsMetaInfo(tabid).ToList();
                GetOracleConnector.DysposeAndClrearCommand();
                command = GetOracleConnector.GetCommandOrCreate;
                AdminOraTyesHelper adminOraHelper = new AdminOraTyesHelper(command);
                List<MetaColumnsDbModel> dbInfoCoumnsBySchema = adminOraHelper.GetColumnsInfo(schemaTable, columnsWithComments, tabid);
                string successMessage = adminOraHelper.ExexuteAddSyncColProc(metaColumns, dbInfoCoumnsBySchema);
                if (callFunction != null && !string.IsNullOrEmpty(callFunction.MSG))
                    successMessage = callFunction.MSG;
        ...
        private string GetColumnsForAddSync(List<FieldProperties> fields)
        {
            try
            {
                if (fields == null)
                ...
                command = GetOracleConnector.GetCommandOrCreate;
                AdminOraTyesHelper adminOraHelper = new AdminOraTyesHelper(command);
                List<MetaColumnsDbModel> dbInfoColumnsBySchema = adminOraHelper.GetColumnsInfo(schemaTable, columnsWithComments, tabid);
                SyncColumnsPatternModel syncPatternModel = new SyncColumnsPatternModel();
                syncPatternModel.DbInfoCoumnsBySchema = dbInfoColumnsBySchema;
                syncPatternModel.MetaColumns = metaColumns;
                SyncColumnsFactory syncFactory = new SyncColumnsFactory();
                BasePatternGridModel result = syncFactory.GetFullGridMdel(syncPatternModel);
                string successMessage = adminOraHelper.ExexuteAddSyncColProc(metaColumns, dbInfoColumnsBySchema);
        ...
        private IList<UserColComments> GetColumnComments(string tableName)
        {
            string sqlText = "SELECT * FROM v_meta_user_col_comments WHERE TABLE_NAME = :tableName";
            OracleParameter oraParam = new OracleParameter("tableName", OracleDbType.Varchar2);
            oraParam.Value = tableName;
            return _entities.ExecuteStoreQuery<UserColComments>(sqlText, oraParam).ToList();
        }
          ...
          string funcText = "", string msg = "", string web_form_name = "", string jsonSqlProcParams = "")
        {
            ...
            try
            {
                ...
                OracleCommand callFunctionCmd = GetOracleConnector.GetCommandWithBeginTransaction; 
                ...
                List<GetParamsInfo> outParameters = null;
                GetParamsInfo resultParam = null;
                GetFileFromTable resultGetFromTableParam = null;
                List<ParamMetaInfo> systemParams = null;
                string clobParamName = null;
                string fileNameParam = null;
                Random random = new Random();
                string postfix = random.Next(1, 1000).ToString();


                if (callFunction != null)
                {
                    if(callFunction.ConvertParams != null && callFunction.ConvertParams.Count() > 0 && funcParams != null && funcParams.Count > 0)
                    {
                        List<ComplexParams> complexParams = SqlStatementParamsParser.GetSqlFuncCallParamsDescription<ComplexParams>(callFunction.PROC_NAME, callFunction.ConvertParams);
                        callFunction.ConvertParamsInfo = complexParams.Where(x => x.Kind == "CONVERT_PARAMS" && x is ConvertParams).Select(c => c as ConvertParams).ToList();

                        callFunction.ConvertParamsInfo.ForEach(x =>
                        {
                            var item = funcParams.FirstOrDefault(c => x.GetFrom == "GRID_COLUMN" &&  c.Name == x.SrcFrom );
                            if (item != null)
                                item.Name = x.ColName;
                        });
                    }
                    if (!string.IsNullOrEmpty(callFunction.OutParams))
                    {
                        outParameters = SqlStatementParamsParser.GetSqlFuncCallParamsDescription<GetParamsInfo>(callFunction.PROC_NAME, callFunction.OutParams).Where(x => x.ColType != null).ToList();
                        GetParamsInfo param = outParameters.FirstOrDefault(x => x.ColType.ToUpper() == "CLOB");
                        if (param == null)
                            return null;
                    ...
                    if (!string.IsNullOrEmpty(callFunction.GetParams))
                    {
                        resultParam = SqlStatementParamsParser.GetSqlFuncCallParamsDescription<GetParamsInfo>(callFunction.PROC_NAME, callFunction.GetParams).FirstOrDefault(x => x.ColType != null);
                        //funcParams.Add(new FieldProperties()
                        //{
                        //    Name = "P_FILENAME",
                        //    Type = "C",
                        //    Value = funcParams.FirstOrDefault(x => x.Name == "TABNAME").Value + postfix
                        //});
                        ...
                        if (clobParam != null && clobParam.Value != null)
                        {
                            ...
                            if (clob != null)
                            {
                                ...
                                string RES = windows.GetString(asciiBytes);
                                result = new GetFileResult() { FileBody = RES, FileName = !string.IsNullOrEmpty(name) ? name : "file.txt", Result = "ok" };
                            }
                        }
                        if (resultParam != null && resultParam is GetFileFromTable)
                        {
                            if (callFunctionCmd.Parameters.Count > 0)
                                callFunctionCmd.Parameters.Clear();
                            ...
                            resultGetFromTableParam = resultParam as GetFileFromTable;
                            FieldProperties fileNameProp = funcParams.FirstOrDefault(x => resultGetFromTableParam.SrcFileName == x.Name);
                            string fileName = fileNameProp != null ? fileNameProp.Value : "scriptFile";
                            string sql = string.Format("SELECT {0} FROM {1} WHERE FILE_NAME = '{2}'", resultGetFromTableParam.ColNameGetFrom,
                                resultGetFromTableParam.TableNameFrom, fileName);
                            callFunctionCmd.CommandText = sql;
                            OracleDataReader reader = callFunctionCmd.ExecuteReader();
                            List<Dictionary<string, object>> list = AllRecordReader.ReadAll(reader).ToList();
                            res = list.First().FirstOrDefault(x => x.Key == resultGetFromTableParam.ColNameGetFrom).Value.ToString();
                            unicodeBytes = unicode.GetBytes(res);
                            asciiBytes = Encoding.Convert(unicode, windows, unicodeBytes);
                            result = new GetFileResult() { ContentResult = asciiBytes, FileName = fileName + ".SQL", Result = "ok" };
                            //string clobFromTable = allData
                        }
                        GetOracleConnector.DisposeWithEndTransaction(true);
                    }
                }
                return result;
            }
            ...
            {
                GetOracleConnector.DisposeWithEndTransaction(false);
                throw e;
            }
            finally
            {
        ...
        public GetDataResultInfo GetPatternData(string patternName)
        {
            return null;
        }
        /// <summary>
        ...
        public GetDataResultInfo GetData(DataModel dataModel)
        {
            ...
            string funNsiEditFParamsString = GetFunNSIEditFParamsString(null, dataModel.CodeOper, dataModel.SParColumn, dataModel.NativeTabelId, dataModel.NsiTableId, dataModel.NsiFuncId);
            ...
            try
            {
                ...
                {
                    CallFunctionBeforeSelectTable(connection, startInfo.AllFieldProperties, startInfo.ProcedureText);
                }
            ...
            catch (Exception ex)
            {
                ...
                if (msg.Contains("SPECIFIED CAST IS NOT VALID"))
                {
                    ...
                    Logger.Error("ReferenceBookAdminRepository. GetData. " + msg);
        ...
        public List<MetaColumnsDbModel> GetDbNativeColumnsMetaInfo(decimal tableId)
        {
        ...
        public CallFunctionMetaInfo CreateCallFunctionCommand(int? tableId, int? funcId, int? codeOper, int? columnId, string funcText = "")
        {
            ...
            OracleCommand callFunctionCmd = GetOracleConnector.InstalAndCreateCommand;
            callFunctionCmd.BindByName = true;
            //строка вызова sql-процедуры находится задана в PROC_NAME
            string procText = !string.IsNullOrEmpty(funcText) ? funcText : callFunction.PROC_NAME;
            if (string.IsNullOrEmpty(procText))
        ...
        /// <returns></returns>
        private CallFunctionMetaInfo GetCallFunction(int tableId, int funcid)
        {
            try
            {
        ...
        private bool TryExecuteSubstituationProcedure(int tableId, SqlOperation operation, List<FieldProperties> fields, List<FieldProperties> updatableRowData = null, string procedureString = "", bool isMultipleProcedure = false)
        {
            ...
            try
            {
                ...
                if (isMultipleProcedure)
                    sqlCommand = this.GetOracleConnector.GetCommandWithBeginTransaction;
                else
        ...
        public CallFunctionMetaInfo GetFunctionsMetaInfo(int? codeOper)
        {
        ...
        public string InsertFilter(List<BarsWeb.Areas.Ndi.Infrastructure.Repository.Helpers.FilterRowInfo> filterRows, string filterStructure, int tableid, string filterName, int saveFilter = 1, string whereClause = null)
        {
            try
            {
                ...
                string clause = BarsWeb.Areas.Ndi.Infrastructure.Repository.Helpers.GetFilterDbInfo.PushRowsFilterList(filterRows, filterStructure, tableid, filterName, saveFilter, whereClause);
