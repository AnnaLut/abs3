...
namespace BarsWeb.Areas.Cash.Infrastructure.Sync
{
    ...
    {
        public AccountSynchronizer(ICenterAccountRepository centerAccountRepository, ISyncRepository syncRepository)
            : base(centerAccountRepository, syncRepository) { }
        ...
        {
            connectionOption.Url = CombineUrl(connectionOption.Url, GetAccountActionMethod + "?mfoCode=" + connectionOption.Mfo);

            // если ранее выполнялась вычитка по данному mfo
            var hasAccounts = CenterAccountRepository.HasAccounts(connectionOption.Mfo);
            if (hasAccounts)
            {
            ...
            */
            ...
            try
            {
                ...
                foreach (var cashAccount in accountsResponse.Accounts)
                {
                    ...
                    CenterAccountRepository.AddAccountData(cashAccount);
