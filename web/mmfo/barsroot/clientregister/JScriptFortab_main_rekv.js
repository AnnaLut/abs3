...
function validOkpoDigitUO(strOKPO) {
    ...
    else if ($get('ddl_TGR').value == '3') // COBUMMFO-8414 -> 9 цифр при виборі Тимчасового реєстру без перевірки КР
    {
        if (strOKPO.length < 9) {
            alert('Реєстраційний (обліковий) номер платника податків з Тимчасового реєстру ДПАУ повинен складатися з 9 цифр. Введіть номер ТРДПАУ повторно.');
            return false;
        }
        else
            return true;
