var TranslateDict = {
	dictCode: [189, 32, 192, 81, 87, 69, 82, 84, 89, 85, 73, 79, 80, 219, 221, 65, 83, 68, 70, 71, 72, 74, 75, 76, 186, 222, 90, 88, 67, 86, 66, 78, 77, 188, 190, 191, -85],
...
var charStack = {
	...
	validateChar: function (item) {
		if (this.stack.length > 0) {
			if (this.stack[this.stack.length - 1] === this.comandChars["rAlt"]) {
				this.setEmpty();
				if (item === 85) {
					return -item;
				} else {
			...
			} if (this.stack[this.stack.length - 1] === this.comandChars["space"]) {
				if (item === this.comandChars["space"]) {
					return 0;
				} else {
					if (jQuery.inArray(item, TranslateDict.dictCode))
						this.setEmpty();
					return item;
				}
			...
			if (item === this.comandChars["rAlt"] || item === this.comandChars["space"])
...
function translateLetters(e) {
	...
	if (e.target.id == "ed_FIO_LN" && k == charStack.comandChars["space"])
		return;
