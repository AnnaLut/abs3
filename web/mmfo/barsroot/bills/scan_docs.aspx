<%@ Page Language="C#" AutoEventWireup="true" CodeFile="scan_docs.aspx.cs"
...
<head runat="server">
    ...
    <script language="javascript" type="text/jscript">
        ...
        function Initialisation(sid, hasValue, pageCount, imageHeight, imageWidth) {
            ...
            //if (window.name == 'DialogClientCard' || window.dialogArguments["windowName"] == 'DialogClientCard') {
                $('#csxi').css({
                    'width': '480px',
                    'height': '640px'
                });
            ...
            SessionID = sid;
            UploadUrl = location.protocol + '//' + location.host + '/barsroot/bills/textboxscanner_upload.aspx?sid=' + sid + '&rnd=' + Math.random();
            //UploadUrl = location.protocol + '//' + location.host + '/barsroot/bills/textboxscanner_upload.aspx?sid=' + sid + '&rnd=' + Math.random();
            //UploadUrl = location.protocol + '//' + location.host + '/barsroot/bills/textboxscanner_upload.aspx?sid=' + sid + '&rnd=' + Math.random();
            DownloadUrl = location.protocol + '//' + location.host + '/barsroot/bills/byteimage_tiffile.ashx?sid=' + sid + '&rnd=' + Math.random();
        ...
        function SaveClick() {
            ...
            if (!csxi.ImageLoaded && imageCount == 0) {
                ...
                //debugger;
                //var imgData = csxi.WriteBinary(2);
                ...
                //if (window.name == 'DialogBills' || window.dialogArguments["windowName"] == 'DialogBills') {
                var success = csxi.PostImage(UploadUrl, 'scanBill.pdf', '', UploadFormat); //передаем файл формата PDF на сервер               
                if (success == false) {
                    alert('Розмір фотографії перевищує 150 кб');
                    csxi.DeleteFile("scanBill.pdf", true);
                }
                else {
                    CloseDialog({ SaveBtnEnabled: true, Status: true, SelectedDevice: GetSelectedDevice(), sessionID: SessionID });
                }
                    //}
                    //else {
...
<body style="font-family: tahoma, arial, sans-serif; font-size: 9pt" onbeforeunload="OnBeforeUnload()" >
