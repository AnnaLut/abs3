...
function Sel_Usr_Ckb(ids) {
    ...
    for (var i = 0; i < idArr.length; i++) {
        ...
        if ('*' === id) {
            for (var i = 0; i < usrObj.selectedRowsIds.length; i++) {
                GetCheckbox(usrObj.selectedRowsIds[i]).checked = false;
            }
            usrObj.selectedRowsIds = [];
            ...
            if (firstWindowLoad) {
                checkBox.checked = true;
                usrObj.selectedRowsIds.push(id);
            } else {
                var pos = GetElemIndex(usrObj.selectedRowsIds, id);
                var amount = pos < 0 ? 0 : 1;
                usrObj.selectedRowsIds.splice(pos, amount);
            }
        } else if (checkBox.checked) {
            GetCheckbox('*').checked = false;
            usrObj.selectedRowsIds.push(id);
        } else {
            alert("Еще Необработанное программистом исключение.");
        }
    }
}
...
function Apply() {
    ...
    if (usrObj.selectedRowId != "*") result["USER"] = usrObj.selectedRowsIds.join(',');
...
function GetCheckbox(id) {
    return document.getElementById('ckb_' + id).children[0];
}
function GetElemIndex(arr, elem) {
    var pos = -1;
