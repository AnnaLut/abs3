...
function InitDefault() {
    ...
    var restriction = getParamFromUrl("restriction", location.href);
    if (restriction == "1")
        v_data[21] = "restriction";
    LoadDefault();
    ...
    if (!window.findls) {
        if (getCookie('rnk'))
            document.getElementById('tbFindRNK').value = getCookie('rnk');
        if (getCookie('okpo'))
            document.getElementById("tbFindOKPO").value = getCookie('okpo');
        if (getCookie('nmk'))
            document.getElementById("tbFindNMK").value = getCookie('nmk');
        if (getCookie('k050'))
            document.getElementById("tbFindK050").value = getCookie('k050');
...
function builtFastFilter() {
    var where = "";
    
    var rnk = document.getElementById("tbFindRNK").value;
    if (rnk)
        where += " AND a.rnk like '" + rnk + "%'";

    var okpo = document.getElementById("tbFindOKPO").value;
    if (okpo)
        where += " AND a.okpo like '" + okpo + "%'";

    var nmk = document.getElementById("tbFindNMK").value;
    if (nmk)
        where += " AND a.nmk like '" + nmk + "%'";

    var k050 = document.getElementById("tbFindK050").value;
    if (k050)
        where += " AND a.k050 like '" + k050 + "%'";

    //save to cookie
    document.cookie = "rnk=" + rnk;
    document.cookie = "okpo=" + okpo;
    document.cookie = "nmk=" + nmk;
    document.cookie = "k050=" + k050;
...
function fnExportToExcel() {
    webService.Cust.callService(onExportExcel, "ShowCustomersAndAccountsExcelExport", v_data, forceExecute);
}
