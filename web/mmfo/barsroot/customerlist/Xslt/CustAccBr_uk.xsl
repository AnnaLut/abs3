...
<TD>
	<xsl:attribute name="onclick"><xsl:text>Sort('DAPP')</xsl:text></xsl:attribute>
...
<TD>
  <xsl:if test="$blk>0"><xsl:attribute name="style">color:white;background-color:purple</xsl:attribute></xsl:if>
