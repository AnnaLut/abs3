...
public partial class DepositBFRowCorrection : Bars.BarsPage
{
	...
	private void FillControls(Decimal info_id)
	{
		...
		try
		{
			...
			cmdSearch.CommandText = "SELECT NLS, " +
				"branch_code,dpt_code,SUM/100,fio,id_code, payoff_date, REF,incorrect,closed,excluded,branch,agency_name,acc_type " +
			...
			if (!rdr.IsDBNull(5))
				textIdCode.Text = Convert.ToString(rdr.GetOracleString(5).Value);
            if (!rdr.IsDBNull(6))
                PayoffDate.Date = rdr.GetOracleDate(6).Value;
            if (!rdr.IsDBNull(7))
                textRef.Text = Convert.ToString(rdr.GetOracleDecimal(7).Value);
			if (!rdr.IsDBNull(8))
			{
			...
			if (!rdr.IsDBNull(10))
			{
				Decimal dummy = Convert.ToDecimal(Convert.ToString(rdr.GetOracleDecimal(10).Value));
				ckExcluded.Checked = (dummy == 1) ?  true : false;
			}

            String tmp_branch = String.Empty;
            if (!rdr.IsDBNull(11))
                tmp_branch = Convert.ToString(rdr.GetOracleString(11).Value);

            if (!rdr.IsDBNull(12))
                textAgency.Text = Convert.ToString(rdr.GetOracleString(12).Value);

            String tmp_acctype = String.Empty;
            if (!rdr.IsDBNull(13))
                tmp_acctype = Convert.ToString(rdr.GetOracleString(13).Value);
            ...
            OracleCommand cmdFillAccTypes = connect.CreateCommand();
            cmdFillAccTypes.CommandText = @"select type, name from v_file_account_types";

            ad.SelectCommand = cmdFillAccTypes;
            ad.Fill(dsAccTypes);
	...
	override protected void OnInit(EventArgs e)
	{
        ...
        dsPretenders.ConnectionString = Bars.Classes.OraConnector.Handler.IOraConnection.GetUserConnectionString();
        dsPretenders.PreliminaryStatement = Bars.Classes.OraConnector.Handler.IOraConnection.GetSetRoleCommand("DPT_ROLE");
        
        dsPretenders.SelectParameters.Add("info_id", TypeCode.Decimal, 
            Convert.ToString(Request["info_id"]));
	}
	...
	private void btAccept_Click(object sender, System.EventArgs e)
	{
        ...
        try
		{
            ...
            else
            {
                ...
                cmdCheckNLS.CommandText = @"select bars.dpt_social.check_account(:nls,:branch,:p_id_code,:p_nmk,:acc_type) from dual";
                ...
                cmdCheckNLS.Parameters.Add("p_id_code", OracleDbType.Varchar2, textIdCode.Text, ParameterDirection.Input);
                cmdCheckNLS.Parameters.Add("p_nmk", OracleDbType.Varchar2, textFIO.Text, ParameterDirection.Input);
                cmdCheckNLS.Parameters.Add("acc_type", OracleDbType.Char, ddAccType.SelectedValue, ParameterDirection.Input);

                var error_text = String.Empty;
                using (var rdr = cmdCheckNLS.ExecuteReader())
                {
                    if (!rdr.Read())
                    {
                        throw new DepositException("Запис в базі про стрічку банківського файлу з номером "
                    ...
                    if (!rdr.IsDBNull(0))
                        error_text = Convert.ToString(rdr.GetOracleString(0).Value);
                }

                if (!String.IsNullOrEmpty(error_text) && !ckExcluded.Checked)
                {
                    message = error_text;//"Введенный счет не существует либо закрыт!\\nВведите корректные данные.";
			...
			cmdUpdate.Parameters.Add("id_code",		OracleDbType.Varchar2,	textIdCode.Text,						ParameterDirection.Input);
            cmdUpdate.Parameters.Add("payoff_date", OracleDbType.Date,      PayoffDate.Date,                        ParameterDirection.Input);
            ...
            cmdUpdate.Parameters.Add("acc_type",    OracleDbType.Varchar2,  ddAccType.SelectedValue,                ParameterDirection.Input);            
