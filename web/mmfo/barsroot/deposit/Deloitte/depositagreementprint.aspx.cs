...
public partial class DepositAgreementPrint : Bars.BarsPage
{
    ...
    /// <summary>
    /// Періодичність випалти відсотків
    /// </summary>
    private Decimal? Freq
    {
        get
        {
            return Request["freq"] == null ? (Decimal?)null : Convert.ToDecimal(Request.QueryString["freq"]);
        }
    }
    #endregion
    ...
    private void Page_Load(object sender, System.EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            {
                _dbLogger.Info("OnChildren", "deposit");
                CreateAgreement();
    ...
    protected void btCreate_Click(object sender, EventArgs e)
    {
        ...
        _dbLogger.Info(String.Format("Текст дод.угоди № {0} по депозитному договору №{1} успішно зформовано та записано в БД.", agr_num, dpt.ID), "deposit");
    }
    ...
    protected void EADocPrint_BeforePrint(object sender, EventArgs e)
    {
        ...
        if ((agr_id == 7) && (!agr_uid.HasValue))
        {
            ...
            if (CreateAgreement())
            {
                _dbLogger.Info("Користувач завершив \"Вступ в право власності\" клієнтом з РНК=" + rnk_tr.ToString(), "deposit");
            }
        }
	    ...
	    case 35:
            ...
            case 37:
                EADocPrint.EAStructID = 213;
                break;
            //Заява про відмову від лонгації
            case 17:
                EADocPrint.EAStructID = 2110;
                break;
    ...
    {
        _dbLogger.Info(String.Format("Користувач поставив відмітку про наявність підпису клієнта (rnk={0}) на дод.угоді №{1} до депозитного договору #{2}", rnk_tr, agr_num, dpt_id), "deposit");
        String url;
        if (Request.QueryString["inherit_irr_id"] != null)
        { url = String.Format("/barsroot/clientproducts/DptClientPortfolioContracts.aspx?cust_id={0}", Request.QueryString["inherit_irr_id"]); }
        else { url = String.Format("/barsroot/deposit/deloitte/DepositContractInfo.aspx?dpt_id={0}&scheme=DELOITTE&rnk_tr={1}", dpt_id, rnk_tr); }
    ...
    protected void btNextAgr_Click(object sender, EventArgs e)
    {
        ...
        if (Request.QueryString["inherit_irr_id"] != null)
        {
            url = String.Format("/barsroot/clientproducts/DptClientPortfolioContracts.aspx?cust_id={0}&scheme=DELOITTE", Request.QueryString["inherit_irr_id"]);
        }
        else
        {
            url = String.Format("/barsroot/deposit/deloitte/DepositContractInfo.aspx?dpt_id={0}&scheme=DELOITTE", dpt_id);
        }
        Response.Redirect(url);
    }
