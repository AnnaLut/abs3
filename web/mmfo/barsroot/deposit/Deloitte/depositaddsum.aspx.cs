...
public partial class DepositAddSum : Bars.BarsPage
{
    ...
    private void Page_Load(object sender, System.EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            _dbLogger.Info("dpt.type= " + dpt.Type);
           ...
           dpt_id.Value = dpt.ID.ToString();
            ...
            {
                _dbLogger.Info("Депозит #" + dpt_id.Value.ToString() + " не передбачає поповнення!", "deposit");
    ...
    private void FillControlsFromClass(Deposit dpt)
    {
        ...
        try
        {
            ...
            if (!rdr.Read())
                throw new DepositException("Не найдена информация о депозите №" + dpt.ID + " тип ="+dpt.Type);
    ...
    private bool AddPaymentEnabled()
    {
        ...
        try
        {
            ...
            _dbLogger.Debug("Вид депозитного договора № " + dpt_id.Value + " допускает операцию пополнения.","deposit");
    ...
    public void CheckStopFunction(Decimal SUM, Decimal Kv, String Nls, String dpt_id)
    {
        OracleConnection connect = new OracleConnection();
        try
        {
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        if (textContractAddSum.ValueDecimal >= textMinAddSum.ValueDecimal)
        {
         //   DBLogger.Info("Сума поповнення депозиту #" + textMinAddSum.ValueDecimal.ToString() + "", "deposit");
            ...
            {
                _dbLogger.Info("Сума поповнення депозиту #" + dpt_id.Value + " підпадає під контроль фінмоніторингу.", "deposit");
    ...
    private void FirstPayment()
    {
        ...
        _dbLogger.Info("поповнення депозиту #" + TT + " ;" + dpt.Currency.ToString() + "; " + dpt.dpt_nls + " ;" + dpt.Client.ID.ToString() , "deposit");
    ...
    {
        _dbLogger.Info("Користувач встановив відмітку про отримання від клієнта підписаного опитувального листа фінмоніторингу (депозит #" + 
