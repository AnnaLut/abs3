...
public partial class DepositAccount : Bars.BarsPage
{
    ...
    private void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            _dbLogger.Info("Пользователь зашёл на страницу задания счетов при заведении депозитного договора без данных о депозите и был перенаправлен на страницу карточки клиента",
                "deposit");
        ...
        else
        {
            ...
            if (Request.QueryString["scheme"] == "DELOITTE")
            {
                ...
                if ((dpt.SetDefaultReceiver() == 1) && (Session["OnTrustee"] == null) && (Session["OnOwner"] == null) && (Session["OnBeneficiary"] == null) && (Session["OnChildren"] == null))//TODO: disable this check, if deposit is for third person
                    // Якщо рахунки виплати по договору заповненні автоматично і вклад не на третіх осіб
                    btNext_ServerClick(sender, e);
                else
                {
    ...
    {
        _dbLogger.Info("Пользователь подтвердил информацию о депозитном договоре и нажал кнопку \"Формировать договор\"",
            "deposit");
        ...
        if ((Session["OnTrustee"] != null) && (Convert.ToString(Session["OnTrustee"]) == "YES"))
        {
            decimal commisrequest = Tools.CreateCommisRequest(dpt.ID, 12);

            decimal dpt_agreement =
          DepositAgreement.Create(dpt.ID, 12, dpt.Client.ID, Convert.ToDecimal(Request["rnk_tr"]), null, null, null, dpt.BeginDate, dpt.EndDate, null, commisrequest, 11111111, dpt.Sum_cent);
            Decimal? WarrantID = ep.DOC_CREATE("SCAN", null, scWarrant.Value, 222, Convert.ToDecimal(Request["rnk_tr"]), dpt.ID);
        }

        //для депозиту, оформленого на ім'я, створюємо довіреність на особу, що оформлювала депозит + 26 ДУ щоб в архіві щось там могли відрізнити(хз що, просили додати)
        if ((Session["OnOwner"] != null) && (Convert.ToString(Session["OnOwner"]) == "YES"))
        {
            decimal commisrequest = Tools.CreateCommisRequest(dpt.ID, 12);
            decimal dpt_agreement =
          DepositAgreement.Create(dpt.ID, 12, dpt.Client.ID, Convert.ToDecimal(Request["rnk_tr"]), null, null, null, dpt.BeginDate, dpt.EndDate, null, commisrequest, 11111111, dpt.Sum_cent);
            dpt_agreement = DepositAgreement.Create(dpt.ID, 26, dpt.Client.ID, Convert.ToDecimal(Request["rnk_tr"]), null, null, null, dpt.BeginDate, dpt.EndDate, null, null, null, null);
        ...
        if (Deposit.GetProductCode(dpt.Type) == 10)
        {
                ...
                ((dpt.Client.Sex == 2) && (DateTime.Now.Date < dpt.Client.BirthDate.AddYears(55))))
            {
                ...
                _dbLogger.Info("Користувач відкрив депозит \"Строковий пенсійний\" клієнту (РНК=" + Convert.ToString(dpt.Client.ID) +
    ...
    {
        _dbLogger.Info("Пользователь нажал кнопку \"назад\" на странице задания счетов при заведении депозитного договора. Номер договора "
