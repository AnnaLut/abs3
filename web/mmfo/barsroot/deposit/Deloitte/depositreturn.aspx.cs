...
public partial class DepositReturn : Bars.BarsPage
{
	...
	private void Page_Load(object sender, EventArgs e)
	{
        ...
        if ((Tools.get_EADocID(dpt.ID) > 0) && (DepositRequest.HasActive(dpt.Client.ID, dpt.ID) == false))
        {
            ...
            _dbLogger.Info("Користувач має авторизуватись по картці бо не має активного запиту на бек-офіс.",
                      "deposit");
            _dbLogger.Info("request=" + DepositRequest.HasActive(dpt.Client.ID, dpt.ID) + "dpt.Client.ID=" + dpt.Client.ID + " rnk_inherit=" + Request["inherit_id"] + " dpt_id=" + dpt.ID,
                      "deposit");
            _dbLogger.Info("eadoc=" + Tools.get_EADocID(dpt.ID),
                        "deposit");
        }
        else
        {
            ...
            _dbLogger.Info("Користувач авторизований по картці або має активний запит на бек-офіс.",
          "deposit");
	...
	private void FillControlsFromClass(Deposit dpt)
	{
        ...
        else
        {
            ...
            if (Request["inherit_id"] != null)
            {
                ...
                //якщо на спадкоємця є активний запит то виплата без картки, інакше авторизація по картці.
                if ((Tools.get_EADocID(dpt.ID) > 0) && (DepositRequest.HasActive(Convert.ToDecimal(rnk.Value), dpt.ID) == true))
                {
                    btNalPay.Visible = true;
                    btAuthorizationByBPK.Visible = false;
                    _dbLogger.Info("rnk.Value=" + Convert.ToDecimal(rnk.Value),
                      "deposit");
                }
            ...
            if (Request["rnk_tr"] != null)
            {
                ...
                if ((Tools.get_EADocID(dpt.ID) > 0) && (DepositRequest.HasActive(Convert.ToDecimal(Request["rnk_tr"]), dpt.ID) == true))
                {
                    btNalPay.Visible = true;
                    btAuthorizationByBPK.Visible = false;
	...
	private void CheckDeposit(Deposit dpt)
	{
		...
		_dbLogger.Debug("Проверка депозита на срок завершения на странице возврата депозита по завершению прошла успешно. Номер договора " + 
	...
	private void btNalPay_ServerClick(object sender, System.EventArgs e)
	{
		...
		_dbLogger.Info("Пользователь нажал на кнопку \"Выплата вклада\" на странице возврата депозита по завершению.  Номер договора " + Convert.ToString(Request["dpt_id"]),
			"deposit");
    ...
    protected void btAuthorizationByBPK_ClientIdentified(object sender, Bars.UserControls.ClientIdentifiedEventArgs e)
    {
        ...
        {
            _dbLogger.Info("Користувач виконав Авторизацію виплати депозиту за допомогою БПК клієнта (rnk=" + 
            ...
            /*String url = "/barsroot/deposit/deloitte/DepositContractInfo.aspx?dpt_id=" + Request.QueryString["dpt_id"] +
                 "&scheme=DELOITTE";*/
            String url = "/barsroot/deposit/deloitte/DepositAddSumComplete.aspx?dest=return&dpt_id=" + Request.QueryString["dpt_id"] +
	...
	private void btPayPercent_ServerClick(object sender, System.EventArgs e)
	{
		...
		_dbLogger.Info("Пользователь нажал на кнопку \"Выплата процентов\" на странице возврата депозита по завершению.  Номер договора " + Convert.ToString(Request["dpt_id"]),
			"deposit");
