...
public partial class DepositClient : Bars.BarsPage
{
    ...
    private void Page_Load(object sender, System.EventArgs e)
    {
        ...
        else
        {
            ...
            {
                _dbLogger.Info("Пользователь получил информацию из страницы SearchResults. agr_id = " + Convert.ToString(Request["agr_id"]),
                    "deposit");
            ...
            //  btSubmit.Visible = true; // якщо прийшли зі сторінки пошуку - лишеємо Реєструвати(переіменовуємо в редагувати), лишаємо Далі + пошук
            btnClear.Visible = false;
          //  btUpdate.Visible = true;
        }
        DisableEdit();
...
//        Client client = ((Deposit)Session["DepositInfo"]).Client;
    ...
    {
         if ( ((Deposit)Session["DepositInfo"]).Client.ID < 0)
            Response.Redirect("/barsroot/clients/customers/index/?custtype=person");

        // Читаємо параметры клиента
        ...
        try
        {
            ...
            if (clientChanged())
            {
                _dbLogger.Info("Внимание! Пользователь пытается зарегистрировать (обновить) данные о клиенте," +
            ...
            if (Tools.Get_CellPhoneVerState(((Deposit)Session["DepositInfo"]).Client.CellPhone))
            {
                ...
                _dbLogger.Info("rnklist = " + rnklist);
                if (rnklist == string.Empty || rnklist == "")
                {
                    //в базу не пишемо, всі зміни в картці клієнта
            ...
            _dbLogger.Info("Пользователь хотел обновить данные о клиенте №" + client.ID.ToString() + " на странице карточки вкладчика(старый деп модуль)",
                "deposit");
            ...
            {
                if (rnklist == "" || rnklist == string.Empty)
                {
    ...
    /// <summary>
    /// ф-ція заборони редагування параметрів. редагування можливе лише в картці клієнта
    /// </summary>
    ...
    {
        _dbLogger.Info("Пользователь подтвердил данные о клиенте на странице карточки вкладчика заведения депозитного договора и перешёл на следующий этап" +
            ". ДРФО клиента = " + ((Deposit)Session["DepositInfo"]).Client.Code,
            "deposit");
        FillClassFromControls();
        _dbLogger.Info("client.CellPhone = " + (((Deposit)Session["DepositInfo"]).Client.CellPhone) + " =>> " + Convert.ToString(Tools.Get_CellPhoneVerState(((Deposit)Session["DepositInfo"]).Client.CellPhone)));
        Client client = ((Deposit)Session["DepositInfo"]).Client;
        ...
        if (Tools.Get_CellPhoneVerState(((Deposit)Session["DepositInfo"]).Client.CellPhone))
        {
            ...
            {
                rnklist = "З вказаним номером телефона вже існують клієнти в відділеннях: " + rnklist;
                cmd = "<script>confirm(\"" +  rnklist + "\");</script>";
            }
            _dbLogger.Info("cmd = " + cmd);
            Response.Write(cmd);
            Response.Flush();
        }
        else
    ...
    private void disableValidators()
    {
        ...
        validatorClientLastName.Enabled = false;
        validatorClientFirstName.Enabled = false;
