...
function fnCloseAcc() {
    if (selectedRowId == null) return;
        ...
        var message = LocalizedString('Message6') + "(acc=" + selectedRowId + ")?";
        ...
        if (result == 1) {
            ...
            webService.Tmp.callService(onCloseAcc, "CloseAccount", selectedRowId, 0);
        }
    }
}
...
function fnShowHist() {
    if (selectedRowId == null) return;
    else window.location.replace("custhistory.aspx?mode=1&rnk=" + selectedRowId + "&type=" + type);
}
//История счета
function fnShowHistAcc() {
    if (selectedRowId == null) return;
    else window.location.replace("showhistory.aspx?acc=" + selectedRowId + "&type=" + type);
}
//Локализация
