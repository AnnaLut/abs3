...
function fnCredits() {
    ...
    location.replace("/barsroot/credit/default.aspx?rnk=" + selectedRowId);
}
...
function fnRedirAcc() {
    if (selectedRowId == null) return;
        ...
        if (accessmode == "0")
            window.location.replace("custacc.aspx?type=0&rnk=" + selectedRowId + "&mod=ro");
        else
            window.location.replace("CustAcc.aspx?type=0&rnk=" + selectedRowId);
    }
}
//---------------------------------------------------------------------
...
function fnRedirPer() {
    if (selectedRowId == null) return;
    if (accessmode == "0") {
        if (document.getElementById("r_" + row_id).BR_OWN == 1)
            window.location.replace("/barsroot/clientregister/registration.aspx?readonly=2&rnk=" + selectedRowId);
        else
            window.location.replace("/barsroot/clientregister/registration.aspx?readonly=3&rnk=" + selectedRowId);
    }
    else {
        if (document.getElementById("r_" + row_id).DAT_OFF != "") {
            window.location.replace("/barsroot/clientregister/registration.aspx?readonly=1&rnk=" + selectedRowId);
        }
        else
            window.location.replace("/barsroot/clientregister/registration.aspx?readonly=0&rnk=" + selectedRowId);
    }
}
//---------------------------------------------------------------------
function fnShowHist() {
    if (selectedRowId == null) return;
    else window.location.replace("CustHistory.aspx?mode=2&rnk=" + selectedRowId + "&type=0");
}
//---------------------------------------------------------------------
//Нажатие
function fnAll() {
