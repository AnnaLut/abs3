...
function InitDptDealParams() {
    ...
    {
        if (type == 0) PopulateDeal();
        else if (type == 1) {
            if (dpu_gen) OpenDealAdd();
            else OpenDeal();
        }
    }
    else {
...
function getFilteredVidd(ddlist) {
    var tail = "flag=1";
    var val = "";
    if ((val = document.getElementById("ddProduct").value) != "")
        tail += " and type_id=" + val;
...
function onGetDepositDealParams(result) {
    ...
    if (nAdd == "0" && nGen == "") HideImg(d.btSos);
    else HideImg(d.btChilds);
    if (nGen != "") SetRO(d.tbND);
    if (mode == 2) {
        HideImg(d.btSave);
        HideImg(d.btPassport);
        HideImg(d.btAccounts);
...
function fnSave() {
    if (!CheckData()) return;
    if (Dialog("Зберегти нові параметри договору!", "confirm") != 1) return;
    var d = document.all;
    var data = new Array();
    data[0] = d.tbND.value;
...
//**********************************************************************//
function fnDopSogl() {
    window.showModalDialog("dptdealparams.aspx?mode=" + mode + "&dpu_id=0&type=1&dpu_gen=" + dpu_id + "&dpu_ad=1", window.dialogArguments, "dialogWidth:790px;dialogHeight:490px;center:yes;edge:sunken;help:no;status:no;");
}
//**********************************************************************//
function fnClose() {
    window.close();
...
function fnClient() {
    var result = window.showModalDialog(url_dlg_mod + 'customer&tail=""', window, "dialogHeight:600px;dialogWidth:600px;center:yes;edge:sunken;help:no;status:no;");
    if (result != null) {
        rnk = result[0];
...
function onGetVal(result) {
    ...
    window["tbDatV"].SetValue(result.value[18].text);
