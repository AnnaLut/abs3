...
var ttProc;
var acc;
var kv, sum, mfo_d, nls_d, nam_d, nb_d, okpo, mfo_p, nls_p, nam_p;
...
//**********************************************************************//
function fnRazm() {
    var url = "/barsroot/docinput/docinput.aspx?tt=" + tt1 + "&Nls_B=" + gE("tbNls").value + "&Kv_B=" + kv + "&nd=" + dpu_id + "&SumC=" + sum;
    window.showModalDialog(encodeURI(url), null, "dialogWidth:680px; dialogHeight:600px; scroll:no; center:yes; status:no");
    fnRefresh();
}
//**********************************************************************//
function fnPopol() {
    var url = "/barsroot/docinput/docinput.aspx?tt=" + tt2 + "&Nls_B=" + gE("tbNls").value + "&Kv_B=" + kv + "&nd=" + dpu_id;
    window.showModalDialog(encodeURI(url), null, "dialogWidth:680px; dialogHeight:600px; scroll:no; center:yes; status:no");
    fnRefresh();
}
//**********************************************************************//
...
function onMakeInt(result) {
    ...
    if (!getError(result)) return;
    if (result.value == 1)
        Dialog("Виникла помилка при нарахуванні відсотків", "alert");
    else {
...
function fnViplat() {
    ...
    var url = "/barsroot/docinput/docinput.aspx?tt=" + ttProc + "&Nls_A=" + gE("tbNlsN").value + "&Kv_A=" + kv + "&nd=" + dpu_id + "&Mfo_B=" + mfo_p + "&SumC=" + s + "&Nls_B=" + nls_p + "&Nam_B=" + nam_p + "&Id_B=" + okpo;
    window.showModalDialog(encodeURI(url), null, "dialogWidth:680px; dialogHeight:600px;scroll:no; center:yes; status:no");
    fnRefresh();
}
//**********************************************************************//
function fnPogash() {
    ...
    var url = "/barsroot/docinput/docinput.aspx?tt=" + tt3 + "&Nls_A=" + gE("tbNls").value + "&Kv_A=" + kv + "&nd=" + dpu_id + "&Mfo_B=" + mfo_d + "&SumC=" + s + "&Nls_B=" + nls_d + "&Nam_B=" + nam_d + "&Id_B=" + okpo;
    window.showModalDialog(encodeURI(url), null, "dialogWidth:680px; dialogHeight:600px;scroll:no; center:yes; status:no");
    fnRefresh();
}
//**********************************************************************//
