...
namespace DocInput
{
    ...
    public class DocService : Bars.BarsWebService
    {
        ...
        public string PayDoc(string[] data, string[] tags, List<LinkedDocs> checkDocsList)
        {
            ...
            oper.OkpoA = data[12];
            // Правекс - хочуть підставляти клієнта замість каси
            //if (Session["DPT_NAM_A"] != null)
            //{
            //    oper.NlsA = Convert.ToString(Session["DPT_NAM_A"]);
            //    oper.OkpoA = Convert.ToString(Session["DPT_ID_A"]);
            //}
            ...
            oper.OkpoB = data[17];
            // Правекс - хочуть підставляти клієнта замість каси            
            //if (Session["DPT_NAM_B"] != null)
            //{
            //    oper.NlsB = Convert.ToString(Session["DPT_NAM_B"]);
            //    oper.OkpoB = Convert.ToString(Session["DPT_ID_B"]);
            //}
                ...
                if (TT_Flags[61] == '1')
                {
                    ...
                    try
                    {
                        if (ourDoc.oDocument())
                        {
                            ...
                            Bars.Logger.DBLogger.Financial("Ввод док Ref" + oper.Ref + ": " + oper.NlsA + "(" + oper.KvA + ") " + oper.SA.ToString("F0") + " -> " + oper.BankB + " " + oper.NlsB + "(" + oper.KvB + ") " + oper.SB.ToString("F0") + "[" + oper.NamB + "],[" + oper.Nazn + "] " + oper.OperId);
                            ...
                            for (int i = 0; i < l_doc_list.Count; i++)
                            {
                                ...
                                ourDoc = new cDoc(oper.con, oper.Ref, oper.check_tt, oper.Dk, oper.Vob,
                                        ...
                                        l_doc_list[i].Nls, l_doc_list[i].NlsName, oper.BankA, "", oper.KvA, Convert.ToDecimal(l_doc_list[i].S) * 100, oper.OkpoA,
                ...
                else
                {
                    if (ourDoc.oDoc())
                    {
                        ...
                        if (!string.IsNullOrEmpty(data[39]) || !string.IsNullOrEmpty(data[40]))
                        {
                            ...
                            Bars.Logger.DBLogger.Info(string.Format(buffer, oper.Ref.ToString(), HttpUtility.UrlDecode(data[39], System.Text.Encoding.UTF8), HttpUtility.UrlDecode(data[40], System.Text.Encoding.UTF8)));
                        }
                        Bars.Logger.DBLogger.Financial("Ввод док Ref" + oper.Ref + ": " + oper.NlsA + "(" + oper.KvA + ") " + oper.SA.ToString("F0") + " -> " + oper.BankB + " " + oper.NlsB + "(" + oper.KvB + ") " + oper.SB.ToString("F0") + "[" + oper.NamB + "],[" + oper.Nazn + "] " + oper.OperId);
        ...
        public string GetFileForPrint(string refernce, bool printTrnModel)
        {
            ...
            try
            {
                cmd.CommandText = conn.GetSetRoleCommand("WR_DOC_INPUT");
                cmd.ExecuteNonQuery();
                cDocPrint ourTick = new cDocPrint(con, long.Parse(refernce), Server.MapPath("/TEMPLATE.RPT/"), printTrnModel);
                return ourTick.GetTicketFileName();
            }
            finally
            {
                con.Close();
        ...
        public void SaveCurRates(string[] data)
        {
            try
            {
                ...
                Bars.Logger.DBLogger.Info("Установка курса валют в отделеннии №" + reader[1] + " пользователем №" + reader[0] + ", " + str_log, "WEB_CURR");
            }
            finally
            {
                DisposeOraConnection();
