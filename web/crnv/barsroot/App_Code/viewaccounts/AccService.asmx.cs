	...
	public class AccService : Bars.BarsWebService
	{
		...
		public object[] Populate(decimal acc, string rnk)
		{
			...
			try
			{
				...
				result[56] = SQL_SELECT_scalar("select nmk||'(РНК '||rnk||',НД '||nd||')' from customer where rnk=:rnk").ToString();
				ClearParameters();
				...
				if (acc != 0)
				{
					...
					object[] temp = SQL_SELECT_reader(@"
					 ...
					 TO_CHAR(daos,'dd/MM/yyyy'), TO_CHAR(dazs,'dd/MM/yyyy'), TO_CHAR(dapp,'dd/MM/yyyy'), mdate, blkd, blkk, 
					 ostc, dos, kos, ostq, dosq, kosq, lim, ostx, 
					 isp, grp, seci, seco, rnk , tobo
		...
		public string[] GetSPCodes(string nbs, string acc)
		{
			...
			try
			{
				...
				if (ms.Accounts.EnhanceCheck)
				{
					...
					if ("2600,2560,2570,2602,2603,2605,2650".Contains(nbs))
			...
			try
			{
				...
				
				{
					for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
					{
						...
						{
							row["HREF"] = "<a href=\"javascript:fnShowDic('" + row["PNSI"] + "','" + row["PNamePK"] + "','" + row["NsiSql"] + "')\">Справочник</a>";
						}
		...
		[WebMethod(EnableSession = true)]
		public object[] ListNbs(string[] data)
		{
			object[] obj = new object[2];
			DataSet ds = new DataSet();
			int count = 0;
			try
			{
				...
				int startpos = Convert.ToInt32(data[4]);
				int pageSize = Convert.ToInt32(data[5]);
				ds = SQL_SELECT_dataset("SELECT trim(nbs) NBS, name NAME, xar FROM PS WHERE length(trim(nbs))=" + length + " AND trim(nbs) BETWEEN '" + sA + "' AND '" + sB + "' and (d_close is null or d_close>=bankdate) ORDER BY " + order, startpos, pageSize);
				obj[0] = ds.GetXml();
				obj[1] = count;
				return obj;
			}
		...
		[WebMethod(EnableSession = true)]
		public string[] getNbs(string id)
		{
			...
			try
			{
				InitOraConnection(Context);
				SetRole(base_role);
				SetParameters("ID", DB_TYPE.Varchar2, id.Trim(), DIRECTION.Input);
				object[] temp = SQL_SELECT_reader("SELECT s.name,x.name,s.pap,p.name FROM PS s,pap p,xar x WHERE TRIM(s.nbs)=:ID AND s.xar=x.xar AND s.pap=p.pap and (d_close is null or d_close>=bankdate)");
			...
			try
			{
				...
				for (int i = 0; i < data.Length - 2; i += 2)
				{
					...
					if (reader.Count > 0)
					{
						...
						if (count == 0)
							SQL_NONQUERY("insert into " + tabname + " (acc" + ((string.IsNullOrEmpty(tag)) ? ("") : (",tag")) + ") values (:acc" + ((string.IsNullOrEmpty(tag)) ? ("") : (",:tag")) + ")");
			...
			try
			{
				...
				for (int i = 0; i < data.Length; i += 2)
				{
					...
					if (Convert.ToDecimal(SQL_SELECT_scalar("SELECT count(acc) FROM acc_tarif WHERE acc=:acc AND kod=:kod")) == 0)
					{
						...
						SQL_NONQUERY("INSERT INTO acc_tarif(acc,kod,tar,pr,smin,smax,bdate,edate) VALUES(:acc,:kod,:tar,:pr,:smin,:smax,:bdat,:edat)");
					}
					else
					{
						...
						else
						{
							...
							SetParameters("acc", DB_TYPE.Decimal, acc, DIRECTION.Input);
							SetParameters("kod", DB_TYPE.Decimal, data[i + 1][0], DIRECTION.Input);
							SQL_NONQUERY("UPDATE acc_tarif SET tar=nvl(:tar,tar), pr=nvl(:pr,pr), smin=:smin, smax=:smax, bdate=:bdat, edate=:edat WHERE acc=:acc AND kod=:kod");
						}
					}
				}
			}
