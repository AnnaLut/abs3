	...
	public class DptUService : BarsWebService
	{
		...
		public object[] GetDepositUDeals(string[] data)
		{
			...
			try
			{
				...
				if (data[9] != string.Empty && data[9].Trim() != "null")
					filter = "AND " + data[9];
				object[] tmp = BindTableWithNewFilter("d.dpu_id DPU_ID, d.nd ND, d.dpu_gen DPU_GEN, d.dpu_add DPU_AD," +
		...
		public object[] GetDepositDealState(string dpu_id, string dpu_gen)
		{
			...
			try
			{
				...
				if (Convert.ToString(result[14]) != "1")
				{
					...
					//принять на депозит 
					if (Convert.ToDecimal(result[8], cinfo) == 0 && datN >= bdate && datZ <= bdate)
						result[27] = 1;
					else
						result[27] = 0;
		...
		public string[] fillTables(string acc, string accN)
		{
			...
			try
			{
				...
				SQL_Reader_Exec("SELECT to_char(fdat,'DD.MM.YYYY'), to_char(kos/100,'99999'), dos/100,(ostf-dos+kos)/100 " +
								"FROM saldoa WHERE acc = :accN ORDER BY fdat desc");
                while (SQL_Reader_Read() && index <= 101)
				{
					ArrayList reader = SQL_Reader_GetValues();
					result[index++] = "2;<a href=# onclick=fnShowSal(" + acc + ",'" + Convert.ToString(reader[0]) + "')>" + Convert.ToString(reader[0]) + "</a>;" + Convert.ToString(reader[1]) + ";" + Convert.ToString(reader[2]) + ";" + Convert.ToString(reader[3]);
				}
			}
			finally
			{
		...
		public string payPenalty(string dpt_id, string date, string sPenalty, string sKv)
		{
			...
			try
			{
				...
				reader = SQL_reader(@"SELECT a.nls, substr(a.nms,1,38), c.okpo
									  ...
									  WHERE a.acc = ca.acc AND ca.rnk = c.rnk AND a.acc = :accn");
				if (reader.Count > 0)
				{
				...
				reader = SQL_reader(@"SELECT a.nls, substr(a.nms,1,38), c.okpo
									  FROM accounts a, cust_acc ca, customer c
									  WHERE a.acc = ca.acc AND ca.rnk = c.rnk AND a.acc = :acc7");
				if (reader.Count > 0)
				{
		...
		public object[] getVal(string id, string dateN)
		{
			...
			try
			{
				...
				SetRole(base_role);
				SetParameters("ID", DB_TYPE.Decimal, id, DIRECTION.Input);
				ArrayList reader = SQL_reader("select d.srok, d.kv, t.lcv, t.name, d.freq_v,f.name, d.br_id,b.name, NVL(d.comproc,0),NVL(d.fl_extend,0), d.id_stop, s.name, d.bsd, d.bsn, d.dpu_type, br.branch, br.name, d.shablon " +
											  "from tabval t, dpu_vidd d,freq f,brates b,dpt_stop s, branch br where br.branch=sys_context('bars_context', 'user_branch') and d.kv=t.kv(+) and d.freq_v=f.freq(+) and d.br_id=b.br_id(+) and d.id_stop=s.id(+) and d.vidd=:id");
				if (reader.Count != 0)
				{
					reader.CopyTo(result);
				}
				if (result[0].ToString() != string.Empty && dateN != string.Empty)
				{
					...
					string dateO = Convert.ToString(SQL_SELECT_scalar("select TO_CHAR(dpu.f_duration(:dateN,:mnth,:days),'dd.MM.yyyy') from dual"));
		...
		{
			if (String.IsNullOrEmpty(acc) || String.IsNullOrEmpty(ref1)) return 0;
			try
			{
				InitOraConnection();
				SetRole(base_role);
		...
		public int Register(string vdat, string sk, string acc1, string ref1, string acc2)
		{
			if (String.IsNullOrEmpty(acc1) || String.IsNullOrEmpty(acc2)
			  ...
			  || String.IsNullOrEmpty(ref1)) return 0;
			try
			{
				InitOraConnection();
				SetRole(base_role);
