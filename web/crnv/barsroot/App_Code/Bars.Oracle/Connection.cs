...
namespace Bars.Oracle
{
    ...
    public class Connection : IOraConnection
    {
        ...
        string GetUserConnectString(HttpContext ctx)
        {
            ...
            bool useSSL = (Bars.Configuration.ConfigurationSettings.AppSettings["CustomAuthentication.AuthSSL"] == "On") ? (true) : (false);
            if (useSSL)
                userLogin = Bars.Configuration.ConfigurationSettings.getUserNameFromCertificate(ctx.Request.ClientCertificate);
            
            // имя пользователя Oracle
            string databaseUser = Bars.Configuration.ConfigurationSettings.GetUserInfo(userLogin).shared_user;
        ...
        public OracleConnection GetUserConnection(HttpContext ctx)
        {
            ...
            DbProviderFactory factory = DbProviderFactories.GetFactory("barsroot.core");
