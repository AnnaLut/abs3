...
public partial class seizure_immobile_adm : Bars.BarsPage
{
    protected OracleConnection con;

    private void FillData()
    {
        Response.Redirect(String.Format("/barsroot/neruhomi/seizure_immobile_adm.aspx?key={0}", Request["KEY"]));
    }
    ...
    protected void bt_del_Click(object sender, ImageClickEventArgs e)
    {
        ...
        try
        {
            ...
            if (Request["KEY"] != null)
            {
                ...
                SQL_NONQUERY(@"begin
                                 delete from part_pay_immobile where key=:key_ and status=2;
                                 update asvo_immobile set fl=5 where key=:key2_;  
