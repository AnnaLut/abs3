		...
		private void TreeLoad()
		{
			...
			{   
				...
				while(rdr.Read())
				{
					...
					listRef.Controls.Add(new LiteralControl("&nbsp<span onselectstart='return false;' onmouseleave=\"this.style.textDecoration='none'\" onmousemove=\"this.style.textDecoration='underline'\" onclick=\"parent.frames['main'].location.href='"+url+"'\" style=\"cursor:hand;font-family:verdana;font-size:10pt;\">"+rdr.GetOracleString(3).Value+"</span>"));
					listRef.Controls.Add(new LiteralControl("</DIV>"));	
				}
				rdr.Close();
			}
