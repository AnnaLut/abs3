...
public partial class DepositFileShow : Bars.BarsPage
{
	...
	private void Page_Load(object sender, System.EventArgs e)
	{
        ...
        {
            redir_uri.Value = "DepositFile_ext.aspx?gb=true";
            redir_uri_create.Value = "DepositFile_ext.aspx?mode=create&gb=true";
            redir_uri_add.Value = "&gb=true";
        }
        else
        {
            redir_uri.Value = "DepositFile_ext.aspx";
            redir_uri_create.Value = "DepositFile_ext.aspx?mode=create";
            redir_uri_add.Value = String.Empty;
        }
	}
    /// <summary>
    /// 
        ...
        try
        {
            ...
            if (new_header_id != decimal.MinValue)
            {
                ...
                Response.Write("<script>alert('Файл був успішно скопійований. Id нового файла = " +
                    Convert.ToString(new_header_id) + " '); location.replace('depositfile_ext.aspx?mode=copy" +
                    inject + "');</script>");
            }
            else
