...
public partial class DepositSearch : Bars.BarsPage
{
	...
	private void btSelect_ServerClick(object sender, System.EventArgs e)
	{
		...
		dpt_id = Convert.ToDecimal(Convert.ToString(dptid.Value));
		...
		switch (action)
		{
            ...
            case "earlyreturn":  // Пошук депозиту для дострокового повернення (ЕБП)
                Response.Redirect("DepositSelectTrustee.aspx?dpt_id=" + dpt_id.ToString() +
            ...
            case "replenish":
                //Response.Redirect("DepositAddSum.aspx?dpt_id=" + dpt_id + "&agr_id=2&fast=Y&rnk_tr=" + cust_id);
                Response.Redirect("DepositAddSum.aspx?dpt_id=" + dpt_id + "&agr_id=2&fast=Y");
                break;
    ...
    private void FillGrid(bool btSearchPressed)
    {
        ...
        if (action == "delete")
            ...
            searchQuery += @"FROM v_dpt_portfolio_active P
            WHERE (DAT_END <= BANKDATE OR DAT_END IS NULL) ";
