...
//                    rnk.Value = dpt.Client.ID.ToString();
    ...
    protected void btShowAccountCard_Click(object sender, EventArgs e)
    {
        ...
        //Response.Redirect("/barsroot/viewaccounts/accountform.aspx?type=0&acc=" + dpt.dpt_acc + "&rnk=" + dpt.Client.ID + "&accessmode=1");

        Random r = new Random();

        ScriptManager.RegisterClientScriptBlock(this.Page, Page.GetType(), "showcard",
            "window.showModalDialog(encodeURI('" +
                        "../viewaccounts/accountform.aspx?type=0&acc=" + dpt.dpt_acc + "&rnk=" +
                        dpt.Client.ID + "&accessmode=1&dpt=rw&code=" + "&code=" + Convert.ToString(r.Next()) +
                        "'),null,'dialogWidth:700px; dialogHeight:800px; center:yes; status:no');__doPostBack('','');",
                        true);

        //Response.Write("<script type='text/javascript' language='javascript'>window.showModalDialog(encodeURI('" +
        //              "../viewaccounts/accountform.aspx?type=0&acc=" + dpt.dpt_acc + "&rnk=" + 
        //              dpt.Client.ID + "&accessmode=1&dpt=rw&code=" + "&code=" + Convert.ToString(r.Next()) +
        //              "'),null,'dialogWidth:700px; dialogHeight:800px; center:yes; status:no');__doPostBack('','');</script>");
    }
