...
function showTerritory()
{
    ...
    {
        if (result[0] != null) document.getElementById('textClientTerritory').value = result[0];
        if (result[1] != null) document.getElementById('textClientSettlement').value = result[1];
        if (result[3] != null) document.getElementById('textClientRegion').value = result[3];
        if (result[4] != null) document.getElementById('textClientDistrict').value = result[4];
...
function showClient()
{
	...
	,null,"dialogWidth:600px; dialogHeight:600px; center:yes; status:no");
}
// Картка клієнта
...
//
function returnAcc(nls,branch,okpo,fio)
{
	var Result	= new Array();
	Result.nls	= nls;
	Result.mfo	= branch;
...
//
function OpenCardAcc(nls,mfo,okpo,fio)
{
	...
	if (result != null)
	{
		...
		document.getElementById(okpo).value	= result.okpo;
...
function FillDpts()
{
	...
	if (l_arr.length == 0)
	{
		alert(LocalizedString('Mes34')/*'Не выбран ни один депозит!'*/);
		return false;
	}
	for (dpt in l_arr) 
	{
...
{    
    ...
    {
	    var url = "DepositPrint.aspx?rand=" + Math.random();
...
{
    var url = "DepositContractInfo.aspx?dpt_id=" + dpt_id + "&readonly=true";
    url +="&code=" + Math.random();
...
function Transfer()
{
	...
	var sumMax = Math.round(parseFloat(document.getElementById('textSUM').value) * denom);
	var sum = Math.round(GetValue('Sum') * denom);
	var com = Math.round(GetValue('Commission') * denom);
