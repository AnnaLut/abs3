...
public partial class DepositClosePayIt : Bars.BarsPage
{
    ...
    private void Page_Load(object sender, System.EventArgs e)
    {
        ...
        if (Request["tt"] == null)
            Response.Redirect("/barsroot/deposit/deloitte/DepositSelectTT.aspx?dest=close&dpt_id=" + Convert.ToString(Request["dpt_id"]));
        else
    ...
    private void FillControls(bool getShtraf, decimal sum2pay, decimal perc2pay, decimal p_int2pay_ing)
    {
        ...
        if (Request["rnk_tr"] != null)
        {
            ...
            DBLogger.Info("Розривати договір прийшла довірена особа з РНК = " + rnk.Value, "deposit");
        ...
        if (Request["inherit_id"] != null)
        {
            ...
            DBLogger.Info("Договор расторгать(делать частичное снятие) пришел наследник. РНК = " + rnk.Value, "deposit");
    ...
    private void btShtraf_ServerClick(object sender, System.EventArgs e)
    {
        ...
        try
        {
            ...
            DBLogger.Info("dpt_web.penalty_payment( " + Convert.ToString(dpt_id) + "," + Convert.ToString(ShtrafSum.ValueDecimal * (ShtrafSum.MinDecimalPlaces == Infragistics.WebUI.WebDataInput.MinDecimalPlaces.Three ? 1000 : 100)) + "," + Convert.ToString(sum2pay) + ", " + Convert.ToString(perc2pay) + ", " + Convert.ToString(p_int2pay_ing) + ") ",
           "deposit");
 ...
 private bool get_nls(Decimal i_Sum, Decimal i_kv)
    ...
    protected void btAuthorizationByBPK_ClientIdentified(object sender, Bars.UserControls.ClientIdentifiedEventArgs e)
    {
        if (Convert.ToInt64(rnk.Value) == e.RNK)
        {
            ...
            DBLogger.Info("Користувач виконав Авторизацію штрафування депозиту за допомогою БПК клієнта (rnk=" + Convert.ToString(e.RNK) +
