...
public partial class sberutls_importproc : Bars.BarsPage
{
    ...
    protected void initCountDoc()
    {
        ...
        SUM_IMPORT.Text = String.Format("{0}",docCntInf.SUM_IMPORT * 100);
        SUM_CHECK.Text = String.Format("{0}",docCntInf.SUM_CHECK * 100);
        SUM_PAYED.Text = String.Format("{0}", docCntInf.SUM_PAYED * 100);
        SUM_DELETED.Text = String.Format("{0}", docCntInf.SUM_DELETED * 100);
        SUM_ERRREQ.Text = String.Format("{0}", docCntInf.SUM_ERRREQ * 100);
    }
