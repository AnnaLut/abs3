...
namespace Bars.DocHand
{
    ...
    public class cDocHandler : IHttpHandler, IRequiresSessionState
    {
        ...
        public class Saldo
        {
            ...
            public string Nls;        // номер счета
            public string Rnk;        // рег .номер клиента
            public string Kv;         // код валюты
            public string Ost;        // сумма
            ...
            public string PersonIdActualDate;
            ...
            public Saldo(HttpContext context, string nls, string kv, string dk, string tt)
            {
                ...
                try
                {
                    ...
                    cmd.CommandText = " SELECT ostc, s.nms,TRIM(NVL(k.nmkk,k.nmk)),k.nmkv,k.okpo, k.country, t.dig, s.rnk "
                    ...
                    if (rdr.Read())
                    {
                        ...
                        if (!rdr.IsDBNull(7)) this.Rnk = Convert.ToString(rdr.GetValue(7));
                    }
                    else
                    {
                        resManager rm = new resManager();
                    ...
                    if (context.Request.QueryString["getperson"] != null)
                    {
                        ...
                        cmd.Parameters.Add("nls", OracleDbType.Varchar2, nls, ParameterDirection.Input);
                        cmd.Parameters.Add("kv", OracleDbType.Decimal, kv, ParameterDirection.Input);
                        cmd.CommandText = "select p.name doc_type_name, d.ser doc_serial, d.numdoc doc_num, d.organ || ' ' || to_char(d.pdate,'DD/MM/YYYY') doc_issue, fio(c.nmk,1) surname, fio(c.nmk,2) name, fio(c.nmk,3) patr, c.nmk, nvl(c.country,804) country, ca.rezid, to_char(d.actual_date,'DD/MM/YYYY') " +
                        ...
                        if (rdr.Read())
                        {
                            ...
                            PersonIdActualDate = Convert.ToString(rdr.GetValue(10));
                        }
                    }
        ...
        public class CrossRate
        {
            ...
            public string CountryCode;  // Код страны
            public string ErrMessage; // сообщение об ошибке
            private OracleConnection con;
            public CrossRate() { }
            public CrossRate(HttpContext context, string kv1, string kv2, string dat)
            {
                ...
                try
                {
                    ...
                    if (kv1 == "978") CountryCode = "000";
                    else {
                        cmd.Parameters.Add("kv", OracleDbType.Decimal, kv1, ParameterDirection.Input);
                        cmd.CommandText = "select decode(count(k040), 1, max(k040), max(r030)) from KL_K040 where r030=:kv";
