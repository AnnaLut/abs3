            ...
            foreach (var acc in ids)
            {
                if (desiredSum == "")
                {
                    ...
                    if (desiredSum == null)
                    {
                        ...
                        decimal desiredSumDecimal = entities.ExecuteStoreQuery<decimal>(@"select get_blim_bycardcode((select card_code from W4_ACC where acc_pk = :acc)) from w4_acc where acc_pk = :acc", acc).FirstOrDefault();
                        desiredSum = desiredSumDecimal.ToString();
