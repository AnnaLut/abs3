...
namespace BarsWeb.Areas.CDO.Common.Services
{
    ...
    public class UserCertificateService : IUserCertificateService
    {
        ...
        public string ConvertUserProfileToXml(RelatedCustomer userProfile)
        {
            ...
            result.Append(userProfile.DocType);
            result.Append("</documentType>");
            ...
            if (userProfile.DocDateTo != null && userProfile.DocType == "7")
            {
                result.Append("<documentDateTo>");
                if(userProfile.DocDateTo.HasValue)
                    result.Append(userProfile.DocDateTo.Value.ToString("yyyy-MM-dd"));
                result.Append("</documentDateTo>");
            }
