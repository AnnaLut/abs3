...
namespace BarsWeb.Areas.Way.Infrastructure.DI.Implementation
{
    public class WayRepository : IWayRepository
    {
        ...
        public IEnumerable<OicFile> Files(string dateFrom, string dateTo)
        {
            ...
            const string query = @"select * from v_ow_files where trunc(file_date) between trunc(:p_dateFrom) and trunc(:p_dateTo) order by id desc";
        ...
        public UpdateStatusDb ImportFile(string fileName, byte[] fileBody)
        {
