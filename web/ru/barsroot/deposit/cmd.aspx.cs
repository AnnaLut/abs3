...
public partial class cmd : Bars.BarsPage
{
    ...
    private void printFile()
    {
        ...
        try
        {
            ...
            if (Session["DPTPRINT_AGRID"] != null)
            {
                _dbLogger.Info("Пользователь распечатал доп.соглашение тип=" + Convert.ToString(Session["DPTPRINT_AGRID"])
            ...
            {
                _dbLogger.Info("Пользователь розпечатал депозитный договор.  Номер договора " + Convert.ToString(Session["DPTPRINT_DPTID"]),
                    "deposit");
