...
public partial class DepositReturn : Bars.BarsPage
{
			...
			if (Convert.ToString(Request["per"]) != "cent")
			{
                ...
                if (Deposit.InheritedDeal(dpt.ID.ToString()))
                {
                  ...
                  if (DateTime.Now.Date < datNotTax)
                  {
                    ...
                    "@" + "begin dpt_web.inherit_payment ( " + Request["dpt_id"] + ", " + Request.QueryString["inherit_id"] + ", :l_taxref_in ); " +
                    "if (:l_taxref_in is not null) then dpt_web.fill_dpt_payments(" + Request.QueryString["dpt_id"] + ",:l_taxref_in); end if; end;";
	...
	private void FillControlsFromClass(Deposit dpt)
	{
		...
		try {
            ...
            rvDepositToPay.MaximumValue = Convert.ToString(dpt.dpt_f_sum);
            rvDepositToPay.ErrorMessage = "Сума має бути > 0 та <= " + rvDepositToPay.MaximumValue;
			...
			dptDepositToPay.Value = Convert.ToDecimal(Convert.ToString(dptSum.Value));
			dptPercentToPay.Value = Convert.ToDecimal(Convert.ToString(dptPercentSum.Value));
            ...
            if (dpt.Sum_denom == 1000)
            {
                ...
                dptDepositToPay.Presiction = 3;
                dptPercentToPay.Presiction = 3;
            }
            ...
            if (Request["inherit_id"] != null)
            {
                ...
                dptDepositToPay.Value = InheritDeposit.ValueDecimal;
                dptPercentToPay.Value = InheritPercent.ValueDecimal;
