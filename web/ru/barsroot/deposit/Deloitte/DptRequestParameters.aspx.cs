...
public partial class deposit_DptRequestParameters : System.Web.UI.Page
{
    ...
    public deposit_DptRequestParameters()
    {
    ...
    private Decimal req_id
    {
        get
        {
            return Convert.ToDecimal(Request["req_id"]);
        }
    }
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        rbContractsClose.Enabled = false;
    }
    ...
    {        
        //передаємо ідентифікатор запиту в запит на перегляд документів з ЕА
        EADocsView.ReqID = Convert.ToString(Request_ID);
        ...
        try
        {
            ...
            if (rdr.Read())
            {
                ...
                if (!rdr.IsDBNull(10))
                {
                    switch (Convert.ToInt16(rdr.GetOracleDecimal(10).Value))
                    {
                        case 0:
                        ...
                        case 1:
                            {
                                tbReqStatus.BackColor = System.Drawing.Color.LightGreen;
                                break;
                            }
                        default:
                    {
        ...
        switch (req_type)
        {
            ...
            case 1:
                ...
                dsReqDetails.SelectCommand = @"select ra.CONTRACT_ID, ra.AMOUNT, d.RNK as CONTRACT_RNK,
                                                  ...
                                                  decode(SubStr(ra.FLAGS,5,1),'1','true','false') as FL_KV                                       
            ...
            default:
                ...
                lbReqStatus.Visible = true;
                tbReqStatus.Visible = true;
                ScriptManager.RegisterStartupScript(this, this.GetType(), "error_mesage", "alert('Невідомий тип запиту!');", true);
                break;
    ...
    protected void btRequestSaveChanges_Click(object sender, EventArgs e)
    {
        ...
        if (req_type == 1)
        {
            foreach (ListViewItem row in lvReqDetails.Items)
            {
                ...
                AccessRow.DPT_ID = Convert.ToDecimal(lvReqDetails.DataKeys[row.DataItemIndex]["CONTRACT_ID"]);
    ...
    {
        if (tbRequestComment.Text == null || String.IsNullOrEmpty(tbRequestComment.Text)) {
            RequestCommentValidator.Text = "Не вказна причина відмови!";
        }
        else
        {
            RequestCommentValidator.Text = "";
        try
        {
            DepositRequest.Process(req_id, -1, tbRequestComment.Text);
            ...
            case "V":
                {
                    ...
                    (lvReqDetails.FindControl("colAgreements") as HtmlTableCell).Style.Add("display", "none");
                    if (req_type != 2)
                    { (lvReqDetails.FindControl("colKv") as HtmlTableCell).Style.Add("display", "none"); }
                    break;
                }
