        ...
        <form id="AddRegularPaymentForm" runat="server" >
            ...
            <div style="padding-left: 15px;">
                <table border="0">
                    <tr>
                        <td>
      <span id="lbNumb" runat="server" title="Номер платіжної інструкції" class="k-label">Номер (автоматичний)
                            </span>
                        </td>
                        <td>
                            <input id="tbNumb" type="text" runat="server" class="k-textbox" style="background-color:whitesmoke" title="Номер платіжної інструкції" tabindex="3" disabled="disabled" readonly="true" />
                        </td>
                    </tr>
                    <tr>
                        <td>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <input id="cbFee" type="checkbox" runat="server" title="Зняти разову комісію?" style="display: none;"/>
                            <!--<label for="cbFee">Разова комісія</label>-->
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <span id="lbFee" runat="server" class="k-label" style="display:none">Сума комісії</span>
                        </td>
                        <td>
                            <input id="tbFee" runat="server" class="k-textbox" title="Сума разової комісії" style="display:none" />
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <span id="lbTTs" runat="server" class="k-label">Операція</span>
                        </td>
                        <td>
                            <input type="text" class="k-textbox" id="tbTTs" runat="server" maxlength="14" tabindex="9" />
                            <a class="k-button" onclick='showTTsReferen(); return false;' id="icoTTs">
                                <i class="pf-icon pf-16 pf-book"></i>
                            </a>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <textarea id="taNazn" rows="4" tabindex="10" style="width: 300px" class="k-textbox" runat="server" title="Призначення платежу" maxlength="160"></textarea>
                        </td>
                    </tr>
                    <tr>
                        <td>
                        ...
                        <td>
                            <input id="StartDate" type="text" tabindex="11" runat="server" class="" />
                        </td>
                    </tr>
                    <tr>
                        <td>
                        ...
                        <td>
                            <input id="EndDate" type="text" tabindex="12" runat="server" class="" />
                        </td>
                    </tr>
                    <tr>
                        <td>
                        ...
                        <td>
                            <select id="Period" runat="server" tabindex="13" style="width:150px"></select>
                        </td>
                    </tr>
                    <tr>
                        <td>
                        ...
                        <td>
                            <select id="Prior" runat="server" tabindex="14" style="width:150px"></select>
                        </td>
                    </tr>
                    <tr>
                        <td>
                        ...
                        <td>
                            <input id="Weekends1" value="+1" runat="server" tabindex="12" type="radio" checked />
                            <label for="Weekends">+1</label>
                            <input id="Weekends2" value="-1" runat="server" tabindex="13" type="radio" />
                            <label for="Weekends">-1</label>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td style="padding-top: 10px;">
                            <a id="btReg" onclick="savePayment(); return false;" tabindex="15" data-role="button" class="k-primary k-button k-button-icontext" role="button" aria-disabled="false" tabindex="0">
                                <span class="k-sprite k-icon k-i-tick"></span>
                                Зберегти
                            </a>
                            <input id="btPrint" onclick="printDocum(); return false;" tabindex="16" type="button" value="Друк" runat="server" class="k-button" />
                        </td>
                    </tr>
                </table>
            </div>
      ...
      <script>
          ...
          var minaddsum = 0;
            $(function () {
                ...
                $("#cbFee").on('change', function () {
                    var lbfee = 'hide', objlbfee = $("#lbFee");
                    var tbfee = 'hide', objtbfee = $("#tbFee");
                ...
                $('#taNazn[maxlength]').keyup(function () {
            ...
            function formAddPaymentValid() {
                ...
                if ($('#tbSum').val() < minaddsum) {
                    result.status = 'error';
                    result.message += '<div class="validate-message">Сума поповнення депозитного рахунку менша за мінімальну суму поповнення виду вкладу!</div>';                    
                }                
                if (minaddsum == 0 && ourMfo == $('#tbMfoB').val() && $('#tbNlsB').val().match(/263*/)) {
                    result.status = 'error';
                    result.message += '<div class="validate-message">Оберіть вкладний рахунок з довідника!</div>';
                }
                if ($('#tbTTs').val() == "") {
                    result.status = 'error';
                    result.message += '<div class="validate-message">Не заповнено код оперції.</div>';
                }
                if ($('#StartDate').val() == "") {
                    result.status = 'error';
                ...
                if (endDate < startDate || endDate == startDate) {
                    ...
                    if (startDate < bankDate) {
                        result.status = 'error';
                        result.message += '<div class="validate-message">"Дата з" не може бути меншою за наступну банківську дату</div>';
                    }
                }
          ...
          // return parametr from url by name
          ...
          // old version
            function getUrlVars(key) {
                var tmp = window.location.href.slice(window.location.href.indexOf('?')).split(/[&?]{1}[\w\d]+=/);
                return tmp[key];
            }
            
            function showAccountsReferen() {
                debugger;
                //var kv = getUrlVars(3); // for chrome use 4
                var kv = getParameterByName('kv');
                bars.ui.handBook('V_ACCCUST_190', function (data) {
                    $("#tbNlsB").val(
                        $(data).map
                        (function () {
                        $("#tbNmkB").val(this.NMK);
                        $("#tbOkpoB").val(this.OKPO);
                            $("#tbSum").val(this.MINSUM);
                            minaddsum = this.MINSUM;
                        return this.NLS;
                    }).get());
                    //$apply();
                },
                {
                    multiSelect: false,
                    clause: " rnk in (select rnk from customer where custtype = 3) and kv = " + kv,
                    columns: "RNK,NLS,NMS,MINSUM",
                    width: "80%"
                });
            }

            function showTTsReferen() {
                //var kv = getUrlVars(3);
                var kv = getParameterByName('kv');
                var fli;
                var ourMfo;
                ...
                {
                    multiSelect: false,
                    clause: "TT in (select TT from w4_sto_tts)" + fli,
                    columns: "TT,NAME",
                    width: "80%"
                });
            }
