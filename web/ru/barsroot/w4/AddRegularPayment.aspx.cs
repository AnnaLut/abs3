...
public partial class w4_AddRegularPayment : Page
{
    ...
    {
        OracleCommand command = connect.CreateCommand();
        command.CommandText = @"select a.kv kv from accounts a 
    ...
    public Decimal MinSumm(string p_nls)
    {
        IOraConnection conn = (IOraConnection)this.Application["OracleConnectClass"];
        OracleCommand cmdMinSumm = new OracleCommand();
        cmdMinSumm.CommandText = @"SELECT NVL (dv.LIMIT, 0) FROM dpt_deposit d, dpt_vidd dv, accounts a 
                                    WHERE d.acc = a.acc AND a.nls = :p_nls and d.vidd = dv.vidd ";
        cmdMinSumm.Parameters.Add("p_nls", OracleDbType.Decimal, p_nls, ParameterDirection.Input);
