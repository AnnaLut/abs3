    ...
    public class DptUService : BarsWebService
    {
        ...
        public object[] GetDepositDealState(string dpu_id, string dpu_gen)
        {
            ...
            try
            {
                ...
                if (Convert.ToString(result[14]) != "1")
                {
                    ...
                    // Розміщення (план. залиш. = 0 AND дата закін. > bdate AND дата початку <= bdate)
                    if (Convert.ToDecimal(result[8], cinfo) == 0 && datO > bdate && datN <= bdate)
                    {
                        // для розміщеного 
                        result[27] = 1;

                        // Поповнення ( допустиме поповн. AND є фактичний залишок 
                        // AND дата закін. > bdate AND дата початку <= bdate)
                        if (Convert.ToDecimal(result[18], cinfo) == 1 &&
                            Convert.ToDecimal(result[32], cinfo) == 1 &&
                            datO > bdate && datN <= bdate)
                        {
                            result[28] = 1;
                        }
                        else
                        ...
                        // Виплата. нарах. %% (наявний план. залишок на рах. нарах.%%)
                        if (Convert.ToDecimal(result[11], cinfo) > 0)
                            result[29] = 1;
                        else
                            result[29] = 0;

                        // штрафование (была ли операция штрафования)
                        ClearParameters();
                        ...
                        if (Convert.ToDecimal(result[32]) > 0 && datN < bdate && datO > bdate && datO != DateTime.MinValue && Convert.ToDecimal(result[8], cinfo) > 0 && stopDone == 0)
                            result[30] = 1;
                        else
                            result[30] = 0;

                        // Погашення ( є плановий залишок AND код штрафу = 0 
                        // OR договір завершився OR договір дозапитання OR відбулося штрафування
                    ...
                    {
                        // для нерозміщеного всі кнопки крім розміщення неактивні
                        result[27] = 0;
                        result[28] = 1;
                        result[29] = 1;
                        result[30] = 1;
        ...
        public string CreateAgreement(Int64 dpu_id, string[] data)
        {
                ...
                case 1:
                    {
                        ...
                        amount = Convert.ToInt32(data[3]) * 100;
                        break;
                    }
                case 4:
                    {
                        ...
                        details = "<details>";
                        details = details + String.Format("<dep><mfo>{0}</mfo><nls>{1}</nls><nmk>{2}</nmk><okpo></okpo></dep>", data[9], data[10], data[11]);
                        // if ( ( frm_Open.dfK013 != '1' ) OR ( NOT frm_Open.cbCOMPROC ) )
                        // details = details + String.Format("<int><mfo>{0}</mfo><nls>{1}</nls><nmk>{2}</nmk><okpo></okpo>{3}</int>", data[12], data[13], data[14], data[15]);
                ...
                case 7:
                    {
                        ...
                        // Set dBEGIN_DATE = frm_Open.dfDATO
                        break;
                    }
                default:
                    {
            ...
            try
            {
                ...
                SetParameters("p_due_date", DB_TYPE.Date, data[2], DIRECTION.Input);
                SetParameters("p_amount", DB_TYPE.Decimal, amount, DIRECTION.Input);
                SetParameters("p_rate", DB_TYPE.Decimal, data[4], DIRECTION.Input);
                SetParameters("p_freq", DB_TYPE.Decimal, data[5], DIRECTION.Input);
                SetParameters("p_begin_date", DB_TYPE.Date, data[6], DIRECTION.Input);
                SetParameters("p_end_date", DB_TYPE.Date, data[7], DIRECTION.Input);
                SetParameters("p_stop_id", DB_TYPE.Decimal, data[8], DIRECTION.Input);
