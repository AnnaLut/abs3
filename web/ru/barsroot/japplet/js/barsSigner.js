...
var barsSign = window.barsSign || (function () {
    ...
    var _moduleTypes = { AX: 0, JAVA: 1, SL: 2, AW: 3, Node: 4 }; // 0 - activeX, 1 - javaApplen, 2 - Silverlight, 3 - Awesomium
    ...
    return {
        ...
        init: function (options) {
            ...
            switch (_options.ModuleType) {
                ...
                case _moduleTypes.Node: _currModule = barsSign.node_module(); break;
...
barsSign.options = {
    ...
    KeyId: "04TST801"                   // идентификатор ключа - берем из настроек
};
...
barsSign.node_module = function () {
    var startUrl = 'http://127.0.0.1:8888/';
    var module = {}; // внутренний объект модуля 
    var aSigner = null;
    var isInitialized = false;
    var _options;
    ...
    module.initialize = function (options) {
        ...
        try {
            var query = {};
            query.SignType = options.SignType;
            var resInit = makeQuery(JSON.stringify(query), 'init');
            if (resInit) {
                if (resInit.status === 0) {
                    alert('Помилка зв\'язку з ЕЦП сервером!');
                    return false;
                }
                else if (resInit.status === 200) {
                    var obj = resInit.responseJSON;
                    if (obj.State === "OK")
                        isInitialized = true;
    ...
    function _signBuffer(strBuf) {
        //var strSingBuf = aSigner.SignBuffer(strBuf);
        var obj = {};
        obj.Encoding = _options.BufferEncoding;//'UTF8';
        obj.BankDate = _options.BankDate;
        obj.IdOper = _options.KeyId;
        obj.Buffer = strBuf;
        var resSign = makeQuery(JSON.stringify(obj), 'sign');
        if (resSign && resSign.status) {
            if (resSign.status === 0) {
                alert('Помилка зв\'язку з ЕЦП сервером!');
                return false;
            }
            else if (resSign.status === 200) {
                var obj = resSign.responseJSON;//JSON.parse(resInit.responseText);
                if (obj.State === "OK") {
                    alert(obj.Sign);
                    return obj.Sign;
                }
        ...
        //barsSign.debug("Call node_module::signBuffer(strBuf)", { "strBuf": strBuf, "strSingBuf": strSingBuf });
        return strSingBuf;
    }
    ...
    function _checkSign(strBuf, strSignBuf, keyId) {
        ...
        obj.Buffer = strBuf;
        obj.Sign = strSignBuf;
        var resSign = makeQuery(JSON.stringify(obj), 'validate');
        if (resSign && resSign.status) {
            if (resSign.status === 0) {
                alert('Помилка зв\'язку з ЕЦП сервером!');
                return false;
