...
function onSetPerVal(result)
{
 ...
 document.all.tbOKPO.value = result.value[30].text;
 if(!copy_on){
    ...
    document.all.tbOKPO.fireEvent("onchange");
     //document.all.tbOKPO.fireEvent("onkeyup");
...
//1 controll mfoField
//2 controll NlsField
function validateMfoAcc(item) {
    
    var v_mfo = document.getElementById('tbMFO').value;
    var v_nls = document.getElementById('tbNlsC').value;
    if (v_nls != cDocVkrz(v_mfo, v_nls))
    {
        //LocalizedString('Message31');
        //alert("Message31");
        //item.focus();
    }
}
//check if fields are ready
function checkIsMfoNlsField() {
    var active = document.activeElement.name;
    var v_mfo = document.getElementById('tbMFO').name;
    var v_nls = document.getElementById('tbNlsC').name;
    var is_recursive = true;
    if (active === undefined) {
        is_recursive = true;
    } else {
        is_recursive = (active !== v_mfo && active !== v_nls);
    }
    return document.getElementById('tbMFO').value != "" && document.getElementById('tbNlsC').value != "" && is_recursive;
}
function checkAcc(item) {
    if (checkIsMfoNlsField()) {
        validateMfoAcc(item);
    }
}
function numbValidate(item) {
    item.value = item.value.replace(/\D+/g, "").substr(0, 14);
    SaveP(item);
}
