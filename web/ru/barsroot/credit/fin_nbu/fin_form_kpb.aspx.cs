...
public partial class credit_fin_form_kpb : Bars.BarsPage
{
    ...
    /// <summary>
    /// Вичитуемо субєктивні показники / грід субєктивних показників
    /// </summary>
...
//            String PAR_BANKYPE;
            ...
            if (PAR_BANKYPE == "SBER" || PAR_BANKYPE == "SBER_EBP") 
            {
                L_VNCRR.Visible = true;
                DL_VNCRR.Visible = true;

                //if (Convert.ToString(Request["custype"]) == "3")
                //{
                //    dd_Tip_o.Visible = true;
                //    l_ddl_tip_o.Visible = true;
                //}
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Request["okpo"] != null) {tbOKPO.Text = Convert.ToString(Request["okpo"]);}
        if (Request["frm"] != null)  {tbFrm.Value = Convert.ToDecimal(Request["frm"]);}
        if (Request["dat"] != null)  { if (!IsPostBack) tbFDat.Text = Convert.ToString(Request["dat"]); }
        if (Request["rnk"] != null)  { tbRNK.Text = Convert.ToString(Request["rnk"]);}
        ...
        if (!IsPostBack)
        {
            try
            {
                ...
                DL_VNCRR.DataSource = SQL_SELECT_dataset("select code from CCK_RATING where ord < 41 ").Tables[0];
    ...
    // -----   Загрузка основних питань --------
    /// <summary>
    /// Загрузка даних для коофіциента показнику боргу(Загрузка основних питань)
    /// </summary>
    protected void Load_Pkpb()
    {
        ...
        try
        {
            ...
            cmd.CommandText = ("select " +
                                "  fin_NBU.ZN_P_ND('KP0', 5, fdat, nd, rnk) as KP0," +
            ...
            if (rdr.Read())
            {
                ...
                dd_Tip_o_ = rdr["KP0"] == DBNull.Value ? (Decimal)0 : (Decimal)rdr["KP0"];
                dd_Tip_o.SelectedValue = Convert.ToString(dd_Tip_o_); 
        ...
        try
        {
            ...
            else
            {
                ...
                cmd.CommandText = (@"select  nvl((fin23),  fin_zp.f_get_fin_nd (rnk , nd,  '110' , dat)) as fin23, 
                                             ...
                                             trim(fin_zp.zn_vncrr(rnk, nd)) VNCRR, sos, tip
                                       from (select tip, rnk, nd, sos,  fin23, kat23, k23, obs23, :dat as dat from v_fin_cc_deal) where rnk = :rnk and nd = :nd");
            }
            OracleDataReader rdr = cmd.ExecuteReader();
            ...
            if (rdr.Read())
            {
                ...
                p_tip_ =  rdr["TIP"] == DBNull.Value ? (String)null : (String)rdr["TIP"];
                ...
                if (Convert.ToString(obs23_) != "0")
                {
                    ...
                    if (sos_ > 0)  
                    {
                        //Dl_obs.ReadOnly = true; 
                        Dl_obs.Enabled = false;
                    }
                    else          
                    { 
                        //Dl_obs.ReadOnly = true;
                ...
                {
                    if (Convert.ToString(vncrr_) == "Д") { DL_VNCRR.SelectedValue = "Г"; }
                    else { DL_VNCRR.SelectedValue = vncrr_; }
                }


                if (!String.IsNullOrEmpty(Convert.ToString(p_tip_)))
                {
                    P_tip.Value = p_tip_;
                }
    ...
    /// <summary>
    /// Зберігаємо субєктивні показники
    /// </summary>
    protected void calculation_ZPR()
    {
        try
        {
            // Зберігаємо дані якщо не спрощена оцінка
            if (dd_Tip_o.SelectedValue == "2")
            {  return;    }
    ...
    protected void BtZPR_Click(object sender, EventArgs e)
    {
        ...
        FillData();
        if (Request["custype"] == null || Request["custype"] == "2")
            if (PAR_BANKYPE == "UPB" || PAR_BANKYPE == "SBER1") Bt_Print.Visible = true;
        }
    ...
    /// <summary>
    ///  Зберігаемо відповіді на обовязкові показники НБУ та ВНКР
    /// </summary>
    protected void Bt_klass_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            InitOraConnection();
            {
                ...
                SetParameters("kod0", DB_TYPE.Varchar2, "KP0", DIRECTION.Input);
                SetParameters("S0", DB_TYPE.Decimal, Convert.ToDecimal(dd_Tip_o.SelectedValue), DIRECTION.Input);
                ...
                SQL_NONQUERY(" declare " +
                                ...
                                "begin  " +
                                "null;" +
                                "fin_nbu.record_fp_nd(:kod0, :S0, idf_, dat_, ND_,RNK_ );  " +
                                ...
                                "fin_nbu.record_fp_nd(:kod6, nvl(:S6,-99), idf_, dat_, ND_, RNK_ );  " +
    ...
    /// <summary>
    /// Розрахунок показника покриття боргу
    /// </summary>
    ...
    /// <summary>
    /// Приводить відповідність ВНКР до класу (для звичайного розрахунку)
    /// </summary>
    protected void f_vnkr_not_spr()
    {
        String l_vnkr;
        l_vnkr = Convert.ToString(Tb_FIN23.Value).Replace("1", "А").Replace("2", "Б").Replace("3", "В").Replace("4", "Г");
        try
        { if (Convert.ToString(Tb_FIN23.Value).Length == 1) { DL_VNCRR.SelectedValue = l_vnkr; } }
        catch (Exception) { };
    ...
    protected void fn23_value(object sender, EventArgs e)
    {
          try
        {
            ...
            if (tbFDat.Text.Length == 0) return;
            if (dd_Tip_o.SelectedValue == "2") {f_vnkr_not_spr();}
    ...
    /// <summary>
    /// Процедура визначення субєктивних показників (автоматично)
    /// </summary>
    /// <param name="sender"></param>
    /// <param name="e"></param>
    ...
    /// <summary>
    /// Відображення екранної форми в залежності від типу оцінки
    /// </summary>
    protected void Type_of_evaluation()
    {
        
        if ((Request["custype"] == "3" || Request["custype"] == "3") && (P_tip.Value == "OW4" || P_tip.Value == "BPK"))
        {
                dd_Tip_o.Visible = true;
                l_ddl_tip_o.Visible = true;
            if (dd_Tip_o.SelectedValue == "0")
            {
                DL_VNCRR.Enabled = true;
                ID2.Visible = false;
                gvMain.Visible = false;
                test.Visible = false;
            }
            else if (dd_Tip_o.SelectedValue == "1")
            {
                DL_VNCRR.Enabled = true;
                ID2.Visible = true;
                gvMain.Visible = true;
            ...
            else if (dd_Tip_o.SelectedValue == "2")
            {
                f_vnkr_not_spr();
                DL_VNCRR.Enabled = false;
                ID2.Visible = true;
                gvMain.Visible = false;
                test.Visible = true;
            }
        ...
        { 
                dd_Tip_o.Visible = false;
                dd_Tip_o.SelectedValue = "1";
                l_ddl_tip_o.Visible = false;
        }
    }
    

    protected void dd_select_change(object sender, EventArgs e)
    {
