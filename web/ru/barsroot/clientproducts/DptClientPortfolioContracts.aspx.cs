...
public partial class deposit_DptClientPortfolioContracts : Bars.BarsPage
{
    ...
    public deposit_DptClientPortfolioContracts()
    {
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            // Населяємо табл. закритих договорів клієнта
            FillGrid("CLOSED", cust_id);
            ...
            if (ClientAccessRights.Get_AccessLevel(cust_id) == LevelState.Limited)
            {
                ...
                gvDepositClosed.Columns[7].Visible = false;
                gvDepositClosed.Columns[8].Visible = false;
        ...
        switch (p_type)
        {
            case "MAIN":
                ...
                searchQuery = @"SELECT P.DPT_ID, P.DPT_NUM DPT_NUM, P.VIDD_NAME type_name, P.DAT_BEGIN datz, P.DAT_END dat_end,
                                       ...
                                       decode(P.DPT_ACCID,P.INT_ACCID,'0.00',to_char((P.INT_SALDO/DPT_CUR_DENOM),'FM999G999G999G990D009')) ost_int, WB
                                  FROM table(dpt_views.get_portfolio(:p_client_id,1)) P
            ...
            case "TRUSTEE":
                ...
                searchQuery = @"SELECT p.dpt_id DPT_ID,  p.dpt_num DPT_NUM, p.cust_name NMK, p.dpt_accnum NLS,
                                       p.dpt_curcode LCV, p.dat_begin DATZ, p.dat_end DAT_END, p.DPT_LOCK, p.ARCHDOC_ID, WB
            ...
            case "BENEFICIARY":
                ...
                searchQuery = @"SELECT p.dpt_id DPT_ID, p.dpt_num DPT_NUM, p.dat_begin DATZ, p.dat_end DAT_END,
                                      ...
                                      (p.dpt_saldo/p.dpt_cur_denom) OST_DEP, (p.int_saldo/dpt_cur_denom) OST_INT,
                                       p.DPT_LOCK, p.ARCHDOC_ID, b.RNK_TR, b.ID TRUSTEE_ID, p.dpt_curid KV, WB
                                  FROM v_dpt_portfolio_ALL_active p,
                                       dpt_trustee b
                                 WHERE p.DAT_END is Not Null
            ...
            case "CLOSED":
                ...
                dsDepositClosed.ConnectionString = Bars.Classes.OraConnector.Handler.IOraConnection.GetUserConnectionString();
                // dsDepositMain.PreliminaryStatement = Bars.Classes.OraConnector.Handler.IOraConnection.GetSetRoleCommand("DPT_ROLE");
                ...
                searchQuery = @"SELECT P.DPT_ID, P.DPT_NUM DPT_NUM, P.VIDD_NAME type_name, P.DAT_BEGIN datz, P.DAT_END dat_end,
                                       ...
                                       decode(P.DPT_ACCID,P.INT_ACCID,'0.00',to_char((P.INT_SALDO/DPT_CUR_DENOM),'FM999G999G999G990D009')) ost_int, WB
                                  FROM table(dpt_views.get_portfolio(:p_client_id,-1)) P
                                 WHERE CUST_ID = :p_client_id";

                dsDepositClosed.WhereParameters.Clear();
                dsDepositClosed.SelectCommand = searchQuery;
                dsDepositClosed.WhereParameters.Add("p_client_id", TypeCode.Int32, Convert.ToString(p_cust_id));

                // gvDepositMain.DataSourceID = "dsSearchDeposit";
                // gvDepositMain.DataBind();

                if (gvDepositClosed.Rows.Count == 0)
                    btSelectClosedContract.Enabled = false;
    ...
    protected void SelectContract_Click(object sender, EventArgs e)
    {
        ...
        {
            // Закритий депозит
            case "btSelectClosedContract":
                gv = (gvDepositClosed as BarsGridViewEx);
                url = url + "&closed=true";
                break;

                // Довірений депозит
            case "btSelectTrusteeContract":
       ...
       /* if (btn.ID == "btSelectTrusteeContract")
        ...
        if (gv.SelectedRows.Count > 0)
        {
            if (gv.DataKeys.Count > 0)
            {
                ...
                {
                    _dbLogger.Info("Користувач вибрав для перегляду договір №" + Convert.ToString(Request["dpt_id"]) +
    ...
    {
        _dbLogger.Info("Користувач створення нового договору для клієнта №" + Request.QueryString["cust_id"] +
             " на сторінці [Портфель договорів].", "deposit");
        Session["OnTrustee"] = "NO";
        Int32 cust_id = Convert.ToInt32(Request["cust_id"]);
    ...
    {
        _dbLogger.Info("Довірена особа розпочала створення нового договору для клієнта №" + Request.QueryString["cust_id"] +
             " на сторінці [Портфель договорів].", "deposit");
    ...
    {
        String url = "&scheme=DELOITTE" + "&inherit_irr_id=" + Request.QueryString["cust_id"];
 ...
 if (dpt_id == null)
                ...
                {
                    _dbLogger.Info("1) виплати спадкоємцю (РНК=" + Request.QueryString["cust_id"] +
                                  ") договір №" + Convert.ToString(dpt_id.Value) + url, "deposit");
                    if (Tools.DPT_HASNO35AGREEMENT(dpt_id.ToString()) + Tools.DPT_IRREVOCABLE(dpt_id.ToString()) >= 1)
                    {
                        _dbLogger.Info("2) виплати спадкоємцю (РНК=" + Request.QueryString["cust_id"] +
                                ") договір №" + Convert.ToString(dpt_id.Value) + url, "deposit");

                        Response.Redirect("/barsroot/deposit/deloitte/DepositAgreement.aspx?dpt_id=" + Convert.ToString(dpt_id) + url);
                    }
                    else
                    { // Дата завершення вкладу

                        DateTime? DateEnd = Convert.ToDateTime(gvHeritage.DataKeys[gvHeritage.SelectedRows[0]].Values[3]);
                        String pKV = Convert.ToString(gvHeritage.DataKeys[gvHeritage.SelectedRows[0]].Values[5]);
                        _dbLogger.Info("pKV = " + pKV);
                        String pTT = "DP1";

                        if (DateEnd > DateTime.Now.Date)
                        {
                        if (pKV == "UAH") { pTT = "DP1"; } else { pTT = "DPG";}
                            _dbLogger.Info("pTT = " + pTT);
                            //DBLogger.Info("/barsroot/deposit/deloitte/DepositClosePayIt.aspx?dpt_id=" +
                            //    Convert.ToString(dpt_id.Value) + "&inherit_id=" + Request.QueryString["cust_id"] +
                            //    "&dest=return&scheme=DELOITTE&tt=" + pTT);
        ...
        if (gvDepositBeneficiary.SelectedRows.Count > 0)
        {
            if (gvDepositBeneficiary.DataKeys.Count > 0)
            {
                ...
                else
                {
                    ...
                    String kv = Convert.ToString(gvDepositBeneficiary.DataKeys[gvDepositBeneficiary.SelectedRows[0]].Values[4]);

                    String cust_id = Request.QueryString["cust_id"];

                    if (!Tools.card_account_exists(Convert.ToDecimal(cust_id), Convert.ToDecimal(kv)))
                    {
                        Response.Write("<script>alert('В клієнта відсутній картковий рахунок в валюті договору!');</script>");
                        return;
                        //throw new DepositException("В клієнта відсутній картковий рахунок. Реєстрація неможлива!");
                       // return;
                    }

                    _dbLogger.Info("Користувач обрав \"Вступ в право власності\" на депозит #" + dpt_id.ToString() +
    ...
    protected void btnSetChild_Click(object sender, EventArgs e)
    {
        if (gvDepositChildren.SelectedRows.Count > 0)
        {
            if (gvDepositChildren.DataKeys.Count > 0)
            {
                ...
                {
                    _dbLogger.Info("Користувач обрав \"Передати депозит малолітній особи\" на депозит #" + dpt_id.ToString(), "deposit");
        ...
        if (gvDepositChildren.SelectedRows.Count > 0)
        {
            if (gvDepositChildren.DataKeys.Count > 0)
            {
                ...
                else
                {
                    ...
                    if (rnk_m != 0)
                    {
                        _dbLogger.Info("По депозитному договору № " + dpt_id.ToString() + " вже є розпорядник з rnk= " + rnk_m.ToString() + ". Реєстрація неможлива!", "deposit");
                    ...
                    _dbLogger.Info("Користувач обрав \"Додати розпорядника для малолітньої особи\" на депозит #" + dpt_id.ToString() + "cust_id =" + cust_id, "deposit");
    ...
    protected void gv_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e != null && e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            String WB = Convert.ToString(gv.DataKeys[e.Row.DataItemIndex]["WB"]);
            Int16 DPTLock = Convert.ToInt16(gv.DataKeys[e.Row.DataItemIndex]["DPT_LOCK"]);
            decimal? dpt_id = Convert.ToDecimal(gv.DataKeys[e.Row.DataItemIndex]["DPT_ID"]);
            decimal DPTIRR = Tools.DPT_IRREVOCABLE(dpt_id.ToString());
            //свойства первой колонки строк
            if (EADocID == 0)
            {
                e.Row.Cells[1].BackColor = System.Drawing.Color.Yellow;
                    e.Row.Cells[1].ForeColor = System.Drawing.Color.Black;
                }
            //свойства остальных строк
            if (WB == "Y")
            ...
            {                                    
                ...
                e.Row.Cells[3].ForeColor = System.Drawing.Color.Bisque;
                e.Row.Cells[3].Font.Bold = false;
                e.Row.Cells[4].BackColor = System.Drawing.Color.Brown;
                e.Row.Cells[4].ForeColor = System.Drawing.Color.Bisque;                
    ...
    {
        _dbLogger.Info("Користувач натиснув кнопку \"Картка Клієнта\" на сторінці \"Портфель договорів\".", "deposit");
    ...
    protected void bntPrint_Click(object sender, EventArgs e)
    {
        if (gvCards.SelectedRows.Count > 0)
        {
            ...
            if (gvCards.DataKeys.Count > 0)
            {
                ...
                _dbLogger.Info("acc_acc = " + acc_acc.ToString(), "deposit");
                _dbLogger.Info("DOC_ID = " + doc_id, "deposit");
    ...
    protected void btnShowAccount_Click(object sender, EventArgs e)
    {
        if (gvCards.SelectedRows.Count > 0)
        {
            ...
            if (gvCards.DataKeys.Count > 0)
            {
                ...
                {
                    _dbLogger.Info("Користувач натиснув кнопку \"Картка рахунку\" на сторінці \"Портфель договорів\".", "deposit");
