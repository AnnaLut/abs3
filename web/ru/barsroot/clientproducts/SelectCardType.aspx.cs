...
public partial class clientproducts_SelectCardType : System.Web.UI.Page
{
    ...
    public clientproducts_SelectCardType()
    {
    ...
    protected void FillProductList(Boolean EmptyLine)
    {
        ...
        dsProduct.SelectCommand = @"select unique product_code, product_name from V_W4_PRODUCT
                                        where grp_code = '"+ Convert.ToString(listProductGrp.SelectedItem.Value)+"'"
                                        + " and proect_id = "+ Convert.ToString(listProject.SelectedItem.Value)
                                        + " and kv = '" + Convert.ToString(listCurrency.SelectedItem.Value)+"'";
    ...
    protected void btnRegisterCard_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            _dbLogger.Info("Користувач заключив новий договір БПК №" + Convert.ToString(Card_ID.Value), "deposit");
