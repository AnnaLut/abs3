...
bars.dupes = bars.dupes || {
            ...
            read: {
                ...
                data: function() {
                    ...
                    return {
                        ...
                        CustomerType: bars.extension.getParamFromUrl('type')
                    }
                }
            }
        },
        requestStart: function () {
            $('#overlayPage').addClass('k-loading-image k-overlay');
        },
        requestEnd: function (e) {
            $('#overlayPage').removeClass('k-loading-image k-overlay');
            if (e.response.ErrorMessage)
                bars.ui.error({ text: e.response.ErrorMessage });
        }
    }),
    ...
    currentDuplicates: new kendo.data.ObservableObject({
        ...
        loadCurrentChildAttributes: function() {
            ...
            $.get(bars.config.urlContent("/Cdm/Deduplicate/GetRnkAttributes?rnk=" + dRnk + "&type=" + getPersonType())).done(function (data) {
        ...
        setCurrentChildAsMain: function() {
            ...
            bars.ui.confirm({ text: "Ви впевнені?" },
                function () {
                    ...
                    $.get(bars.config.urlContent("/Cdm/Deduplicate/SetNewMainCard?mRnk=" + self.rnk + "&dRnk=" + self.currentChildCard.D_RNK + "&type=" + getPersonType())).done(function (data) {
                        if (data.status === "ok") {
        ...
        ignoreCurrentChild: function () {
            ...
            bars.ui.confirm({ text: "Ви впевнені?" },
                function () {
                    bars.ui.loader("body", true);
                    $.get(bars.config.urlContent("/Cdm/Deduplicate/IgnoreChild?mRnk=" + self.rnk + "&dRnk=" + self.currentChildCard.D_RNK + '&type=' + getPersonType())).done(function (data) {
                        if (data.status === "ok") {
        ...
        mergeCurrentChild: function () {
            ...
            bars.ui.confirm({ text: "Ви впевнені?" },
                function () {
                    bars.ui.loader("body", true);
                    $.get(bars.config.urlContent("/Cdm/Deduplicate/MergeDupes?mRnk=" + self.rnk + "&dRnk=" + self.currentChildCard.D_RNK + '&type=' + getPersonType())).done(function (data) {
                        if (data.status === "ok") {
                            bars.dupes.selectCustomer(self.rnk);
                        } else {
                            bars.ui.alert({ text: data.message });
            ...
            bars.ui.confirm({ text: "Ви впевнені?" },
                function() {
                    ...
                    $.ajax({
                        ...
                        data: { rnk: self.rnk, attributes: attrNames, values: values, type: getPersonType() },
                        dataType: "json",
                        traditional: true,
    ...
    selectCustomer: function (rnk) {
        var self = bars.dupes;
        $.get(bars.config.urlContent("/Cdm/Deduplicate/GetRnkDuplicates?rnk=" + rnk  + "&type=" + getPersonType())).done(function (data) {
            if (data.MainCard) {

                self.currentDuplicates.set("rnk", rnk);
                self.currentDuplicates.set("mainCard", data.MainCard);
                self.currentDuplicates.set("mainQualityCard", data.MainCard.CARD_QUALITY == null ? ' ' : data.MainCard.CARD_QUALITY + '%');
                ...
                if (data.ChildCards.length > 0) {
                    ...
                    self.currentDuplicates.set("currentChildQualityCard", data.ChildCards[0].CARD_QUALITY == null ? ' ' : data.ChildCards[0].CARD_QUALITY + '%');
                }
