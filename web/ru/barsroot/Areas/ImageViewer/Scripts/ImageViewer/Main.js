...
function getImagesTypes() {
	if (g_ImagesTypes == null) {
		var dataSource = CreateKendoDataSource({
			transport: {read:{url: bars.config.urlContent("/api/imageviewertypes")}},
...
function GetPhoto(rnk, image_type, successCallback) {
	Waiting(true);
	AJAX({ srcSettings: {
		url: bars.config.urlContent("/api/imageviewergetphoto"),
...
function initMainGrid() {
    ...
    fillKendoGrid("#gridMain", {
		...
		pageSize: 10,
		transport: { read: {
				url: bars.config.urlContent("/api/imageviewersearchmain"),
