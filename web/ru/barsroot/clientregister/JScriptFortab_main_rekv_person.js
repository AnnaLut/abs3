...
function ShowInputFIO() {
    if (dispFullFIO) {
        ...
        if (tmpSel == '1') {
            $('#trPersonFIO td').show();
            $('#trPersonFIONR td').hide();
        } else {
            $('#trPersonFIO td').hide();
            $('#trPersonFIONR td').show();
        }
    } else {
        $('#trPersonFIONR td').hide();
        $('#trPersonFIO td').hide();
...
function EditNMKtext() {
    ...
    if ($get('ddl_CODCAGENT').selectedIndex < 0 || !$get('ddl_CODCAGENT').item($get('ddl_CODCAGENT').selectedIndex) || $get('ddl_CODCAGENT').item($get('ddl_CODCAGENT').selectedIndex).value.substr(0, 1) == '1') {
