...
function initShowHidePhoneButton() {
    var res = ExecSync('CheckUserAcessMode', {}).d;
    var accesIsAllowed = false;

    if (res == "R" || res == "RW")
        accesIsAllowed = true;
    else if(res != "NA")
    {
        // show message that some error has occured:
        barsUiError({ text: res });
    }
...
function ChangeMobilePhone(elem) {
    //alert("deb_changed");
    //debugger;
    //alert(parent.obj_Parameters);

    //for (var key in parent.obj_Parameters) {
    //    if (Object.prototype.hasOwnProperty.call(parent.obj_Parameters, key)) {
    //        var val = parent.obj_Parameters[key];
    //        alert(key + " " + parent.obj_Parameters[key]);
    //    }
    //}
    ...
    if (elem) {
        ...
        if (parent.obj_Parameters['CUSTTYPE'] === 'person' && !parent.isCustomerSpd()) {
            var validationResult = ExecSync('ValidateMobilePhone', { rnk: (parent.obj_Parameters['ID'] === '' ? 0 : parent.obj_Parameters['ID']), phone: mobNum + "&" + parent.obj_Parameters["OKPO"] }).d;
...
$(document).ready(function () {
    ...
    //недаэмо редагувати мобільний телефон користувачам рівня 3
    initPhone();
    if ($("#b_depth").val() == 0) {
